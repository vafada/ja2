...
*/
  ...
  export function InitPopupMenu(
    ...
    while (
      (usMenuHeight >= usY &&
        (ubDirection == DIR_UPLEFT || ubDirection == DIR_UPRIGHT)) ||
      ...
      gPopup.ubMaxEntriesPerColumn = Math.trunc(
    ...
    for (ubColumn = 0; ubColumn < gPopup.ubColumns; ubColumn++) {
      for (ubEntry = 0; ubEntry < gPopup.ubMaxEntriesPerColumn; ubEntry++) {
        if (ubCounter >= gPopup.ubNumEntries) break; // done (don't want to process undefined entries...)
        usCurrStrWidth =
          16 + StringPixLength(GetPopupMenuString(ubCounter), gPopup.usFont);
  ...
  function RenderPopupMenu(): void {
    ...
    for (ubColumn = 0; ubColumn < gPopup.ubColumns; ubColumn++) {
      for (ubEntry = 0; ubEntry < gPopup.ubMaxEntriesPerColumn; ubEntry++) {
        ...
        usStringWidth =
          14 + StringPixLength(GetPopupMenuString(ubCounter), gPopup.usFont);
        // Horizontally center the string inside the popup menu
        usX =
          usStart +
          Math.trunc((gPopup.ubColumnWidth[ubColumn] - usStringWidth) / 2);
  ...
  function GetPopupIndexFromMousePosition(): UINT8 {
    ...
    let ubCount: UINT8;
    if (
      gusMouseXPos >= gPopup.usLeft &&
      gusMouseXPos <= gPopup.usRight &&
      gusMouseYPos > gPopup.usTop && // one pixel gap on top ignored
      gusMouseYPos < gPopup.usBottom - 2
    ) {
      // two pixel gap on bottom ignored
      ...
      ubNumEntriesDown =
        Math.trunc((gusMouseYPos - gPopup.usTop - 1) / gusEntryHeight) + 1;
  ...
  function PopupMenuHandle(): void {
    ...
    if (gPopup.ubActiveType == POPUP_ACTIVETYPE_NOT_YET_DETERMINED) {
      ...
      if (
        ...
        gusMouseXPos <= gPopup.usRight &&
        gusMouseYPos > gPopup.usTop && // one pixel gap on top ignored
        gusMouseYPos < gPopup.usBottom - 1
      ) {
        // two pixel gap on bottom ignored
    ...
    if (!gPopup.fUseKeyboardInfoUntilMouseMoves) {
      ...
      gusMouseXPos != gPopup.usLastMouseX ||
      gusMouseYPos != gPopup.usLastMouseY
    ) {
