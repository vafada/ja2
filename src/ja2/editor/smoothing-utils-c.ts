namespace ja2 {
  ...
  export function SearchForWallType(iMapIndex: UINT32): UINT16 {
    ...
    if (gfBasement) {
      ...
      if (usWallType == 0xffff) usWallType = Enum313.FIRSTWALL;
      return usWallType;
    }
    while (sRadius < 32) {
      ...
      for (y = sRadius; y >= -sRadius; y--)
        for (x = -sRadius; x <= sRadius; x++) {
          if (
            Math.abs(x) == Math.abs(sRadius) ||
            Math.abs(y) == Math.abs(sRadius)
          ) {
  ...
  function RoofAtGridNo(iMapIndex: UINT32): boolean {
    ...
    while (pRoof) {
      if (pRoof.usIndex != NO_TILE) {
        uiTileType = GetTileType(pRoof.usIndex);
        if (
          uiTileType >= Enum313.FIRSTROOF &&
          uiTileType <= Enum313.SECONDSLANTROOF
        )
          return true;
        pRoof = pRoof.pNext;
      }
  ...
  export function ValidDecalPlacement(iMapIndex: UINT32): boolean {
    if (
      GetVerticalWall(iMapIndex) ||
      GetHorizontalWall(iMapIndex) ||
      GetVerticalFence(iMapIndex) ||
  ...
  export function ChangeVerticalWall(
    iMapIndex: UINT32,
    usNewPiece: UINT16,
  ): void {
    let pWall: LEVELNODE | null;
