namespace ja2 {
  ...
  let TerrainTileButtonRegion: MOUSE_REGION[] /* [NUM_TERRAIN_TILE_REGIONS] */ =
    createArrayFrom(NUM_TERRAIN_TILE_REGIONS, createMouseRegion);
  ...
  function InitEditorRegions(): void {
    ...
    MSYS_DefineRegion(
      ...
      MSYS_PRIORITY_NORMAL,
      0,
      MouseMovedInItemsRegion,
      MouseClickedInItemsRegion,
    );
    MSYS_DisableRegion(ItemsRegion);
  ...
  function LoadEditorImages(): void {
    ...
    giEditMercDirectionIcons[0] = LoadGenericButtonIcon(
      "EDITOR//arrowsoff.sti",
    );
    giEditMercDirectionIcons[1] = LoadGenericButtonIcon("EDITOR//arrowson.sti");

    giEditMercImage[0] = LoadButtonImage(
    ...
    giEditMercImage[1] = LoadButtonImage(
  ...
  function ShowEditorToolbar(iNewTaskMode: INT32): void {
    switch (iNewTaskMode) {
      case Enum36.TASK_TERRAIN:
        ShowEditorButtons(
          Enum32.FIRST_TERRAIN_BUTTON,
          Enum32.LAST_TERRAIN_BUTTON,
        );
        break;
      case Enum36.TASK_BUILDINGS:
        ShowEditorButtons(
          Enum32.FIRST_BUILDINGS_BUTTON,
          Enum32.LAST_BUILDINGS_BUTTON,
        );
        break;
      ...
      case Enum36.TASK_MERCS:
        ShowEditorButtons(
          FIRST_MERCS_TEAMMODE_BUTTON,
          LAST_MERCS_TEAMMODE_BUTTON,
        );
        break;
      case Enum36.TASK_MAPINFO:
        ShowEditorButtons(
          Enum32.FIRST_MAPINFO_BUTTON,
          Enum32.LAST_MAPINFO_BUTTON,
        );
        break;
      case Enum36.TASK_OPTIONS:
        ShowEditorButtons(
          Enum32.FIRST_OPTIONS_BUTTON,
          Enum32.LAST_OPTIONS_BUTTON,
        );
        break;
      default:
        return;
  ...
  export function EnableEditorTaskbar(): void {
    ...
    if (iCurrentTaskbar == Enum36.TASK_ITEMS) DetermineItemsScrolling();
  }
  ...
  export function ClearTaskbarRegion(
    sLeft: INT16,
    sTop: INT16,
    sRight: INT16,
    sBottom: INT16,
  ...
  // look.
  export function DrawEditorInfoBox(
  ...
  function ClickEditorButtons(
    iFirstEditorButtonID: INT32,
    iLastEditorButtonID: INT32,
  ): void {
    let i: INT32;
    let b: GUI_BUTTON;
  ...
  export function UnclickEditorButtons(
    iFirstEditorButtonID: INT32,
    iLastEditorButtonID: INT32,
  ): void {
    let i: INT32;
  ...
  export function HideEditorButtons(
    iFirstEditorButtonID: INT32,
    iLastEditorButtonID: INT32,
  ): void {
    let i: INT32;
  ...
  export function ShowEditorButtons(
    iFirstEditorButtonID: INT32,
    iLastEditorButtonID: INT32,
  ): void {
    let i: INT32;
  ...
  export function DisableEditorButtons(
    iFirstEditorButtonID: INT32,
    iLastEditorButtonID: INT32,
  ): void {
    let i: INT32;
  ...
  function BuildTriggerName(pItem: OBJECTTYPE): string {
    ...
    if (pItem.usItem == Enum225.SWITCH) {
      if (pItem.bFrequency == PANIC_FREQUENCY) szItemName = "Panic Trigger1";
      ...
      else szItemName = swprintf("Trigger%d", pItem.bFrequency - 50);
    } else {
      // action item
      ...
      else szItemName = swprintf("Action%d", pItem.bFrequency - 50);
    }
  ...
  function RenderDoorLockInfo(): void {
    ...
    for (i = 0; i < gubNumDoors; i++) {
      ...
      if (DoorTable[i].ubTrapID) {
        ...
        xp =
          sScreenX + 20 - Math.trunc(StringPixLength(str, FONT10ARIAL()) / 2);
        yp = sScreenY;
        mprintf(xp, yp, str);
  ...
  function RenderSelectedItemBlownUp(): void {
    ...
    BltVideoObjectOutlineFromIndex(
      FRAME_BUFFER,
      uiVideoObjectIndex,
      Item[gpItem.usItem].ubGraphicNum,
      xp,
      yp,
    ...
    SetFontShadow(FONT_NEARBLACK);
    if (
      gpItem.usItem == Enum225.ACTION_ITEM ||
      gpItem.usItem == Enum225.SWITCH
    ) {
    ...
    xp =
      sScreenX -
      Math.trunc((StringPixLength(szItemName, FONT10ARIAL()) - 40) / 2);
    yp -= 10;
    ...
    if (gpItem.usItem == Enum225.ACTION_ITEM) {
      ...
      xp =
        sScreenX -
        Math.trunc((StringPixLength(pStr, FONT10ARIALBOLD()) - 40) / 2);
      yp += 10;
  ...
  function RenderEditorInfo(): void {
    ...
    mprintfEditor(
      50 - Math.trunc(StringPixLength(FPSText, FONT12POINT1()) / 2),
      463,
      FPSText,
    );
    ...
    switch (iCurrentTaskbar) {
      ...
      case Enum36.TASK_TERRAIN:
        if (gusSelectionType == Enum33.LINESELECTION)
          wszSelType[Enum33.LINESELECTION] = swprintf(
            "Width: %d",
            gusSelectionWidth,
          );
        DrawEditorInfoBox(
          wszSelType[gusSelectionType],
          FONT12POINT1(),
          220,
          430,
          60,
  ...
  export function ProcessEditorRendering(): void {
    ...
    if (gfRenderDrawingMode) {
      if (
        iCurrentTaskbar == Enum36.TASK_BUILDINGS ||
        iCurrentTaskbar == Enum36.TASK_TERRAIN ||
    ...
    if (gfSummaryWindowActive) RenderSummaryWindow();
    else if (!gfGotoGridNoUI && !InOverheadMap()) RenderMercStrings();
    ...
    if (!gfSummaryWindowActive && !gfGotoGridNoUI && !InOverheadMap()) {
      ...
      if (
        (iDrawMode == Enum38.DRAW_MODE_PLACE_ITEM &&
          eInfo.uiItemType == Enum35.TBAR_MODE_ITEM_KEYS) ||
