namespace ja2 {
  ...
  function EditModeInit(): boolean {
    let x: UINT32;
    let i: INT32;
    let LColors: SGPPaletteEntry[] /* [2] */ = createArrayFrom(
      2,
    ...
    for (
      x = Enum32.LAST_EDITORTAB_BUTTON + 1;
      x < Enum32.NUMBER_EDITOR_BUTTONS;
      x++
    )
    ...
    else gusLightLevel = EDITOR_LIGHT_MAX - LightGetAmbient();
  ...
  function EditModeShutdown(): boolean {
    ...
    if (gfNeedToInitGame) {
      ...
      if (!gfBasement && !gfCaves) LightSetBaseLevel(gusGameLightLevel);
  ...
  function SetBackgroundTexture(): void {
    ...
    for (cnt = 0; cnt < WORLD_MAX; cnt++) {
      ...
      else AddLandToHead(cnt, usIndex); // no floor so no worries.
    }
  }
  ...
  function DrawTempMouseCursorObject(): boolean {
    ...
    let usUseIndex__Pointer = createPointer(
      () => usUseIndex,
      (v) => (usUseIndex = v),
    );
    let usUseObjIndex: UINT16 = 0;
    let usUseObjIndex__Pointer = createPointer(
      () => usUseObjIndex,
      (v) => (usUseObjIndex = v),
    );
    ...
    if (
      ...
      if ((iCurBankMapIndex = MAPROWCOLTOPOS(sMouseY_M, sMouseX_M)) < 0x8000) {
        ...
        if (iDrawMode == Enum38.DRAW_MODE_SMART_WALLS) {
          if (
            !CalcWallInfoUsingSmartMethod(
              iCurBankMapIndex,
              usUseObjIndex__Pointer,
              usUseIndex__Pointer,
            )
          ) {
            return false;
          ...
          if (
            !CalcWindowInfoUsingSmartMethod(
              iCurBankMapIndex,
              usUseObjIndex__Pointer,
              usUseIndex__Pointer,
            )
          ...
          if (
            !CalcBrokenWallInfoUsingSmartMethod(
              iCurBankMapIndex,
              usUseObjIndex__Pointer,
              usUseIndex__Pointer,
            )
  ...
  export function ShowCurrentDrawingMode(): void {
    ...
    let usObjIndex__Pointer = createPointer(
      () => usObjIndex,
      (v) => (usObjIndex = v),
    );
    ...
    if (usUseIndex != 0xffff && usObjIndex != 0xffff) {
      pETRLEObject =
        gTileDatabase[gTileTypeStartIndex[usObjIndex]].hTileSurface
          .pETRLEObject[usUseIndex];
    ...
    switch (iEditorToolbarState) {
      ...
      case Enum35.TBAR_MODE_FAKE_LIGHTING:
        ...
        if (gfFakeLights) {
          gusSavedLightLevel = gusLightLevel;
          gusLightLevel = EDITOR_LIGHT_FAKE;
        } else gusLightLevel = gusSavedLightLevel;
        LightSetBaseLevel(EDITOR_LIGHT_MAX - gusLightLevel);
        LightSpriteRenderAll();
        break;
  ...
  function HandleKeyboardShortcuts(): void {
    while (DequeueEvent(EditorInputEvent)) {
      if (
        !HandleSummaryInput(EditorInputEvent) &&
        !HandleTextInput(EditorInputEvent) &&
          ...
          switch (EditorInputEvent.usParam) {
            ...
            case INSERT:
              if (iDrawMode == Enum38.DRAW_MODE_GROUND) {
                ...
                iDrawMode ==
                Enum38.DRAW_MODE_GROUND + Enum38.DRAW_MODE_FILL_AREA
              ) {
                iDrawMode -= Enum38.DRAW_MODE_FILL_AREA;
            ...
            case ENTER:
              if (gfEditingDoor) {
                ...
                iCurrentTaskbar == Enum36.TASK_MERCS &&
                gsSelectedMercID != -1
              )
            ...
            case DEL:
              if (iCurrentTaskbar == Enum36.TASK_ITEMS) DeleteSelectedItem();
              else if (gsSelectedMercID != -1) {
                if (
                  gubCurrMercMode == Enum42.MERC_INVENTORYMODE &&
                  gbCurrSelect != -1
                ) {
                  DeleteSelectedMercsItem();
                  gbCurrSelect = -1;
                  gfRenderTaskbar = true;
                } else DeleteSelectedMerc();
              } else iCurrentAction = Enum37.ACTION_QUICK_ERASE;
              break;
            ...
            case ESC:
              ...
              else if (gfEditingDoor) KillDoorEditing();
              else iCurrentAction = Enum37.ACTION_EXIT_EDITOR;
              break;

            // Select next/prev terrain tile to draw with.
            case SHIFT_LEFTARROW:
              CurrentPaste -=
                gfShowTerrainTileButtons && CurrentPaste > 0 ? 1 : 0;
              break;
            case SHIFT_RIGHTARROW:
              CurrentPaste +=
                gfShowTerrainTileButtons && CurrentPaste < 8 ? 1 : 0;
              break;
            ...
            case F4:
              ...
              giMusicID++;
              if (giMusicID >= Enum327.NUM_MUSIC) giMusicID = 0;
              break;
            ...
            case F7:
              if (gfBasement) {
                ...
                if (usRoofType == 0xffff) usRoofType = Enum313.FIRSTROOF;
            ...
            case F11:
              CreateMessageBox(
                "Are you sure you wish to reverse the schedules?",
              );
              gfScheduleReversalPending = true;
              break;
            case F12:
              CreateMessageBox(
                "Are you sure you wish to clear all of the schedules?",
              );
            ...
            case "9".charCodeAt(0):
              if (iCurrentTaskbar == Enum36.TASK_MERCS) {
                ...
                else iCurrentAction = Enum37.ACTION_SET_WAYPOINT;
            ...
            case "b".charCodeAt(0):
              ...
              else SetEditorBuildingTaskbarMode(Enum32.BUILDING_NEW_ROOM);
              TerrainTileDrawMode = TERRAIN_TILES_BRETS_STRANGEMODE;
              break;
            case "c".charCodeAt(0):
              if (
                EditorInputEvent.usKeyState & CTRL_DOWN &&
                iCurrentTaskbar == Enum36.TASK_MERCS
              ) {
            ...
            case "h".charCodeAt(0):
              ...
              else UnclickEditorButton(Enum32.BUILDING_TOGGLE_ROOF_VIEW);
              UpdateRoofsView();
              break;

            case "i".charCodeAt(0):
              if (!InOverheadMap()) GoIntoOverheadMap();
              else KillOverheadMap();
              break;
            ...
            case "T".charCodeAt(0):
              ...
              HandleKeyboardShortcuts__fShowTrees =
                !HandleKeyboardShortcuts__fShowTrees;
            ...
            case "w".charCodeAt(0): // walls (full building)
              ...
              else UnclickEditorButton(Enum32.BUILDING_TOGGLE_WALL_VIEW);
              UpdateWallsView();
              break;
            case "v".charCodeAt(0):
              if (
    ...
    switch (iCurrentAction) {
      ...
      case Enum37.ACTION_NEW_MAP:
        ...
        if (gfPendingBasement)
          CreateMessageBox(
            "Delete current map and start a new basement level?",
          );
      ...
      case Enum37.ACTION_SET_NEW_BACKGROUND:
        if (!fBeenWarned) CreateMessageBox(" Wipe out ground textures? ");
        else {
      ...
      case Enum37.ACTION_SUB_INDEX_DWN:
        ...
        switch (iDrawMode) {
          case Enum38.DRAW_MODE_BANKS:
            if (iNumBanksSelected > 0)
              iCurBank =
                (iCurBank + (iNumBanksSelected - 1)) % iNumBanksSelected;
            break;
          case Enum38.DRAW_MODE_ROADS:
            if (iNumRoadsSelected > 0)
              iCurBank =
                (iCurBank + (iNumRoadsSelected - 1)) % iNumRoadsSelected;
            break;
          case Enum38.DRAW_MODE_WALLS:
            if (iNumWallsSelected > 0)
              iCurBank =
                (iCurBank + (iNumWallsSelected - 1)) % iNumWallsSelected;
            break;
          case Enum38.DRAW_MODE_DOORS:
            if (iNumDoorsSelected > 0)
              iCurBank =
                (iCurBank + (iNumDoorsSelected - 1)) % iNumDoorsSelected;
            break;
          case Enum38.DRAW_MODE_WINDOWS:
            if (iNumWindowsSelected > 0)
              iCurBank =
                (iCurBank + (iNumWindowsSelected - 1)) % iNumWindowsSelected;
            break;
          case Enum38.DRAW_MODE_ROOFS:
            if (iNumRoofsSelected > 0)
              iCurBank =
                (iCurBank + (iNumRoofsSelected - 1)) % iNumRoofsSelected;
            break;
          case Enum38.DRAW_MODE_BROKEN_WALLS:
            if (iNumBrokenWallsSelected > 0)
              iCurBank =
                (iCurBank + (iNumBrokenWallsSelected - 1)) %
                iNumBrokenWallsSelected;
            break;
          case Enum38.DRAW_MODE_DECOR:
            if (iNumDecorSelected > 0)
              iCurBank =
                (iCurBank + (iNumDecorSelected - 1)) % iNumDecorSelected;
            break;
          case Enum38.DRAW_MODE_DECALS:
            if (iNumDecalsSelected > 0)
              iCurBank =
                (iCurBank + (iNumDecalsSelected - 1)) % iNumDecalsSelected;
            break;
          case Enum38.DRAW_MODE_FLOORS:
            if (iNumFloorsSelected > 0)
              iCurBank =
                (iCurBank + (iNumFloorsSelected - 1)) % iNumFloorsSelected;
            break;
          case Enum38.DRAW_MODE_TOILET:
            if (iNumToiletsSelected > 0)
              iCurBank =
                (iCurBank + (iNumToiletsSelected - 1)) % iNumToiletsSelected;
            break;
          case Enum38.DRAW_MODE_NEWROOF:
            if (iNumNewRoofsSelected > 0)
              iCurBank =
                (iCurBank + (iNumNewRoofsSelected - 1)) % iNumNewRoofsSelected;
            break;
          case Enum38.DRAW_MODE_OSTRUCTS:
            fDontUseRandom = true;
            if (iNumOStructsSelected > 0)
              iCurBank =
                (iCurBank + (iNumOStructsSelected - 1)) % iNumOStructsSelected;
            break;
          ...
          case Enum38.DRAW_MODE_OSTRUCTS1:
            ...
            if (iNumOStructs1Selected > 0)
              iCurBank =
                (iCurBank + (iNumOStructs1Selected - 1)) %
          ...
          case Enum38.DRAW_MODE_OSTRUCTS2:
            ...
            if (iNumOStructs2Selected > 0)
              iCurBank =
                (iCurBank + (iNumOStructs2Selected - 1)) %
          ...
          case Enum38.DRAW_MODE_DEBRIS:
            ...
            if (iNumDebrisSelected > 0)
              iCurBank =
                (iCurBank + (iNumDebrisSelected - 1)) % iNumDebrisSelected;
            break;
        }
        break;
  ...
  export function CreateNewMap(): void {
    ...
    } else SetEditorSmoothingMode(Enum231.SMOOTHING_NORMAL);
    fNewMap = false;
    RemoveAllFromUndoList();
  ...
  function WaitForSelectionWindowResponse(): UINT32 {
    ...
    if (DoWindowSelection()) {
      ...
      if (
        ...
        if (CheckForSlantRoofs()) iDrawMode = Enum38.DRAW_MODE_SLANTED_ROOF;
      }
      InvalidateScreen();
      ExecuteBaseDirtyRectQueue();
      EndFrameBufferRender();
  ...
  export function PlaceLight(
    ...
    if ((iLightHandle = LightSpriteCreate(Filename, sType)) == -1) {
      ...
      if ((iLightHandle = LightCreateOmni(ubIntensity, sRadius)) == -1) {
        ...
        DebugMsg(
          ...
          DBG_LEVEL_1,
          FormatString(
            "PlaceLight: Can't create light template for radius %d",
            sRadius,
          ),
        );
        return false;
      ...
      if (!LightSave(iLightHandle, Filename)) {
        ...
        DebugMsg(
          ...
          DBG_LEVEL_1,
          FormatString(
            "PlaceLight: Can't save light template for radius %d",
            sRadius,
          ),
        );
        return false;
      ...
      if ((iLightHandle = LightSpriteCreate(Filename, sType)) == -1) {
        ...
        DebugMsg(
          ...
          DBG_LEVEL_1,
          FormatString(
            "PlaceLight: Can't create light sprite of radius %d",
            sRadius,
          ),
        );
        return false;
    ...
    if (!LightSpritePower(iLightHandle, true)) {
      ...
      DebugMsg(
        TOPIC_GAME,
        DBG_LEVEL_1,
        FormatString("PlaceLight: Can't turn on light %d", iLightHandle),
      );
      return false;
    }
    ...
    if (!LightSpritePosition(iLightHandle, iMapX, iMapY)) {
      ...
      DebugMsg(
        ...
        DBG_LEVEL_1,
        FormatString(
          "PlaceLight: Can't set light position for light %d",
          iLightHandle,
        ),
      );
      return false;
    ...
    if (
      (usTmpIndex = TypeExistsInObjectLayer(iMapIndex, Enum313.GOODRING)) === -1
    ) {
      AddObjectToHead(iMapIndex, Enum312.GOODRING1);
      (<LEVELNODE>gpWorldLevelData[iMapIndex].pObjectHead).ubShadeLevel =
        DEFAULT_SHADE_LEVEL;
    }
  ...
  export function RemoveLight(iMapX: INT16, iMapY: INT16): boolean {
    ...
    for (iCount = 0; iCount < MAX_LIGHT_SPRITES; iCount++) {
      if (LightSprites[iCount].uiFlags & LIGHT_SPR_ACTIVE) {
        if (
          LightSprites[iCount].iX == iMapX &&
          LightSprites[iCount].iY == iMapY
        ) {
  ...
  export function ShowLightPositionHandles(): void {
    ...
    for (iCount = 0; iCount < MAX_LIGHT_SPRITES; iCount++) {
      if (LightSprites[iCount].uiFlags & LIGHT_SPR_ACTIVE) {
        ...
        if (!fSoldierLight) {
          iMapIndex =
            LightSprites[iCount].iY * WORLD_COLS + LightSprites[iCount].iX;
          if (
            (usTmpIndex = TypeExistsInObjectLayer(
              iMapIndex,
              Enum313.GOODRING,
            )) === -1
          ) {
  ...
  function CheckForFences(): boolean {
    ...
    for (usCheck = 0; usCheck < iNumOStructs2Selected; usCheck++) {
      ...
      if (T.pDBStructureRef == null) fFence = false;
  ...
  function EnsureStatusOfEditorButtons(): void {
    ...
    else EnableEditorButton(Enum32.BUILDING_TOGGLE_ERASEMODE);
    ...
    if (iDrawMode == Enum38.DRAW_MODE_GROUND + Enum38.DRAW_MODE_FILL_AREA)
      ClickEditorButton(Enum32.TERRAIN_FILL_AREA);
    else UnclickEditorButton(Enum32.TERRAIN_FILL_AREA);

    // Make sure that the fake-lighting button is down if appropriate
    if (gfFakeLights) ClickEditorButton(Enum32.MAPINFO_TOGGLE_FAKE_LIGHTS);
    else UnclickEditorButton(Enum32.MAPINFO_TOGGLE_FAKE_LIGHTS);
  }
  ...
  function HandleMouseClicksInGameScreen(): void {
    ...
    let sY: INT16;
    let fPrevState: boolean;
    if (!GetMouseXY(sGridX__Pointer, sGridY__Pointer)) return;
    if (
      iCurrentTaskbar == Enum36.TASK_OPTIONS ||
      iCurrentTaskbar == Enum36.TASK_NONE
    ) {
    ...
    if (_LeftButtonDown()) {
        ...
        for (sY = gSelectRegion.iTop; sY <= gSelectRegion.iBottom; sY++) {
          for (sX = gSelectRegion.iLeft; sX <= gSelectRegion.iRight; sX++) {
            if (iDrawMode == Enum38.DRAW_MODE_LIGHT + Enum38.DRAW_MODE_ERASE) {
              RemoveLight(sX, sY);
            } else EraseMapTile(MAPROWCOLTOPOS(sY, sX));
          }
        }
      ...
      switch (iDrawMode) {
        ...
        case Enum38.DRAW_MODE_CAVES:
          if (
            gusSelectionType >= Enum33.SMALLSELECTION &&
            gusSelectionType <= Enum33.XLARGESELECTION
          )
  ...
  export function ProcessAreaSelection(fWithLeftButton: boolean): void {
    ...
    switch (iDrawMode) {
      ...
      case Enum38.DRAW_MODE_SAW_ROOM:
        ...
        break;
      case Enum38.DRAW_MODE_CAVES:
        if (fWithLeftButton) AddCaveSectionToWorld(gSelectRegion);
  ...
  export function EditScreenHandle(): UINT32 {
    ...
    if (gfWorldLoaded && gMapInformation.ubMapVersion <= 7 && !gfCorruptMap) {
      ScreenMsg(
        FONT_MCOLOR_RED,
        MSG_ERROR,
        "Map data has just been corrupted.  Don't save, don't quit, get Kris!  If he's not here, save the map using a temp filename and document everything you just did, especially your last action!",
      );
      gfCorruptMap = true;
    }
    if (gfWorldLoaded && gubScheduleID > 40 && !gfCorruptSchedules) {
      ...
      if (gubScheduleID > 32) {
        ScreenMsg(
          FONT_MCOLOR_RED,
          MSG_ERROR,
          "Schedule data has just been corrupted.  Don't save, don't quit, get Kris!  If he's not here, save the map using a temp filename and document everything you just did, especially your last action!",
        );
    ...
    if (
      !gfGotoGridNoUI &&
      iDrawMode != Enum38.DRAW_MODE_SHOW_TILESET &&
      !gfSummaryWindowActive &&
      !gfEditingDoor &&
      !gfNoScroll &&
      !InOverheadMap()
    )
