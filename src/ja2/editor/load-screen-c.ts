namespace ja2 {
  ...
  function LoadSaveScreenEntry(): void {
    ...
    zOrigName = swprintf(
      "%s Map (*.dat)",
      iCurrentAction == Enum37.ACTION_SAVE_MAP ? "Save" : "Load",
    );
    ...
    if (gfDeleteFile) {
      if (gfMessageBoxResult) {
        ...
        if (curr) {
          ...
          if (!temp) gzFilename = "";
          else gzFilename = swprintf("%s", temp.FileInfo.zFileName);
          if (ValidFilename()) {
  ...
  export function LoadSaveScreenHandle(): UINT32 {
    ...
    if (gbCurrentFileIOStatus) {
      // loading or saving map
      let uiScreen: UINT32;
    ...
    for (
      x = 0;
      x < iTopFileShown && x < iTotalFiles && FListNode != null;
      x++
    ) {
    ...
    if (gfNoFiles) {
      ...
      for (
        x = iTopFileShown;
        x < iTopFileShown + 8 && x < iTotalFiles && FListNode != null;
        x++
      ) {
        ...
        mprintf(
          186,
          73 + (x - iTopFileShown) * 15,
          "%s",
          FListNode.FileInfo.zFileName,
        );
        FListNode = FListNode.pNext;
      }
  ...
  function CreateFileDialog(zTitle: string /* Pointer<UINT16> */): void {
    ...
    iFileDlgButtons[0] = CreateTextButton(
      "Okay",
      FONT12POINT1(),
      FONT_BLACK,
      FONT_BLACK,
    ...
    // Scroll buttons
    iFileDlgButtons[2] = CreateSimpleButton(
      426,
      92,
      "EDITOR//uparrow.sti",
      BUTTON_NO_TOGGLE,
    ...
    iFileDlgButtons[3] = CreateSimpleButton(
      426,
      182,
    ...
    // File list window
    iFileDlgButtons[4] = CreateHotSpot(
      179 + 4,
      69 + 3,
      179 + 4 + 240,
      69 + 120 + 3,
    ...
    if (iCurrentAction == Enum37.ACTION_SAVE_MAP) {
      ...
      iFileDlgButtons[6] = CreateCheckBoxButton(
        183,
        229,
        "EDITOR//smcheckbox.sti",
        MSYS_PRIORITY_HIGH,
        UpdateWorldInfoCallback,
      );
    ...
    if (fEntering) {
      ...
      for (
        ...
        if (iCurrFileShown == x - iTopFileShown) {
          FListNode.FileInfo.zFileName = FListNode.FileInfo.zFileName.substring(
            0,
            30,
          );
  ...
  export function AddToFDlgList(
    pList: FDLG_LIST | null,
    pInfo: GETFILESTRUCT,
  ): FDLG_LIST {
    let pNode: FDLG_LIST;
    ...
    if (
      pList.FileInfo.zFileName
        .toLowerCase()
        .localeCompare(pInfo.zFileName.toLowerCase()) > 0
    ) {
  ...
  function RemoveFromFDlgList(
    head: Pointer<FDLG_LIST | null>,
    node: FDLG_LIST,
  ): boolean {
    let curr: FDLG_LIST | null;
    curr = head.value;
    while (curr) {
      if (curr == node) {
        if (head.value == node) head.value = <FDLG_LIST>head.value.pNext;
  ...
  function SetTopFileToLetter(usLetter: UINT16): void {
    ...
    while (curr) {
      ...
      if (usNodeLetter < "a".charCodeAt(0)) usNodeLetter += 32; // convert uppercase to lower case A=65, a=97
    ...
    if (FileList) {
      ...
      SetInputFieldStringWith8BitString(
        0,
        (<FDLG_LIST>prev).FileInfo.zFileName,
      );
    }
  }
  ...
  function HandleMainKeyEvents(pEvent: InputAtom): void {
    ...
    switch (pEvent.usParam) {
      case ENTER:
        if (gfNoFiles && iCurrentAction == Enum37.ACTION_LOAD_MAP) break;
        gfDestroyFDlg = true;
        iFDlgState =
          iCurrentAction == Enum37.ACTION_SAVE_MAP
            ? Enum50.DIALOG_SAVE
      ...
      case DNARROW:
        iCurrFileShown++;
        if (iCurrFileShown >= iTotalFiles) iCurrFileShown = iTotalFiles - 1;
        else if (iTopFileShown < iCurrFileShown - 7) iTopFileShown++;
        break;
      case HOME:
      case CTRL_HOME:
        iTopFileShown = 0;
      ...
      default:
        ...
        if (
          (pEvent.usParam >= "a".charCodeAt(0) &&
            pEvent.usParam <= "z".charCodeAt(0)) ||
          (pEvent.usParam >= "A".charCodeAt(0) &&
            pEvent.usParam <= "Z".charCodeAt(0))
        ) {
          if (
  ...
  function SetGlobalSectorValues(
    szFilename: string /* Pointer<UINT16> */,
  ): void {
    ...
    if (ValidCoordinate()) {
      ...
      else gWorldSectorY = gzFilename.charCodeAt(0) - "a".charCodeAt(0) + 1;
      if (gzFilename[1] == "1" && gzFilename[2] >= "0" && gzFilename[2] <= "6")
        gWorldSectorX =
          (gzFilename.charCodeAt(1) - 0x30) * 10 +
          (gzFilename.charCodeAt(2) - 0x30);
      else gWorldSectorX = gzFilename.charCodeAt(1) - 0x30;
  ...
  function ProcessFileIO(): UINT32 {
    ...
    switch (gbCurrentFileIOStatus) {
      case Enum51.INITIATE_MAP_SAVE: // draw save message
        ...
        usStartX =
          320 - Math.trunc(StringPixLength(zOrigName, LARGEFONT1()) / 2);
      ...
      case Enum51.LOADING_MAP: // load map
        ...
        } else gusLightLevel = EDITOR_LIGHT_MAX - ubAmbientLightLevel;
  ...
  function ValidFilename(): boolean {
    ...
    if (gzFilename != "") {
      ...
      if (pDest === -1) pDest = gzFilename.indexOf(".DAT");
  ...
  export function ExternalLoadMap(
    szFilename: string /* Pointer<UINT16> */,
  ): boolean {
    Assert(szFilename);
    if (!szFilename.length) return false;
  ...
  export function ExternalSaveMap(
    szFilename: string /* Pointer<UINT16> */,
  ): boolean {
    Assert(szFilename);
    if (!szFilename.length) return false;
