...
*/
  ...
  function AddMapIndexToTree(usMapIndex: UINT16): boolean {
    ...
    if (curr.usMapIndex < parent.usMapIndex) parent.left = curr;
  ...
  export function AddLightToUndoList(
    iMapIndex: INT32,
    iLightRadius: INT32,
    ubLightID: UINT8,
  ): void {
  ...
  function AddToUndoListCmd(iMapIndex: INT32, iCmdCount: INT32): boolean {
    ...
    while (pStructure) {
      for (
        ubLoop = 1;
        ubLoop < pStructure.pDBStructureRef.pDBStructure.ubNumberOfTiles;
        ubLoop++
      ) {
        ...
        iCoveredMapIndex =
          pStructure.sBaseGridNo +
          pStructure.pDBStructureRef.ppTile[ubLoop].sPosRelToBase;
  ...
  function CheckForMultiTilesInTreeAndAddToUndoList(
    node: MapIndexBinaryTree,
  ): void {
  ...
  export function ExecuteUndoList(): boolean {
    ...
    while (iCurCount < iCmdCount && gpTileUndoStack != null) {
      ...
      if (gpTileUndoStack.pData.fLightSaved) {
        ...
        if (!gpTileUndoStack.pData.ubLightRadius) RemoveLight(sX, sY);
        else
          PlaceLight(
            gpTileUndoStack.pData.ubLightRadius,
            sX,
            sY,
        ...
        SwapMapElementWithWorld(
          iUndoMapIndex,
          createPropertyPointer(gpTileUndoStack.pData, "pMapTile"),
        );
  ...
  function SmoothUndoMapTileTerrain(
    iWorldTile: INT32,
    pUndoTile: MAP_ELEMENT,
  ): void {
  ...
  function DeleteMapElementContentsAfterCreationFail(
    pNewMapElement: MAP_ELEMENT,
  ): void {
...
*/
  function CopyMapElementFromWorld(
    pNewMapElement: MAP_ELEMENT,
    iMapIndex: INT32,
  ): boolean {
    ...
    for (x = 0; x < 9; x++) {
      if (x == 1 || x == 5)
        // skip the pLandStart and pMercLevel LEVELNODES
        continue;
      tail = null;
  ...
  function SwapMapElementWithWorld(
    iMapIndex: INT32,
    pUndoMapElement: Pointer<MAP_ELEMENT>,
  ): boolean {
