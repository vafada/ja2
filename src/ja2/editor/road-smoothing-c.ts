namespace ja2 {
  ...
  let gsRoadMacroStartIndex: INT16[] /* [NUM_ROAD_MACROS] */ = createArray(
    Enum54.NUM_ROAD_MACROS,
    0,
  );
  ...
  // gridnos).
  export function PlaceRoadMacroAtGridNo(
    iMapIndex: INT32,
    iMacroID: INT32,
  ): void {
    let i: INT32;
  ...
  export function ReplaceObsoleteRoads(): void {
    ...
    for (i = 0; i < WORLD_MAX; i++) {
      ...
      while (pObject) {
        if (
          pObject.usIndex >= Enum312.FIRSTROAD1 &&
          pObject.usIndex <= Enum312.FIRSTROAD32
        ) {
