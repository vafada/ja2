namespace ja2 {
  ...
  export function GetActionItemName(
    pItem: OBJECTTYPE,
  ...
  let guiAttachmentButton: UINT32[] /* [NUM_ATTACHMENT_BUTTONS] */ =
    createArray(Enum46.NUM_ATTACHMENT_BUTTONS, 0);
  let gfAttachment: boolean[] /* [NUM_ATTACHMENT_BUTTONS] */ = createArray(
    Enum46.NUM_ATTACHMENT_BUTTONS,
    false,
  );
  ...
  export let gpItem: OBJECTTYPE /* Pointer<OBJECTTYPE> */ = <OBJECTTYPE>(
    (<unknown>null)
  );
  export let gfShowItemStatsPanel: boolean;
  export let gsItemGridNo: INT16;

  export let gpEditingItemPool: ITEM_POOL /* Pointer<ITEM_POOL> */ = <
    ITEM_POOL
  >(<unknown>null);

  export function ShowItemStatsPanel(): void {
    ShowEditorButtons(
      Enum32.FIRST_ITEMSTATS_BUTTON,
      Enum32.LAST_ITEMSTATS_BUTTON,
    );
  ...
  export function SpecifyItemToEdit(
    pItem: OBJECTTYPE | null,
    iMapIndex: INT32,
  ): void {
  ...
  export function UpdateItemStatsPanel(): void {
    ...
    if (
      gpItem &&
      iCurrentTaskbar == Enum36.TASK_ITEMS &&
      gbEditingMode != Enum47.EDITING_TRIGGERS &&
      gbEditingMode != Enum47.EDITING_ACTIONITEMS
    ) {
    ...
    if (gpEditingItemPool) {
      ...
      if (iPercent == 100) SetFontForeground(FONT_YELLOW);
      else if (iPercent >= 50) SetFontForeground(FONT_ORANGE);
  ...
  function RealisticOnlyCheckboxCallback(btn: GUI_BUTTON, reason: INT32): void {
    if (reason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
      ButtonList[giRealisticCheckboxButton].uiFlags |=
  ...
  function SciFiOnlyCheckboxCallback(btn: GUI_BUTTON, reason: INT32): void {
    if (reason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
      ...
      ButtonList[giSciFiCheckboxButton].uiFlags |=
        BUTTON_CLICKED_ON | BUTTON_DIRTY;
      ButtonList[giBothCheckboxButton].uiFlags &= ~BUTTON_CLICKED_ON;
      ButtonList[giBothCheckboxButton].uiFlags |= BUTTON_DIRTY;
      gWorldItems[gpEditingItemPool.iItemIndex].usFlags &= ~(
  ...
  function BothModesCheckboxCallback(btn: GUI_BUTTON, reason: INT32): void {
    if (reason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
      ...
      ButtonList[giSciFiCheckboxButton].uiFlags &= ~BUTTON_CLICKED_ON;
      ButtonList[giSciFiCheckboxButton].uiFlags |= BUTTON_DIRTY;
      ButtonList[giBothCheckboxButton].uiFlags |=
        BUTTON_CLICKED_ON | BUTTON_DIRTY;
      gWorldItems[gpEditingItemPool.iItemIndex].usFlags &= ~(
        WORLD_ITEM_REALISTIC_ONLY | WORLD_ITEM_SCIFI_ONLY
      );
  ...
  function SetupGameTypeFlags(): void {
    if (gpEditingItemPool) {
      giBothCheckboxButton = CreateCheckBoxButton(
        573,
        365,
        "EDITOR//radiobutton.sti",
        MSYS_PRIORITY_NORMAL,
        BothModesCheckboxCallback,
      );
      SetButtonFastHelpText(
        giBothCheckboxButton,
        "Item appears in both Sci-Fi and Realistic modes. (|B)",
      );
      giRealisticCheckboxButton = CreateCheckBoxButton(
        ...
        "EDITOR//radiobutton.sti",
        MSYS_PRIORITY_NORMAL,
        RealisticOnlyCheckboxCallback,
      );
      SetButtonFastHelpText(
        giRealisticCheckboxButton,
        "Item appears in |Realistic mode only.",
      );
      giSciFiCheckboxButton = CreateCheckBoxButton(
        616,
        365,
        "EDITOR//radiobutton.sti",
        MSYS_PRIORITY_NORMAL,
        SciFiOnlyCheckboxCallback,
      );
      SetButtonFastHelpText(
        giSciFiCheckboxButton,
        "Item appears in |Sci-Fi mode only.",
      );
  ...
  function SetupGunGUI(): void {
    ...
    if (ValidAttachment(Enum225.SILENCER, gpItem.usItem)) {
      guiAttachmentButton[Enum46.SILENCER_ATTACHMENT_BUTTON] = CreateTextButton(
        "SILENCER",
        SMALLCOMPFONT(),
        FONT_YELLOW,
        FONT_BLACK,
      ...
      if (FindAttachment(gpItem, Enum225.SILENCER) != -1) {
        ButtonList[
          guiAttachmentButton[Enum46.SILENCER_ATTACHMENT_BUTTON]
    ...
    if (ValidAttachment(Enum225.SNIPERSCOPE, gpItem.usItem)) {
      guiAttachmentButton[Enum46.SNIPERSCOPE_ATTACHMENT_BUTTON] =
        CreateTextButton(
          "SNIPERSCOPE",
          SMALLCOMPFONT(),
          FONT_YELLOW,
    ...
    if (ValidAttachment(Enum225.LASERSCOPE, gpItem.usItem)) {
      guiAttachmentButton[Enum46.LASERSCOPE_ATTACHMENT_BUTTON] =
        CreateTextButton(
          "LASERSCOPE",
          SMALLCOMPFONT(),
          FONT_YELLOW,
    ...
    if (ValidAttachment(Enum225.BIPOD, gpItem.usItem)) {
      guiAttachmentButton[Enum46.BIPOD_ATTACHMENT_BUTTON] = CreateTextButton(
        "BIPOD",
        SMALLCOMPFONT(),
        FONT_YELLOW,
        FONT_BLACK,
      ...
      if (FindAttachment(gpItem, Enum225.BIPOD) != -1) {
        ButtonList[
          guiAttachmentButton[Enum46.BIPOD_ATTACHMENT_BUTTON]
        ].uiFlags |= BUTTON_CLICKED_ON;
        gfAttachment[3] = true;
      }
    }
    ...
    if (ValidAttachment(Enum225.DUCKBILL, gpItem.usItem)) {
      guiAttachmentButton[Enum46.DUCKBILL_ATTACHMENT_BUTTON] = CreateTextButton(
        "DUCKBILL",
        SMALLCOMPFONT(),
        FONT_YELLOW,
        FONT_BLACK,
      ...
      if (FindAttachment(gpItem, Enum225.DUCKBILL) != -1) {
        ButtonList[
          guiAttachmentButton[Enum46.DUCKBILL_ATTACHMENT_BUTTON]
        ].uiFlags |= BUTTON_CLICKED_ON;
        gfAttachment[4] = true;
      }
    }
    ...
    if (ValidAttachment(Enum225.UNDER_GLAUNCHER, gpItem.usItem)) {
      guiAttachmentButton[Enum46.GLAUNCHER_ATTACHMENT_BUTTON] =
        CreateTextButton(
          "G-LAUNCHER",
          SMALLCOMPFONT(),
          FONT_YELLOW,
  ...
  function ExtractAndUpdateGunGUI(): void {
    ...
    if (i == -1) i = 20 + Random(81);
    else i = Math.min(i, 100);
    gpItem.bGunStatus = i;
    SetInputFieldStringWithNumericStrictValue(1, i);
    // Update the ammo
    i = GetNumericStrictValueFromField(2);
    if (i == -1) i = Random(1 + Weapon[gpItem.usItem].ubMagSize);
    else i = Math.min(i, Weapon[gpItem.usItem].ubMagSize);
    ...
    if (gpEditingItemPool) {
      ...
      giDefaultExistChance =
        giDefaultExistChance == -1
          ? 100
          : Math.max(1, Math.min(giDefaultExistChance, 100));
      gWorldItems[gpEditingItemPool.iItemIndex].ubNonExistChance =
        100 - giDefaultExistChance;
  ...
  function ExtractAndUpdateAmmoGUI(): void {
    ...
    i = GetNumericStrictValueFromField(1);
    if (i == -1) i = 1 + Random(Item[gpItem.usItem].ubPerPocket);
    else i = Math.max(1, Math.min(i, Item[gpItem.usItem].ubPerPocket));
  ...
  function ExtractAndUpdateMoneyGUI(): void {
    ...
    i = GetNumericStrictValueFromField(1);
    if (i == -1) i = Random(20000);
    else i = Math.max(1, Math.min(i, 20000));
  ...
  function SetupOwnershipGUI(): void {
    ...
    giOwnershipGroupButton = CreateTextButton(
      gszCivGroupNames[gpItem.ubOwnerCivGroup],
      SMALLCOMPFONT(),
      FONT_YELLOW,
      FONT_BLACK,
      BUTTON_USE_DEFAULT,
  ...
  function ExtractAndUpdateOwnershipGUI(): void {
    ...
    i = GetNumericStrictValueFromField(1);
    if (i == -1) i = Random(0);
    else i = Math.max(0, Math.min(i, 255));
  ...
  function ExtractAndUpdateActionItemsGUI(): void {
    ...
    i = GetNumericStrictValueFromField(2);
    i = i == -1 ? 0 : Math.min(i, 20);
    if (i != gpItem.bTrap) gbDefaultBombTrapLevel = i;
    gpItem.bTrap = i;
    SetInputFieldStringWithNumericStrictValue(2, i);
  ...
  function AlarmTriggerCheckboxCallback(btn: GUI_BUTTON, reason: INT32): void {
    if (reason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
      ...
      else gpItem.fFlags &= ~OBJECT_ALARM_TRIGGER;
    }
  }
    ...
    if (gpEditingItemPool) {
      ...
      if (
        ...
        SetButtonFastHelpText(
          ...
          "If the panic trigger is an alarm trigger,\nenemies won't attempt to use it if they\nare already aware of your presence.",
        );
  ...
  function ToggleAttachment(btn: GUI_BUTTON, reason: INT32): void {
    if (reason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
      ...
      for (i = 0; i < Enum46.NUM_ATTACHMENT_BUTTONS; i++) {
        ...
        if (
          guiAttachmentButton[i] != -1 &&
          btn == ButtonList[guiAttachmentButton[i]]
        ) {
  ...
  export function ChangeActionItem(
    pItem: OBJECTTYPE,
    bActionItemIndex: INT8,
  ): void {
  ...
  function ReEvaluateAttachmentStatii(): void {
    ...
    for (i = 0; i < Enum46.NUM_ATTACHMENT_BUTTONS; i++) {
      if (
        guiAttachmentButton[i] != -1 &&
        !(ButtonList[guiAttachmentButton[i]].uiFlags & BUTTON_CLICKED_ON)
      ) {
        ...
        else DisableButton(guiAttachmentButton[i]);
      }
    }
  }
}
