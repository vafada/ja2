namespace ja2 {
  ...
  export function KillBuilding(iMapIndex: UINT32): void {
    ...
    if (GridNoOnVisibleWorldTile(iMapIndex + WORLD_COLS))
      KillBuilding(iMapIndex + WORLD_COLS);
    if (GridNoOnVisibleWorldTile(iMapIndex + 1)) KillBuilding(iMapIndex + 1);
    if (GridNoOnVisibleWorldTile(iMapIndex - 1)) KillBuilding(iMapIndex - 1);

    if (gfBasement) RebuildRoof(iMapIndex, 0);
  }

  export let gpBuildingLayoutList: BUILDINGLAYOUTNODE | null /* Pointer<BUILDINGLAYOUTNODE> */ =
    null;
  ...
  function BuildLayout(iMapIndex: INT32, iOffset: INT32): void {
    ...
    curr = gpBuildingLayoutList;
    while (curr) {
      if (iMapIndex == curr.sGridNo) return;
      curr = curr.next;
    }
  ...
  export function CopyBuilding(iMapIndex: INT32): void {
    AssertMsg(
      !gpBuildingLayoutList,
      "Error:  Attempting to copy building multiple times.",
    );
  ...
  function SortBuildingLayout(iMapIndex: INT32): void {
    ...
    if (iMapIndex < gsBuildingLayoutAnchorGridNo) {
      ...
      while (gpBuildingLayoutList) {
        ...
        if (prevBest) prevBest.next = (<BUILDINGLAYOUTNODE>best).next;
  ...
  function PasteMapElementToNewMapElement(
    iSrcGridNo: INT32,
    iDstGridNo: INT32,
  ): void {
    ...
    while (pNode) {
      if (pNode == pSrcMapElement.pLandStart)
        gpWorldLevelData[iDstGridNo].pLandStart = AddLandToTail(
          iDstGridNo,
          pNode.usIndex,
        );
      else AddLandToTail(iDstGridNo, pNode.usIndex);
      pNode = pNode.pNext;
    }
  ...
  let iDoorButton: INT32[] /* [NUM_DOOR_BUTTONS] */ = createArray(
    Enum34.NUM_DOOR_BUTTONS,
    0,
  );
  ...
  export function InitDoorEditing(iMapIndex: INT32): void {
    ...
    iDoorButton[Enum34.DOOR_BACKGROUND] = CreateTextButton(
      "",
      0,
      0,
      0,
    ...
    SpecifyDisabledButtonStyle(
      iDoorButton[Enum34.DOOR_BACKGROUND],
      Enum29.DISABLED_STYLE_NONE,
    );
    iDoorButton[Enum34.DOOR_OKAY] = CreateTextButton(
      "Okay",
      FONT12POINT1(),
      FONT_BLACK,
      FONT_BLACK,
    ...
    iDoorButton[Enum34.DOOR_CANCEL] = CreateTextButton(
      "Cancel",
      FONT12POINT1(),
      FONT_BLACK,
      FONT_BLACK,
    ...
    iDoorButton[Enum34.DOOR_LOCKED] = CreateCheckBoxButton(
      210,
      215,
      "EDITOR//SmCheckbox.sti",
      MSYS_PRIORITY_HIGH,
      DoorToggleLockedCallback,
    );

    pDoor = FindDoorInfoAtGridNo(iDoorMapIndex);
    if (pDoor) {
      if (pDoor.fLocked) {
        ButtonList[iDoorButton[Enum34.DOOR_LOCKED]].uiFlags |=
          BUTTON_CLICKED_ON;
      }
  ...
  export function ExtractAndUpdateDoorInfo(): void {
    ...
    if (fCursor) {
      ...
      if (!fCursorExists) AddTopmostToHead(iDoorMapIndex, Enum312.ROTATINGKEY1);
  ...
  export function RenderDoorEditingWindow(): void {
    ...
    mprintf(
      210,
      140,
      "Editing lock attributes at map index %d.",
      iDoorMapIndex,
    );
  ...
  export function KillDoorEditing(): void {
    ...
    for (i = 0; i < Enum34.NUM_DOOR_BUTTONS; i++) RemoveButton(iDoorButton[i]);
    gfEditingDoor = false;
    KillTextInputMode();
  }
