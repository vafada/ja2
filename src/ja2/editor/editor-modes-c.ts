namespace ja2 {
  ...
  export function SetEditorBuildingTaskbarMode(usNewMode: UINT16): void {
    ...
    if (fNewWalls !== undefined && fNewWalls != fBuildingShowWalls) {
      if (fNewWalls) ClickEditorButton(Enum32.BUILDING_TOGGLE_WALL_VIEW);
    ...
    if (fNewRoofs !== undefined && fNewRoofs != fBuildingShowRoofs) {
      if (fNewRoofs) ClickEditorButton(Enum32.BUILDING_TOGGLE_ROOF_VIEW);
    ...
    if (fNewRoomInfo !== undefined && fNewRoomInfo != fBuildingShowRoomInfo) {
      if (fNewRoomInfo) ClickEditorButton(Enum32.BUILDING_TOGGLE_INFO_VIEW);
      else UnclickEditorButton(Enum32.BUILDING_TOGGLE_INFO_VIEW);
  ...
  export function SetEditorSmoothingMode(ubNewMode: UINT8): void {
    UnclickEditorButtons(
      Enum32.MAPINFO_RADIO_NORMAL,
      Enum32.MAPINFO_RADIO_CAVES,
    );
    ...
    switch (ubNewMode) {
      case Enum231.SMOOTHING_NORMAL:
        ...
        if (iCurrentTaskbar == Enum36.TASK_BUILDINGS)
          ShowEditorButtons(
            Enum32.BUILDING_NEW_ROOM,
            Enum32.BUILDING_MOVE_BUILDING,
          );
        EnableEditorButtons(
          Enum32.BUILDING_SMART_WALLS,
          Enum32.BUILDING_SMART_BROKEN_WALLS,
        );
        break;
