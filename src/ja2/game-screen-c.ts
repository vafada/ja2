namespace ja2 {
  ...
  export function MainGameScreenInit(): boolean {
    ...
    giFPSOverlay = RegisterVideoOverlay(
      VOVERLAY_STARTDISABLED | VOVERLAY_DIRTYBYTEXT,
      VideoOverlayDesc,
    );
    ...
    VideoOverlayDesc.BltCallback = BlitMFont;
    giCounterPeriodOverlay = RegisterVideoOverlay(
      VOVERLAY_STARTDISABLED | VOVERLAY_DIRTYBYTEXT,
      VideoOverlayDesc,
    );
  ...
  export function EnterTacticalScreen(): void {
    ...
    if (!gfTacticalPlacementGUIActive) {
      ...
      if (
        gsCurInterfacePanel < 0 ||
        gsCurInterfacePanel >= Enum215.NUM_UI_PANELS
      )
  ...
  export function MainGameScreenHandle(): UINT32 {
    ...
    if (
      ...
      ShouldTheHelpScreenComeUp(Enum17.HELP_SCREEN_TACTICAL, false)
    ) {
  ...
  */
        ...
        if (GetJA2Clock() - gTacticalStatus.uiTimeSinceDemoOn > 3000) {
          if (gTacticalStatus.ubCurrentTeam != gbPlayerNum) {
            AdjustNoAPToFinishMove(
              MercPtrs[gTacticalStatus.ubEnemySightingOnTheirTurnEnemyID],
              false,
            );
          }
          MercPtrs[
    ...
    if (fFirstTimeInGameScreen) {
      ...
      if (!gfTacticalPlacementGUIActive) {
        if (
          ...
          OK_INTERRUPT_MERC(MercPtrs[gusSelectedSoldier])
        ) {
  ...
  function TacticalScreenLocateToSoldier(): void {
    ...
    if (gubPreferredInitialSelectedGuy != NOBODY) {
      ...
      if (
        OK_CONTROLLABLE_MERC(MercPtrs[gubPreferredInitialSelectedGuy]) &&
