namespace ja2 {
  ...
  let gSmokeEffectData: SMOKEEFFECT[] /* [NUM_SMOKE_EFFECT_SLOTS] */ =
    createArrayFrom(NUM_SMOKE_EFFECT_SLOTS, createSmokeEffect);
  ...
  function GetFreeSmokeEffect(): INT32 {
    ...
    for (uiCount = 0; uiCount < guiNumSmokeEffects; uiCount++) {
      if (gSmokeEffectData[uiCount].fAllocated == false) return uiCount;
    }
  ...
  export function NewSmokeEffect(
    sGridNo: INT16,
    usItem: UINT16,
    bLevel: INT8,
    ubOwner: UINT8,
    ...
    switch (usItem) {
      ...
      case Enum225.LARGE_CREATURE_GAS:
        ...
        ubDuration = 3;
        ubStartRadius =
          Explosive[Item[Enum225.LARGE_CREATURE_GAS].ubClassIndex].ubRadius;
        break;
  ...
  export function AddSmokeEffectToTile(
    ...
    // Set world flags
    gpWorldLevelData[sGridNo].ubExtFlags[bLevel] |=
      FromSmokeTypeToWorldFlags(bType);
  ...
  export function RemoveSmokeEffectFromTile(
    sGridNo: INT16,
    bLevel: INT8,
  ): void {
    ...
    if (
      GetCachedAniTileOfType(sGridNo, ubLevelID, ANITILE_SMOKE_EFFECT) == null
    ) {
      ...
      if (pSmoke.fAllocated) {
        ...
        if (gTacticalStatus.uiFlags & INCOMBAT) {
          ...
          if (uiTime - pSmoke.uiTimeOfLastUpdate > 10) {
            ...
            usNumUpdates = Math.trunc(
              (uiTime - pSmoke.uiTimeOfLastUpdate) / 10,
            );
          }
        }
          ...
          for (cnt2 = 0; cnt2 < usNumUpdates; cnt2++) {
            ...
            if (fSpreadEffect) {
              ...
              if (pSmoke.bAge <= pSmoke.ubDuration) {
                ...
                if (
                  pSmoke.ubRadius <
                  Explosive[Item[pSmoke.usItem].ubClassIndex].ubRadius
                ) {
                  pSmoke.ubRadius++;
                }
              } else {
  ...
  */
    ...
    if (guiSaveGameVersion < 75) {
      ...
      for (uiCount = 0; uiCount < guiNumSmokeEffects; uiCount++) {
        ...
        if (gSmokeEffectData[uiCount].fAllocated) {
          ...
          SpreadEffect(
            gSmokeEffectData[uiCount].sGridNo,
            gSmokeEffectData[uiCount].ubRadius,
            gSmokeEffectData[uiCount].usItem,
            gSmokeEffectData[uiCount].ubOwner,
            1,
            bLevel,
            uiCount,
          );
  ...
  export function SaveSmokeEffectsToMapTempFile(
    sMapX: INT16,
    sMapY: INT16,
    bMapZ: INT8,
  ): boolean {
  ...
  export function LoadSmokeEffectsFromMapTempFile(
    sMapX: INT16,
    sMapY: INT16,
    bMapZ: INT8,
  ): boolean {
