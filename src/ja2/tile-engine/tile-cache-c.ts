namespace ja2 {
  ...
  export let gpTileCache: TILE_CACHE_ELEMENT[] /* Pointer<TILE_CACHE_ELEMENT> */ =
    <TILE_CACHE_ELEMENT[]>(<unknown>null);
  let gpTileCacheStructInfo: TILE_CACHE_STRUCT[] /* Pointer<TILE_CACHE_STRUCT> */ =
    <TILE_CACHE_STRUCT[]>(<unknown>null);
  ...
  export function InitTileCache(): boolean {
    ...
    // load any we find...
    const tileCacheDirName = fs
      .readdirSync(".")
      .find((entry) => entry.toLowerCase() === "tilecache");
    if (tileCacheDirName) {
      const fileNames = fs
        .readdirSync(tileCacheDirName)
        .filter((entry) => entry.toLowerCase().endsWith(".jsd"));
      ...
      if (sFiles > 0) {
        ...
        for (let i = 0; i < sFiles; i++) {
          gpTileCacheStructInfo[cnt].Filename = sprintf(
            "%s\\%s",
            tileCacheDirName,
            fileNames[i],
          );
          ...
          // Get root name
          gpTileCacheStructInfo[cnt].zRootName = GetRootName(
            gpTileCacheStructInfo[cnt].Filename,
          );

          // Load struc data....
          gpTileCacheStructInfo[cnt].pStructureFileRef = LoadStructureFile(
            path.join(tileCacheDirName, fileNames[i]),
          );
  ...
  function FindCacheStructDataIndex(
    cFilename: string /* Pointer<INT8> */,
  ): INT16 {
    let cnt: UINT32;

    for (cnt = 0; cnt < guiNumTileCacheStructs; cnt++) {
      if (
        gpTileCacheStructInfo[cnt].zRootName.toLowerCase() ===
        cFilename.toLowerCase()
      ) {
        return cnt;
      }
    }
  ...
  export function GetCachedTile(cFilename: string /* Pointer<INT8> */): INT32 {
    ...
    for (cnt = 0; cnt < guiMaxTileCacheSize; cnt++) {
      if (gpTileCache[cnt].pImagery == null) {
        ...
        gpTileCache[cnt].sStructRefID = FindCacheStructDataIndex(
          gpTileCache[cnt].zRootName,
        );
        ...
        if (gpTileCache[cnt].sStructRefID != -1) {
          AddZStripInfoToVObject(
            pImagery.vo,
            <STRUCTURE_FILE_REF>(
              gpTileCacheStructInfo[gpTileCache[cnt].sStructRefID]
                .pStructureFileRef
            ),
            true,
            0,
  ...
  function GetCachedTileStructureRef(iIndex: INT32): STRUCTURE_FILE_REF | null {
    ...
    return gpTileCacheStructInfo[gpTileCache[iIndex].sStructRefID]
      .pStructureFileRef;
  }

  export function GetCachedTileStructureRefFromFilename(
    cFilename: string /* Pointer<INT8> */,
  ...
  export function CheckForAndAddTileCacheStructInfo(
    pNode: LEVELNODE,
    sGridNo: INT16,
    usIndex: UINT16,
    usSubIndex: UINT16,
    ...
    if (pStructureFileRef != null) {
      if (
        !AddStructureToWorld(
          sGridNo,
          0,
          pStructureFileRef.pDBStructureRef[usSubIndex],
          pNode,
        )
      ) {
        if (giDefaultStructIndex != -1) {
          pStructureFileRef =
            gpTileCacheStructInfo[giDefaultStructIndex].pStructureFileRef;
  ...
  export function CheckForAndDeleteTileCacheStructInfo(
    pNode: LEVELNODE,
    usIndex: UINT16,
  ): void {
    let pStructureFileRef: STRUCTURE_FILE_REF | null;
    ...
    if (usIndex >= TILE_CACHE_START_INDEX) {
      pStructureFileRef = GetCachedTileStructureRef(
        usIndex - TILE_CACHE_START_INDEX,
      );
