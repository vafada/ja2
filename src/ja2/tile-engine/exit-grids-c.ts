namespace ja2 {
  ...
  function ConvertINT32ToExitGrid(
    iExitGridInfo: INT32,
    pExitGrid: EXITGRID,
  ): void {
  ...
  export function AddExitGridToWorld(
    iMapIndex: INT32,
    pExitGrid: EXITGRID,
  ): void {
    let pShadow: LEVELNODE | null;
  ...
  export function RemoveExitGridFromWorld(iMapIndex: INT32): void {
    let usDummy: UINT16;
    if (
      (usDummy = TypeExistsInShadowLayer(iMapIndex, Enum313.MOCKFLOOR)) !== -1
    ) {
  ...
  export function LoadExitGrids(buffer: Buffer, offset: number): number {
    ...
    usNumSaved = buffer.readUInt16LE(offset);
    offset += 2;
    ...
    for (x = 0; x < usNumSaved; x++) {
      ...
      readExitGrid(exitGrid, buffer, offset);
      offset += 5;
  ...
  export function AttemptToChangeFloorLevel(bRelativeZLevel: INT8): void {
    ...
    if (bRelativeZLevel == -1 && !gbWorldSectorZ) {
      ScreenMsg(
        FONT_DKYELLOW,
        MSG_INTERFACE,
        pMessageStrings[Enum333.MSG_CANT_GO_UP],
        ubLookForLevel,
      );
      return;
    }
    ...
    if (bRelativeZLevel == 1 && gbWorldSectorZ == 3) {
      ScreenMsg(
        FONT_DKYELLOW,
        MSG_INTERFACE,
        pMessageStrings[Enum333.MSG_CANT_GO_DOWN],
        ubLookForLevel,
      );
      return;
    }
    ...
    for (i = 0; i < WORLD_MAX; i++) {
      if (GetExitGrid(i, gExitGrid)) {
        if (gExitGrid.ubGotoSectorZ == ubLookForLevel) {
          ...
          // to the new sector.
          MoveAllGroupsInCurrentSectorToSector(
            gWorldSectorX,
            gWorldSectorY,
            ubLookForLevel,
          );
          if (ubLookForLevel)
            ScreenMsg(
              FONT_YELLOW,
              MSG_INTERFACE,
              pMessageStrings[Enum333.MSG_ENTERING_LEVEL],
              ubLookForLevel,
            );
          else
            ScreenMsg(
              FONT_YELLOW,
              MSG_INTERFACE,
              pMessageStrings[Enum333.MSG_LEAVING_BASEMENT],
            );
  ...
  export function FindGridNoFromSweetSpotCloseToExitGrid(
    ...
    for (cnt1 = sBottom; cnt1 <= sTop; cnt1++) {
      leftmost =
        Math.trunc((pSoldier.sGridNo + WORLD_COLS * cnt1) / WORLD_COLS) *
        WORLD_COLS;
        ...
        if (
          ...
          if (NewOKDestination(pSoldier, sGridNo, true, pSoldier.bLevel)) {
            if (GetExitGrid(sGridNo, ExitGrid)) {
              ...
              if (
                ExitGrid.ubGotoSectorX == ubGotoSectorX &&
  ...
  export function FindClosestExitGrid(
    pSoldier: SOLDIERTYPE,
    sSrcGridNo: INT16,
    ubRadius: INT8,
  ): UINT16 {
    ...
    for (cnt1 = sBottom; cnt1 <= sTop; cnt1++) {
      leftmost =
        Math.trunc((sSrcGridNo + WORLD_COLS * cnt1) / WORLD_COLS) * WORLD_COLS;
