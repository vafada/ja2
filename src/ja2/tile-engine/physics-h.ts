namespace ja2 {
  ...
  export function readRealObject(
    o: REAL_OBJECT,
    buffer: Buffer,
    offset: number = 0,
  ): number {
    ...
    o.TestZTarget = buffer.readFloatLE(offset);
    offset += 4;
    o.OneOverMass = buffer.readFloatLE(offset);
    offset += 4;
    o.AppliedMu = buffer.readFloatLE(offset);
    offset += 4;
    ...
    o.CollisionElasticity = buffer.readFloatLE(offset);
    offset += 4;
    ...
    o.sConsecutiveCollisions = buffer.readInt16LE(offset);
    offset += 2;
    o.sConsecutiveZeroVelocityCollisions = buffer.readInt16LE(offset);
    offset += 2;
    o.iOldCollisionCode = buffer.readInt32LE(offset);
    offset += 4;

    o.dLifeLength = buffer.readFloatLE(offset);
    offset += 4;
    o.dLifeSpan = buffer.readFloatLE(offset);
    offset += 4;
    offset = readObjectType(o.Obj, buffer, offset);
    o.fFirstTimeMoved = Boolean(buffer.readUInt8(offset++));
    offset++; // padding
    o.sFirstGridNo = buffer.readInt16LE(offset);
    offset += 2;
    o.ubOwner = buffer.readUInt8(offset++);
    o.ubActionCode = buffer.readUInt8(offset++);
    offset += 2; // padding
    o.uiActionData = buffer.readUInt32LE(offset);
    offset += 4;
    o.fDropItem = Boolean(buffer.readUInt8(offset++));
    offset += 3; // padding
    o.uiNumTilesMoved = buffer.readUInt32LE(offset);
    offset += 4;
    ...
    o.sLevelNodeGridNo = buffer.readInt16LE(offset);
    offset += 2;
    o.iSoundID = buffer.readInt32LE(offset);
    offset += 4;
  ...
  export function writeRealObject(
    o: REAL_OBJECT,
    buffer: Buffer,
    offset: number = 0,
  ): number {
    ...
    offset = buffer.writeUInt8(
      Number(o.fEndedWithCollisionPositionSet),
      offset,
    );
    offset = writePadding(buffer, offset, 3); // padding
