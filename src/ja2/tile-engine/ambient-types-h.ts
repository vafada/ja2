namespace ja2 {
  ...
  export function readAmbientDataStruct(
    o: AMBIENTDATA_STRUCT,
    buffer: Buffer,
    offset: number = 0,
  ): number {
    o.uiMinTime = buffer.readUInt32LE(offset);
    offset += 4;
    o.uiMaxTime = buffer.readUInt32LE(offset);
    offset += 4;
    o.ubTimeCatagory = buffer.readUInt8(offset++);
    o.zFilename = readStringNL(buffer, "ascii", offset, SGPFILENAME_LEN);
    offset += SGPFILENAME_LEN;
    offset += 3; // padding
    o.uiVol = buffer.readUInt32LE(offset);
    offset += 4;
    return offset;
  }

  export function writeAmbientDataStruct(
    o: AMBIENTDATA_STRUCT,
    buffer: Buffer,
    offset: number = 0,
  ): number {
