namespace ja2 {
  ...
  export function DetermineRGBDistributionSettings(): void {
    ...
    if (!SetFileManCurrentDirectory(ShadeTableDir)) {
      if (!MakeFileManDirectory(ShadeTableDir)) {
        AssertMsg(
          0,
          "ShadeTable directory doesn't exist, and couldn't create one!",
        );
      }
      if (!SetFileManCurrentDirectory(ShadeTableDir)) {
    ...
    ({
      usRedMask: uiRBitMask,
      usGreenMask: uiGBitMask,
      usBlueMask: uiBBitMask,
    } = GetPrimaryRGBDistributionMasks());

    if (fLoadedPrevRGBDist) {
      if (
        uiRBitMask != uiPrevRBitMask ||
        uiGBitMask != uiPrevGBitMask ||
  ...
  export function LoadShadeTable(
    pObj: SGPVObject,
    uiTileTypeIndex: UINT32,
  ): boolean {
    let hfile: HWFILE;
    let i: INT32;
    let uiNumBytesRead: UINT32;
  ...
  export function SaveShadeTable(
    pObj: SGPVObject,
    uiTileTypeIndex: UINT32,
  ): boolean {
    let hfile: HWFILE;
    ...
    for (i = 0; i < 16; i++) {
      buffer = Buffer.from(
        pObj.pShades[i].buffer,
        pObj.pShades[i].byteOffset,
        pObj.pShades[i].byteLength,
      );
