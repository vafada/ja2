namespace ja2 {
  ...
  export function CreateAnimationTile(
    pAniParams: ANITILE_PARAMS,
  ): ANITILE | null {
    ...
    pNewAniNode.pLevelNode.uiFlags |=
    ...
    if (uiFlags & ANITILE_CACHEDTILE) {
      ...
      pNewAniNode.usNumFrames = (<TILE_ANIMATION_DATA>(
        gTileDatabase[usTileIndex].pAnimData
      )).ubNumFrames;
    }
  ...
  export function UpdateAniTiles(): void {
    ...
    while (pAniNode != null) {
      ...
      if (
        uiClock - pNode.uiTimeLastUpdate > pNode.sDelay &&
            ...
            if (pNode.sCurrentFrame == pNode.ubKeyFrame1) {
              switch (pNode.uiKeyFrame1Code) {
                ...
                case Enum311.ANI_KEYFRAME_CHAIN_WATER_EXPLOSION:
                  IgniteExplosion(
                    pNode.ubUserData2,
                    pNode.pLevelNode.sRelativeX,
                    pNode.pLevelNode.sRelativeY,
                    0,
                    pNode.sGridNo,
                    pNode.uiUserData,
                    0,
                ...
                case Enum311.ANI_KEYFRAME_DO_SOUND:
                  PlayJA2Sample(
                    pNode.uiUserData,
                    RATE_11025,
                    SoundVolume(MIDVOLUME, pNode.uiUserData3),
                    1,
                    SoundDir(pNode.uiUserData3),
                  );
                  break;
              }
            }
            ...
            if (pNode.sCurrentFrame == pNode.ubKeyFrame2) {
              ...
              switch (pNode.uiKeyFrame2Code) {
                case Enum311.ANI_KEYFRAME_BEGIN_DAMAGE:
                  ubExpType =
                    Explosive[Item[pNode.uiUserData].ubClassIndex].ubType;

                  if (
                    ubExpType == Enum287.EXPLOSV_TEARGAS ||
                    ubExpType == Enum287.EXPLOSV_MUSTGAS ||
                    ubExpType == Enum287.EXPLOSV_SMOKE
                  ) {
                    // Do sound....
                    ...
                    SpreadEffect(
                      pNode.sGridNo,
                      Explosive[Item[pNode.uiUserData].ubClassIndex].ubRadius,
                      pNode.uiUserData,
                      pNode.ubUserData2,
                      0,
                      gExplosionData[pNode.uiUserData3].Params.bLevel,
  ...
  export function GetCachedAniTileOfType(
    sGridNo: INT16,
    ubLevelID: UINT8,
    uiFlags: UINT32,
  ): ANITILE | null {
