namespace ja2 {
  ...
  export function createProfile(): UINT8[][] {
    return createArrayFrom(PROFILE_X_SIZE, () =>
      createArray(PROFILE_Y_SIZE, 0),
    );
  }
  ...
  export function copyDbStructureTile(
    destination: DB_STRUCTURE_TILE,
    source: DB_STRUCTURE_TILE,
  ) {
  ...
  export function readDbStructureTile(
    o: DB_STRUCTURE_TILE,
    buffer: Buffer,
    offset: number = 0,
  ): number {
    o.sPosRelToBase = buffer.readInt16LE(offset);
    offset += 2;
  ...
  export function writeDbStructureTile(
    o: DB_STRUCTURE_TILE,
    buffer: Buffer,
    offset: number = 0,
  ): number {
  ...
  export function readDbStructure(
    o: DB_STRUCTURE,
    buffer: Buffer,
    offset: number = 0,
  ): number {
    ...
    offset += 4;
    o.usStructureNumber = buffer.readUInt16LE(offset);
    offset += 2;
  ...
  export function writeDbStructure(
    o: DB_STRUCTURE,
    buffer: Buffer,
    offset: number = 0,
  ): number {
  ...
  export function readStructureFileHeader(
    o: STRUCTURE_FILE_HEADER,
    buffer: Buffer,
    offset: number = 0,
  ): number {
    o.szId = readStringNL(buffer, "ascii", offset, offset + 4);
    offset += 4;
    ...
    o.usNumberOfStructuresStored = buffer.readUInt16LE(offset);
    offset += 2;
    o.usStructureDataSize = buffer.readUInt16LE(offset);
    offset += 2;
    o.fFlags = buffer.readUInt8(offset++);
    offset = readUIntArray(o.bUnused, buffer, offset, 1);
    o.usNumberOfImageTileLocsStored = buffer.readUInt16LE(offset);
    offset += 2;
