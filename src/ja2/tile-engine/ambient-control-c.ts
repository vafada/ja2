namespace ja2 {
  export let gAmbData: AMBIENTDATA_STRUCT[] /* [MAX_AMBIENT_SOUNDS] */ =
    createArrayFrom(MAX_AMBIENT_SOUNDS, createAmbientDataStruct);
    ...
    if (!gfBasement && !gfCaves) {
      if (LoadAmbientControlFile(ubAmbientID)) {
        ...
        DebugMsg(
          TOPIC_JA2,
          DBG_LEVEL_0,
          FormatString("Cannot load Ambient data for tileset"),
        );
      }
    }
  }
  ...
  function StartSteadyStateAmbient(ubVolume: UINT32, ubLoops: UINT32): UINT32 {
    ...
    return SoundPlay(
      gSteadyStateAmbientTable[gubCurrentSteadyStateAmbience].zSoundNames[
  ...
  function SetSteadyStateAmbience(ubAmbience: UINT8): boolean {
    ...
    for (
      cnt = Number(fInNight) * 4;
      cnt < Math.trunc(NUM_SOUNDS_PER_TIMEFRAME / 2);
      cnt++
    ) {
