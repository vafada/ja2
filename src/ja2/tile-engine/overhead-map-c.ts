namespace ja2 {
  ...
  let gSmTileSurf: SMALL_TILE_SURF[] /* [NUMBEROFTILETYPES] */ =
    createArrayFrom(Enum313.NUMBEROFTILETYPES, createSmallTileSurf);
  let gSmTileDB: SMALL_TILE_DB[] /* [NUMBEROFTILES] */ = createArrayFrom(
  ...
  export function InitNewOverheadDB(ubTilesetID: UINT8): void {
    ...
    for (uiLoop = 0; uiLoop < Enum313.NUMBEROFTILETYPES; uiLoop++) {
      ...
      // Adjust for BPP
      cFileBPP = FilenameForBPP(
        gTilesets[ubTilesetID].TileSurfaceFilenames[uiLoop],
      );
  ...
  function GetClosestItemPool(
    sSweetGridNo: INT16,
    ppReturnedItemPool: Pointer<ITEM_POOL>,
    ubRadius: UINT8,
    bLevel: INT8,
  ): boolean {
    let sTop: INT16;
  ...
  function GetClosestMercInOverheadMap(
    sSweetGridNo: INT16,
    ppReturnedSoldier: Pointer<SOLDIERTYPE>,
    ubRadius: UINT8,
  ): boolean {
    let sTop: INT16;
    let sBottom: INT16;
    ...
    for (cnt1 = sBottom; cnt1 <= sTop; cnt1++) {
      ...
      for (cnt2 = sLeft; cnt2 <= sRight; cnt2++) {
        ...
        if (
          ...
          // Go on sweet stop
          if (
            gpWorldLevelData[sGridNo].pMercHead != null &&
            (<LEVELNODE>gpWorldLevelData[sGridNo].pMercHead).pSoldier
              .bVisible != -1
          ) {
            uiRange = GetRangeInCellCoordsFromGridNoDiff(sSweetGridNo, sGridNo);

            if (uiRange < uiLowestRange) {
              ppReturnedSoldier.value = (<LEVELNODE>(
  ...
  function DisplayMercNameInOverhead(pSoldier: SOLDIERTYPE): void {
    ...
    ({ sScreenX: sWorldScreenX, sScreenY: sWorldScreenY } =
      GetWorldXYAbsoluteScreenXY(
    ...
    sWorldScreenY =
      gsStartRestrictedY +
      Math.trunc(sWorldScreenY / 5) +
      Math.trunc(pSoldier.sHeightAdjustment / 5) +
      Math.trunc(gpWorldLevelData[pSoldier.sGridNo].sHeight / 5) -
      8;
  ...
  export function HandleOverheadMap(): void {
    ...
    if (!gfEditMode && !gfTacticalPlacementGUIActive) {
      ...
      let pItemPool__Pointer = createPointer(
        () => pItemPool,
        (v) => (pItemPool = v),
      );
      ...
      if (GetOverheadMouseGridNo(usMapPos__Pointer)) {
        ...
        if (GetClosestItemPool(usMapPos, pItemPool__Pointer, 1, 0)) {
          ...
          if (
            ConditionalGetCurInteractiveTileGridNoAndStructure(
              createPointer(
                () => sIntTileGridNo,
                (v) => (sIntTileGridNo = v),
              ),
  ...
  export function GoIntoOverheadMap(): void {
    ...
    hVObject.pShades[2] = Create16BPPPaletteShaded(
      hVObject.pPaletteEntry,
      0,
      0,
      0,
  ...
  export function RenderOverheadMap(
    ...
    if (gfOverheadMapDirty) {
      ...
      do {
        ...
        do {
          ...
          if (usTileIndex < GRIDSIZE) {
            sHeight = Math.trunc(GetOffsetLandHeight(usTileIndex) / 5);
            sModifiedHeight = Math.trunc(
              GetModifiedOffsetLandHeight(usTileIndex) / 5,
            );
            ...
            while (pNode != null) {
              if (pNode.usIndex < Enum312.NUMBEROFTILES) {
                ...
                if (!(pNode.uiFlags & LEVELNODE_ITEM)) {
                  ...
                  if (
                    gTileDatabase[pNode.usIndex].uiFlags & IGNORE_WORLD_HEIGHT
                  ) {
                    sY -= sModifiedHeight;
                  } else {
                    sY -= sHeight;
            ...
            while (pNode != null) {
              if (pNode.usIndex < Enum312.NUMBEROFTILES) {
                ...
                if (!(pNode.uiFlags & LEVELNODE_ITEM)) {
                  ...
                  sY =
                    sTempPosY_S -
                    Math.trunc(gTileDatabase[pNode.usIndex].sOffsetHeight / 5);
                ...
                if (pNode.usIndex < Enum312.NUMBEROFTILES) {
                  if (!(pNode.uiFlags & LEVELNODE_HIDDEN)) {
                    ...
                    sY =
                      sTempPosY_S -
                      Math.trunc(
                        gTileDatabase[pNode.usIndex].sOffsetHeight / 5,
                      ) -
                      sHeight;
      ...
      if (gMapInformation.ubRestrictedScrollID != 0) {
        ({ sX1, sY1, sX2, sY2 } = CalculateRestrictedMapCoords(
          Enum245.NORTH,
          sEndXS,
          sEndYS,
        ));
  ...
  function RenderOverheadOverlays(): void {
    ...
    for (i = 0; i < end; i++) {
      ...
      ({ sScreenX: sX, sScreenY: sY } = GetOverheadScreenXYFromGridNo(
        pSoldier.sGridNo,
      ));
      // Now, draw his "doll"
      ...
      //}
        ...
        gpTacticalPlacementHilightedSoldier == pSoldier &&
...
*/
  ...
  function MoveInOverheadRegionCallback(
    reg: MOUSE_REGION,
    reason: INT32,
  ): void {
  ...
  function ClickOverheadRegionCallback(reg: MOUSE_REGION, reason: INT32): void {
    ...
    if (reason & MSYS_CALLBACK_REASON_LBUTTON_DWN) {
      ...
      ({ uiCellX, uiCellY } = GetFromAbsoluteScreenXYWorldXY(
        sWorldScreenX,
        sWorldScreenY,
      ));
  ...
  function GetOverheadScreenXYFromGridNo(sGridNo: INT16): {
    sScreenX: INT16;
    sScreenY: INT16;
  } {
    let sScreenX: INT16;
    ...
    ({ sScreenX, sScreenY } = GetWorldXYAbsoluteScreenXY(
      Math.trunc(CenterX(sGridNo) / CELL_X_SIZE),
      Math.trunc(CenterY(sGridNo) / CELL_Y_SIZE),
    ));
  ...
  export function GetOverheadMouseGridNo(psGridNo: Pointer<INT16>): boolean {
    ...
    if (OverheadRegion.uiFlags & MSYS_MOUSE_IN_AREA) {
      ...
      // Get gridNo
      psGridNo.value = MAPROWCOLTOPOS(
        Math.trunc(uiCellY / CELL_Y_SIZE),
        Math.trunc(uiCellX / CELL_X_SIZE),
      );
  ...
  function GetOverheadMouseGridNoForFullSoldiersGridNo(
  ...
  export function CalculateRestrictedMapCoords(
    bDirection: INT8,
    sEndXS: INT16,
    sEndYS: INT16,
  ): { sX1: INT16; sY1: INT16; sX2: INT16; sY2: INT16 } {
    let sX1: INT16 = 0;
    let sY1: INT16 = 0;
    let sX2: INT16 = 0;
    let sY2: INT16 = 0;
    ...
    switch (bDirection) {
      ...
      case Enum245.SOUTH:
        ...
        sX2 = sEndXS;
        sY1 = Math.trunc(
          (NORMAL_MAP_SCREEN_HEIGHT - Math.abs(NORMAL_MAP_SCREEN_BY - gsBLY)) /
            5,
        );
        sY2 = sEndYS;
        break;
      ...
      case Enum245.EAST:
        sX1 = Math.trunc(
          (NORMAL_MAP_SCREEN_WIDTH - Math.abs(NORMAL_MAP_SCREEN_X - gsTRX)) / 5,
        );
        sX2 = sEndXS;
        sY1 = 0;
        sY2 = sEndYS;
