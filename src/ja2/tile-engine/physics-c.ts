namespace ja2 {
  ...
  const OBJECT_DETONATE_ON_IMPACT = (o: REAL_OBJECT) =>
    o.Obj.usItem == Enum225.MORTAR_SHELL; // && ( o->ubActionCode == THROW_ARM_ITEM || pObject->fTestObject ) )
  ...
  export let ObjectSlots: REAL_OBJECT[] /* [NUM_OBJECT_SLOTS] */ =
    createArrayFrom(NUM_OBJECT_SLOTS, createRealObject);
  ...
  const SCALE_VERT_VAL_TO_HORZ = (f: number) =>
    (f / HEIGHT_UNITS) * CELL_X_SIZE;
  const SCALE_HORZ_VAL_TO_VERT = (f: number) =>
    (f / CELL_X_SIZE) * HEIGHT_UNITS;
  ...
  function GetFreeObjectSlot(): INT32 {
    ...
    for (uiCount = 0; uiCount < guiNumObjectSlots; uiCount++) {
      if (ObjectSlots[uiCount].fAllocated == false) return uiCount;
    }

    if (guiNumObjectSlots < NUM_OBJECT_SLOTS) return guiNumObjectSlots++;
  ...
  export function CreatePhysicalObject(
    ...
    // Calculate gridNo
    pObject.sGridNo = MAPROWCOLTOPOS(
      Math.trunc(yPos / CELL_Y_SIZE),
      Math.trunc(xPos / CELL_X_SIZE),
    );
    ...
    if (pObject.sGridNo != NOWHERE) {
      pObject.Position.z += CONVERT_PIXELS_TO_HEIGHTUNITS(
        gpWorldLevelData[pObject.sGridNo].sHeight,
      );
      pObject.EndedWithCollisionPosition.z += CONVERT_PIXELS_TO_HEIGHTUNITS(
        gpWorldLevelData[pObject.sGridNo].sHeight,
      );
    }
  ...
  function SimulateObject(pObject: REAL_OBJECT, deltaT: FLOAT): void {
    ...
    let iCollisionID__Pointer = createPointer(
      () => iCollisionID,
      (v) => (iCollisionID = v),
    );
    ...
    if (pObject.fAlive) {
      ...
      while (CurrentTime < TargetTime) {
        ...
        if (
          !PhysicsHandleCollisions(pObject, iCollisionID__Pointer, DeltaTime)
        ) {
          fEndThisObject = true;
          break;
        }
  ...
  function PhysicsUpdateLife(pObject: REAL_OBJECT, DeltaTime: FLOAT): boolean {
    ...
    if (!pObject.fAlive) {
      ...
      if (!pObject.fTestObject) {
        ...
        if (
          pObject.ubActionCode == Enum258.THROW_ARM_ITEM &&
          !pObject.fInWater
        ) {
          ...
          if (
            !pObject.fInWater ||
            !(Item[pObject.Obj.usItem].fFlags & ITEM_SINKS)
          ) {
        ...
        // Make impact noise....
        if (
          pObject.Obj.usItem == Enum225.ROCK ||
          pObject.Obj.usItem == Enum225.ROCK2
        ) {
          MakeNoise(
            pObject.ubOwner,
            pObject.sGridNo,
            0,
            gpWorldLevelData[pObject.sGridNo].ubTerrainID,
            9 + PreRandom(9),
            Enum236.NOISE_ROCK_IMPACT,
          );
          ...
          MakeNoise(
            ...
            gpWorldLevelData[pObject.sGridNo].ubTerrainID,
            9 + PreRandom(9),
            Enum236.NOISE_GRENADE_IMPACT,
          );
        }

        if (
          !pObject.fTestObject &&
          pObject.iOldCollisionCode == Enum229.COLLISION_GROUND
        ) {
          PlayJA2Sample(
            Enum330.THROW_IMPACT_2,
            RATE_11025,
            SoundVolume(MIDVOLUME, pObject.sGridNo),
            1,
  ...
  function PhysicsIntegrate(pObject: REAL_OBJECT, DeltaTime: FLOAT): boolean {
    ...
    if (
      pObject.Obj.usItem == Enum225.MORTAR_SHELL &&
      !pObject.fTestObject &&
      pObject.ubActionCode == Enum258.THROW_ARM_ITEM
    ) {
      ...
      if (pObject.OldVelocity.z >= 0 && pObject.Velocity.z < 0) {
        if (pObject.iSoundID == NO_SAMPLE) {
          pObject.iSoundID = PlayJA2Sample(
            Enum330.MORTAR_WHISTLE,
            RATE_11025,
            HIGHVOLUME,
            1,
            MIDDLEPAN,
  ...
  function PhysicsHandleCollisions(
    pObject: REAL_OBJECT,
    piCollisionID: Pointer<INT32>,
    DeltaTime: FLOAT,
  ): boolean {
    let dDeltaX: FLOAT;
    let dDeltaY: FLOAT;
    ...
    if (PhysicsCheckForCollisions(pObject, piCollisionID)) {
      ...
      if (pObject.sConsecutiveZeroVelocityCollisions > 3) {
        ...
        if (
          piCollisionID.value == Enum229.COLLISION_STRUCTURE_Z ||
          piCollisionID.value == Enum229.COLLISION_ROOF ||
          piCollisionID.value == Enum229.COLLISION_GROUND
        ) {
        ...
        PhysicsResolveCollision(
          pObject,
          pObject.CollisionVelocity,
          pObject.CollisionNormal,
  ...
  function PhysicsCheckForCollisions(
    pObject: REAL_OBJECT,
    piCollisionID: Pointer<INT32>,
  ): boolean {
    let vTemp: vector_3 = createVector3();
    ...
    if (pObject.fTestObject != TEST_OBJECT_NO_COLLISIONS) {
      iCollisionCode = CheckForCollision(
        dX,
        dY,
        dZ,
        dDeltaX,
    ...
    if (pObject.fTestObject == TEST_OBJECT_ANY_COLLISION) {
      if (
        iCollisionCode != Enum229.COLLISION_GROUND &&
        iCollisionCode != Enum229.COLLISION_ROOF &&
        iCollisionCode != Enum229.COLLISION_WATER &&
        iCollisionCode != Enum229.COLLISION_NONE
      ) {
    ...
    if (pObject.fTestObject == TEST_OBJECT_NOTWALLROOF_COLLISIONS) {
      ...
      if (
        ...
        iCollisionCode != Enum229.COLLISION_NONE &&
        iCollisionCode != Enum229.COLLISION_ROOF &&
        iCollisionCode != Enum229.COLLISION_INTERIOR_ROOF &&
        iCollisionCode != Enum229.COLLISION_WALL_SOUTHEAST &&
        iCollisionCode != Enum229.COLLISION_WALL_SOUTHWEST &&
        iCollisionCode != Enum229.COLLISION_WALL_NORTHEAST &&
        iCollisionCode != Enum229.COLLISION_WALL_NORTHWEST
      ) {
        if (
          pObject.fFirstTimeMoved ||
          pObject.sFirstGridNo == pObject.sGridNo
        ) {
          iCollisionCode = Enum229.COLLISION_NONE;
        }
    ...
    if (iCollisionCode > Enum229.COLLISION_NONE) {
      ...
      if (
        iCollisionCode == Enum229.COLLISION_WINDOW_NORTHWEST ||
        iCollisionCode == Enum229.COLLISION_WINDOW_NORTHEAST ||
        iCollisionCode == Enum229.COLLISION_WINDOW_SOUTHWEST ||
        iCollisionCode == Enum229.COLLISION_WINDOW_SOUTHEAST
      ) {
        if (!pObject.fTestObject) {
          // Break window!
          PhysicsDebugMsg(
            FormatString("Object %d: Collision Window", pObject.iID),
          );

          sGridNo = MAPROWCOLTOPOS(
            Math.trunc(pObject.Position.y / CELL_Y_SIZE),
            Math.trunc(pObject.Position.x / CELL_X_SIZE),
          );
        ...
        if (pObject.fVisible) {
          if (pObject.fTestObject == NO_TEST_OBJECT) {
            ...
            if (pObject.ubActionCode == Enum258.THROW_ARM_ITEM) {
              ...
              AniParams.uiKeyFrame1Code =
                Enum311.ANI_KEYFRAME_CHAIN_WATER_EXPLOSION;
            ...
            pNode.pLevelNode.sRelativeZ = CONVERT_HEIGHTUNITS_TO_PIXELS(
              pObject.Position.z,
            );
          }
        }
      ...
      if (fDoCollision) {
        ...
        if (pObject.fPotentialForDebug) {
          ...
          PhysicsDebugMsg(
            FormatString(
              "Object %d: Collision Normal %f %f %f",
              pObject.iID,
              vTemp.x,
              vTemp.y,
              vTemp.z,
          ...
          PhysicsDebugMsg(
            FormatString(
              "Object %d: Collision OldPos %f %f %f",
              pObject.iID,
              pObject.Position.x,
              pObject.Position.y,
              pObject.Position.z,
          ...
          PhysicsDebugMsg(
            FormatString(
              "Object %d: Collision Velocity %f %f %f",
              pObject.iID,
              pObject.CollisionVelocity.x,
              pObject.CollisionVelocity.y,
  ...
  function PhysicsResolveCollision(
    pObject: REAL_OBJECT,
    pVelocity: vector_3,
    pNormal: vector_3,
    ...
    ImpulseNumerator =
      -1 * CoefficientOfRestitution * VDotProduct(pVelocity, pNormal);
      ...
      if (pObject.fVisible) {
        ...
        if (
          ...
          if (sNewGridNo != pObject.sGridNo) {
            ...
            pNode.uiFlags |=
              LEVELNODE_USEABSOLUTEPOS |
              LEVELNODE_IGNOREHEIGHT |
              LEVELNODE_PHYSICSOBJECT |
              LEVELNODE_DYNAMIC;
        ...
        if (pObject.pNode != null) {
          ...
          pTrav =
            hVObject.pETRLEObject[
              gTileDatabase[pObject.pNode.usIndex].usRegionIndex
            ];
          ...
          pObject.pNode.sRelativeZ = CONVERT_HEIGHTUNITS_TO_PIXELS(
            pObject.Position.z,
          );
          ...
          (<LEVELNODE>pObject.pShadow).sRelativeZ =
            gpWorldLevelData[pObject.sGridNo].sHeight;
        }
      }
    }
  ...
  function FindBestForceForTrajectory(
    sSrcGridNo: INT16,
    sGridNo: INT16,
    sStartZ: INT16,
    sEndZ: INT16,
    ...
    do {
      ...
      dTestRange = CalculateObjectTrajectory(
        sEndZ,
        pItem,
        vPosition,
        vForce,
      ...
      // How have we done?
      // < 5% off...
      if (Math.abs(dTestDiff / dRange) < 0.01) {
        break;
      }
  ...
  function FindFinalGridNoGivenDirectionGridNoForceAngle(
    sSrcGridNo: INT16,
    sGridNo: INT16,
    sStartZ: INT16,
    sEndZ: INT16,
  ...
  function FindBestAngleForTrajectory(
    sSrcGridNo: INT16,
    sGridNo: INT16,
    sStartZ: INT16,
    sEndZ: INT16,
    ...
    do {
      ...
      // How have we done?
      // < 5% off...
      if (Math.abs(dTestDiff / dRange) < 0.05) {
        break;
      }
  ...
  function CalculateObjectTrajectory(
    sTargetZ: INT16,
    pItem: OBJECTTYPE,
    vPosition: vector_3,
    vForce: vector_3,
    psFinalGridNo: Pointer<INT16>,
  ): FLOAT {
    let iID: INT32;
    let pObject: REAL_OBJECT;
    let dDiffX: FLOAT;
  ...
  function ChanceToGetThroughObjectTrajectory(
    sTargetZ: INT16,
    pItem: OBJECTTYPE,
    vPosition: vector_3,
    vForce: vector_3,
    psNewGridNo: Pointer<INT16>,
    pbLevel: Pointer<INT8>,
    fFromUI: boolean,
  ): INT32 {
    let iID: INT32;
    let pObject: REAL_OBJECT;
    ...
    if (psNewGridNo != null) {
      ...
      if (fFromUI) {
        psNewGridNo.value = MAPROWCOLTOPOS(
          Math.trunc(pObject.Position.y / CELL_Y_SIZE),
          Math.trunc(pObject.Position.x / CELL_X_SIZE),
        );
      } else {
        psNewGridNo.value = MAPROWCOLTOPOS(
          Math.trunc(pObject.EndedWithCollisionPosition.y / CELL_Y_SIZE),
          Math.trunc(pObject.EndedWithCollisionPosition.x / CELL_X_SIZE),
        );
      }

      pbLevel.value = GET_OBJECT_LEVEL(
        pObject.EndedWithCollisionPosition.z -
          CONVERT_PIXELS_TO_HEIGHTUNITS(
            gpWorldLevelData[psNewGridNo.value].sHeight,
          ),
      );
    }
  ...
  export function CalculateLaunchItemAngle(
    ...
    dAngle = FindBestAngleForTrajectory(
      pSoldier.sGridNo,
      sGridNo,
      GET_SOLDIER_THROW_HEIGHT(pSoldier.bLevel),
      ubHeight,
      dForce,
      pItem,
      psGridNo,
  ...
  function CalculateLaunchItemBasicParams(
    pSoldier: SOLDIERTYPE,
    pItem: OBJECTTYPE,
    sGridNo: INT16,
    ubLevel: UINT8,
    ...
    if (
      fArmed &&
      (usLauncher == Enum225.MORTAR || pItem.usItem == Enum225.MORTAR)
    ) {
      // Start at 0....
    ...
    if (
      fArmed &&
      (usLauncher == Enum225.GLAUNCHER ||
        usLauncher == Enum225.UNDER_GLAUNCHER ||
        pItem.usItem == Enum225.GLAUNCHER ||
        pItem.usItem == Enum225.UNDER_GLAUNCHER)
    ) {
    ...
    if (!fLauncher) {
      ...
      if (fMortar || fGLauncher) {
        // find min force
        dMinForce = CalculateForceFromRange(
          Math.trunc(sMinRange / 10),
          Math.PI / 4,
        );
  ...
  export function CalculateLaunchItemChanceToGetThrough(
    pSoldier: SOLDIERTYPE,
    pItem: OBJECTTYPE,
    sGridNo: INT16,
    ubLevel: UINT8,
    ...
    CalculateLaunchItemBasicParams(
      pSoldier,
      pItem,
      sGridNo,
      ubLevel,
    ...
    // OK, we have our force, calculate change to get through without collide
    if (
      ChanceToGetThroughObjectTrajectory(
        sEndZ,
        pItem,
        vPosition,
        vForce,
  ...
  function CalculateSoldierMaxForce(
    pSoldier: SOLDIERTYPE,
    dDegrees: FLOAT,
    pItem: OBJECTTYPE,
    fArmed: boolean,
  ...
  export function CalculateLaunchItemParamsForThrow(
    pSoldier: SOLDIERTYPE,
    sGridNo: INT16,
    ubLevel: UINT8,
    sEndZ: INT16,
  ...
  function CheckForObjectHittingMerc(
    pObject: REAL_OBJECT,
    usStructureID: UINT16,
  ): void {
    let pSoldier: SOLDIERTYPE;
      ...
      if (usStructureID < INVALID_STRUCTURE_ID) {
        if (pObject.ubLastTargetTakenDamage != usStructureID) {
          ...
          EVENT_SoldierGotHit(
            ...
            0,
            pObject.ubOwner,
            FIRE_WEAPON_TOSSED_OBJECT_SPECIAL,
            0,
            0,
            NOWHERE,
          );
    ...
    if (pObject.fTestObject == NO_TEST_OBJECT) {
      if (pObject.ubActionCode == Enum258.THROW_TARGET_MERC_CATCH) {
        ...
        if (uiSpacesAway < 4 && !pObject.fAttemptedCatch) {
          if (
            pSoldier.usAnimState != Enum193.CATCH_STANDING &&
            pSoldier.usAnimState != Enum193.CATCH_CROUCHED &&
            pSoldier.usAnimState != Enum193.LOWER_RIFLE
          ) {
            if (gAnimControl[pSoldier.usAnimState].ubHeight == ANIM_STAND) {
              EVENT_InitNewSoldierAnim(
                pSoldier,
                Enum193.CATCH_STANDING,
                0,
                false,
              );
            } else if (
  ...
  function DoCatchObject(pObject: REAL_OBJECT): boolean {
    ...
    if (fGoodCatch) {
      ...
      ScreenMsg(
        ...
        MSG_INTERFACE,
        pMessageStrings[Enum333.MSG_MERC_CAUGHT_ITEM],
        pSoldier.name,
  ...
  function HandleArmedObjectImpact(pObject: REAL_OBJECT): void {
    ...
    sZ =
      CONVERT_HEIGHTUNITS_TO_PIXELS(pObject.Position.z) -
      gpWorldLevelData[pObject.sGridNo].sHeight;
    ...
    if (fCheckForDuds) {
      ...
      if (
        ...
        (pObj.bStatus[0] >= USABLE &&
          PreRandom(100) < pObj.bStatus[0] + PreRandom(50))
      ) {
        fDoImpact = true;
      } // didn't go off!
      else {
  ...
  function RandomGridFromRadius(
    ...
    do {
      ...
      if (
        (sX < ubMinRadius || sY < ubMinRadius) &&
        ubMaxRadius != ubMinRadius
      ) {
        continue;
      }
