...
*/
  ...
  export function FreeStructureFile(
    pStructureFile: STRUCTURE_FILE_REF,
  ): boolean {
    if (!pStructureFile) {
      return false;
    }
  ...
  function LoadStructureData(
    szFileName: string /* STR */,
    pFileRef: STRUCTURE_FILE_REF,
  ): UINT32 {
    ...
    fOk =
      (uiBytesRead = FileRead(hInput, buffer, STRUCTURE_FILE_HEADER_SIZE)) !==
      -1;
    ...
    if (Header.fFlags & STRUCTURE_FILE_CONTAINS_AUXIMAGEDATA) {
      ...
      pFileRef.pAuxData = createArrayFrom(
        Header.usNumberOfImages,
        createAuxObjectData,
      );
      ...
      if (Header.usNumberOfImageTileLocsStored > 0) {
        uiDataSize =
          RELATIVE_TILE_LOCATION_SIZE * Header.usNumberOfImageTileLocsStored;
        pFileRef.pTileLocData = createArrayFrom(
          Header.usNumberOfImageTileLocsStored,
          createRelativeTileLocation,
        );
        ...
        readObjectArray(
          pFileRef.pTileLocData,
          buffer,
          0,
          readRelativeTileLocation,
        );
      }
    }
  ...
  function CreateFileStructureArrays(
    pFileRef: STRUCTURE_FILE_REF,
    uiDataSize: UINT32,
  ): boolean {
    let pCurrent: number;
    ...
    pDBStructureRef = wrapDbStructureRefs(
      createArrayFrom(pFileRef.usNumberOfStructures, createDbStructureRef),
    );
    ...
    for (usLoop = 0; usLoop < pFileRef.usNumberOfStructuresStored; usLoop++) {
      ...
      ppTileArray = createArrayFrom(
        pDBStructureRef[usIndex].pDBStructure.ubNumberOfTiles,
        createDbStructureTile,
      );
      ...
      for (
        usTileLoop = 0;
        usTileLoop < pDBStructureRef[usIndex].pDBStructure.ubNumberOfTiles;
        usTileLoop++
      ) {
        if (pCurrent + DB_STRUCTURE_SIZE > uiDataSize) {
          // gone past end of file block?!
        ...
        ppTileArray[usTileLoop].sPosRelToBase =
          ppTileArray[usTileLoop].bXPosRelToBase +
  ...
  export function LoadStructureFile(
    szFileName: string /* STR */,
  ): STRUCTURE_FILE_REF | null {
  ...
  function CreateStructureFromDB(
  ...
  function OkayToAddStructureToTile(
    sBaseGridNo: INT16,
    sCubeOffset: INT16,
    pDBStructureRef: DB_STRUCTURE_REF,
    ubTileIndex: UINT8,
    sExclusionID: INT16,
    fIgnorePeople: boolean,
  ): boolean {
    ...
    if (
      gpWorldLevelData[sBaseGridNo].sHeight != gpWorldLevelData[sGridNo].sHeight
    ) {
  ...
  */
    ...
    while (pExistingStructure != null) {
      if (sCubeOffset == pExistingStructure.sCubeOffset) {
        ...
        if (pDBStructure.fFlags & STRUCTURE_OBSTACLE) {
          ...
          if (
            pExistingStructure.fFlags & STRUCTURE_OBSTACLE &&
            !(pDBStructure.fFlags & STRUCTURE_MOBILE)
          ) {
            if (
              pExistingStructure.fFlags & STRUCTURE_PASSABLE &&
            ...
            if (
              !(
                pExistingStructure.fFlags & STRUCTURE_ANYDOOR &&
              ...
              for (bLoop = 1; bLoop < 4; bLoop++) {
                switch (pExistingStructure.ubWallOrientation) {
                  ...
                  default:
                    // @%?@#%?@%
                    sOtherGridNo = NewGridNo(
                      sGridNo,
                      DirectionInc(Enum245.SOUTHEAST),
                    );
                }
                for (
                  bLoop2 = 0;
                  bLoop2 < pDBStructure.ubNumberOfTiles;
                  bLoop2++
                ) {
                  if (
                    sBaseGridNo + ppTile[bLoop2].sPosRelToBase ==
                    sOtherGridNo
                  ) {
          ...
          if (
            pExistingStructure.fFlags & STRUCTURE_WALLSTUFF &&
            pDBStructure.ubWallOrientation ==
              pExistingStructure.ubWallOrientation
          ) {
            return false;
          } else if (
            !(pExistingStructure.fFlags & (STRUCTURE_CORPSE | STRUCTURE_PERSON))
          ) {
            ...
            for (bLoop = 1; bLoop < 4; bLoop++) {
              ...
              for (
                ubTileIndex = 0;
                ubTileIndex < pDBStructure.ubNumberOfTiles;
                ubTileIndex++
              ) {
                pOtherExistingStructure = FindStructureByID(
                  sOtherGridNo,
  ...
  export function InternalOkayToAddStructureToWorld(
    sBaseGridNo: INT16,
    bLevel: INT8,
    pDBStructureRef: DB_STRUCTURE_REF,
    sExclusionID: INT16,
  ...
  */
    ...
    for (
      ubLoop = 0;
      ubLoop < pDBStructureRef.pDBStructure.ubNumberOfTiles;
      ubLoop++
    ) {
  ...
  export function OkayToAddStructureToWorld(
    sBaseGridNo: INT16,
    bLevel: INT8,
    pDBStructureRef: DB_STRUCTURE_REF,
    sExclusionID: INT16,
  ...
  function AddStructureToTile(
    pMapElement: MAP_ELEMENT | null,
  ...
  function InternalAddStructureToWorld(
    sBaseGridNo: INT16,
    bLevel: INT8,
    pDBStructureRef: DB_STRUCTURE_REF | null,
    pLevelNode: LEVELNODE | null,
  ): STRUCTURE | null {
    ...
    ppStructure = createArrayFrom(
      pDBStructure.ubNumberOfTiles,
      createStructure,
    );

    for (ubLoop = BASE_TILE; ubLoop < pDBStructure.ubNumberOfTiles; ubLoop++) {
      // for each tile, create the appropriate STRUCTURE struct
      ppStructure[ubLoop] = <STRUCTURE>(
        CreateStructureFromDB(pDBStructureRef, ubLoop)
      );
      ...
      if (pLevelNode.uiFlags & LEVELNODE_SOLDIER) {
        ...
        pLevelNode.uiFlags & LEVELNODE_ROTTINGCORPSE ||
      ...
      if (ubLoop == BASE_TILE) {
        ...
        if (gpWorldLevelData[sGridNo].sHeight != sBaseTileHeight) {
          ...
          for (ubLoop2 = BASE_TILE; ubLoop2 < ubLoop; ubLoop2++) {
            DeleteStructureFromTile(
              gpWorldLevelData[ppStructure[ubLoop2].sGridNo],
              ppStructure[ubLoop2],
            );
          }
          return null;
  ...
  export function AddStructureToWorld(
    sBaseGridNo: INT16,
    bLevel: INT8,
    pDBStructureRef: DB_STRUCTURE_REF | null,
    pLevelN: LEVELNODE | null,
  ): boolean {
    let pStructure: STRUCTURE | null;

    pStructure = InternalAddStructureToWorld(
      sBaseGridNo,
      bLevel,
      pDBStructureRef,
      pLevelN,
  ...
  function DeleteStructureFromTile(
    pMapElement: MAP_ELEMENT,
  ...
  export function DeleteStructureFromWorld(
    pStructure: STRUCTURE | null,
  ): boolean {
    ...
    fRecompileMPs =
      ...
      !((pBaseStructure.fFlags & STRUCTURE_MOBILE) != 0);
    if (fRecompileMPs) {
      fRecompileExtraRadius =
        (pBaseStructure.fFlags & STRUCTURE_WALLSTUFF) != 0;
    } else {
  ...
  function InternalSwapStructureForPartner(
    sGridNo: INT16,
    pStructure: STRUCTURE | null,
    fFlipSwitches: boolean,
    fStoreInMap: boolean,
  ): STRUCTURE | null {
    // switch structure
    ...
    if (
      pBaseStructure.pDBStructureRef.pDBStructure.bPartnerDelta ==
      NO_PARTNER_STRUCTURE
    ) {
      return null;
    }
    fDoor = (pBaseStructure.fFlags & STRUCTURE_ANYDOOR) > 0;
    pLevelNode = FindLevelNodeBasedOnStructure(
      pBaseStructure.sGridNo,
      pBaseStructure,
    );
    if (pLevelNode == null) {
      return null;
    ...
    pPartnerDBStructure = (
      pBaseStructure.pDBStructureRef as _DB_STRUCTURE_REF_WRAPPER
    ).getPartner(bDelta);
    ...
    pNewBaseStructure = InternalAddStructureToWorld(
      sGridNo,
      Math.trunc(sCubeOffset / PROFILE_Z_SIZE),
      pPartnerDBStructure,
      pLevelNode,
    );
  ...
  export function SwapStructureForPartner(
    sGridNo: INT16,
    pStructure: STRUCTURE | null,
  ): STRUCTURE | null {
  ...
  export function SwapStructureForPartnerWithoutTriggeringSwitches(
    sGridNo: INT16,
    pStructure: STRUCTURE | null,
  ): STRUCTURE | null {
  ...
  export function SwapStructureForPartnerAndStoreChangeInMap(
    sGridNo: INT16,
    pStructure: STRUCTURE | null,
  ): STRUCTURE | null {
  ...
  export function FindNextStructure(
    pStructure: STRUCTURE,
    fFlags: UINT32,
  ): STRUCTURE | null {
    let pCurrent: STRUCTURE | null;
  ...
  export function FindBaseStructure(
    pStructure: STRUCTURE | null,
  ): STRUCTURE | null {
  ...
  export function GetTallestStructureHeight(
    sGridNo: INT16,
    fOnRoof: boolean,
  ): INT8 {
    let pCurrent: STRUCTURE | null;
  ...
  export function GetStructureTargetHeight(
    sGridNo: INT16,
    fOnRoof: boolean,
  ): INT8 {
    let pCurrent: STRUCTURE | null;
  ...
  export function StructureDensity(
    pStructure: STRUCTURE | null,
    pubLevel0: Pointer<UINT8>,
    pubLevel1: Pointer<UINT8>,
    pubLevel2: Pointer<UINT8>,
    pubLevel3: Pointer<UINT8>,
  ): boolean {
    let ubLoopX: UINT8;
    let ubLoopY: UINT8;
    let ubShapeValue: UINT8;
  ...
  export function DamageStructure(
    ...
    if (
      pStructure.pDBStructureRef.pDBStructure.ubArmour ==
        Enum309.MATERIAL_INDESTRUCTABLE_METAL ||
      pStructure.pDBStructureRef.pDBStructure.ubArmour ==
        Enum309.MATERIAL_INDESTRUCTABLE_STONE
    ) {
      return 0;
    }
  ...
  /* static */ let DebugStructurePage1__WallOrientationString: string[] /* CHAR16[5][15] */ =
    ["None", "Inside left", "Inside right", "Outside left", "Outside right"];
  ...
  */
    ...
    if (pStructure != null) {
      if (pStructure.fFlags & STRUCTURE_GENERIC) {
        ...
        gprintf(
          0,
          LINE_HEIGHT * 1,
          "Wall with orientation %s",
          DebugStructurePage1__WallOrientationString[
        ...
        gprintf(
          0,
          LINE_HEIGHT * 1,
          "%s sliding door with orientation %s",
          pStructure.fFlags & STRUCTURE_OPEN ? "Open" : "Closed",
          DebugStructurePage1__WallOrientationString[
            pStructure.ubWallOrientation
          ],
        );
      ...
      gprintf(
        0,
        LINE_HEIGHT * 2,
        "Structure height %d, cube offset %d, armour %d, HP %d",
        bHeight,
      ...
      if (
        StructureDensity(
          pStructure,
          createElementPointer(bDens, 0),
          createElementPointer(bDens, 1),
          createElementPointer(bDens, 2),
          createElementPointer(bDens, 3),
        ) == true
      ) {
        gprintf(
          0,
          LINE_HEIGHT * 3,
          "Structure fill %d%%/%d%%/%d%%/%d%% density %d",
          bDens[0],
          bDens[1],
          bDens[2],
          bDens[3],
    ...
    gprintf(
      ...
      LINE_HEIGHT * 13,
      "N %d NE %d E %d SE %d",
      gubWorldMovementCosts[sGridNo][Enum245.NORTH][gsInterfaceLevel],
      gubWorldMovementCosts[sGridNo][Enum245.NORTHEAST][gsInterfaceLevel],
      gubWorldMovementCosts[sGridNo][Enum245.EAST][gsInterfaceLevel],
      gubWorldMovementCosts[sGridNo][Enum245.SOUTHEAST][gsInterfaceLevel],
    );
    gprintf(
      0,
      LINE_HEIGHT * 14,
      "S %d SW %d W %d NW %d",
      gubWorldMovementCosts[sGridNo][Enum245.SOUTH][gsInterfaceLevel],
      gubWorldMovementCosts[sGridNo][Enum245.SOUTHWEST][gsInterfaceLevel],
      gubWorldMovementCosts[sGridNo][Enum245.WEST][gsInterfaceLevel],
      gubWorldMovementCosts[sGridNo][Enum245.NORTHWEST][gsInterfaceLevel],
    );
    gprintf(
      0,
      LINE_HEIGHT * 15,
      "Ground smell %d strength %d",
    ...
    gprintf(
      ...
      LINE_HEIGHT * 16,
      "Adj soldiers %d",
      gpWorldLevelData[sGridNo].ubAdjacentSoldierCnt,
    );
  }

  export function AddZStripInfoToVObject(
    hVObject: SGPVObject,
    pStructureFileRef: STRUCTURE_FILE_REF,
    fFromAnimation: boolean,
    sSTIStartIndex: INT16,
  ): boolean {
    let uiLoop: UINT32;
    let ubLoop2: UINT8;
    ...
    for (
      uiLoop = 0;
      uiLoop < pStructureFileRef.usNumberOfStructures;
      uiLoop++
    ) {
    ...
    hVObject.ppZStripInfo = createArray(
      hVObject.usNumberOfObjects,
      <ZStripInfo>(<unknown>null),
    );
    ...
    for (
      uiLoop = sSTIStartIndex;
      uiLoop < hVObject.usNumberOfObjects;
      uiLoop++
    ) {
      // Defualt to true
      fCopyIntoVo = true;
      ...
      if (
        fCopyIntoVo &&
        sStructIndex < pStructureFileRef.usNumberOfStructures
      ) {
        pDBStructure =
        ...
        if (
          ...
          if (
            ...
            if (pDBStructure.fFlags & (STRUCTURE_MOBILE | STRUCTURE_CORPSE)) {
              ...
              sOffsetX =
                sOffsetX -
                pDBStructure.bZTileOffsetX * Math.trunc(WORLD_TILE_X / 2) +
                pDBStructure.bZTileOffsetY * Math.trunc(WORLD_TILE_X / 2);
              sOffsetY =
                sOffsetY -
                pDBStructure.bZTileOffsetY * Math.trunc(WORLD_TILE_Y / 2);
            }
            ...
            if (sOffsetX <= 0) {
              ...
              sRightHalfWidth =
                usWidth + sOffsetX - Math.trunc(WORLD_TILE_X / 2);
              ...
              if (sRightHalfWidth >= 0) {
                ...
                sLeftHalfWidth =
                  usWidth - (sRightHalfWidth % Math.trunc(WORLD_TILE_X / 2));
                sRightHalfWidth = 0;
              }
            ...
            if (sRightHalfWidth > 0) {
              ...
              if (sRightHalfWidth > Math.trunc(WORLD_TILE_X / 2)) {
                ubNumDecreasing = Math.trunc(
                  sRightHalfWidth / Math.trunc(WORLD_TILE_X / 2),
                );
              }
            }
            if (sLeftHalfWidth > 0) {
            ...
            } // right side only; offset is at least 20 (= WORLD_TILE_X / 2)
            else {
              if (sOffsetX > WORLD_TILE_X) {
                pCurr.ubFirstZStripWidth =
                  Math.trunc(WORLD_TILE_X / 2) -
                  ((sOffsetX - WORLD_TILE_X) % Math.trunc(WORLD_TILE_X / 2));
              } else {
            ...
            pCurr.ubNumberOfZChanges =
              ubNumIncreasing + ubNumStable + ubNumDecreasing;
  ...
  export function GetBlockingStructureInfo(
    sGridNo: INT16,
    bDir: INT8,
    bNextDir: INT8,
    bLevel: INT8,
    pStructHeight: Pointer<INT8>,
    ppTallestStructure: Pointer<STRUCTURE | null>,
    fWallsBlock: boolean,
  ): INT8 {
    let pCurrent: STRUCTURE | null;
  ...
  export function FindStructureBySavedInfo(
    sGridNo: INT16,
    ubType: UINT8,
  ...
  export function GetStructureOpenSound(
    pStructure: STRUCTURE,
    fClose: boolean,
  ): UINT32 {
  ...
  class _DB_STRUCTURE_REF_WRAPPER implements DB_STRUCTURE_REF {
    constructor(
      public readonly array: DB_STRUCTURE_REF[],
