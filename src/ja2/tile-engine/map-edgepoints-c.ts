namespace ja2 {
  ...
  export let gps1stNorthEdgepointArray: INT16[] /* Pointer<INT16> */ = <
    INT16[]
  >(<unknown>null);
  export let gps1stEastEdgepointArray: INT16[] /* Pointer<INT16> */ = <INT16[]>(
    (<unknown>null)
  );
  export let gps1stSouthEdgepointArray: INT16[] /* Pointer<INT16> */ = <
    INT16[]
  >(<unknown>null);
  export let gps1stWestEdgepointArray: INT16[] /* Pointer<INT16> */ = <INT16[]>(
    (<unknown>null)
  );
  ...
  // dynamic arrays that contain the valid gridno's for each edge
  let gps2ndNorthEdgepointArray: INT16[] /* Pointer<INT16> */ = <INT16[]>(
    (<unknown>null)
  );
  let gps2ndEastEdgepointArray: INT16[] /* Pointer<INT16> */ = <INT16[]>(
    (<unknown>null)
  );
  let gps2ndSouthEdgepointArray: INT16[] /* Pointer<INT16> */ = <INT16[]>(
    (<unknown>null)
  );
  let gps2ndWestEdgepointArray: INT16[] /* Pointer<INT16> */ = <INT16[]>(
    (<unknown>null)
  );
  // contains the size for each array
  ...
  function ValidateEdgepoints(): void {
    ...
    for (i = 0; i < gus1stNorthEdgepointArraySize; i++) {
      if (VerifyEdgepoint(Soldier, gps1stNorthEdgepointArray[i])) {
        gps1stNorthEdgepointArray[usValidEdgepoints] =
          gps1stNorthEdgepointArray[i];
    ...
    for (i = 0; i < gus1stEastEdgepointArraySize; i++) {
      if (VerifyEdgepoint(Soldier, gps1stEastEdgepointArray[i])) {
        gps1stEastEdgepointArray[usValidEdgepoints] =
          gps1stEastEdgepointArray[i];
    ...
    for (i = 0; i < gus1stSouthEdgepointArraySize; i++) {
      if (VerifyEdgepoint(Soldier, gps1stSouthEdgepointArray[i])) {
        gps1stSouthEdgepointArray[usValidEdgepoints] =
          gps1stSouthEdgepointArray[i];
    ...
    for (i = 0; i < gus1stWestEdgepointArraySize; i++) {
      if (VerifyEdgepoint(Soldier, gps1stWestEdgepointArray[i])) {
        gps1stWestEdgepointArray[usValidEdgepoints] =
          gps1stWestEdgepointArray[i];
    ...
    for (i = 0; i < gus2ndNorthEdgepointArraySize; i++) {
      if (VerifyEdgepoint(Soldier, gps2ndNorthEdgepointArray[i])) {
        gps2ndNorthEdgepointArray[usValidEdgepoints] =
          gps2ndNorthEdgepointArray[i];
    ...
    for (i = 0; i < gus2ndEastEdgepointArraySize; i++) {
      if (VerifyEdgepoint(Soldier, gps2ndEastEdgepointArray[i])) {
        gps2ndEastEdgepointArray[usValidEdgepoints] =
          gps2ndEastEdgepointArray[i];
    ...
    for (i = 0; i < gus2ndSouthEdgepointArraySize; i++) {
      if (VerifyEdgepoint(Soldier, gps2ndSouthEdgepointArray[i])) {
        gps2ndSouthEdgepointArray[usValidEdgepoints] =
          gps2ndSouthEdgepointArray[i];
    ...
    for (i = 0; i < gus2ndWestEdgepointArraySize; i++) {
      if (VerifyEdgepoint(Soldier, gps2ndWestEdgepointArray[i])) {
        gps2ndWestEdgepointArray[usValidEdgepoints] =
          gps2ndWestEdgepointArray[i];
  ...
  function CompactEdgepointArray(
    psArray: Pointer<INT16[]>,
    pusMiddleIndex: Pointer<UINT16>,
    pusArraySize: Pointer<UINT16>,
  ): void {
    let i: INT32;
  ...
  function InternallyClassifyEdgepoints(
    pSoldier: SOLDIERTYPE,
    sGridNo: INT16,
    psArray1: Pointer<INT16[]>,
    pusMiddleIndex1: Pointer<UINT16>,
    pusArraySize1: Pointer<UINT16>,
    psArray2: Pointer<INT16[]>,
    pusMiddleIndex2: Pointer<UINT16>,
    pusArraySize2: Pointer<UINT16>,
  ): void {
    let i: INT32;
        ...
        if (i < pusMiddleIndex1.value) {
          ...
          for (i = pusMiddleIndex1.value; i < pusArraySize1.value; i++) {
            if (
              EdgepointsClose(
                pSoldier,
                psArray1.value[us1stBenchmarkID],
                psArray1.value[i],
              )
            ) {
              us2ndBenchmarkID = i;
          ...
          for (i = 0; i < pusMiddleIndex1.value; i++) {
            if (
              EdgepointsClose(
                pSoldier,
                psArray1.value[us2ndBenchmarkID],
                psArray1.value[i],
              )
            ) {
              us1stBenchmarkID = i;
    ...
    if (us1stBenchmarkID != 0xffff) {
      for (i = us1stBenchmarkID; i > 0; i--) {
        if (
          !EdgepointsClose(pSoldier, psArray1.value[i], psArray1.value[i - 1])
        ) {
  ...
  function ClassifyEdgepoints(): void {
    ...
    if (gMapInformation.sNorthGridNo != -1) {
      sGridNo = FindNearestEdgepointOnSpecifiedEdge(
        gMapInformation.sNorthGridNo,
        Enum291.NORTH_EDGEPOINT_SEARCH,
      );
      if (sGridNo != NOWHERE) {
        InternallyClassifyEdgepoints(
          ...
          createPointer(
            ...
            (v) => (gps1stNorthEdgepointArray = v),
          ),
          createPointer(
            () => gus1stNorthEdgepointMiddleIndex,
            (v) => (gus1stNorthEdgepointMiddleIndex = v),
          ),
          createPointer(
            () => gus1stNorthEdgepointArraySize,
            (v) => (gus1stNorthEdgepointArraySize = v),
          ),
          createPointer(
            () => gps2ndNorthEdgepointArray,
            (v) => (gps2ndNorthEdgepointArray = v),
          ),
          createPointer(
            () => gus2ndNorthEdgepointMiddleIndex,
            (v) => (gus2ndNorthEdgepointMiddleIndex = v),
          ),
          createPointer(
      ...
      if (sGridNo != NOWHERE) {
        InternallyClassifyEdgepoints(
          ...
          createPointer(
            ...
            (v) => (gps1stEastEdgepointArray = v),
          ),
          createPointer(
            () => gus1stEastEdgepointMiddleIndex,
            (v) => (gus1stEastEdgepointMiddleIndex = v),
          ),
          createPointer(
            () => gus1stEastEdgepointArraySize,
            (v) => (gus1stEastEdgepointArraySize = v),
          ),
          createPointer(
            () => gps2ndEastEdgepointArray,
            (v) => (gps2ndEastEdgepointArray = v),
          ),
          createPointer(
            () => gus2ndEastEdgepointMiddleIndex,
            (v) => (gus2ndEastEdgepointMiddleIndex = v),
          ),
          createPointer(
    ...
    if (gMapInformation.sSouthGridNo != -1) {
      sGridNo = FindNearestEdgepointOnSpecifiedEdge(
        ...
        Enum291.SOUTH_EDGEPOINT_SEARCH,
      );
      if (sGridNo != NOWHERE) {
        InternallyClassifyEdgepoints(
          Soldier,
          ...
          createPointer(
            ...
            (v) => (gps1stSouthEdgepointArray = v),
          ),
          createPointer(
            () => gus1stSouthEdgepointMiddleIndex,
            (v) => (gus1stSouthEdgepointMiddleIndex = v),
          ),
          createPointer(
            () => gus1stSouthEdgepointArraySize,
            (v) => (gus1stSouthEdgepointArraySize = v),
          ),
          createPointer(
            () => gps2ndSouthEdgepointArray,
            (v) => (gps2ndSouthEdgepointArray = v),
          ),
          createPointer(
            () => gus2ndSouthEdgepointMiddleIndex,
            (v) => (gus2ndSouthEdgepointMiddleIndex = v),
          ),
          createPointer(
            () => gus2ndSouthEdgepointArraySize,
            (v) => (gus2ndSouthEdgepointArraySize = v),
          ),
        );
      }
    }
      ...
      if (sGridNo != NOWHERE) {
        InternallyClassifyEdgepoints(
          ...
          createPointer(
            ...
            (v) => (gps1stWestEdgepointArray = v),
          ),
          createPointer(
            () => gus1stWestEdgepointMiddleIndex,
            (v) => (gus1stWestEdgepointMiddleIndex = v),
          ),
          createPointer(
            () => gus1stWestEdgepointArraySize,
            (v) => (gus1stWestEdgepointArraySize = v),
          ),
          createPointer(
            () => gps2ndWestEdgepointArray,
            (v) => (gps2ndWestEdgepointArray = v),
          ),
          createPointer(
            () => gus2ndWestEdgepointMiddleIndex,
            (v) => (gus2ndWestEdgepointMiddleIndex = v),
          ),
          createPointer(
            () => gus2ndWestEdgepointArraySize,
            (v) => (gus2ndWestEdgepointArraySize = v),
          ),
        );
      }
    }
  ...
  export function GenerateMapEdgepoints(): void {
    ...
    if (gMapInformation.sNorthGridNo != -1) {
      ...
      if (gus1stNorthEdgepointArraySize) {
        ...
        gps1stNorthEdgepointArray = createArray(
          gus1stNorthEdgepointArraySize,
          0,
        );
        for (i = 0; i < gus1stNorthEdgepointArraySize; i++)
    ...
    if (gMapInformation.sSouthGridNo != -1) {
      ...
      if (gus1stSouthEdgepointArraySize) {
        // Allocate and copy over the valid gridnos.
        gps1stSouthEdgepointArray = createArray(
          gus1stSouthEdgepointArraySize,
          0,
        );
        for (i = 0; i < gus1stSouthEdgepointArraySize; i++)
    ...
    if (
      ...
      !(
        gpWorldLevelData[gMapInformation.sIsolatedGridNo].uiFlags &
      ...
      if (gMapInformation.sNorthGridNo != -1) {
        ...
        if (gus2ndNorthEdgepointArraySize) {
          // Allocate and copy over the valid gridnos.
          gps2ndNorthEdgepointArray = createArray(
            gus2ndNorthEdgepointArraySize,
            0,
          );
          for (i = 0; i < gus2ndNorthEdgepointArraySize; i++)
      ...
      if (gMapInformation.sEastGridNo != -1) {
        ...
        if (gus2ndEastEdgepointArraySize) {
          // Allocate and copy over the valid gridnos.
          gps2ndEastEdgepointArray = createArray(
            gus2ndEastEdgepointArraySize,
            0,
          );
          for (i = 0; i < gus2ndEastEdgepointArraySize; i++)
      ...
      if (gMapInformation.sSouthGridNo != -1) {
        ...
        if (gus2ndSouthEdgepointArraySize) {
          // Allocate and copy over the valid gridnos.
          gps2ndSouthEdgepointArray = createArray(
            gus2ndSouthEdgepointArraySize,
            0,
          );
          for (i = 0; i < gus2ndSouthEdgepointArraySize; i++)
      ...
      if (gMapInformation.sWestGridNo != -1) {
        ...
        if (gus2ndWestEdgepointArraySize) {
          // Allocate and copy over the valid gridnos.
          gps2ndWestEdgepointArray = createArray(
            gus2ndWestEdgepointArraySize,
            0,
          );
          for (i = 0; i < gus2ndWestEdgepointArraySize; i++)
  ...
  function OldLoadMapEdgepoints(buffer: Buffer, offset: number): number {
    gus1stNorthEdgepointArraySize = buffer.readUInt16LE(offset);
    offset += 2;
    gus1stNorthEdgepointMiddleIndex = buffer.readUInt16LE(offset);
    offset += 2;
    if (gus1stNorthEdgepointArraySize) {
    ...
    gus1stEastEdgepointArraySize = buffer.readUInt16LE(offset);
    offset += 2;
    gus1stEastEdgepointMiddleIndex = buffer.readUInt16LE(offset);
    offset += 2;
    if (gus1stEastEdgepointArraySize) {
      gps1stEastEdgepointArray = createArray(gus1stEastEdgepointArraySize, 0);
      offset = readIntArray(gps1stEastEdgepointArray, buffer, offset, 2);
    }
    gus1stSouthEdgepointArraySize = buffer.readUInt16LE(offset);
    offset += 2;
    gus1stSouthEdgepointMiddleIndex = buffer.readUInt16LE(offset);
    offset += 2;
    if (gus1stSouthEdgepointArraySize) {
    ...
    gus1stWestEdgepointArraySize = buffer.readUInt16LE(offset);
    offset += 2;
    gus1stWestEdgepointMiddleIndex = buffer.readUInt16LE(offset);
    offset += 2;
    if (gus1stWestEdgepointArraySize) {
      gps1stWestEdgepointArray = createArray(gus1stWestEdgepointArraySize, 0);
  ...
  export function LoadMapEdgepoints(
    buffer: Buffer,
    offset: number,
  ): { result: boolean; offset: number } {
    TrashMapEdgepoints();
    ...
    gus2ndNorthEdgepointArraySize = buffer.readUInt16LE(offset);
    offset += 2;
    gus2ndNorthEdgepointMiddleIndex = buffer.readUInt16LE(offset);
    offset += 2;
    if (gus2ndNorthEdgepointArraySize) {
    ...
    gus2ndEastEdgepointArraySize = buffer.readUInt16LE(offset);
    offset += 2;
    gus2ndEastEdgepointMiddleIndex = buffer.readUInt16LE(offset);
    offset += 2;
    if (gus2ndEastEdgepointArraySize) {
    ...
    gus2ndSouthEdgepointArraySize = buffer.readUInt16LE(offset);
    offset += 2;
    gus2ndSouthEdgepointMiddleIndex = buffer.readUInt16LE(offset);
    offset += 2;
    if (gus2ndSouthEdgepointArraySize) {
    ...
    gus2ndWestEdgepointArraySize = buffer.readUInt16LE(offset);
    offset += 2;
    gus2ndWestEdgepointMiddleIndex = buffer.readUInt16LE(offset);
    offset += 2;
    if (gus2ndWestEdgepointArraySize) {
  ...
  export function ChooseMapEdgepoint(ubStrategicInsertionCode: UINT8): UINT16 {
    ...
    switch (ubStrategicInsertionCode) {
      ...
      default:
        AssertMsg(
          false,
          "ChooseMapEdgepoints:  Failed to pass a valid strategic insertion code.",
        );
        break;
    }
    if (!usArraySize) {
  ...
  export function ChooseMapEdgepoints(
    pMapEdgepointInfo: MAPEDGEPOINTINFO,
    ubStrategicInsertionCode: UINT8,
    ubNumDesiredPoints: UINT8,
  ): void {
    let psArray: INT16[];
    let usArraySize: UINT16 = 0;
    let i: INT32 = -1;
    ...
    AssertMsg(
      ...
      FormatString(
        "ChooseMapEdgepoints:  Desired points = %d, valid range is 1-32",
        ubNumDesiredPoints,
      ),
    );
    // First validate and get access to the correct array based on strategic direction.
    ...
    for (i = 0; i < usArraySize; i++) {
      if (
        GetTerrainType(psArray[i]) == Enum315.MED_WATER ||
        GetTerrainType(psArray[i]) == Enum315.DEEP_WATER
      ) {
        if (i == usArraySize - 1) {
  ...
  export function SearchForClosestPrimaryMapEdgepoint(
    sGridNo: INT16,
    ...
    if (gsReservedIndex >= 20) {
      ...
      AssertMsg(
        0,
        "All closest map edgepoints have been reserved.  We should only have 20 soldiers maximum...",
      );
    }
      ...
      case Enum175.INSERTION_CODE_NORTH:
        ...
        if (!usArraySize)
          AssertMsg(
            false,
            FormatString(
              "Sector %s%d level %d doesn't have any north mapedgepoints. LC:1",
      ...
      case Enum175.INSERTION_CODE_EAST:
        ...
        if (!usArraySize)
          AssertMsg(
            false,
            FormatString(
              "Sector %s%d level %d doesn't have any east mapedgepoints. LC:1",
              String.fromCharCode(gWorldSectorY + "A".charCodeAt(0) - 1),
              gWorldSectorX,
              gbWorldSectorZ,
            ),
      ...
      case Enum175.INSERTION_CODE_SOUTH:
        ...
        if (!usArraySize)
          AssertMsg(
            false,
            FormatString(
              "Sector %s%d level %d doesn't have any south mapedgepoints. LC:1",
              String.fromCharCode(gWorldSectorY + "A".charCodeAt(0) - 1),
              gWorldSectorX,
              gbWorldSectorZ,
            ),
      ...
      case Enum175.INSERTION_CODE_WEST:
        ...
        if (!usArraySize)
          AssertMsg(
            false,
            FormatString(
              "Sector %s%d level %d doesn't have any west mapedgepoints. LC:1",
              String.fromCharCode(gWorldSectorY + "A".charCodeAt(0) - 1),
              gWorldSectorX,
              gbWorldSectorZ,
            ),
  ...
  export function SearchForClosestSecondaryMapEdgepoint(
    sGridNo: INT16,
    ubInsertionCode: UINT8,
  ): INT16 {
    let i: INT32;
      ...
      case Enum175.INSERTION_CODE_NORTH:
        ...
        if (!usArraySize)
          AssertMsg(
            false,
            FormatString(
              "Sector %s%d level %d doesn't have any isolated north mapedgepoints. KM:1",
              String.fromCharCode(gWorldSectorY + "A".charCodeAt(0) - 1),
              gWorldSectorX,
              gbWorldSectorZ,
            ),
      ...
      case Enum175.INSERTION_CODE_EAST:
        ...
        if (!usArraySize)
          AssertMsg(
            false,
            FormatString(
              "Sector %s%d level %d doesn't have any isolated east mapedgepoints. KM:1",
              String.fromCharCode(gWorldSectorY + "A".charCodeAt(0) - 1),
              gWorldSectorX,
              gbWorldSectorZ,
            ),
      ...
      case Enum175.INSERTION_CODE_SOUTH:
        ...
        if (!usArraySize)
          AssertMsg(
            false,
            FormatString(
              "Sector %s%d level %d doesn't have any isolated south mapedgepoints. KM:1",
              String.fromCharCode(gWorldSectorY + "A".charCodeAt(0) - 1),
              gWorldSectorX,
              gbWorldSectorZ,
            ),
      ...
      case Enum175.INSERTION_CODE_WEST:
        ...
        if (!usArraySize)
          AssertMsg(
            false,
            FormatString(
              "Sector %s%d level %d doesn't have any isolated west mapedgepoints. KM:1",
              String.fromCharCode(gWorldSectorY + "A".charCodeAt(0) - 1),
              gWorldSectorX,
              gbWorldSectorZ,
            ),
