namespace ja2 {
  ...
  const SET_MOVEMENTCOST = (a: number, b: number, c: number, d: number) =>
    gubWorldMovementCosts[a][b][c] < d
      ? (gubWorldMovementCosts[a][b][c] = d)
      : 0;
  const FORCE_SET_MOVEMENTCOST = (a: number, b: number, c: number, d: number) =>
    (gubWorldMovementCosts[a][b][c] = d);
  const SET_CURRMOVEMENTCOST = (usGridNo: number, a: number, b: number) =>
    SET_MOVEMENTCOST(usGridNo, a, 0, b);
  ...
  export let TileSurfaceFilenames: string[] /* CHAR8[NUMBEROFTILETYPES][32] */ =
    createArray(Enum313.NUMBEROFTILETYPES, "");
  let gbNewTileSurfaceLoaded: boolean[] /* INT8[NUMBEROFTILETYPES] */ =
  ...
  export let gubWorldMovementCosts: UINT8[][][] /* [WORLD_MAX][MAXDIR][2] */ =
    createArrayFrom(WORLD_MAX, () =>
      createArrayFrom(MAXDIR, () => createArray(2, 0)),
    );
  ...
  function DOIT(): void {
    ...
    for (uiLoop = 0; uiLoop < WORLD_MAX; uiLoop++) {
      ...
      while (pStruct != null) {
        ...
        if (
          pStruct.usIndex >= Enum312.DEBRISWOOD1 &&
          pStruct.usIndex <= Enum312.DEBRISWEEDS10
        ) {
  ...
  function LoadTileSurfaces(
    ppTileSurfaceFilenames: string[] /* [][32] */,
    ubTilesetID: UINT8,
  ): boolean {
    ...
    for (uiLoop = 0; uiLoop < Enum313.NUMBEROFTILETYPES; uiLoop++) {
      uiPercentage = Math.trunc(
        (uiLoop * 100) / (Enum313.NUMBEROFTILETYPES - 1),
      );
      ...
      if (ppTileSurfaceFilenames == null) {
        ...
        if (ppTileSurfaceFilenames[uiLoop] != "") {
          ...
          if (!gbDefaultSurfaceUsed[uiLoop]) {
            TileSurfaceFilenames[uiLoop] =
              gTilesets[Enum316.GENERIC_1].TileSurfaceFilenames[uiLoop]; //(char *)(ppTileSurfaceFilenames + (65 * uiLoop)) );
            if (
              AddTileSurface(
                gTilesets[Enum316.GENERIC_1].TileSurfaceFilenames[uiLoop],
                uiLoop,
                Enum316.GENERIC_1,
                false,
              ) == false
  ...
  function AddTileSurface(
    cFilename: string /* Pointer<char> */,
    ubType: UINT32,
    ubTilesetID: UINT8,
    fGetFromRoot: boolean,
  ): boolean {
    // Add tile surface
  ...
  export function BuildTileShadeTables(): void {
    ...
    if (!SetFileManCurrentDirectory(ShadeTableDir)) {
      AssertMsg(
        0,
        "Can't set the directory to Data\\ShadeTable.  Kris' big problem!",
      );
    }
    ...
    if (
      ...
      if (
        gpLightColors[0].peRed != BuildTileShadeTables__ubLastRed ||
        gpLightColors[0].peGreen != BuildTileShadeTables__ubLastGreen ||
        gpLightColors[0].peBlue != BuildTileShadeTables__ubLastBlue
      ) {
    ...
    for (uiLoop = 0; uiLoop < Enum313.NUMBEROFTILETYPES; uiLoop++) {
      if (gTileSurfaceArray[uiLoop] != null) {
        ...
        if (gbNewTileSurfaceLoaded[uiLoop] || gfEditorForceShadeTableRebuild) {
          ...
          RenderProgressBar(
            0,
            Math.trunc((uiLoop * 100) / Enum313.NUMBEROFTILETYPES),
          );
          CreateTilePaletteTables(
            gTileSurfaceArray[uiLoop].vo,
            uiLoop,
            fForceRebuildForSlot,
          );
        }
      }
    }
    ...
    for (sGridNo = 0; sGridNo < WORLD_MAX; sGridNo++) {
      if (GridNoOnVisibleWorldTile(sGridNo)) {
        ...
        if (giCurrentTilesetID == Enum316.TEMP_19) {
          ...
          if (pNode != null) {
            if (
              pNode.usIndex == Enum312.ANOTHERDEBRIS4 ||
              pNode.usIndex == Enum312.ANOTHERDEBRIS6 ||
              pNode.usIndex == Enum312.ANOTHERDEBRIS7
            ) {
        ...
        if (
          pNode == null ||
          pNode.usIndex >= Enum312.NUMBEROFTILES ||
          gTileDatabase[pNode.usIndex].ubTerrainID == Enum315.NO_TERRAIN
        ) {
          // Try terrain instead!
        ...
        if (pTileElement.ubNumberOfTiles > 1) {
          ...
          for (ubLoop = 0; ubLoop < pTileElement.ubNumberOfTiles; ubLoop++) {
            ...
            gpWorldLevelData[sTempGridNo].ubTerrainID =
              pTileElement.ubTerrainID;
          }
        } else {
  ...
  function CompileTileMovementCosts(usGridNo: UINT16): void {
    ...
    if (GridNoOnVisibleWorldTile(usGridNo)) {
      ...
      for (ubDirLoop = 0; ubDirLoop < 8; ubDirLoop++) {
        if (
          gpWorldLevelData[usGridNo].sHeight !=
    ...
    if (gpWorldLevelData[usGridNo].pStructureHead != null) {
      ...
      if (pLand != null) {
        ...
        for (
          ubDirLoop = 0;
          ubDirLoop < Enum245.NUM_WORLD_DIRECTIONS;
          ubDirLoop++
        ) {
          SET_CURRMOVEMENTCOST(
            usGridNo,
            ubDirLoop,
      ...
      do {
        if (pStructure.sCubeOffset == STRUCTURE_ON_GROUND) {
          if (pStructure.fFlags & STRUCTURE_PASSABLE) {
            if (
              pStructure.fFlags & STRUCTURE_WIREFENCE &&
            ...
            if (
              pStructure.fFlags & STRUCTURE_FENCE &&
              ...
              switch (pStructure.ubWallOrientation) {
                ...
                case Enum314.INSIDE_TOP_RIGHT:
                  ...
                  if (
                    gubWorldMovementCosts[usGridNo + 1][Enum245.EAST][0] <
                    TRAVELCOST_BLOCKED
                  ) {
                    FORCE_SET_MOVEMENTCOST(
                      usGridNo + 1,
                  ...
                  if (
                    gubWorldMovementCosts[usGridNo - 1][Enum245.WEST][0] <
                    TRAVELCOST_BLOCKED
                  ) {
                    FORCE_SET_MOVEMENTCOST(
                      usGridNo - 1,
              ...
              if (
                FindStructure(usGridNo - WORLD_COLS, STRUCTURE_OBSTACLE) ==
                  null &&
          ...
          } else if (pStructure.fFlags & STRUCTURE_ANYDOOR) {
            /*&& (pStructure->fFlags & STRUCTURE_OPEN))*/
            ...
            if (
              ...
              switch (pStructure.ubWallOrientation) {
                ...
                case Enum314.INSIDE_TOP_RIGHT:
                  ...
                  SET_MOVEMENTCOST(
                    ...
                    Enum245.NORTHWEST,
                    0,
                    TRAVELCOST_DOOR_OPEN_E,
                  );
                  SET_MOVEMENTCOST(
                    usGridNo + WORLD_COLS - 1,
                    Enum245.SOUTHWEST,
                    0,
                    TRAVELCOST_DOOR_OPEN_NE,
                  );
                  break;
              ...
              switch (pStructure.ubWallOrientation) {
                ...
                case Enum314.INSIDE_TOP_LEFT:
                  ...
                  SET_MOVEMENTCOST(
                    ...
                    Enum245.NORTHWEST,
                    0,
                    TRAVELCOST_DOOR_OPEN_S,
                  );
                  SET_MOVEMENTCOST(
                    usGridNo - WORLD_COLS + 1,
                    Enum245.NORTHEAST,
                    0,
                    TRAVELCOST_DOOR_OPEN_SW,
                  );
                  break;
                default:
                  // door with no orientation specified!?
              ...
              pStructure.fFlags & STRUCTURE_SLIDINGDOOR &&
  ...
  export function RecompileLocalMovementCosts(sCentreGridNo: INT16): void {
    ...
    for (
      sGridY = sCentreGridY - LOCAL_RADIUS;
      sGridY < sCentreGridY + LOCAL_RADIUS;
      sGridY++
    ) {
      for (
        sGridX = sCentreGridX - LOCAL_RADIUS;
        sGridX < sCentreGridX + LOCAL_RADIUS;
        sGridX++
      ) {
    ...
    for (
      sGridY = sCentreGridY - LOCAL_RADIUS - 1;
      sGridY < sCentreGridY + LOCAL_RADIUS + 1;
      sGridY++
    ) {
      for (
        sGridX = sCentreGridX - LOCAL_RADIUS - 1;
        sGridX < sCentreGridX + LOCAL_RADIUS + 1;
        sGridX++
      ) {
        usGridNo = MAPROWCOLTOPOS(sGridY, sGridX);
  ...
  export function RecompileLocalMovementCostsFromRadius(
    sCentreGridNo: INT16,
    bRadius: INT8,
  ): void {
    let usGridNo: INT16;
    ...
    if (bRadius == 0) {
      ...
      for (
        sGridY = sCentreGridY - bRadius;
        sGridY < sCentreGridY + bRadius;
        sGridY++
      ) {
        for (
          sGridX = sCentreGridX - bRadius;
          sGridX < sCentreGridX + bRadius;
          sGridX++
        ) {
          usGridNo = MAPROWCOLTOPOS(sGridY, sGridX);
      ...
      // edges of the recompiled areas are correct (i.e. there could be spillover)
      for (
        sGridY = sCentreGridY - bRadius - 1;
        sGridY < sCentreGridY + bRadius + 1;
        sGridY++
      ) {
        for (
          sGridX = sCentreGridX - bRadius - 1;
          sGridX < sCentreGridX + bRadius + 1;
          sGridX++
        ) {
          usGridNo = MAPROWCOLTOPOS(sGridY, sGridX);
  ...
  export function AddTileToRecompileArea(sGridNo: INT16): void {
    ...
    gpWorldLevelData[sGridNo].ubExtFlags[0] |=
      MAPELEMENT_EXT_RECALCULATE_MOVEMENT;
  ...
  export function RecompileLocalMovementCostsInAreaWithFlags(): void {
    ...
    for (
      sGridY = gsRecompileAreaTop;
      sGridY <= gsRecompileAreaBottom;
      sGridY++
    ) {
      for (
        sGridX = gsRecompileAreaLeft;
        sGridX < gsRecompileAreaRight;
        sGridX++
      ) {
        usGridNo = MAPROWCOLTOPOS(sGridY, sGridX);
        if (
          usGridNo < WORLD_MAX &&
          gpWorldLevelData[usGridNo].ubExtFlags[0] &
            MAPELEMENT_EXT_RECALCULATE_MOVEMENT
        ) {
  ...
  export function RecompileLocalMovementCostsForWall(
    sGridNo: INT16,
    ubOrientation: UINT8,
  ): void {
    let bDirLoop: INT8;
  ...
  export function SaveWorld(puiFilename: string /* Pointer<UINT8> */): boolean {
    ...
    let ubCombine: UINT8;
    let bCounts: UINT8[][] /* [WORLD_MAX][8] */ = createArrayFrom(
      WORLD_MAX,
      () => createArray(8, 0),
    );
    let buffer: Buffer;
    ...
    for (cnt = 0; cnt < WORLD_MAX; cnt++) {
      ...
      if (LayerCount > 10) {
        ...
        gfErrorCatch = true;
        gzErrorCatchString = swprintf(
          "Warnings %d -- Last warning:  Land count warning of %d for gridno %d.",
          uiNumWarningsCaught,
          LayerCount,
          cnt,
        );
      ...
      ubCombine =
        (LayerCount & 0xf) | ((gpWorldLevelData[cnt].uiFlags & 0xf) << 4);
      // Write combination
      ...
      while (pObject != null) {
        ...
        if (!(pObject.uiFlags & LEVELNODE_ITEM)) {
          ...
          if (uiTileType < Enum313.FIRSTPOINTERS) ObjectCount++;
        }
      ...
      if (ObjectCount > 10) {
        ...
        gfErrorCatch = true;
        gzErrorCatchString = swprintf(
          "Warnings %d -- Last warning:  Object count warning of %d for gridno %d.",
          uiNumWarningsCaught,
          ObjectCount,
          cnt,
        );
      ...
      if (StructCount > 10) {
        ...
        gfErrorCatch = true;
        gzErrorCatchString = swprintf(
          "Warnings %d -- Last warning:  Struct count warning of %d for gridno %d.",
          uiNumWarningsCaught,
          StructCount,
          cnt,
        );
      ...
      if (ShadowCount > 10) {
        ...
        gfErrorCatch = true;
        gzErrorCatchString = swprintf(
          "Warnings %d -- Last warning:  Shadow count warning of %d for gridno %d.",
          uiNumWarningsCaught,
          ShadowCount,
          cnt,
        );
      ...
      if (RoofCount > 10) {
        ...
        gfErrorCatch = true;
        gzErrorCatchString = swprintf(
          "Warnings %d -- Last warning:  Roof count warning of %d for gridno %d.",
          uiNumWarningsCaught,
          RoofCount,
          cnt,
        );
      ...
      if (OnRoofCount > 10) {
        ...
        gfErrorCatch = true;
        gzErrorCatchString = swprintf(
          "Warnings %d -- Last warning:  OnRoof count warning of %d for gridno %d.",
          uiNumWarningsCaught,
          OnRoofCount,
          cnt,
        );
    ...
    for (cnt = 0; cnt < WORLD_MAX; cnt++) {
      if (bCounts[cnt][0] == 0) {
        ...
        while (pTailLand != null) {
          ...
          ubType = uiType;
          ubTypeSubIndex = GetTypeSubIndexFromTileIndexChar(
            uiType,
            pTailLand.usIndex,
          );
      ...
      while (pObject != null) {
        ...
        if (!(pObject.uiFlags & LEVELNODE_ITEM)) {
          ...
          if (uiType < Enum313.FIRSTPOINTERS) {
            ...
            ubType = uiType;
            usTypeSubIndex = GetTypeSubIndexFromTileIndex(
              uiType,
              pObject.usIndex,
            );
            buffer.writeUInt8(ubType, 0);
      ...
      if (IsTreePresentAtGridno(cnt)) {
        ...
        for (dir = 0; dir < NUM_DIR_SEARCHES; dir++) {
          sNewGridNo = NewGridNo(
            cnt,
            DirectionInc(bDirectionsForShadowSearch[dir]),
          );
  ...
  export function EvaluateWorld(
    ...
    if (!gfUpdatingNow) SetRelativeStartAndEndPercentage(0, 0, 100, str);
    else SetRelativeStartAndEndPercentage(0, MasterStart, MasterEnd, str);
    ...
    // skip JA2 Version ID
    dMajorMapVersion = buffer.readFloatLE(offset);
    offset += 4;
    if (dMajorMapVersion >= 4.0) {
      ubMinorMapVersion = buffer.readUInt8(offset++);
    }

    // Read FLAGS FOR WORLD
    uiFlags = buffer.readUInt32LE(offset);
    offset += 4;

    // Read tilesetID
    iTilesetID = buffer.readInt32LE(offset);
    offset += 4;
    ...
    if (uiFlags & MAP_WORLDITEMS_SAVED) {
      ...
      temp = buffer.readUInt32LE(offset);
      offset += 4;
    ...
    if (uiFlags & MAP_WORLDLIGHTS_SAVED) {
      ...
      pSummary.usNumLights = buffer.readUInt16LE(offset);
      offset += 2;
    ...
    if (uiFlags & MAP_FULLSOLDIER_SAVED) {
      ...
      for (i = 0; i < pSummary.MapInfo.ubNumIndividuals; i++) {
        ...
        if (
          basic.bOrders == Enum241.RNDPTPATROL ||
          basic.bOrders == Enum241.POINTPATROL
        ) {
        ...
        if (basic.fDetailedPlacement) {
          ...
          if (priority.ubProfile != NO_PROFILE) pTeam.ubProfile++;
          else pTeam.ubDetailed++;
          if (basic.bTeam == CIV_TEAM) {
            if (priority.ubScheduleID) pSummary.ubCivSchedules++;
            if (priority.bBodyType == Enum194.COW) pSummary.ubCivCows++;
        ...
        if (basic.bTeam == ENEMY_TEAM) {
          switch (basic.ubSoldierClass) {
            case Enum262.SOLDIER_CLASS_ADMINISTRATOR:
              ...
              if (basic.fDetailedPlacement) {
                if (priority.ubProfile != NO_PROFILE) pSummary.ubAdminProfile++;
                else pSummary.ubAdminDetailed++;
              }
              break;
            case Enum262.SOLDIER_CLASS_ELITE:
              ...
              if (basic.fDetailedPlacement) {
                if (priority.ubProfile != NO_PROFILE) pSummary.ubEliteProfile++;
                else pSummary.ubEliteDetailed++;
              }
              break;
            case Enum262.SOLDIER_CLASS_ARMY:
              ...
              if (basic.fDetailedPlacement) {
                if (priority.ubProfile != NO_PROFILE) pSummary.ubTroopProfile++;
                else pSummary.ubTroopDetailed++;
              }
              break;
          }
        } else if (basic.bTeam == CREATURE_TEAM) {
          if (basic.bBodyType == Enum194.BLOODCAT) pTeam.ubNumAnimals++;
        }
        pTeam.ubTotal++;
      }
    ...
    if (uiFlags & MAP_EXITGRIDS_SAVED) {
      ...
      cnt = buffer.readUInt16LE(offset);
      offset += 2;

      for (i = 0; i < cnt; i++) {
        usMapIndex = buffer.readUInt16LE(offset);
        offset += 2;
  ...
  export function LoadWorld(puiFilename: string /* Pointer<UINT8> */): boolean {
    ...
    // Read JA2 Version ID
    dMajorMapVersion = buffer.readFloatLE(bufferOffset);
    bufferOffset += 4;
    ...
    // Read FLAGS FOR WORLD
    uiFlags = buffer.readUInt32LE(bufferOffset);
    bufferOffset += 4;

    iTilesetID = buffer.readInt32LE(bufferOffset);
    bufferOffset += 4;
    ...
    // Load soldier size
    uiSoldierSize = buffer.readUInt32LE(bufferOffset);
    bufferOffset += 4;
    ...
    if (0) {
      ...
      for (cnt = 0; cnt < WORLD_MAX; cnt++) {
        ...
        for (cnt2 = 0; cnt2 < bCounts[cnt][1]; cnt2++) {
          ubType = buffer.readUInt8(bufferOffset++);
          usTypeSubIndex = buffer.readUInt16LE(bufferOffset);
          bufferOffset += 2;
          if (ubType >= Enum313.FIRSTPOINTERS) {
            continue;
          }
    ...
    if (uiFlags & MAP_EDGEPOINTS_SAVED) {
      ...
      RenderProgressBar(0, 0);
      let result: boolean;
      ({ result, offset: bufferOffset } = LoadMapEdgepoints(
        buffer,
        bufferOffset,
      ));
      if (!result) fGenerateEdgePoints = true; // only if the map had the older edgepoint system
    } else {
  ...
  export function LoadMapTileset(iTilesetID: INT32): boolean {
    ...
    if (
      LoadTileSurfaces(
        gTilesets[iTilesetID].TileSurfaceFilenames,
        iTilesetID,
      ) == false
    ) {
      return false;
    ...
    if (gTilesets[iTilesetID].MovementCostFnc != null) {
      ...
      DebugMsg(
        ...
        DBG_LEVEL_3,
        FormatString(
          "Tileset %d has no callback function for movement costs. Using default.",
          iTilesetID,
        ),
      );
  ...
  export function SetLoadOverrideParams(
    fForceLoad: boolean,
    fForceFile: boolean,
    zLoadName: string | null /* Pointer<CHAR8> */,
  ): void {
  ...
  function GetWireframeGraphicNumToUseForWall(
    sGridNo: INT16,
        ...
        while (pStructure != null) {
            ...
            if (pStructure.fFlags & STRUCTURE_OPENABLE) {
              ...
              switch (ubWallOrientation) {
                ...
                case Enum314.INSIDE_TOP_LEFT:
                  ...
                  if (
                    IsRoofVisibleForWireframe(sGridNo) &&
                    !(gpWorldLevelData[sGridNo].uiFlags & MAPELEMENT_REVEALED)
                  ) {
                    AddWireFrame(
                      cnt,
                      Enum312.WIREFRAMES4,
                      (gpWorldLevelData[sGridNo].uiFlags &
                        MAPELEMENT_REVEALED) !=
                        0,
                    );
                  }
          ...
          // Check for windows
            ...
            if (pStructure.fFlags & STRUCTURE_WALLSTUFF) {
              ...
              usWireFrameIndex = GetWireframeGraphicNumToUseForWall(
                cnt,
                pStructure,
              );
  ...
  function SaveMapLights(hfile: HWFILE): void {
    ...
    let LColors: SGPPaletteEntry[] /* [3] */ = createArrayFrom(
      3,
    ...
    uiBytesWritten = FileWrite(
      hfile,
      buffer,
      SGP_PALETTE_ENTRY_SIZE * ubNumColors,
    );
      ...
      if (LightSprites[cnt].uiFlags & LIGHT_SPR_ACTIVE) {
        ...
        for (cnt2 = 0; cnt2 < MAX_NUM_SOLDIERS && !fSoldierLight; cnt2++) {
          if ((pSoldier = GetSoldier(cnt2)) !== null) {
            if (pSoldier.iLight == cnt) fSoldierLight = true;
          }
        }
    ...
    for (cnt = 0; cnt < MAX_LIGHT_SPRITES; cnt++) {
      if (LightSprites[cnt].uiFlags & LIGHT_SPR_ACTIVE) {
        ...
        if (!fSoldierLight) {
          ...
          writeStringNL(
            pLightNames[LightSprites[cnt].iTemplate],
            buffer,
            0,
            ubStrLen,
            "ascii",
  ...
  function LoadMapLights(buffer: Buffer, offset: number): number {
    ...
    usNumLights = buffer.readUInt16LE(offset);
    offset += 2;
    ...
    if (!gfEditMode) {
      uiHour = GetWorldHour();
      if (
        uiHour >= NIGHT_TIME_LIGHT_START_HOUR ||
        uiHour < NIGHT_TIME_LIGHT_END_HOUR
      ) {
        fNightTime = true;
      }
    ...
    for (cnt = 0; cnt < usNumLights; cnt++) {
      ...
      if (ubStrLen) {
        str = readStringNL(buffer, "ascii", offset, offset + ubStrLen);
        offset += ubStrLen;
      } else {
        str = "";
      }
