namespace ja2 {
  ...
  const GetMapXYWorldYFromCellCoords = (
    sWorldCellX: INT16,
    sWorldCellY: INT16,
    sWorldY: INT16,
  ) => {
  ...
  export const ObjectZLevel = (
    TileElem: TILE_ELEMENT,
    uiTileElemFlags: UINT32,
    sMapX: INT16,
    sMapY: INT16,
  ): INT16 => {
  ...
  export const StructZLevel = (
    uiLevelNodeFlags: UINT32,
    pCorpse: ROTTING_CORPSE,
    pNode: LEVELNODE,
    uiAniTileFlags: UINT32,
    sMapX: INT16,
    sMapY: INT16,
  ): INT16 => {
    let sWorldY: INT16 = GetMapXYWorldY(sMapX, sMapY);
    ...
    if (uiLevelNodeFlags & LEVELNODE_ROTTINGCORPSE) {
      if (pCorpse.def.usFlags & ROTTING_CORPSE_VEHICLE) {
        if (pNode.pStructureData != null) {
          sZOffsetX =
            pNode.pStructureData.pDBStructureRef.pDBStructure.bZTileOffsetX;
          sZOffsetY =
            pNode.pStructureData.pDBStructureRef.pDBStructure.bZTileOffsetY;
        }
  ...
  export const SoldierZLevel = (
    pSoldier: SOLDIERTYPE,
    pNode: LEVELNODE,
    sMapX: INT16,
    sMapY: INT16,
    gsForceSoldierZLevel: INT16,
  ): INT16 => {
    let sWorldY: INT16;
    let sZOffsetX: INT16 = -1;
    let sZOffsetY: INT16 = -1;
  ...
  export function setZValue(
    zBuffer: Uint8ClampedArray,
    index: number,
    value: number,
  ) {
