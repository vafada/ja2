namespace ja2 {
  ...
  // mouse regions
  let pFilesRegions: MOUSE_REGION[] /* [MAX_FILES_PAGE] */ = createArrayFrom(
    MAX_FILES_PAGE,
    createMouseRegion,
  );

  function AddFilesToPlayersLog(
    ubCode: UINT8,
    uiDate: UINT32,
    ubFormat: UINT8,
    pFirstPicFile: string | null /* STR8 */,
    pSecondPicFile: string | null /* STR8 */,
  ): UINT32 {
    ...
    uiId = ProcessAndEnterAFilesRecord(
      ubCode,
      uiDate,
      ubFormat,
      pFirstPicFile,
  ...
  function ProcessAndEnterAFilesRecord(
    ubCode: UINT8,
    uiDate: UINT32,
    ubFormat: UINT8,
    pFirstPicFile: string | null /* STR8 */,
  ...
  function DisplayFilesList(): void {
    ...
    while (pFilesList) {
      //&&(iCounter < MAX_FILES_LIST_LENGTH))
      ...
      mprintf(
        FILES_SENDER_TEXT_X,
        (iCounter + 9) * BLOCK_HEIGHT + iCounter * 2 - 2,
  ...
  function InitializeFilesMouseRegions(): void {
    ...
    for (iCounter = 0; iCounter < MAX_FILES_PAGE; iCounter++) {
      MSYS_DefineRegion(
        pFilesRegions[iCounter],
        FILES_LIST_X,
        FILES_LIST_Y + iCounter * (BLOCK_HEIGHT + 2),
        FILES_LIST_X + FILES_LIST_WIDTH,
        FILES_LIST_Y + (iCounter + 1) * (BLOCK_HEIGHT + 2),
  ...
  function DisplayFormattedText(): boolean {
    ...
    if (pFilesList.ubFormat < Enum78.ENRICO_BACKGROUND) {
      sString = LoadEncryptedDataFromFile(
        "BINARYDATA\\Files.edt",
        FILE_STRING_SIZE * iOffSet * 2,
    ...
    switch (pFilesList.ubFormat) {
      ...
      case 1:
        ...
        ({ usWidth: usFirstWidth, usHeight: usFirstHeight } =
          GetVideoObjectETRLESubregionProperties(uiFirstTempPicture, 0));
        ...
        BltVideoObject(
          ...
          FILE_VIEWER_X +
            4 +
            Math.trunc((FILE_VIEWER_WIDTH - usFirstWidth) / 2),
          FILE_VIEWER_Y + 10,
          VO_BLT_SRCTRANSPARENCY,
          null,
        );
      ...
      case 2:
        ...
        ({ usWidth: usFirstWidth, usHeight: usFirstHeight } =
          GetVideoObjectETRLESubregionProperties(uiFirstTempPicture, 0));
        ({ usWidth: usSecondWidth, usHeight: usSecondHeight } =
          GetVideoObjectETRLESubregionProperties(uiSecondTempPicture, 0));
  ...
  function HandleSpecialFiles(ubFormat: UINT8): boolean {
    ...
    switch (ubFormat) {
      case 255:
        ...
        pTempString = GetFirstStringOnThisPage(
          pFileStringList,
          FILES_TEXT_FONT(),
          350,
          FILE_GAP,
        ...
        while (pTempString) {
          ...
          if (
            ...
            // now print it
            iYPositionOnPage += IanDisplayWrappedString(
              iFileStartX,
              FILE_VIEWER_Y + iYPositionOnPage,
              iFileLineWidth,
              FILE_GAP,
              uiFont,
              FILE_TEXT_COLOR,
  ...
  function CreateButtonsForFilesPage(): void {
    ...
    giFilesPageButtonsImage[0] = LoadButtonImage(
      "LAPTOP\\arrows.sti",
      -1,
      0,
      -1,
    ...
    giFilesPageButtons[0] = QuickCreateButton(
      ...
      PREVIOUS_FILE_PAGE_BUTTON_X,
      PREVIOUS_FILE_PAGE_BUTTON_Y,
      BUTTON_TOGGLE,
      MSYS_PRIORITY_HIGHEST - 1,
      BtnGenericMouseMoveButtonCallback,
      BtnPreviousFilePageCallback,
    );

    giFilesPageButtonsImage[1] = LoadButtonImage(
      "LAPTOP\\arrows.sti",
      -1,
      6,
      -1,
    ...
    giFilesPageButtons[1] = QuickCreateButton(
      ...
      NEXT_FILE_PAGE_BUTTON_X,
      NEXT_FILE_PAGE_BUTTON_Y,
      BUTTON_TOGGLE,
      MSYS_PRIORITY_HIGHEST - 1,
      BtnGenericMouseMoveButtonCallback,
  ...
  function CreateWidthRecordsForAruloIntelFile(): FileRecordWidth {
    ...
    pTempRecord = CreateRecordWidth(
      FILES_COUNTER_1_WIDTH,
      350,
      MAX_FILE_MESSAGE_PAGE_SIZE,
      0,
    );
  ...
  function ClearOutWidthRecordsList(
    pFileRecordWidthList: FileRecordWidth,
  ): void {
  ...
  function HandleSpecialTerroristFile(
    iFileNumber: INT32,
    sPictureName: string /* STR */,
  ): boolean {
    let iCounter: INT32 = 0;
    let sString: string /* wchar_t[2048] */;
    let pTempString: FileString | null = null;
