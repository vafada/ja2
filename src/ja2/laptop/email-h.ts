namespace ja2 {
  ...
  export const AIM_MEDICAL_DEPOSIT_REFUND =
    MERC_NEW_SITE_ADDRESS + MERC_NEW_SITE_ADDRESS_LENGTH;
  export const AIM_MEDICAL_DEPOSIT_REFUND_LENGTH = 3;
  export const IMP_EMAIL_PROFILE_RESULTS =
    AIM_MEDICAL_DEPOSIT_REFUND + AIM_MEDICAL_DEPOSIT_REFUND_LENGTH;
  export const IMP_EMAIL_PROFILE_RESULTS_LENGTH = 1;
  export const MERC_WARNING =
    IMP_EMAIL_PROFILE_RESULTS_LENGTH + IMP_EMAIL_PROFILE_RESULTS;
  ...
  export const MERC_FIRST_WARNING =
    NEW_MERCS_AT_MERC + NEW_MERCS_AT_MERC_LENGTH;
  export const MERC_FIRST_WARNING_LENGTH = 2;
  // merc up a level emails
  export const MERC_UP_LEVEL_BIFF =
    MERC_FIRST_WARNING + MERC_FIRST_WARNING_LENGTH;
  ...
  const MERC_UP_LEVEL_GASKET =
    MERC_UP_LEVEL_LENGTH_HAYWIRE + MERC_UP_LEVEL_HAYWIRE;
  const MERC_UP_LEVEL_LENGTH_GASKET = 2;
  const MERC_UP_LEVEL_RAZOR =
    MERC_UP_LEVEL_LENGTH_GASKET + MERC_UP_LEVEL_GASKET;
  ...
  export const AIM_REPLY_BARRY =
    MERC_UP_LEVEL_LENGTH_BUBBA + MERC_UP_LEVEL_BUBBA;
  ...
  export const LACK_PLAYER_PROGRESS_1 =
    KING_PIN_LETTER + KING_PIN_LETTER_LENGTH;
  export const LACK_PLAYER_PROGRESS_1_LENGTH = 3;

  export const LACK_PLAYER_PROGRESS_2 =
    LACK_PLAYER_PROGRESS_1 + LACK_PLAYER_PROGRESS_1_LENGTH;
  export const LACK_PLAYER_PROGRESS_2_LENGTH = 3;

  export const LACK_PLAYER_PROGRESS_3 =
    LACK_PLAYER_PROGRESS_2 + LACK_PLAYER_PROGRESS_2_LENGTH;
  export const LACK_PLAYER_PROGRESS_3_LENGTH = 3;

  // A package from bobby r has arrived in Drassen
  export const BOBBYR_SHIPMENT_ARRIVED =
    LACK_PLAYER_PROGRESS_3 + LACK_PLAYER_PROGRESS_3_LENGTH;
  export const BOBBYR_SHIPMENT_ARRIVED_LENGTH = 4;

  // John Kulba has left the gifts for theplayers in drassen
  export const JOHN_KULBA_GIFT_IN_DRASSEN =
    BOBBYR_SHIPMENT_ARRIVED + BOBBYR_SHIPMENT_ARRIVED_LENGTH;
  export const JOHN_KULBA_GIFT_IN_DRASSEN_LENGTH = 4;

  // when a merc dies on ANOTHER assignment ( ie not with the player )
  export const MERC_DIED_ON_OTHER_ASSIGNMENT =
    JOHN_KULBA_GIFT_IN_DRASSEN + JOHN_KULBA_GIFT_IN_DRASSEN_LENGTH;
  export const MERC_DIED_ON_OTHER_ASSIGNMENT_LENGTH = 5;

  export const INSUR_1HOUR_FRAUD =
    MERC_DIED_ON_OTHER_ASSIGNMENT + MERC_DIED_ON_OTHER_ASSIGNMENT_LENGTH;
  export const INSUR_1HOUR_FRAUD_LENGTH = 3;

  // when a merc is fired, and is injured
  export const AIM_MEDICAL_DEPOSIT_PARTIAL_REFUND =
    INSUR_1HOUR_FRAUD + INSUR_1HOUR_FRAUD_LENGTH;
  export const AIM_MEDICAL_DEPOSIT_PARTIAL_REFUND_LENGTH = 3;

  // when a merc is fired, and is dead
  export const AIM_MEDICAL_DEPOSIT_NO_REFUND =
  ...
  export const BOBBY_R_MEDUNA_SHIPMENT =
  ...
  export function readSavedEmailStruct(
    o: SavedEmailStruct,
    buffer: Buffer,
    offset: number = 0,
  ): number {
    o.usOffset = buffer.readUInt16LE(offset);
    offset += 2;
    o.usLength = buffer.readUInt16LE(offset);
    offset += 2;
    o.ubSender = buffer.readUInt8(offset++);
    offset += 3; // padding
    o.iDate = buffer.readUInt32LE(offset);
    offset += 4;
    o.iId = buffer.readInt32LE(offset);
    offset += 4;
    o.iFirstData = buffer.readInt32LE(offset);
    offset += 4;
    o.uiSecondData = buffer.readUInt32LE(offset);
    offset += 4;
    o.iThirdData = buffer.readInt32LE(offset);
    offset += 4;
    o.iFourthData = buffer.readInt32LE(offset);
    offset += 4;
    o.uiFifthData = buffer.readUInt32LE(offset);
    offset += 4;
    o.uiSixData = buffer.readUInt32LE(offset);
    offset += 4;
  ...
  export function writeSavedEmailStruct(
    o: SavedEmailStruct,
    buffer: Buffer,
    offset: number = 0,
  ): number {
