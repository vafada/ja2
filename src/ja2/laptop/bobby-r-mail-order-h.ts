namespace ja2 {
  ...
  export function readNewBobbyRayOrderStruct(
    o: NewBobbyRayOrderStruct,
    buffer: Buffer,
    offset: number = 0,
  ): number {
    ...
    for (let i = 0; i < MAX_PURCHASE_AMOUNT; i++) {
      offset = readBobbyRayPurchaseStruct(
        o.BobbyRayPurchase[i],
        buffer,
        offset,
      );
    }
    ...
    offset += 3; // padding
    o.uiPackageWeight = buffer.readUInt32LE(offset);
    offset += 4;
    o.uiOrderedOnDayNum = buffer.readUInt32LE(offset);
    offset += 4;
  ...
  export function writeNewBobbyRayOrderStruct(
    o: NewBobbyRayOrderStruct,
    buffer: Buffer,
    offset: number = 0,
  ) {
    ...
    for (let i = 0; i < MAX_PURCHASE_AMOUNT; i++) {
      offset = writeBobbyRayPurchaseStruct(
        o.BobbyRayPurchase[i],
        buffer,
        offset,
      );
