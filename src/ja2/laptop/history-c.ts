namespace ja2 {
  ...
  export function SetHistoryFact(
    ubCode: UINT8,
    ubSecondCode: UINT8,
    uiDate: UINT32,
    sSectorX: INT16,
    ...
    uiId = ProcessAndEnterAHistoryRecord(
      ubCode,
      uiDate,
      ubSecondCode,
      sSectorX,
    ...
    ScreenMsg(
      ...
      MSG_INTERFACE,
      pMessageStrings[Enum333.MSG_HISTORY_UPDATED],
    );
  ...
  export function AddHistoryToPlayersLog(
    ubCode: UINT8,
    ubSecondCode: UINT8,
    uiDate: UINT32,
    sSectorX: INT16,
  ...
  export function GameInitHistory(): void {
    ...
    AddHistoryToPlayersLog(
      Enum83.HISTORY_ACCEPTED_ASSIGNMENT_FROM_ENRICO,
      0,
      GetWorldTotalMin(),
      -1,
      -1,
  ...
  */
  ...
  function CreateHistoryButtons(): void {
    ...
    giHistoryButtonImage[Enum82.PREV_PAGE_BUTTON] = LoadButtonImage(
      "LAPTOP\\arrows.sti",
      -1,
      0,
      -1,
    ...
    giHistoryButton[Enum82.PREV_PAGE_BUTTON] = QuickCreateButton(
      giHistoryButtonImage[Enum82.PREV_PAGE_BUTTON],
      PREV_BTN_X,
      BTN_Y,
      BUTTON_TOGGLE,
      MSYS_PRIORITY_HIGHEST - 1,
      BtnGenericMouseMoveButtonCallback,
      BtnHistoryDisplayPrevPageCallBack,
    );

    // the next page button
    giHistoryButtonImage[Enum82.NEXT_PAGE_BUTTON] = LoadButtonImage(
      "LAPTOP\\arrows.sti",
      -1,
      6,
      -1,
    ...
    giHistoryButton[Enum82.NEXT_PAGE_BUTTON] = QuickCreateButton(
      giHistoryButtonImage[Enum82.NEXT_PAGE_BUTTON],
      NEXT_BTN_X,
      BTN_Y,
      BUTTON_TOGGLE,
      MSYS_PRIORITY_HIGHEST - 1,
      BtnGenericMouseMoveButtonCallback,
      BtnHistoryDisplayNextPageCallBack,
    );
  ...
  function BtnHistoryDisplayPrevPageCallBack(
    btn: GUI_BUTTON,
    reason: INT32,
  ): void {
    // force redraw
  ...
  function BtnHistoryDisplayNextPageCallBack(
    btn: GUI_BUTTON,
    reason: INT32,
  ): void {
    if (reason & MSYS_CALLBACK_REASON_LBUTTON_DWN) {
  ...
  function IncrementCurrentPageHistoryDisplay(): boolean {
    ...
    if (
      Math.trunc(uiFileSize / uiSizeOfRecordsOnEachPage) + 1 <
...
*/
  ...
  function ProcessAndEnterAHistoryRecord(
    ubCode: UINT8,
    uiDate: UINT32,
    ubSecondCode: UINT8,
    sSectorX: INT16,
  ...
  function DisplayHistoryListHeaders(): void {
    ...
    // the date header
    ({ sX: usX, sY: usY } = FindFontCenterCoordinates(
      RECORD_DATE_X + RECORD_DATE_WIDTH + 5,
      0,
      RECORD_LOCATION_WIDTH,
      0,
      pHistoryHeaders[3],
      HISTORY_TEXT_FONT(),
    ));
    ...
    // event header
    ({ sX: usX, sY: usY } = FindFontCenterCoordinates(
      RECORD_DATE_X + RECORD_DATE_WIDTH + RECORD_LOCATION_WIDTH + 5,
      0,
      RECORD_LOCATION_WIDTH,
      0,
      pHistoryHeaders[3],
  ...
  function DrawHistoryRecordsText(): void {
    ...
    for (iCounter; iCounter < NUM_RECORDS_PER_PAGE; iCounter++) {
      ...
      mprintf(
        RECORD_DATE_X + RECORD_LOCATION_WIDTH + RECORD_DATE_WIDTH + 15,
        RECORD_Y + iCounter * BOX_HEIGHT + 3,
        sString,
      );
      ...
      if (pCurHistory.sSectorX == -1 || pCurHistory.sSectorY == -1) {
        ({ sX, sY } = FindFontCenterCoordinates(
          RECORD_DATE_X + RECORD_DATE_WIDTH,
          0,
          RECORD_LOCATION_WIDTH + 10,
          0,
          pHistoryLocations[0],
          HISTORY_TEXT_FONT(),
        ));
        ...
        sString = GetSectorIDString(
          pCurHistory.sSectorX,
          pCurHistory.sSectorY,
          pCurHistory.bSectorZ,
          true,
        );
        ({ sX, sY } = FindFontCenterCoordinates(
          RECORD_DATE_X + RECORD_DATE_WIDTH,
  ...
  */
    ...
    sString = swprintf(
      "%s %d - %d",
      pHistoryHeaders[2],
      Math.trunc(pCurrentHistory.uiDate / (24 * 60)),
      Math.trunc(uiLastDate / (24 * 60)),
    );
    mprintf(HISTORY_DATE_X, HISTORY_DATE_Y, sString);
  ...
  function ProcessHistoryTransactionString(pHistory: HistoryUnit): string {
    ...
    switch (pHistory.ubCode) {
      ...
      case Enum83.HISTORY_HIRED_MERC_FROM_AIM:
        pString = swprintf(
          pHistoryStrings[Enum83.HISTORY_HIRED_MERC_FROM_AIM],
      ...
      case Enum83.HISTORY_MERC_KILLED:
        if (pHistory.ubSecondCode != NO_PROFILE)
          pString = swprintf(
            pHistoryStrings[Enum83.HISTORY_MERC_KILLED],
            gMercProfiles[pHistory.ubSecondCode].zName,
          );
        break;
      ...
      case Enum83.HISTORY_HIRED_MERC_FROM_MERC:
        pString = swprintf(
          pHistoryStrings[Enum83.HISTORY_HIRED_MERC_FROM_MERC],
          gMercProfiles[pHistory.ubSecondCode].zName,
        );
        break;
      ...
      case Enum83.HISTORY_ACCEPTED_ASSIGNMENT_FROM_ENRICO:
        pString =
          pHistoryStrings[Enum83.HISTORY_ACCEPTED_ASSIGNMENT_FROM_ENRICO];
        break;
      ...
      case Enum83.HISTORY_PURCHASED_INSURANCE:
        pString = swprintf(
          pHistoryStrings[Enum83.HISTORY_PURCHASED_INSURANCE],
          gMercProfiles[pHistory.ubSecondCode].zNickname,
        );
        break;
      case Enum83.HISTORY_CANCELLED_INSURANCE:
        pString = swprintf(
          pHistoryStrings[Enum83.HISTORY_CANCELLED_INSURANCE],
          gMercProfiles[pHistory.ubSecondCode].zNickname,
        );
        break;
      case Enum83.HISTORY_INSURANCE_CLAIM_PAYOUT:
        pString = swprintf(
          pHistoryStrings[Enum83.HISTORY_INSURANCE_CLAIM_PAYOUT],
          gMercProfiles[pHistory.ubSecondCode].zNickname,
        );
        break;
      ...
      case Enum83.HISTORY_EXTENDED_CONTRACT_1_DAY:
        pString = swprintf(
          pHistoryStrings[Enum83.HISTORY_EXTENDED_CONTRACT_1_DAY],
          gMercProfiles[pHistory.ubSecondCode].zNickname,
        );
        break;
      ...
      case Enum83.HISTORY_EXTENDED_CONTRACT_1_WEEK:
        pString = swprintf(
          pHistoryStrings[Enum83.HISTORY_EXTENDED_CONTRACT_1_WEEK],
          gMercProfiles[pHistory.ubSecondCode].zNickname,
        );
        break;
      ...
      case Enum83.HISTORY_EXTENDED_CONTRACT_2_WEEK:
        pString = swprintf(
          pHistoryStrings[Enum83.HISTORY_EXTENDED_CONTRACT_2_WEEK],
          gMercProfiles[pHistory.ubSecondCode].zNickname,
        );
        break;
      ...
      case Enum83.HISTORY_MERC_FIRED:
        pString = swprintf(
          pHistoryStrings[Enum83.HISTORY_MERC_FIRED],
          gMercProfiles[pHistory.ubSecondCode].zNickname,
        );
        break;
      ...
      case Enum83.HISTORY_MERC_QUIT:
        pString = swprintf(
          pHistoryStrings[Enum83.HISTORY_MERC_QUIT],
          gMercProfiles[pHistory.ubSecondCode].zNickname,
        );
        break;
      ...
      case Enum83.HISTORY_TALKED_TO_MINER:
        pString = swprintf(
          pHistoryStrings[Enum83.HISTORY_TALKED_TO_MINER],
          pTownNames[pHistory.ubSecondCode],
        );
        break;
      case Enum83.HISTORY_LIBERATED_TOWN:
        pString = swprintf(
          pHistoryStrings[Enum83.HISTORY_LIBERATED_TOWN],
          pTownNames[pHistory.ubSecondCode],
        );
        break;
      ...
      case Enum83.HISTORY_MERC_MARRIED_OFF:
        pString = swprintf(
          pHistoryStrings[Enum83.HISTORY_MERC_MARRIED_OFF],
          gMercProfiles[pHistory.ubSecondCode].zNickname,
        );
        break;
      case Enum83.HISTORY_MERC_CONTRACT_EXPIRED:
        pString = swprintf(
          pHistoryStrings[Enum83.HISTORY_MERC_CONTRACT_EXPIRED],
          gMercProfiles[pHistory.ubSecondCode].zName,
        );
        break;
      case Enum83.HISTORY_RPC_JOINED_TEAM:
        pString = swprintf(
          pHistoryStrings[Enum83.HISTORY_RPC_JOINED_TEAM],
          gMercProfiles[pHistory.ubSecondCode].zName,
        );
        break;
      ...
      case Enum83.HISTORY_MINE_REOPENED:
        // all the same format
        pString = swprintf(
          pHistoryStrings[pHistory.ubCode],
          pTownNames[pHistory.ubSecondCode],
        );
        break;
  ...
  function LoadInHistoryRecords(uiPage: UINT32): boolean {
    ...
    if (
      Math.trunc(
        (FileGetSize(hFileHandle) - 1) /
          (NUM_RECORDS_PER_PAGE * SIZE_OF_HISTORY_FILE_RECORD),
      ) +
        1 <
      uiPage
    ) {
    ...
    FileSeek(
      hFileHandle,
      (uiPage - 1) * NUM_RECORDS_PER_PAGE * SIZE_OF_HISTORY_FILE_RECORD,
  ...
  function WriteOutHistoryRecords(uiPage: UINT32): boolean {
    ...
    hFileHandle = FileOpen(
      HISTORY_DATA_FILE,
      FILE_OPEN_EXISTING | FILE_ACCESS_WRITE,
      false,
    );
    ...
    FileSeek(
      hFileHandle,
      4 + (uiPage - 1) * NUM_RECORDS_PER_PAGE * SIZE_OF_HISTORY_FILE_RECORD,
      FILE_SEEK_FROM_START,
    );

    uiByteCount =
      /*sizeof( INT32 )+ */ (uiPage - 1) *
      NUM_RECORDS_PER_PAGE *
      SIZE_OF_HISTORY_FILE_RECORD;
    // file exists, read in data, continue until end of page
    buffer = Buffer.allocUnsafe(SIZE_OF_HISTORY_FILE_RECORD);
    while (iCount < NUM_RECORDS_PER_PAGE && fOkToContinue) {
  ...
  function SetLastPageInHistoryRecords(): void {
    ...
    guiLastPageInHistoryRecordsList = Math.trunc(
  ...
  export function ResetHistoryFact(
    ubCode: UINT8,
    sSectorX: INT16,
    sSectorY: INT16,
  ): void {
    ...
    while (pList) {
      if (
        pList.ubSecondCode == ubCode &&
        pList.ubCode == Enum83.HISTORY_QUEST_STARTED
      ) {
        // reset color
        pList.ubColor = 0;
        fFound = true;
