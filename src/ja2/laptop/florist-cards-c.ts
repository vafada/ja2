namespace ja2 {
  ...
  const FLORIST_CARD_TITLE_SENTENCE_WIDTH =
  ...
  let gSelectedFloristCardsRegion: MOUSE_REGION[] /* [9] */ = createArrayFrom(
    9,
    createMouseRegion,
  );
  ...
  export function EnterFloristCards(): boolean {
    ...
    for (j = 0; j < 3; j++) {
      ...
      for (i = 0; i < 3; i++) {
        MSYS_DefineRegion(
          gSelectedFloristCardsRegion[ubCount],
          usPosX,
          usPosY,
          usPosX + FLORIST_CARD_CARD_WIDTH,
          usPosY + FLORIST_CARD_CARD_HEIGHT,
          MSYS_PRIORITY_HIGH,
          Enum317.CURSOR_WWW,
          MSYS_NO_CALLBACK,
    ...
    guiFlowerCardsButtonImage = LoadButtonImage(
    ...
    guiFlowerCardsBackButton = CreateIconAndTextButton(
      ...
      sFloristCards[Enum348.FLORIST_CARDS_BACK],
  ...
  export function RenderFloristCards(): void {
    ...
    DrawTextToScreen(
      sFloristCards[Enum348.FLORIST_CARDS_CLICK_SELECTION],
    ...
    for (j = 0; j < 3; j++) {
      ...
      for (i = 0; i < 3; i++) {
        ...
        usHeightOffset = Math.trunc(
          (FLORIST_CARD_TEXT_HEIGHT - usHeightOffset) / 2,
        );

        IanDisplayWrappedString(
          usPosX + 7,
          usPosY + 10 + usHeightOffset,
          FLORIST_CARD_TEXT_WIDTH,
          2,
          FLORIST_CARDS_SENTENCE_FONT(),
          FLORIST_CARDS_SENTENCE_COLOR,
  ...
  function SelectFloristCardsRegionCallBack(
  ...
  function BtnFlowerCardsBackButtonCallback(
    btn: GUI_BUTTON,
    reason: INT32,
  ): void {
