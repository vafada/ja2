namespace ja2 {
  ...
  function CreateConfirmButtons(): void {
    ...
    giIMPConfirmButtonImage[0] = LoadButtonImage(
      "LAPTOP\\button_2.sti",
      -1,
      0,
      -1,
    ...
    giIMPConfirmButtonImage[1] = LoadButtonImage(
      "LAPTOP\\button_2.sti",
      -1,
      0,
      -1,
  ...
  function AddCharacterToPlayersTeam(): boolean {
    ...
    SetProfileFaceData(
      HireMercStruct.ubProfileID,
      200 + iPortraitNumber,
      uiEyeXPositions[iPortraitNumber],
      uiEyeYPositions[iPortraitNumber],
      uiMouthXPositions[iPortraitNumber],
      uiMouthYPositions[iPortraitNumber],
    );
    ...
    if (reason & MSYS_CALLBACK_REASON_LBUTTON_DWN) {
      ...
      if (btn.uiFlags & BUTTON_CLICKED_ON) {
        ...
        AddHistoryToPlayersLog(
          Enum83.HISTORY_CHARACTER_GENERATED,
          0,
          GetWorldTotalMin(),
          -1,
          -1,
        ...
        AddFutureDayStrategicEvent(
          Enum132.EVENT_DAY2_ADD_EMAIL_FROM_IMP,
          60 * 7,
          0,
          2,
        );
...
*/
  ...
  const PROFILE_HAS_SKILL_TRAIT = (p: MERCPROFILESTRUCT, t: number) =>
    p.bSkillTrait == t || p.bSkillTrait2 == t;
  ...
  function MakeProfileInvItemAnySlot(
  ...
  function MakeProfileInvItemThisSlot(
    pProfile: MERCPROFILESTRUCT,
    uiPos: UINT32,
    usItem: UINT16,
    ubStatus: UINT8,
  ...
  function FirstFreeBigEnoughPocket(
    pProfile: MERCPROFILESTRUCT,
    usItem: UINT16,
  ): INT32 {
    let uiPos: UINT32;
    ...
    if (Item[usItem].ubPerPocket != 0) {
      ...
      for (
        uiPos = Enum261.SMALLPOCK1POS;
        uiPos <= Enum261.SMALLPOCK8POS;
        uiPos++
      ) {
  ...
  function WriteOutCurrentImpCharacter(iProfileId: INT32): void {
    ...
    if (
      (uiBytesWritten = FileWrite(hFile, buffer, MERC_PROFILE_STRUCT_SIZE)) ===
      -1
    ) {
      return;
    }
  ...
  function LoadInCurrentImpCharacter(): void {
    ...
    buffer = Buffer.allocUnsafe(MERC_PROFILE_STRUCT_SIZE);
    if (
      (uiBytesRead = FileRead(hFile, buffer, MERC_PROFILE_STRUCT_SIZE)) === -1
    ) {
      return;
    }
  ...
  export function ResetIMPCharactersEyesAndMouthOffsets(
    ubMercProfileID: UINT8,
  ): void {
    // ATE: Check boundary conditions!
    if (
      gMercProfiles[ubMercProfileID].ubFaceIndex - 200 > 16 ||
      ubMercProfileID >= Enum268.PROF_HUMMER
    ) {
      return;
    }
