namespace ja2 {
  ...
  const MAX_NUMBER_OLD_MERCS_ON_PAGE =
  ...
  const AIM_ALUMNI_NAME_WIDTH =
    AIM_ALUMNI_ALUMNI_FRAME_WIDTH - AIM_ALUMNI_NAME_OFFSET_X * 2;
  ...
  const AIM_ALUMNI_PAGE_END_X =
    AIM_ALUMNI_PAGE1_X +
    (BOTTOM_BUTTON_START_WIDTH + BOTTOM_BUTTON_START_WIDTH) * 3;
  ...
  const AIM_POPUP_X =
    LAPTOP_SCREEN_UL_X + Math.trunc((500 - AIM_POPUP_WIDTH) / 2);
  ...
  const AIM_ALUMNI_POPUP_NAME_X =
    AIM_ALUMNI_FACE_PANEL_X + AIM_ALUMNI_FACE_PANEL_WIDTH + 10;
  const AIM_ALUMNI_POPUP_NAME_Y = AIM_ALUMNI_FACE_PANEL_Y + 20;

  const AIM_ALUMNI_POPUP_DESC_X = AIM_POPUP_X + 8;
  const AIM_ALUMNI_POPUP_DESC_Y =
    AIM_ALUMNI_FACE_PANEL_Y + AIM_ALUMNI_FACE_PANEL_HEIGHT + 5;

  const AIM_ALUMNI_DONE_WIDTH = 36;
  const AIM_ALUMNI_DONE_X =
    AIM_POPUP_X + AIM_POPUP_WIDTH - AIM_ALUMNI_DONE_WIDTH - 7;
  ...
  let gMercAlumniFaceMouseRegions: MOUSE_REGION[] /* [MAX_NUMBER_OLD_MERCS_ON_PAGE] */ =
    createArrayFrom(MAX_NUMBER_OLD_MERCS_ON_PAGE, createMouseRegion);
  ...
  export function EnterAimArchives(): boolean {
    ...
    guiAlumniPageButtonImage = LoadButtonImage(
    ...
    for (i = 0; i < 3; i++) {
      guiAlumniPageButton[i] = CreateIconAndTextButton(
        ...
        AimAlumniText[i],
        AIM_ALUMNI_PAGE_FONT(),
        AIM_ALUMNI_PAGE_COLOR_UP,
        DEFAULT_SHADOW,
        AIM_ALUMNI_PAGE_COLOR_DOWN,
        DEFAULT_SHADOW,
        TEXT_CJUSTIFIED,
        usPosX,
        AIM_ALUMNI_PAGE1_Y,
  ...
  export function RenderAimArchives(): void {
    ...
    // Draw Link Title
    DrawTextToScreen(
      AimAlumniText[Enum361.AIM_ALUMNI_ALUMNI],
      AIM_ALUMNI_TITLE_X,
      AIM_ALUMNI_TITLE_Y,
      AIM_ALUMNI_TITLE_WIDTH,
      AIM_ALUMNI_TITLE_FONT(),
      AIM_ALUMNI_TITLE_COLOR,
      FONT_MCOLOR_BLACK,
      false,
      CENTER_JUSTIFIED,
    );
    ...
    for (y = 0; y < ubNumRows; y++) {
      for (x = 0; x < AIM_ALUMNI_NUM_FACE_COLS; x++) {
        ...
        sText = LoadEncryptedDataFromFile(
          AIM_ALUMNI_NAME_FILE,
          uiStartLoc,
          AIM_ALUMNI_NAME_SIZE,
        );
        DrawTextToScreen(
          sText,
          usPosX + AIM_ALUMNI_NAME_OFFSET_X,
          usPosY + AIM_ALUMNI_NAME_OFFSET_Y,
          AIM_ALUMNI_NAME_WIDTH,
          AIM_ALUMNI_NAME_FONT(),
          AIM_ALUMNI_NAME_COLOR,
          FONT_MCOLOR_BLACK,
          false,
          CENTER_JUSTIFIED,
        );
  ...
  function SelectAlumniFaceRegionCallBack(
  ...
  function DisplayAlumniOldMercPopUp(): void {
    ...
    uiStartLoc =
      AIM_ALUMNI_FILE_RECORD_SIZE * gubDrawOldMerc + AIM_ALUMNI_FULL_NAME_SIZE;
    sDesc = LoadEncryptedDataFromFile(
      AIM_ALUMNI_FILE,
      uiStartLoc,
      AIM_ALUMNI_DECRIPTION_SIZE,
    );
    ...
    ShadowVideoSurfaceRect(
      FRAME_BUFFER,
      AIM_POPUP_X + AIM_POPUP_SHADOW_GAP,
    ...
    DrawTextToScreen(
      AimAlumniText[Enum361.AIM_ALUMNI_DONE],
      AIM_ALUMNI_DONE_X + 1,
      usPosY - AIM_ALUMNI_DONE_HEIGHT + 3,
      AIM_ALUMNI_DONE_WIDTH,
      AIM_ALUMNI_POPUP_NAME_FONT(),
      AIM_ALUMNI_POPUP_NAME_COLOR,
      FONT_MCOLOR_BLACK,
      false,
      CENTER_JUSTIFIED,
    );
    ...
    sName = LoadEncryptedDataFromFile(
      AIM_ALUMNI_FILE,
      uiStartLoc,
      AIM_ALUMNI_FULL_NAME_SIZE,
    );

    DrawTextToScreen(
      sName,
      AIM_ALUMNI_POPUP_NAME_X,
      AIM_ALUMNI_POPUP_NAME_Y,
      0,
      AIM_ALUMNI_POPUP_NAME_FONT(),
      AIM_ALUMNI_POPUP_NAME_COLOR,
      FONT_MCOLOR_BLACK,
    ...
    DisplayWrappedString(
      AIM_ALUMNI_POPUP_DESC_X,
      AIM_ALUMNI_POPUP_DESC_Y,
      AIM_POPUP_TEXT_WIDTH,
      2,
      AIM_ALUMNI_POPUP_FONT(),
      AIM_ALUMNI_POPUP_COLOR,
      sDesc,
      FONT_MCOLOR_BLACK,
      false,
      LEFT_JUSTIFIED,
  ...
  function InitAlumniFaceRegions(): void {
    ...
    if (gubPageNum == 2) usNumRows = 2;
    else usNumRows = AIM_ALUMNI_NUM_FACE_ROWS;
    ...
    for (y = 0; y < usNumRows; y++) {
      for (x = 0; x < AIM_ALUMNI_NUM_FACE_COLS; x++) {
        MSYS_DefineRegion(
          gMercAlumniFaceMouseRegions[i],
          usPosX,
          usPosY,
          usPosX + AIM_ALUMNI_ALUMNI_FACE_WIDTH,
          usPosY + AIM_ALUMNI_ALUMNI_FACE_HEIGHT,
          MSYS_PRIORITY_HIGH,
          Enum317.CURSOR_WWW,
          MSYS_NO_CALLBACK,
  ...
  /* static */ let CreateDestroyDoneMouseRegion__DoneRegionCreated: boolean =
    false;
  ...
  function SelectAlumniDoneRegionCallBack(
    pRegion: MOUSE_REGION,
    iReason: INT32,
  ): void {
    if (iReason & MSYS_CALLBACK_REASON_INIT) {
