namespace ja2 {
  ...
  function DisplayActivationStringCursor(): void {
    ...
    LineDraw(
      ...
      Get16BPPColor(
        FROMRGB(
          GlowColorsList[DisplayActivationStringCursor__iCurrentState][0],
          GlowColorsList[DisplayActivationStringCursor__iCurrentState][1],
          GlowColorsList[DisplayActivationStringCursor__iCurrentState][2],
        ),
      ),
      pDestBuf,
    );
...
*/
    ...
    switch (uiKey) {
      case BACKSPACE:
        if (iStringPos >= 0) {
          ...
          // null out char
          pPlayerActivationString = pPlayerActivationString.substring(
            0,
            iStringPos,
          );
          ...
          // move back cursor
          uiCursorPosition =
            StringPixLength(pPlayerActivationString, FONT14ARIAL()) +
            IMP_PLAYER_ACTIVATION_STRING_X;
  ...
  function ProcessPlayerInputActivationString(): void {
    ...
    if (
      (pPlayerActivationString == "XEP624" ||
        pPlayerActivationString == "xep624") &&
      LaptopSaveInfo.fIMPCompletedFlag == false &&
      Number(LaptopSaveInfo.gfNewGameLaptop) < 2
    ) {
  ...
  */
  ...
  function CreateIMPHomePageButtons(): void {
    ...
    // ths about us button
    giIMPHomePageButtonImage[0] = LoadButtonImage(
      "LAPTOP\\button_1.sti",
      -1,
      0,
      -1,
