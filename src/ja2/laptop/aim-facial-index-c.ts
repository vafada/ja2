namespace ja2 {
  ...
  let guiAimFiFace: UINT32[] /* [MAX_NUMBER_MERCS] */ = createArray(
    MAX_NUMBER_MERCS,
    0,
  );
  ...
  let gMercFaceMouseRegions: MOUSE_REGION[] /* [MAX_NUMBER_MERCS] */ =
    createArrayFrom(MAX_NUMBER_MERCS, createMouseRegion);
  ...
  export function EnterAimFacialIndex(): boolean {
    ...
    for (y = 0; y < AIM_FI_NUM_MUHSHOTS_Y; y++) {
      for (x = 0; x < AIM_FI_NUM_MUHSHOTS_X; x++) {
        MSYS_DefineRegion(
          gMercFaceMouseRegions[i],
          usPosX,
          usPosY,
          usPosX + AIM_FI_PORTRAIT_WIDTH,
          usPosY + AIM_FI_PORTRAIT_HEIGHT,
          MSYS_PRIORITY_HIGH,
          Enum317.CURSOR_WWW,
          SelectMercFaceMoveRegionCallBack,
        ...
        sTemp = sprintf(
          "%s%s.sti",
          sFaceLoc,
          AimMercArray[i].toString().padStart(2, "0"),
        );
        VObjectDesc.fCreateFlags = VOBJECT_CREATE_FROMFILE;
        VObjectDesc.ImageFile = FilenameForBPP(sTemp);
        if (!(guiAimFiFace[i] = AddVideoObject(VObjectDesc))) return false;
  ...
  export function RenderAimFacialIndex(): boolean {
    ...
    if (gubCurrentListMode == AIM_ASCEND)
      sString = swprintf(
        AimFiText[Enum360.AIM_FI_AIM_MEMBERS_SORTED_ASCENDING],
        AimFiText[gubCurrentSortMode],
      );
    else
      sString = swprintf(
        AimFiText[Enum360.AIM_FI_AIM_MEMBERS_SORTED_DESCENDING],
        AimFiText[gubCurrentSortMode],
      );

    DrawTextToScreen(
      sString,
      AIM_FI_MEMBER_TEXT_X,
      AIM_FI_MEMBER_TEXT_Y,
      AIM_FI_MEMBER_TEXT_WIDTH,
      AIM_MAINTITLE_FONT(),
      AIM_MAINTITLE_COLOR,
      FONT_MCOLOR_BLACK,
      false,
    ...
    for (y = 0; y < AIM_FI_NUM_MUHSHOTS_Y; y++) {
      for (x = 0; x < AIM_FI_NUM_MUHSHOTS_X; x++) {
        ...
        DrawTextToScreen(
          gMercProfiles[AimMercArray[i]].zNickname,
          usPosX - AIM_FI_NNAME_OFFSET_X,
          usPosY + AIM_FI_NNAME_OFFSET_Y,
          AIM_FI_NNAME_WIDTH,
          AIM_FONT12ARIAL(),
    ...
    DrawTextToScreen(
      AimFiText[Enum360.AIM_FI_LEFT_CLICK],
    ...
    DrawTextToScreen(
      AimFiText[Enum360.AIM_FI_TO_SELECT],
      AIM_FI_LEFT_CLICK_TEXT_X,
    ...
    DrawTextToScreen(
      AimFiText[Enum360.AIM_FI_RIGHT_CLICK],
    ...
    DrawTextToScreen(
      AimFiText[Enum360.AIM_FI_TO_ENTER_SORT_PAGE],
      AIM_FI_RIGHT_CLICK_TEXT_X,
      AIM_FI_LEFT_CLICK_TEXT_Y + AIM_FI_CLICK_DESC_TEXT_Y_OFFSET,
      AIM_FI_CLICK_TEXT_WIDTH,
      AIM_FI_HELP_FONT(),
  ...
  function SelectMercFaceRegionCallBack(
  ...
  function SelectScreenRegionCallBack(
    pRegion: MOUSE_REGION,
    iReason: INT32,
  ): void {
    if (iReason & MSYS_CALLBACK_REASON_INIT) {
  ...
  function SelectMercFaceMoveRegionCallBack(
    pRegion: MOUSE_REGION,
    reason: INT32,
  ): void {
    let ubMercNum: UINT8;
    ...
    usPosY =
      AIM_FI_FIRST_MUGSHOT_Y +
      (AIM_FI_PORTRAIT_HEIGHT + AIM_FI_MUGSHOT_GAP_Y) * usPosY;

    usPosX = ubMercNum % AIM_FI_NUM_MUHSHOTS_X;
    usPosX =
      AIM_FI_FIRST_MUGSHOT_X +
      (AIM_FI_PORTRAIT_WIDTH + AIM_FI_MUGSHOT_GAP_X) * usPosX;
  ...
  function DrawMercsFaceToScreen(
    ubMercID: UINT8,
    usPosX: UINT16,
    usPosY: UINT16,
    ubImage: UINT8,
    ...
    if (IsMercDead(AimMercArray[ubMercID])) {
      ...
      DrawTextToScreen(
        AimFiText[Enum360.AIM_FI_DEAD],
        usPosX + AIM_FI_AWAY_TEXT_OFFSET_X,
        usPosY + AIM_FI_AWAY_TEXT_OFFSET_Y,
        AIM_FI_AWAY_TEXT_OFFSET_WIDTH,
        FONT10ARIAL(),
        145,
        FONT_MCOLOR_BLACK,
        false,
    ...
    // else if the merc is currently a POW or, the merc was fired as a pow
      ...
      gMercProfiles[AimMercArray[ubMercID]].bMercStatus ==
        MERC_FIRED_AS_A_POW ||
      ...
      ShadowVideoSurfaceRect(
        ...
        usPosX + AIM_FI_FACE_OFFSET,
        usPosY + AIM_FI_FACE_OFFSET,
        usPosX + 48 + AIM_FI_FACE_OFFSET,
        usPosY + 43 + AIM_FI_FACE_OFFSET,
      );
      DrawTextToScreen(
        pPOWStrings[0],
        usPosX + AIM_FI_AWAY_TEXT_OFFSET_X,
    ...
    // if the merc is away, shadow his/her face and blit 'away' over top
      ...
      DrawTextToScreen(
        AimFiText[Enum360.AIM_FI_DEAD + 1],
        usPosX + AIM_FI_AWAY_TEXT_OFFSET_X,
        usPosY + AIM_FI_AWAY_TEXT_OFFSET_Y,
        AIM_FI_AWAY_TEXT_OFFSET_WIDTH,
        FONT10ARIAL(),
