namespace ja2 {
  ...
  function FindAutobandageClimbPoint(
    ...
    for (ubLoop = 0; ubLoop < ubNumClimbSpots; ubLoop++) {
      ...
      if (
        ubWhoIsThere != NOBODY &&
        !CanCharacterAutoBandageTeammate(MercPtrs[ubWhoIsThere])
      ) {
        continue;
      }
  ...
  export function CanCharacterAutoBandageTeammate(
  ...
  // can this soldier autobandage others in sector
  export function CanCharacterBeAutoBandagedByTeammate(
    pSoldier: SOLDIERTYPE,
  ): boolean {
    // if the soldier isn't active or in sector, we have problems..leave
    if (
  ...
  function FindBestPatient(
    pSoldier: SOLDIERTYPE,
    pfDoClimb: Pointer<boolean>,
  ): INT8 {
    let cnt: UINT8;
    let cnt2: UINT8;
    ...
    for (
      pPatient = MercPtrs[cnt];
      cnt <= gTacticalStatus.Team[OUR_TEAM].bLastID;
      cnt++, pPatient = MercPtrs[cnt]
    ) {
      ...
      if (
        ...
        if (bPatientPriority >= bBestPriority) {
          if (!ClimbingNecessary(pSoldier, pPatient.sGridNo, pPatient.bLevel)) {
            ...
            sAdjacentGridNo = FindAdjacentGridEx(
              pSoldier,
              sPatientGridNo,
            ...
            if (
              ...
              for (cnt2 = 0; cnt2 < Enum245.NUM_WORLD_DIRECTIONS; cnt2++) {
                ...
                if (
                  WhoIsThere2(sPatientGridNo, pPatient.bLevel) == pPatient.ubID
                ) {
            ...
            if (sAdjacentGridNo != -1) {
              ...
              if (sPathCost != 0) {
                ...
                if (
                  pPatient.ubAutoBandagingMedic != NOBODY &&
                  ...
                  sOtherAdjacentGridNo = FindAdjacentGridEx(
                    pOtherMedic,
                    sPatientGridNo,
                    ubDirection__Pointer,
                    sAdjustedGridNo__Pointer,
                  ...
                  if (sOtherAdjacentGridNo != -1) {
                    if (sOtherAdjacentGridNo == pOtherMedic.sGridNo) {
                      ...
                      sOtherMedicPathCost = PlotPath(
                        pOtherMedic,
                        sOtherAdjacentGridNo,
                        NO_COPYROUTE,
                        NO_PLOT,
                        TEMPORARY,
                        Enum193.RUNNING,
  ...
  export function DecideAutoBandage(pSoldier: SOLDIERTYPE): INT8 {
    ...
    pSoldier.bAction = FindBestPatient(
      pSoldier,
      createPointer(
        () => fDoClimb,
        (v) => (fDoClimb = v),
