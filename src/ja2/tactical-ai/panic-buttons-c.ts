namespace ja2 {
  export function MakeClosestEnemyChosenOne(): void {
    ...
    for (cnt = 0; cnt < guiNumMercSlots; cnt++) {
      ...
      if (!pSoldier) {
        // if this merc is inactive, or not here
        continue;
      }
  ...
  export function PossiblyMakeThisEnemyChosenOne(pSoldier: SOLDIERTYPE): void {
    ...
    uiPercentEnemiesKilled = Math.trunc(
      (100 * gTacticalStatus.ubArmyGuysKilled) /
        (gTacticalStatus.Team[ENEMY_TEAM].bMenInSector +
          gTacticalStatus.ubArmyGuysKilled),
    );
    if (
      gTacticalStatus.ubPanicTolerance[bPanicTrigger] > uiPercentEnemiesKilled
    ) {
      // not yet... not yet
      return;
    }
  ...
  export function PanicAI(
    pSoldier: SOLDIERTYPE,
    ubCanMove: boolean /* UINT8 */,
  ): INT8 {
    ...
    if (gTacticalStatus.fPanicFlags & PANIC_TRIGGERS_HERE) {
      ...
      if (pSoldier.ubID == gTacticalStatus.ubTheChosenOne) {
         ...
         *** COMMENTED OUT BECAUSE WE DON'T HAVE SUPPORT ROUTINES YET
          ...
          if (pSoldier.sGridNo == sPanicTriggerGridNo) {
          ...
          } // we are NOT at the HandGrid spot
          else {
            ...
            if (ubCanMove) {
              ...
              } // Oh oh, the chosen one can't get to the trigger!
              else {
                gTacticalStatus.ubTheChosenOne = NOBODY; // strip him of his Chosen One status
                MakeClosestEnemyChosenOne(); // and replace him!
              }
            } // can't move, wait 1 turn
            else {
              pSoldier.usActionData = NOWHERE;
              return Enum289.AI_ACTION_NONE;
            }
    ...
    for (bLoop = 0; bLoop < NUM_PANIC_TRIGGERS; bLoop++) {
      if (gTacticalStatus.sPanicTriggerGridNo[bLoop] != NOWHERE) {
        ...
        if (gWorldSectorX == TIXA_SECTOR_X && gWorldSectorY == TIXA_SECTOR_Y) {
          ...
          if (
            bLoop > 0 &&
            gTacticalStatus.sPanicTriggerGridNo[bLoop - 1] != NOWHERE
          ) {
            break;
          }
        }

        sDistance = PythSpacesAway(
          pSoldier.sGridNo,
          gTacticalStatus.sPanicTriggerGridNo[bLoop],
        );
  ...
  export function HeadForTheStairCase(pSoldier: SOLDIERTYPE): INT8 {
    ...
    if (
      pBasementInfo &&
      pBasementInfo.uiTimeCurrentSectorWasLastLoaded != 0 &&
      pBasementInfo.ubNumElites +
        pBasementInfo.ubNumTroops +
        pBasementInfo.ubNumAdmins <
        5
    ) {
      return Enum289.AI_ACTION_NONE;
    }
    ...
    if (PythSpacesAway(pSoldier.sGridNo, STAIRCASE_GRIDNO) < 2) {
      ...
      if (
        LegalNPCDestination(pSoldier, STAIRCASE_GRIDNO, ENSURE_PATH, WATEROK, 0)
      ) {
