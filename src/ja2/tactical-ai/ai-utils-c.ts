namespace ja2 {
  ...
  export function OKToAttack(pSoldier: SOLDIERTYPE, target: number): INT8 {
    ...
    if (Item[pSoldier.inv[Enum261.HANDPOS].usItem].usItemClass == IC_GUN) {
      ...
      Item[pSoldier.inv[Enum261.HANDPOS].usItem].usItemClass == IC_LAUNCHER
    ) {
      if (
        FindLaunchable(pSoldier, pSoldier.inv[Enum261.HANDPOS].usItem) ==
        NO_SLOT
      ) {
        // if ( !ItemHasAttachments( &(pSoldier->inv[HANDPOS]) ) )
        return NOSHOOT_NOLOAD;
  ...
  export function StanceChange(
    pSoldier: SOLDIERTYPE,
    ubAttackAPCost: UINT8,
  ): UINT8 {
    ...
    if (PTR_STANDING(pSoldier)) {
      if (pSoldier.bActionPoints - ubAttackAPCost >= AP_CROUCH) {
        if (
          pSoldier.bActionPoints - ubAttackAPCost >= AP_CROUCH + AP_PRONE &&
          IsValidStance(pSoldier, ANIM_PRONE) &&
          ConsiderProne(pSoldier)
        ) {
          return ANIM_PRONE;
      ...
      if (
        pSoldier.bActionPoints - ubAttackAPCost >= AP_PRONE &&
        IsValidStance(pSoldier, ANIM_PRONE) &&
        ConsiderProne(pSoldier)
      ) {
        return ANIM_PRONE;
  ...
  export function ShootingStanceChange(
    pSoldier: SOLDIERTYPE,
    pAttack: ATTACKTYPE,
    ...
    for (bLoop = 0; bLoop < 3; bLoop++) {
      ...
      switch (bLoop) {
        ...
        case 1:
          if (
            !InternalIsValidStance(pSoldier, bDesiredDirection, ANIM_CROUCH)
          ) {
            continue;
          }
    ...
    if (
      bBestStanceDiff == 0 ||
      Math.trunc(
        (uiBestChanceOfDamage - uiCurrChanceOfDamage) / bBestStanceDiff,
  ...
  export function DetermineMovementMode(
    ...
    if (pSoldier.fUIMovementFast) {
      ...
      if (
        ...
        MovementMode[bAction][
          Urgency[pSoldier.bAlertStatus][pSoldier.bAIMorale]
        ] == Enum193.RUNNING
      ) {
        return Enum193.WALKING;
      } else if (
        bAction == Enum289.AI_ACTION_SEEK_NOISE &&
  ...
  export function NewDest(pSoldier: SOLDIERTYPE, usGridNo: UINT16): void {
    ...
    if (
      IS_MERC_BODY_TYPE(pSoldier) &&
      pSoldier.bAction == Enum289.AI_ACTION_TAKE_COVER &&
      (pSoldier.bOrders == Enum242.DEFENSIVE ||
        pSoldier.bOrders == Enum242.CUNNINGSOLO ||
        pSoldier.bOrders == Enum242.CUNNINGAID) &&
      ...
      usMovementMode =
        MovementMode[Enum289.AI_ACTION_TAKE_COVER][
          Urgency[pSoldier.bAlertStatus][pSoldier.bAIMorale]
        ];
      if (usMovementMode != Enum193.SWATTING) {
        ...
        if (
          EnoughPoints(
            pSoldier,
            RecalculatePathCost(pSoldier, Enum193.SWATTING),
            0,
            false,
          ) ||
          (pSoldier.bLastAction == Enum289.AI_ACTION_TAKE_COVER &&
        ...
        case AI_ACTION_RUN_AWAY:
          ...
          default:
            if (PreRandom(5 - SoldierDifficultyLevel(pSoldier)) == 0) {
              let sClosestNoise: INT16 = MostImportantNoiseHeard(
                pSoldier,
                null,
                null,
                null,
              ...
              if (
                sClosestNoise != NOWHERE &&
                PythSpacesAway(pSoldier.sGridNo, sClosestNoise) <
                  MaxDistanceVisible() + 10
              ) {
                pSoldier.usUIMovementMode = Enum193.SWATTING;
                fSet = true;
              }
  ...
  export function IsActionAffordable(pSoldier: SOLDIERTYPE): boolean {
    ...
    switch (pSoldier.bAction) {
      ...
      case Enum289.AI_ACTION_THROW_KNIFE:
        ...
        bMinPointsNeeded = MinAPsToAttack(
          pSoldier,
  ...
  */
    ...
    for (uiLoop = 0; uiLoop < guiNumMercSlots; uiLoop++) {
      ...
      if (
        (!pFriend.bNeutral && pSoldier.bSide == pFriend.bSide) ||
    ...
    while (ubFriendCount && !fFound) {
      ...
      if (
        ...
        while (ubDirsLeft-- && !fFound) {
          ...
          usDest = NewGridNo(
            Menptr[ubFriendID].sGridNo,
            DirectionInc(usDirection + 1),
          );
          ...
          if (!fRangeRestricted || SpacesAway(usOrigin, usDest) <= usMaxDist) {
            if (
              LegalNPCDestination(pSoldier, usDest, ENSURE_PATH, NOWATER, 0)
            ) {
  ...
  export function RandDestWithinRange(pSoldier: SOLDIERTYPE): INT16 {
    ...
    if (pSoldier.ubBodyType == Enum194.LARVAE_MONSTER) {
      ...
      while (ubTriesLeft-- && !fFound) {
        ...
        if (
          !LegalNPCDestination(pSoldier, sRandDest, ENSURE_PATH, NOWATER, 0)
        ) {
          sRandDest = NOWHERE;
          continue; // try again!
        }
      ...
      while (ubTriesLeft-- && !fFound) {
        ...
        if (
          ubRoom &&
          (ubTempRoom = InARoom(sRandDest)) !== -1 &&
          ubTempRoom != ubRoom
        ) {
  ...
  export function ClosestReachableDisturbance(
    pSoldier: SOLDIERTYPE,
    ubUnconsciousOK: UINT8,
    pfChangeLevel: Pointer<boolean>,
  ): INT16 {
    let psLastLoc: INT16;
    ...
    let fClimbingNecessary__Pointer = createPointer(
      () => fClimbingNecessary,
    ...
    let sClimbGridNo__Pointer = createPointer(
      () => sClimbGridNo,
      (v) => (sClimbGridNo = v),
    );
    ...
    if (sClosestEnemy != NOWHERE) {
      iPathCost = EstimatePathCostToLocation(
        pSoldier,
        sClosestEnemy,
        bClosestLevel,
        false,
        fClimbingNecessary__Pointer,
        sClimbGridNo__Pointer,
      );
      // if we can get there
      if (iPathCost != 0) {
    ...
    if (
      ...
      pSoldier.sNoiseGridno != sClosestDisturbance
    ) {
    ...
    if (
      pusNoiseGridNo.value != NOWHERE &&
      pusNoiseGridNo.value != sClosestDisturbance
    ) {
      // test this gridno, too
      ...
      if (
        pSoldier.bLevel != bLevel ||
        PythSpacesAway(pSoldier.sGridNo, sGridNo) >= 6 ||
  ...
  export function ClosestKnownOpponent(
    pSoldier: SOLDIERTYPE,
    psGridNo: Pointer<INT16> | null,
    pbLevel: Pointer<INT8> | null,
  ): INT16 {
    let sGridNo: INT16;
    ...
    for (uiLoop = 0; uiLoop < guiNumMercSlots; uiLoop++) {
      ...
      if (
        ...
        SameBuilding(pSoldier.sGridNo, sGridNo)
      ) {
        continue;
      }
  ...
  export function ClosestSeenOpponent(
    pSoldier: SOLDIERTYPE,
    psGridNo: Pointer<INT16> | null,
    pbLevel: Pointer<INT8> | null,
  ): INT16 {
  ...
  export function ClosestPC(
    pSoldier: SOLDIERTYPE,
    psDistance: Pointer<INT16> | null,
  ): INT16 {
    ...
    for (; ubLoop <= gTacticalStatus.Team[gbPlayerNum].bLastID; ubLoop++) {
      ...
      if (
        pTargetSoldier.bTeam != pSoldier.bTeam &&
        pSoldier.bOppList[ubLoop] != SEEN_CURRENTLY
      ) {
        sDist += 10;
      }
  ...
  function FindClosestClimbPointAvailableToAI(
    pSoldier: SOLDIERTYPE,
    sStartGridNo: INT16,
  ...
  export function ClimbingNecessary(
    pSoldier: SOLDIERTYPE,
    sDestGridNo: INT16,
    bDestLevel: INT8,
  ): boolean {
  ...
  export function GetInterveningClimbingLocation(
    pSoldier: SOLDIERTYPE,
    sDestGridNo: INT16,
    bDestLevel: INT8,
    pfClimbingNecessary: Pointer<boolean>,
  ): INT16 {
    if (pSoldier.bLevel == bDestLevel) {
      if (
        pSoldier.bLevel == 0 ||
        ...
        return FindClosestClimbPointAvailableToAI(
          pSoldier,
          pSoldier.sGridNo,
          pSoldier.sGridNo,
          false,
  ...
  export function EstimatePathCostToLocation(
    pSoldier: SOLDIERTYPE,
    sDestGridNo: INT16,
    bDestLevel: INT8,
    fAddCostAfterClimbingUp: boolean,
    pfClimbingNecessary: Pointer<boolean>,
    psClimbGridNo: Pointer<INT16>,
  ): INT16 {
    let sPathCost: INT16;
    ...
    if (pSoldier.bLevel == bDestLevel) {
      if (
        ...
        // different buildings!
        // yes, pass in same gridno twice... want closest climb-down spot for building we are on!
        sClimbGridNo = FindClosestClimbPointAvailableToAI(
          pSoldier,
          pSoldier.sGridNo,
          pSoldier.sGridNo,
          false,
  ...
  function GuySawEnemyThisTurnOrBefore(pSoldier: SOLDIERTYPE): boolean {
    ...
    for (ubTeamLoop = 0; ubTeamLoop < MAXTEAMS; ubTeamLoop++) {
      if (gTacticalStatus.Team[ubTeamLoop].bSide != pSoldier.bSide) {
        ...
        for (
          ubIDLoop = gTacticalStatus.Team[ubTeamLoop].bFirstID;
          ubIDLoop <= gTacticalStatus.Team[ubTeamLoop].bLastID;
          ubIDLoop++
        ) {
  ...
  export function ClosestReachableFriendInTrouble(
    pSoldier: SOLDIERTYPE,
    pfClimbingNecessary: Pointer<boolean>,
  ): INT16 {
    let uiLoop: UINT32;
    ...
    for (uiLoop = 0; uiLoop < guiNumMercSlots; uiLoop++) {
      ...
      if (
        !(
          pFriend.bUnderFire ||
          (pFriend.ubID ==
            gTacticalStatus.Team[pFriend.bTeam].ubLastMercToRadio &&
      ...
      sPathCost = EstimatePathCostToLocation(
        pSoldier,
        pFriend.sGridNo,
        pFriend.bLevel,
        true,
  ...
  export function InWaterGasOrSmoke(
    pSoldier: SOLDIERTYPE,
    sGridNo: INT16,
  ): boolean {
    ...
    // smoke
    if (
      gpWorldLevelData[sGridNo].ubExtFlags[pSoldier.bLevel] &
  ...
  export function CalcMorale(pSoldier: SOLDIERTYPE): INT8 {
    ...
    for (uiLoop = 0; uiLoop < guiNumMercSlots; uiLoop++) {
      ...
      if (pbPersOL == NOT_HEARD_OR_SEEN && pbPublOL == NOT_HEARD_OR_SEEN) {
      ...
      } // decide which opplist is more current
      else {
        // if personal knowledge is more up to date or at least equal
        if (
          gubKnowledgeValue[pbPublOL - OLDEST_HEARD_VALUE][
        ...
        else bMostRecentOpplistValue = pbPublOL; // use public
      }

      iPercent = ThreatPercent[bMostRecentOpplistValue - OLDEST_HEARD_VALUE];

      sOppThreatValue = Math.trunc(
        (iPercent *
      ...
      for (uiLoop2 = 0; uiLoop2 < guiNumMercSlots; uiLoop2++) {
        ...
        iPercent =
          ThreatPercent[pFriend.bOppList[pOpponent.ubID] - OLDEST_HEARD_VALUE];
        ...
        sFrndThreatValue = Math.trunc(
          (iPercent *
            CalcManThreatValue(pFriend, pOpponent.sGridNo, false, pSoldier)) /
            100,
        );
    ...
    if (sMorale <= 25)
      ...
      // odds between 1:4 and 1:2
      bMoraleCategory = Enum244.MORALE_WORRIED;
    else if (sMorale <= 150)
      // odds between 1:2 and 3:2
      bMoraleCategory = Enum244.MORALE_NORMAL;
    else if (sMorale <= 300)
      // odds between 3:2 and 3:1
      bMoraleCategory = Enum244.MORALE_CONFIDENT; // odds better than 3:1
    else bMoraleCategory = Enum244.MORALE_FEARLESS;
    ...
    if (pSoldier.bBreath > 75) bMoraleCategory++;
    else {
    ...
    if (pSoldier.bLife > 75) bMoraleCategory++;
    else {
  ...
  */
    ...
    if (
      bMoraleCategory == Enum244.MORALE_HOPELESS &&
      (pSoldier.bAttitude == Enum242.BRAVESOLO ||
        pSoldier.bAttitude == Enum242.BRAVEAID)
    )
      bMoraleCategory = Enum244.MORALE_WORRIED;
  ...
  export function CalcManThreatValue(
    pEnemy: SOLDIERTYPE,
    sMyGrid: INT16,
    ubReduceForCover: boolean,
    pMe: SOLDIERTYPE,
  ): INT32 {
    ...
    if (
      ...
      !(pEnemy.uiStatusFlags & SOLDIER_BOXER)
    ) {
      iThreatValue = -999;
      return iThreatValue;
    }
    ...
    if (fForCreature) {
      ...
      iThreatValue = Math.trunc(
        (iThreatValue * 10) / (10 + PythSpacesAway(sMyGrid, pEnemy.sGridNo)),
      );
    } else {
      ...
      if (pEnemy.bAssignment < Enum117.ON_DUTY) {
        ...
        if (Item[pEnemy.inv[Enum261.HANDPOS].usItem].usItemClass & IC_WEAPON) {
          ...
          iThreatValue +=
            Weapon[pEnemy.inv[Enum261.HANDPOS].usItem].ubDeadliness;
        }
      }
    ...
    if (sMyGrid != NOWHERE) {
      ...
      if (pEnemy.sLastTarget == sMyGrid) {
        ...
        if (
          pEnemy.bDirection ==
          atan8(
            CenterX(pEnemy.sGridNo),
            CenterY(pEnemy.sGridNo),
            CenterX(sMyGrid),
            CenterY(sMyGrid),
          )
        ) {
  ...
  export function RoamingRange(
    pSoldier: SOLDIERTYPE,
    pusFromGridNo: Pointer<INT16>,
  ): INT16 {
    ...
    if (
      pSoldier.bOrders == Enum241.POINTPATROL ||
      pSoldier.bOrders == Enum241.RNDPTPATROL
    ) {
    ...
    switch (pSoldier.bOrders) {
      ...
      case Enum241.STATIONARY:
        if (
          ...
          (pSoldier.bAlertStatus < Enum243.STATUS_BLACK && !pSoldier.bUnderFire)
        ) {
          return 0;
        } else {
          return 5;
  ...
  export function SoldierDifficultyLevel(pSoldier: SOLDIERTYPE): UINT8 {
    ...
    bDifficultyBase =
      1 + Math.trunc(CalcDifficultyModifier(pSoldier.ubSoldierClass) / 34);
  ...
  export function ValidCreatureTurn(
    pCreature: SOLDIERTYPE,
    bNewDirection: INT8,
  ): boolean {
    let bDirChange: INT8;
    let bTempDir: INT8;
