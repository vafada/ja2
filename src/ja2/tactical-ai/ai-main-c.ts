namespace ja2 {
  ...
  */
    ...
    } else if (!(pSoldier.fAIFlags & AI_HANDLE_EVERY_FRAME)) {
      // if set to handle every frame, ignore delay!
      //#ifndef AI_PROFILING
    ...
    if (
      (pSoldier.uiStatusFlags & SOLDIER_VEHICLE && !TANK(pSoldier)) ||
      AM_A_ROBOT(pSoldier)
    ) {
      // bail out!
    ...
    if (
      ...
      (pSoldier.ubProfile == Enum268.SERGEANT ||
        pSoldier.ubProfile == Enum268.MIKE ||
        ...
        if (pSoldier.bAction == Enum289.AI_ACTION_FIRE_GUN) {
          ...
          if (guiNumObjectSlots == 0) {
            ...
            DebugMsg(
              ...
              DBG_LEVEL_3,
              FormatString(
                ">>>>>> Attack busy count lobotomized due to new situation for %d",
                pSoldier.ubID,
              ),
            );
            gTacticalStatus.ubAttackBusyCount = 0;
    ...
    if (gfTurnBasedAI) {
      if (
        GetJA2Clock() - gTacticalStatus.uiTimeSinceMercAIStart >
          DEADLOCK_DELAY &&
        !gfUIInDeadlock
      ) {
    ...
    if (pSoldier.bAction == Enum289.AI_ACTION_NONE) {
      ...
      if (
        pSoldier.bAction >= FIRST_MOVEMENT_ACTION &&
        pSoldier.bAction <= LAST_MOVEMENT_ACTION &&
        !pSoldier.fDelayedMovement
      ) {
        if (pSoldier.usPathIndex == pSoldier.usPathDataSize) {
          if (pSoldier.sAbsoluteFinalDestination != NOWHERE) {
            if (
              !ACTING_ON_SCHEDULE(pSoldier) &&
              SpacesAway(pSoldier.sGridNo, pSoldier.sAbsoluteFinalDestination) <
                4
            ) {
            ...
            if (pSoldier.sAbsoluteFinalDestination == pSoldier.sGridNo) {
              ...
              if (
                ...
                pSoldier.ubQuoteActionID >=
                  Enum290.QUOTE_ACTION_ID_TRAVERSE_EAST &&
                pSoldier.ubQuoteActionID <=
                  Enum290.QUOTE_ACTION_ID_TRAVERSE_NORTH
              ) {
    ...
    if (gfTurnBasedAI) {
        ...
        if (OK_INSECTOR_MERC(MercPtrs[ubID])) {
          if (MercPtrs[ubID].fCloseCall) {
            if (
              ...
              MercPtrs[ubID].bNumHitsThisTurn == 0 &&
              !(
                MercPtrs[ubID].usQuoteSaidExtFlags &
      ...
      if (
        ...
        if (
          ...
          ubFirstProfile =
            CivilianGroupMembersChangeSidesWithinProximity(pSoldier);
      ...
      if (pSoldier.bActive && pSoldier.bInSector) {
        if (pSoldier.uiStatusFlags & SOLDIER_UNDERAICONTROL) {
          ...
          DebugMsg(
            TOPIC_JA2,
            DBG_LEVEL_3,
            FormatString("Number of bullets in the air is %ld", guiNumBullets),
          );

          DebugMsg(
            TOPIC_JA2,
            DBG_LEVEL_3,
            FormatString("Setting attack busy count to 0 from deadlock break"),
          );
          gTacticalStatus.ubAttackBusyCount = 0;
  ...
  function FindAdjacentSpotBeside(
    pSoldier: SOLDIERTYPE,
    sGridno: INT16,
  ): INT16 {
    let cnt: INT32;
    let mods: INT16[] /* [4] */ = [-1, -MAPWIDTH, 1, MAPWIDTH];
  ...
  function GetMostThreateningOpponent(pSoldier: SOLDIERTYPE): UINT8 {
    ...
    for (uiLoop = 0; uiLoop < guiNumMercSlots; uiLoop++) {
      ...
      if (
        ...
        pTargetSoldier.bSide == pSoldier.bSide
      ) {
        continue;
      }
      ...
      if (
        ...
        pTargetSoldier.ubProfile != 64
      ) {
        continue; // next opponent
      }

      iThreatVal = CalcManThreatValue(
        pTargetSoldier,
        pSoldier.sGridNo,
        true,
        pSoldier,
  ...
  export function FreeUpNPCFromPendingAction(
      ...
      if (
        ...
        if (pSoldier.ubProfile != NO_PROFILE) {
          if (pSoldier.ubQuoteRecord == Enum213.NPC_ACTION_KYLE_GETS_MONEY) {
            ...
            TriggerNPCWithGivenApproach(
              pSoldier.ubProfile,
              Enum296.APPROACH_DONE_GET_ITEM,
              true,
            );
          }
        }
  ...
  export function FreeUpNPCFromTurning(
    pSoldier: SOLDIERTYPE,
    bLook: INT8,
  ): void {
    // if NPC is in the process of changing facing, mark him as being done!
    if (
      pSoldier.bAction == Enum289.AI_ACTION_CHANGE_FACING &&
  ...
  export function FreeUpNPCFromStanceChange(pSoldier: SOLDIERTYPE): void {
    ...
    if (pSoldier.bActionInProgress) {
      ...
      if (
        ...
        pSoldier.bAction == Enum289.AI_ACTION_COWER ||
        pSoldier.bAction == Enum289.AI_ACTION_STOP_COWERING
      ) {
        // yes we were - are we finished?
        if (
          gAnimControl[pSoldier.usAnimState].ubHeight == pSoldier.usActionData
        ) {
  ...
  export let Threat: THREATTYPE[] /* [MAXMERCS] */ = createArrayFrom(
    MAXMERCS,
    createThreatType,
  );
  ...
  export let ThreatPercent: number[] /* [10] */ = [
    20, 40, 60, 80, 25, 100, 90, 75, 60, 45,
  ];
  ...
  export function NPCDoesAct(pSoldier: SOLDIERTYPE): void {
    ...
    if (
      gTacticalStatus.uiFlags & TURNBASED &&
      !(gTacticalStatus.uiFlags & INCOMBAT) &&
      (pSoldier.bAction == Enum289.AI_ACTION_FIRE_GUN ||
        pSoldier.bAction == Enum289.AI_ACTION_TOSS_PROJECTILE ||
        pSoldier.bAction == Enum289.AI_ACTION_KNIFE_MOVE ||
        pSoldier.bAction == Enum289.AI_ACTION_KNIFE_STAB ||
        pSoldier.bAction == Enum289.AI_ACTION_THROW_KNIFE)
    ) {
...
*/
  ...
  export function ActionInProgress(pSoldier: SOLDIERTYPE): boolean {
    ...
    if (
      pSoldier.sFinalDestination != NOWHERE &&
 ...
 */
  ...
  export function RefreshAI(pSoldier: SOLDIERTYPE): void {
    ...
    if (pSoldier.bAlertStatus == Enum243.STATUS_YELLOW) SkipCoverCheck = false;
  ...
  function AIDecideRadioAnimation(pSoldier: SOLDIERTYPE): void {
    if (
      ...
      pSoldier.ubBodyType != Enum194.BIGMALE
    ) {
  ...
  export function ExecuteAction(pSoldier: SOLDIERTYPE): boolean {
    ...
    DebugAI(
      FormatString(
        "%d does %s (a.d. %d) at time %ld",
        pSoldier.ubID,
        gzActionStr[pSoldier.bAction],
    ...
    switch (pSoldier.bAction) {
      ...
      case Enum289.AI_ACTION_RUN:
        if (gfTurnBasedAI && pSoldier.bAlertStatus < Enum243.STATUS_BLACK) {
          ...
          // check for loop
            ...
            pSoldier.usActionData == pSoldier.sLastTwoLocations[1] &&
            pSoldier.sGridNo == pSoldier.sLastTwoLocations[0]
          ) {
            DebugAI(
              FormatString("%d in movement loop, aborting turn", pSoldier.ubID),
            );
          ...
          if (gTacticalStatus.uiFlags & INCOMBAT) {
            if (Random(2) == 0) {
              PlaySoldierJA2Sample(
                pSoldier.ubID,
                Enum330.BLOODCAT_GROWL_1 + Random(4),
                RATE_11025,
      ...
      case Enum289.AI_ACTION_KNIFE_MOVE: // preparing to stab opponent
        if (pSoldier.bAction == Enum289.AI_ACTION_KNIFE_MOVE) {
          // if statement because toss falls through
          pSoldier.usUIMovementMode = DetermineMovementMode(
            pSoldier,
            Enum289.AI_ACTION_KNIFE_MOVE,
          );
        }
      ...
      case Enum289.AI_ACTION_THROW_KNIFE: // throw knife at nearby opponent
        ...
        if (iRetCode != ITEM_HANDLE_OK) {
          if (iRetCode != ITEM_HANDLE_BROKEN) {
            // if the item broke, this is 'legal' and doesn't need reporting
            DebugAI(
              FormatString(
                "AI %d got error code %ld from HandleItem, doing action %d, has %d APs... aborting deadlock!",
                pSoldier.ubID,
                iRetCode,
                pSoldier.bAction,
            ...
            ScreenMsg(
              ...
              MSG_BETAVERSION,
              "AI %d got error code %ld from HandleItem, doing action %d... aborting deadlock!",
              pSoldier.ubID,
              iRetCode,
              pSoldier.bAction,
            );
      ...
      case Enum289.AI_ACTION_PULL_TRIGGER: // activate an adjacent panic trigger
        ...
        if (
          FindStructure(
            pSoldier.sGridNo + DirectionInc(Enum245.NORTH),
            STRUCTURE_SWITCH,
          )
        ) {
      ...
      case Enum289.AI_ACTION_LOCK_DOOR:
        {
          ...
          StartInteractiveObject(
            sDoorGridNo,
            (<STRUCTURE>pStructure).usStructureID,
            pSoldier,
            bDirection,
          );
      ...
      case Enum243.STATUS_GREEN:
        if (!CREATURE_OR_BLOODCAT(pSoldier)) {
          if (
            ...
            pSoldier.ubCivilianGroup != Enum246.QUEENS_CIV_GROUP
          ) {
  ...
  export function HandleInitialRedAlert(
    bTeam: INT8,
    ubCommunicate: boolean /* UINT8 */,
  ): void {
    /*
   if (ubCommunicate)
    {
  ...
  function ManChecksOnFriends(pSoldier: SOLDIERTYPE): void {
    ...
    for (uiLoop = 0; uiLoop < guiNumMercSlots; uiLoop++) {
      ...
      if (PythSpacesAway(pSoldier.sGridNo, pFriend.sGridNo) <= sDistVisible) {
        ...
        if (
          SoldierToSoldierLineOfSightTest(pSoldier, pFriend, sDistVisible, true)
        ) {
          // if my friend is in battle or something is clearly happening there
          if (
            pFriend.bAlertStatus >= Enum243.STATUS_RED ||
            pFriend.bUnderFire ||
            ...
            if (
              pFriend.bAlertStatus == Enum243.STATUS_YELLOW &&
      ...
      if (
        ...
        if (gTacticalStatus.ubAttackBusyCount != 0) {
          DebugMsg(
            ...
            DBG_LEVEL_3,
            FormatString(
              "BBBBBB bNewSituation is set for %d when ABC !=0.",
              pSoldier.ubID,
            ),
          );
        }
