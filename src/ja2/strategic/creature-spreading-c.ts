namespace ja2 {
  ...
  function NewDirective(
    ...
    curr.pLevel = <UNDERGROUND_SECTORINFO>(
      FindUnderGroundSector(ubSectorX, ubSectorY, ubSectorZ)
    );
    if (!curr.pLevel) {
      AssertMsg(
        false,
        FormatString(
          "Could not find underground sector node (%s%db_%d) that should exist.",
  ...
  export function InitCreatureQuest(): void {
    ...
    if (
      gMineStatus[Enum174.DRASSEN_MINE].fAttackedHeadMiner ||
      gMineStatus[Enum174.DRASSEN_MINE].uiOreRunningOutPoint ||
      StrategicMap[SECTOR_INFO_TO_STRATEGIC_INDEX(Enum123.SEC_D13)]
        .fEnemyControlled
    ) {
    ...
    if (
      gMineStatus[Enum174.CAMBRIA_MINE].fAttackedHeadMiner ||
      gMineStatus[Enum174.CAMBRIA_MINE].uiOreRunningOutPoint ||
      StrategicMap[SECTOR_INFO_TO_STRATEGIC_INDEX(Enum123.SEC_H8)]
        .fEnemyControlled
    ) {
      // If head miner was attacked, ore will/has run out, or enemy controlled
    ...
    if (
      gMineStatus[Enum174.ALMA_MINE].fAttackedHeadMiner ||
      gMineStatus[Enum174.ALMA_MINE].uiOreRunningOutPoint ||
      StrategicMap[SECTOR_INFO_TO_STRATEGIC_INDEX(Enum123.SEC_I14)]
        .fEnemyControlled
    ) {
      // If head miner was attacked, ore will/has run out, or enemy controlled
    ...
    if (
      gMineStatus[Enum174.GRUMM_MINE].fAttackedHeadMiner ||
      gMineStatus[Enum174.GRUMM_MINE].uiOreRunningOutPoint ||
      StrategicMap[SECTOR_INFO_TO_STRATEGIC_INDEX(Enum123.SEC_H3)]
        .fEnemyControlled
    ) {
      // If head miner was attacked, ore will/has run out, or enemy controlled
    ...
    iNumMinesInfectible =
      Number(fMineInfectible[0]) +
      Number(fMineInfectible[1]) +
      Number(fMineInfectible[2]) +
      Number(fMineInfectible[3]);
    ...
    switch (gGameOptions.ubDifficultyLevel) {
      case Enum9.DIF_LEVEL_EASY:
        ...
        AddPeriodStrategicEvent(
          Enum132.EVENT_CREATURE_SPREAD,
          EASY_SPREAD_TIME_IN_MINUTES,
          0,
        );
        break;
      case Enum9.DIF_LEVEL_MEDIUM:
  ...
  function PlaceNewCreature(
    node: CREATURE_DIRECTIVE | null,
    iDistance: INT32,
  ): boolean {
    ...
    if (giHabitatedDistance == iDistance) {
      ...
      if (
        node.pLevel.ubNumCreatures < MAX_STRATEGIC_TEAM_SIZE ||
        (node.pLevel.ubNumCreatures < 32 &&
          node.pLevel.ubCreatureHabitat == Enum128.QUEEN_LAIR)
      ) {
        ...
        switch (gGameOptions.ubDifficultyLevel) {
          ...
          case Enum9.DIF_LEVEL_MEDIUM: // 80%
            iAbsoluteMaxPopulation = Math.trunc(
              (iAbsoluteMaxPopulation * 4) / 5,
            );
            break;
        ...
        iMaxPopulation =
          100 - Math.trunc((iDistance * 100) / giHabitatedDistance);
        iMaxPopulation = Math.max(iMaxPopulation, 25);
        // Now, convert the previous value into a numeric population.
        iMaxPopulation = Math.trunc(
          (iAbsoluteMaxPopulation * iMaxPopulation) / 100,
        );
        ...
        iChanceToPopulate =
          100 - Math.trunc((node.pLevel.ubNumCreatures * 100) / iMaxPopulation);

        if (
          !node.pLevel.ubNumCreatures ||
          (iChanceToPopulate > Random(100) &&
  ...
  export function SpreadCreatures(): void {
    ...
    switch (gGameOptions.ubDifficultyLevel) {
      case Enum9.DIF_LEVEL_EASY:
        usNewCreatures =
          EASY_QUEEN_REPRODUCTION_BASE +
          Random(1 + EASY_QUEEN_REPRODUCTION_BONUS);
        break;
      case Enum9.DIF_LEVEL_MEDIUM:
        usNewCreatures =
          NORMAL_QUEEN_REPRODUCTION_BASE +
          Random(1 + NORMAL_QUEEN_REPRODUCTION_BONUS);
        break;
      case Enum9.DIF_LEVEL_HARD:
        usNewCreatures =
          HARD_QUEEN_REPRODUCTION_BASE +
          Random(1 + HARD_QUEEN_REPRODUCTION_BONUS);
        break;
    }
  ...
  function AddCreaturesToBattle(
    ubNumYoungMales: UINT8,
    ubNumYoungFemales: UINT8,
    ubNumAdultMales: UINT8,
    ubNumAdultFemales: UINT8,
  ): void {
    let iRandom: INT32;
    ...
    while (
      ...
      if (ubNumYoungMales && iRandom < ubNumYoungMales) {
        ...
        ubNumYoungFemales &&
        iRandom < ubNumYoungMales + ubNumYoungFemales
      ) {
        ubNumYoungFemales--;
        pSoldier = TacticalCreateCreature(Enum194.YAF_MONSTER);
      } else if (
        ubNumAdultMales &&
        iRandom < ubNumYoungMales + ubNumYoungFemales + ubNumAdultMales
      ) {
        ubNumAdultMales--;
      ...
      if (gsCreatureInsertionCode != Enum175.INSERTION_CODE_GRIDNO) {
        if (ubCurrSlot < MapEdgepointInfo.ubNumPoints) {
          ...
          pSoldier.usStrategicInsertionData =
            MapEdgepointInfo.sGridNo[ubCurrSlot++];
        } else {
  ...
  function ChooseTownSectorToAttack(
    ubSectorID: UINT8,
    fOverrideTest: boolean,
  ): void {
    let iRandom: INT32;
    let ubSectorX: UINT8;
    let ubSectorY: UINT8;
    ...
    if (!fOverrideTest) {
      ...
      switch (ubSectorID) {
        case Enum123.SEC_D13: // DRASSEN
          if (iRandom < 45) ubSectorID = Enum123.SEC_D13;
          else if (iRandom < 70) ubSectorID = Enum123.SEC_C13;
          else ubSectorID = Enum123.SEC_B13;
          break;
        case Enum123.SEC_H3: // GRUMM
          if (iRandom < 35) ubSectorID = Enum123.SEC_H3;
          else if (iRandom < 55) ubSectorID = Enum123.SEC_H2;
          else if (iRandom < 70) ubSectorID = Enum123.SEC_G2;
          else if (iRandom < 85) ubSectorID = Enum123.SEC_H1;
          else ubSectorID = Enum123.SEC_G1;
          break;
        case Enum123.SEC_H8: // CAMBRIA
          if (iRandom < 35) ubSectorID = Enum123.SEC_H8;
          else if (iRandom < 55) ubSectorID = Enum123.SEC_G8;
          else if (iRandom < 70) ubSectorID = Enum123.SEC_F8;
          else if (iRandom < 85) ubSectorID = Enum123.SEC_G9;
          else ubSectorID = Enum123.SEC_F9;
          break;
        case Enum123.SEC_I14: // ALMA
          if (iRandom < 45) ubSectorID = Enum123.SEC_I14;
          else if (iRandom < 65) ubSectorID = Enum123.SEC_I13;
          else if (iRandom < 85) ubSectorID = Enum123.SEC_H14;
          else ubSectorID = Enum123.SEC_H13;
          break;
        default:
          Assert(0);
          return;
  ...
  export function CreatureAttackTown(
    ubSectorID: UINT8,
    fOverrideTest: boolean,
  ): void {
    ...
    if (gfWorldLoaded && gTacticalStatus.fEnemyInSector) {
      ...
      AddStrategicEvent(
        Enum132.EVENT_CREATURE_ATTACK,
        GetWorldTotalMin() + Random(10),
        ubSectorID,
      );
      return;
    }
    ...
    if (PlayerGroupsInSector(ubSectorX, ubSectorY, 0)) {
      ...
      if (
        ubSectorX == gWorldSectorX &&
        ubSectorY == gWorldSectorY &&
        !gbWorldSectorZ
      ) {
        ...
        if (guiCurrentScreen == Enum26.GAME_SCREEN) {
          ...
          gubCreatureBattleCode =
            Enum129.CREATURE_BATTLE_CODE_PREBATTLEINTERFACE;
        }
      } else {
      ...
      !StrategicMap[ubSectorX + MAP_WORLD_X * ubSectorY].fEnemyControlled
    ) {
  ...
  function CreaturesInUndergroundSector(
    ubSectorID: UINT8,
    ubSectorZ: UINT8,
  ): UINT8 {
    let pSector: UNDERGROUND_SECTORINFO | null;
    ...
    if (!gMineStatus[ubMineIndex].fPrevInvadedByMonsters) {
      switch (ubMineIndex) {
        case Enum179.MINE_GRUMM:
          if (CreaturesInUndergroundSector(Enum123.SEC_H3, 1)) return false;
          if (CreaturesInUndergroundSector(Enum123.SEC_I3, 1)) return false;
          if (CreaturesInUndergroundSector(Enum123.SEC_I3, 2)) return false;
          if (CreaturesInUndergroundSector(Enum123.SEC_H3, 2)) return false;
          if (CreaturesInUndergroundSector(Enum123.SEC_H4, 2)) return false;
          break;
        case Enum179.MINE_CAMBRIA:
          if (CreaturesInUndergroundSector(Enum123.SEC_H8, 1)) return false;
          if (CreaturesInUndergroundSector(Enum123.SEC_H9, 1)) return false;
          break;
        case Enum179.MINE_ALMA:
          if (CreaturesInUndergroundSector(Enum123.SEC_I14, 1)) return false;
          if (CreaturesInUndergroundSector(Enum123.SEC_J14, 1)) return false;
          break;
        case Enum179.MINE_DRASSEN:
          if (CreaturesInUndergroundSector(Enum123.SEC_D13, 1)) return false;
          if (CreaturesInUndergroundSector(Enum123.SEC_E13, 1)) return false;
          break;
  ...
  export function DetermineCreatureTownComposition(
    ...
    while (i--) {
      iRandom = Random(100);
      if (iRandom < ubYoungMalePercentage) ubNumYoungMales++;
      else if (iRandom < ubYoungFemalePercentage) ubNumYoungFemales++;
      else if (iRandom < ubAdultMalePercentage) ubNumAdultMales++;
      else ubNumAdultFemales++;
    }
  ...
  export function DetermineCreatureTownCompositionBasedOnTacticalInformation(
    ubNumYoungMales: UINT8,
    ubNumYoungFemales: UINT8,
    ubNumAdultMales: UINT8,
    ubNumAdultFemales: UINT8,
    ...
    for (
      i = gTacticalStatus.Team[CREATURE_TEAM].bFirstID;
  ...
  export function PrepareCreaturesForBattle(): boolean {
    ...
    if (!gubCreatureBattleCode) {
      ...
      if (LColors[0].peBlue) gfUseCreatureMusic = true;
      else gfUseCreatureMusic = false;
    ...
    while (i--) {
      iRandom = Random(100);
      if (iRandom < ubLarvaePercentage) ubNumLarvae++;
      else if (iRandom < ubInfantPercentage) ubNumInfants++;
      else if (iRandom < ubYoungMalePercentage) ubNumYoungMales++;
      else if (iRandom < ubYoungFemalePercentage) ubNumYoungFemales++;
      else if (iRandom < ubAdultMalePercentage) ubNumAdultMales++;
      else ubNumAdultFemales++;
    }
    ...
    if (gbWorldSectorZ) {
      ...
      pUndergroundSector = FindUnderGroundSector(
        gWorldSectorX,
        gWorldSectorY,
        gbWorldSectorZ,
      );
      if (!pUndergroundSector) {
        ...
        AssertMsg(
          0,
          "Please report underground sector you are in or going to and send save if possible.  KM : 0",
        );
        return false;
      }
      pUndergroundSector.ubCreaturesInBattle =
        pUndergroundSector.ubNumCreatures;
    } else {
      let pSector: SECTORINFO;
      pSector = SectorInfo[SECTOR(gWorldSectorX, gWorldSectorY)];
  ...
  export function CreatureNightPlanning(): void {
    ...
    if (ubNumCreatures > 1 && ubNumCreatures * 10 > PreRandom(100)) {
      ...
      AddStrategicEvent(
        Enum132.EVENT_CREATURE_ATTACK,
        GetWorldTotalMin() + 1 + PreRandom(429),
        Enum123.SEC_H3,
      );
    }
  ...
  export function CheckConditionsForTriggeringCreatureQuest(
    sSectorX: INT16,
    sSectorY: INT16,
    bSectorZ: INT8,
  ): void {
  ...
  export function LoadCreatureDirectives(
    hFile: HWFILE,
  ...
  export function ForceCreaturesToAvoidMineTemporarily(
    ubMineIndex: UINT8,
  ): void {
    gMineStatus[Enum179.MINE_GRUMM].usValidDayCreaturesCanInfest =
      GetWorldDay() + 2;
  }
  ...
  export function GetWarpOutOfMineCodes(
    psSectorX: Pointer<INT16>,
    psSectorY: Pointer<INT16>,
    pbSectorZ: Pointer<INT8>,
    psInsertionGridNo: Pointer<INT16>,
  ): boolean {
    ...
    switch (iSwitchValue) {
      case 1: // Drassen
        if (
          (gWorldSectorX == 13 && gWorldSectorY == 6 && gbWorldSectorZ == 3) ||
      ...
      case 3: // Cambria
        if (
          (gWorldSectorX == 8 && gWorldSectorY == 9 && gbWorldSectorZ == 3) ||
      ...
      case 2: // Alma
        if (
          (gWorldSectorX == 13 && gWorldSectorY == 11 && gbWorldSectorZ == 3) ||
      ...
      case 4: // Grumm
        if (
          (gWorldSectorX == 4 && gWorldSectorY == 7 && gbWorldSectorZ == 3) ||
