namespace ja2 {
  ...
  export function InitializeProfilesForTownReputation(): void {
    ...
    for (uiProfileId = 0; uiProfileId < FIRST_NPC; uiProfileId++) {
      gMercProfiles[uiProfileId].bMercTownReputation.fill(
        INITIAL_TOWN_REPUTATION,
      );
    }
  }
  ...
  */
  ...
  function GetTownOpinionOfMercForSoldier(
  ...
  function UpdateTownOpinionOfThisMerc(
    ubProfileId: UINT8,
    ubTownId: UINT8,
    bAmount: INT8,
  ): void {
    ...
    if (
      ...
      // maximize
      gMercProfiles[ubProfileId].bMercTownReputation[ubTownId] =
        MAX_TOWN_OPINION;
    } else if (
      gMercProfiles[ubProfileId].bMercTownReputation[ubTownId] + bAmount <
      MIN_TOWN_OPINION
  ...
  function UpdateTownOpinionOfThisMercForSoldier(
    pSoldier: SOLDIERTYPE,
    ubTownId: UINT8,
    bAmount: INT8,
  ): void {
  ...
  export function HandleSpreadOfAllTownsOpinion(): void {
    ...
    ScreenMsg(
      MSG_FONT_RED,
      MSG_DEBUG,
      "%s - Spreading town opinions about mercs",
      gswzWorldTimeStr,
    );
    ...
    for (iCounterA = FIRST_TOWN; iCounterA < Enum135.NUM_TOWNS; iCounterA++) {
      for (iCounterB = iCounterA; iCounterB < Enum135.NUM_TOWNS; iCounterB++) {
        if (iCounterA != iCounterB) {
          ...
          HandleOpinionOfTownsAboutSoldier(
            bOpinionOfTownA,
            bOpinionOfTownB,
            iDistanceBetweenTowns,
            ubProfileId,
          );
        }
      }
  ...
  function HandleOpinionOfTownsAboutSoldier(
    bTownA: INT8,
    bTownB: INT8,
    iDistanceBetweenThem: INT32,
    ubProfile: UINT8,
  ): void {
