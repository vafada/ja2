namespace ja2 {
  ...
  export function createMineLocationTypeFrom(
    sSectorX: INT16,
    sSectorY: INT16,
    bAssociatedTown: INT8,
  ): MINE_LOCATION_TYPE {
    return {
      sSectorX,
      sSectorY,
  ...
  export function readMineStatusType(
    o: MINE_STATUS_TYPE,
    buffer: Buffer,
    offset: number = 0,
  ): number {
    ...
    o.uiMaxRemovalRate = buffer.readUInt32LE(offset);
    offset += 4;
    o.uiRemainingOreSupply = buffer.readUInt32LE(offset);
    offset += 4;
    o.uiOreRunningOutPoint = buffer.readUInt32LE(offset);
    offset += 4;
    ...
    o.usValidDayCreaturesCanInfest = buffer.readUInt16LE(offset);
    offset += 2;
    o.uiTimePlayerProductionStarted = buffer.readUInt32LE(offset);
    offset += 4;
  ...
  export function writeMineStatusType(
    o: MINE_STATUS_TYPE,
    buffer: Buffer,
    offset: number = 0,
  ): number {
  ...
  export function createHeadMinerTypeFrom(
    usProfileId: UINT16,
    bQuoteNum: INT8[],
    ubExternalFace: UINT8,
  ): HEAD_MINER_TYPE {
