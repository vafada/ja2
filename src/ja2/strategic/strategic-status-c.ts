namespace ja2 {
  ...
  export function CalcDeathRate(): UINT8 {
    ...
    if (gStrategicStatus.uiManDaysPlayed > 0) {
      ...
      uiDeathRate = Math.trunc(
        (gStrategicStatus.ubMercDeaths * DEATH_RATE_SEVERITY * 100) /
  ...
  function LackOfProgressTolerance(): UINT8 {
    if (gGameOptions.ubDifficultyLevel >= Enum9.DIF_LEVEL_HARD) {
      ...
      return (
        7 -
        Enum9.DIF_LEVEL_MEDIUM +
        Math.trunc(gStrategicStatus.ubHighestProgress / 42)
      );
    } else {
      return (
        6 -
  ...
  export function HandleEnricoEmail(): void {
      ...
      if (gStrategicStatus.ubNumberOfDaysOfInactivity >= ubTolerance) {
        if (gStrategicStatus.ubNumberOfDaysOfInactivity == ubTolerance) {
          ...
          if (
            !(
              gStrategicStatus.usEnricoEmailFlags &
              ENRICO_EMAIL_SENT_LACK_PROGRESS1
            )
          ) {
            bComplaint = 1;
          } else if (
            !(
              gStrategicStatus.usEnricoEmailFlags &
              ENRICO_EMAIL_SENT_LACK_PROGRESS2
            )
          ) {
            bComplaint = 2;
          } else if (
            !(
              gStrategicStatus.usEnricoEmailFlags &
              ENRICO_EMAIL_SENT_LACK_PROGRESS3
            )
          ) {
            bComplaint = 3;
          }
        ...
        if (bComplaint != 0) {
          switch (bComplaint) {
            case 3:
              AddEmail(
                LACK_PLAYER_PROGRESS_3,
                LACK_PLAYER_PROGRESS_3_LENGTH,
                Enum75.MAIL_ENRICO,
                GetWorldTotalMin(),
              );
              gStrategicStatus.usEnricoEmailFlags |=
            ...
            case 2:
              AddEmail(
                LACK_PLAYER_PROGRESS_2,
                LACK_PLAYER_PROGRESS_2_LENGTH,
                Enum75.MAIL_ENRICO,
                GetWorldTotalMin(),
              );
              gStrategicStatus.usEnricoEmailFlags |=
            ...
            default:
              AddEmail(
                LACK_PLAYER_PROGRESS_1,
                LACK_PLAYER_PROGRESS_1_LENGTH,
                Enum75.MAIL_ENRICO,
                GetWorldTotalMin(),
              );
              gStrategicStatus.usEnricoEmailFlags |=
          ...
          AddHistoryToPlayersLog(
            Enum83.HISTORY_ENRICO_COMPLAINED,
            0,
            GetWorldTotalMin(),
            -1,
            -1,
    ...
    // at night...
    gStrategicStatus.ubNumNewSectorsVisitedToday = Math.trunc(
      Math.min(
  ...
  export function TrackEnemiesKilled(
    ubKilledHow: UINT8,
    ...
    if (ubKilledHow != Enum189.ENEMY_KILLED_TOTAL) {
      gStrategicStatus.usEnemiesKilled[Enum189.ENEMY_KILLED_TOTAL][
        bRankIndex
      ]++;
    }
  }
