namespace ja2 {
  ...
  export function SetThisSectorAsPlayerControlled(
    sMapX: INT16,
    sMapY: INT16,
    bMapZ: INT8,
    fContested: boolean,
    ...
    */
      ...
      if (
        sMapX == BOBBYR_SHIPPING_DEST_SECTOR_X &&
        sMapY == BOBBYR_SHIPPING_DEST_SECTOR_Y
      ) {
        ...
        if (
          ...
          AddEmail(
            BOBBYR_NOW_OPEN,
            BOBBYR_NOW_OPEN_LENGTH,
            Enum75.BOBBY_R,
            GetWorldTotalMin(),
          );
          LaptopSaveInfo.ubHaveBeenToBobbyRaysAtLeastOnceWhileUnderConstruction =
            Enum99.BOBBYR_ALREADY_SENT_EMAIL;
        }
      }
      ...
      if (fWasEnemyControlled && fContested) {
        ...
        if (bTownId >= FIRST_TOWN && bTownId < Enum135.NUM_TOWNS) {
          ...
          if (bTownId != Enum135.OMERTA || GetWorldDay() != 1) {
            ...
            if (
              !bMapZ &&
              ubSectorID != Enum123.SEC_J9 &&
              ubSectorID != Enum123.SEC_K4
            ) {
              HandleMoraleEvent(
                null,
                Enum234.MORALE_TOWN_LIBERATED,
                sMapX,
                sMapY,
                bMapZ,
              );
              HandleGlobalLoyaltyEvent(
                Enum190.GLOBAL_LOYALTY_GAIN_TOWN_SECTOR,
                sMapX,
                sMapY,
                bMapZ,
              );
        ...
        if (IsThereAMineInThisSector(sMapX, sMapY)) {
          if (GetTotalLeftInMine(GetMineIndexForSector(sMapX, sMapY)) > 0) {
            HandleMoraleEvent(
              null,
              Enum234.MORALE_MINE_LIBERATED,
              sMapX,
              sMapY,
              bMapZ,
            );
            HandleGlobalLoyaltyEvent(
              Enum190.GLOBAL_LOYALTY_GAIN_MINE,
              sMapX,
              sMapY,
              bMapZ,
            );
        ...
        if (IsThisSectorASAMSector(sMapX, sMapY, bMapZ)) {
          if (
            1 /*!GetSectorFlagStatus( sMapX, sMapY, bMapZ, SF_SECTOR_HAS_BEEN_LIBERATED_ONCE ) */
          ) {
            // SAM site liberated for first time, schedule meanwhile
            HandleMeanWhileEventPostingForSAMLiberation(
              GetSAMIdFromSector(sMapX, sMapY, bMapZ),
            );
          }

          HandleMoraleEvent(
            null,
            Enum234.MORALE_SAM_SITE_LIBERATED,
            sMapX,
            sMapY,
            bMapZ,
          );
          HandleGlobalLoyaltyEvent(
            Enum190.GLOBAL_LOYALTY_GAIN_SAM,
            sMapX,
            sMapY,
            bMapZ,
          );
          ...
          if (
            !SectorInfo[SECTOR(sMapX, sMapY)].fSurfaceWasEverPlayerControlled
          ) {
            // grant grace period
        ...
        if (
          bMapZ == 0 &&
          ((sMapY == MAP_ROW_M && sMapX >= 2 && sMapX <= 6) ||
  ...
  export function SetThisSectorAsEnemyControlled(
    sMapX: INT16,
    sMapY: INT16,
    bMapZ: INT8,
    fContested: boolean,
    ...
    if (bMapZ == 0) {
      ...
      if (fWasPlayerControlled) {
        ...
        if (bTownId >= FIRST_TOWN && bTownId < Enum135.NUM_TOWNS) {
          ...
          if (
            ...
            HandleMoraleEvent(
              null,
              Enum234.MORALE_TOWN_LOST,
              sMapX,
              sMapY,
              bMapZ,
            );
            HandleGlobalLoyaltyEvent(
              Enum190.GLOBAL_LOYALTY_LOSE_TOWN_SECTOR,
              sMapX,
              sMapY,
              bMapZ,
            );
        ...
        if (IsThereAMineInThisSector(sMapX, sMapY)) {
          ...
          if (GetTotalLeftInMine(GetMineIndexForSector(sMapX, sMapY)) > 0) {
            ...
            HandleMoraleEvent(
              null,
              Enum234.MORALE_MINE_LOST,
              sMapX,
              sMapY,
              bMapZ,
            );
            HandleGlobalLoyaltyEvent(
              Enum190.GLOBAL_LOYALTY_LOSE_MINE,
              sMapX,
              sMapY,
              bMapZ,
            );
        ...
        if (IsThisSectorASAMSector(sMapX, sMapY, bMapZ)) {
          HandleMoraleEvent(
            null,
            Enum234.MORALE_SAM_SITE_LOST,
            sMapX,
            sMapY,
            bMapZ,
          );
          HandleGlobalLoyaltyEvent(
            Enum190.GLOBAL_LOYALTY_LOSE_SAM,
            sMapX,
            sMapY,
            bMapZ,
          );
      ...
      if (
        GetSectorFlagStatus(sMapX, sMapY, bMapZ, SF_ALREADY_VISITED) == true
      ) {
...
*/
  ...
  export function ReplaceSoldierProfileInPlayerGroup(
    ubGroupID: UINT8,
    ubOldProfile: UINT8,
    ubNewProfile: UINT8,
  ): void {
