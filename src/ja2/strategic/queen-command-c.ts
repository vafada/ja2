namespace ja2 {
  ...
  export let SectorInfo: SECTORINFO[] /* [256] */ = createArrayFrom(
    256,
    createSectorInfo,
  );
  ...
  export let gsInterrogationGridNo: INT16[] /* [3] */ = [7756, 7757, 7758];

  function ValidateEnemiesHaveWeapons(): void {}

  // Counts enemies and crepitus, but not bloodcats.
  export function NumHostilesInSector(
    sSectorX: INT16,
    sSectorY: INT16,
    sSectorZ: INT16,
  ): UINT8 {
  ...
  export function NumEnemiesInAnySector(
    sSectorX: INT16,
    sSectorY: INT16,
    sSectorZ: INT16,
  ): UINT8 {
  ...
  export function NumStationaryEnemiesInSector(
    sSectorX: INT16,
    sSectorY: INT16,
  ): UINT8 {
    let pSector: SECTORINFO;
    ...
    if (
      gGarrisonGroup[pSector.ubGarrisonID].ubComposition == Enum174.ROADBLOCK
    ) {
  ...
  export function NumMobileEnemiesInSector(
    sSectorX: INT16,
    sSectorY: INT16,
  ): UINT8 {
    let pGroup: GROUP | null;
  ...
  function GetNumberOfMobileEnemiesInSector(
    sSectorX: INT16,
    sSectorY: INT16,
  ): { ubNumAdmins: UINT8; ubNumTroops: UINT8; ubNumElites: UINT8 } {
  ...
  function GetNumberOfStationaryEnemiesInSector(
    sSectorX: INT16,
    sSectorY: INT16,
  ): { ubNumAdmins: UINT8; ubNumTroops: UINT8; ubNumElites: UINT8 } {
    let ubNumAdmins: UINT8;
  ...
  export function GetNumberOfEnemiesInSector(
    sSectorX: INT16,
    sSectorY: INT16,
  ): { ubNumAdmins: UINT8; ubNumTroops: UINT8; ubNumElites: UINT8 } {
    let ubNumAdmins: UINT8;
    ...
    ({
      ubNumAdmins: ubNumStationaryAdmins,
      ubNumTroops: ubNumStationaryTroops,
      ubNumElites: ubNumStationaryElites,
    } = GetNumberOfStationaryEnemiesInSector(sSectorX, sSectorY));

    ({
      ubNumAdmins: ubNumMobileAdmins,
      ubNumTroops: ubNumMobileTroops,
      ubNumElites: ubNumMobileElites,
    } = GetNumberOfMobileEnemiesInSector(sSectorX, sSectorY));
  ...
  export function EndTacticalBattleForEnemy(): void {
    ...
    if (gbWorldSectorZ > 0) {
      ...
      pSector = <UNDERGROUND_SECTORINFO>(
  ...
  function NumFreeEnemySlots(): UINT8 {
    ...
    for (
      i = gTacticalStatus.Team[ENEMY_TEAM].bFirstID;
      i <= gTacticalStatus.Team[ENEMY_TEAM].bLastID;
      i++
    ) {
      pSoldier = Menptr[i];
  ...
  export function PrepareEnemyForSectorBattle(): boolean {
    ...
    if (ubTotalAdmins + ubTotalTroops + ubTotalElites > 32) {
      ...
      ubTotalElites = Math.min(
        32 - ubTotalAdmins + ubTotalTroops,
        ubTotalElites,
      );
    }
  ...
  function PrepareEnemyForUndergroundBattle(): boolean {
    ...
    while (pUnderground) {
      if (
        pUnderground.ubSectorX == gWorldSectorX &&
        ...
        if (
          pUnderground.ubNumAdmins ||
          pUnderground.ubNumTroops ||
          pUnderground.ubNumElites
        ) {
          ubTotalAdmins =
            pUnderground.ubNumAdmins - pUnderground.ubAdminsInBattle;
          ubTotalTroops =
            pUnderground.ubNumTroops - pUnderground.ubTroopsInBattle;
          ubTotalElites =
            pUnderground.ubNumElites - pUnderground.ubElitesInBattle;
  ...
  export function ProcessQueenCmdImplicationsOfDeath(
    ...
    switch (pSoldier.ubProfile) {
      ...
      case Enum268.IGGY:
        if (
          ...
          !gubFact[Enum170.FACT_IGGY_AVAILABLE_TO_ARMY]
        ) {
          // Iggy is on our team!
          break;
        }
    ...
    if (
      pSoldier.bNeutral ||
      (pSoldier.bTeam != ENEMY_TEAM && pSoldier.bTeam != CREATURE_TEAM)
    )
      return;
        ...
        if (pSector) {
          switch (pSoldier.ubSoldierClass) {
            ...
            case Enum262.SOLDIER_CLASS_CREATURE:
              ...
              AddStrategicEventUsingSeconds(
                Enum132.EVENT_CHECK_IF_MINE_CLEARED,
                GetWorldTotalSeconds() + 15,
                0,
              );
  ...
  function AddEnemiesToBattle(
    pGroup: GROUP,
    ubStrategicInsertionCode: UINT8,
    ubNumAdmins: UINT8,
    ubNumTroops: UINT8,
    ubNumElites: UINT8,
    fMagicallyAppeared: boolean,
  ): void {
    ...
    ChooseMapEdgepoints(
      MapEdgepointInfo,
      ubStrategicInsertionCode,
      ubNumAdmins + ubNumElites + ubNumTroops,
    );
    ubCurrSlot = 0;
    while (ubTotalSoldiers) {
      if (ubNumElites && Random(ubTotalSoldiers) < ubNumElites) {
        ...
        ubNumTroops &&
        Random(ubTotalSoldiers) < ubNumElites + ubNumTroops
      ) {
        ubNumTroops--;
        ubTotalSoldiers--;
        ...
        ubNumAdmins &&
        Random(ubTotalSoldiers) < ubNumElites + ubNumTroops + ubNumAdmins
      ) {
        ubNumAdmins--;
        ubTotalSoldiers--;
  ...
  export function LoadUnderGroundSectorInfoFromSavedGame(
    hFile: HWFILE,
  ): boolean {
  ...
  export function FindUnderGroundSector(
    sMapX: INT16,
    sMapY: INT16,
    bMapZ: UINT8,
  ): UNDERGROUND_SECTORINFO | null {
    let pUnderground: UNDERGROUND_SECTORINFO | null;
    pUnderground = gpUndergroundSectorInfoHead;
  ...
  export function EnemyCapturesPlayerSoldier(pSoldier: SOLDIERTYPE): void {
    ...
    if (
      ...
      iNumEnemiesInSector == 0
    ) {
      InternalStartQuest(
        Enum169.QUEST_HELD_IN_ALMA,
        gWorldSectorX,
        gWorldSectorY,
        false,
      );
    ...
    if (gubQuest[Enum169.QUEST_HELD_IN_ALMA] == QUESTNOTSTARTED) {
      ...
      for (i = 0; i < Enum261.NUM_INV_SLOTS; i++) {
        if (pSoldier.inv[i].usItem != 0) {
          ...
          WorldItem.sGridNo =
            EnemyCapturesPlayerSoldier__sAlmaCaptureItemsGridNo[
      ...
      pSoldier.ubStrategicInsertionCode = Enum175.INSERTION_CODE_GRIDNO;
      pSoldier.usStrategicInsertionData =
        EnemyCapturesPlayerSoldier__sAlmaCaptureGridNos[
          gStrategicStatus.ubNumCapturedForRescue
        ];
      ...
      for (i = 0; i < Enum261.NUM_INV_SLOTS; i++) {
        if (pSoldier.inv[i].usItem != 0) {
          WorldItem.fExists = true;
          WorldItem.sGridNo =
            EnemyCapturesPlayerSoldier__sInterrogationItemGridNo[
              gStrategicStatus.ubNumCapturedForRescue
            ];
          WorldItem.ubLevel = 0;
          WorldItem.usFlags = 0;
  ...
  export function PlayerSectorDefended(ubSectorID: UINT8): boolean {
    ...
    if (
      pSector.ubNumberOfCivsAtLevel[Enum126.GREEN_MILITIA] +
      pSector.ubNumberOfCivsAtLevel[Enum126.REGULAR_MILITIA] +
      pSector.ubNumberOfCivsAtLevel[Enum126.ELITE_MILITIA]
    ) {
      // militia in sector
      return true;
    }
    if (
      FindMovementGroupInSector(SECTORX(ubSectorID), SECTORY(ubSectorID), true)
    ) {
      // player in sector
      return true;
    }
