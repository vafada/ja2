namespace ja2 {
  ...
  export function readSectorInfo(
    o: SECTORINFO,
    buffer: Buffer,
    offset: number = 0,
  ): number {
    ...
    offset++; // padding
    o.ubDayOfLastCreatureAttack = buffer.readUInt32LE(offset);
    offset += 4;
    o.uiFacilitiesFlags = buffer.readUInt32LE(offset);
    offset += 4;
    ...
    o.usUNUSEDMilitiaLevels = buffer.readUInt16LE(offset);
    offset += 2;
    o.ubUNUSEDNumberOfJoeBlowCivilians = buffer.readUInt8(offset++);
    offset += 3; // padding
    o.uiTimeCurrentSectorWasLastLoaded = buffer.readUInt32LE(offset);
    offset += 4;
    o.ubUNUSEDNumberOfEnemiesThoughtToBeHere = buffer.readUInt8(offset++);
    offset += 3; // padding
    o.uiTimeLastPlayerLiberated = buffer.readUInt32LE(offset);
    offset += 4;
    ...
    o.uiNumberOfWorldItemsInTempFileThatCanBeSeenByPlayer =
  ...
  export function writeSectorInfo(
    o: SECTORINFO,
    buffer: Buffer,
    offset: number = 0,
  ): number {
    ...
    offset = buffer.writeUInt8(
      o.ubUNUSEDNumberOfEnemiesThoughtToBeHere,
      offset,
    );
    offset = writePadding(buffer, offset, 3); // padding
    offset = buffer.writeUInt32LE(o.uiTimeLastPlayerLiberated, offset);
    offset = buffer.writeUInt8(
      Number(o.fSurfaceWasEverPlayerControlled),
      offset,
    );
  ...
  export function readUndergroundSectorInfo(
    o: UNDERGROUND_SECTORINFO,
    buffer: Buffer,
    offset: number = 0,
  ): number {
    o.uiFlags = buffer.readInt32LE(offset);
    offset += 4;
  ...
  export function writeUndergroundSectorInfo(
    o: UNDERGROUND_SECTORINFO,
    buffer: Buffer,
    offset: number = 0,
  ): number {
