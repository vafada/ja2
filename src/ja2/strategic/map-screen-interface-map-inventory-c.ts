namespace ja2 {
  ...
  let MapInventoryPoolSlots: MOUSE_REGION[] /* [MAP_INVENTORY_POOL_SLOT_COUNT] */ =
    createArrayFrom(MAP_INVENTORY_POOL_SLOT_COUNT, createMouseRegion);
  ...
  export let fMapInventoryItemCompatable: boolean[] /* [MAP_INVENTORY_POOL_SLOT_COUNT] */ =
    createArray(MAP_INVENTORY_POOL_SLOT_COUNT, false);
  ...
  function RenderItemInPoolSlot(
    iCurrentSlot: INT32,
    iFirstSlotOnPage: INT32,
  ): boolean {
    ...
    if (
      pInventoryPoolList[iCurrentSlot + iFirstSlotOnPage].o.ubNumberOfObjects ==
      0
    ) {
      return false;
    }

    hHandle = GetVideoObject(
    ...
    // set sx and sy
    sX =
      MAP_INVENTORY_POOL_SLOT_OFFSET_X +
      MAP_INVENTORY_POOL_SLOT_START_X +
      MAP_INVEN_SPACE_BTWN_SLOTS * Math.trunc(iCurrentSlot / MAP_INV_SLOT_COLS);
    sY =
      MAP_INVENTORY_POOL_SLOT_START_Y +
      MAP_INVEN_SLOT_HEIGHT * (iCurrentSlot % MAP_INV_SLOT_COLS);

    // CENTER IN SLOT!
    sCenX =
      sX +
      Math.trunc(Math.abs(MAP_INVEN_SPACE_BTWN_SLOTS - usWidth) / 2) -
      pTrav.sOffsetX;
    sCenY =
      sY +
      Math.trunc(Math.abs(MAP_INVEN_SLOT_HEIGHT - 5 - usHeight) / 2) -
      pTrav.sOffsetY;
    ...
    INVRenderItem(
      guiSAVEBUFFER,
      null,
      pInventoryPoolList[iCurrentSlot + iFirstSlotOnPage].o,
      sX + 7,
      sY,
      60,
      25,
    ...
    DrawItemUIBarEx(
      pInventoryPoolList[iCurrentSlot + iFirstSlotOnPage].o,
      0,
      ITEMDESC_ITEM_STATUS_INV_POOL_OFFSET_X +
        MAP_INVENTORY_POOL_SLOT_START_X +
        MAP_INVEN_SPACE_BTWN_SLOTS *
          Math.trunc(iCurrentSlot / MAP_INV_SLOT_COLS),
      ITEMDESC_ITEM_STATUS_INV_POOL_OFFSET_Y +
        MAP_INVENTORY_POOL_SLOT_START_Y +
        MAP_INVEN_SLOT_HEIGHT * (iCurrentSlot % MAP_INV_SLOT_COLS),
      ITEMDESC_ITEM_STATUS_WIDTH_INV_POOL,
    ...
    if (
      !(
        pInventoryPoolList[iCurrentSlot + iFirstSlotOnPage].usFlags &
    ...
    sString =
      ShortItemNames[
        pInventoryPoolList[iCurrentSlot + iFirstSlotOnPage].o.usItem
      ];

    if (StringPixLength(sString, MAP_IVEN_FONT()) >= MAP_INVEN_SLOT_WIDTH) {
      sString = ReduceStringLength(
        sString,
        MAP_INVEN_SLOT_WIDTH - StringPixLength(" ...", MAP_IVEN_FONT()),
        MAP_IVEN_FONT(),
      );
    }

    ({ sX: sWidth, sY: sHeight } = FindFontCenterCoordinates(
      4 +
        MAP_INVENTORY_POOL_SLOT_START_X +
  ...
  */
  ...
  function UpdateHelpTextForInvnentoryStashSlots(): void {
    ...
    for (iCounter = 0; iCounter < MAP_INVENTORY_POOL_SLOT_COUNT; iCounter++) {
      if (
        ...
        pStr = GetHelpTextForItem(
          pInventoryPoolList[iCounter + iFirstSlotOnPage].o,
          null,
        );
  ...
  /* static */ let CreateDestroyMapInventoryPoolButtons__fCreated: boolean =
    false;
  export function CreateDestroyMapInventoryPoolButtons(
    fExitFromMapScreen: boolean,
  ): void {
  ...
  */
  ...
  export function CancelSectorInventoryDisplayIfOn(
    fExitFromMapScreen: boolean,
  ): void {
  ...
  function SaveSeenAndUnseenItems(): void {
    ...
    if (iTotalNumberItems > 0) {
      ...
      for (iCounter = 0; iCounter < iTotalNumberOfSlots; iCounter++) {
        if (pInventoryPoolList[iCounter].o.ubNumberOfObjects > 0) {
          // copy object stuff
          copyWorldItem(
            pSeenItemsList[iItemCount],
            pInventoryPoolList[iCounter],
          );
          ...
          if (pSeenItemsList[iItemCount].sGridNo == 0) {
            if (iItemCount > 0) {
              ...
              pSeenItemsList[iItemCount].sGridNo =
                pSeenItemsList[iItemCount - 1].sGridNo;
            } else {
    ...
    if (
      ...
      ReBuildWorldItemStashForLoadedSector(
        iItemCount,
  ...
  function MapInvenPoolScreenMaskCallback(
  ...
  function CreateMapInventoryPoolSlots(): void {
    ...
    for (iCounter = 0; iCounter < MAP_INVENTORY_POOL_SLOT_COUNT; iCounter++) {
      ...
      MSYS_DefineRegion(
        MapInventoryPoolSlots[iCounter],
        sULX,
        sULY,
        sBRX,
        sBRY,
  ...
  function MapInvenPoolSlots(pRegion: MOUSE_REGION, iReason: INT32): void {
    ...
    if (iReason & MSYS_CALLBACK_REASON_RBUTTON_UP) {
      ...
      if (gpItemPointer == null) {
        ...
        if (
          pInventoryPoolList[
            iCurrentInventoryPoolPage * MAP_INVENTORY_POOL_SLOT_COUNT + iCounter
          ].o.usItem == NOTHING
        )
          return;
      }
      ...
      if (gpItemPointer == null) {
        ...
        if (
          ...
          sDistanceFromObject = PythSpacesAway(
            sObjectSourceGridNo,
            pSoldier.sGridNo,
          );
  ...
  function CreateMapInventoryButtons(): void {
    guiMapInvenButtonImage[0] = LoadButtonImage(
    ...
    guiMapInvenButton[0] = QuickCreateButton(
      ...
      MapInventoryPoolNextBtn,
    );

    guiMapInvenButtonImage[1] = LoadButtonImage(
      "INTERFACE\\map_screen_bottom_arrows.sti",
      9,
      0,
      -1,
  ...
  function BuildStashForSelectedSector(
    sMapX: INT16,
    sMapY: INT16,
    sMapZ: INT16,
  ): void {
    ...
    iSize =
      iSize -
      (iSize % MAP_INVENTORY_POOL_SLOT_COUNT) +
    ...
    iLastInventoryPoolPage = Math.trunc(
      (iTotalNumberOfSlots - 1) / MAP_INVENTORY_POOL_SLOT_COUNT,
    );
    ...
    if (
      ...
      */
      ...
      if (guiNumWorldItems > uiItemCount) {
        pUnSeenItems = createArrayFrom(
          guiNumWorldItems - uiItemCount,
          createWorldItem,
        );
        ...
        for (iCounter = 0; iCounter < guiNumWorldItems; iCounter++) {
          ...
          //							gWorldItems[ iCounter ].fExists )
          if (
            IsMapScreenWorldItemInvisibleInMapInventory(gWorldItems[iCounter])
          ) {
            // one more item
      ...
      if (uiTotalNumberOfRealItems > 0) {
        ...
        pTotalSectorList = createArrayFrom(
      ...
      */
        ...
        if (
          pTotalSectorList[iCounter].fExists &&
          pTotalSectorList[iCounter].o.usItem > Enum225.MAXITEMS
        ) {
          ScreenMsg(
            FONT_MCOLOR_LTYELLOW,
            MSG_BETAVERSION,
            "The %d item in the list is NOT valid. Please send save.  DF 1.",
            iCounter,
          );
        }
        ...
        if (
          ...
          // one more item
          copyWorldItem(
            pInventoryPoolList[uiItemCount],
            pTotalSectorList[iCounter],
          );
      ...
      if (uiTotalNumberOfRealItems > uiItemCount) {
        pUnSeenItems = createArrayFrom(
          uiTotalNumberOfRealItems - uiItemCount,
          createWorldItem,
        );
        ...
        */
          if (
            IsMapScreenWorldItemInvisibleInMapInventory(
              pTotalSectorList[iCounter],
            )
          ) {
            // one more item
            copyWorldItem(
              pUnSeenItems[uiItemCount],
              pTotalSectorList[iCounter],
            );
  ...
  function ReBuildWorldItemStashForLoadedSector(
    iNumberSeenItems: INT32,
    iNumberUnSeenItems: INT32,
    pSeenItemsList: WORLDITEM[],
    pUnSeenItemsList: WORLDITEM[],
  ): void {
    ...
    for (iCounter = 0; iCounter < iNumberSeenItems; iCounter++) {
      uiTotalNumberOfVisibleItems +=
        pSeenItemsList[iCounter].o.ubNumberOfObjects;
    }
  ...
  function ReSizeStashListByThisAmount(iNumberOfItems: INT32): void {
    ...
    // rebuild stash
    pInventoryPoolList = pInventoryPoolList.concat(
      createArrayFrom(iNumberOfItems, createWorldItem),
    );
  ...
  function GetSizeOfStashInSector(
    sMapX: INT16,
    sMapY: INT16,
    sMapZ: INT16,
    fCountStacksAsOne: boolean,
  ): INT32 {
  ...
  function GetObjFromInventoryStashSlot(
    pInventorySlot: OBJECTTYPE,
    pItemPtr: OBJECTTYPE,
  ): boolean {
    // item ptr
    if (!pItemPtr) {
  ...
  function RemoveObjectFromStashSlot(
    pInventorySlot: OBJECTTYPE,
    pItemPtr: OBJECTTYPE,
  ): boolean {
    if (!pInventorySlot) {
  ...
  function PlaceObjectInInventoryStash(
    pInventorySlot: OBJECTTYPE,
    pItemPtr: OBJECTTYPE,
  ): boolean {
    ...
    if (pInventorySlot.ubNumberOfObjects == 0) {
      ...
      if (ubNumberToDrop != pItemPtr.ubNumberOfObjects) {
        ...
        for (
          ...
          ubLoop < pItemPtr.ubNumberOfObjects;
          ubLoop++
        ) {
      ...
      if (pItemPtr.usItem == pInventorySlot.usItem) {
        if (pItemPtr.usItem == Enum225.MONEY) {
          ...
          pInventorySlot.bStatus[0] = Math.trunc(
            (pInventorySlot.bMoneyStatus * pInventorySlot.uiMoneyAmount +
              pItemPtr.bMoneyStatus * pItemPtr.uiMoneyAmount) /
  ...
  export function AutoPlaceObjectInInventoryStash(
    pItemPtr: OBJECTTYPE,
  ): boolean {
    let ubNumberToDrop: UINT8;
    let ubSlotLimit: UINT8;
  ...
  function DisplayPagesForMapInventoryPool(): void {
    ...
    sString = swprintf(
      "%d / %d",
      iCurrentInventoryPoolPage + 1,
      iLastInventoryPoolPage + 1,
    );

    // grab centered coords
    ({ sX, sY } = FindFontCenterCoordinates(
      MAP_INVENTORY_POOL_PAGE_X,
      MAP_INVENTORY_POOL_PAGE_Y,
      MAP_INVENTORY_POOL_PAGE_WIDTH,
      MAP_INVENTORY_POOL_PAGE_HEIGHT,
      sString,
      MAP_SCREEN_FONT(),
    ));
  ...
  function DrawNumberOfIventoryPoolItems(): void {
    ...
    // grab centered coords
    ({ sX, sY } = FindFontCenterCoordinates(
      MAP_INVENTORY_POOL_NUMBER_X,
      MAP_INVENTORY_POOL_PAGE_Y,
      MAP_INVENTORY_POOL_NUMBER_WIDTH,
      MAP_INVENTORY_POOL_PAGE_HEIGHT,
      sString,
      MAP_SCREEN_FONT(),
    ));
  ...
  function CreateMapInventoryPoolDoneButton(): void {
    // create done button
    guiMapInvenButtonImage[2] = LoadButtonImage(
      "INTERFACE\\done_button.sti",
      -1,
      0,
      -1,
      1,
  ...
  function DisplayCurrentSector(): void {
    ...
    sString = swprintf(
      "%s%s%s",
      pMapVertIndex[sSelMapY],
      pMapHortIndex[sSelMapX],
    ...
    // grab centered coords
    ({ sX, sY } = FindFontCenterCoordinates(
      MAP_INVENTORY_POOL_LOC_X,
      MAP_INVENTORY_POOL_PAGE_Y,
      MAP_INVENTORY_POOL_LOC_WIDTH,
      MAP_INVENTORY_POOL_PAGE_HEIGHT,
      sString,
      MAP_SCREEN_FONT(),
    ));
  ...
  function DrawTextOnMapInventoryBackground(): void {
    ...
    usStringHeight = DisplayWrappedString(
      268,
      342,
      53,
      1,
    ...
    DisplayWrappedString(
      268,
      342 - Math.trunc(usStringHeight / 2),
      53,
      1,
      MAP_IVEN_FONT(),
      FONT_BEIGE,
  ...
  function DrawTextOnSectorInventory(): void {
    ...
    ({ sX, sY } = FindFontCenterCoordinates(
      MAP_INVENTORY_POOL_SLOT_START_X,
      MAP_INVENTORY_POOL_SLOT_START_Y - 20,
      630 - MAP_INVENTORY_POOL_SLOT_START_X,
      GetFontHeight(FONT14ARIAL()),
      sString,
      FONT14ARIAL(),
    ));
  ...
  export function HandleFlashForHighLightedItem(): void {
    ...
    if (iDifference > DELAY_FOR_HIGHLIGHT_ITEM_FLASH) {
      ...
      // flip flag
      fFlashHighLightInventoryItemOnradarMap =
        !fFlashHighLightInventoryItemOnradarMap;
  ...
  /* static */ let HandleMouseInCompatableItemForMapSectorInventory__fItemWasHighLighted: boolean =
    false;
  function HandleMouseInCompatableItemForMapSectorInventory(
    iCurrentSlot: INT32,
  ): void {
    ...
    if (giCompatibleItemBaseTime == 0) {
      ...
      if (
        HandleMouseInCompatableItemForMapSectorInventory__fItemWasHighLighted ==
        true
      ) {
        fTeamPanelDirty = true;
        fMapPanelDirty = true;
  ...
  export function IsMapScreenWorldItemVisibleInMapInventory(
    pWorldItem: WORLDITEM,
  ): boolean {
    if (
      pWorldItem.bVisible == 1 &&
      pWorldItem.fExists &&
      pWorldItem.o.usItem != Enum225.SWITCH &&
      pWorldItem.o.usItem != Enum225.ACTION_ITEM &&
  ...
  // CJC look here to add/remove checks for the sector inventory
  function IsMapScreenWorldItemInvisibleInMapInventory(
    pWorldItem: WORLDITEM,
  ): boolean {
    if (
      pWorldItem.fExists &&
  ...
  function CheckGridNoOfItemsInMapScreenMapInventory(): void {
    ...
    for (iCnt = 0; iCnt < iTotalNumberItems; iCnt++) {
      if (
        pInventoryPoolList[iCnt].sGridNo == NOWHERE &&
        !(
          pInventoryPoolList[iCnt].usFlags &
          WORLD_ITEM_GRIDNO_NOT_SET_USE_ENTRY_POINT
        )
      ) {
        // set the flag
        pInventoryPoolList[iCnt].usFlags |=
    ...
    for (iCnt = 0; iCnt < uiNumberOfUnSeenItems; iCnt++) {
      if (
        pUnSeenItems[iCnt].sGridNo == NOWHERE &&
        !(
  ...
  function SortSectorInventory(
    pInventory: WORLDITEM[],
    uiSizeOfArray: UINT32,
  ): void {
    pInventory.sort(MapScreenSectorInventoryCompare);
  }

  function MapScreenSectorInventoryCompare(
    pNum1: WORLDITEM,
    pNum2: WORLDITEM,
  ): INT32 {
  ...
  function CanPlayerUseSectorInventory(pSelectedSoldier: SOLDIERTYPE): boolean {
    ...
    fInCombat = GetCurrentBattleSectorXYZAndReturnTRUEIfThereIsABattle(
      createPointer(
        () => sSectorX,
        (v) => (sSectorX = v),
      ),
