namespace ja2 {
  function DelayEventIfBattleInProgress(pEvent: STRATEGICEVENT): boolean {
    ...
    if (gTacticalStatus.fEnemyInSector) {
      pNewEvent = AddAdvancedStrategicEvent(
        pEvent.ubEventType,
        pEvent.ubCallbackID,
        pEvent.uiTimeStamp + 180 + Random(121),
        pEvent.uiParam,
      );
      Assert(pNewEvent);
  ...
  export function ExecuteStrategicEvent(pEvent: STRATEGICEVENT): boolean {
    ...
    switch (pEvent.ubCallbackID) {
      ...
      case Enum132.EVENT_DAY2_ADD_EMAIL_FROM_IMP:
        AddEmail(
          IMP_EMAIL_PROFILE_RESULTS,
          IMP_EMAIL_PROFILE_RESULTS_LENGTH,
          Enum75.IMP_PROFILE_RESULTS,
          GetWorldTotalMin(),
        );
        break;
