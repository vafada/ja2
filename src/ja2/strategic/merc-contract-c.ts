namespace ja2 {
  ...
  let ContractRenewalList: CONTRACT_NEWAL_LIST_NODE[] /* [20] */ =
    createArrayFrom(20, createContractRenewalListNode);
  ...
  export function SaveContractRenewalDataToSaveGameFile(
    hFile: HWFILE,
  ): boolean {
    ...
    writeObjectArray(
      ...
      buffer,
      0,
      writeContractRenewalListNode,
    );

    uiNumBytesWritten = FileWrite(
      hFile,
      buffer,
      20 * CONTRACT_RENEWAL_LIST_NODE_SIZE,
    );
  ...
  export function LoadContractRenewalDataFromSaveGameFile(
    hFile: HWFILE,
  ): boolean {
    ...
    if (ubNumContractRenewals > 0) {
      for (cnt = 0; cnt < ubNumContractRenewals; cnt++) {
        ...
        pSoldier = FindSoldierByProfileID(
          ContractRenewalList[cnt].ubProfileID,
          false,
        ); // Steve Willis, 80
  ...
  export function HandleContractRenewalSequence(): void {
    ...
    if (gfContractRenewalSquenceOn) {
      ...
      // Get soldier - if there is none, adavance to next
      pSoldier = FindSoldierByProfileID(
        ContractRenewalList[ubCurrentContractRenewal].ubProfileID,
        false,
      ); // Steve Willis, 80
        ...
        if (ContractIsExpiring(pSoldier)) {
          ...
          if (!WillMercRenew(pSoldier, false)) {
            ...
            HandleImportantMercQuote(
              pSoldier,
              Enum202.QUOTE_MERC_LEAVING_ALSUCO_SOON,
            );

            // Do special dialogue event...
            SpecialCharacterDialogueEvent(
              DIALOGUE_SPECIAL_EVENT_CONTRACT_NOGO_TO_RENEW,
              pSoldier.ubID,
              0,
              0,
              0,
            ...
            if (
              pSoldier.ubContractRenewalQuoteCode ==
              SOLDIER_CONTRACT_RENEW_QUOTE_NOT_USED
            ) {
              SpecialCharacterDialogueEvent(
  ...
  export function MercContractHandling(
    pSoldier: SOLDIERTYPE,
    ubDesiredAction: UINT8,
  ): boolean {
    ...
    if (
      ...
      //  DEF:  Removed cause they can extend a 1 day contract && ( iContractLength > 1 )
      // check if player can afford insurance, if not, tell them
      iCostOfInsurance = CalculateInsuranceContractCost(
        iContractLength,
        pSoldier.ubProfile,
      );
      ...
      if (iCostOfInsurance > LaptopSaveInfo.iCurrentBalance) {
        ...
        HandleNotifyPlayerCanAffordInsurance(
          pSoldier,
          iContractLength,
          iCostOfInsurance,
        );
  ...
  export function WillMercRenew(
    ...
    if (pSoldier.fSignedAnotherContract) {
      ...
      if (fSayQuote == true) {
        ...
        HandleImportantMercQuote(
          pSoldier,
          Enum202.QUOTE_WONT_RENEW_CONTRACT_LAME_REFUSAL,
        );
        SpecialCharacterDialogueEvent(
          DIALOGUE_SPECIAL_EVENT_LOCK_INTERFACE,
          0,
    ...
    for (i = 0; i < 5; i++) {
      ...
      if (IsMercOnTeamAndAlive(bMercID)) {
        ...
        if (i == 0) usBuddyQuote = Enum202.QUOTE_RENEWING_CAUSE_BUDDY_1_ON_TEAM;
        else if (i == 1)
          usBuddyQuote = Enum202.QUOTE_RENEWING_CAUSE_BUDDY_2_ON_TEAM;
        else
          usBuddyQuote =
            Enum202.QUOTE_RENEWING_CAUSE_LEARNED_TO_LIKE_BUDDY_ON_TEAM;
    ...
    for (i = 0; i < 2; i++) {
      ...
      if (IsMercOnTeamAndInOmertaAlreadyAndAlive(bMercID)) {
        if (gMercProfiles[pSoldier.ubProfile].bHatedCount[i] == 0) {
          ...
          fUnhappy = true;
        } // else tolerance is > 0, only gripe if in same sector
        else {
          pHated = FindSoldierByProfileID(bMercID, true);
          if (
            pHated &&
            pHated.sSectorX == pSoldier.sSectorX &&
        ...
        if (fUnhappy) {
          ...
          else usReasonQuote = Enum202.QUOTE_HATE_MERC_2_ON_TEAM_WONT_RENEW;
      ...
      if (bMercID >= 0) {
        if (IsMercOnTeamAndInOmertaAlreadyAndAlive(bMercID)) {
          if (gMercProfiles[pSoldier.ubProfile].bLearnToHateCount == 0) {
            ...
            fUnhappy = true;
            usReasonQuote =
              Enum202.QUOTE_LEARNED_TO_HATE_MERC_1_ON_TEAM_WONT_RENEW;
          } else if (
            gMercProfiles[pSoldier.ubProfile].bLearnToHateCount <=
            Math.trunc(gMercProfiles[pSoldier.ubProfile].bLearnToHateTime / 2)
          ) {
            pHated = FindSoldierByProfileID(bMercID, true);
            if (
              pHated &&
    ...
    if (fSayQuote) {
      if (fUnhappy) {
        if (fBuddyAround) {
          if (
            GetMercPrecedentQuoteBitStatus(
              pSoldier.ubProfile,
              GetQuoteBitNumberFromQuoteID(usBuddyQuote),
            ) == true
          ) {
            fSayPrecedent = true;
          } else {
            SetMercPrecedentQuoteBitStatus(
              pSoldier.ubProfile,
              GetQuoteBitNumberFromQuoteID(usBuddyQuote),
            );
          }
          ...
          if (
            GetMercPrecedentQuoteBitStatus(
              pSoldier.ubProfile,
              GetQuoteBitNumberFromQuoteID(usReasonQuote),
            ) == true
          ) {
            fSayPrecedent = true;
          } else {
    ...
    if (fSayPrecedent) {
      HandleImportantMercQuote(
        pSoldier,
        Enum202.QUOTE_PRECEDENT_TO_REPEATING_ONESELF_RENEW,
      );
    }
  ...
  function HandleSoldierLeavingForAnotherContract(pSoldier: SOLDIERTYPE): void {
    if (pSoldier.fSignedAnotherContract) {
      ...
      gMercProfiles[pSoldier.ubProfile].uiDayBecomesAvailable +=
        1 + Random(6 + Math.trunc(pSoldier.bExpLevel / 2)); // 1-(6 to 11) days
    }
  }
...
*/
  ...
  export function CheckIfMercGetsAnotherContract(pSoldier: SOLDIERTYPE): void {
    ...
    // aim merc?
    if (pSoldier.ubWhatKindOfMercAmI != Enum260.MERC_TYPE__AIM_MERC) return;
    ...
    if (!pSoldier.fSignedAnotherContract) {
      ...
      uiFullDaysRemaining = Math.trunc(
        (pSoldier.iEndofContractTime - GetWorldTotalMin()) / (24 * 60),
      );
  ...
  export function BeginStrategicRemoveMerc(
    pSoldier: SOLDIERTYPE,
    fAddRehireButton: boolean,
  ): boolean {
    InterruptTime();
    PauseGame();
    LockPauseState(8);
  ...
  export function StrategicRemoveMerc(pSoldier: SOLDIERTYPE): boolean {
    ...
    // else if the merc was fired
      ...
      ubHistoryCode == Enum83.HISTORY_MERC_FIRED ||
      pSoldier.bAssignment == Enum117.ASSIGNMENT_POW
    ) {
  ...
  function CalculateMedicalDepositRefund(pSoldier: SOLDIERTYPE): void {
    ...
    // else the player is injured, refund a partial amount
      ...
      // use the medical deposit in pSoldier, not in profile, which goes up with leveling
      iRefundAmount = Math.trunc(
        Math.trunc(pSoldier.bLife / pSoldier.bLifeMax) *
          pSoldier.usMedicalDeposit +
          0.5,
      );
      ...
      AddEmailWithSpecialData(
        ...
        AIM_MEDICAL_DEPOSIT_PARTIAL_REFUND_LENGTH,
        Enum75.AIM_SITE,
        GetWorldTotalMin(),
        iRefundAmount,
        pSoldier.ubProfile,
  ...
  function NotifyPlayerOfMercDepartureAndPromptEquipmentPlacement(
    pSoldier: SOLDIERTYPE,
    fAddRehireButton: boolean,
  ): void {
    ...
    zShortTownIDString = GetShortSectorString(
      pSoldier.sSectorX,
      pSoldier.sSectorY,
    );
    ...
    if (pSoldier.ubProfile >= FIRST_RPC && pSoldier.ubProfile < FIRST_NPC) {
      if (gMercProfiles[pSoldier.ubProfile].bSex == Enum272.MALE) {
        sString = swprintf(
          pMercHeLeaveString[4],
          pSoldier.name,
          zShortTownIDString,
        );
      } else {
        sString = swprintf(
          pMercSheLeaveString[4],
          pSoldier.name,
          zShortTownIDString,
        );
      }
    ...
    // check if drassen controlled
      ...
      StrategicMap[AIRPORT_X + MAP_WORLD_X * AIRPORT_Y].fEnemyControlled ==
      false
    ) {
      if (
        pSoldier.sSectorX == AIRPORT_X &&
        pSoldier.sSectorY == AIRPORT_Y &&
        pSoldier.bSectorZ == 0
      ) {
        if (gMercProfiles[pSoldier.ubProfile].bSex == Enum272.MALE) {
        ...
        if (gMercProfiles[pSoldier.ubProfile].bSex == Enum272.MALE) {
          sString = swprintf(
            pMercHeLeaveString[0],
            pSoldier.name,
            zShortTownIDString,
          );
        } else {
          sString = swprintf(
            pMercSheLeaveString[0],
            pSoldier.name,
            zShortTownIDString,
          );
        }
      ...
      if (
        pSoldier.sSectorX == OMERTA_LEAVE_EQUIP_SECTOR_X &&
        pSoldier.sSectorY == OMERTA_LEAVE_EQUIP_SECTOR_Y &&
        pSoldier.bSectorZ == 0
      ) {
        if (gMercProfiles[pSoldier.ubProfile].bSex == Enum272.MALE) {
        ...
        if (gMercProfiles[pSoldier.ubProfile].bSex == Enum272.MALE) {
          sString = swprintf(
            pMercHeLeaveString[1],
            pSoldier.name,
            zShortTownIDString,
          );
        } else {
          sString = swprintf(
            pMercSheLeaveString[1],
            pSoldier.name,
            zShortTownIDString,
          );
        }
    ...
    if (guiTacticalInterfaceFlags & INTERFACE_MAPSCREEN) {
      ...
      if (fInSector == false) {
        ...
        DoMessageBox(
          ...
          sString,
          guiCurrentScreen,
          MSG_BOX_FLAG_USE_CENTERING_RECT |
            (fAddRehireButton
              ? MSG_BOX_FLAG_GENERICCONTRACT
              : MSG_BOX_FLAG_GENERIC),
          MercDepartEquipmentBoxCallBack,
  ...
  function MercDepartEquipmentBoxCallBack(bExitValue: UINT8): void {
    ...
    if (bExitValue == MSG_BOX_RETURN_OK) {
      ...
      if (
        pLeaveSoldier.ubWhatKindOfMercAmI == Enum260.MERC_TYPE__AIM_MERC &&
        ubQuitType != Enum83.HISTORY_MERC_FIRED
      ) {
      ...
      if (
        StrategicMap[
          BOBBYR_SHIPPING_DEST_SECTOR_X +
            BOBBYR_SHIPPING_DEST_SECTOR_Y * MAP_WORLD_X
  ...
  export function FindOutIfAnyMercAboutToLeaveIsGonnaRenew(): void {
    ...
    for (iCounter = 0; iCounter < iNumberOnTeam; iCounter++) {
      ...
      if (pSoldier.ubWhatKindOfMercAmI == Enum260.MERC_TYPE__AIM_MERC) {
        ...
        if (ContractIsGoingToExpireSoon(pSoldier)) {
          ...
          ContractRenewalList[ubNumContractRenewals].ubProfileID =
            pSoldier.ubProfile;
    ...
    if (ubNumMercs == 0 && pSoldierWhoWillQuit != null) {
      ...
      pSoldierWhoWillQuit.ubContractRenewalQuoteCode =
        SOLDIER_CONTRACT_RENEW_QUOTE_115_USED;
    } else {
      // OK, pick one....
      if (ubNumMercs > 0) {
  ...
  function HandleNotifyPlayerCantAffordInsurance(): void {
    DoScreenIndependantMessageBox(
      zMarksMapScreenText[9],
      MSG_BOX_FLAG_OK,
      null,
    );
  }

  function HandleNotifyPlayerCanAffordInsurance(
    pSoldier: SOLDIERTYPE,
    ubLength: UINT8,
    iCost: INT32,
  ): void {
    ...
    DoScreenIndependantMessageBox(
      sString,
      MSG_BOX_FLAG_YESNO,
      ExtendMercInsuranceContractCallBack,
    );
  ...
  export function GetHourWhenContractDone(pSoldier: SOLDIERTYPE): UINT32 {
    ...
    uiArriveHour = Math.trunc(
      (pSoldier.uiTimeSoldierWillArrive -
        Math.trunc(pSoldier.uiTimeSoldierWillArrive / 1440) * 1440) /
        60,
    );
