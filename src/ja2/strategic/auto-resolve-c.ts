namespace ja2 {
  ...
  const CELL_CREATURE =
    CELL_AF_CREATURE | CELL_AM_CREATURE | CELL_YF_CREATURE | CELL_YM_CREATURE;
  ...
  const CELL_INVOLVEDINCOMBAT =
    CELL_FIREDATTARGET | CELL_DODGEDATTACK | CELL_HITBYATTACKER;
  ...
  function PlayAutoResolveSample(
    usNum: UINT32,
    usRate: UINT32,
    ubVolume: UINT32,
    ubLoops: UINT32,
  ...
  function PlayAutoResolveSampleFromFile(
    szFileName: string /* STR8 */,
    usRate: UINT32,
    ubVolume: UINT32,
    ubLoops: UINT32,
    uiPan: UINT32,
  ...
  export function EliminateAllEnemies(
    ubSectorX: UINT8,
    ubSectorY: UINT8,
  ): void {
    ...
    let i: INT32;
    let ubNumEnemies: UINT8[] /* [NUM_ENEMY_RANKS] */ = createArray(
      Enum188.NUM_ENEMY_RANKS,
      0,
    );
    ...
    if (!gpAR) {
      ({
        ubNumAdmins: ubNumEnemies[0],
        ubNumTroops: ubNumEnemies[1],
        ubNumElites: ubNumEnemies[2],
      } = GetNumberOfEnemiesInSector(ubSectorX, ubSectorY));

      for (
        ubRankIndex = 0;
        ubRankIndex < Enum188.NUM_ENEMY_RANKS;
        ubRankIndex++
      ) {
        for (i = 0; i < ubNumEnemies[ubRankIndex]; i++) {
          ...
          TrackEnemiesKilled(
            Enum189.ENEMY_KILLED_IN_AUTO_RESOLVE,
            RankIndexToSoldierClass(ubRankIndex),
          );
        }
      }
  ...
  function DoTransitionFromPreBattleInterfaceToAutoResolve(): void {
    ...
    while (iPercentage < 100) {
      ...
      iLeft =
        sStartLeft +
        Math.trunc(((sEndLeft - sStartLeft + 1) * iPercentage) / 100);
      iTop =
        sStartTop + Math.trunc(((sEndTop - sStartTop + 1) * iPercentage) / 100);
  ...
  export function EnterAutoResolveMode(
    ubSectorX: UINT8,
    ubSectorY: UINT8,
  ): void {
  ...
  export function AutoResolveScreenHandle(): UINT32 {
    ...
    if (gpAR.fEnteringAutoResolve) {
      ...
      if (gfExtraBuffer) DoTransitionFromPreBattleInterfaceToAutoResolve();
      else gpAR.fExpanding = true;
  ...
  function AssociateEnemiesWithStrategicGroups(): void {
    ...
    if (gubEnemyEncounterCode == Enum164.CREATURE_ATTACK_CODE) return;
    ...
    while (pGroup) {
      if (
        !pGroup.fPlayer &&
        pGroup.ubSectorX == gpAR.ubSectorX &&
  ...
  function CalculateSoldierCells(fReset: boolean): void {
    ...
    gpAR.uiTimeSlice = Math.trunc(
      (gpAR.uiTimeSlice * gpAR.ubTimeModifierPercentage) / 100,
    );
    ...
    if (gpAR.ubMercs) {
      iStartY =
        iTop +
        Math.trunc(
          (gpAR.sHeight - ((gpAR.ubMercRows + gpAR.ubCivRows) * 47 + 7)) / 2,
        ) +
        6;
      y = gpAR.ubMercRows;
      x = gpAR.ubMercCols;
      i = gpAR.ubMercs;
      gapStartRow =
        gpAR.ubMercRows - gpAR.ubMercRows * gpAR.ubMercCols + gpAR.ubMercs;
      for (x = 0; x < gpAR.ubMercCols; x++)
        for (y = 0; i && y < gpAR.ubMercRows; y++, i--) {
          ...
          MSYS_DefineRegion(
            <MOUSE_REGION>gpMercs[index].pRegion,
            gpMercs[index].xp,
            gpMercs[index].yp,
            gpMercs[index].xp + 50,
            gpMercs[index].yp + 44,
    ...
    if (gpAR.ubCivs) {
      ...
      i = gpAR.ubCivs;
      gapStartRow =
        gpAR.ubCivRows - gpAR.ubCivRows * gpAR.ubCivCols + gpAR.ubCivs;
    ...
    if (gpAR.ubEnemies) {
      iStartY =
        iTop + Math.trunc((gpAR.sHeight - (gpAR.ubEnemyRows * 47 + 7)) / 2) + 5;
      y = gpAR.ubEnemyRows;
      x = gpAR.ubEnemyCols;
      i = gpAR.ubEnemies;
      gapStartRow =
        gpAR.ubEnemyRows - gpAR.ubEnemyRows * gpAR.ubEnemyCols + gpAR.ubEnemies;
      for (x = 0; x < gpAR.ubEnemyCols; x++)
        for (y = 0; i && y < gpAR.ubEnemyRows; y++, i--) {
          ...
          if (gubEnemyEncounterCode != Enum164.CREATURE_ATTACK_CODE) {
            if (index < gpAR.ubElites) gpEnemies[index].uiFlags = CELL_ELITE;
            else if (index < gpAR.ubElites + gpAR.ubTroops)
              gpEnemies[index].uiFlags = CELL_TROOP;
            else gpEnemies[index].uiFlags = CELL_ADMIN;
          } else {
            if (index < gpAR.ubAFCreatures)
              ...
              index <
              gpAR.ubYFCreatures + gpAR.ubAMCreatures + gpAR.ubAFCreatures
            )
              gpEnemies[index].uiFlags = CELL_YF_CREATURE;
            else gpEnemies[index].uiFlags = CELL_YM_CREATURE;
          }
        }
    }
  }
  ...
  function RenderSoldierCell(pCell: SOLDIERCELL): void {
    ...
    if (
      pCell.pSoldier.bLife > 0 &&
      pCell.pSoldier.bLife < OKLIFE &&
      !(
        pCell.uiFlags &
        (CELL_HITBYATTACKER | CELL_HITLASTFRAME | CELL_CREATURE)
      )
    ) {
  ...
  function RenderSoldierCellBars(pCell: SOLDIERCELL): void {
    ...
    // BREATH BAR
    iStartY =
      pCell.yp + 29 - Math.trunc((25 * pCell.pSoldier.bBreathMax) / 100);
    ColorFillVideoSurfaceArea(
      FRAME_BUFFER,
      pCell.xp + 41,
      iStartY,
  ...
  function BuildInterfaceBuffer(): void {
    ...
    if ((gpAR.iInterfaceBuffer = AddVideoSurface(vs_desc)) === -1)
      AssertMsg(
        false,
        "Failed to allocate memory for autoresolve interface buffer.",
      );
  ...
  function ExpandWindow(): void {
    ...
    if (!gpAR.ExRect.iLeft && !gpAR.ExRect.iRight) {
      ...
      for (i = 0; i < Enum119.DONEWIN_BUTTON; i++) HideButton(gpAR.iButton[i]);
    } else {
      ...
      BlitBufferToBuffer(
        ...
        gpAR.ExRect.iTop,
        1,
        gpAR.ExRect.iBottom - gpAR.ExRect.iTop + 1,
      );
      InvalidateRegion(
        gpAR.ExRect.iLeft,
        gpAR.ExRect.iTop,
      ...
      BlitBufferToBuffer(
        ...
        gpAR.ExRect.iLeft,
        gpAR.ExRect.iTop,
        gpAR.ExRect.iRight - gpAR.ExRect.iLeft + 1,
        1,
      );
      InvalidateRegion(
        gpAR.ExRect.iLeft,
      ...
      if (
        uiCurrentTime >= gpAR.uiStartExpanding &&
        uiCurrentTime <= gpAR.uiEndExpanding
      ) {
        // Debug purposes
        ...
        uiPercent = Math.trunc(
          ((uiCurrentTime - gpAR.uiStartExpanding) * 100) / uiTimeRange,
        );
        ...
        if (OldRect.iLeft <= gpAR.Rect.iLeft)
          gpAR.ExRect.iLeft =
            OldRect.iLeft +
            Math.trunc(((gpAR.Rect.iLeft - OldRect.iLeft) * uiPercent) / 100);
        else
          gpAR.ExRect.iLeft =
            gpAR.Rect.iLeft +
            Math.trunc(((OldRect.iLeft - gpAR.Rect.iLeft) * uiPercent) / 100);
        // Top
        if (OldRect.iTop <= gpAR.Rect.iTop)
          gpAR.ExRect.iTop =
            OldRect.iTop +
            Math.trunc(((gpAR.Rect.iTop - OldRect.iTop) * uiPercent) / 100);
        else
          gpAR.ExRect.iTop =
            gpAR.Rect.iTop +
            Math.trunc(((OldRect.iTop - gpAR.Rect.iTop) * uiPercent) / 100);
        // Right
        if (OldRect.iRight <= gpAR.Rect.iRight)
          gpAR.ExRect.iRight =
            OldRect.iRight +
            Math.trunc(((gpAR.Rect.iRight - OldRect.iRight) * uiPercent) / 100);
        else
          gpAR.ExRect.iRight =
            gpAR.Rect.iRight +
            Math.trunc(((OldRect.iRight - gpAR.Rect.iRight) * uiPercent) / 100);
        // Bottom
        if (OldRect.iBottom <= gpAR.Rect.iBottom)
          gpAR.ExRect.iBottom =
            OldRect.iBottom +
            Math.trunc(
              ((gpAR.Rect.iBottom - OldRect.iBottom) * uiPercent) / 100,
            );
        else
          gpAR.ExRect.iBottom =
            gpAR.Rect.iBottom +
            Math.trunc(
              ((OldRect.iBottom - gpAR.Rect.iBottom) * uiPercent) / 100,
            );
      } else {
    ...
    RectangleDraw(
      ...
      gpAR.ExRect.iRight,
      gpAR.ExRect.iBottom,
      Get16BPPColor(FROMRGB(200, 200, 100)),
      pDestBuf,
    );
    UnLockVideoSurface(FRAME_BUFFER);
    // left
  ...
  export function VirtualSoldierDressWound(
    ...
    if (pSoldier.inv[0].usItem == Enum225.MEDICKIT)
    ...
    if (pVictim.bLife >= OKLIFE) bBelowOKlife = 0;
    else bBelowOKlife = OKLIFE - pVictim.bLife;
  ...
  function AutoBandageMercs(): UINT32 {
    ...
    for (i = 0; i < gpAR.ubMercs; i++) {
      if (
        gpMercs[i].pSoldier.bLife >= OKLIFE &&
        ...
        while (gpMercs[i].pSoldier.bBleeding) {
          ...
          uiPointsUsed = VirtualSoldierDressWound(
            gpMercs[i].pSoldier,
            gpMercs[i].pSoldier,
            pKit,
            usKitPts,
    ...
    if (fComplete) {
      DoScreenIndependantMessageBox(
        gzLateLocalizedString[13],
        MSG_BOX_FLAG_OK,
        AutoBandageFinishedCallback,
      );
    } else {
      DoScreenIndependantMessageBox(
        gzLateLocalizedString[10],
        MSG_BOX_FLAG_OK,
        AutoBandageFinishedCallback,
      );
    }
  ...
  function RenderAutoResolve(): void {
    ...
    xp =
      gpAR.sCenterStartX +
      70 -
      Math.trunc(StringPixLength(str, FONT10ARIALBOLD()) / 2);
    ...
    xp =
      gpAR.sCenterStartX +
      70 -
      Math.trunc(StringPixLength(str, FONT10ARIAL()) / 2);
    yp += 11;
    mprintf(xp, yp, str);
    ...
    xp =
      gpAR.sCenterStartX +
      70 -
      Math.trunc(StringPixLength(str, FONT14ARIAL()) / 2);
    yp += 11;
    mprintf(xp, yp, str);

    if (gpAR.fPendingSurrender) {
      DisplayWrappedString(
        gpAR.sCenterStartX + 16,
        230 + gpAR.bVerticalOffset,
        108,
        2,
        FONT10ARIAL(),
        FONT_YELLOW,
        gpStrategicString[Enum365.STR_ENEMY_SURRENDER_OFFER],
        FONT_BLACK,
        false,
        LEFT_JUSTIFIED,
      );
    ...
    if (gpAR.ubBattleStatus != Enum120.BATTLE_IN_PROGRESS) {
      ...
      if (!gpAR.fMoraleEventsHandled) {
        ...
        if (CheckFact(Enum170.FACT_FIRST_BATTLE_FOUGHT, 0) == false) {
          ...
          SetTheFirstBattleSector(
            gpAR.ubSectorX + gpAR.ubSectorY * MAP_WORLD_X,
          );
        ...
        switch (gpAR.ubBattleStatus) {
          case Enum120.BATTLE_VICTORY:
            ...
            SectorInfo[
              SECTOR(gpAR.ubSectorX, gpAR.ubSectorY)
            ].bLastKnownEnemies = 0;
          ...
          case Enum120.BATTLE_CAPTURED:
            ...
            gsEnemyGainedControlOfSectorID = SECTOR(
              gpAR.ubSectorX,
              gpAR.ubSectorY,
            );
            break;
        ...
        case Enum120.BATTLE_CAPTURED:
          if (gpAR.ubBattleStatus == Enum120.BATTLE_SURRENDERED) {
            ...
            DisplayWrappedString(
              ...
              FONT10ARIAL(),
              FONT_YELLOW,
              gpStrategicString[Enum365.STR_ENEMY_CAPTURED],
              FONT_BLACK,
              false,
              LEFT_JUSTIFIED,
            );
      ...
      xp =
        gpAR.sCenterStartX +
        70 -
        Math.trunc(StringPixLength(str, BLOCKFONT2()) / 2);
      ...
      str = swprintf(
        "%s:  %dm %ss",
        gpStrategicString[Enum365.STR_AR_TIME_ELAPSED],
        Math.trunc(gpAR.uiTotalElapsedBattleTimeInMilliseconds / 60000),
        Math.trunc((gpAR.uiTotalElapsedBattleTimeInMilliseconds % 60000) / 1000)
          .toString()
          .padStart(2, "0"),
      );
      xp =
  ...
  */
    gpAR.iButtonImage[Enum119.PAUSE_BUTTON] = LoadButtonImage(
      "Interface\\AutoBtns.sti",
      -1,
      0,
      -1,
      7,
    ...
    // Have the other buttons hook into the first button containing the images.
    gpAR.iButtonImage[Enum119.PLAY_BUTTON] = UseLoadedButtonImage(
      gpAR.iButtonImage[Enum119.PAUSE_BUTTON],
      -1,
      1,
      -1,
      8,
    ...
    gpAR.iButtonImage[Enum119.FAST_BUTTON] = UseLoadedButtonImage(
      gpAR.iButtonImage[Enum119.PAUSE_BUTTON],
      -1,
      2,
      -1,
    ...
    gpAR.iButtonImage[Enum119.FINISH_BUTTON] = UseLoadedButtonImage(
      gpAR.iButtonImage[Enum119.PAUSE_BUTTON],
      -1,
      3,
      -1,
    ...
    gpAR.iButtonImage[Enum119.YES_BUTTON] = UseLoadedButtonImage(
      gpAR.iButtonImage[Enum119.PAUSE_BUTTON],
      -1,
      4,
      -1,
    ...
    gpAR.iButtonImage[Enum119.NO_BUTTON] = UseLoadedButtonImage(
      gpAR.iButtonImage[Enum119.PAUSE_BUTTON],
      -1,
      5,
      -1,
    ...
    gpAR.iButtonImage[Enum119.BANDAGE_BUTTON] = UseLoadedButtonImage(
      gpAR.iButtonImage[Enum119.PAUSE_BUTTON],
      -1,
      6,
      -1,
    ...
    gpAR.iButtonImage[Enum119.RETREAT_BUTTON] = UseLoadedButtonImage(
      gpAR.iButtonImage[Enum119.PAUSE_BUTTON],
      -1,
      14,
      -1,
    ...
    gpAR.iButtonImage[Enum119.DONEWIN_BUTTON] = UseLoadedButtonImage(
      gpAR.iButtonImage[Enum119.PAUSE_BUTTON],
      -1,
      14,
      -1,
    ...
    gpAR.iButtonImage[Enum119.DONELOSE_BUTTON] = UseLoadedButtonImage(
      gpAR.iButtonImage[Enum119.PAUSE_BUTTON],
      -1,
      16,
      -1,
    ...
    ubEliteMilitia = MilitiaInSectorOfRank(
      gpAR.ubSectorX,
      gpAR.ubSectorY,
      Enum126.ELITE_MILITIA,
    );
    ubRegMilitia = MilitiaInSectorOfRank(
      gpAR.ubSectorX,
      gpAR.ubSectorY,
      Enum126.REGULAR_MILITIA,
    );
    ubGreenMilitia = MilitiaInSectorOfRank(
      gpAR.ubSectorX,
      gpAR.ubSectorY,
    ...
    for (i = 0; i < gpAR.ubCivs; i++) {
      ...
      if (i < ubEliteMilitia) {
        gpCivs[i].pSoldier = <SOLDIERTYPE>(
          TacticalCreateMilitia(Enum262.SOLDIER_CLASS_ELITE_MILITIA)
        );
        ...
        gpCivs[i].pSoldier = <SOLDIERTYPE>(
          TacticalCreateMilitia(Enum262.SOLDIER_CLASS_REG_MILITIA)
        );
        if (gpCivs[i].pSoldier.ubBodyType == Enum194.REGFEMALE) {
        ...
        gpCivs[i].pSoldier = <SOLDIERTYPE>(
          TacticalCreateMilitia(Enum262.SOLDIER_CLASS_GREEN_MILITIA)
        );
        if (gpCivs[i].pSoldier.ubBodyType == Enum194.REGFEMALE) {
    ...
    if (gubEnemyEncounterCode != Enum164.CREATURE_ATTACK_CODE) {
      for (i = 0, index = 0; i < gpAR.ubElites; i++, index++) {
        ...
        gpEnemies[index].pSoldier.name =
          gpStrategicString[Enum365.STR_AR_ELITE_NAME];
      }
      for (i = 0; i < gpAR.ubTroops; i++, index++) {
        ...
        gpEnemies[index].pSoldier.sSectorY = gpAR.ubSectorY;
        gpEnemies[index].pSoldier.name =
          gpStrategicString[Enum365.STR_AR_TROOP_NAME];
      }
      for (i = 0; i < gpAR.ubAdmins; i++, index++) {
        ...
        gpEnemies[index].pSoldier.sSectorY = gpAR.ubSectorY;
        gpEnemies[index].pSoldier.name =
          gpStrategicString[Enum365.STR_AR_ADMINISTRATOR_NAME];
      }
      AssociateEnemiesWithStrategicGroups();
    } else {
      for (i = 0, index = 0; i < gpAR.ubAFCreatures; i++, index++) {
        gpEnemies[index].pSoldier = <SOLDIERTYPE>(
          TacticalCreateCreature(Enum194.ADULTFEMALEMONSTER)
        );
        gpEnemies[index].uiVObjectID = gpAR.iFaces;
        ...
        gpEnemies[index].pSoldier.sSectorY = gpAR.ubSectorY;
        gpEnemies[index].pSoldier.name =
          gpStrategicString[Enum365.STR_AR_CREATURE_NAME];
      }
      for (i = 0; i < gpAR.ubAMCreatures; i++, index++) {
        gpEnemies[index].pSoldier = <SOLDIERTYPE>(
          TacticalCreateCreature(Enum194.AM_MONSTER)
        );
        gpEnemies[index].uiVObjectID = gpAR.iFaces;
      ...
      for (i = 0; i < gpAR.ubYFCreatures; i++, index++) {
        gpEnemies[index].pSoldier = <SOLDIERTYPE>(
          TacticalCreateCreature(Enum194.YAF_MONSTER)
        );
        gpEnemies[index].uiVObjectID = gpAR.iFaces;
      ...
      for (i = 0; i < gpAR.ubYMCreatures; i++, index++) {
        gpEnemies[index].pSoldier = <SOLDIERTYPE>(
          TacticalCreateCreature(Enum194.YAM_MONSTER)
        );
        gpEnemies[index].uiVObjectID = gpAR.iFaces;
    ...
    if (
      gpAR.ubSectorX == gWorldSectorX &&
      gpAR.ubSectorY == gWorldSectorY &&
      !gbWorldSectorZ
    ) {
    ...
    gpAR.iButton[Enum119.PLAY_BUTTON] = QuickCreateButton(
      gpAR.iButtonImage[Enum119.PLAY_BUTTON],
      gpAR.sCenterStartX + 11,
      240 + gpAR.bVerticalOffset,
      BUTTON_TOGGLE,
      MSYS_PRIORITY_HIGH,
      DEFAULT_MOVE_CALLBACK(),
      PlayButtonCallback,
    );
    gpAR.iButton[Enum119.FAST_BUTTON] = QuickCreateButton(
      gpAR.iButtonImage[Enum119.FAST_BUTTON],
      gpAR.sCenterStartX + 51,
      240 + gpAR.bVerticalOffset,
      BUTTON_TOGGLE,
      MSYS_PRIORITY_HIGH,
      DEFAULT_MOVE_CALLBACK(),
    ...
    gpAR.iButton[Enum119.FINISH_BUTTON] = QuickCreateButton(
      gpAR.iButtonImage[Enum119.FINISH_BUTTON],
      gpAR.sCenterStartX + 91,
      240 + gpAR.bVerticalOffset,
      BUTTON_TOGGLE,
      MSYS_PRIORITY_HIGH,
      DEFAULT_MOVE_CALLBACK(),
    ...
    gpAR.iButton[Enum119.PAUSE_BUTTON] = QuickCreateButton(
      gpAR.iButtonImage[Enum119.PAUSE_BUTTON],
      gpAR.sCenterStartX + 11,
      274 + gpAR.bVerticalOffset,
      BUTTON_TOGGLE,
      MSYS_PRIORITY_HIGH,
      DEFAULT_MOVE_CALLBACK(),
      PauseButtonCallback,
    ...
    gpAR.iButton[Enum119.RETREAT_BUTTON] = QuickCreateButton(
      gpAR.iButtonImage[Enum119.RETREAT_BUTTON],
      gpAR.sCenterStartX + 51,
      274 + gpAR.bVerticalOffset,
      BUTTON_TOGGLE,
      MSYS_PRIORITY_HIGH,
      DEFAULT_MOVE_CALLBACK(),
      RetreatButtonCallback,
    );
    if (!gpAR.ubMercs) {
      DisableButton(gpAR.iButton[Enum119.RETREAT_BUTTON]);
    }
    SpecifyGeneralButtonTextAttributes(
      gpAR.iButton[Enum119.RETREAT_BUTTON],
      gpStrategicString[Enum365.STR_AR_RETREAT_BUTTON],
      BLOCKFONT2(),
      169,
      FONT_NEARBLACK,
    );

    gpAR.iButton[Enum119.BANDAGE_BUTTON] = QuickCreateButton(
      gpAR.iButtonImage[Enum119.BANDAGE_BUTTON],
      gpAR.sCenterStartX + 11,
      245 + gpAR.bVerticalOffset,
      BUTTON_NO_TOGGLE,
      MSYS_PRIORITY_HIGH,
      DEFAULT_MOVE_CALLBACK(),
      BandageButtonCallback,
    );

    gpAR.iButton[Enum119.DONEWIN_BUTTON] = QuickCreateButton(
      gpAR.iButtonImage[Enum119.DONEWIN_BUTTON],
      gpAR.sCenterStartX + 51,
      245 + gpAR.bVerticalOffset,
      BUTTON_TOGGLE,
      MSYS_PRIORITY_HIGH,
    ...
    SpecifyGeneralButtonTextAttributes(
      gpAR.iButton[Enum119.DONEWIN_BUTTON],
      gpStrategicString[Enum365.STR_AR_DONE_BUTTON],
      BLOCKFONT2(),
      169,
      FONT_NEARBLACK,
    );

    gpAR.iButton[Enum119.DONELOSE_BUTTON] = QuickCreateButton(
      gpAR.iButtonImage[Enum119.DONELOSE_BUTTON],
      gpAR.sCenterStartX + 25,
      245 + gpAR.bVerticalOffset,
      BUTTON_TOGGLE,
      MSYS_PRIORITY_HIGH,
      DEFAULT_MOVE_CALLBACK(),
    ...
    SpecifyGeneralButtonTextAttributes(
      gpAR.iButton[Enum119.DONELOSE_BUTTON],
      gpStrategicString[Enum365.STR_AR_DONE_BUTTON],
      BLOCKFONT2(),
      169,
      FONT_NEARBLACK,
    );
    gpAR.iButton[Enum119.YES_BUTTON] = QuickCreateButton(
      gpAR.iButtonImage[Enum119.YES_BUTTON],
      gpAR.sCenterStartX + 21,
      257 + gpAR.bVerticalOffset,
      BUTTON_TOGGLE,
      MSYS_PRIORITY_HIGH,
      DEFAULT_MOVE_CALLBACK(),
      AcceptSurrenderCallback,
    );
    gpAR.iButton[Enum119.NO_BUTTON] = QuickCreateButton(
      gpAR.iButtonImage[Enum119.NO_BUTTON],
      gpAR.sCenterStartX + 81,
      257 + gpAR.bVerticalOffset,
      BUTTON_TOGGLE,
      MSYS_PRIORITY_HIGH,
      DEFAULT_MOVE_CALLBACK(),
      RejectSurrenderCallback,
    );
    HideButton(gpAR.iButton[Enum119.YES_BUTTON]);
    HideButton(gpAR.iButton[Enum119.NO_BUTTON]);
  ...
  function RemoveAutoResolveInterface(fDeleteForGood: boolean): void {
        ...
        if (i >= gpAR.iActualMercFaces)
          ...
          if (!gpMercs[i].pSoldier.bLife) {
            ...
            ChangeSoldiersAssignment(
              gpMercs[i].pSoldier,
              Enum117.ASSIGNMENT_DEAD,
            );

            AddDeadSoldierToUnLoadedSector(
              gpAR.ubSectorX,
              gpAR.ubSectorY,
              0,
              gpMercs[i].pSoldier,
            ...
            gpMercs[i].pSoldier.ubStrategicInsertionCode =
              Enum175.INSERTION_CODE_CENTER;
          }
      ...
      for (i = 0; i < gpAR.iNumMercFaces; i++) {
        if (
          ...
          if (gpMercs[i].pSoldier.ubGroupID != ubCurrentGroupID) {
            ...
            PlayerGroupArrivedSafelyInSector(
              GetGroup(gpMercs[i].pSoldier.ubGroupID),
              fFirstGroup,
            );
            fFirstGroup = false;
          }
    ...
    for (i = 0; i < MAX_ALLOWABLE_MILITIA_PER_SECTOR; i++) {
      if (gpCivs[i].pSoldier) {
        ...
        if (
          fDeleteForGood &&
          gpCivs[i].pSoldier.bLife < Math.trunc(OKLIFE / 2)
        ) {
          AddDeadSoldierToUnLoadedSector(
            gpAR.ubSectorX,
            gpAR.ubSectorY,
          ...
          if (
            ...
            if (ubPromotions) {
              if (ubPromotions == 2) {
                ...
                Enum262.SOLDIER_CLASS_GREEN_MILITIA
              ) {
                gbGreenToRegPromotions++;
                gbMilitiaPromotions++;
              } else if (
                gpCivs[i].pSoldier.ubSoldierClass ==
                Enum262.SOLDIER_CLASS_REG_MILITIA
              ) {
    ...
    for (i = 0; i < 32; i++) {
      if (gpEnemies[i].pSoldier) {
        if (fDeleteForGood && gpEnemies[i].pSoldier.bLife < OKLIFE) {
          TrackEnemiesKilled(
            Enum189.ENEMY_KILLED_IN_AUTO_RESOLVE,
            gpEnemies[i].pSoldier.ubSoldierClass,
          );
          HandleGlobalLoyaltyEvent(
            Enum190.GLOBAL_LOYALTY_ENEMY_KILLED,
            gpAR.ubSectorX,
    ...
    if (fDeleteForGood) {
      ...
      WarpGameTime(
        Math.trunc(gpAR.uiTotalElapsedBattleTimeInMilliseconds / 1000),
  ...
  function PauseButtonCallback(btn: GUI_BUTTON, reason: INT32): void {
    if (reason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
      ButtonList[gpAR.iButton[Enum119.PLAY_BUTTON]].uiFlags &=
        ~BUTTON_CLICKED_ON;
      ButtonList[gpAR.iButton[Enum119.FAST_BUTTON]].uiFlags &=
        ~BUTTON_CLICKED_ON;
      ButtonList[gpAR.iButton[Enum119.FINISH_BUTTON]].uiFlags &=
        ~BUTTON_CLICKED_ON;
      gpAR.fPaused = true;
    }
  }
  ...
  function PlayButtonCallback(btn: GUI_BUTTON, reason: INT32): void {
    if (reason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
      ButtonList[gpAR.iButton[Enum119.PAUSE_BUTTON]].uiFlags &=
        ~BUTTON_CLICKED_ON;
      ButtonList[gpAR.iButton[Enum119.FAST_BUTTON]].uiFlags &=
        ~BUTTON_CLICKED_ON;
      ButtonList[gpAR.iButton[Enum119.FINISH_BUTTON]].uiFlags &=
        ~BUTTON_CLICKED_ON;
      gpAR.uiTimeSlice = Math.trunc(
        (1000 * gpAR.ubTimeModifierPercentage) / 100,
      );
      gpAR.fPaused = false;
    }
  }
  ...
  function DetermineBandageButtonState(): void {
    ...
    if (!fFound) {
      ...
      SetButtonFastHelpText(
        gpAR.iButton[Enum119.BANDAGE_BUTTON],
        gzLateLocalizedString[11],
      );
      return;
    }
  ...
  function MercCellMouseClickCallback(reg: MOUSE_REGION, reason: INT32): void {
    if (reason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
      ...
      // Gets to retreat after a total of 2 attacks.
      pCell.usNextAttack =
        (1000 + pCell.usNextAttack * 5 + PreRandom(2000 - pCell.usAttack)) * 2;
  ...
  function CalculateAutoResolveInfo(): void {
    ...
    if (gubEnemyEncounterCode != Enum164.CREATURE_ATTACK_CODE) {
      ({
        ubNumAdmins: gpAR.ubAdmins,
        ubNumTroops: gpAR.ubTroops,
        ubNumElites: gpAR.ubElites,
      } = GetNumberOfEnemiesInSector(gpAR.ubSectorX, gpAR.ubSectorY));
      gpAR.ubEnemies = Math.min(
      ...
      if (gfTransferTacticalOppositionToAutoResolve) {
        ...
        ({
          ...
          ubNumAdultMales: gpAR.ubAMCreatures,
          ubNumAdultFemales: gpAR.ubAFCreatures,
        } = DetermineCreatureTownComposition(
          gubNumCreaturesAttackingTown,
          gpAR.ubYMCreatures,
          gpAR.ubYFCreatures,
          gpAR.ubAMCreatures,
          gpAR.ubAFCreatures,
  ...
  function ResetAutoResolveInterface(): void {
    ...
    while (
      gpAR.iNumMercFaces > gpAR.ubMercs &&
      gpAR.iNumMercFaces > gpAR.iActualMercFaces
    ) {
      // Removing temp mercs
      gpAR.iNumMercFaces--;
  ...
  function CalculateRowsAndColumns(): void {
    ...
    if (gpAR.ubMercs && gpAR.ubCivs && gpAR.ubMercCols != gpAR.ubCivCols) {
      if (gpAR.ubMercCols < gpAR.ubCivCols) {
        ...
        gpAR.ubMercRows = Math.trunc(
          (gpAR.ubMercs + gpAR.ubMercCols - 1) / gpAR.ubMercCols,
        );
      } else {
        gpAR.ubCivCols = gpAR.ubMercCols;
        gpAR.ubCivRows = Math.trunc(
          (gpAR.ubCivs + gpAR.ubCivCols - 1) / gpAR.ubCivCols,
        );
      }
    }
    ...
    if (gpAR.ubMercCols + gpAR.ubEnemyCols == 9) gpAR.sWidth = 640;
    else
      gpAR.sWidth =
        146 +
        55 *
          (Math.max(Math.max(gpAR.ubMercCols, gpAR.ubCivCols), 2) +
            Math.max(gpAR.ubEnemyCols, 2));

    gpAR.sCenterStartX =
      323 -
      Math.trunc(gpAR.sWidth / 2) +
      Math.max(Math.max(gpAR.ubMercCols, 2), Math.max(gpAR.ubCivCols, 2)) * 55;

    // Anywhere from 48*3 to 48*10
    gpAR.sHeight =
      48 *
      Math.max(3, Math.max(gpAR.ubMercRows + gpAR.ubCivRows, gpAR.ubEnemyRows));
    ...
    if (
      ...
      if (pCell.pSoldier.bLife >= OKLIFE) {
        ...
        xp =
          pCell.xp +
          25 -
          Math.trunc(StringPixLength(pStr, SMALLCOMPFONT()) / 2);
        yp = pCell.yp + 12;
        mprintf(xp, yp, str);
      }
    }
  ...
  function CreateTempPlayerMerc(): void {
    ...
    gpMercs[gpAR.iNumMercFaces].pSoldier = <SOLDIERTYPE>TacticalCreateSoldier(
      MercCreateStruct,
      createPointer(
        () => ubID,
        (v) => (ubID = v),
  ...
  function ResetNextAttackCounter(pCell: SOLDIERCELL): void {
    ...
    pCell.usNextAttack =
      1000 + pCell.usNextAttack * 5 + PreRandom(2000 - pCell.usAttack);
    if (pCell.uiFlags & CELL_CREATURE) {
  ...
  function CalculateAttackValues(): void {
    ...
    for (i = 0; i < gpAR.ubMercs; i++) {
      ...
      if (pCell.usAttack < 1000) {
        ...
        pCell.usAttack =
          pCell.usAttack + Math.trunc((1000 - pCell.usAttack) / 4);
      }
      usBreathStrengthPercentage =
        100 - Math.trunc((100 - pCell.pSoldier.bBreathMax) / 3);
      pCell.usAttack = Math.trunc(
        (pCell.usAttack * usBreathStrengthPercentage) / 100,
      );
      pCell.usDefence =
        pSoldier.bAgility +
        pSoldier.bWisdom +
  ...
  function ChooseTarget(pAttacker: SOLDIERCELL): SOLDIERCELL {
    ...
    if (pAttacker.uiFlags & (CELL_ENEMY | CELL_CREATURE)) {
      ...
      while (iAvailableTargets) {
        pTarget =
          index < gpAR.ubMercs ? gpMercs[index] : gpCivs[index - gpAR.ubMercs];
      ...
      if (!IsBattleOver()) {
        AssertMsg(
          0,
          FormatString(
            "***Please send PRIOR save and screenshot of this message***  iAvailableTargets %d, index %d, iRandom %d, defence %d. ",
            iAvailableTargets,
            index,
            iRandom,
            gpAR.usPlayerDefence,
          ),
        );
      }
    } else {
    ...
    for (i = 0; i < Enum261.NUM_INV_SLOTS; i++) {
      ...
      if (Item[pItem.usItem].usItemClass == IC_GUN) {
        ...
        if (gpAR.fUnlimitedAmmo) {
          PlayAutoResolveSample(
            Weapon[pItem.usItem].sSound,
            RATE_11025,
            50,
            1,
            MIDDLEPAN,
        ...
        if (!pItem.ubGunShotsLeft) {
          ...
          if (pItem.ubGunShotsLeft && Weapon[pItem.usItem].sLocknLoadSound) {
            PlayAutoResolveSample(
              Weapon[pItem.usItem].sLocknLoadSound,
              RATE_11025,
              50,
              1,
              MIDDLEPAN,
  ...
  function AttackTarget(pAttacker: SOLDIERCELL, pTarget: SOLDIERCELL): void {
    ...
    else usAttack = 950 + PreRandom(50);
    if (
      pTarget.uiFlags & CELL_RETREATING &&
      !(pAttacker.uiFlags & CELL_FEMALECREATURE)
    ) {
    ...
    else usDefence = 950 + PreRandom(50);
    if (pAttacker.uiFlags & CELL_FEMALECREATURE) {
      ...
      if (TargetHasLoadedGun(pTarget.pSoldier)) {
        ...
        if (!(pAttacker.uiFlags & CELL_CREATURE)) {
          ...
          if (fKnife) usAttack = Math.trunc((usAttack * 6) / 10);
          else usAttack = Math.trunc((usAttack * 4) / 10);
        }
      }
    }
    ...
    if (!fMelee) {
      ubImpact =
        Weapon[pAttacker.pSoldier.inv[pAttacker.bWeaponSlot].usItem].ubImpact;
      iRandom = PreRandom(100);
      if (iRandom < 15) ubLocation = AIM_SHOT_HEAD;
      else if (iRandom < 30) ubLocation = AIM_SHOT_LEGS;
      else ubLocation = AIM_SHOT_TORSO;
      ubAccuracy = Math.trunc(
        (usAttack - usDefence + PreRandom(usDefence - pTarget.usDefence)) / 10,
      );
      iImpact = BulletImpact(
        pAttacker.pSoldier,
        pTarget.pSoldier,
        ubLocation,
        ubImpact,
      ...
      PlayAutoResolveSample(
        Enum330.BULLET_IMPACT_1 + PreRandom(3),
        RATE_11025,
        50,
        1,
        MIDDLEPAN,
      ...
      if (pAttacker.bWeaponSlot != Enum261.HANDPOS) {
        ...
        copyObjectType(
          pAttacker.pSoldier.inv[Enum261.HANDPOS],
          pAttacker.pSoldier.inv[pAttacker.bWeaponSlot],
        );
        iImpact = HTHImpact(
          pAttacker.pSoldier,
          pTarget.pSoldier,
      ...
      if (
        pTarget.pSoldier.bLife >= CONSCIOUSNESS ||
        pTarget.uiFlags & CELL_CREATURE
      ) {
        if (gpAR.fSound)
          DoMercBattleSound(
            pTarget.pSoldier,
            Enum259.BATTLE_SOUND_HIT1 + PreRandom(2),
          );
      }
      if (
        !(pTarget.uiFlags & CELL_CREATURE) &&
        iNewLife < OKLIFE &&
        pTarget.pSoldier.bLife >= OKLIFE
      ) {
      ...
      if (fKnife || fClaw) {
        if (
          pTarget.pSoldier.bLifeMax - pTarget.pSoldier.bBleeding - iImpact >=
          pTarget.pSoldier.bLife
        )
          pTarget.pSoldier.bBleeding += iImpact;
        else
          pTarget.pSoldier.bBleeding =
            pTarget.pSoldier.bLifeMax - pTarget.pSoldier.bLife;
      }
      if (!pTarget.pSoldier.bLife) {
        gpAR.fRenderAutoResolve = true;
        if (pTarget.uiFlags & CELL_MERC) {
  ...
  function TargetHitCallback(pTarget: SOLDIERCELL, index: INT32): void {
    ...
    switch (pTarget.pSoldier.ubBodyType) {
      ...
      case Enum194.YAM_MONSTER:
        pTarget.usHitDamage[index] = Math.trunc(
          (pTarget.usHitDamage[index] + 2) / 4,
        );
        break;
      case Enum194.ADULTFEMALEMONSTER:
      case Enum194.AM_MONSTER:
        pTarget.usHitDamage[index] = Math.trunc(
          (pTarget.usHitDamage[index] + 3) / 6,
        );
        break;
      case Enum194.QUEENMONSTER:
        pTarget.usHitDamage[index] = Math.trunc(
          (pTarget.usHitDamage[index] + 4) / 8,
        );
        break;
    }
    ...
    if (pTarget.uiFlags & CELL_MERC && pTarget.usHitDamage[index]) {
      ...
      StatChange(
        pTarget.pSoldier,
        EXPERAMT,
        5 * Math.trunc(pTarget.usHitDamage[index] / 10),
        0,
      );
    }
    ...
    if (
      pTarget.pSoldier.bLifeMax -
        pTarget.pSoldier.bBleeding -
        pTarget.usHitDamage[index] >=
      pTarget.pSoldier.bLife
    )
  ...
  function IsBattleOver(): boolean {
    ...
    if (gpAR.ubBattleStatus != Enum120.BATTLE_IN_PROGRESS) return true;
    for (i = 0; i < gpAR.ubMercs; i++) {
  ...
  function AttemptPlayerCapture(): boolean {
    ...
    if (
      GetWorldDay() < STARTDAY_ALLOW_PLAYER_CAPTURE_FOR_RESCUE &&
      !gpAR.fAllowCapture
    ) {
      return false;
    }
  ...
  function ProcessBattleFrame(): void {
    ...
    if (!gotoContinueBattle) {
      ...
      if (gpAR.uiTimeSlice < 0xffffffff) {
        ProcessBattleFrame__iTimeSlice = Math.trunc(
          (uiDiff * gpAR.uiTimeSlice) / 1000,
        );
      } else {
    ...
    while (gotoContinueBattle || ProcessBattleFrame__iTimeSlice > 0) {
      if (!gotoContinueBattle) {
        ProcessBattleFrame__uiSlice = Math.min(
          ProcessBattleFrame__iTimeSlice,
          1000,
        );
        if (gpAR.ubBattleStatus == Enum120.BATTLE_IN_PROGRESS)
          gpAR.uiTotalElapsedBattleTimeInMilliseconds +=
            ProcessBattleFrame__uiSlice;

        // Now process each of the players
        ProcessBattleFrame__iTotal =
          gpAR.ubMercs + gpAR.ubCivs + gpAR.ubEnemies + 1;
        ProcessBattleFrame__iMercs = ProcessBattleFrame__iMercsLeft =
          gpAR.ubMercs;
        ProcessBattleFrame__iCivs = ProcessBattleFrame__iCivsLeft = gpAR.ubCivs;
        ProcessBattleFrame__iEnemies = ProcessBattleFrame__iEnemiesLeft =
          gpAR.ubEnemies;
        for (i = 0; i < gpAR.ubMercs; i++)
      ...
      while (gotoContinueBattle || --ProcessBattleFrame__iTotal) {
        ...
        if (!gotoContinueBattle) {
          if (
            (ProcessBattleFrame__iTimeSlice != 0x7fffffff &&
              GetJA2Clock() > gpAR.uiCurrTime + 17) ||
            (!gpAR.fInstantFinish &&
              iAttacksThisFrame >
                Math.trunc((gpAR.ubMercs + gpAR.ubCivs + gpAR.ubEnemies) / 4))
          ) {
        ...
        found = false;
        if (
          ProcessBattleFrame__iMercs &&
          iRandom < ProcessBattleFrame__iMercsLeft
        ) {
          ...
          ProcessBattleFrame__iCivs &&
          iRandom <
            ProcessBattleFrame__iMercsLeft + ProcessBattleFrame__iCivsLeft
        ) {
        ...
        } else AssertMsg(0, "Logic error in ProcessBattleFrame()");
        ...
        if (
          pAttacker.pSoldier.bLife < OKLIFE ||
        ...
        if (iTime > 0) {
          ...
          if (pAttacker.usAttack) {
            ...
            if (pAttacker.uiFlags & CELL_CREATURE && PreRandom(100) < 7)
              PlayAutoResolveSample(
                Enum330.ACR_SMELL_THREAT + PreRandom(2),
                RATE_11025,
                50,
                1,
                MIDDLEPAN,
              );
            else AttackTarget(pAttacker, pTarget);
      ...
      if (ProcessBattleFrame__iTimeSlice != 0x7fffffff) {
        //|| !gpAR->fInstantFinish )
  ...
  export function GetCurrentBattleSectorXYZ(): {
    sSectorX: INT16;
    sSectorY: INT16;
    sSectorZ: INT16;
  } {
  ...
  export function GetCurrentBattleSectorXYZAndReturnTRUEIfThereIsABattle(
    psSectorX: Pointer<INT16>,
    psSectorY: Pointer<INT16>,
    psSectorZ: Pointer<INT16>,
  ): boolean {
    if (gpAR) {
