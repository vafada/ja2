namespace ja2 {
  ...
  export function copyStrategicEvent(
    destination: STRATEGICEVENT,
    source: STRATEGICEVENT,
  ) {
  ...
  export function readStrategicEvent(
    o: STRATEGICEVENT,
    buffer: Buffer,
    offset: number = 0,
  ): number {
    ...
    offset += 4;
    o.uiParam = buffer.readUInt32LE(offset);
    offset += 4;
    o.uiTimeOffset = buffer.readUInt32LE(offset);
    offset += 4;
  ...
  export function writeStrategicEvent(
    o: STRATEGICEVENT,
    buffer: Buffer,
    offset: number = 0,
  ): number {
