namespace ja2 {
  ...
  export let gubFact: (boolean | number)[] /* UINT8[NUM_FACTS] */ = createArray(
    NUM_FACTS,
    false,
  ...
  export function SetFactTrue(usFact: UINT16): void {
    ...
    if (
      usFact == Enum170.FACT_ESTONI_REFUELLING_POSSIBLE &&
      CheckFact(usFact, 0) == false
    ) {
      // give him some gas...
  ...
  function CheckNPCWounded(
    ubProfileID: UINT8,
    fByPlayerOnly: boolean,
  ): boolean {
    ...
    if (pSoldier && pSoldier.bLife < pSoldier.bLifeMax) {
      if (fByPlayerOnly) {
        if (
          gMercProfiles[ubProfileID].ubMiscFlags &
  ...
  function CheckNPCWithin(
    ...
    return (
      PythSpacesAway(pFirstNPC.sGridNo, pSecondNPC.sGridNo) <= ubMaxDistance
    );
  }
  ...
  function CheckNPCIsEnemy(ubProfileID: UINT8): boolean {
    ...
    if (pNPC.bSide == gbPlayerNum || pNPC.bNeutral) {
      if (pNPC.ubCivilianGroup != Enum246.NON_CIV_GROUP) {
        ...
        return (
          gTacticalStatus.fCivGroupHostile[pNPC.ubCivilianGroup] >=
  ...
  function NumWoundedMercsNearby(ubProfileID: UINT8): INT8 {
    ...
    for (uiLoop = 0; uiLoop < guiNumMercSlots; uiLoop++) {
      ...
      if (
        ...
        if (
          PythSpacesAway(sGridNo, pSoldier.sGridNo) <= HOSPITAL_PATIENT_DISTANCE
        ) {
          bNumber++;
        }
      }
  ...
  function NPCInRoomRange(
    ...
    if (
      !pNPC ||
      gubWorldRoomInfo[pNPC.sGridNo] < ubRoomID1 ||
  ...
  function CheckTalkerFemale(): boolean {
    if (
      gpSrcSoldier &&
      gpSrcSoldier.bTeam == gbPlayerNum &&
      gpSrcSoldier.ubProfile != NO_PROFILE
    ) {
      return gMercProfiles[gpSrcSoldier.ubProfile].bSex == Enum272.FEMALE;
    } else if (
      gpDestSoldier &&
      gpDestSoldier.bTeam == gbPlayerNum &&
  ...
  function CheckTalkerUnpropositionedFemale(): boolean {
    if (
      ...
      if (
        !(
          gMercProfiles[gpDestSoldier.ubProfile].ubMiscFlags2 &
          PROFILE_MISC_FLAG2_ASKED_BY_HICKS
        )
      ) {
        return gMercProfiles[gpDestSoldier.ubProfile].bSex == Enum272.FEMALE;
    ...
    for (uiLoop = 0; uiLoop < guiNumMercSlots; uiLoop++) {
      ...
      if (
        ...
        if (
          ...
          gMercProfiles[pSoldier.ubProfile].bSex == Enum272.MALE
        ) {
  ...
  function FemalePresent(ubProfileID: UINT8): boolean {
    ...
    for (uiLoop = 0; uiLoop < guiNumMercSlots; uiLoop++) {
      ...
      if (
        ...
        if (
          pSoldier.ubProfile != NO_PROFILE &&
          gMercProfiles[pSoldier.ubProfile].bSex == Enum272.FEMALE
        ) {
  ...
  function CheckPlayerHasHead(): boolean {
    ...
    for (
      ...
      if (pSoldier.bActive && pSoldier.bLife > 0) {
        if (
          FindObjInObjRange(pSoldier, Enum225.HEAD_2, Enum225.HEAD_7) != NO_SLOT
        ) {
          return true;
        }
      }
  ...
  function CheckNPCSector(
    ...
    if (pSoldier) {
      ...
      gMercProfiles[ubProfileID].sSectorX == sSectorX &&
  ...
  function InTownSectorWithTrainingLoyalty(
    sSectorX: INT16,
    sSectorY: INT16,
  ): boolean {
    let ubTown: UINT8;
    ...
    if (
      ubTown != Enum135.BLANK_SECTOR &&
      gTownLoyalty[ubTown].fStarted &&
      gfTownUsesLoyalty[ubTown]
    ) {
  ...
  export function CheckFact(usFact: UINT16, ubProfileID: UINT8): boolean {
    ...
    switch (usFact) {
      case Enum170.FACT_DIMITRI_DEAD:
        gubFact[usFact] =
          gMercProfiles[Enum268.DIMITRI].bMercStatus == MERC_IS_DEAD;
        break;
      case Enum170.FACT_CURRENT_SECTOR_IS_SAFE:
        gubFact[Enum170.FACT_CURRENT_SECTOR_IS_SAFE] = !(
      ...
      case Enum170.FACT_NEW_BOBBYRAY_SHIPMENT_WAITING:
        if (
          gubFact[Enum170.FACT_PABLO_PUNISHED_BY_PLAYER] == true &&
          gubFact[Enum170.FACT_PABLO_RETURNED_GOODS] == false &&
          gMercProfiles[Enum268.PABLO].bMercStatus != MERC_IS_DEAD
        ) {
          gubFact[Enum170.FACT_BOBBYRAY_SHIPMENT_IN_TRANSIT] = false;
          gubFact[Enum170.FACT_NEW_BOBBYRAY_SHIPMENT_WAITING] = false;
        } else {
          if (CheckForNewShipment()) {
            // if new stuff waiting unseen in Drassen
            gubFact[Enum170.FACT_BOBBYRAY_SHIPMENT_IN_TRANSIT] = false;
            gubFact[Enum170.FACT_NEW_BOBBYRAY_SHIPMENT_WAITING] = true;
          } else if (CountNumberOfBobbyPurchasesThatAreInTransit() > 0) {
            // if stuff in transit
      ...
      case Enum170.FACT_NPC_WOUNDED_BY_PLAYER:
        gubFact[Enum170.FACT_NPC_WOUNDED_BY_PLAYER] = CheckNPCWounded(
          ubProfileID,
          true,
        );
        break;
      case Enum170.FACT_IRA_NOT_PRESENT:
        gubFact[Enum170.FACT_IRA_NOT_PRESENT] = !CheckNPCWithin(
          ubProfileID,
          Enum268.IRA,
          10,
        );
      ...
      case Enum170.FACT_IRA_UNHIRED_AND_ALIVE:
        if (
          gMercProfiles[Enum268.IRA].bMercStatus != MERC_IS_DEAD &&
          CheckNPCSector(Enum268.IRA, 10, 1, 1) &&
          !(
            gMercProfiles[Enum268.IRA].ubMiscFlags & PROFILE_MISC_FLAG_RECRUITED
          )
        ) {
      ...
      case Enum170.FACT_NPC_BLEEDING_BUT_OKAY:
        if (
          ...
          CheckNPCInOkayHealth(ubProfileID)
        ) {
      ...
      case Enum170.FACT_PLAYER_HAS_HEAD_AND_CARMEN_IN_SAN_MONA:
        gubFact[usFact] =
          CheckNPCSector(Enum268.CARMEN, 5, MAP_ROW_C, 0) &&
          CheckPlayerHasHead();
        break;

      case Enum170.FACT_PLAYER_HAS_HEAD_AND_CARMEN_IN_CAMBRIA:
        gubFact[usFact] =
          CheckNPCSector(Enum268.CARMEN, 9, MAP_ROW_G, 0) &&
          CheckPlayerHasHead();
        break;

      case Enum170.FACT_PLAYER_HAS_HEAD_AND_CARMEN_IN_DRASSEN:
        gubFact[usFact] =
          CheckNPCSector(Enum268.CARMEN, 13, MAP_ROW_C, 0) &&
          CheckPlayerHasHead();
        break;

      case Enum170.FACT_NPC_OWED_MONEY:
        gubFact[Enum170.FACT_NPC_OWED_MONEY] =
          gMercProfiles[ubProfileID].iBalance < 0;
        break;

      case Enum170.FACT_FATHER_DRUNK:
        gubFact[Enum170.FACT_FATHER_DRUNK] =
          gMercProfiles[Enum268.FATHER].bNPCData >= 5;
        break;

      case Enum170.FACT_MICKY_DRUNK:
        gubFact[Enum170.FACT_MICKY_DRUNK] =
          gMercProfiles[Enum268.MICKY].bNPCData >= 5;
        break;
      ...
      case Enum170.FACT_BRENDA_IN_STORE_AND_ALIVE:
        ...
        // ensure in a building and nearby
          ...
          gubFact[Enum170.FACT_BRENDA_IN_STORE_AND_ALIVE] = CheckNPCWithin(
            ubProfileID,
            85,
            12,
          );
      ...
      case Enum170.FACT_BRENDA_DEAD:
        gubFact[Enum170.FACT_BRENDA_DEAD] =
          gMercProfiles[85].bMercStatus == MERC_IS_DEAD;
        break;
      case Enum170.FACT_NPC_IS_ENEMY:
        gubFact[Enum170.FACT_NPC_IS_ENEMY] =
          CheckNPCIsEnemy(ubProfileID) ||
          Boolean(
            gMercProfiles[ubProfileID].ubMiscFlags2 &
...
case FACT_SKYRIDER_CLOSE_TO_CHOPPER:
      ...
      */
      ...
      case Enum170.FACT_MARIA_ESCORTED_AT_LEATHER_SHOP:
        gubFact[usFact] =
          CheckNPCIsEPC(Enum268.MARIA) && NPCInRoom(Enum268.MARIA, 2);
        break;
      case Enum170.FACT_PC_STRONG_AND_LESS_THAN_3_MALES_PRESENT:
        gubFact[usFact] =
          CheckTalkerStrong() && NumMalesPresent(ubProfileID) < 3;
        break;
      ...
      case Enum170.FACT_JOEY_NEAR_MARTHA:
        gubFact[usFact] =
          CheckNPCWithin(90, 109, 5) &&
          (CheckGuyVisible(Enum268.MARTHA, Enum268.JOEY) ||
            CheckGuyVisible(Enum268.JOEY, Enum268.MARTHA));
        break;
      ...
      case Enum170.FACT_SLAY_IN_SECTOR:
        gubFact[usFact] =
          gMercProfiles[Enum268.SLAY].sSectorX == gWorldSectorX &&
          gMercProfiles[Enum268.SLAY].sSectorY == gWorldSectorY &&
          gMercProfiles[Enum268.SLAY].bSectorZ == gbWorldSectorZ;
        break;
      case Enum170.FACT_SLAY_HIRED_AND_WORKED_FOR_48_HOURS:
        gubFact[usFact] =
          Boolean(
            gMercProfiles[Enum268.SLAY].ubMiscFlags &
              PROFILE_MISC_FLAG_RECRUITED,
          ) && gMercProfiles[Enum268.SLAY].usTotalDaysServed > 1;
        break;
      case Enum170.FACT_SHANK_IN_SQUAD_BUT_NOT_SPEAKING:
        gubFact[usFact] =
          FindSoldierByProfileID(Enum268.SHANK, true) != null &&
          Boolean(
            gMercProfiles[Enum268.SHANK].ubMiscFlags &
              PROFILE_MISC_FLAG_RECRUITED,
          ) &&
          (gpSrcSoldier == null || gpSrcSoldier.ubProfile != Enum268.SHANK);
        break;
      ...
      case Enum170.FACT_QUEEN_DEAD:
        gubFact[usFact] =
          gMercProfiles[Enum268.QUEEN].bMercStatus == MERC_IS_DEAD;
        break;
      ...
      case Enum170.FACT_MINE_PRODUCING_BUT_LOYALTY_LOW:
        gubFact[usFact] =
          HasHisMineBeenProducingForPlayerForSomeTime(ubProfileID) &&
      ...
      case Enum170.FACT_DYNAMO_IN_J9:
        gubFact[usFact] =
          CheckNPCSector(Enum268.DYNAMO, 9, MAP_ROW_J, 0) &&
          Boolean(NumEnemiesInAnySector(9, 10, 0));
        break;
      case Enum170.FACT_DYNAMO_ALIVE:
        gubFact[usFact] =
          gMercProfiles[Enum268.DYNAMO].bMercStatus != MERC_IS_DEAD;
        break;
      case Enum170.FACT_DYNAMO_SPEAKING_OR_NEARBY:
        gubFact[usFact] =
          gpSrcSoldier != null &&
          (gpSrcSoldier.ubProfile == Enum268.DYNAMO ||
            (CheckNPCWithin(gpSrcSoldier.ubProfile, Enum268.DYNAMO, 10) &&
              CheckGuyVisible(gpSrcSoldier.ubProfile, Enum268.DYNAMO)));
        break;
      ...
      case Enum170.FACT_JOHN_AND_MARY_EPCS:
        gubFact[usFact] =
          CheckNPCIsEPC(Enum268.JOHN) && CheckNPCIsEPC(Enum268.MARY);
        break;
      ...
      case Enum170.FACT_RECEIVING_INCOME_FROM_DCAC:
        gubFact[usFact] =
          PredictDailyIncomeFromAMine(Enum179.MINE_DRASSEN) > 0 &&
          PredictDailyIncomeFromAMine(Enum179.MINE_ALMA) > 0 &&
          PredictDailyIncomeFromAMine(Enum179.MINE_CAMBRIA) > 0 &&
          PredictDailyIncomeFromAMine(Enum179.MINE_CHITZENA) > 0;
        break;
      ...
      case Enum170.FACT_PLAYER_BEEN_TO_K4:
        {
          ...
          pUnderGroundSector = <UNDERGROUND_SECTORINFO>(
            FindUnderGroundSector(4, MAP_ROW_K, 1)
          );
      ...
      case Enum170.FACT_WARDEN_DEAD:
        gubFact[usFact] =
          gMercProfiles[Enum268.WARDEN].bMercStatus == MERC_IS_DEAD;
        break;
      ...
      case Enum170.FACT_LOYALTY_OKAY:
        ...
        if (
          bTown != Enum135.BLANK_SECTOR &&
          gTownLoyalty[bTown].fStarted &&
          gfTownUsesLoyalty[bTown]
        ) {
          gubFact[usFact] =
            gTownLoyalty[bTown].ubRating >= LOYALTY_LOW_THRESHOLD &&
      ...
      case Enum170.FACT_LOYALTY_HIGH:
        ...
        if (
          ...
          gubFact[usFact] =
            gTownLoyalty[gMercProfiles[ubProfileID].bTown].ubRating >=
            LOYALTY_HIGH_THRESHOLD;
        } else {
          gubFact[usFact] = false;
        }
        break;
      ...
      case Enum170.FACT_ELGIN_ALIVE:
        gubFact[usFact] =
          gMercProfiles[Enum268.DRUGGIST].bMercStatus != MERC_IS_DEAD;
        break;

      case Enum170.FACT_SPEAKER_AIM_OR_AIM_NEARBY:
        gubFact[usFact] =
          gpDestSoldier !== null && AIMMercWithin(gpDestSoldier.sGridNo, 10);
        break;
      ...
      case Enum170.FACT_PLAYER_OWNS_2_TOWNS_INCLUDING_OMERTA:
        gubFact[usFact] =
          GetNumberOfWholeTownsUnderControl() == 3 &&
          IsTownUnderCompleteControlByPlayer(Enum135.OMERTA);
        break;

      case Enum170.FACT_PLAYER_OWNS_3_TOWNS_INCLUDING_OMERTA:
        gubFact[usFact] =
          GetNumberOfWholeTownsUnderControl() == 5 &&
          IsTownUnderCompleteControlByPlayer(Enum135.OMERTA);
        break;

      case Enum170.FACT_PLAYER_OWNS_4_TOWNS_INCLUDING_OMERTA:
        gubFact[usFact] =
          GetNumberOfWholeTownsUnderControl() >= 6 &&
          IsTownUnderCompleteControlByPlayer(Enum135.OMERTA);
        break;
      ...
      case Enum170.FACT_FATHER_DRUNK_AND_SCIFI_OPTION_ON:
        gubFact[usFact] =
          gMercProfiles[Enum268.FATHER].bNPCData >= 5 && gGameOptions.fSciFi;
        break;

      case Enum170.FACT_BLOODCAT_QUEST_STARTED_TWO_DAYS_AGO:
        gubFact[usFact] =
          gubQuest[Enum169.QUEST_BLOODCATS] != QUESTNOTSTARTED &&
          GetWorldTotalMin() - GetTimeQuestWasStarted(Enum169.QUEST_BLOODCATS) >
            Math.trunc((2 * NUM_SEC_IN_DAY) / NUM_SEC_IN_MIN);
        break;
      ...
      case Enum170.FACT_KINGPIN_NOT_IN_OFFICE:
        gubFact[usFact] = !(
          ...
          NPCInRoomRange(Enum268.KINGPIN, 30, 39)
        );
        // 30 to 39
        break;
      ...
      case Enum170.FACT_NPC_HOSTILE_OR_PISSED_OFF:
        gubFact[usFact] =
          CheckNPCIsEnemy(ubProfileID) ||
          Boolean(
            gMercProfiles[ubProfileID].ubMiscFlags3 &
      ...
      case Enum170.FACT_TONY_IN_BUILDING:
        gubFact[usFact] =
          CheckNPCSector(Enum268.TONY, 5, MAP_ROW_C, 0) &&
          NPCInRoom(Enum268.TONY, 50);
        break;
      ...
      case Enum170.FACT_ROCKET_RIFLE_EXISTS:
        gubFact[usFact] = ItemTypeExistsAtLocation(
          10472,
      ...
      case Enum170.FACT_DOREEN_ALIVE:
        gubFact[usFact] =
          gMercProfiles[Enum268.DOREEN].bMercStatus != MERC_IS_DEAD;
        break;

      case Enum170.FACT_WALDO_ALIVE:
        gubFact[usFact] =
          gMercProfiles[Enum268.WALDO].bMercStatus != MERC_IS_DEAD;
        break;

      case Enum170.FACT_PERKO_ALIVE:
        gubFact[usFact] =
          gMercProfiles[Enum268.PERKO].bMercStatus != MERC_IS_DEAD;
        break;

      case Enum170.FACT_TONY_ALIVE:
        gubFact[usFact] =
          gMercProfiles[Enum268.TONY].bMercStatus != MERC_IS_DEAD;
        break;

      case Enum170.FACT_VINCE_ALIVE:
        gubFact[usFact] =
          gMercProfiles[Enum268.VINCE].bMercStatus != MERC_IS_DEAD;
        break;

      case Enum170.FACT_JENNY_ALIVE:
        gubFact[usFact] =
          gMercProfiles[Enum268.JENNY].bMercStatus != MERC_IS_DEAD;
        break;

      case Enum170.FACT_ARNOLD_ALIVE:
        gubFact[usFact] =
          gMercProfiles[Enum268.ARNIE].bMercStatus != MERC_IS_DEAD;
        break;
      ...
      case Enum170.FACT_PLAYER_IN_CONTROLLED_DRASSEN_MINE:
        gubFact[usFact] =
          GetIdOfMineForSector(gWorldSectorX, gWorldSectorY, gbWorldSectorZ) ==
            Enum179.MINE_DRASSEN &&
          !StrategicMap[gWorldSectorX + MAP_WORLD_X * gWorldSectorY]
            .fEnemyControlled;
        break;
      ...
      case Enum170.FACT_ENOUGH_LOYALTY_TO_TRAIN_MILITIA:
        gubFact[usFact] = InTownSectorWithTrainingLoyalty(
          gWorldSectorX,
          gWorldSectorY,
        );
        break;
      ...
      case Enum170.FACT_84_AND_85_TRUE:
        gubFact[usFact] =
          CheckFact(84, ubProfileID) &&
          CheckFact(Enum170.FACT_HANS_AT_SPOT, ubProfileID);
        break;
      ...
      case Enum170.FACT_DYNAMO_NOT_SPEAKER:
        gubFact[usFact] = !(
          gpSrcSoldier != null && gpSrcSoldier.ubProfile == Enum268.DYNAMO
        );
        break;
      ...
      case Enum170.FACT_VEHICLE_PRESENT:
        gubFact[usFact] =
          CheckFact(Enum170.FACT_OK_USE_HUMMER, ubProfileID) &&
          (FindSoldierByProfileID(Enum268.PROF_HUMMER, true) != null ||
      ...
      case 245: // Can dimitri be recruited? should be true if already true, OR if Miguel has been recruited already
        gubFact[usFact] =
          gubFact[usFact] ||
          FindSoldierByProfileID(Enum268.MIGUEL, true) !== null;
      /*
                      case FACT_:
                              gubFact[usFact] = ;
                              break;
  ...
  export function InternalStartQuest(
    ubQuest: UINT8,
    sSectorX: INT16,
    sSectorY: INT16,
  ...
  export function InternalEndQuest(
    ubQuest: UINT8,
    sSectorX: INT16,
    sSectorY: INT16,
    fUpdateHistory: boolean,
