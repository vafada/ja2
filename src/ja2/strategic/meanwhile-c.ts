namespace ja2 {
  ...
  let gusMeanWhileGridNo: UINT16[] /* [] */ = [
    12248, 12248, 12248, 12248, 12248, 12248, 12248, 12248, 12248, 12248, 12248,
    8075, 12248, 12248, 12248, 12248, 12248,
  ];
  ...
  export let gCurrentMeanwhileDef: MEANWHILE_DEFINITION =
    createMeanwhileDefinition();
  export let gMeanwhileDef: MEANWHILE_DEFINITION[] /* [NUM_MEANWHILES] */ =
    createArrayFrom(Enum160.NUM_MEANWHILES, createMeanwhileDefinition);
  ...
  let gNPCSaveData: NPC_SAVE_INFO[] /* [MAX_MEANWHILE_PROFILES] */ =
    createArrayFrom(MAX_MEANWHILE_PROFILES, createNpcSaveInfo);
  ...
  function GetFreeNPCSave(): INT32 {
    ...
    for (uiCount = 0; uiCount < guiNumNPCSaves; uiCount++) {
      if (gNPCSaveData[uiCount].ubProfile == NO_PROFILE) return uiCount;
    }

    if (guiNumNPCSaves < MAX_MEANWHILE_PROFILES) return guiNumNPCSaves++;
  ...
  export function ScheduleMeanwhileEvent(
    pMeanwhileDef: MEANWHILE_DEFINITION,
    uiTime: UINT32,
  ): void {
    ...
    copyMeanwhileDefinition(
      gMeanwhileDef[pMeanwhileDef.ubMeanwhileID],
      pMeanwhileDef,
    );
    ...
    AddStrategicEvent(
      Enum132.EVENT_MEANWHILE,
      uiTime,
      pMeanwhileDef.ubMeanwhileID,
    );
  }
  ...
  function DoneFadeOutMeanwhile(): void {
    ...
    SetCurrentWorldSector(
      gCurrentMeanwhileDef.sSectorX,
      gCurrentMeanwhileDef.sSectorY,
      0,
    );
  ...
  function DoneFadeInMeanwhile(): void {
    ...
    if (gfInMeanwhile) {
      ...
      TriggerNPCRecordImmediately(
        gCurrentMeanwhileDef.ubNPCNumber,
  ...
  function ProcessImplicationsOfMeanwhile(): void {
    switch (gCurrentMeanwhileDef.ubMeanwhileID) {
      case Enum160.END_OF_PLAYERS_FIRST_BATTLE:
        if (gGameOptions.ubDifficultyLevel == Enum9.DIF_LEVEL_HARD) {
          ...
          ExecuteStrategicAIAction(
            Enum173.STRATEGIC_AI_ACTION_WAKE_QUEEN,
            0,
            0,
          );
        }
        HandleNPCDoAction(
          Enum268.QUEEN,
          Enum213.NPC_ACTION_SEND_SOLDIERS_TO_BATTLE_LOCATION,
          0,
        );
        break;
      case Enum160.CAMBRIA_LIBERATED:
      ...
      case Enum160.DRASSEN_LIBERATED:
        ...
        HandleNPCDoAction(
          Enum268.QUEEN,
          Enum213.NPC_ACTION_SEND_SOLDIERS_TO_DRASSEN,
          0,
        );
        break;
      case Enum160.CREATURES:
      ...
      case Enum160.NW_SAM:
        ExecuteStrategicAIAction(
          Enum213.NPC_ACTION_SEND_TROOPS_TO_SAM,
          SAM_1_X,
          SAM_1_Y,
        );
        break;
  ...
  function DoneFadeOutMeanwhileOnceDone(): void {
    ...
    ChangeSelectedMapSector(
      gsOldSelectedSectorX,
      gsOldSelectedSectorY,
      gsOldSelectedSectorZ,
    );
  ...
  export function HandleMeanWhileEventPostingForTownLiberation(
    bTownId: UINT8,
  ): void {
  ...
  export function HandleMeanWhileEventPostingForSAMLiberation(
    bSamId: INT8,
  ): void {
    let uiTime: UINT32 = 0;
    let MeanwhileDef: MEANWHILE_DEFINITION = createMeanwhileDefinition();
  ...
  */
  ...
  export function HandleFirstBattleEndingWhileInTown(
    sSectorX: INT16,
    sSectorY: INT16,
    bSectorZ: INT16,
    fFromAutoResolve: boolean,
  ): void {
    let bTownId: INT8 = 0;
    let sSector: INT16 = 0;
