namespace ja2 {
  ...
  let iUpdateBoxWaitingList: INT32[] /* [MAX_CHARACTER_COUNT] */ = createArray(
    MAX_CHARACTER_COUNT,
    0,
  );

  let pFastHelpMapScreenList: FASTHELPREGION[] /* [MAX_MAPSCREEN_FAST_HELP] */ =
    createArrayFrom(MAX_MAPSCREEN_FAST_HELP, createFastHelpRegion);

  // the move menu region
  let gMoveMenuRegion: MOUSE_REGION[] /* [MAX_POPUP_BOX_STRING_COUNT] */ =
    createArrayFrom(MAX_POPUP_BOX_STRING_COUNT, createMouseRegion);
  ...
  let pSoldierMovingList: SOLDIERTYPE[] /* Pointer<SOLDIERTYPE>[MAX_CHARACTER_COUNT] */ =
    createArray(MAX_CHARACTER_COUNT, <SOLDIERTYPE>(<unknown>null));
  let fSoldierIsMoving: boolean[] /* [MAX_CHARACTER_COUNT] */ = createArray(
    MAX_CHARACTER_COUNT,
    false,
  );

  let pUpdateSoldierBox: SOLDIERTYPE[] /* Pointer<SOLDIERTYPE>[SIZE_OF_UPDATE_BOX] */ =
    createArray(SIZE_OF_UPDATE_BOX, <SOLDIERTYPE>(<unknown>null));

  let giUpdateSoldierFaces: INT32[] /* [SIZE_OF_UPDATE_BOX] */ = createArray(
    SIZE_OF_UPDATE_BOX,
    0,
  );

  // the squads thata re moving
  let iSquadMovingList: INT32[] /* [NUMBER_OF_SQUADS] */ = createArray(
    Enum275.NUMBER_OF_SQUADS,
    0,
  );
  let fSquadIsMoving: boolean[] /* INT32[NUMBER_OF_SQUADS] */ = createArray(
    Enum275.NUMBER_OF_SQUADS,
    false,
  );

  // the vehicles thata re moving
  let iVehicleMovingList: INT32[] /* [NUMBER_OF_SQUADS] */ = createArray(
    Enum275.NUMBER_OF_SQUADS,
    0,
  );
  let fVehicleIsMoving: boolean[] /* INT32[NUMBER_OF_SQUADS] */ = createArray(
    Enum275.NUMBER_OF_SQUADS,
    false,
  );
  ...
  export let gCharactersList: MapScreenCharacterSt[] /* [MAX_CHARACTER_COUNT + 1] */ =
    createArrayFrom(MAX_CHARACTER_COUNT + 1, createMapScreenCharacterSt);
  ...
  export let fSelectedListOfMercsForMapScreen: boolean[] /* [MAX_CHARACTER_COUNT] */ =
    createArray(MAX_CHARACTER_COUNT, false);
  ...
  let iOldContractTimes: INT32[] /* [MAX_CHARACTER_COUNT] */ = createArray(
    MAX_CHARACTER_COUNT,
    0,
  );
  ...
  // the leave item list
  let gpLeaveListHead: MERC_LEAVE_ITEM[] /* Pointer<MERC_LEAVE_ITEM>[NUM_LEAVE_LIST_SLOTS] */ =
    createArray(NUM_LEAVE_LIST_SLOTS, <MERC_LEAVE_ITEM>(<unknown>null));

  // holds ids of mercs who left stuff behind
  let guiLeaveListOwnerProfileId: UINT32[] /* [NUM_LEAVE_LIST_SLOTS] */ =
    createArray(NUM_LEAVE_LIST_SLOTS, 0);
  ...
  let giDblClickTimersForMoveBoxMouseRegions: INT32[] /* [MAX_POPUP_BOX_STRING_COUNT] */ =
    createArray(MAX_POPUP_BOX_STRING_COUNT, 0);
  ...
  export function ResetSelectedListForMapScreen(): void {
    ...
    if (
      bSelectedInfoChar != -1 &&
      gCharactersList[bSelectedInfoChar].fValid == true
    ) {
  ...
  export function ToggleEntryInSelectedList(bEntry: INT8): void {
    ...
    fSelectedListOfMercsForMapScreen[bEntry] =
      !fSelectedListOfMercsForMapScreen[bEntry];
  ...
  export function ResetAssignmentsForMercsTrainingUnpaidSectorsInSelectedList(
    bAssignment: INT8,
  ): void {
    let iCounter: INT32 = 0;
  ...
  export function ResetAssignmentOfMercsThatWereTrainingMilitiaInThisSector(
    sSectorX: INT16,
    sSectorY: INT16,
  ): void {
    let iCounter: INT32 = 0;
...
*/
  ...
  export function DeselectSelectedListMercsWhoCantMoveWithThisGuy(
      ...
      if (gCharactersList[iCounter].fValid == true) {
        if (fSelectedListOfMercsForMapScreen[iCounter] == true) {
          ...
          // reject those not a squad (vehicle handled above)
            ...
            if (
              pSoldier.sSectorX != pSoldier2.sSectorX ||
            ...
            if (
              (pSoldier.fBetweenSectors || pSoldier2.fBetweenSectors) &&
  ...
  function AnyMercInSameSquadOrVehicleIsSelected(
    pSoldier: SOLDIERTYPE,
  ): boolean {
    let iCounter: INT32 = 0;
    let pSoldier2: SOLDIERTYPE;
    ...
    for (iCounter = 0; iCounter < MAX_CHARACTER_COUNT; iCounter++) {
      if (gCharactersList[iCounter].fValid == true) {
        ...
        if (fSelectedListOfMercsForMapScreen[iCounter] == true) {
          ...
          if (
            pSoldier2.bAssignment == Enum117.VEHICLE &&
  ...
  /* static */ let RestoreBackgroundForAssignmentGlowRegionList__iOldAssignmentLine: INT32 =
    -1;
  export function RestoreBackgroundForAssignmentGlowRegionList(): void {
    ...
    if (
      RestoreBackgroundForAssignmentGlowRegionList__iOldAssignmentLine !=
      giAssignHighLine
    ) {
      // restore background
  ...
  /* static */ let RestoreBackgroundForDestinationGlowRegionList__iOldDestinationLine: INT32 =
    -1;
  export function RestoreBackgroundForDestinationGlowRegionList(): void {
    ...
    if (
      RestoreBackgroundForDestinationGlowRegionList__iOldDestinationLine !=
      giDestHighLine
    ) {
      // restore background
      RestoreExternBackgroundRect(
  ...
  /* static */ let RestoreBackgroundForContractGlowRegionList__iOldContractLine: INT32 =
    -1;
  export function RestoreBackgroundForContractGlowRegionList(): void {
    ...
    if (
      RestoreBackgroundForContractGlowRegionList__iOldContractLine !=
      giContractHighLine
    ) {
      // restore background
      RestoreExternBackgroundRect(
  ...
  /* static */ let RestoreBackgroundForSleepGlowRegionList__iOldSleepHighLine: INT32 =
    -1;
  export function RestoreBackgroundForSleepGlowRegionList(): void {
    ...
    if (
      RestoreBackgroundForSleepGlowRegionList__iOldSleepHighLine !=
      giSleepHighLine
    ) {
      // restore background
      RestoreExternBackgroundRect(
  ...
  export function PlayGlowRegionSound(): void {
    ...
    PlayGlowRegionSound__uiSoundId = PlayJA2SampleFromFile(
      "Sounds\\glowclick.wav",
      RATE_11025,
      MIDVOLUME,
      1,
      MIDDLEPAN,
  ...
  export function CharacterIsGettingPathPlotted(sCharNumber: INT16): boolean {
    ...
    if (
      (giDestHighLine != -1 && IsEntryInSelectedListSet(giDestHighLine)) ||
  ...
  export function IsCharacterSelectedForAssignment(
    sCharNumber: INT16,
  ): boolean {
    // valid character number?
    if (sCharNumber < 0 || sCharNumber >= MAX_CHARACTER_COUNT) {
...
*/
  ...
  export function DoMapMessageBoxWithRect(
    ubStyle: UINT8,
  ...
  export function DoMapMessageBox(
    ubStyle: UINT8,
    zString: string /* Pointer<INT16> */,
    uiExitScreen: UINT32,
    usFlags: UINT16,
  ...
  export function CheckAndUpdateBasedOnContractTimes(): void {
    ...
    for (iCounter = 0; iCounter < MAX_CHARACTER_COUNT; iCounter++) {
      if (gCharactersList[iCounter].fValid == true) {
        // what kind of merc
        if (
          Menptr[gCharactersList[iCounter].usSolID].ubWhatKindOfMercAmI ==
          Enum260.MERC_TYPE__AIM_MERC
        ) {
          // amount of time left on contract
          iTimeRemaining =
            Menptr[gCharactersList[iCounter].usSolID].iEndofContractTime -
            GetWorldTotalMin();
          ...
          iTimeRemaining =
            Menptr[gCharactersList[iCounter].usSolID].iTotalContractLength;
  ...
  export function HandleDisplayOfSelectedMercArrows(): void {
    ...
    for (ubCount = 0; ubCount < MAX_CHARACTER_COUNT; ubCount++) {
      if (gCharactersList[ubCount].fValid == true) {
        ...
        if (
          IsEntryInSelectedListSet(ubCount) == true ||
          (bSelectedDestChar != -1
  ...
  /* static */ let HandleDisplayOfItemPopUpForSector__fWasInited: boolean =
    false;
  export function HandleDisplayOfItemPopUpForSector(
    sMapX: INT16,
    sMapY: INT16,
    sMapZ: INT16,
  ): void {
    ...
    if (
      fMapInventoryPoolInited &&
      HandleDisplayOfItemPopUpForSector__fWasInited
    ) {
  ...
  function CreateScreenMaskForInventoryPoolPopUp(): void {
    ...
    MSYS_DefineRegion(
      ...
      MSYS_NO_CURSOR,
      MSYS_NO_CALLBACK,
      InventoryScreenMaskBtnCallback,
    );
  }
  ...
  function InventoryScreenMaskBtnCallback(
  ...
  export function HandleLeavingOfEquipmentInCurrentSector(
    uiMercId: UINT32,
  ): void {
    ...
    if (
      Menptr[uiMercId].sSectorX != gWorldSectorX ||
      ...
      if (
        Menptr[uiMercId].ubStrategicInsertionCode ==
      ...
      if (Menptr[uiMercId].inv[iCounter].ubNumberOfObjects > 0) {
        if (
          ...
          AddItemsToUnLoadedSector(
            Menptr[uiMercId].sSectorX,
            Menptr[uiMercId].sSectorY,
            Menptr[uiMercId].bSectorZ,
            sGridNo,
            1,
            [Menptr[uiMercId].inv[iCounter]],
            Menptr[uiMercId].bLevel,
            WOLRD_ITEM_FIND_SWEETSPOT_FROM_GRIDNO | WORLD_ITEM_REACHABLE,
            0,
            1,
            false,
          );
  ...
  export function HandleMercLeavingEquipmentInOmerta(uiMercId: UINT32): void {
    ...
    if ((iSlotIndex = SetUpDropItemListForMerc(uiMercId)) != -1) {
      ...
      AddStrategicEvent(
        Enum132.EVENT_MERC_LEAVE_EQUIP_IN_OMERTA,
        GetWorldTotalMin() + 6 * 60,
        iSlotIndex,
      );
    } else {
      ...
      AssertMsg(
        false,
        "HandleMercLeavingEquipmentInOmerta: No more free slots, equipment lost",
      );
    }
  }
  ...
  export function HandleMercLeavingEquipmentInDrassen(uiMercId: UINT32): void {
    ...
    if ((iSlotIndex = SetUpDropItemListForMerc(uiMercId)) != -1) {
      // post event to drop it there 6 hours later
      AddStrategicEvent(
        Enum132.EVENT_MERC_LEAVE_EQUIP_IN_DRASSEN,
        GetWorldTotalMin() + 6 * 60,
        iSlotIndex,
      );
    } else {
      ...
      AssertMsg(
        false,
        "HandleMercLeavingEquipmentInDrassen: No more free slots, equipment lost",
      );
    }
  }
  ...
  export function HandleEquipmentLeftInOmerta(uiSlotIndex: UINT32): void {
    ...
    while (pItem) {
      if (
        gWorldSectorX != OMERTA_LEAVE_EQUIP_SECTOR_X ||
        ...
        AddItemsToUnLoadedSector(
          OMERTA_LEAVE_EQUIP_SECTOR_X,
          OMERTA_LEAVE_EQUIP_SECTOR_Y,
          OMERTA_LEAVE_EQUIP_SECTOR_Z,
          OMERTA_LEAVE_EQUIP_GRIDNO,
          1,
          [pItem.o],
          0,
          WORLD_ITEM_REACHABLE,
  ...
  export function AddItemToLeaveIndex(
    o: OBJECTTYPE,
    uiSlotIndex: UINT32,
  ): boolean {
  ...
  // store owner's profile id for the items added to this leave slot index
  function SetUpMercAboutToLeaveEquipment(
    ubProfileId: UINT32,
    uiSlotIndex: UINT32,
  ): void {
    Assert(uiSlotIndex < NUM_LEAVE_LIST_SLOTS);
...
*/
  ...
  export function UpdateCharRegionHelpText(): void {
    ...
    if (
      ...
      if (pSoldier.bAssignment != Enum117.ASSIGNMENT_POW) {
        if (pSoldier.bLife != 0) {
          if (
            AM_A_ROBOT(MercPtrs[gCharactersList[bSelectedInfoChar].usSolID])
          ) {
            // robot (condition only)
            sString = swprintf(
              "%s: %d/%d",
              pMapScreenStatusStrings[3],
              pSoldier.bLife,
              pSoldier.bLifeMax,
            );
          } else if (
            Menptr[gCharactersList[bSelectedInfoChar].usSolID].uiStatusFlags &
            SOLDIER_VEHICLE
          ) {
      ...
      } // can't toggle it, don't show any inventory help text
      else {
  ...
  export function UpdateMapScreenAssignmentPositions(): void {
    ...
    */
    ...
    AttributePosition.iY = TrainPosition.iY =
      AssignmentPosition.iY +
      (GetFontHeight(MAP_SCREEN_FONT()) + 2) * Enum148.ASSIGN_MENU_TRAIN;

    VehiclePosition.iY =
      AssignmentPosition.iY +
      (GetFontHeight(MAP_SCREEN_FONT()) + 2) * Enum148.ASSIGN_MENU_VEHICLE;
    SquadPosition.iY = AssignmentPosition.iY;
  ...
  export function RandomMercInGroupSaysQuote(
    pGroup: GROUP,
    usQuoteNum: UINT16,
  ): void {
    ...
    if (
      (gfTacticalTraversal || pGroup.ubSectorZ > 0) &&
  ...
  export function MapscreenCanPassItemToCharNum(iNewCharSlot: INT32): boolean {
    ...
    if (fShowMapInventoryPool && !gpItemPointerSoldier && fMapInventoryItem) {
      ...
      if (
        pNewSoldier.sSectorX != sSelMapX ||
        pNewSoldier.sSectorY != sSelMapY ||
        pNewSoldier.bSectorZ != iCurrentMapSectorZ
      ) {
        return false;
      }
    ...
    if (pOldSoldier != null) {
      ...
      if (
        pNewSoldier.sSectorX != pOldSoldier.sSectorX ||
      ...
      if (pNewSoldier.fBetweenSectors) {
        ...
        if (
          pNewSoldier.bAssignment == Enum117.VEHICLE &&
  ...
  export function HandleMinerEvent(
    bMinerNumber: UINT8,
    sSectorX: INT16,
    sSectorY: INT16,
    sQuoteNumber: INT16,
    fForceMapscreen: boolean,
  ): void {
    ...
    if (fFromMapscreen) {
      ...
      CharacterDialogueWithSpecialEvent(
        uiExternalFaceProfileIds[bMinerNumber],
        sQuoteNumber,
        bMinerNumber,
        DIALOGUE_EXTERNAL_NPC_UI,
        false,
        false,
        DIALOGUE_SPECIAL_EVENT_MINESECTOREVENT,
        Enum155.START_RED_SECTOR_LOCATOR,
        1,
      );
      CharacterDialogue(
        uiExternalFaceProfileIds[bMinerNumber],
        sQuoteNumber,
        uiExternalStaticNPCFaces[bMinerNumber],
        DIALOGUE_EXTERNAL_NPC_UI,
        false,
        false,
      );
      CharacterDialogueWithSpecialEvent(
        ...
        false,
        DIALOGUE_SPECIAL_EVENT_MINESECTOREVENT,
        Enum155.STOP_RED_SECTOR_LOCATOR,
        1,
      );
    } // stay in tactical
    else {
  ...
  export function SetUpFastHelpListRegions(
    iXPosition: INT32[] /* [] */,
    iYPosition: INT32[] /* [] */,
    iWidth: INT32[] /* [] */,
  ...
  /* static */ let HandleShowingOfTacticalInterfaceFastHelpText__fTextActive: boolean =
    false;
  export function HandleShowingOfTacticalInterfaceFastHelpText(): void {
    if (fInterfaceFastHelpTextActive) {
      DisplayFastHelpRegions(
        pFastHelpMapScreenList,
        giSizeOfInterfaceFastHelpTextList,
      );
      ...
      HandleShowingOfTacticalInterfaceFastHelpText__fTextActive
    ) {
  ...
  function DisplayFastHelpRegions(
    pRegion: FASTHELPREGION[],
    iSize: INT32,
  ): void {
    let iCounter: INT32 = 0;
  ...
  function DisplayFastHelpForInitialTripInToMapScreen(
    pRegion: FASTHELPREGION,
  ): void {
  ...
  export function DisplayMapScreenFastHelpList(): void {
    ...
    DisplayFastHelpForInitialTripInToMapScreen(
      pFastHelpMapScreenList[iCounter],
    );
  ...
  export function SetUpMapScreenFastHelpText(): void {
    ...
    for (
      iCounter = 0;
      iCounter < NUMBER_OF_MAPSCREEN_HELP_MESSAGES;
      iCounter++
    ) {
      pFastHelpMapScreenList[iCounter].iX =
        pMapScreenFastHelpLocationList[iCounter].iX;
      pFastHelpMapScreenList[iCounter].iY =
        pMapScreenFastHelpLocationList[iCounter].iY;
      pFastHelpMapScreenList[iCounter].iW =
        pMapScreenFastHelpWidthList[iCounter];
      pFastHelpMapScreenList[iCounter].FastHelpText =
        pMapScreenFastHelpTextList[iCounter];
    }
  ...
  */
  ...
  /* static */ let SetUpShutDownMapScreenHelpTextScreenMask__fCreated: boolean =
    false;
    ...
    if (
      ...
      if (gTacticalStatus.fDidGameJustStart) {
        MSYS_DefineRegion(
          ...
          pMapScreenFastHelpLocationList[9].iX,
  ...
  function MapScreenHelpTextScreenMaskBtnCallback(
    pRegion: MOUSE_REGION,
    iReason: INT32,
  ): void {
  ...
  function IsSoldierSelectedForMovement(pSoldier: SOLDIERTYPE | null): boolean {
    ...
    for (
      iCounter = 0;
      iCounter < giNumberOfSoldiersInSectorMoving;
      iCounter++
    ) {
      if (
        pSoldierMovingList[iCounter] == pSoldier &&
        fSoldierIsMoving[iCounter]
      ) {
        return true;
      }
    }
  ...
  function IsVehicleSelectedForMovement(iVehicleId: INT32): boolean {
    ...
    for (
      iCounter = 0;
      iCounter < giNumberOfVehiclesInSectorMoving;
      iCounter++
    ) {
      if (
        iVehicleMovingList[iCounter] == iVehicleId &&
        fVehicleIsMoving[iCounter]
      ) {
        return true;
      }
    }
  ...
  function SelectVehicleForMovement(
    iVehicleId: INT32,
    fAndAllOnBoard: boolean,
  ): void {
    let iCounter: INT32 = 0;
    let iCount: INT32 = 0;
        ...
        for (iCount = 0; iCount < 10; iCount++) {
          ...
          if (fAndAllOnBoard) {
            ...
            if (pPassenger && pPassenger.bActive) {
              ...
              if (
                CanMoveBoxSoldierMoveStrategically(pPassenger, fFirstFailure)
              ) {
  ...
  function HowManyMovingSoldiersInVehicle(iVehicleId: INT32): INT32 {
    ...
    for (
      ...
      if (
        pSoldierMovingList[iCounter].bAssignment == Enum117.VEHICLE &&
  ...
  export function CreateDestroyMovementBox(
    sSectorX: INT16,
    sSectorY: INT16,
    sSectorZ: INT16,
  ): void {
  ...
  export function SetUpMovingListsForSector(
    sSectorX: INT16,
    sSectorY: INT16,
    sSectorZ: INT16,
  ): void {
  ...
  function CreatePopUpBoxForMovementBox(): void {
    ...
    SetBoxLineForeground(
      ghMoveBox,
      GetNumberOfLinesOfTextInBox(ghMoveBox) - 1,
      FONT_YELLOW,
    );
    ...
    if (IsAnythingSelectedForMoving()) {
      SetBoxLineForeground(
        ghMoveBox,
        GetNumberOfLinesOfTextInBox(ghMoveBox) - 2,
        FONT_YELLOW,
      );
    }
    ...
    if (Position.iX + Dimensions.iRight >= MAP_VIEW_START_X + MAP_VIEW_WIDTH) {
      Position.iX = Math.max(
        MAP_VIEW_START_X,
        MAP_VIEW_START_X + MAP_VIEW_WIDTH - Dimensions.iRight,
      );
      SetBoxPosition(ghMoveBox, Position);
    }

    if (
      Position.iY + Dimensions.iBottom >=
      MAP_VIEW_START_Y + MAP_VIEW_HEIGHT
    ) {
      Position.iY = Math.max(
        MAP_VIEW_START_Y,
        MAP_VIEW_START_Y + MAP_VIEW_HEIGHT - Dimensions.iBottom,
      );
      SetBoxPosition(ghMoveBox, Position);
    }
  }
  ...
  function AddStringsToMoveBox(): void {
    ...
    for (iCount = 0; iCount < giNumberOfSquadsInSectorMoving; iCount++) {
      ...
      for (iCountB = 0; iCountB < giNumberOfSoldiersInSectorMoving; iCountB++) {
        if (
          pSoldierMovingList[iCountB].bAssignment == iSquadMovingList[iCount]
        ) {
          // add mercs in squads
          if (
            IsSoldierSelectedForMovement(pSoldierMovingList[iCountB]) == true
          ) {
    ...
    for (iCount = 0; iCount < giNumberOfVehiclesInSectorMoving; iCount++) {
      ...
      if (fVehicleIsMoving[iCount]) {
        sString = swprintf(
          "*%s*",
          pVehicleStrings[
            pVehicleList[iVehicleMovingList[iCount]].ubVehicleType
          ],
        );
      } else {
        sString = swprintf(
      ...
      for (iCountB = 0; iCountB < giNumberOfSoldiersInSectorMoving; iCountB++) {
        if (
          pSoldierMovingList[iCountB].bAssignment == Enum117.VEHICLE &&
    ...
    for (iCount = 0; iCount < giNumberOfSoldiersInSectorMoving; iCount++) {
      ...
      if (
        pSoldierMovingList[iCount].bAssignment >= Enum117.ON_DUTY &&
        pSoldierMovingList[iCount].bAssignment != Enum117.VEHICLE
      ) {
        if (fFirstOne) {
  ...
  function BuildMouseRegionsForMoveBox(): void {
    ...
    // grab height of font
    iFontHeight =
      GetLineSpace(ghMoveBox) + GetFontHeight(GetBoxFont(ghMoveBox));
    ...
    // box heading
    MSYS_DefineRegion(
      gMoveMenuRegion[iCounter],
      iBoxXPosition,
      iBoxYPosition + iFontHeight * iCounter,
      iBoxXPosition + iBoxWidth,
      iBoxYPosition + iFontHeight * (iCounter + 1),
      MSYS_PRIORITY_HIGHEST,
      MSYS_NO_CURSOR,
      MSYS_NO_CALLBACK,
      MSYS_NO_CALLBACK,
    ...
    while (iCounter < iTotalNumberOfLines) {
      ...
      for (iCount = 0; iCount < giNumberOfSquadsInSectorMoving; iCount++) {
        ...
        for (
          iCountB = 0;
          iCountB < giNumberOfSoldiersInSectorMoving;
          iCountB++
        ) {
          if (
            pSoldierMovingList[iCountB].bAssignment == iSquadMovingList[iCount]
  ...
  function ClearMouseRegionsForMoveBox(): void {
    ...
    for (
      iCounter = 0;
      iCounter < GetNumberOfLinesOfTextInBox(ghMoveBox);
      iCounter++
    ) {
      // remove this region
  ...
  function MoveMenuBtnCallback(pRegion: MOUSE_REGION, iReason: INT32): void {
    ...
    if (iReason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
      if (
        iClickTime - giDblClickTimersForMoveBoxMouseRegions[iMoveBoxLine] <
        ...
        if (iRegionType == Enum145.SQUAD_REGION) {
          ...
          if (fVehicleIsMoving[iListIndex] == true) {
            ...
            // vehicle goes
            SelectVehicleForMovement(
              iVehicleMovingList[iListIndex],
              AND_ALL_ON_BOARD,
            );
          }
          ...
          AssertMsg(
            0,
            FormatString(
              "MoveMenuBtnCallback: Invalid regionType %d, moveBoxLine %d",
              iRegionType,
              iMoveBoxLine,
            ),
          );
  ...
  function CanMoveBoxSoldierMoveStrategically(
    pSoldier: SOLDIERTYPE,
  ...
  function SelectAllOtherSoldiersInList(): void {
    ...
    for (
      ...
      if (
        pSoldierMovingList[iCounter].bAssignment >= Enum117.ON_DUTY &&
        pSoldierMovingList[iCounter].bAssignment != Enum117.VEHICLE
      ) {
        if (
          CanMoveBoxSoldierMoveStrategically(
            pSoldierMovingList[iCounter],
            false,
          )
        ) {
          fSoldierIsMoving[iCounter] = true;
  ...
  function HandleMoveoutOfSectorMovementTroops(): void {
    ...
    for (
      ...
      if (pSoldier.bAssignment < Enum117.ON_DUTY) {
        ...
        if (
          fSoldierIsMoving[iCounter] !=
          IsSquadSelectedForMovement(pSoldier.bAssignment)
        ) {
      ...
      // if in a vehicle
        ...
        if (
          fSoldierIsMoving[iCounter] !=
          IsVehicleSelectedForMovement(pSoldier.iVehicleId)
        ) {
          // split the guy from his vehicle to any other compatible squad
          fCheckForCompatibleSquad = true;
        }
      } // on his own - not on a squad or in a vehicle
      else {
        ...
        if (iSquadNumber != -1) {
          ...
          if (iSquadNumber != -1) {
            ...
            AssertMsg(
              0,
              FormatString(
                "HandleMoveoutOfSectorMovementTroops: AddCharacterToUniqueSquad failed, iCounter %d",
                iCounter,
              ),
            );
            // toggle whether he's going or not to try and recover somewhat gracefully
...
*/
  ...
  function IsThisSquadInThisSector(
    sSectorX: INT16,
    sSectorY: INT16,
    bSectorZ: INT8,
    bSquadValue: INT8,
    ...
    for (bCounter = 0; bCounter < Enum275.NUMBER_OF_SQUADS; bCounter++) {
      ...
      if (
        ...
        if (IsThisSquadFull(bCounter) == false) {
          ...
          if (
            IsSquadSelectedForMovement(bCounter) ==
            IsSoldierSelectedForMovement(pSoldier)
          ) {
  ...
  function MoveScreenMaskBtnCallback(
    pRegion: MOUSE_REGION,
    iReason: INT32,
  ): void {
  ...
  export function AddSoldierToWaitingListQueue(pSoldier: SOLDIERTYPE): void {
    ...
    SpecialCharacterDialogueEvent(
      ...
      Enum204.UPDATE_BOX_REASON_ADDSOLDIER,
      iSoldierId,
      0,
      0,
      0,
  ...
  export function AddReasonToWaitingListQueue(iReason: INT32): void {
    SpecialCharacterDialogueEvent(
      DIALOGUE_ADD_EVENT_FOR_SOLDIER_UPDATE_BOX,
      Enum204.UPDATE_BOX_REASON_SET_REASON,
      iReason,
      0,
      0,
      0,
  ...
  export function AddDisplayBoxToWaitingQueue(): void {
    SpecialCharacterDialogueEvent(
      DIALOGUE_ADD_EVENT_FOR_SOLDIER_UPDATE_BOX,
      Enum204.UPDATE_BOX_REASON_SHOW_BOX,
      0,
      0,
      0,
      0,
  ...
  export function DisplaySoldierUpdateBox(): void {
    ...
    if (
      iNumberOfMercsOnUpdatePanel > NUMBER_OF_MERCS_FOR_FOUR_WIDTH_UPDATE_PANEL
    ) {
      fFourWideMode = true;
    }

    // get number of rows
    iNumberHigh = fFourWideMode
      ? Math.trunc(
    ...
    for (iCounter = 0; iCounter < iUpperLimit; iCounter++) {
      ...
      iFaceY =
        iY +
        Math.trunc(iCounter / iNumberWide) * TACT_UPDATE_MERC_FACE_X_HEIGHT;
    ...
    if (fFourWideMode) {
      ...
      BltVideoObject(
        ...
        hBackGroundHandle,
        19,
        iX - 4 + TACT_UPDATE_MERC_FACE_X_WIDTH,
        iY +
          iNumberHigh * TACT_UPDATE_MERC_FACE_X_HEIGHT +
          REASON_FOR_SOLDIER_UPDATE_OFFSET_Y +
          3,
        VO_BLT_SRCTRANSPARENCY,
        null,
    ...
    for (iCounter = 0; iCounter < iNumberWide; iCounter++) {
      ...
      BltVideoObject(
        ...
        hBackGroundHandle,
        1,
        iX + TACT_UPDATE_MERC_FACE_X_WIDTH * iCounter,
        iY - 4,
        VO_BLT_SRCTRANSPARENCY,
        null,
      );
    ...
    if (fFourWideMode) {
      DisplayWrappedString(
        ...
        MAP_SCREEN_FONT(),
        FONT_WHITE,
        pUpdateMercStrings[iReasonForSoldierUpDate],
        FONT_BLACK,
        false,
        CENTER_JUSTIFIED,
      );
    ...
    RestoreExternBackgroundRect(
      iX - 5,
      iY - 5,
      iUpdatePanelWidth + 10,
      iUpdatePanelHeight + 6,
    );

    CreateDestroyUpdatePanelButtons(
      iX,
      iY + iUpdatePanelHeight - 18,
      fFourWideMode,
    );
  ...
  function CreateDestroyUpdatePanelButtons(
    iX: INT32,
    iY: INT32,
    fFourWideMode: boolean,
  ): void {
    if (
      fShowUpdateBox == true &&
      ...
      guiUpdatePanelButtonsImage[0] = LoadButtonImage(
      ...
      guiUpdatePanelButtonsImage[1] = LoadButtonImage(
        "INTERFACE\\group_confirm_tactical.sti",
        -1,
        7,
        -1,
      ...
      if (fFourWideMode) {
        guiUpdatePanelButtons[0] = QuickCreateButton(
          ...
          iX - 4 + TACT_UPDATE_MERC_FACE_X_WIDTH + 4,
          iY,
          BUTTON_TOGGLE,
          MSYS_PRIORITY_HIGHEST - 1,
          BtnGenericMouseMoveButtonCallback,
          ContinueUpdateButtonCallback,
        );

        guiUpdatePanelButtons[1] = QuickCreateButton(
          guiUpdatePanelButtonsImage[1],
          iX - 4 + 2 * TACT_UPDATE_MERC_FACE_X_WIDTH + 4,
          iY,
          BUTTON_TOGGLE,
          MSYS_PRIORITY_HIGHEST - 1,
          BtnGenericMouseMoveButtonCallback,
          StopUpdateButtonCallback,
        );
      } else {
        guiUpdatePanelButtons[0] = QuickCreateButton(
          guiUpdatePanelButtonsImage[0],
        ...
        guiUpdatePanelButtons[1] = QuickCreateButton(
          guiUpdatePanelButtonsImage[1],
          iX + TACT_UPDATE_MERC_FACE_X_WIDTH,
          iY,
          BUTTON_TOGGLE,
          MSYS_PRIORITY_HIGHEST - 1,
          BtnGenericMouseMoveButtonCallback,
...
*/
  ...
  function RenderSoldierSmallFaceForUpdatePanel(
    iIndex: INT32,
    iX: INT32,
    iY: INT32,
  ): void {
  ...
  export function UpdateHelpTextForMapScreenMercIcons(): void {
    if (
      bSelectedInfoChar == -1 ||
      gCharactersList[bSelectedInfoChar].fValid == false
    ) {
      ...
      if (
        Menptr[gCharactersList[bSelectedInfoChar].usSolID]
          .ubWhatKindOfMercAmI == Enum260.MERC_TYPE__AIM_MERC
      ) {
      ...
      if (
        Menptr[gCharactersList[bSelectedInfoChar].usSolID].usLifeInsurance > 0
      ) {
      ...
      if (
        Menptr[gCharactersList[bSelectedInfoChar].usSolID].usMedicalDeposit > 0
      ) {
  ...
  /* static */ let CreateDestroyInsuranceMouseRegionForMercs__fCreated: boolean =
    false;
  export function CreateDestroyInsuranceMouseRegionForMercs(
    fCreate: boolean,
  ): void {
    if (
      ...
      MSYS_DefineRegion(
        ...
        CHAR_ICON_CONTRACT_Y + CHAR_ICON_HEIGHT,
        MSYS_PRIORITY_HIGH - 1,
        MSYS_NO_CURSOR,
        MSYS_NO_CALLBACK,
        MSYS_NO_CALLBACK,
...
*/
  ...
  export function NotifyPlayerWhenEnemyTakesControlOfImportantSector(
    sSectorX: INT16,
    sSectorY: INT16,
    bSectorZ: INT8,
    fContested: boolean,
    ...
    if (IsThereAMineInThisSector(sSectorX, sSectorY)) {
      ...
      if (
        GetMaxDailyRemovalFromMine(bMineIndex) > 0 &&
        SpokenToHeadMiner(bMineIndex)
      ) {
  ...
  export function NotifyPlayerOfInvasionByEnemyForces(
    sSectorX: INT16,
    sSectorY: INT16,
    bSectorZ: INT8,
  ...
  function CanCharacterMoveInStrategic(
    pSoldier: SOLDIERTYPE,
    pbErrorNumber: Pointer<INT8>,
  ): boolean {
    ...
    if (
      pSoldier.sSectorX == 12 &&
      pSoldier.sSectorY == MAP_ROW_L &&
      pSoldier.bSectorZ == 0 &&
      !pSoldier.fBetweenSectors &&
      gMercProfiles[Enum268.ELDIN].bMercStatus != MERC_IS_DEAD
    ) {
      let ubRoom: UINT8;
      let cnt: UINT8;
      let pSoldier2: SOLDIERTYPE;
      ...
      if (
        ...
        for (
          pSoldier2 = MercPtrs[cnt];
          cnt <= gTacticalStatus.Team[gbPlayerNum].bLastID;
          cnt++, pSoldier2 = MercPtrs[cnt]
        ) {
    ...
    if (AM_A_ROBOT(pSoldier)) {
      ...
      if (
        ...
        (pSoldier.bAssignment < Enum117.ON_DUTY &&
          !IsRobotControllerInSquad(pSoldier.bAssignment))
      ) {
  ...
  export function CanEntireMovementGroupMercIsInMove(
    pSoldier: SOLDIERTYPE,
    pbErrorNumber: Pointer<INT8>,
  ): boolean {
    ...
    for (iCounter = 0; iCounter < MAX_CHARACTER_COUNT; iCounter++) {
      if (gCharactersList[iCounter].fValid == true) {
        ...
        if (pCurrentSoldier.uiStatusFlags & SOLDIER_VEHICLE) {
          // IS a vehicle
          ubCurrentGroup =
            pVehicleList[pCurrentSoldier.bVehicleID].ubMovementGroup;
        } else if (pCurrentSoldier.bAssignment == Enum117.VEHICLE) {
          // IN a vehicle
          ubCurrentGroup =
            pVehicleList[pCurrentSoldier.iVehicleId].ubMovementGroup;
        } else {
  ...
  function CanSoldierMoveWithVehicleId(
    pSoldier: SOLDIERTYPE,
    iVehicle1Id: INT32,
  ): boolean {
    ...
    if (iVehicle2Id != -1) {
      ...
      if (
        iVehicle1Id == iHelicopterVehicleId ||
        iVehicle2Id == iHelicopterVehicleId
      ) {
        return false;
      }
      ...
      if (
        pVehicle1.sSectorX == pVehicle2.sSectorX &&
  ...
  export function HandleBlitOfSectorLocatorIcon(
    sSectorX: INT16,
    sSectorY: INT16,
    sSectorZ: INT16,
    ubLocatorID: UINT8,
    ...
    switch (ubLocatorID) {
      ...
      case Enum156.LOCATOR_COLOR_RED:
        ...
        HandleBlitOfSectorLocatorIcon__ubFrame =
          HandleBlitOfSectorLocatorIcon__ubFrame % 13;
        break;
      case Enum156.LOCATOR_COLOR_YELLOW:
        ubBaseFrame = 13;
        HandleBlitOfSectorLocatorIcon__ubFrame =
          13 + (HandleBlitOfSectorLocatorIcon__ubFrame % 13);
        break;
      default:
        // not supported
        return;
    ...
    if (ANIMATED_BATTLEICON_FRAME_TIME < uiTimer - guiSectorLocatorBaseTime) {
      ...
      if (
        HandleBlitOfSectorLocatorIcon__ubFrame >
        ubBaseFrame + MAX_FRAME_COUNT_FOR_ANIMATED_BATTLE_ICON
      ) {
  ...
  export function CheckIfSalaryIncreasedAndSayQuote(
    pSoldier: SOLDIERTYPE,
    fTriggerContractMenu: boolean,
  ): boolean {
    Assert(pSoldier);
    ...
    if (pSoldier.fContractPriceHasIncreased) {
      if (fTriggerContractMenu) {
        ...
        TacticalCharacterDialogueWithSpecialEvent(
          pSoldier,
          0,
          DIALOGUE_SPECIAL_EVENT_SHOW_CONTRACT_MENU,
          0,
          0,
        );
      } else {
