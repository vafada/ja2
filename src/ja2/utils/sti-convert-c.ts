namespace ja2 {
  ...
  export function WriteSTIFile(
    pData: Buffer,
    pPalette: SGPPaletteEntry[],
    sWidth: INT16,
    sHeight: INT16,
    cOutputName: string /* STR */,
    fFlags: UINT32,
    ...
    let pOutputBuffer__Pointer = createPointer(
      () => pOutputBuffer,
      (v) => (pOutputBuffer = v),
    );
    let uiCompressedSize: UINT32;
    let uiCompressedSize__Pointer: Pointer<UINT32> = createPointer(
      () => uiCompressedSize,
      (v) => (uiCompressedSize = v),
    );
    ...
    let usNumberOfSubImages__Pointer: Pointer<UINT32> = createPointer(
      () => usNumberOfSubImages,
      (v) => (usNumberOfSubImages = v),
    );
    ...
    if (Header.fFlags & STCI_INDEXED && fFlags & CONVERT_ETRLE_COMPRESS) {
      if (
        !ConvertToETRLE(
          pOutputBuffer__Pointer,
          uiCompressedSize__Pointer,
          pSubImageBuffer,
          usNumberOfSubImages__Pointer,
          pData,
          sWidth,
          sHeight,
          fFlags,
    ...
    if (Header.fFlags & STCI_INDEXED) {
      ...
      if (Header.fFlags & STCI_ETRLE_COMPRESSED) {
        buffer = Buffer.allocUnsafe(
          pSubImageBuffer.length * STCI_SUBIMAGE_SIZE,
        );
    ...
    // write file data
    if (
      Header.fFlags & STCI_ZLIB_COMPRESSED ||
      Header.fFlags & STCI_ETRLE_COMPRESSED
    ) {
  ...
  function ConvertToETRLE(
    ppDest: Pointer<Buffer>,
    puiDestLen: Pointer<UINT32>,
    ppSubImageBuffer: STCISubImage[],
    pusNumberOfSubImages: Pointer<UINT16>,
    p8BPPBuffer: Buffer,
    usWidth: UINT16,
    usHeight: UINT16,
    fFlags: UINT32,
    ...
    let sCurrX: INT16;
    let sCurrX__Pointer: Pointer<INT16> = createPointer(
      () => sCurrX,
      (v) => (sCurrX = v),
    );
    let sCurrY: INT16;
    let sCurrY__Pointer: Pointer<INT16> = createPointer(
      () => sCurrY,
      (v) => (sCurrY = v),
    );
    let sNextX: INT16;
    let sNextX__Pointer: Pointer<INT16> = createPointer(
      () => sNextX,
      (v) => (sNextX = v),
    );
    let sNextY: INT16;
    let sNextY__Pointer: Pointer<INT16> = createPointer(
      () => sNextY,
      (v) => (sNextY = v),
    );
    ...
    if (fFlags & CONVERT_ETRLE_COMPRESS_SINGLE) {
      ...
      uiSubImageCompressedSize = ETRLECompressSubImage(
        pOutputNext,
        uiSpaceLeft,
        p8BPPBuffer,
        usWidth,
      ...
      while (fContinue) {
        ...
        if (
          !DetermineSubImageSize(p8BPPBuffer, usWidth, usHeight, pCurrSubImage)
        ) {
          fOk = false;
          break;
        }
  ...
  function ETRLECompressSubImage(
    pDest: Buffer,
    uiDestLen: UINT32,
    p8BPPBuffer: Buffer,
    usWidth: UINT16,
    usHeight: UINT16,
    pSubImage: STCISubImage,
  ): UINT32 {
    let usLoop: UINT16;
    let uiScanLineCompressedSize: UINT32;
    let uiSpaceLeft: UINT32 = uiDestLen;
    let uiOffset: UINT32;
    let uiOffset__Pointer: Pointer<UINT32> = createPointer(
      () => uiOffset,
      (v) => (uiOffset = uiOffset),
    );
    let pCurrent: number;
    ...
    for (usLoop = 0; usLoop < pSubImage.usHeight; usLoop++) {
      uiScanLineCompressedSize = ETRLECompress(
        pDest,
        uiSpaceLeft,
        p8BPPBuffer.subarray(pCurrent),
        pSubImage.usWidth,
      );
    ...
    while (uiSourceLoc < uiSourceLen && uiDestLoc < uiDestLen) {
      if (pSource[uiSourceLoc] == TCI) {
        ...
        do {
          ...
          uiCurrentSourceLoc < uiSourceLen &&
          pSource[uiCurrentSourceLoc] == TCI &&
  ...
  function GoPastWall(
    psNewX: Pointer<INT16>,
    psNewY: Pointer<INT16>,
    usWidth: UINT16,
    usHeight: UINT16,
    pCurrent: Buffer,
    sCurrX: INT16,
  ...
  function GoToNextSubImage(
    ...
    if (
      !DetermineOffset(uiOffset__Pointer, usWidth, usHeight, sCurrX, sCurrY)
    ) {
      return false;
    }
  ...
  function DetermineSubImageSize(
    p8BPPBuffer: Buffer,
    usWidth: UINT16,
    usHeight: UINT16,
    pSubImage: STCISubImage,
  ...
  function DetermineSubImageUsedSize(
    p8BPPBuffer: Buffer,
    usWidth: UINT16,
    usHeight: UINT16,
    pSubImage: STCISubImage,
    ...
    let sNewValue: INT16;
    let sNewValue__Pointer: Pointer<INT16> = createPointer(
      () => sNewValue,
      (v) => (sNewValue = v),
    );
  ...
  function CheckForDataInRows(
    psYValue: Pointer<INT16>,
    sYIncrement: INT16,
    p8BPPBuffer: Buffer,
    usWidth: UINT16,
    usHeight: UINT16,
  ...
  function CheckForDataInCols(
    psXValue: Pointer<INT16>,
    sXIncrement: INT16,
    p8BPPBuffer: Buffer,
    usWidth: UINT16,
    usHeight: UINT16,
    ...
    for (usLoop = 0; usLoop < pSubImage.usWidth; usLoop++) {
      ...
      pCurrent = p8BPPBuffer.subarray(uiOffset);
      pCurrent = CheckForDataInRowOrColumn(
        pCurrent,
        usWidth,
        pSubImage.usHeight,
      );
  ...
  function CheckForDataInRowOrColumn(
    pPixel: Buffer,
    usIncrement: UINT16,
    usNumberOfPixels: UINT16,
  ): Buffer | null {
