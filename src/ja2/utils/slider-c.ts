namespace ja2 {
  ...
  export function AddSlider(
    ...
    // checks
    if (ubStyle >= Enum329.NUM_SLIDER_STYLES) return -1;
    ...
    switch (ubStyle) {
      case Enum329.SLIDER_VERTICAL_STEEL:
        ...
        MSYS_DefineRegion(
          ...
          usPosX - Math.trunc(pNewSlider.usWidth / 2),
          usPosY,
          usPosX + Math.trunc(pNewSlider.usWidth / 2),
          pNewSlider.usPosY + pNewSlider.usHeight,
          sPriority,
          usCursor,
          SelectedSliderMovementCallBack,
          SelectedSliderButtonCallBack,
        );
        MSYS_SetRegionUserData(
          pNewSlider.ScrollAreaMouseRegion,
          1,
          pNewSlider.uiSliderID,
        );
        break;
      ...
      default:
        ...
        MSYS_DefineRegion(
          pNewSlider.ScrollAreaMouseRegion,
          usPosX,
          usPosY - DEFUALT_SLIDER_SIZE,
          pNewSlider.usPosX + pNewSlider.usWidth,
          usPosY + DEFUALT_SLIDER_SIZE,
          sPriority,
          usCursor,
          SelectedSliderMovementCallBack,
  ...
  export function RenderAllSliderBars(): void {
    ...
    if (gfLeftButtonState && gpCurrentSlider != null) {
      ...
      if (gusMouseYPos < gpCurrentSlider.usPosY) usPosY = 0;
      else usPosY = gusMouseYPos - gpCurrentSlider.usPosY;
  ...
  function RenderSelectedSliderBar(pSlider: SLIDER): void {
    if (pSlider.uiFlags & SLIDER_VERTICAL) {
      ...
      OptDisplayLine(
        pSlider.usPosX + 1,
        pSlider.usPosY - 1,
        pSlider.usPosX + pSlider.usWidth - 1,
        pSlider.usPosY - 1,
      ...
      OptDisplayLine(
        pSlider.usPosX,
        pSlider.usPosY,
        pSlider.usPosX + pSlider.usWidth,
        pSlider.usPosY,
        pSlider.usBackGroundColor,
      );
      OptDisplayLine(
  ...
  function RenderSliderBox(pSlider: SLIDER): void {
    ...
    if (pSlider.uiFlags & SLIDER_VERTICAL) {
      ...
      DestRect.iTop =
        ...
        Math.trunc(pSlider.ubSliderHeight / 2);
      ...
      if (!(pSlider.LastRect.iLeft == 0 && pSlider.LastRect.iRight == 0)) {
        ...
        BlitBufferToBuffer(
          guiSAVEBUFFER,
          guiRENDERBUFFER,
          pSlider.LastRect.iLeft,
          pSlider.LastRect.iTop,
          pSlider.ubSliderWidth,
  ...
  function SelectedSliderMovementCallBack(
  ...
  function SelectedSliderButtonCallBack(
    pRegion: MOUSE_REGION,
    iReason: INT32,
  ): void {
    let uiSelectedSlider: UINT32;
  ...
  function CalculateNewSliderIncrement(
    uiSliderID: UINT32,
    usPos: UINT16,
  ): void {
    ...
    if (pSlider.uiFlags & SLIDER_VERTICAL) {
      if (usPos >= pSlider.usHeight * 0.99) fLastSpot = true;

      if (usPos <= pSlider.usHeight * 0.01) fFirstSpot = true;

      // pSlider->usNumberOfIncrements
      if (fFirstSpot) dNewIncrement = 0;
      else if (fLastSpot) dNewIncrement = pSlider.usNumberOfIncrements;
      else
        dNewIncrement =
          (usPos / pSlider.usHeight) * pSlider.usNumberOfIncrements;
    } else {
      dNewIncrement = (usPos / pSlider.usWidth) * pSlider.usNumberOfIncrements;
    }

    pSlider.usCurrentIncrement = Math.trunc(dNewIncrement + 0.5);
    ...
    if (usOldIncrement != pSlider.usCurrentIncrement) {
      if (pSlider.uiFlags & SLIDER_VERTICAL) {
        ...
        pSlider.SliderChangeCallback(
          pSlider.usNumberOfIncrements - pSlider.usCurrentIncrement,
        );
      } else {
        // Call the call back for the slider
  ...
  function CalculateNewSliderBoxPosition(pSlider: SLIDER): void {
    ...
    if (pSlider.uiFlags & SLIDER_VERTICAL) {
      ...
      // else if the box is in the first position
        ...
        pSlider.usCurrentSliderBoxPosition =
          pSlider.usPosY +
          Math.trunc(pSlider.usHeight / pSlider.usNumberOfIncrements) *
      ...
      if (pSlider.usCurrentIncrement == pSlider.usNumberOfIncrements) {
        ...
        pSlider.usCurrentSliderBoxPosition =
          pSlider.usPosX +
          Math.trunc(pSlider.usWidth / pSlider.usNumberOfIncrements) *
            pSlider.usCurrentIncrement;
      }
  ...
  export function SetSliderValue(uiSliderID: UINT32, uiNewValue: UINT32): void {
    ...
    else pSlider.usCurrentIncrement = uiNewValue;
