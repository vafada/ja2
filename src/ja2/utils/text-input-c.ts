namespace ja2 {
  ...
  export function KillTextInputMode(): void {
    ...
    if (!gpTextInputHead) gpActive = <TEXTINPUTNODE>(<unknown>null);
  }
  ...
  export function AddTextInputField(
    sLeft: INT16,
    sTop: INT16,
    sWidth: INT16,
    sHeight: INT16,
    bPriority: INT8,
    szInitText: string /* Pointer<UINT16> */,
    ubMaxChars: UINT8,
    usInputType: UINT16,
  ): void {
    ...
    if (usInputType == Enum383.INPUTTYPE_EXCLUSIVE_24HOURCLOCK) ubMaxChars = 6;
  ...
  export function AddUserInputField(userFunction: INPUT_CALLBACK | null): void {
    ...
    pNode.next = null;
    if (!gpTextInputHead) {
      // first entry, so we don't start with text input.
      gfEditingText = false;
      gpTextInputHead = gpTextInputTail = pNode;
      pNode.prev = null;
      pNode.ubID = 0;
  ...
  function RemoveTextInputField(ubField: UINT8): void {
    ...
    AssertMsg(
      0,
      "Attempt to remove a text input field that doesn't exist.  Check your IDs.",
    );
  }
  ...
  export function SetInputFieldStringWith16BitString(
    ubField: UINT8,
    szNewText: string /* Pointer<UINT16> */,
  ): void {
    let curr: TEXTINPUTNODE | null;
    curr = gpTextInputHead;
    while (curr) {
      if (curr.ubID == ubField) {
        if (szNewText) {
          ...
          AssertMsg(
            0,
            FormatString(
              "Attempting to illegally set text into user field %d",
              curr.ubID,
            ),
          );
        }
  ...
  export function SetInputFieldStringWith8BitString(
    ubField: UINT8,
    szNewText: string /* Pointer<UINT8> */,
  ): void {
    let curr: TEXTINPUTNODE | null;
    curr = gpTextInputHead;
    while (curr) {
  ...
  // field will be blank.
  export function SetInputFieldStringWithNumericStrictValue(
    ubField: UINT8,
    iNumber: INT32,
  ): void {
    let curr: TEXTINPUTNODE | null;
    ...
    while (curr) {
      if (curr.ubID == ubField) {
        ...
        if (iNumber < 0)
          // negative number converts to blank string
          curr.szString = "";
        else {
          let iMax: INT32 = Math.pow(10.0, curr.ubMaxChars);
          if (iNumber > iMax)
            // set string to max value based on number of chars.
            curr.szString = swprintf("%d", iMax - 1); // set string to the number given
          else curr.szString = swprintf("%d", iNumber);
        }
  ...
  export function SelectNextField(): void {
    ...
    if (!gpActive) return;
    if (gpActive.szString) RenderInactiveTextFieldNode(gpActive);
  ...
  function SelectPrevField(): void {
    ...
    while (!fDone) {
      ...
      if (!gpActive) gpActive = <TEXTINPUTNODE>gpTextInputTail;
      if (gpActive.fEnabled) {
        fDone = true;
        if (gpActive.szString) {
          gubStartHilite = 0;
  ...
  export function SetTextInputRegularColors(
    ubForeColor: UINT8,
    ubShadowColor: UINT8,
  ): void {
  ...
  export function SetTextInputHilitedColors(
    ubForeColor: UINT8,
    ubShadowColor: UINT8,
    ubBackColor: UINT8,
  ): void {
  ...
  function SetDisabledTextFieldColors(
    ubForeColor: UINT8,
    ubShadowColor: UINT8,
    usTextFieldColor: UINT16,
  ): void {
  ...
  export function SetBevelColors(
    usBrighterColor: UINT16,
    usDarkerColor: UINT16,
  ): void {
  ...
  export function HandleTextInput(Event: InputAtom): boolean {
    ...
    if (
      Event.usKeyState & ALT_DOWN ||
      (Event.usKeyState & CTRL_DOWN && Event.usParam != DEL)
    )
      return false;
    // F1-F12 regardless of state are processed externally as well.
    if (
      (Event.usParam >= F1 && Event.usParam <= F12) ||
      (Event.usParam >= SHIFT_F1 && Event.usParam <= SHIFT_F12)
    ) {
      return false;
    }
    if (
      Event.usParam == "%".charCodeAt(0) ||
      Event.usParam == "\\".charCodeAt(0)
    ) {
    ...
    switch (gpActive.usInputType) {
      case Enum383.INPUTTYPE_EXCLUSIVE_DOSFILENAME: // dos file names
        if (
          ...
          if (
            !gubCursorPos &&
            uiKey >= "0".charCodeAt(0) &&
            uiKey <= "9".charCodeAt(0)
          ) {
      ...
      case Enum383.INPUTTYPE_EXCLUSIVE_COORDINATE: // coordinates such as a9, z78, etc.
        if (!gubCursorPos) {
          // first char is an lower case alpha
      ...
      case Enum383.INPUTTYPE_EXCLUSIVE_24HOURCLOCK:
        if (!gubCursorPos) {
          ...
          if (uiKey >= "0".charCodeAt(0) && uiKey <= "9".charCodeAt(0)) {
            if (gpActive.szString[0] == "2" && uiKey > "3".charCodeAt(0)) break;
            AddChar(uiKey);
          }
          if (!gpActive.szString[2]) AddChar(":".charCodeAt(0));
          else gubCursorPos++;
        } else if (gubCursorPos == 2) {
          if (uiKey == ":".charCodeAt(0)) AddChar(uiKey);
  ...
  function AddChar(uiKey: UINT32): void {
    ...
    if (gpActive.ubStrLen >= gpActive.ubMaxChars) {
      ...
      gpActive.szString =
        gpActive.szString.substring(0, gpActive.ubStrLen - 1) +
        String.fromCharCode(uiKey);
      return;
      ...
      gpActive.szString =
        gpActive.szString.substring(0, gubCursorPos) +
        String.fromCharCode(uiKey) +
        gpActive.szString.substring(gubCursorPos);
      gpActive.ubStrLen++;
      gubCursorPos++;
    }
  }
  ...
  function RemoveChar(ubArrayIndex: UINT8): void {
    ...
    while (ubArrayIndex < gpActive.ubStrLen) {
      gpActive.szString =
        gpActive.szString.substring(0, ubArrayIndex) +
        gpActive.szString.substring(ubArrayIndex + 1);
      ubArrayIndex++;
      fDeleting = true;
    }
  ...
  function MouseMovedInTextRegionCallback(
    reg: MOUSE_REGION,
    reason: INT32,
  ): void {
    let curr: TEXTINPUTNODE | null;
    if (gfLeftButtonState) {
      if (
        ...
        iCurrCharPos = 0;
        gubCursorPos = 0;
        iNextCharPos = Math.trunc(
          StringPixLengthArg(pColors.usFont, 1, gpActive.szString) / 2,
        );
        while (
          iCurrCharPos + Math.trunc((iNextCharPos - iCurrCharPos) / 2) <
            iClickX &&
          gubCursorPos < gpActive.ubStrLen
        ) {
          gubCursorPos++;
          iCurrCharPos = iNextCharPos;
          iNextCharPos = StringPixLengthArg(
            pColors.usFont,
            gubCursorPos + 1,
            gpActive.szString,
          );
  ...
  function MouseClickedInTextRegionCallback(
    reg: MOUSE_REGION,
    reason: INT32,
  ): void {
    let curr: TEXTINPUTNODE | null;
  ...
  function RenderBackgroundField(pNode: TEXTINPUTNODE): void {
    ...
    if (pColors.fBevelling) {
      ColorFillVideoSurfaceArea(
        FRAME_BUFFER,
        pNode.region.RegionTopLeftX,
        pNode.region.RegionTopLeftY,
        pNode.region.RegionBottomRightX,
        pNode.region.RegionBottomRightY,
        pColors.usDarkerColor,
      );
      ColorFillVideoSurfaceArea(
        FRAME_BUFFER,
        pNode.region.RegionTopLeftX + 1,
        pNode.region.RegionTopLeftY + 1,
        pNode.region.RegionBottomRightX,
        pNode.region.RegionBottomRightY,
    ...
    else usColor = pColors.usTextFieldColor;

    ColorFillVideoSurfaceArea(
      FRAME_BUFFER,
      pNode.region.RegionTopLeftX + 1,
      pNode.region.RegionTopLeftY + 1,
      pNode.region.RegionBottomRightX - 1,
      pNode.region.RegionBottomRightY - 1,
      usColor,
    );
  ...
  function RenderActiveTextField(): void {
    ...
    usOffset = Math.trunc(
      (gpActive.region.RegionBottomRightY -
        gpActive.region.RegionTopLeftY -
    ...
    if (gfHiliteMode && gubStartHilite != gubEndHilite) {
      ...
      for (i = 0; i < gpActive.ubStrLen; i++) {
        uiCursorXPos =
          StringPixLengthArg(pColors.usFont, i, gpActive.szString) + 3;
        ...
        if (gpActive.szString[i] != "%") {
          mprintf(
            uiCursorXPos + gpActive.region.RegionTopLeftX,
      ...
      mprintf(
        gpActive.region.RegionTopLeftX + 3,
        gpActive.region.RegionTopLeftY + usOffset,
        str,
      );
    }
  ...
  export function RenderInactiveTextField(ubID: UINT8): void {
    ...
    mprintf(
      pNode.region.RegionTopLeftX + 3,
      pNode.region.RegionTopLeftY + usOffset,
      str,
    );
    RestoreFontSettings();
  }
  ...
  export function RenderAllTextFields(): void {
    ...
    curr = gpTextInputHead;
    while (curr) {
      if (curr != gpActive) RenderInactiveTextFieldNode(curr);
      else RenderActiveTextField();
      curr = curr.next;
    }
  }
  ...
  export function RestoreSavedTextInputMode(): void {
    if (!pSavedHead)
      AssertMsg(
        0,
        "Attempting to restore saved text input stack head, when one doesn't exist.",
      );
  ...
  export function GetExclusive24HourTimeValueFromField(ubField: UINT8): UINT16 {
    ...
    while (curr) {
      if (curr.ubID == ubField) {
        ...
        if (
          (curr.szString[0] == "2" &&
            curr.szString[1] >= "0" && // 20-23
            curr.szString[1] <= "3") ||
          (curr.szString[0] >= "0" &&
            curr.szString[0] <= "1" && // 00-19
            curr.szString[1] >= "0" &&
            curr.szString[1] <= "9")
        ) {
          ...
          if (
            curr.szString[2] == ":" &&
            curr.szString.length == 5 && //	:
            curr.szString[3] >= "0" &&
            curr.szString[3] <= "5" && // 0-5
            curr.szString[4] >= "0" &&
            curr.szString[4] <= "9"
          ) {
            // 0-9
            // Hours
            usTime =
              ((curr.szString.charCodeAt(0) - 0x30) * 10 +
                curr.szString.charCodeAt(1) -
                0x30) *
              60;
            // Minutes
            usTime +=
              (curr.szString.charCodeAt(3) - 0x30) * 10 +
              curr.szString.charCodeAt(4) -
              0x30;
            return usTime;
          }
        }
    ...
    AssertMsg(
      false,
      FormatString(
        "GetExclusive24HourTimeValueFromField: Invalid field %d",
        ubField,
      ),
    );
    return 0xffff;
  ...
  // Utility functions for the INPUTTYPE_EXCLUSIVE_24HOURCLOCK input type.
  export function SetExclusive24HourTimeValue(
    ubField: UINT8,
    usTime: UINT16,
  ): void {
    let curr: TEXTINPUTNODE | null;
    ...
    while (curr) {
      if (curr.ubID == ubField) {
        ...
        curr.szString += String.fromCharCode(
          (Math.trunc(usTime / 60) % 10) + 0x30,
        ); // 1 hour
  ...
  function DoublePercentileCharacterFromStringIntoString(
    pSrcString: string /* Pointer<UINT16> */,
  ): string {
