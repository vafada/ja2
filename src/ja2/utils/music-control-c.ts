namespace ja2 {
  ...
  export function MusicPlay(uiNum: UINT32): boolean {
    ...
    if (uiMusicHandle != SOUND_ERROR) {
      DebugMsg(
        TOPIC_JA2,
        DBG_LEVEL_3,
        FormatString("Music PLay %d %d", uiMusicHandle, gubMusicMode),
      );
  ...
  function MusicStop(): boolean {
    if (uiMusicHandle != NO_SAMPLE) {
      DebugMsg(
        TOPIC_JA2,
        DBG_LEVEL_3,
        FormatString("Music Stop %d %d", uiMusicHandle, gubMusicMode),
      );
    ...
    if (COUNTERDONE(Enum386.MUSICOVERHEAD)) {
      ...
      if (gfMusicEnded) {
        ...
        DebugMsg(
          TOPIC_JA2,
          DBG_LEVEL_3,
          FormatString("Music End Loop %d %d", uiMusicHandle, gubMusicMode),
        );
  ...
  export function SetMusicMode(ubMusicMode: UINT8): boolean {
    ...
    if (ubMusicMode == Enum328.MUSIC_RESTORE) {
      if (
        bPreviousMode == Enum328.MUSIC_TACTICAL_VICTORY ||
        bPreviousMode == Enum328.MUSIC_TACTICAL_DEATH
      ) {
    ...
    if (gubOldMusicMode != ubMusicMode) {
      ...
      DebugMsg(
        TOPIC_JA2,
        DBG_LEVEL_3,
        FormatString("Music New Mode %d %d", uiMusicHandle, gubMusicMode),
      );
  ...
  function MusicStopCallback(pData: any): void {
    DebugMsg(
      TOPIC_JA2,
      DBG_LEVEL_3,
      FormatString("Music EndCallback %d %d", uiMusicHandle, gubMusicMode),
    );
