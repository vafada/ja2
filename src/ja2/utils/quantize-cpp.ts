namespace ja2 {
  export class CQuantizer {
      ...
      for (i = 0; i < iHeight; i++) {
        for (j = 0; j < iWidth; j++) {
          ...
          this.AddColor(
            ...
            createPointer(
              () => this.m_nLeafCount,
              (v) => (this.m_nLeafCount = v),
            ),
            this.m_pReducibleNodes,
          );
    ...
    AddColor(
      ppNode: Pointer<NODE | null>,
      r: BYTE,
      g: BYTE,
      b: BYTE,
      nColorBits: UINT32,
      nLevel: UINT32,
      pLeafCount: Pointer<UINT32>,
      pReducibleNodes: NODE[],
    ): void {
      ...
      //
        ...
        let nIndex: number =
          ...
          ((b & mask[nLevel]) >> shift);
        this.AddColor(
          createElementPointer(ppNode.value.pChild, nIndex),
          r,
          g,
          b,
          nColorBits,
    ...
    CreateNode(
      ...
      if (pNode.bIsLeaf) pLeafCount.value++;
      else {
    ...
    GetPaletteColors(
      pTree: NODE,
      prgb: SGPPaletteEntry[],
      pIndex: Pointer<UINT32>,
    ): void {
      if (pTree.bIsLeaf) {
        prgb[pIndex.value].peRed = Math.trunc(
          pTree.nRedSum / pTree.nPixelCount,
        );
        prgb[pIndex.value].peGreen = Math.trunc(
          pTree.nGreenSum / pTree.nPixelCount,
        );
        prgb[pIndex.value].peBlue = Math.trunc(
          pTree.nBlueSum / pTree.nPixelCount,
        );
