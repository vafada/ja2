namespace ja2 {
  export function LoadEncryptedDataFromFile(
    ...
    if (!hFile) {
      DebugMsg(
        TOPIC_JA2,
        DBG_LEVEL_3,
        "LoadEncryptedDataFromFile: Failed to FileOpen",
      );
    ...
    if (FileSeek(hFile, uiSeekFrom, FILE_SEEK_FROM_START) == false) {
      ...
      DebugMsg(
        TOPIC_JA2,
        DBG_LEVEL_3,
        "LoadEncryptedDataFromFile: Failed FileSeek",
      );
      return <string>(<unknown>null);
    }
    ...
    if ((uiBytesRead = FileRead(hFile, buffer, uiSeekAmount)) === -1) {
      ...
      DebugMsg(
        TOPIC_JA2,
        DBG_LEVEL_3,
        "LoadEncryptedDataFromFile: Failed FileRead",
      );
      return <string>(<unknown>null);
    }
