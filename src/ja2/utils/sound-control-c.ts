...
*/
  ...
  export function PlayJA2StreamingSample(
    usNum: UINT32,
    usRate: UINT32,
    ubVolume: UINT32,
    ubLoops: UINT32,
  ...
  export function PlayJA2SampleFromFile(
  ...
  export function PlayJA2StreamingSampleFromFile(
    szFileName: string /* STR8 */,
    usRate: UINT32,
    ubVolume: UINT32,
    ubLoops: UINT32,
    uiPan: UINT32,
    EndsCallback: SOUND_STOP_CALLBACK | null,
  ): UINT32 {
    // does the same thing as PlayJA2Sound, but one only has to pass the filename, not the index of the sound array
  ...
  export function PlayJA2Ambient(
    usNum: UINT32,
    ubVolume: UINT32,
    ubLoops: UINT32,
  ): UINT32 {
  ...
  export function PlaySoldierJA2Sample(
    usID: UINT16,
    usNum: UINT32,
    usRate: UINT32,
    ubVolume: UINT32,
    ...
    if (!(gTacticalStatus.uiFlags & LOADING_SAVED_GAME)) {
      ...
      if (MercPtrs[usID].bVisible != -1 || !fCheck) {
        return PlayJA2Sample(
          usNum,
          usRate,
          CalculateSoundEffectsVolume(ubVolume),
          ubLoops,
          uiPan,
        );
      }
  ...
  export function CalculateSpeechVolume(uiVolume: UINT32): UINT32 {
    return Math.trunc((uiVolume / HIGHVOLUME) * guiSpeechVolume + 0.5);
  }

  export function CalculateSoundEffectsVolume(uiVolume: UINT32): UINT32 {
    return Math.trunc((uiVolume / HIGHVOLUME) * guiSoundEffectsVolume + 0.5);
  }
  ...
  export function SoundDir(sGridNo: INT16): INT8 {
    ...
    sMiddleX =
      gsTopLeftWorldX + Math.trunc((gsBottomRightWorldX - gsTopLeftWorldX) / 2);
    ...
    if ((sAbsDif = Math.abs(sDif)) > 64) {
      ...
      if (sAbsDif > Math.trunc((gsBottomRightWorldX - gsTopLeftWorldX) / 2)) {
        ...
        if (sDif > 0) return LEFTSIDE;
        else return RIGHTSIDE;
      }
    } // hardly any difference, so sound should be played from middle
    else return MIDDLE;
  }
  ...
  export function SoundVolume(bInitialVolume: INT8, sGridNo: INT16): INT8 {
    ...
    sMiddleX =
      gsTopLeftWorldX + Math.trunc((gsBottomRightWorldX - gsTopLeftWorldX) / 2);
    sMiddleY =
      gsTopLeftWorldY + Math.trunc((gsBottomRightWorldY - gsTopLeftWorldY) / 2);
    ...
    if (sAbsDifX > 64 || sAbsDifY > 64) {
      ...
      // Is it outside the screen?
      if (
        sAbsDifX > Math.trunc((gsBottomRightWorldX - gsTopLeftWorldX) / 2) ||
        sAbsDifY > Math.trunc((gsBottomRightWorldY - gsTopLeftWorldY) / 2)
      ) {
  ...
  let gPositionSndData: POSITIONSND[] /* [NUM_POSITION_SOUND_EFFECT_SLOTS] */ =
    createArrayFrom(NUM_POSITION_SOUND_EFFECT_SLOTS, createPositionSnd);
  ...
  function GetFreePositionSnd(): INT32 {
    ...
    for (uiCount = 0; uiCount < guiNumPositionSnds; uiCount++) {
      if (gPositionSndData[uiCount].fAllocated == false) return uiCount;
    }
  ...
  export function NewPositionSnd(
    sGridNo: INT16,
    uiFlags: UINT32,
    uiData: SOLDIERTYPE,
    iSoundToPlay: UINT32,
  ...
  export function SetPositionSndGridNo(
    iPositionSndIndex: INT32,
    sGridNo: INT16,
  ): void {
    let pPositionSnd: POSITIONSND;
    ...
    for (cnt = 0; cnt < guiNumPositionSnds; cnt++) {
      ...
      if (pPositionSnd.fAllocated) {
        if (pPositionSnd.fInActive) {
          ...
          // Begin sound effect
          // Volume 0
          pPositionSnd.iSoundSampleID = PlayJA2Sample(
            pPositionSnd.iSoundToPlay,
            RATE_11025,
            0,
            0,
            MIDDLEPAN,
  ...
  function PositionSoundVolume(bInitialVolume: INT8, sGridNo: INT16): INT8 {
    ...
    // Scale
    return (
      bInitialVolume * Math.trunc((sMaxSoundDist - sSoundDist) / sMaxSoundDist)
    );
  }
