namespace ja2 {
  ...
  export const UPDATECOUNTER = (c: number) =>
    giTimerCounters[c] - BASETIMESLICE < 0
      ? (giTimerCounters[c] = 0)
      : (giTimerCounters[c] -= BASETIMESLICE);
  export const RESETCOUNTER = (c: number) =>
    (giTimerCounters[c] = giTimerIntervals[c]);
  export const COUNTERDONE = (c: number) =>
    giTimerCounters[c] == 0 ? true : false;

  export const UPDATETIMECOUNTER = (c: number) =>
    c - BASETIMESLICE < 0 ? 0 : c - BASETIMESLICE;
  export const RESETTIMECOUNTER = (d: number) => d;

  export const TIMECOUNTERDONE = (c: number, d: number) =>
    c == 0 ? true : false;
