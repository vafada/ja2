namespace ja2 {
  ...
  let ubLevelMoveLink: INT8[] /* [10] */ = [1, 2, 3, 4, 0, 0, 0, 0, 0, 0];

  // Soldier List used for all soldier overhead interaction
  export let Menptr: SOLDIERTYPE[] /* [TOTAL_SOLDIERS] */ = createArrayFrom(
    TOTAL_SOLDIERS,
    createSoldierType,
  );
  export let MercPtrs: SOLDIERTYPE[] /* Pointer<SOLDIERTYPE>[TOTAL_SOLDIERS] */ =
    createArray(TOTAL_SOLDIERS, <SOLDIERTYPE>(<unknown>null));

  export let MercSlots: SOLDIERTYPE[] /* Pointer<SOLDIERTYPE>[TOTAL_SOLDIERS] */ =
    createArray(TOTAL_SOLDIERS, <SOLDIERTYPE>(<unknown>null));
  export let guiNumMercSlots: UINT32 = 0;

  let AwaySlots: SOLDIERTYPE[] /* Pointer<SOLDIERTYPE>[TOTAL_SOLDIERS] */ =
    createArray(TOTAL_SOLDIERS, <SOLDIERTYPE>(<unknown>null));
  ...
  function GetFreeMercSlot(): INT32 {
    ...
    for (uiCount = 0; uiCount < guiNumMercSlots; uiCount++) {
      if (MercSlots[uiCount] == null) return uiCount;
    }
  ...
  function GetFreeAwaySlot(): INT32 {
    ...
    for (uiCount = 0; uiCount < guiNumAwaySlots; uiCount++) {
      if (AwaySlots[uiCount] == null) return uiCount;
    }
  ...
  export function InitOverhead(): boolean {
    ...
    for (cnt = 0; cnt < MAXTEAMS; cnt++) {
      ...
      for (
        ...
        cnt2 <= gTacticalStatus.Team[cnt].bLastID;
        cnt2++
      ) {
  ...
  */
  ...
  function NextAIToHandle(uiCurrAISlot: UINT32): boolean {
    ...
    for (; cnt < guiNumMercSlots; cnt++) {
      if (
        MercSlots[cnt] &&
        (MercSlots[cnt].bTeam != gbPlayerNum ||
  ...
  export function ExecuteOverhead(): boolean {
    ...
    let sAPCost: INT16 = 0;
    let sAPCost__Pointer = createPointer(
      () => sAPCost,
      (v) => (sAPCost = v),
    );
    let sBPCost: INT16 = 0;
    let sBPCost__Pointer = createPointer(
      () => sBPCost,
      (v) => (sBPCost = v),
    );
    let dXPos: FLOAT;
    ...
    let fKeepMoving__Pointer = createPointer(
      () => fKeepMoving,
      (v) => (fKeepMoving = v),
    );
    ...
    if (COUNTERDONE(Enum386.TOVERHEAD)) {
      ...
      if (
        gfPauseAllAI &&
        giPauseAllAITimer &&
        ExecuteOverhead__iTimerVal - giPauseAllAITimer > PAUSE_ALL_AI_DELAY
      ) {
      ...
      if (!gfPauseAllAI) {
        ...
        if (
          ...
          if (
            ExecuteOverhead__iTimerVal - giRTAILastUpdateTime >
            RT_DELAY_BETWEEN_AI_HANDLING
          ) {
      ...
      for (cnt = 0; cnt < guiNumMercSlots; cnt++) {
        ...
        if (pSoldier != null) {
          ...
          if (pSoldier.fReloading) {
            if (
              TIMECOUNTERDONE(pSoldier.ReloadCounter, pSoldier.sReloadDelay)
            ) {
          ...
          if (pSoldier.fBeginFade) {
            if (TIMECOUNTERDONE(pSoldier.FadeCounter, NEW_FADE_DELAY)) {
              ...
              if (pSoldier.fBeginFade == 1) {
                ...
                if (
                  bShadeLevel <=
                  (<LEVELNODE>gpWorldLevelData[pSoldier.sGridNo].pLandHead)
                    .ubShadeLevel
                ) {
                  bShadeLevel = (<LEVELNODE>(
          ...
          if (pSoldier.bLastRenderVisibleValue != pSoldier.bVisible) {
            ...
            if (
              pSoldier.bVisible == -1 &&
              pSoldier.bLastRenderVisibleValue >= 0
            ) {
              if (pSoldier.sGridNo != NOWHERE) {
                pSoldier.ubFadeLevel = (<LEVELNODE>(
                  gpWorldLevelData[pSoldier.sGridNo].pLandHead
                )).ubShadeLevel;
              } else {
                let i: number = 0;
            ...
            if (
              !(
                gAnimControl[pSoldier.usAnimState].uiFlags &
                  (ANIM_MOVING | ANIM_SPECIALMOVE) && pSoldier.fNoAPToFinishMove
              ) &&
              ...
              if (
                !(gAnimControl[pSoldier.usAnimState].uiFlags & ANIM_SPECIALMOVE)
              ) {
              ...
              if (
                ...
                if (pSoldier.fPastXDest && pSoldier.fPastYDest) {
                  ...
                  if (pSoldier.sFinalDestination == pSoldier.sGridNo) {
                    ...
                    if (
                      pSoldier.usAnimState == Enum193.JUMP_OVER_BLOCKING_PERSON
                    ) {
                    ...
                    if (pSoldier.ubWaitActionToDo) {
                      ...
                      DebugMsg(
                        ...
                        DBG_LEVEL_3,
                        FormatString(
                          "We are inside the IF PENDING Animation with soldier #%d",
                          pSoldier.ubID,
                        ),
                      );
                      ...
                      if (pSoldier.ubPendingAction == Enum257.MERC_PICKUPITEM) {
                        ...
                        pSoldier.ubPendingAction == Enum257.MERC_RELOADROBOT
                      ) {
                    ...
                    if (
                      ...
                      Enum290.QUOTE_ACTION_ID_CHECKFORDEST
                    ) {
                ...
                if (!pSoldier.fPausedMove && fKeepMoving) {
                  ...
                  if (pSoldier.usAnimState == Enum193.WALKING) {
                    MoveMerc(
                      pSoldier,
                      gubAnimWalkSpeeds[pSoldier.ubBodyType].dMovementChange,
                      dAngle,
                      true,
                    );
                  } else {
                    MoveMerc(
                      pSoldier,
                      gAnimControl[pSoldier.usAnimState].dMovementChange,
                      dAngle,
                      true,
                    );
                  }
          ...
          if (
            ...
            if (
              ...
              if (
                GetJA2Clock() - ExecuteOverhead__iTimerVal >
                RT_AI_TIMESLICE
              ) {
  ...
  function HaltGuyFromNewGridNoBecauseOfNoAPs(pSoldier: SOLDIERTYPE): void {
    ...
    if (pSoldier.bTeam == gbPlayerNum && gTacticalStatus.uiFlags & INCOMBAT) {
      ScreenMsg(
        ...
        MSG_INTERFACE,
        TacticalStr[Enum335.GUY_HAS_RUN_OUT_OF_APS_STR],
        pSoldier.name,
      );
    }
  ...
  export function HandleGotoNewGridNo(
    pSoldier: SOLDIERTYPE,
    pfKeepMoving: Pointer<boolean>,
    fInitialMove: boolean,
    usAnimState: UINT16,
  ): boolean {
    let sAPCost: INT16;
    let sBPCost: INT16;
    ...
    let sMineGridNo__Pointer = createPointer(
      () => sMineGridNo,
      (v) => (sMineGridNo = v),
    );
    ...
    if (pSoldier.bBreath < OKBREATH) {
      ...
      if (pSoldier.bBreath == 0) {
        ...
        DebugMsg(
          TOPIC_JA2,
          DBG_LEVEL_3,
          FormatString("HandleGotoNewGridNo() Failed: Out of Breath"),
        );
        return false;
      }
      ...
      if (pSoldier.bCollapsed) {
        ...
        DebugMsg(
          TOPIC_JA2,
          DBG_LEVEL_3,
          FormatString("HandleGotoNewGridNo() Failed: Has Collapsed"),
        );
        pSoldier.bBreathCollapsed = true;
        pSoldier.bEndDoorOpenCode = 0;
        return false;
    ...
    if (
      ...
      if (EnoughPoints(pSoldier, sAPCost, sBPCost, false)) {
        ...
        sOverFenceGridNo = NewGridNo(
          usNewGridNo,
          DirectionInc(pSoldier.usPathingData[pSoldier.usPathIndex + 1]),
        );
      ...
      if (bDirection == Enum245.NORTH || bDirection == Enum245.WEST) {
        ...
        DebugMsg(
          ...
          DBG_LEVEL_3,
          FormatString(
            "HandleGotoNewGridNo() Failed: Open door - invalid approach direction",
          ),
        );
      ...
      if (pStructure == null) {
        DebugMsg(
          TOPIC_JA2,
          DBG_LEVEL_3,
          FormatString("HandleGotoNewGridNo() Failed: Door does not exist"),
        );
        HaltGuyFromNewGridNoBecauseOfNoAPs(pSoldier);
        pSoldier.bEndDoorOpenCode = 0;
        pfKeepMoving.value = false;
    ...
    if (
      ...
      DebugMsg(
        ...
        DBG_LEVEL_3,
        FormatString(
          "HandleGotoNewGridNo() Failed: Tile %d Was blocked",
          usNewGridNo,
        ),
      );
    ...
    if (
      NearbyGroundSeemsWrong(pSoldier, usNewGridNo, false, sMineGridNo__Pointer)
    ) {
      if (pSoldier.uiStatusFlags & SOLDIER_PC) {
        ...
        if (!(gTacticalStatus.uiFlags & INCOMBAT)) {
          ...
          for (
            pSoldier2 = MercPtrs[cnt2];
            cnt2 >= gTacticalStatus.Team[gbPlayerNum].bFirstID;
            cnt2--, pSoldier2 = MercPtrs[cnt2]
          ) {
            if (pSoldier2.bActive) {
              EVENT_StopMerc(
                pSoldier2,
                pSoldier2.sGridNo,
                pSoldier2.bDirection,
              );
            }
          }
        } else {
        ...
        if (sMineGridNo != NOWHERE) {
          ...
          gpWorldLevelData[sMineGridNo].uiFlags |=
    ...
    if (pSoldier.fNoAPToFinishMove && !fInitialMove) {
      DebugMsg(
        TOPIC_JA2,
        DBG_LEVEL_3,
        FormatString("HandleGotoNewGridNo() Failed: No APs to finish move set"),
      );
      pSoldier.bEndDoorOpenCode = 0;
      pfKeepMoving.value = false;
    } else if (
      ...
      DebugMsg(
        TOPIC_JA2,
        DBG_LEVEL_3,
        FormatString("HandleGotoNewGridNo() Failed: No Path"),
      );
      pSoldier.bEndDoorOpenCode = 0;
      pfKeepMoving.value = false;
    }
    ...
    //}
      ...
      if (pSoldier.usPathIndex > 0) {
        ...
        if (
          gpWorldLevelData[pSoldier.sGridNo].ubExtFlags[pSoldier.bLevel] &
            ANY_SMOKE_EFFECT &&
          PreRandom(5) == 0
        ) {
          ...
          if (!AM_A_ROBOT(pSoldier)) {
            if (
              ...
              if (
                ...
                // check for gas mask
                pExplosive =
                  Explosive[Item[Enum225.TEARGAS_GRENADE].ubClassIndex];
              }
            }
            if (
              gpWorldLevelData[pSoldier.sGridNo].ubExtFlags[pSoldier.bLevel] &
              ...
              if (
                ...
                pExplosive =
                  Explosive[Item[Enum225.MUSTARD_GRENADE].ubClassIndex];
              }
            }
          }
          if (
            ...
            if (!(pSoldier.fHitByGasFlags & Enum264.HIT_BY_CREATUREGAS)) {
              // gas mask doesn't help vs creaturegas
              pExplosive =
          ...
          if (pExplosive) {
            ...
            DishOutGasDamage(
              ...
              100 *
                (pExplosive.ubStunDamage +
                  PreRandom(Math.trunc(pExplosive.ubStunDamage / 2))),
              NOBODY,
            );
          }
        }
        ...
        if (!fDontContinue) {
          if (
            (pSoldier.bOverTerrainType == Enum315.FLAT_FLOOR ||
            ...
            if (
              ...
              if (pSoldier.bTeam == gbPlayerNum) {
                ScreenMsg(
                  FONT_MCOLOR_LTYELLOW,
                  MSG_UI_FEEDBACK,
                  Message[Enum334.STR_SLIPPED_MARBLES],
                  pSoldier.name,
                );
              }
          ...
          if (
            ...
            OKFallDirection(
              pSoldier,
              pSoldier.sGridNo + DirectionInc(pSoldier.bDirection),
              pSoldier.bLevel,
              pSoldier.bDirection,
              pSoldier.usAnimState,
            )
        ...
        if (pSoldier.ubBodyType == Enum194.BLOODCAT) {
          if (
            ...
            if (!pSoldier.bReverse) {
              ...
              ChangeSoldierState(
                pSoldier,
                Enum193.BLOODCAT_WALK_BACKWARDS,
                1,
                true,
              );
            }
      ...
      DebugMsg(
        ...
        DBG_LEVEL_3,
        FormatString(
          "HandleGotoNewGridNo() Failed: No APs %d %d",
          sAPCost,
  ...
  function HandleAtNewGridNo(
    ...
    if (gTacticalStatus.fInterruptOccurred) {
      ...
      if (gTacticalStatus.fEnemySightingOnTheirTurn) {
        if (
          gTacticalStatus.ubEnemySightingOnTheirTurnEnemyID == pSoldier.ubID
        ) {
    ...
    if (pSoldier.bTeam == gbPlayerNum) {
      if (pSoldier.ubWhatKindOfMercAmI == Enum260.MERC_TYPE__EPC) {
        ...
        if (
          ...
          pSoldier.ubProfile == Enum268.MARIA &&
          pSoldier.sSectorX == 6 &&
          ...
          if (NPCInRoom(Enum268.ANGEL, 2)) {
            // room 2 is leather shop
            //	UnRecruitEPC( MARIA );
            TriggerNPCRecord(Enum268.ANGEL, 12);
          }
        } else if (
          pSoldier.ubProfile == Enum268.JOEY &&
          pSoldier.sSectorX == 8 &&
      ...
      // Drassen stuff for John & Mary
        ...
        gubQuest[Enum169.QUEST_ESCORT_TOURISTS] == QUESTINPROGRESS &&
        pSoldier.sSectorX == 13 &&
        pSoldier.sSectorY == MAP_ROW_B &&
        pSoldier.bSectorZ == 0
      ) {
  ...
  export function InternalSelectSoldier(
    usSoldierID: UINT16,
    fAcknowledge: boolean,
    fForceReselect: boolean,
    fFromUI: boolean,
  ): void {
    ...
    if (!OK_INTERRUPT_MERC(pSoldier)) {
      ...
      if (fFromUI) {
        ScreenMsg(
          FONT_MCOLOR_LTYELLOW,
          MSG_UI_FEEDBACK,
          TacticalStr[Enum335.MERC_IS_UNAVAILABLE_STR],
          pSoldier.name,
        );
      }
      return;
    ...
    if (gusSelectedSoldier != NO_SOLDIER) {
      ...
      if (pOldSoldier.uiStatusFlags & SOLDIER_GREEN_RAY) {
        LightHideRays(
          Math.trunc(pOldSoldier.dXPos / CELL_X_SIZE),
          Math.trunc(pOldSoldier.dYPos / CELL_Y_SIZE),
        );
    ...
    if (
      ...
      gCurrentUIMode != Enum206.LOCKUI_MODE &&
      gCurrentUIMode != Enum206.LOCKOURTURN_UI_MODE
    ) {
  ...
  export function InternalLocateGridNo(sGridNo: UINT16, fForce: boolean): void {
    ...
    ({ sX: sNewCenterWorldX, sY: sNewCenterWorldY } =
      ConvertGridNoToCenterCellXY(sGridNo));
  ...
  export function SlideToLocation(
    usReasonID: UINT16,
    sDestGridNo: INT16,
  ): void {
    ...
    if (!fMissionFailed) {
        ...
        if (
          ...
          switch (bBuddyIndex) {
            case 0:
              ...
              TacticalCharacterDialogue(
                pTeamSoldier,
                Enum202.QUOTE_BUDDY_ONE_KILLED,
              );
              break;
            case 1:
              // buddy #2 died!
              TacticalCharacterDialogue(
                pTeamSoldier,
                Enum202.QUOTE_BUDDY_TWO_KILLED,
              );
              break;
            case 2:
              ...
              TacticalCharacterDialogue(
                pTeamSoldier,
                Enum202.QUOTE_LEARNED_TO_LIKE_MERC_KILLED,
              );
              break;
            default:
              break;
      ...
      switch (pSoldier.ubProfile) {
        case Enum268.SLAY:
          ...
          if (
            pTeamSoldier &&
            pTeamSoldier.bAttitude == Enum242.ATTACKSLAYONLY &&
  ...
  export function HandleNPCTeamMemberDeath(pSoldierOld: SOLDIERTYPE): void {
    ...
    if (pSoldierOld.ubProfile != NO_PROFILE) {
      ...
      if (
        !(pSoldierOld.uiStatusFlags & SOLDIER_VEHICLE) &&
        !TANK(pSoldierOld)
      ) {
    ...
    if (pSoldierOld.bTeam == CIV_TEAM) {
      ...
      switch (pSoldierOld.ubProfile) {
        ...
        case Enum268.PABLO:
          AddFutureDayStrategicEvent(
            Enum132.EVENT_SECOND_AIRPORT_ATTENDANT_ARRIVED,
            480 + Random(60),
            0,
            1,
          );
        ...
        */
        ...
        case Enum268.JOEY:
          ...
          if (
            ...
            /* Delayed loyalty effects elimininated.  Sep.12/98.  ARM
                                                  ...
                                                  AddStrategicEvent( EVENT_TOWN_LOYALTY_UPDATE, GetWorldTotalMin() + 30, uiLoyaltyValue );
          */
            DecrementTownLoyalty(
              Enum135.CAMBRIA,
              LOYALTY_PENALTY_MARTHA_HEART_ATTACK,
            );
          } // Martha doesn't see it.  She lives, but Joey is found a day or so later anyways
          else {
            /*
                                                  // create the event value, for town CAMBRIA
        ...
        case Enum268.DYNAMO:
          ...
          if (gubQuest[Enum169.QUEST_FREE_DYNAMO] == QUESTINPROGRESS) {
            EndQuest(
              Enum169.QUEST_FREE_DYNAMO,
              pSoldierOld.sSectorX,
              pSoldierOld.sSectorY,
            );
          }
          break;
        case Enum268.KINGPIN:
          ...
          if (gubQuest[Enum169.QUEST_KINGPIN_MONEY] == QUESTINPROGRESS) {
            EndQuest(
              Enum169.QUEST_KINGPIN_MONEY,
              pSoldierOld.sSectorX,
              pSoldierOld.sSectorY,
            );
            HandleNPCDoAction(
              Enum268.KINGPIN,
              Enum213.NPC_ACTION_GRANT_EXPERIENCE_3,
              0,
            );
          }
          SetFactTrue(Enum170.FACT_KINGPIN_DEAD);
          ExecuteStrategicAIAction(
            Enum173.STRATEGIC_AI_ACTION_KINGPIN_DEAD,
            0,
            0,
          );
          break;
        case Enum268.DOREEN:
          ...
          IncrementTownLoyalty(
            Enum135.DRASSEN,
            LOYALTY_BONUS_CHILDREN_FREED_DOREEN_KILLED,
          );
      ...
      if (
        pSoldierOld.ubBodyType != Enum194.CROW &&
        pSoldierOld.ubBodyType != Enum194.COW
      ) {
      ...
      if (bMilitiaRank != -1) {
        ...
        StrategicRemoveMilitiaFromSector(
          gWorldSectorX,
          gWorldSectorY,
          bMilitiaRank,
          1,
      ...
      HandleGlobalLoyaltyEvent(
        Enum190.GLOBAL_LOYALTY_NATIVE_KILLED,
        gWorldSectorX,
        gWorldSectorY,
        gbWorldSectorZ,
      );
    } // enemies and creatures... should any of this stuff not be called if a creature dies?
    else {
      ...
      if (pSoldierOld.bTeam == ENEMY_TEAM) {
        ...
        TrackEnemiesKilled(
          Enum189.ENEMY_KILLED_IN_TACTICAL,
          pSoldierOld.ubSoldierClass,
        );
      }
      ...
      if (
        ...
        HandleMoraleEvent(
          MercPtrs[pSoldierOld.ubAttackerID],
          Enum234.MORALE_KILLED_ENEMY,
          gWorldSectorX,
          gWorldSectorY,
          gbWorldSectorZ,
        );
      ...
      HandleGlobalLoyaltyEvent(
        Enum190.GLOBAL_LOYALTY_ENEMY_KILLED,
        gWorldSectorX,
        gWorldSectorY,
        gbWorldSectorZ,
      );
      ...
      if (pSoldierOld.ubProfile == Enum268.QUEEN) {
        HandleMoraleEvent(
          null,
          Enum234.MORALE_DEIDRANNA_KILLED,
          gWorldSectorX,
          gWorldSectorY,
          gbWorldSectorZ,
        );
        ...
        HandleMoraleEvent(
          null,
          Enum234.MORALE_MONSTER_QUEEN_KILLED,
          gWorldSectorX,
          gWorldSectorY,
          gbWorldSectorZ,
        );
  ...
  export function CheckForPotentialAddToBattleIncrement(
    pSoldier: SOLDIERTYPE,
  ): void {
  ...
  export function MakeCivHostile(pSoldier: SOLDIERTYPE, bNewSide: INT8): void {
    ...
    switch (pSoldier.ubProfile) {
      ...
      case Enum268.ANGEL:
        if (
          ...
          gubQuest[Enum169.QUEST_RESCUE_MARIA] == QUESTDONE
        ) {
          bNewSide = 2;
        }
        break;
    ...
    if (
      ...
      pSoldier.ubProfile == Enum268.GENERAL
    ) {
  ...
  export function CivilianGroupMembersChangeSidesWithinProximity(
    pAttacked: SOLDIERTYPE,
  ): UINT8 {
    let pSoldier: SOLDIERTYPE;
    ...
    for (
      ...
      if (
        ...
        if (
          pSoldier.ubCivilianGroup == pAttacked.ubCivilianGroup &&
          ...
          if (
            ...
            PythSpacesAway(pSoldier.sGridNo, pAttacked.sGridNo) <
              MaxDistanceVisible() ||
            (pAttacked.ubAttackerID != NOBODY &&
              PythSpacesAway(
                pSoldier.sGridNo,
                MercPtrs[pAttacked.ubAttackerID].sGridNo,
              ) < MaxDistanceVisible())
          ) {
            MakeCivHostile(pSoldier, 2);
  ...
  export function CivilianGroupMemberChangesSides(
    pAttacked: SOLDIERTYPE,
  ): SOLDIERTYPE {
  ...
  */
    ...
    if (
      ...
      AddStrategicEvent(
        Enum132.EVENT_MAKE_CIV_GROUP_HOSTILE_ON_NEXT_SECTOR_ENTRANCE,
        GetWorldTotalMin() + 300,
        pNewAttacked.ubCivilianGroup,
      );
      gTacticalStatus.fCivGroupHostile[pNewAttacked.ubCivilianGroup] =
  ...
  export function CivilianGroupChangesSides(ubCivilianGroup: UINT8): void {
    ...
    for (
      ...
      if (
        ...
        if (
          pSoldier.ubCivilianGroup == ubCivilianGroup &&
          pSoldier.ubBodyType != Enum194.COW
        ) {
          MakeCivHostile(pSoldier, 2);
          if (pSoldier.bOppCnt > 0) {
  ...
  */
  ...
  function MilitiaChangesSides(): void {
    ...
    for (
      pSoldier = MercPtrs[cnt];
      cnt <= gTacticalStatus.Team[MILITIA_TEAM].bLastID;
      cnt++, pSoldier = MercPtrs[cnt]
    ) {
      if (pSoldier.bActive && pSoldier.bInSector && pSoldier.bLife) {
        MakeCivHostile(pSoldier, 2);
...
*/
  ...
  export function FindNextActiveAndAliveMerc(
    pSoldier: SOLDIERTYPE,
  ...
  export function FindNextActiveSquad(pSoldier: SOLDIERTYPE): SOLDIERTYPE {
    ...
    for (
      cnt = pSoldier.bAssignment + 1;
      cnt < Enum275.NUMBER_OF_SQUADS;
      cnt++
    ) {
      for (cnt2 = 0; cnt2 < NUMBER_OF_SOLDIERS_PER_SQUAD; cnt2++) {
        if (
          Squad[cnt][cnt2] != null &&
          Squad[cnt][cnt2].bInSector &&
          OK_INTERRUPT_MERC(Squad[cnt][cnt2]) &&
          OK_CONTROLLABLE_MERC(Squad[cnt][cnt2]) &&
  ...
  export function FindPrevActiveAndAliveMerc(
    pSoldier: SOLDIERTYPE,
    fGoodForLessOKLife: boolean,
    fOnlyRegularMercs: boolean,
  ): UINT8 {
  ...
  export function NewOKDestination(
    pCurrSoldier: SOLDIERTYPE,
    sGridNo: INT16,
    fPeopleToo: boolean,
    bLevel: INT8,
  ): boolean {
      ...
      if (gpWorldLevelData[sGridNo].pStructureHead != null) {
        ...
        while (pStructure != null) {
          if (!(pStructure.fFlags & STRUCTURE_PASSABLE)) {
            ...
            if (
              pStructure.fFlags & STRUCTURE_MOBILE &&
  ...
  function NewOKDestinationAndDirection(
    pCurrSoldier: SOLDIERTYPE,
    sGridNo: INT16,
    bDirection: INT8,
    fPeopleToo: boolean,
      ...
      if (pStructureFileRef) {
...

        {
          ...
          fOk = InternalOkayToAddStructureToWorld(
            sGridNo,
            pCurrSoldier.bLevel,
            pStructureFileRef.pDBStructureRef[gOneCDirection[bLoop]],
            usStructureID,
            !fPeopleToo,
          );
          if (fOk) {
  ...
  export function IsLocationSittable(
    iMapIndex: INT32,
    fOnRoof: boolean,
  ): boolean {
    let pStructure: STRUCTURE | null;
  ...
  export function IsLocationSittableExcludingPeople(
    iMapIndex: INT32,
    fOnRoof: boolean,
  ): boolean {
    let pStructure: STRUCTURE | null;
  ...
  export function TeamMemberNear(
    bTeam: INT8,
    sGridNo: INT16,
    iRange: INT32,
  ): boolean {
    ...
    for (
      bLoop = gTacticalStatus.Team[bTeam].bFirstID, pSoldier = MercPtrs[bLoop];
      bLoop <= gTacticalStatus.Team[bTeam].bLastID;
      bLoop++, pSoldier = MercPtrs[bLoop]
    ) {
      if (
        pSoldier.bActive &&
        pSoldier.bInSector &&
  ...
  export function FindAdjacentGridEx(
    pSoldier: SOLDIERTYPE,
    sGridNo: INT16,
    pubDirection: Pointer<UINT8> | null,
    psAdjustedGridNo: Pointer<INT16> | null,
    fForceToPerson: boolean,
    fDoor: boolean,
  ): INT16 {
    ...
    if (
      ...
      !FindStructure(sGridNo, STRUCTURE_SWITCH)
    ) {
      ...
      if (fDoor) {
        ...
        if (pDoor != null) {
          ...
          if (
            ubWallOrientation == Enum314.OUTSIDE_TOP_LEFT ||
            ubWallOrientation == Enum314.INSIDE_TOP_LEFT
          ) {
            // To the south!
          ...
          if (
            ubWallOrientation == Enum314.OUTSIDE_TOP_RIGHT ||
            ubWallOrientation == Enum314.INSIDE_TOP_RIGHT
          ) {
            // TO the east!
    ...
    if (
      (sOkTest = NewOKDestination(pSoldier, sGridNo, true, pSoldier.bLevel))
    ) {
      // no problem going there! nobody on it!
    ...
    for (cnt = 0; cnt < 4; cnt++) {
      ...
      if (fDoor) {
        if (
          gubWorldMovementCosts[sSpot][ubTestDirection][pSoldier.bLevel] >=
          TRAVELCOST_BLOCKED
        ) {
      ...
      if (pDoor != null) {
        ...
        if (
          ...
          if (
            sDirs[cnt] == Enum245.NORTH ||
            sDirs[cnt] == Enum245.WEST ||
            sDirs[cnt] == Enum245.SOUTH
          )
            continue;
        }
  ...
  export function FindNextToAdjacentGridEx(
    pSoldier: SOLDIERTYPE,
    sGridNo: INT16,
    pubDirection: Pointer<UINT8>,
    psAdjustedGridNo: Pointer<INT16>,
    fForceToPerson: boolean,
    fDoor: boolean,
  ): INT16 {
  ...
  if (sCloseGridNo != NOWHERE)
{
  ...
  */
  ...
  export function FindAdjacentPunchTarget(
    pSoldier: SOLDIERTYPE,
    pTargetSoldier: SOLDIERTYPE,
    psAdjustedTargetGridNo: Pointer<INT16>,
    pubDirection: Pointer<UINT8>,
  ): INT16 {
    let cnt: INT16;
    let sSpot: INT16;
    ...
    for (cnt = 0; cnt < Enum245.NUM_WORLD_DIRECTIONS; cnt++) {
      ...
      if (
        DoorTravelCost(
          pSoldier,
          sSpot,
          gubWorldMovementCosts[sSpot][cnt][pSoldier.bLevel],
          false,
          null,
        ) >= TRAVELCOST_BLOCKED
      ) {
  ...
  export function UIOKMoveDestination(
    pSoldier: SOLDIERTYPE,
    usMapPos: UINT16,
  ): UINT8 {
  ...
  export function HandleTeamServices(ubTeamNum: UINT8): void {
    ...
    for (
      pTeamSoldier = MercPtrs[cnt];
      cnt <= gTacticalStatus.Team[ubTeamNum].bLastID;
      cnt++, pTeamSoldier = MercPtrs[cnt]
    ) {
      if (
        pTeamSoldier.bLife >= OKLIFE &&
        ...
        if (pTeamSoldier.usAnimState == Enum193.GIVING_AID) {
          ...
          // Get victim pointer
          usSoldierIndex = WhoIsThere2(
            pTeamSoldier.sTargetGridNo,
            pTeamSoldier.bLevel,
          );
          if (usSoldierIndex != NOBODY) {
            ...
            if (pTargetSoldier.ubServiceCount) {
              ...
              uiPointsUsed = SoldierDressWound(
                pTeamSoldier,
                pTargetSoldier,
                usKitPts,
                usKitPts,
              ...
              if (!pTargetSoldier.bBleeding && pTargetSoldier.bLife >= OKLIFE) {
                ScreenMsg(
                  FONT_MCOLOR_LTYELLOW,
                  MSG_INTERFACE,
                  TacticalStr[Enum335.MERC_IS_ALL_BANDAGED_STR],
                  pTargetSoldier.name,
                );
              ...
              UseKitPoints(
                pTeamSoldier.inv[Enum261.HANDPOS],
                uiPointsUsed,
                pTeamSoldier,
              );
              ...
              if (usKitPts <= 0) {
                ...
                if (bSlot != NO_SLOT) {
                  SwapObjs(
                    pTeamSoldier.inv[Enum261.HANDPOS],
                    pTeamSoldier.inv[bSlot],
                  );
                } else {
                  ScreenMsg(
                    FONT_MCOLOR_LTYELLOW,
                    MSG_INTERFACE,
                    TacticalStr[Enum335.MERC_IS_OUT_OF_BANDAGES_STR],
                    pTeamSoldier.name,
                  );
        ...
        if (pSoldier.bInSector && pSoldier.ubBodyType != Enum194.CROW) {
          ...
          if (pSoldier.ubProfile != NO_PROFILE) {
            if (pSoldier.bTeam == CIV_TEAM && pSoldier.bNeutral) {
              ...
              if (
                gMercProfiles[pSoldier.ubProfile].sPreCombatGridNo == 0 ||
                gMercProfiles[pSoldier.ubProfile].sPreCombatGridNo == NOWHERE
              ) {
                gMercProfiles[pSoldier.ubProfile].sPreCombatGridNo =
                  pSoldier.sGridNo;
              }
            } else {
              gMercProfiles[pSoldier.ubProfile].sPreCombatGridNo = NOWHERE;
  ...
  export function EnterCombatMode(ubStartingTeam: UINT8): void {
    ...
    if (gTacticalStatus.uiFlags & INCOMBAT) {
      DebugMsg(
        TOPIC_JA2,
        DBG_LEVEL_3,
        "Can't enter combat when already in combat",
      );
    ...
    if (NumCapableEnemyInSector() == 0) {
      DebugMsg(
        TOPIC_JA2,
        DBG_LEVEL_3,
        "Can't enter combat when no capable enemies",
      );
  ...
  export function ExitCombatMode(): void {
    ...
    gTacticalStatus.uiTimeSinceLastOpplistDecay = Math.max(
      0,
      GetWorldTotalSeconds() - TIME_BETWEEN_RT_OPPLIST_DECAYS,
    );
  ...
  export function SetEnemyPresence(): void {
    ...
    if (!gTacticalStatus.fEnemyInSector && gTacticalStatus.fVirginSector) {
      ...
      ScreenMsg(
        FONT_MCOLOR_LTYELLOW,
        MSG_INTERFACE,
        TacticalStr[Enum335.ENEMY_IN_SECTOR_STR],
      );
  ...
  function SoldierHasSeenEnemiesLastFewTurns(
    pTeamSoldier: SOLDIERTYPE,
  ): boolean {
    let cnt2: INT32;
    let pSoldier: SOLDIERTYPE;
    let cnt: INT32;
    ...
    for (cnt = 0; cnt < MAXTEAMS; cnt++) {
      if (gTacticalStatus.Team[cnt].bSide != pTeamSoldier.bSide) {
        ...
        cnt2 = gTacticalStatus.Team[cnt].bFirstID;
        for (
          pSoldier = MercPtrs[cnt2];
          cnt2 <= gTacticalStatus.Team[cnt].bLastID;
          cnt2++, pSoldier = MercPtrs[cnt2]
        ) {
          if (
            pSoldier.bActive &&
            pSoldier.bInSector &&
            ...
            if (
              !CONSIDERED_NEUTRAL(pTeamSoldier, pSoldier) &&
              pTeamSoldier.bSide != pSoldier.bSide
            ) {
              // Have we not seen this guy.....
              if (
                pTeamSoldier.bOppList[cnt2] >= SEEN_CURRENTLY &&
  ...
  function WeSawSomeoneThisTurn(): boolean {
    ...
    for (uiLoop = 0; uiLoop < guiNumMercSlots; uiLoop++) {
      ...
      if (pSoldier != null) {
        if (pSoldier.bTeam == gbPlayerNum) {
          for (
            uiLoop2 = gTacticalStatus.Team[ENEMY_TEAM].bFirstID;
            uiLoop2 < TOTAL_SOLDIERS;
            uiLoop2++
          ) {
  ...
  export function CheckForEndOfCombatMode(
    fIncrementTurnsNotSeen: boolean,
  ): boolean {
    let pTeamSoldier: SOLDIERTYPE;
    let cnt: UINT32 = 0;
      ...
      for (cnt = 0; cnt < guiNumMercSlots; cnt++) {
        ...
        if (
          ...
          if (SoldierHasSeenEnemiesLastFewTurns(pTeamSoldier)) {
            ...
            if (
              pTeamSoldier.bTeam == gbPlayerNum ||
              (pTeamSoldier.bTeam == MILITIA_TEAM && pTeamSoldier.bSide == 0)
            ) {
              // or friendly militia
    ...
    if (gTacticalStatus.bConsNumTurnsNotSeen > 1) {
      ...
      if (fSayQuote && gTacticalStatus.uiFlags & IN_CREATURE_LAIR) {
        SayQuoteFromAnyBodyInSector(
          Enum202.QUOTE_WORRIED_ABOUT_CREATURE_PRESENCE,
        );
      }
      // Are we fighting bloodcats?
        ...
        if (fSayQuote) {
          ...
          if (NumEnemyInSectorNotDeadOrDying() > 0) {
            SayQuoteFromAnyBodyInSector(
              Enum202.QUOTE_WARNING_OUTSTANDING_ENEMY_AFTER_RT,
            );
          }
        }
      }
  ...
  export function CheckForEndOfBattle(fAnEnemyRetreated: boolean): boolean {
    ...
    if (fBattleLost) {
      ...
      HandleGlobalLoyaltyEvent(
        Enum190.GLOBAL_LOYALTY_BATTLE_LOST,
        gWorldSectorX,
        gWorldSectorY,
        gbWorldSectorZ,
      );
    ...
    if (fBattleWon) {
      if (gTacticalStatus.bBoxingState == Enum247.NOT_BOXING) {
        // if boxing don't do any of this stuff
      ...
      if (gTacticalStatus.bBoxingState == Enum247.NOT_BOXING) {
        ...
        if (
          gTacticalStatus.bNumFoughtInBattle[ENEMY_TEAM] +
          ...
          // Loop through all mercs and make go
          for (
            pTeamSoldier = MercPtrs[gTacticalStatus.Team[gbPlayerNum].bFirstID];
            cnt <= gTacticalStatus.Team[gbPlayerNum].bLastID;
            cnt++, pTeamSoldier = MercPtrs[cnt]
          ) {
                ...
                if (pTeamSoldier.bTeam == gbPlayerNum) {
                  ...
                  if (
                    ...
                    if (pTeamSoldier.bAssignment < Enum117.ON_DUTY) {
                      ...
                      if (
                        gAnimControl[pTeamSoldier.usAnimState].ubHeight !=
                        ANIM_STAND
                      ) {
                        ChangeSoldierStance(pTeamSoldier, ANIM_STAND);
                      } else {
                        // If they are aiming, end aim!
                        usAnimState = PickSoldierReadyAnimation(
                          pTeamSoldier,
                          true,
                        );
          ...
          HandleGlobalLoyaltyEvent(
            Enum190.GLOBAL_LOYALTY_BATTLE_WON,
            gWorldSectorX,
            gWorldSectorY,
            gbWorldSectorZ,
          );
          ...
          if (
            ...
            (gfLastMercTalkedAboutKillingID != NOBODY &&
              !(
                MercPtrs[gfLastMercTalkedAboutKillingID].uiStatusFlags &
                SOLDIER_MONSTER
              ))
          ) {
          ...
          if (fAnEnemyRetreated) {
            ...
            if (
              ...
              gfLastMercTalkedAboutKillingID != NOBODY &&
              MercPtrs[gfLastMercTalkedAboutKillingID].ubBodyType ==
                Enum194.BLOODCAT
            ) {
  ...
  export function CycleThroughKnownEnemies(): void {
    ...
    if (CycleThroughKnownEnemies__fFirstTime) {
      ...
      CycleThroughKnownEnemies__usStartToLook =
        gTacticalStatus.Team[gbPlayerNum].bLastID;
    }

    for (
      cnt = gTacticalStatus.Team[gbPlayerNum].bLastID, pSoldier = MercPtrs[cnt];
      cnt < TOTAL_SOLDIERS;
      cnt++, pSoldier = MercPtrs[cnt]
    ) {
  ...
  export function PlayerTeamFull(): boolean {
    ...
    if (
      CountNonVehiclesOnPlayerTeam() <=
      gTacticalStatus.Team[gbPlayerNum].bLastID - 2
    ) {
      return false;
    }
  ...
  export function NumEnemyInSector(): UINT8 {
    ...
    // Loop through all mercs and make go
    for (
      cnt = 0, pTeamSoldier = Menptr[cnt];
      cnt < TOTAL_SOLDIERS;
      cnt++, pTeamSoldier = Menptr[cnt]
    ) {
      if (
        pTeamSoldier.bActive &&
        pTeamSoldier.bInSector &&
    ...
    if (bNumDead + bNumNotOK == bNumInBattle || fOnlyEPCsLeft) {
      ...
      if (bNumDead != bNumInBattle) {
        ...
        for (
          ...
          if (pTeamSoldier.bActive && pTeamSoldier.bInSector) {
            if (
              (pTeamSoldier.bLife != 0 && pTeamSoldier.bLife < OKLIFE) ||
              AM_AN_EPC(pTeamSoldier) ||
              AM_A_ROBOT(pTeamSoldier)
            ) {
  ...
  function AttackOnGroupWitnessed(
    pSoldier: SOLDIERTYPE,
    pTarget: SOLDIERTYPE,
  ): boolean {
    let uiSlot: UINT32;
    ...
    for (uiSlot = 0; uiSlot < guiNumMercSlots; uiSlot++) {
      ...
      if (
        pGroupMember &&
        pGroupMember.ubCivilianGroup == pTarget.ubCivilianGroup &&
        pGroupMember != pTarget
      ) {
        if (
          pGroupMember.bOppList[pSoldier.ubID] == SEEN_CURRENTLY ||
          pGroupMember.bOppList[pTarget.ubID] == SEEN_CURRENTLY
        ) {
          return true;
        }
        if (
          SpacesAway(pGroupMember.sGridNo, pSoldier.sGridNo) < 12 ||
  ...
  function HandleSuppressionFire(
    ubTargetedMerc: UINT8,
    ubCausedAttacker: UINT8,
  ): void {
    let bTolerance: INT8;
    let sClosestOpponent: INT16;
    let sClosestOppLoc: INT16 = 0;
    let sClosestOppLoc__Pointer = createPointer(
      () => sClosestOppLoc,
      (v) => (sClosestOppLoc = v),
    );
    ...
    for (uiLoop = 0; uiLoop < guiNumMercSlots; uiLoop++) {
      ...
      if (
        ...
        ubPointsLost = Math.trunc(
          (Math.trunc((pSoldier.ubSuppressionPoints * 6) / (bTolerance + 6)) *
            2 +
            1) /
            2,
        );
        ...
        // morale modifier
        if (
          Math.trunc(ubTotalPointsLost / 2) >
          Math.trunc(pSoldier.ubAPsLostToSuppression / 2)
        ) {
          for (
            ubLoop2 = 0;
            ubLoop2 <
        ...
        if (
          pSoldier.ubSuppressionPoints >= Math.trunc(130 / (6 + bTolerance))
        ) {
          ...
          switch (gAnimControl[pSoldier.usAnimState].ubEndHeight) {
            ...
            case ANIM_CROUCH:
              if (
                ubTotalPointsLost >= AP_PRONE &&
                IsValidStance(pSoldier, ANIM_PRONE)
              ) {
                sClosestOpponent = ClosestKnownOpponent(
                  pSoldier,
                  sClosestOppLoc__Pointer,
                  null,
                );
                if (
            ...
            default: // standing!
              if (
                ...
                ubTotalPointsLost >= AP_CROUCH + AP_PRONE &&
        ...
        if (
          ...
          pSoldier.ubSuppressionPoints > 8 &&
          ...
          if (
            ...
            if (pSoldier.ubProfile != 53) {
              TacticalCharacterDialogue(
                pSoldier,
                Enum202.QUOTE_UNDER_HEAVY_FIRE,
              );
            }
          }
        }
  ...
  export function ProcessImplicationsOfPCAttack(
    pSoldier: SOLDIERTYPE,
    ppTarget: Pointer<SOLDIERTYPE>,
    bReason: INT8,
  ): boolean {
    ...
    if (
      pTarget.ubProfile == Enum268.PABLO &&
    ...
    if (gTacticalStatus.bBoxingState == Enum247.BOXING) {
      ...
      if (
        (pSoldier.usAttackingWeapon != NOTHING &&
          pSoldier.usAttackingWeapon != Enum225.BRASS_KNUCKLES) ||
    ...
    // JA2 Gold: fix Slay
      ...
      pTarget.bTeam == CIV_TEAM &&
      pTarget.bNeutral &&
      pTarget.ubProfile == Enum268.SLAY &&
      ...
      if (
        ...
        pTarget.ubProfile == Enum268.PABLO &&
        pTarget.bLife >= OKLIFE &&
        CheckFact(Enum170.FACT_PABLO_PUNISHED_BY_PLAYER, 0) &&
        !CheckFact(38, 0)
      ) {
        ...
        if (
          pTarget.ubProfile == NO_PROFILE ||
          !(
            gMercProfiles[pTarget.ubProfile].ubMiscFlags3 &
      ...
      if (pTarget.ubProfile == Enum268.CARMEN) {
        // Carmen
        // Special stuff for Carmen the bounty hunter
        if (pSoldier.ubProfile != Enum268.SLAY) {
          // attacked by someone other than Slay
          // change attitude
      ...
      if (
        pTarget.ubCivilianGroup &&
        (pTarget.bTeam == gbPlayerNum || pTarget.bNeutral)
      ) {
        ...
        pTarget.ubCivilianGroup != Enum246.NON_CIV_GROUP &&
        !(pTarget.uiStatusFlags & SOLDIER_BOXER)
      ) {
        ...
        if (
          ...
          if (Item[pTarget.inv[Enum261.HANDPOS].usItem].usItemClass == IC_GUN) {
            ...
            HandleItem(
              ...
              pSoldier.sGridNo,
              pSoldier.bLevel,
              pTarget.inv[Enum261.HANDPOS].usItem,
              false,
            );
          }
        }
  ...
  function InternalReduceAttackBusyCount(
    ubID: UINT8,
    fCalledByAttacker: boolean,
    ubTargetID: UINT8,
  ): SOLDIERTYPE | null {
    ...
    if (
      ...
      if (gTacticalStatus.ubAttackBusyCount > 0) {
        DebugMsg(
          ...
          DBG_LEVEL_3,
          FormatString(
            "!!!!!!! Starting suppression, attack count now %d",
            gTacticalStatus.ubAttackBusyCount,
          ),
        );
        return pTarget;
    ...
    if (pSoldier) {
      ...
      if (pTarget && pSoldier.uiStatusFlags & SOLDIER_ATTACK_NOTICED) {
        ...
        if (pTarget.ubBodyType != Enum194.CROW) {
          if (pSoldier.bTeam == gbPlayerNum) {
            fEnterCombat = ProcessImplicationsOfPCAttack(
              pSoldier,
              createPointer(
                () => <SOLDIERTYPE>pTarget,
                (v) => (pTarget = v),
            ...
            if (!fEnterCombat) {
              DebugMsg(
                TOPIC_JA2,
                DBG_LEVEL_3,
                ">>Not entering combat as a result of PC attack",
              );
            }
          }
        }
    ...
    if (gTacticalStatus.fKilledEnemyOnAttack) {
      ...
      HandleKilledQuote(
        MercPtrs[gTacticalStatus.ubEnemyKilledOnAttack],
    ...
    if (
      gTacticalStatus.fItemsSeenOnAttack &&
    ...
    if (gTacticalStatus.uiFlags & CHECK_SIGHT_AT_END_OF_ATTACK) {
      ...
      for (
        pSightSoldier = MercPtrs[ubLoop];
        ubLoop <= gTacticalStatus.Team[gbPlayerNum].bLastID;
        ubLoop++, pSightSoldier = MercPtrs[ubLoop]
      ) {
  ...
  export function ReduceAttackBusyCount(
    ubID: UINT8,
    fCalledByAttacker: boolean,
  ): SOLDIERTYPE | null {
    if (ubID == NOBODY) {
  ...
  export function FreeUpAttackerGivenTarget(
    ubID: UINT8,
    ubTargetID: UINT8,
  ): SOLDIERTYPE | null {
    // Strange as this may seem, this function returns a pointer to
  ...
  export function ReduceAttackBusyGivenTarget(
    ubID: UINT8,
    ubTargetID: UINT8,
  ): SOLDIERTYPE | null {
    // Strange as this may seem, this function returns a pointer to
  ...
  export function SetActionToDoOnceMercsGetToLocation(
    ubActionCode: UINT8,
    bNumMercsWaiting: INT8,
    uiData1: UINT32,
    uiData2: UINT32,
    uiData3: UINT32,
  ): void {
  ...
  export function RemoveSoldierFromTacticalSector(
    pSoldier: SOLDIERTYPE,
    fAdjustSelected: boolean,
  ): void {
    let ubID: UINT8;
  ...
  export function InitializeTacticalStatusAtBattleStart(): void {
    ...
    for (
      cnt = gTacticalStatus.Team[0].bFirstID;
      cnt <= gTacticalStatus.Team[0].bLastID;
      cnt++
    ) {
      pSoldier = MercPtrs[cnt];
      pSoldier.bShock = 0;
  ...
  function DeathTimerCallback(): void {
    if (
      gTacticalStatus.Team[CREATURE_TEAM].bMenInSector >
  ...
  export function HostileCiviliansPresent(): boolean {
    ...
    for (
      iLoop = gTacticalStatus.Team[CIV_TEAM].bFirstID;
      iLoop <= gTacticalStatus.Team[CIV_TEAM].bLastID;
      iLoop++
    ) {
      pSoldier = MercPtrs[iLoop];
    ...
    if (gubQuest[Enum169.QUEST_CREATURES] != QUESTDONE) {
      if (gTacticalStatus.uiFlags & IN_CREATURE_LAIR) {
        if (!(gTacticalStatus.uiFlags & INCOMBAT)) {
          ...
          if (
            uiTime - gTacticalStatus.uiCreatureTenseQuoteLastUpdate >
            gTacticalStatus.sCreatureTenseQuoteDelay * 1000
          ) {
  ...
  function DoCreatureTensionQuote(pSoldier: SOLDIERTYPE): void {
    ...
    switch (iRandomQuote) {
      ...
      case 1:
        ...
        if (
          !(
            pSoldier.usQuoteSaidFlags &
            SOLDIER_QUOTE_SAID_SPOTTING_CREATURE_ATTACK
          )
        ) {
          // set flag
          pSoldier.usQuoteSaidFlags |=
      ...
      case 2:
        ...
        if (
          !(
            pSoldier.usQuoteSaidFlags &
            SOLDIER_QUOTE_SAID_WORRIED_ABOUT_CREATURES
          )
        ) {
          // set flag
          pSoldier.usQuoteSaidFlags |=
