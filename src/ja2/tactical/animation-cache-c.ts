namespace ja2 {
  ...
  export function InitAnimationCache(
    usSoldierID: UINT16,
    pAnimCache: AnimationSurfaceCacheType,
  ): boolean {
    let cnt: UINT32;
    ...
    AnimDebugMsg(
      FormatString(
        "*** Initializing anim cache surface for soldier %d",
        usSoldierID,
      ),
    );
    ...
    AnimDebugMsg(
      FormatString(
        "*** Initializing anim cache hit counter for soldier %d",
        usSoldierID,
      ),
    );
  ...
  export function DeleteAnimationCache(
    usSoldierID: UINT16,
    pAnimCache: AnimationSurfaceCacheType,
  ): void {
    // Allocate entries
    if (pAnimCache.usCachedSurfaces != null) {
      AnimDebugMsg(
        FormatString(
          "*** Removing Anim Cache surface for soldier %d",
          usSoldierID,
        ),
      );
    ...
    if (pAnimCache.sCacheHits != null) {
      AnimDebugMsg(
        FormatString(
          "*** Removing Anim Cache hit counter for soldier %d",
          usSoldierID,
        ),
      );
  ...
  export function GetCachedAnimationSurface(
    usSoldierID: UINT16,
    pAnimCache: AnimationSurfaceCacheType,
    usSurfaceIndex: UINT16,
    usCurrentAnimation: UINT16,
  ): boolean {
    let cnt: UINT8;
    ...
    if (pAnimCache.ubCacheSize == guiCacheSize) {
      ...
      UnLoadAnimationSurface(
        usSoldierID,
        pAnimCache.usCachedSurfaces[ubLowestIndex],
      );
  ...
  export function UnLoadCachedAnimationSurfaces(
    usSoldierID: UINT16,
    pAnimCache: AnimationSurfaceCacheType,
  ): void {
    let cnt: UINT8;
