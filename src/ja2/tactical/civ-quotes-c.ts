namespace ja2 {
  ...
  let gCivQuotes: CIV_QUOTE[] /* [NUM_CIV_QUOTES] */ = createArrayFrom(
    Enum201.NUM_CIV_QUOTES,
    createCivQuote,
  );
  ...
  let gusCivQuoteBoxWidth__Pointer = createPointer(
    () => gusCivQuoteBoxWidth,
    (v) => (gusCivQuoteBoxWidth = v),
  );
  let gusCivQuoteBoxHeight: UINT16;
  let gusCivQuoteBoxHeight__Pointer = createPointer(
    () => gusCivQuoteBoxHeight,
    (v) => (gusCivQuoteBoxHeight = v),
  );
  ...
  function GetCivQuoteText(ubCivQuoteID: UINT8, ubEntryID: UINT8): string {
    ...
    if (ubCivQuoteID == CIV_QUOTE_HINT) {
      if (gbWorldSectorZ > 0) {
        ...
        zFileName = sprintf(
          "NPCDATA\\CIV%s.edt",
          Enum201.CIV_QUOTE_MINERS_NOT_FOR_PLAYER.toString().padStart(2, "0"),
        );
      } else {
        zFileName = sprintf(
          "NPCData\\%s%d.edt",
      ...
      zFileName = sprintf(
        "NPCDATA\\CIV%s.edt",
        ubCivQuoteID.toString().padStart(2, "0"),
      );
    }
  ...
  function ShutDownQuoteBox(fForce: boolean): void {
    ...
    if (GetJA2Clock() - gCivQuoteData.uiTimeOfCreation > 300 || fForce) {
      ...
      if (
        gCivQuoteData.pCiv &&
        gCivQuoteData.pCiv.bAction == Enum289.AI_ACTION_OFFER_SURRENDER
      ) {
        DoMessageBox(
  ...
  export function GetCivType(pCiv: SOLDIERTYPE): INT8 {
    ...
    if (gWorldSectorX == 10 && gWorldSectorY == 6 && gbWorldSectorZ == 0) {
      // 2 ) the only female....
      if (
        pCiv.ubCivilianGroup == 0 &&
        pCiv.bTeam != gbPlayerNum &&
        pCiv.ubBodyType == Enum194.REGFEMALE
      ) {
        // She's a ho!
  ...
  function RenderCivQuoteBoxOverlay(pBlitter: VIDEO_OVERLAY): void {
    if (gCivQuoteData.iVideoOverlay != -1) {
      ...
      InvalidateRegion(
        pBlitter.sX,
        pBlitter.sY,
        pBlitter.sX + gusCivQuoteBoxWidth,
        pBlitter.sY + gusCivQuoteBoxHeight,
      );
    }
  }

  function QuoteOverlayClickCallback(
  ...
  export function BeginCivQuote(
    ...
    gCivQuoteData.iDialogueBox = PrepareMercPopupBox(
      ...
      gzCivQuote,
      DIALOGUE_DEFAULT_WIDTH,
      0,
      0,
      0,
      gusCivQuoteBoxWidth__Pointer,
      gusCivQuoteBoxHeight__Pointer,
    );
  ...
  function DetermineCivQuoteEntry(
    pCiv: SOLDIERTYPE,
    pubCivHintToUse: Pointer<UINT8>,
    fCanUseHints: boolean,
  ): UINT8 {
    let ubCivType: UINT8;
    let bTownId: INT8;
    ...
    if (ubCivType == CIV_TYPE_ENEMY) {
      ...
      if (
        pCiv.bAction == Enum289.AI_ACTION_TOSS_PROJECTILE ||
        pCiv.bAction == Enum289.AI_ACTION_FIRE_GUN ||
        pCiv.bAction == Enum289.AI_ACTION_FIRE_GUN ||
        pCiv.bAction == Enum289.AI_ACTION_KNIFE_MOVE
      ) {
    ...
    if (
      pCiv.uiStatusFlags & SOLDIER_COWERING ||
    ...
    if (pCiv.ubCivilianGroup == Enum246.FACTORY_KIDS_GROUP) {
      // Check fact.....
      if (
        CheckFact(Enum170.FACT_DOREEN_HAD_CHANGE_OF_HEART, 0) ||
        !CheckFact(Enum170.FACT_DOREEN_ALIVE, 0)
      ) {
    ...
    if (fCanUseHints) {
      bCivHint = ConsiderCivilianQuotes(
        gWorldSectorX,
        gWorldSectorY,
        gbWorldSectorZ,
        false,
  ...
  export function HandleCivQuote(): void {
    if (gCivQuoteData.bActive) {
      // Check for min time....
      if (
        GetJA2Clock() - gCivQuoteData.uiTimeOfCreation >
        gCivQuoteData.uiDelayTime
      ) {
        // Stop!
        ShutDownQuoteBox(true);
      }
  ...
  export function StartCivQuote(pCiv: SOLDIERTYPE): void {
    ...
    let ubCivHintToUse__Pointer = createPointer(
      () => ubCivHintToUse,
      (v) => (ubCivHintToUse = v),
    );
    ...
    if (pCiv.bCurrentCivQuote == CIV_QUOTE_HINT) {
      ...
      ubCivQuoteID = DetermineCivQuoteEntry(
        pCiv,
        ubCivHintToUse__Pointer,
        false,
      );
    } else {
      // Determine which quote to say.....
    ...
    if (ubCivQuoteID != CIV_QUOTE_HINT) {
      if (pCiv.bCurrentCivQuote == -1) {
        // Pick random one
        pCiv.bCurrentCivQuote = Random(
          gCivQuotes[ubCivQuoteID].ubNumEntries - 2,
        );
        pCiv.bCurrentCivQuoteDelta = 0;
      }
