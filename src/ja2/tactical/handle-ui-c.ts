namespace ja2 {
  ...
  let gpRequesterTargetMerc: SOLDIERTYPE | null /* Pointer<SOLDIERTYPE> */ =
    null;
  ...
  export let guiCurrentEvent__Pointer = createPointer(
    () => guiCurrentEvent,
    (v) => (guiCurrentEvent = v),
  );
  ...
  export function HandleTacticalUI(): UINT32 {
    ...
    if (gfUIInterfaceSetBusy) {
      if (GetJA2Clock() - guiUIInterfaceBusyTime > 25000) {
        ...
        DebugMsg(
          TOPIC_JA2,
          DBG_LEVEL_3,
          FormatString("Setting attack busy count to 0 due to ending AI lock"),
        );
    ...
    if (
      ...
      if (
        FindSoldier(
          ...
          createPointer(
            () => gusUIFullTargetID,
            (v) => (gusUIFullTargetID = v),
          ),
          createPointer(
            () => guiUIFullTargetFlags,
            (v) => (guiUIFullTargetFlags = v),
          ),
      ...
      if (
        FindSoldier(
          usMapPos,
          createPointer(
            () => gusUISelectiveTargetID,
            (v) => (gusUISelectiveTargetID = v),
          ),
          createPointer(
            () => guiUISelectiveTargetFlags,
            (v) => (guiUISelectiveTargetFlags = v),
          ),
    ...
    if (
      gEvents[uiNewEvent].uiFlags & UIEVENT_SNAPMOUSE &&
      gEvents[uiNewEvent].fFirstTime
    ) {
      // Save mouse position
    ...
    if (
      gEvents[uiNewEvent].ChangeToUIMode != gCurrentUIMode &&
      gEvents[uiNewEvent].ChangeToUIMode != Enum206.DONT_CHANGEMODE
    ) {
    ...
    if (gEvents[uiNewEvent].fDoneMenu == true) {
      if (
        gCurrentUIMode == Enum206.MENU_MODE ||
        gCurrentUIMode == Enum206.POPUP_MODE ||
        gCurrentUIMode == Enum206.LOOKCURSOR_MODE
      ) {
    ...
    if (gTacticalStatus.fAtLeastOneGuyOnMultiSelect) {
      ...
      if (
        ...
        gCurrentUIMode != Enum206.RUBBERBAND_MODE &&
        gCurrentUIMode != Enum206.ADJUST_STANCE_MODE &&
        gCurrentUIMode != Enum206.TALKCURSOR_MODE &&
        gCurrentUIMode != Enum206.LOOKCURSOR_MODE
      ) {
        ResetMultiSelection();
  ...
  function SetUIMouseCursor(): void {
    ...
    let uiTraverseTimeInMinutes__Pointer = createPointer(
    ...
    if (gCurrentUIMode == Enum206.MOVE_MODE) {
      ...
      if (gfUIShowExitEast) {
        ...
        if (
          OKForSectorExit(
            Enum186.EAST_STRATEGIC_MOVE,
            0,
            uiTraverseTimeInMinutes__Pointer,
          )
        ) {
          if (gfUIConfirmExitArrows) {
        ...
        if (gusMouseYPos < 478) {
          ...
          if (gfScrollPending || gfScrollInertia) {
            ...
            SetCurrentCursorFromDatabase(
              gUICursors[guiNewUICursor].usFreeCursorName,
            );
  ...
  export function SetUIKeyboardHook(
    KeyboardHookFnc: UIKEYBOARD_HOOK | null,
  ): void {
    ...
    if (
      ...
      if (bReturnCode == MERC_HIRE_FAILED) {
        ScreenMsg(
          FONT_ORANGE,
          MSG_BETAVERSION,
          "Merc hire failed:  Either already hired or dislikes you.",
        );
      } else if (bReturnCode == MERC_HIRE_OVER_20_MERCS_HIRED) {
        ScreenMsg(
          FONT_ORANGE,
          MSG_BETAVERSION,
          "Can't hire more than 20 mercs.",
        );
      } else {
        // Get soldier from profile
        pSoldier = <SOLDIERTYPE>(
          FindSoldierByProfileID(UIHandleNewMerc__ubTemp, false)
        );
  ...
  function UIHandleNewBadMerc(pUIEvent: UI_EVENT): UINT32 {
    ...
    if (
      ...
      usRandom = Random(10);
      if (usRandom < 4) pSoldier = TacticalCreateAdministrator();
      else if (usRandom < 8) pSoldier = TacticalCreateArmyTroop();
      else pSoldier = TacticalCreateEliteEnemy();
  ...
  function UIHandleMOnTerrain(pUIEvent: UI_EVENT | null): UINT32 {
    ...
    if (!UIHandleOnMerc(true)) {
      // Are we over items...
      if (
        (pItemPool = GetItemPool(usMapPos, gsInterfaceLevel)) &&
        ...
        if (UIHandleMOnTerrain__fOverItems) {
          ...
          if (
            gsInterfaceLevel == UIHandleMOnTerrain__bLevelForItemsOver &&
            usMapPos == UIHandleMOnTerrain__sGridNoForItemsOver
          ) {
            // Check timer...
      ...
      if (gusSelectedSoldier != NO_SOLDIER) {
        ...
        pIntNode = GetCurInteractiveTileGridNo(
          createPointer(
            () => sIntTileGridNo,
            (v) => (sIntTileGridNo = v),
          ),
        ...
        if (pSoldier.bLife < OKLIFE) {
          ...
          UIOKMoveDestination(pSoldier, usMapPos) != 1 &&
          pIntNode == null
        ) {
          // ERASE PATH
          ErasePath(true);
          ...
          if (
            !UIHandleInteractiveTilesAndItemsOnTerrain(
              pSoldier,
              usMapPos,
              false,
              true,
            ...
            if (
              ...
              // If so, draw path, etc
              fSetCursor = HandleUIMovementCursor(
                pSoldier,
                uiCursorFlags,
                usMapPos,
                0,
  ...
  function UIHandleAOnTerrain(pUIEvent: UI_EVENT): UINT32 {
    ...
    if ((pSoldier = GetSoldier(gusSelectedSoldier)) !== null) {
      ...
      guiNewUICursor = GetProperItemCursor(
        gusSelectedSoldier,
          ...
          if (
            ...
            if (
              ...
              InternalDoMercBattleSound(
                pSoldier,
                Enum259.BATTLE_SOUND_OK1,
                BATTLE_SND_LOWER_VOLUME,
              );
            } else {
              ScreenMsg(
                FONT_MCOLOR_LTYELLOW,
  ...
  function UIHandleMAdjustStanceMode(pUIEvent: UI_EVENT): UINT32 {
    ...
    let bNewDirection__Pointer = createPointer(
      () => bNewDirection,
      (v) => (bNewDirection = v),
    );
...

    {
      if (UIHandleMAdjustStanceMode__gusAnchorMouseY > gusMouseYPos) {
        ...
        if ((pSoldier = GetSoldier(gusSelectedSoldier)) !== null) {
          if (
            iPosDiff < GO_MOVE_ONE &&
            UIHandleMAdjustStanceMode__ubUpHeight >= 1
          ) {
            // Change arrows to move down arrow + show
            // guiShowUPDownArrows = ARROWS_SHOW_UP_ABOVE_Y;
            guiShowUPDownArrows =
              ARROWS_SHOW_DOWN_BESIDE | ARROWS_SHOW_UP_BESIDE;
            gbAdjustStanceDiff = 0;
            gbClimbID = 0;
          } else if (
            iPosDiff > GO_MOVE_ONE &&
            iPosDiff < GO_MOVE_TWO &&
            UIHandleMAdjustStanceMode__ubUpHeight >= 1
          ) {
            // guiShowUPDownArrows = ARROWS_SHOW_UP_ABOVE_G;
            if (
              UIHandleMAdjustStanceMode__ubUpHeight == 1 &&
              UIHandleMAdjustStanceMode__ubNearHeigherLevel
            ) {
      ...
      if (UIHandleMAdjustStanceMode__gusAnchorMouseY < gusMouseYPos) {
        ...
        if ((pSoldier = GetSoldier(gusSelectedSoldier)) !== null) {
          if (
            ...
            if (
              UIHandleMAdjustStanceMode__ubDownDepth == 1 &&
              UIHandleMAdjustStanceMode__ubNearLowerLevel
            ) {
    ...
    if (iHandleReturn < 0) {
      ...
      if (iHandleReturn == ITEM_HANDLE_NOROOM) {
        ScreenMsg(
          FONT_MCOLOR_LTYELLOW,
          MSG_UI_FEEDBACK,
          pMessageStrings[Enum333.MSG_CANT_FIRE_HERE],
        );
        return;
      }
    }
  ...
  function AttackRequesterCallback(bExitValue: UINT8): void {
    if (bExitValue == MSG_BOX_RETURN_YES) {
      gTacticalStatus.ubLastRequesterTargetID = (<SOLDIERTYPE>(
        gpRequesterTargetMerc
      )).ubProfile;

      UIHandleMercAttack(
        <SOLDIERTYPE>gpRequesterMerc,
        gpRequesterTargetMerc,
        gsRequesterGridNo,
      );
    }
    ...
    if (gusSelectedSoldier != NO_SOLDIER) {
      ...
      if ((pSoldier = GetSoldier(gusSelectedSoldier)) !== null) {
        ...
        if (pTSoldier != null) {
          ...
          if (
            (pTSoldier.bTeam == gbPlayerNum ||
              pTSoldier.bTeam == MILITIA_TEAM) &&
            Item[pSoldier.inv[Enum261.HANDPOS].usItem].usItemClass !=
              IC_MEDKIT &&
            pSoldier.inv[Enum261.HANDPOS].usItem != Enum225.GAS_CAN &&
            ...
            zStr = swprintf(
              TacticalStr[Enum335.ATTACK_OWN_GUY_PROMPT],
              pTSoldier.name,
            );
  ...
  function UIHandlePADJAdjustStance(pUIEvent: UI_EVENT): UINT32 {
    ...
    if (gusSelectedSoldier != NO_SOLDIER && gbAdjustStanceDiff != 0) {
      // Get soldier
      if ((pSoldier = GetSoldier(gusSelectedSoldier)) !== null) {
        ubNewStance = GetAdjustedAnimHeight(
          gAnimControl[pSoldier.usAnimState].ubEndHeight,
          gbAdjustStanceDiff,
        );
      ...
      if ((pSoldier = GetSoldier(gusSelectedSoldier)) !== null) {
        ...
        if (ubItemCursor == BOMBCURS) {
          ...
          if (
            EnoughPoints(
              pSoldier,
              GetTotalAPsToDropBomb(pSoldier, usMapPos),
              0,
              true,
            )
          ) {
  ...
  export function SelectedMercCanAffordMove(): boolean {
    ...
    if ((pSoldier = GetSoldier(gusSelectedSoldier)) !== null) {
      ...
      sAPCost += GetAPsToChangeStance(
        pSoldier,
        gAnimControl[pSoldier.usUIMovementMode].ubHeight,
      );
  ...
  export function GetMercClimbDirection(ubSoldierID: UINT8): {
    fNearLowerLevel: boolean;
    fNearHeigherLevel: boolean;
  } {
  ...
  export function ToggleLookCursorMode(
    puiNewEvent: Pointer<UINT32> | null,
  ): void {
    // Toggle modes
  ...
  export function UIHandleOnMerc(fMovementMode: boolean): boolean {
    ...
    if (fFoundMerc) {
      ...
      if (uiMercFlags & OWNED_MERC) {
        ...
        (uiMercFlags & ENEMY_MERC || uiMercFlags & NEUTRAL_MERC) &&
      ...
      if (pSoldier.bCollapsed && pSoldier.bBreath < OKBREATH) {
        ...
        if (pSoldier.uiStatusFlags & SOLDIER_VEHICLE) {
          ScreenMsg(
            FONT_MCOLOR_LTYELLOW,
            MSG_UI_FEEDBACK,
            TacticalStr[Enum335.VEHICLES_NO_STANCE_CHANGE_STR],
          );
        } else if (pSoldier.uiStatusFlags & SOLDIER_ROBOT) {
          ScreenMsg(
            FONT_MCOLOR_LTYELLOW,
            MSG_UI_FEEDBACK,
            TacticalStr[Enum335.ROBOT_NO_STANCE_CHANGE_STR],
          );
        } else {
          if (pSoldier.bCollapsed) {
            ScreenMsg(
              FONT_MCOLOR_LTYELLOW,
              MSG_UI_FEEDBACK,
              pMessageStrings[Enum333.MSG_CANT_CHANGE_STANCE],
              pSoldier.name,
            );
          } else {
            ScreenMsg(
              FONT_MCOLOR_LTYELLOW,
              MSG_UI_FEEDBACK,
              TacticalStr[Enum335.CANNOT_STANCE_CHANGE_STR],
              pSoldier.name,
            );
          }
        }
  ...
  export function GetCursorMovementFlags(): UINT32 {
    ...
    if (
      gusMouseXPos != GetCursorMovementFlags__usOldMouseXPos ||
      gusMouseYPos != GetCursorMovementFlags__usOldMouseYPos
    ) {
      uiCursorFlags |= MOUSE_MOVING;

      // IF CURSOR WAS PREVIOUSLY STATIONARY, MAKE THE ADDITIONAL CHECK OF GRID POS CHANGE
      if (
        GetCursorMovementFlags__fStationary &&
  ...
  export function HandleUIMovementCursor(
    pSoldier: SOLDIERTYPE,
    ...
    if (
      ...
      if (
        uiFlags == MOVEUI_TARGET_MERCS ||
        uiFlags == MOVEUI_TARGET_MERCSFORAID
      ) {
        if (
          gfUIFullTargetFound != HandleUIMovementCursor__fTargetFound ||
  ...
  function DrawUIMovementPath(
    ...
    let sActionGridNo: INT16;
    let pStructure: STRUCTURE | null = <STRUCTURE>(<unknown>null);
    let pStructure__Pointer = createPointer(
      () => pStructure,
      (v) => (pStructure = v),
    );
    ...
    let sIntTileGridNo: INT16 = 0;
    let sIntTileGridNo__Pointer = createPointer(
      () => sIntTileGridNo,
      (v) => (sIntTileGridNo = v),
    );
    let pIntTile: LEVELNODE | null;
    ...
    let ubMercID__Pointer = createPointer(
      () => ubMercID,
      (v) => (ubMercID = v),
    );
    ...
    if (uiFlags == MOVEUI_TARGET_INTTILES) {
      ...
      // For repair, check if we are over a vehicle, then get gridnot to edge of that vehicle!
      if (
        IsRefuelableStructAtGridNo(usMapPos, ubMercID__Pointer) ==
        <boolean>(<unknown>2)
      ) {
        let sNewGridNo: INT16;
        let ubDirection: UINT8;
      ...
      if (gfUIFullTargetFound) {
        ...
        for (cnt = 0; cnt < Enum245.NUM_WORLD_DIRECTIONS; cnt++) {
          ...
          if (
            gubWorldMovementCosts[sSpot][cnt][gsInterfaceLevel] >=
            TRAVELCOST_BLOCKED
          ) {
            continue;
          }
  ...
  export function UIMouseOnValidAttackLocation(pSoldier: SOLDIERTYPE): boolean {
    ...
    if (gfUIFullTargetFound && ubItemCursor != KNIFECURS) {
      ...
      if (
        ...
        Item[usInHand].usItemClass != IC_MEDKIT
      ) {
        return false;
      }
    }
    ...
    if (Item[usInHand].usItemClass == IC_MEDKIT) {
      ...
      if (pTSoldier.uiStatusFlags & (SOLDIER_VEHICLE | SOLDIER_ROBOT)) {
        ScreenMsg(
          FONT_MCOLOR_LTYELLOW,
          MSG_UI_FEEDBACK,
          TacticalStr[Enum335.CANNOT_DO_FIRST_AID_STR],
          pTSoldier.name,
        );
        return false;
      }
      ...
      if (pSoldier.bMedical == 0) {
        ScreenMsg(
          FONT_MCOLOR_LTYELLOW,
          MSG_UI_FEEDBACK,
          pMessageStrings[Enum333.MSG_MERC_HAS_NO_MEDSKILL],
          pSoldier.name,
        );
        return false;
      }
      ...
      if (pTSoldier.bBleeding == 0 && pTSoldier.bLife >= OKLIFE) {
        ScreenMsg(
          FONT_MCOLOR_LTYELLOW,
          MSG_UI_FEEDBACK,
          TacticalStr[Enum335.CANNOT_NO_NEED_FIRST_AID_STR],
          pTSoldier.name,
        );
        return false;
      }
  ...
  export function UIOkForItemPickup(
    pSoldier: SOLDIERTYPE,
    sGridNo: INT16,
  ): boolean {
    let sAPCost: INT16;
  ...
  function SoldierCanAffordNewStance(
    pSoldier: SOLDIERTYPE,
  ...
  function SetConfirmMovementModeCursor(
    pSoldier: SOLDIERTYPE,
    fFromMove: boolean,
  ): void {
    if (
  ...
  function GetGridNoScreenXY(
    sGridNo: INT16,
    pScreenX: Pointer<INT16>,
    pScreenY: Pointer<INT16>,
  ): void {
    let sScreenX: INT16;
    let sScreenY: INT16;
    let sOffsetX: INT16;
    ...
    for (
      ...
      if (pSoldier.bActive && pSoldier.bInSector) {
        if (pSoldier.uiStatusFlags & SOLDIER_MULTI_SELECTED) {
          ...
          if (
            !gGameSettings.fOptions[Enum8.TOPTION_MUTE_CONFIRMATIONS] &&
            fAcknowledge
          )
            InternalDoMercBattleSound(
              pSoldier,
  ...
  function UIHandleRubberBandOnTerrain(pUIEvent: UI_EVENT): UINT32 {
    ...
    let sScreenX__Pointer = createPointer(
      () => sScreenX,
      (v) => (sScreenX = v),
    );
    let sScreenY: INT16 = 0;
    let sScreenY__Pointer = createPointer(
      () => sScreenY,
      (v) => (sScreenY = v),
    );
    let iTemp: INT32;
  ...
  export function IsValidTalkableNPC(
    ubSoldierID: UINT8,
    fGive: boolean,
    fAllowMercs: boolean,
    fCheckCollapsed: boolean,
    ...
    if (
      pSoldier.ubProfile != NO_PROFILE &&
      (gCurrentUIMode == Enum206.TALKCURSOR_MODE || fGive) &&
      AM_AN_EPC(pSoldier)
    ) {
      fValidGuy = true;
    }
    ...
    if (gfUIFullTargetFound) {
      ...
      if (IsValidTalkableNPC(gusUIFullTargetID, false, true, false)) {
          ...
          if (
            ...
            if (pTSoldier.ubProfile != NO_PROFILE) {
              ScreenMsg(
                FONT_MCOLOR_LTYELLOW,
                MSG_UI_FEEDBACK,
                TacticalStr[Enum335.NO_LOS_TO_TALK_TARGET],
                pSoldier.name,
                pTSoldier.name,
              );
            } else {
        ...
        if (pTSoldier.bTeam == gbPlayerNum && !AM_AN_EPC(pTSoldier)) {
          ...
          if (
            gMercProfiles[pTSoldier.ubProfile].bAttitude != Enum271.ATT_NORMAL
          ) {
            ubDiceRoll = Random(3);
          } else {
            ubDiceRoll = Random(2);
          ...
          if (
            pTSoldier.ubWhatKindOfMercAmI == Enum260.MERC_TYPE__PLAYER_CHARACTER
          ) {
            ubDiceRoll = 0;
          }
  ...
  export function BeginDisplayTimedCursor(
    uiCursorID: UINT32,
    uiDelay: UINT32,
  ): void {
  ...
  /* static */ let UIHandleInteractiveTilesAndItemsOnTerrain__fOverPool: boolean =
    false;
  /* static */ let UIHandleInteractiveTilesAndItemsOnTerrain__fOverEnemy: boolean =
    false;
  function UIHandleInteractiveTilesAndItemsOnTerrain(
    pSoldier: SOLDIERTYPE,
    usMapPos: INT16,
    fUseOKCursor: boolean,
    fItemsOnlyIfOnIntTiles: boolean,
  ): INT8 {
    let pItemPool: ITEM_POOL | null;
    let fSetCursor: boolean;
    let uiCursorFlags: UINT32;
  ...
  export function SelectedGuyInBusyAnimation(): boolean {
    ...
    if (gusSelectedSoldier != NOBODY) {
      ...
      if (
        pSoldier.usAnimState == Enum193.LOB_ITEM ||
        pSoldier.usAnimState == Enum193.THROW_ITEM ||
        pSoldier.usAnimState == Enum193.PICKUP_ITEM ||
        pSoldier.usAnimState == Enum193.DROP_ITEM ||
        pSoldier.usAnimState == Enum193.OPEN_DOOR ||
        pSoldier.usAnimState == Enum193.OPEN_STRUCT ||
        pSoldier.usAnimState == Enum193.OPEN_STRUCT ||
        pSoldier.usAnimState == Enum193.END_OPEN_DOOR ||
        pSoldier.usAnimState == Enum193.END_OPEN_LOCKED_DOOR ||
        pSoldier.usAnimState == Enum193.ADJACENT_GET_ITEM ||
        pSoldier.usAnimState == Enum193.DROP_ADJACENT_OBJECT ||
        pSoldier.usAnimState == Enum193.OPEN_DOOR_CROUCHED ||
        pSoldier.usAnimState == Enum193.BEGIN_OPENSTRUCT_CROUCHED ||
        pSoldier.usAnimState == Enum193.CLOSE_DOOR_CROUCHED ||
        pSoldier.usAnimState == Enum193.OPEN_DOOR_CROUCHED ||
        ...
        pSoldier.usAnimState == Enum193.END_OPEN_LOCKED_DOOR_CROUCHED ||
        pSoldier.usAnimState == Enum193.END_OPENSTRUCT_LOCKED_CROUCHED ||
  ...
  export function ValidQuickExchangePosition(): boolean {
    ...
    if (gfUIFullTargetFound) {
      ...
      if (
        (pOverSoldier.bTeam != gbPlayerNum && pOverSoldier.bNeutral) ||
  ...
  // to jump over people.
  export function IsValidJumpLocation(
    pSoldier: SOLDIERTYPE,
    sGridNo: INT16,
    fCheckForPath: boolean,
  ): boolean {
    ...
    let iDoorGridNo__Pointer = createPointer(
      () => iDoorGridNo,
      (v) => (iDoorGridNo = v),
    );
    ...
    for (cnt = 0; cnt < 4; cnt++) {
      ...
      ubMovementCost =
        gubWorldMovementCosts[sIntSpot][sDirs[cnt]][pSoldier.bLevel];
        ...
        if (NewOKDestination(pSoldier, sGridNo, true, gsInterfaceLevel)) {
          ...
          if (!pSoldier.fTurningUntilDone) {
            ...
            if (
              ...
              gAnimControl[MercPtrs[ubGuyThere].usAnimState].ubHeight ==
                ANIM_PRONE
            ) {
              // It's a GO!
              return true;
