namespace ja2 {
  ...
  function readTempNPCQuoteInfoSave(
    o: TempNPCQuoteInfoSave,
    buffer: Buffer,
    offset: number = 0,
  ): number {
    o.usFlags = buffer.readUInt16LE(offset);
    offset += 2;
    o.sRequiredItem = buffer.readInt16LE(offset);
    offset += 2;
    o.usGoToGridno = buffer.readUInt16LE(offset);
    offset += 2;
    return offset;
  }

  function writeTempNPCQuoteInfoSave(
    o: TempNPCQuoteInfoSave,
    buffer: Buffer,
    offset: number = 0,
  ): number {
  ...
  export function SaveMapTempFilesToSavedGameFile(hFile: HWFILE): boolean {
    ...
    for (sMapY = 1; sMapY <= 16; sMapY++) {
      for (sMapX = 1; sMapX <= 16; sMapX++) {
        ...
        if (
          SectorInfo[SECTOR(sMapX, sMapY)].uiFlags & SF_ITEM_TEMP_FILE_EXISTS
        ) {
          AddTempFileToSavedGame(
            hFile,
            SF_ITEM_TEMP_FILE_EXISTS,
            sMapX,
            sMapY,
            0,
          );
        ...
        if (
          SectorInfo[SECTOR(sMapX, sMapY)].uiFlags &
          SF_ROTTING_CORPSE_TEMP_FILE_EXISTS
        ) {
          AddTempFileToSavedGame(
            hFile,
            SF_ROTTING_CORPSE_TEMP_FILE_EXISTS,
        ...
        if (
          SectorInfo[SECTOR(sMapX, sMapY)].uiFlags &
          SF_DOOR_STATUS_TEMP_FILE_EXISTS
        ) {
          AddTempFileToSavedGame(
            hFile,
            SF_DOOR_STATUS_TEMP_FILE_EXISTS,
        ...
        if (
          SectorInfo[SECTOR(sMapX, sMapY)].uiFlags &
          SF_SMOKE_EFFECTS_TEMP_FILE_EXISTS
        ) {
          AddTempFileToSavedGame(
            hFile,
            SF_SMOKE_EFFECTS_TEMP_FILE_EXISTS,
  ...
  export function SaveWorldItemsToTempItemFile(
    sMapX: INT16,
    sMapY: INT16,
    bMapZ: INT8,
    uiNumberOfItems: UINT32,
    pData: WORLDITEM[] | null,
  ): boolean {
    let hFile: HWFILE;
    ...
    if (uiNumberOfItems != 0) {
      ...
      uiNumBytesWritten = FileWrite(
        hFile,
        buffer,
        uiNumberOfItems * WORLD_ITEM_SIZE,
      );
  ...
  export function LoadWorldItemsFromTempItemFile(
    sMapX: INT16,
    sMapY: INT16,
    bMapZ: INT8,
    pData: WORLDITEM[],
  ...
  export function GetNumberOfWorldItemsFromTempItemFile(
    sMapX: INT16,
    sMapY: INT16,
    bMapZ: INT8,
    fIfEmptyCreate: boolean,
  ): UINT32 {
    ...
    if (!FileExistsNoDB(zMapName)) {
      if (fIfEmptyCreate) {
        let TempWorldItems: WORLDITEM[] /* [10] */ = createArrayFrom(
          10,
          createWorldItem,
        );
  ...
  export function AddItemsToUnLoadedSector(
    sMapX: INT16,
    sMapY: INT16,
    bMapZ: INT8,
    sGridNo: INT16,
    uiNumberOfItemsToAdd: UINT32,
    pObject: OBJECTTYPE[],
    ubLevel: UINT8,
    usFlags: UINT16,
    bRenderZHeightAboveLevel: INT8,
    bVisible: INT8,
    fReplaceEntireFile: boolean,
  ): boolean {
    let uiNumberOfItems: UINT32 = 0;
    ...
    for (uiLoop1 = 0; uiLoop1 < uiNumberOfItemsToAdd; uiLoop1++) {
      ...
      if (
        ...
        ScreenMsg(
          ...
          MSG_BETAVERSION,
          "Error: Trying to add item ( %d: %s ) to invalid gridno in unloaded sector. Please Report.",
          pWorldItems[cnt].o.usItem,
          ItemNames[pWorldItems[cnt].o.usItem],
        );
      }
  ...
  export function SaveCurrentSectorsInformationToTempItemFile(): boolean {
    ...
    HandleAllReachAbleItemsInTheSector(
      gWorldSectorX,
      gWorldSectorY,
      gbWorldSectorZ,
    );
    ...
    if (
      ...
      DebugMsg(
        ...
        DBG_LEVEL_3,
        FormatString(
          "SaveCurrentSectorsInformationToTempItemFile:  failed in SaveRevealedStatusArrayToRevealedTempFile()",
        ),
      );
      return false;
    }
    ...
    if (
      ...
      DebugMsg(
        ...
        DBG_LEVEL_3,
        FormatString(
          "SaveCurrentSectorsInformationToTempItemFile:  failed in SaveDoorStatusArrayToDoorStatusTempFile()",
        ),
      );
      return false;
    }
    ...
    if (
      !NewWayOfSavingEnemyAndCivliansToTempFile(
        gWorldSectorX,
        gWorldSectorY,
        gbWorldSectorZ,
        true,
      ...
      DebugMsg(
        ...
        DBG_LEVEL_3,
        FormatString(
          "SaveCurrentSectorsInformationToTempItemFile:  failed in NewWayOfSavingEnemyAndCivliansToTempFile( Enemy, Creature Team )",
        ),
      );
      return false;
    }
    ...
    if (
      ...
      DebugMsg(
        ...
        DBG_LEVEL_3,
        FormatString(
          "SaveCurrentSectorsInformationToTempItemFile:  failed in NewWayOfSavingEnemyAndCivliansToTempFile( Civ Team )",
        ),
      );
      return false;
    }
    ...
    if (
      ...
      DebugMsg(
        ...
        DBG_LEVEL_3,
        FormatString(
          "SaveCurrentSectorsInformationToTempItemFile:  failed in SaveSmokeEffectsToMapTempFile",
        ),
      );
      return false;
    }
    ...
    if (
      ...
      DebugMsg(
        ...
        DBG_LEVEL_3,
        FormatString(
          "SaveCurrentSectorsInformationToTempItemFile:  failed in SaveLightEffectsToMapTempFile",
        ),
      );
      return false;
    }
  ...
  export function HandleAllReachAbleItemsInTheSector(
    sSectorX: INT16,
    sSectorY: INT16,
    bSectorZ: INT8,
  ): void {
    ...
    if (gMapInformation.sCenterGridNo != -1) {
      ...
      AssertMsg(
        false,
        FormatString(
          "Map %s%d_b%d does not have any entry points!",
          String.fromCharCode("A".charCodeAt(0) + gWorldSectorY - 1),
          gWorldSectorX,
          gbWorldSectorZ,
        ),
      );
    ...
    if (gMapInformation.sIsolatedGridNo != -1) {
      ...
      for (
        uiCounter = gTacticalStatus.Team[gbPlayerNum].bFirstID;
        uiCounter < gTacticalStatus.Team[gbPlayerNum].bLastID;
        uiCounter++
      ) {
    ...
    for (uiCounter = 0; uiCounter < guiNumWorldItems; uiCounter++) {
      ...
      if (gWorldItems[uiCounter].o.bTrap > 0) {
        ...
        ItemTypeExistsAtLocation(
          gWorldItems[uiCounter].sGridNo,
          Enum225.OWNERSHIP,
          gWorldItems[uiCounter].ubLevel,
          null,
        )
      ) {
        fReachable = false;
        ...
        gpWorldLevelData[gWorldItems[uiCounter].sGridNo].uiFlags &
        MAPELEMENT_REACHABLE
      ) {
        ...
        for (ubDir = 0; ubDir < Enum245.NUM_WORLD_DIRECTIONS; ubDir += 2) {
          ...
          if (sNewLoc != gWorldItems[uiCounter].sGridNo) {
            ...
            if (gpWorldLevelData[sNewLoc].uiFlags & MAPELEMENT_REACHABLE) {
              ubMovementCost =
                gubWorldMovementCosts[gWorldItems[uiCounter].sGridNo][
                  gOppositeDirection[ubDir]
                ][0];
  ...
  export function LoadCurrentSectorsInformationFromTempItemsFile(): boolean {
    ...
    if (AreInMeanwhile()) {
      ...
      if (GetMeanwhileID() == Enum160.INTERROGATION) {
        ...
        if (
          ...
          if (
            !LoadAndAddWorldItemsFromTempFile(
              gWorldSectorX,
              gWorldSectorY,
              gbWorldSectorZ,
            )
    ...
    if (
      ...
      fUsedTempFile = true;
      if (
        !LoadSmokeEffectsFromMapTempFile(
          gWorldSectorX,
          gWorldSectorY,
          gbWorldSectorZ,
        )
  ...
  function SetLastTimePlayerWasInSector(): void {
    if (!gbWorldSectorZ)
      ...
      let pTempNode: UNDERGROUND_SECTORINFO | null =
      ...
      while (pTempNode) {
        if (
          pTempNode.ubSectorX == gWorldSectorX &&
  ...
  function GetLastTimePlayerWasInSector(): UINT32 {
    if (!gbWorldSectorZ)
      return SectorInfo[SECTOR(gWorldSectorX, gWorldSectorY)]
  ...
  function LoadAndAddWorldItemsFromTempFile(
    sMapX: INT16,
    sMapY: INT16,
    bMapZ: INT8,
  ): boolean {
    ...
    for (cnt = 0; cnt < uiNumberOfItems; cnt++) {
      ...
      if (pWorldItems[cnt].fExists) {
        ...
        if (pWorldItems[cnt].usFlags & WOLRD_ITEM_FIND_SWEETSPOT_FROM_GRIDNO) {
          sNewGridNo = FindNearestAvailableGridNoForItem(
            pWorldItems[cnt].sGridNo,
            5,
          );
        ...
        AddItemToPool(
          pWorldItems[cnt].sGridNo,
          pWorldItems[cnt].o,
          pWorldItems[cnt].bVisible,
          pWorldItems[cnt].ubLevel,
          pWorldItems[cnt].usFlags,
  ...
  function AddTempFileToSavedGame(
    hFile: HWFILE,
    uiType: UINT32,
    sMapX: INT16,
    sMapY: INT16,
  ...
  function RetrieveTempFileFromSavedGame(
    hFile: HWFILE,
    uiType: UINT32,
    sMapX: INT16,
    sMapY: INT16,
  ...
  function SaveRottingCorpsesToTempCorpseFile(
    sMapX: INT16,
    sMapY: INT16,
    bMapZ: INT8,
  ): boolean {
  ...
  */
    ...
    for (iCount = 0; iCount < giNumRottingCorpse; iCount++) {
      if (gRottingCorpse[iCount].fActivated == true) {
        ...
        uiNumBytesWritten = FileWrite(
          hFile,
          buffer,
          ROTTING_CORPSE_DEFINITION_SIZE,
        );
  ...
  function DeleteTempItemMapFile(
    sMapX: INT16,
    sMapY: INT16,
    bMapZ: INT8,
  ): boolean {
  ...
  */
  ...
  function LoadRottingCorpsesFromTempCorpseFile(
    sMapX: INT16,
    sMapY: INT16,
    bMapZ: INT8,
  ): boolean {
  ...
  */
      ...
      if (!fDontAddCorpse) {
        ...
        if (AddRottingCorpse(def) == -1) {
          DebugMsg(
            TOPIC_JA2,
            DBG_LEVEL_3,
            FormatString("Failed to add a corpse to GridNo # %d", def.sGridNo),
          );
  ...
  export function AddWorldItemsToUnLoadedSector(
    sMapX: INT16,
    sMapY: INT16,
    bMapZ: INT8,
    sGridNo: INT16,
    uiNumberOfItems: UINT32,
    pWorldItem: WORLDITEM[],
    fOverWrite: boolean,
  ): boolean {
    let uiLoop: UINT32;
    ...
    for (uiLoop = 0; uiLoop < uiNumberOfItems; uiLoop++) {
      ...
      if (pWorldItem[uiLoop].fExists) {
        AddItemsToUnLoadedSector(
          ...
          sMapY,
          bMapZ,
          pWorldItem[uiLoop].sGridNo,
          1,
          [pWorldItem[uiLoop].o],
          pWorldItem[uiLoop].ubLevel,
          pWorldItem[uiLoop].usFlags,
          pWorldItem[uiLoop].bRenderZHeightAboveLevel,
          pWorldItem[uiLoop].bVisible,
          fLoop,
        );
  ...
  function LoadNPCInformationFromProfileStruct(): void {
    ...
    for (cnt = FIRST_RPC; cnt < NUM_PROFILES; cnt++) {
      ...
      if ((pSoldier = GetSoldier(sSoldierID)) === null) continue;

      // load quote info if it exists
      if (
        gMercProfiles[cnt].ubMiscFlags &
  ...
  */
  ...
  export function GetNumberOfActiveWorldItemsFromTempFile(
    sMapX: INT16,
    sMapY: INT16,
    bMapZ: INT8,
  ): UINT32 {
    ...
    if (bMapZ == 0) {
      ...
      while (TempNode) {
        if (
          TempNode.ubSectorX == sMapX &&
          TempNode.ubSectorY == sMapY &&
    ...
    } else pNumberOfData = 0;

    return pNumberOfData;
  }

  function DoesTempFileExistsForMap(
    uiType: UINT32,
    sMapX: INT16,
    sMapY: INT16,
    bMapZ: INT8,
  ...
  function InitTempNpcQuoteInfoForNPCFromTempFile(): boolean {
    ...
    let TempNpcQuote: TempNPCQuoteInfoSave[] /* [NUM_NPC_QUOTE_RECORDS] */ =
      createArrayFrom(NUM_NPC_QUOTE_RECORDS, createTempNPCQuoteInfoSave);
    let uiSizeOfTempArray: UINT32 =
      TEMP_NPC_QUOTE_INFO_SAVE_SIZE * NUM_NPC_QUOTE_RECORDS;
    let usCnt1: UINT16;
    let hFile: HWFILE;
    let buffer: Buffer;
    ...
    for (usCnt1 = 0; usCnt1 < NUM_PROFILES - FIRST_RPC; usCnt1++) {
      ...
      for (ubCnt = 0; ubCnt < NUM_NPC_QUOTE_RECORDS; ubCnt++) {
        if (gpNPCQuoteInfoArray[usCnt1]) {
          TempNpcQuote[ubCnt].usFlags =
            gpNPCQuoteInfoArray[usCnt1][ubCnt].fFlags;
          TempNpcQuote[ubCnt].sRequiredItem =
            gpNPCQuoteInfoArray[usCnt1][ubCnt].sRequiredItem;
          TempNpcQuote[ubCnt].usGoToGridno =
            gpNPCQuoteInfoArray[usCnt1][ubCnt].usGoToGridno;
        }
      }
  ...
  function SaveTempNpcQuoteInfoForNPCToTempFile(ubNpcId: UINT8): boolean {
    ...
    if (gpNPCQuoteInfoArray[ubNpcId]) {
      ...
      for (ubCnt = 0; ubCnt < NUM_NPC_QUOTE_RECORDS; ubCnt++) {
        TempNpcQuote[ubCnt].usFlags =
          gpNPCQuoteInfoArray[ubNpcId][ubCnt].fFlags;
        TempNpcQuote[ubCnt].sRequiredItem =
          gpNPCQuoteInfoArray[ubNpcId][ubCnt].sRequiredItem;
        TempNpcQuote[ubCnt].usGoToGridno =
          gpNPCQuoteInfoArray[ubNpcId][ubCnt].usGoToGridno;
      }
      ...
      gMercProfiles[ubNpcId].ubMiscFlags |=
        PROFILE_MISC_FLAG_TEMP_NPC_QUOTE_DATA_EXISTS;
  ...
  function LoadTempNpcQuoteInfoForNPCFromTempFile(ubNpcId: UINT8): boolean {
    ...
    if (gpNPCQuoteInfoArray[ubNpcId] == null) {
      gpNPCQuoteInfoArray[ubNpcId] = createArrayFrom(
    ...
    hFile = FileOpen(
      NPC_TEMP_QUOTE_FILE,
      FILE_ACCESS_READ | FILE_OPEN_ALWAYS,
      false,
    );
    if (hFile == 0) {
      // Error opening temp npc quote info
  ...
  export function ChangeNpcToDifferentSector(
    ubNpcId: UINT8,
    sSectorX: INT16,
    sSectorY: INT16,
    bSectorZ: INT8,
  ...
  */
  ...
  export function AddRottingCorpseToUnloadedSectorsRottingCorpseFile(
    sMapX: INT16,
    sMapY: INT16,
    bMapZ: INT8,
    pRottingCorpseDef: ROTTING_CORPSE_DEFINITION,
  ): boolean {
    let hFile: HWFILE;
  ...
  */
  ...
  function SetUnderGroundSectorFlag(
    sSectorX: INT16,
    sSectorY: INT16,
    ubSectorZ: UINT8,
    uiFlagToSet: UINT32,
  ...
  function ReSetUnderGroundSectorFlag(
    sSectorX: INT16,
    sSectorY: INT16,
    ubSectorZ: UINT8,
    uiFlagToSet: UINT32,
  ...
  function GetUnderGroundSectorFlagStatus(
    sSectorX: INT16,
    sSectorY: INT16,
    ubSectorZ: UINT8,
    uiFlagToCheck: UINT32,
  ): boolean {
    let pTempNode: UNDERGROUND_SECTORINFO | null = gpUndergroundSectorInfoHead;
    ...
    while (pTempNode) {
      if (
        ...
        // set the flag indicating that ther is a temp item file exists for the sector
        if (pTempNode.uiFlags & uiFlagToCheck) return true;
        else return false;
      }
      pTempNode = pTempNode.next;
    }
  ...
  export function SetSectorFlag(
    ...
    if (bMapZ == 0) SectorInfo[SECTOR(sMapX, sMapY)].uiFlags |= uiFlagToSet;
    else SetUnderGroundSectorFlag(sMapX, sMapY, bMapZ, uiFlagToSet);

    return true;
  }

  export function ReSetSectorFlag(
    sMapX: INT16,
    sMapY: INT16,
    bMapZ: UINT8,
    uiFlagToSet: UINT32,
    ...
    if (bMapZ == 0) SectorInfo[SECTOR(sMapX, sMapY)].uiFlags &= ~uiFlagToSet;
    else ReSetUnderGroundSectorFlag(sMapX, sMapY, bMapZ, uiFlagToSet);

    return true;
  }

  export function GetSectorFlagStatus(
    sMapX: INT16,
    sMapY: INT16,
    bMapZ: UINT8,
    uiFlagToSet: UINT32,
    ...
    if (bMapZ == 0)
      return SectorInfo[SECTOR(sMapX, sMapY)].uiFlags & uiFlagToSet
        ? true
        : false;
    else
  ...
  export function AddDeadSoldierToUnLoadedSector(
    sMapX: INT16,
    sMapY: INT16,
    bMapZ: UINT8,
    ...
    let pWorldItems: WORLDITEM[] /* Pointer<WORLDITEM> */ = <WORLDITEM[]>(
      (<unknown>null)
    );
    let i: UINT32;
    let bCount: UINT8 = 0;
    ...
    } else AssertMsg(0, "ERROR!!	Flag not is Switch statement");
    ...
    else uiDeathAnim = uiPossibleDeathAnims[Random(4)];
  ...
  function GetRotationArray(): UINT8[] {
    ...
    if (guiJA2EncryptionSet < BASE_NUMBER_OF_ROTATION_ARRAYS * 6) {
      if (guiJA2EncryptionSet < BASE_NUMBER_OF_ROTATION_ARRAYS * 3) {
        return gubEncryptionArray1[
          guiJA2EncryptionSet % (BASE_NUMBER_OF_ROTATION_ARRAYS * 3)
        ];
      } else {
        return gubEncryptionArray2[
          guiJA2EncryptionSet % (BASE_NUMBER_OF_ROTATION_ARRAYS * 3)
        ];
      }
    } else {
      if (guiJA2EncryptionSet < BASE_NUMBER_OF_ROTATION_ARRAYS * 9) {
        return gubEncryptionArray3[
          guiJA2EncryptionSet % (BASE_NUMBER_OF_ROTATION_ARRAYS * 3)
        ];
      } else {
        return gubEncryptionArray4[
          guiJA2EncryptionSet % (BASE_NUMBER_OF_ROTATION_ARRAYS * 3)
        ];
      }
    }
  ...
  export function NewJA2EncryptedFileRead(
    hFile: HWFILE,
    pDest: Buffer,
  ...
  export function NewJA2EncryptedFileWrite(
    hFile: HWFILE,
    pDest: Buffer,
    ...
    fRet =
      (uiBytesWritten = FileWrite(hFile, pMemBlock, uiBytesToWrite)) !== -1;
  ...
  let ubRotationArray: UINT8[] /* [46] */ = [
    ...
    215, 76, 163, 187, 55, 49, 65, 48, 156, 140, 201, 68, 184, 13, 45, 69, 102,
    185, 122, 225, 23, 250, 160, 220, 114, 240, 64, 175, 57, 233,
  ];

  export function JA2EncryptedFileRead(
    hFile: HWFILE,
    pDest: Buffer,
    uiBytesToRead: UINT32,
  ): UINT32 {
  ...
  export function JA2EncryptedFileWrite(
    hFile: HWFILE,
    pDest: Buffer,
    uiBytesToWrite: UINT32,
  ): UINT32 {
  ...
  export function GetMapTempFileName(
    uiType: UINT32,
    sMapX: INT16,
    sMapY: INT16,
    bMapZ: INT8,
  ...
  export function GetNumberOfVisibleWorldItemsFromSectorStructureForSector(
    sMapX: INT16,
    sMapY: INT16,
    bMapZ: INT8,
  ): UINT32 {
    ...
    if (bMapZ == 0) {
      uiNumberOfItems =
        SectorInfo[SECTOR(sMapX, sMapY)]
          .uiNumberOfWorldItemsInTempFileThatCanBeSeenByPlayer;
    } else {
      ...
      if (pSector != null) {
        ...
        uiNumberOfItems =
          pSector.uiNumberOfWorldItemsInTempFileThatCanBeSeenByPlayer;
      }
    }
    ...
    if (gfWorldLoaded) {
      ...
      if (
        ...
        uiNumberOfItems = UpdateLoadedSectorsItemInventory(
          sMapX,
          sMapY,
          bMapZ,
          uiNumberOfItems,
  ...
  export function SetNumberOfVisibleWorldItemsInSectorStructureForSector(
    sMapX: INT16,
    sMapY: INT16,
    bMapZ: INT8,
    uiNumberOfItems: UINT32,
    ...
    if (bMapZ == 0) {
      SectorInfo[
        SECTOR(sMapX, sMapY)
      ].uiNumberOfWorldItemsInTempFileThatCanBeSeenByPlayer = uiNumberOfItems;
    } else {
      // find the underground sector
      pSector = FindUnderGroundSector(sMapX, sMapY, bMapZ);
      if (pSector != null) {
  ...
  function SynchronizeItemTempFileVisbleItemsToSectorInfoVisbleItems(
    sMapX: INT16,
    sMapY: INT16,
    bMapZ: INT8,
    fLoadingGame: boolean,
  ): void {
    let uiTotalNumberOfItems: UINT32 = 0;
    let uiTotalNumberOfRealItems: UINT32 = 0;
    let pTotalSectorList: WORLDITEM[] /* Pointer<WORLDITEM> */ = <WORLDITEM[]>(
      (<unknown>null)
    );
    ...
    fReturn =
      (uiTotalNumberOfRealItems = GetNumberOfActiveWorldItemsFromTempFile(
        sMapX,
        sMapY,
        bMapZ,
      )) !== -1;
    ...
    fReturn =
      (uiTotalNumberOfItems = GetNumberOfWorldItemsFromTempItemFile(
        sMapX,
        sMapY,
        bMapZ,
        false,
    ...
    for (iCounter = 0; iCounter < uiTotalNumberOfRealItems; iCounter++) {
      ...
      if (
        IsMapScreenWorldItemVisibleInMapInventory(pTotalSectorList[iCounter])
      ) {
  ...
  function UpdateLoadedSectorsItemInventory(
    sMapX: INT16,
    sMapY: INT16,
    bMapZ: INT8,
    uiNumberOfItems: UINT32,
