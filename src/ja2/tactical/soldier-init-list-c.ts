namespace ja2 {
  ...
  export function KillSoldierInitList(): void {
    ...
    if (gfOriginalList) gOriginalSoldierInitListHead = null;
    else gAlternateSoldierInitListHead = null;
  }

  export function AddBasicPlacementToSoldierInitList(
    ...
    if (!gSoldierInitHead) {
      ...
      if (gfOriginalList) gOriginalSoldierInitListHead = curr;
      else gAlternateSoldierInitListHead = curr;
  ...
  export function RemoveSoldierNodeFromInitList(
    pNode: SOLDIERINITNODE | null,
  ): void {
    if (!pNode) return;
    if (gfOriginalList) gMapInformation.ubNumIndividuals--;
    ...
    if (pNode == gSoldierInitHead) {
      ...
      if (gfOriginalList) gOriginalSoldierInitListHead = gSoldierInitHead;
      else gAlternateSoldierInitListHead = gSoldierInitHead;
  ...
  export function LoadSoldiersFromMap(buffer: Buffer, offset: number): number {
    ...
    if (ubNumIndividuals > MAX_INDIVIDUALS) {
      AssertMsg(
        false,
        "Corrupt map check failed.  ubNumIndividuals is greater than MAX_INDIVIDUALS.",
      );
  ...
  function SortSoldierInitList(): void {
    ...
    while (curr) {
      if (
        ...
        curr.pDetailedPlacement.ubProfile == Enum268.FREDO
      ) {
        fFredoAtStart = true;
        break;
      }
  ...
  export function AddPlacementToWorld(curr: SOLDIERINITNODE): boolean {
    ...
    if (curr.pDetailedPlacement) {
      ...
      if (ubProfile != NO_PROFILE && !gfEditMode) {
        if (
          gMercProfiles[ubProfile].sSectorX != gWorldSectorX ||
      ...
      if (!gfEditMode) {
        ...
        if (FindSoldierByProfileID(Enum194.ICECREAMTRUCK, true) != null) {
          if (curr.pDetailedPlacement.bBodyType == Enum194.ICECREAMTRUCK) {
            ...
            if (
              gMercProfiles[Enum268.HAMOUS].sSectorX != gWorldSectorX ||
              gMercProfiles[Enum268.HAMOUS].sSectorY != gWorldSectorY ||
              gMercProfiles[Enum268.HAMOUS].bSectorZ
            ) {
            ...
            if (
              gMercProfiles[Enum268.HAMOUS].ubMiscFlags &
    ...
    if (!gfEditMode) {
        ...
        if (gWorldSectorX == 5 && gWorldSectorY == MAP_ROW_C) {
          ...
          if (
            ...
            if (tempDetailedPlacement.ubProfile == NO_PROFILE) {
              ...
              (ubRoom = InARoom(tempDetailedPlacement.sInsertionGridNo)) !==
                -1 &&
              IN_BROTHEL(ubRoom)
            ) {
          ...
          if (gubFact[Enum170.FACT_QUEEN_DEAD]) {
            if (
              tempDetailedPlacement.ubCivilianGroup == Enum246.QUEENS_CIV_GROUP
            ) {
            ...
            if (
              ...
              tempDetailedPlacement.ubProfile == Enum268.QUEEN
            ) {
          ...
          if (
            !StrategicMap[TIXA_SECTOR_X + TIXA_SECTOR_Y * MAP_WORLD_X]
              .fEnemyControlled &&
            tempDetailedPlacement.ubProfile == NO_PROFILE &&
            tempDetailedPlacement.bBodyType != Enum194.HATKIDCIV &&
            tempDetailedPlacement.bBodyType != Enum194.KIDCIV
          ) {
            // not there
            return true;
          }
      ...
      if (
        tempDetailedPlacement.bTeam == ENEMY_TEAM &&
        tempDetailedPlacement.ubSoldierClass == Enum262.SOLDIER_CLASS_ELITE
      ) {
  ...
  function AddPlacementToWorldByProfileID(ubProfile: UINT8): void {
    ...
    while (curr) {
      if (
        curr.pDetailedPlacement &&
        curr.pDetailedPlacement.ubProfile == ubProfile &&
        !curr.pSoldier
      ) {
  ...
  export function AddSoldierInitListTeamToWorld(
    bTeam: INT8,
    ubMaxNum: UINT8,
  ): UINT8 {
    ...
    while (
      ...
      if (curr.pBasicPlacement.bTeam == bTeam) {
        if (
          ubSlotsAvailable <= ubSlotsToFill ||
          Random(ubSlotsAvailable) < ubSlotsToFill
        ) {
  ...
  export function AddSoldierInitListEnemyDefenceSoldiers(
    ubTotalAdmin: UINT8,
    ubTotalTroops: UINT8,
    ubTotalElite: UINT8,
  ): void {
    ...
    let ubElitePDSlots__Pointer = createPointer(
      () => ubElitePDSlots,
      (v) => (ubElitePDSlots = v),
    );
    let ubEliteDSlots: UINT8 = 0;
    let ubEliteDSlots__Pointer = createPointer(
      () => ubEliteDSlots,
      (v) => (ubEliteDSlots = v),
    );
    let ubElitePSlots: UINT8 = 0;
    let ubElitePSlots__Pointer = createPointer(
      () => ubElitePSlots,
      (v) => (ubElitePSlots = v),
    );
    let ubEliteBSlots: UINT8 = 0;
    let ubEliteBSlots__Pointer = createPointer(
      () => ubEliteBSlots,
      (v) => (ubEliteBSlots = v),
    );
    let ubTroopPDSlots: UINT8 = 0;
    let ubTroopPDSlots__Pointer = createPointer(
      () => ubTroopPDSlots,
      (v) => (ubTroopPDSlots = v),
    );
    let ubTroopDSlots: UINT8 = 0;
    let ubTroopDSlots__Pointer = createPointer(
      () => ubTroopDSlots,
      (v) => (ubTroopDSlots = v),
    );
    let ubTroopPSlots: UINT8 = 0;
    let ubTroopPSlots__Pointer = createPointer(
      () => ubTroopPSlots,
      (v) => (ubTroopPSlots = v),
    );
    let ubTroopBSlots: UINT8 = 0;
    let ubTroopBSlots__Pointer = createPointer(
      () => ubTroopBSlots,
      (v) => (ubTroopBSlots = v),
    );
    let ubAdminPDSlots: UINT8 = 0;
    let ubAdminPDSlots__Pointer = createPointer(
      () => ubAdminPDSlots,
      (v) => (ubAdminPDSlots = v),
    );
    let ubAdminDSlots: UINT8 = 0;
    let ubAdminDSlots__Pointer = createPointer(
      () => ubAdminDSlots,
      (v) => (ubAdminDSlots = v),
    );
    let ubAdminPSlots: UINT8 = 0;
    let ubAdminPSlots__Pointer = createPointer(
      () => ubAdminPSlots,
      (v) => (ubAdminPSlots = v),
    );
    let ubAdminBSlots: UINT8 = 0;
    let ubAdminBSlots__Pointer = createPointer(
      () => ubAdminBSlots,
      (v) => (ubAdminBSlots = v),
    );
    ...
    let ubTotalAdmin__Pointer = createPointer(
      () => ubTotalAdmin,
      (v) => (ubTotalAdmin = v),
    );
    let ubTotalTroops__Pointer = createPointer(
      () => ubTotalTroops,
      (v) => (ubTotalTroops = v),
    );
    let ubTotalElite__Pointer = createPointer(
      () => ubTotalElite,
      (v) => (ubTotalElite = v),
    );
        ...
        switch (curr.pBasicPlacement.ubSoldierClass) {
          case Enum262.SOLDIER_CLASS_ELITE:
            if (
              ...
              ubElitePDSlots++;
            else if (curr.pBasicPlacement.fPriorityExistance) ubElitePSlots++;
            else if (curr.pDetailedPlacement) ubEliteDSlots++;
            else ubEliteBSlots++;
            break;
          case Enum262.SOLDIER_CLASS_ADMINISTRATOR:
            if (
              ...
              ubAdminPDSlots++;
            else if (curr.pBasicPlacement.fPriorityExistance) ubAdminPSlots++;
            else if (curr.pDetailedPlacement) ubAdminDSlots++;
            else ubAdminBSlots++;
            break;
          case Enum262.SOLDIER_CLASS_ARMY:
            if (
              ...
              ubTroopPDSlots++;
            else if (curr.pBasicPlacement.fPriorityExistance) ubTroopPSlots++;
            else if (curr.pDetailedPlacement) ubTroopDSlots++;
            else ubTroopBSlots++;
            break;
        }
      }
    ...
    for (
      ubCurrClass = Enum262.SOLDIER_CLASS_ADMINISTRATOR;
      ubCurrClass <= Enum262.SOLDIER_CLASS_ARMY;
      ubCurrClass++
    ) {
      ...
      while (
        ...
        if (!curr.pSoldier && curr.pBasicPlacement.bTeam == ENEMY_TEAM) {
          if (curr.pBasicPlacement.ubSoldierClass == ubCurrClass) {
            if (
              pCurrSlots.value <= pCurrTotal.value ||
              Random(pCurrSlots.value) < pCurrTotal.value
            ) {
              // found matching team, so add this soldier to the game.
              if (AddPlacementToWorld(curr)) {
                pCurrTotal.value--;
  ...
  export function AddSoldierInitListMilitia(
    ubNumGreen: UINT8,
    ubNumRegs: UINT8,
    ubNumElites: UINT8,
  ): void {
    ...
    let ubEliteSlots__Pointer = createPointer(
      () => ubEliteSlots,
      (v) => (ubEliteSlots = v),
    );
    let ubRegSlots: UINT8 = 0;
    let ubRegSlots__Pointer = createPointer(
      () => ubRegSlots,
      (v) => (ubRegSlots = v),
    );
    let ubGreenSlots: UINT8 = 0;
    let ubGreenSlots__Pointer = createPointer(
      () => ubGreenSlots,
      (v) => (ubGreenSlots = v),
    );
    let ubFreeSlots: UINT8;
    ...
    let pCurrTotal: Pointer<UINT8>;
    let ubCurrClass: UINT8;
    let ubNumGreen__Pointer = createPointer(
      () => ubNumGreen,
      (v) => (ubNumGreen = v),
    );
    let ubNumRegs__Pointer = createPointer(
      () => ubNumRegs,
      (v) => (ubNumRegs = v),
    );
    let ubNumElites__Pointer = createPointer(
      () => ubNumElites,
      (v) => (ubNumElites = v),
    );
    ...
    while (curr && curr.pBasicPlacement.fPriorityExistance && ubMaxNum) {
      ...
      if (
        ...
        if (
          ...
          curr.pBasicPlacement.ubSoldierClass =
            Enum262.SOLDIER_CLASS_ELITE_MILITIA;
          ubNumElites--;
        } else if (
          ubNumRegs &&
          curr.pBasicPlacement.ubSoldierClass == Enum262.SOLDIER_CLASS_ARMY
        ...
        if (fDoPlacement) {
          ...
          if (curr.pDetailedPlacement) {
            ...
            curr.pBasicPlacement.bRelativeEquipmentLevel =
              RandomizeRelativeLevel(
                curr.pBasicPlacement.bRelativeEquipmentLevel,
                curr.pBasicPlacement.ubSoldierClass,
              );
  ...
  export function AddSoldierInitListCreatures(
    fQueen: boolean,
    ubNumLarvae: UINT8,
    ubNumInfants: UINT8,
    ...
    if (fQueen) {
      ...
      while (curr) {
        if (
          ...
          curr.pBasicPlacement.bBodyType == Enum194.QUEENMONSTER
        ) {
          if (!AddPlacementToWorld(curr)) {
            fQueen = false;
            break;
    ...
    while (curr && ubFreeSlots && ubNumCreatures) {
      if (!curr.pSoldier && curr.pBasicPlacement.bTeam == CREATURE_TEAM) {
        ...
        // the number of enemies, the lower the chance of accepting the slot.
        if (
          ubFreeSlots <= ubNumCreatures ||
          Random(ubFreeSlots) < ubNumCreatures
        ) {
          // Choose which team to use.
          ...
          if (ubNumLarvae && iRandom < ubNumLarvae) {
            ...
            ubNumYoungMales &&
            iRandom < ubNumLarvae + ubNumInfants + ubNumYoungMales
          ) {
            ubNumYoungMales--;
            curr.pBasicPlacement.bBodyType = Enum194.YAM_MONSTER;
          } else if (
            ubNumYoungFemales &&
            iRandom <
              ubNumLarvae + ubNumInfants + ubNumYoungMales + ubNumYoungFemales
          ) {
            ubNumYoungFemales--;
            curr.pBasicPlacement.bBodyType = Enum194.YAF_MONSTER;
          } else if (
  ...
  function FindSoldierInitNodeWithProfileID(
    usProfile: UINT16,
  ): SOLDIERINITNODE | null {
    let curr: SOLDIERINITNODE | null;
    curr = gSoldierInitHead;
    while (curr) {
      if (
        curr.pDetailedPlacement &&
        curr.pDetailedPlacement.ubProfile == usProfile
      )
        return curr;
      curr = curr.next;
    }
  ...
  export function FindSoldierInitNodeWithID(
    usID: UINT16,
  ): SOLDIERINITNODE | null {
    let curr: SOLDIERINITNODE | null;
    curr = gSoldierInitHead;
    while (curr) {
      if (curr.pSoldier && curr.pSoldier.ubID == usID) return curr;
      curr = curr.next;
    }
    return null;
  }
  ...
  export function EvaluateDeathEffectsToSoldierInitList(
      ...
      if (gTacticalStatus.uiFlags & LOADING_SAVED_GAME) {
        ...
        while (curr) {
          if (curr.ubNodeID == ubNodeID) {
            ...
            if (
              (ubSoldierID >= gTacticalStatus.Team[ENEMY_TEAM].bFirstID &&
  ...
  export function AddSoldierInitListBloodcats(): void {
    ...
    if (pSector.bBloodCatPlacements) {
      ...
      if (
        bBloodCatPlacements != pSector.bBloodCatPlacements &&
  ...
  function FindSoldierInitListNodeByProfile(
    ubProfile: UINT8,
  ): SOLDIERINITNODE | null {
    let curr: SOLDIERINITNODE | null;
  ...
  export function AddProfilesUsingProfileInsertionData(): void {
    ...
    for (i = FIRST_RPC; i < Enum268.PROF_HUMMER; i++) {
      ...
      if (
        gMercProfiles[i].sSectorX != gWorldSectorX ||
      ...
      if (pSoldier) {
        ...
        pSoldier.ubStrategicInsertionCode =
          gMercProfiles[i].ubStrategicInsertionCode;
        pSoldier.usStrategicInsertionData =
          gMercProfiles[i].usStrategicInsertionData;
        UpdateMercInSector(
          pSoldier,
          gWorldSectorX,
          gWorldSectorY,
        ...
        if (gMercProfiles[i].ubQuoteRecord) {
          ...
          if (
            pSoldier.ubQuoteActionID == Enum290.QUOTE_ACTION_ID_CHECKFORDEST
          ) {
        ...
        if (curr) {
          ...
          if (
            ...
            let pSchedule: SCHEDULENODE | null = GetSchedule(
              curr.pDetailedPlacement.ubScheduleID,
            );
            if (pSchedule) {
  ...
  export function AddProfilesNotUsingProfileInsertionData(): void {
    ...
    while (curr) {
      if (
        ...
        !gMercProfiles[curr.pDetailedPlacement.ubProfile]
  ...
  export function NewWayOfLoadingEnemySoldierInitListLinks(
    hfile: HWFILE,
  ): boolean {
    let uiNumBytesRead: UINT32;
    let curr: SOLDIERINITNODE | null;
  ...
  export function LookAtButDontProcessEnemySoldierInitListLinks(
    hfile: HWFILE,
  ): boolean {
    let uiNumBytesRead: UINT32;
    let curr: SOLDIERINITNODE | null;
    ...
    while (curr) {
      if (curr.pDetailedPlacement) {
        if (curr.pBasicPlacement.bTeam == ENEMY_TEAM) {
          ...
          curr.pBasicPlacement.bBodyType = -1;
          curr.pBasicPlacement.bRelativeAttributeLevel = RandomizeRelativeLevel(
            curr.pBasicPlacement.bRelativeAttributeLevel,
            curr.pBasicPlacement.ubSoldierClass,
          );
          curr.pBasicPlacement.bRelativeEquipmentLevel = RandomizeRelativeLevel(
            curr.pBasicPlacement.bRelativeEquipmentLevel,
            curr.pBasicPlacement.ubSoldierClass,
          );
        }
