namespace ja2 {
  ...
  class _OBJECTTYPE implements OBJECTTYPE {
    ...
    constructor() {
      ...
      this.ubGunUnused = new Uint8Array(
        this.sharedBuffer.buffer,
        this.sharedBuffer.byteOffset + 7,
        MAX_OBJECTS_PER_SLOT - 6,
      );
      this.ubShotsLeft = new Uint8Array(
        this.sharedBuffer.buffer,
        this.sharedBuffer.byteOffset,
        MAX_OBJECTS_PER_SLOT,
      );
      this.bStatus = new Int8Array(
        this.sharedBuffer.buffer,
        this.sharedBuffer.byteOffset,
        MAX_OBJECTS_PER_SLOT,
      );
      this.ubMoneyUnused = new Uint8Array(
        this.sharedBuffer.buffer,
        this.sharedBuffer.byteOffset + 8,
        MAX_OBJECTS_PER_SLOT - 5,
      );
      this.bKeyStatus = new Int8Array(
        this.sharedBuffer.buffer,
        this.sharedBuffer.byteOffset,
        6,
      );
      this.ubKeyUnused = new Uint8Array(
        this.sharedBuffer.buffer,
        this.sharedBuffer.byteOffset + 7,
        1,
      );
      this.ubOwnershipUnused = new Uint8Array(
        this.sharedBuffer.buffer,
        this.sharedBuffer.byteOffset + 2,
        6,
      );
  ...
  export function copyObjectType(destination: OBJECTTYPE, source: OBJECTTYPE) {
    ...
    (<_OBJECTTYPE>source).sharedBuffer.copy(
      (<_OBJECTTYPE>destination).sharedBuffer,
    );
  ...
  export function readObjectType(
    o: OBJECTTYPE,
    buffer: Buffer,
    offset: number = 0,
  ): number {
    o.usItem = buffer.readUInt16LE(offset);
    offset += 2;
...
*/
  ...
  // PARENT TYPES
  export const IC_WEAPON =
    IC_GUN | IC_BLADE | IC_THROWING_KNIFE | IC_LAUNCHER | IC_TENTACLES;
  export const IC_EXPLOSV = IC_GRENADE | IC_BOMB;

  export const IC_BOBBY_GUN = IC_GUN | IC_LAUNCHER;
  export const IC_BOBBY_MISC =
    IC_GRENADE |
    IC_BOMB |
    IC_MISC |
    IC_MEDKIT |
  ...
  export const IF_STANDARD_BLADE =
    ITEM_DAMAGEABLE |
    ITEM_WATER_DAMAGES |
    ITEM_REPAIRABLE |
    ITEM_SHOW_STATUS |
    ITEM_METAL |
    ITEM_SINKS;
  export const IF_STANDARD_ARMOUR =
    ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_SHOW_STATUS | ITEM_SINKS;
  export const IF_STANDARD_KIT =
    ITEM_DAMAGEABLE | ITEM_SHOW_STATUS | ITEM_SINKS;
  export const IF_STANDARD_CLIP = ITEM_SINKS | ITEM_METAL;

  export const EXPLOSIVE_GUN = (x: number) =>
    x == Enum225.ROCKET_LAUNCHER || x == Enum225.TANK_CANNON;
  ...
  export function createInvTypeFrom(
    usItemClass: UINT32,
    ubClassIndex: UINT8,
    ubCursor: UINT8,
    bSoundType: INT8,
