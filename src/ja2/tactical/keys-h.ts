namespace ja2 {
  ...
  export function createKeyFrom(
    usItem: UINT16,
    fFlags: UINT8,
    usSectorFound: UINT16,
    usDateFound: UINT16,
  ): KEY {
    return {
      usItem,
  ...
  export function readKey(o: KEY, buffer: Buffer, offset: number = 0): number {
    ...
    offset++; // padding
    o.usSectorFound = buffer.readUInt16LE(offset);
    offset += 2;
    o.usDateFound = buffer.readUInt16LE(offset);
    offset += 2;
    return offset;
  }
...
*/
  ...
  export function readLock(
    ...
    o.ubEditorName = readStringNL(
      buffer,
      "ascii",
      offset,
      offset + MAXLOCKDESCLENGTH,
    );
    offset += MAXLOCKDESCLENGTH;
    o.usKeyItem = buffer.readUInt16LE(offset);
    offset += 2;
  ...
  export function readDoorStatus(
    o: DOOR_STATUS,
    buffer: Buffer,
    offset: number = 0,
  ): number {
  ...
  export function writeDoorStatus(
    o: DOOR_STATUS,
    buffer: Buffer,
    offset: number = 0,
  ): number {
