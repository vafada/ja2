namespace ja2 {
  export let gubVehicleMovementGroups: INT8[] /* [MAX_VEHICLES] */ =
    createArray(MAX_VEHICLES, 0);

  // the list of vehicles
  export let pVehicleList: VEHICLETYPE[] /* Pointer<VEHICLETYPE> */ = <
    VEHICLETYPE[]
  >(<unknown>null);
...
*/
  ...
  export function InitVehicles(): void {
    ...
    for (cnt = 0; cnt < MAX_VEHICLES; cnt++) {
      // create mvt groups
      gubVehicleMovementGroups[cnt] = CreateNewVehicleGroupDepartingFromSector(
        1,
        1,
        cnt,
      );
  ...
  export function SetVehicleValuesIntoSoldierType(pVehicle: SOLDIERTYPE): void {
    pVehicle.name =
      zVehicleName[pVehicleList[pVehicle.bVehicleID].ubVehicleType];
  ...
  export function RemoveVehicleFromList(iId: INT32): boolean {
    ...
    if (pVehicleList[iId].pMercPath != null) {
      pVehicleList[iId].pMercPath = ClearStrategicPathList(
      ...
      for (iCounter = 0; iCounter < ubNumberOfVehicles; iCounter++) {
        ...
        if (pVehicleList[iCounter].fValid) {
          ...
          if (pVehicleList[iCounter].pMercPath) {
            ...
            pVehicleList[iCounter].pMercPath = ClearStrategicPathList(
  ...
  */
  ...
  export function IsThisVehicleAccessibleToSoldier(
    ...
    if (
      pSoldier.sSectorX != pVehicleList[iId].sSectorX ||
  ...
  function AddSoldierToVehicle(pSoldier: SOLDIERTYPE, iId: INT32): boolean {
    ...
    if (GetNumberInVehicle(iId) == 0 && pVehicleSoldier) {
      ...
      RemovePlayerFromGroup(
        SquadMovementGroups[pVehicleSoldier.bAssignment],
        pVehicleSoldier,
      );
    ...
    for (
      iCounter = 0;
      iCounter < iSeatingCapacities[pVehicleList[iId].ubVehicleType];
      iCounter++
    ) {
    ...
    if (pVehicleSoldier) {
      ...
      PlayJA2Sample(
        pVehicleList[pVehicleSoldier.bVehicleID].iOutOfSound,
        RATE_11025,
        SoundVolume(HIGHVOLUME, pVehicleSoldier.sGridNo),
        1,
      ...
      if (pVehicleList[iId].pPassengers[iCounter] == null) {
        ...
        if (pVehicleSoldier) {
          ...
          EVENT_SetSoldierPosition(
            pSoldier,
            pVehicleSoldier.dXPos,
            pVehicleSoldier.dYPos,
          );
  ...
  export function SetSoldierExitVehicleInsertionData(
    pSoldier: SOLDIERTYPE,
    iId: INT32,
  ): void {
    if (iId == iHelicopterVehicleId && !pSoldier.bInSector) {
      if (
        pSoldier.sSectorX != BOBBYR_SHIPPING_DEST_SECTOR_X ||
  ...
  function RemoveSoldierFromVehicle(
    pSoldier: SOLDIERTYPE,
    iId: INT32,
  ): boolean {
    ...
    for (
      ...
      if (pVehicleList[iId].pPassengers[iCounter] == pSoldier) {
        ...
        pVehicleList[iId].pPassengers[iCounter].sSectorY =
          pVehicleList[iId].sSectorY;
        pVehicleList[iId].pPassengers[iCounter].sSectorX =
          pVehicleList[iId].sSectorX;
        pVehicleList[iId].pPassengers[iCounter].bSectorZ =
          pVehicleList[iId].sSectorZ;
...
*/
  ...
  export function MoveCharactersPathToVehicle(pSoldier: SOLDIERTYPE): boolean {
    ...
    if (
      ...
      pSoldier.pMercPath = <PathSt>(
        ClearStrategicPathList(pSoldier.pMercPath, 0)
      );
      return false;
    }
    ...
    // move to beginning
    pVehicleList[iId].pMercPath = MoveToBeginningOfPathList(
      pVehicleList[iId].pMercPath,
    );
  ...
  function AddVehicleMembersToMvtGroup(iId: INT32): boolean {
    ...
    for (
      ...
      if (pVehicleList[iId].pPassengers[iCounter] != null) {
        AddPlayerToGroup(
          pVehicleList[iId].ubMovementGroup,
          pVehicleList[iId].pPassengers[iCounter],
        );
      }
    }
  ...
  function InjurePersonInVehicle(
    iId: INT32,
    pSoldier: SOLDIERTYPE,
    ubPointsOfDmg: UINT8,
  ): boolean {
  ...
  export function KillAllInVehicle(iId: INT32): boolean {
    ...
    for (
      ...
      if (pVehicleList[iId].pPassengers[iCounter] != null) {
        if (
          KillPersonInVehicle(iId, pVehicleList[iId].pPassengers[iCounter]) ==
          false
        ) {
          return false;
        }
  ...
  export function AnyAccessibleVehiclesInSoldiersSector(
    pSoldier: SOLDIERTYPE,
  ): boolean {
    let iCounter: INT32 = 0;
  ...
  export function IsEnoughSpaceInVehicle(iID: INT32): boolean {
    ...
    if (
      GetNumberInVehicle(iID) ==
      iSeatingCapacities[pVehicleList[iID].ubVehicleType]
    ) {
      return false;
    }
  ...
  export function PutSoldierInVehicle(
    pSoldier: SOLDIERTYPE,
    bVehicleId: INT8,
  ): boolean {
    let pVehicleSoldier: SOLDIERTYPE;
    ...
    if (
      ...
      pSoldier.sSectorY != gWorldSectorY ||
      pSoldier.bSectorZ != 0 ||
      bVehicleId == iHelicopterVehicleId
    ) {
      // add the soldier
  ...
  function GetVehicleSoldierPointerFromPassenger(
  ...
  export function ExitVehicle(pSoldier: SOLDIERTYPE): boolean {
    ...
    if (pVehicle.uiStatusFlags & SOLDIER_VEHICLE) {
      ...
      PlayJA2Sample(
        pVehicleList[pVehicle.bVehicleID].iOutOfSound,
        RATE_11025,
        SoundVolume(HIGHVOLUME, pVehicle.sGridNo),
        1,
        SoundDir(pVehicle.sGridNo),
      );
      return true;
    }
  ...
  export function AddPassangersToTeamPanel(iId: INT32): void {
    ...
    for (
      cnt = 0;
      cnt < iSeatingCapacities[pVehicleList[iId].ubVehicleType];
      cnt++
    ) {
  ...
  export function VehicleTakeDamage(
    ubID: UINT8,
    ubReason: UINT8,
    sDamage: INT16,
    sGridNo: INT16,
    ...
    if (pVehicleList[ubID].fDestroyed == false) {
      switch (ubReason) {
        ...
        case TAKE_DAMAGE_STRUCTURE_EXPLOSION:
          HandleCriticalHitForVehicleInLocation(
            ubID,
            sDamage,
            sGridNo,
            ubAttackerID,
  ...
  function HandleCriticalHitForVehicleInLocation(
    ubID: UINT8,
    sDmg: INT16,
    sGridNo: INT16,
    ubAttackerID: UINT8,
  ...
  export function RepairVehicle(
    iVehicleId: INT32,
    bRepairPtsLeft: INT8,
    pfNothingToRepair: Pointer<boolean>,
  ): INT8 {
    let pVehicleSoldier: SOLDIERTYPE;
    ...
    // Repair
    pVehicleSoldier.bLife += Math.trunc(
      bRepairPtsLeft / VEHICLE_REPAIR_POINTS_DIVISOR,
    );
    ...
    // Calculate pts used;
    bRepairPtsUsed =
      (pVehicleSoldier.bLife - bOldLife) * VEHICLE_REPAIR_POINTS_DIVISOR;
...
*/
  ...
  export function AdjustVehicleAPs(
    pSoldier: SOLDIERTYPE,
    ubPoints: UINT8,
  ): UINT8 {
    ...
    pubDeducations +=
      pVehicleList[pSoldier.bVehicleID].sCriticalHits[
        Enum281.ENGINE_HIT_LOCATION
      ] * COST_PER_ENGINE_CRIT;

    // handle each tire
    for (
      iCounter = Enum281.RF_TIRE_HIT_LOCATION;
      iCounter < Enum281.LR_TIRE_HIT_LOCATION;
      iCounter++
    ) {
      ...
      if (pVehicleList[cnt].fValid) {
        ...
        for (ubPassengerCnt = 0; ubPassengerCnt < 10; ubPassengerCnt++) {
          ...
          if (pVehicleList[cnt].pPassengers[ubPassengerCnt]) {
            ...
            TempVehicle.pPassengers[ubPassengerCnt] = <any>(
              pVehicleList[cnt].pPassengers[ubPassengerCnt].ubProfile
            );
          }
        }
  ...
  export function LoadVehicleInformationFromSavedGameFile(
    hFile: HWFILE,
          ...
          for (ubPassengerCnt = 0; ubPassengerCnt < 10; ubPassengerCnt++) {
            if (uiSavedGameVersion < 86) {
              if (pVehicleList[cnt].pPassengers[ubPassengerCnt] != <any>0) {
                ...
                pVehicleList[cnt].pPassengers[ubPassengerCnt] = <SOLDIERTYPE>(
                  FindSoldierByProfileID(
                    <UINT8>(
                      (<unknown>pVehicleList[cnt].pPassengers[ubPassengerCnt])
                    ),
                    false,
                  )
                );
              ...
              if (
                pVehicleList[cnt].pPassengers[ubPassengerCnt] != <any>NO_PROFILE
              ) {
                // ! The id of the soldier was saved in the passenger pointer.  The passenger pointer is converted back
                // ! to a UINT8 so we can get the REAL pointer to the soldier.
                pVehicleList[cnt].pPassengers[ubPassengerCnt] = <SOLDIERTYPE>(
  ...
  export function SetVehicleSectorValues(
    iVehId: INT32,
    ubSectorX: UINT8,
    ubSectorY: UINT8,
  ): void {
  ...
  export function UpdateAllVehiclePassengersGridNo(
    pSoldier: SOLDIERTYPE,
  ): void {
    let iCounter: INT32;
    let iId: INT32;
  ...
  export function LoadVehicleMovementInfoFromSavedGameFile(
    hFile: HWFILE,
  ): boolean {
    let cnt: INT32;
    let pGroup: GROUP;
  ...
  export function NewSaveVehicleMovementInfoToSavedGameFile(
    hFile: HWFILE,
  ): boolean {
    let uiNumBytesWritten: UINT32 = 0;
    let uiSaveSize: UINT32 = 0;
  ...
  export function NewLoadVehicleMovementInfoFromSavedGameFile(
    hFile: HWFILE,
  ): boolean {
    let uiNumBytesRead: UINT32 = 0;
    let uiSaveSize: UINT32 = 0;
  ...
  function TeleportVehicleToItsClosestSector(
    iVehicleId: INT32,
    ubGroupID: UINT8,
  ): void {
    let pGroup: GROUP;
  ...
  function CanSoldierDriveVehicle(
    pSoldier: SOLDIERTYPE,
    iVehicleId: INT32,
    fIgnoreAsleep: boolean,
  ): boolean {
    Assert(pSoldier);
  ...
  export function SoldierMustDriveVehicle(
    pSoldier: SOLDIERTYPE,
    iVehicleId: INT32,
    fTryingToTravel: boolean,
  ): boolean {
    Assert(pSoldier);
    ...
    if (
      !fTryingToTravel &&
      !pVehicleList[iVehicleId].fBetweenSectors &&
  ...
  function OnlyThisSoldierCanDriveVehicle(
    pThisSoldier: SOLDIERTYPE,
    iVehicleId: INT32,
  ): boolean {
    let iCounter: INT32 = 0;
    let pSoldier: SOLDIERTYPE;
  ...
  export function IsSoldierInThisVehicleSquad(
    pSoldier: SOLDIERTYPE,
    bSquadNumber: INT8,
  ): boolean {
  ...
  export function PickRandomPassengerFromVehicle(
    pSoldier: SOLDIERTYPE,
  ): SOLDIERTYPE | null {
