...
*/
  ...
  export function ValidateEntryPointGridNo(sGridNo: Pointer<INT16>): boolean {
    ...
    if (sGridNo.value < 0) return false; // entry point is non-existant
    ...
    if (sWorldY < sTopLimit) {
      ({ uiCellX: iNewMapX, uiCellY: iNewMapY } =
        GetFromAbsoluteScreenXYWorldXY(sWorldX, sTopLimit));
    } else if (sWorldY > sBottomLimit) {
      ({ uiCellX: iNewMapX, uiCellY: iNewMapY } =
        GetFromAbsoluteScreenXYWorldXY(sWorldX, sBottomLimit));
    } else {
      return false; // already valid
    }

    sGridNo.value = MAPROWCOLTOPOS(
      Math.trunc(iNewMapY / 10),
      Math.trunc(iNewMapX / 10),
    );
  ...
  function UpdateOldVersionMap(): void {
    ...
    if (gMapInformation.ubMapVersion < 20) {
      ...
      ValidateEntryPointGridNo(
        createPropertyPointer(gMapInformation, "sNorthGridNo"),
      );
      ValidateEntryPointGridNo(
        createPropertyPointer(gMapInformation, "sEastGridNo"),
      );
      ValidateEntryPointGridNo(
        createPropertyPointer(gMapInformation, "sSouthGridNo"),
      );
      ValidateEntryPointGridNo(
        createPropertyPointer(gMapInformation, "sWestGridNo"),
      );
      ValidateEntryPointGridNo(
        createPropertyPointer(gMapInformation, "sCenterGridNo"),
      );
      ValidateEntryPointGridNo(
        createPropertyPointer(gMapInformation, "sIsolatedGridNo"),
      );
    }
  ...
  export function ValidateAndUpdateMapVersionIfNecessary(): void {
    ...
    if (gMapInformation.ubMapVersion < MINOR_MAP_VERSION) {
      ...
      AssertMsg(
        0,
        "Map version is greater than the current version (old ja2.exe?)",
      );
    }
