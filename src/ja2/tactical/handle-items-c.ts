namespace ja2 {
  ...
  let FlashItemSlots: ITEM_POOL_LOCATOR[] /* [NUM_ITEM_FLASH_SLOTS] */ =
    createArrayFrom(NUM_ITEM_FLASH_SLOTS, createItemPoolLocator);
  ...
  export function HandleCheckForBadChangeToGetThrough(
    pSoldier: SOLDIERTYPE,
    pTargetSoldier: SOLDIERTYPE | null,
    sTargetGridNo: INT16,
    bLevel: INT8,
  ): boolean {
  ...
  export function HandleItem(
    ...
    if (!(gTacticalStatus.uiFlags & INCOMBAT)) {
      DebugMsg(
        TOPIC_JA2,
        DBG_LEVEL_3,
        FormatString("Setting attack busy count to 0 due to no combat"),
      );
      ...
      if (pSoldier.ubProfile != NO_PROFILE) {
        ...
        if (
          ...
          ((pTargetSoldier.ubProfile == NO_PROFILE &&
            ...
            gMercProfiles[pTargetSoldier.ubProfile].ubMiscFlags3 &
              PROFILE_MISC_FLAG3_GOODGUY)
        ) {
        ...
        if (
          pSoldier.ubCivilianGroup == Enum246.REBEL_CIV_GROUP &&
          (pTargetSoldier.ubCivilianGroup == Enum246.REBEL_CIV_GROUP ||
            (pTargetSoldier.bNeutral &&
              pTargetSoldier.ubProfile == NO_PROFILE &&
              pTargetSoldier.ubCivilianGroup == Enum246.NON_CIV_GROUP &&
              pTargetSoldier.ubBodyType != Enum194.CROW))
        ) {
          TacticalCharacterDialogue(pSoldier, Enum202.QUOTE_REFUSING_ORDER);
          return ITEM_HANDLE_REFUSAL;
    ...
    // Check HAND ITEM
    if (
      Item[usHandItem].usItemClass == IC_GUN ||
      Item[usHandItem].usItemClass == IC_THROWING_KNIFE
    ) {
      // WEAPONS
      if (
        ...
        if (pSoldier.ubProfile != NO_PROFILE) {
          if (
            ...
            if (
              pSoldier.inv[pSoldier.ubAttackingHand].ubImprintID == NO_PROFILE
            ) {
              ...
              if (pSoldier.bTeam == gbPlayerNum) {
                ...
                ScreenMsg(
                  ...
                  MSG_INTERFACE,
                  '"%s"',
                  TacticalStr[Enum335.GUN_GOT_FINGERPRINT],
                );
      ...
      ({ fAddingTurningCost, fAddingRaiseGunCost } =
        GetAPChargeForShootOrStabWRTGunRaises(pSoldier, sTargetGridNo, 1));
    ...
    if (Item[usHandItem].usItemClass == IC_BLADE) {
      ...
      if (pSoldier.ubBodyType == Enum194.BLOODCAT) {
        sActionGridNo = FindNextToAdjacentGridEx(
          pSoldier,
          usGridNo,
          ubDirection__Pointer,
          sAdjustedGridNo__Pointer,
        ...
        PythSpacesAway(pSoldier.sGridNo, usGridNo) > 1
      ) {
        sActionGridNo = FindNextToAdjacentGridEx(
          pSoldier,
          usGridNo,
    ...
    if (
      Item[usHandItem].usItemClass == IC_GRENADE ||
    ...
    if (Item[usHandItem].ubCursor == INVALIDCURS) {
      // Found detonator...
      if (
        FindAttachment(pSoldier.inv[usHandItem], Enum225.DETONATOR) !=
          ITEM_NOT_FOUND ||
        FindAttachment(pSoldier.inv[usHandItem], Enum225.REMDETONATOR)
      ) {
  ...
  export function HandleSoldierDropBomb(
    pSoldier: SOLDIERTYPE,
    sGridNo: INT16,
  ): void {
    ...
    if (
      ...
      if (ArmBomb(pSoldier.inv[Enum261.HANDPOS], 0)) {
        ...
        pSoldier.inv[Enum261.HANDPOS].bTrap = Math.min(
          10,
          Math.trunc(EffectiveExplosive(pSoldier) / 20) +
            Math.trunc(EffectiveExpLevel(pSoldier) / 3),
        );
  ...
  export function HandleSoldierUseRemote(
    pSoldier: SOLDIERTYPE,
    sGridNo: INT16,
  ): void {
    StartBombMessageBox(pSoldier, sGridNo);
  ...
  export function HandleSoldierThrowItem(
    pSoldier: SOLDIERTYPE,
    sGridNo: INT16,
  ): void {
    ...
    switch (gAnimControl[pSoldier.usAnimState].ubHeight) {
      case ANIM_STAND:
        ...
        if (sGridNo == pSoldier.sGridNo) {
          ...
          if (
            GetRangeFromGridNoDiff(sGridNo, pSoldier.sGridNo) < MIN_LOB_RANGE
          ) {
  ...
  export function SoldierGiveItem(
    pSoldier: SOLDIERTYPE,
  ...
  export function SoldierDropItem(
    pSoldier: SOLDIERTYPE,
    pObj: OBJECTTYPE,
  ): boolean {
    pSoldier.pTempObject = createObjectType();
  ...
  export function SoldierPickupItem(
    pSoldier: SOLDIERTYPE,
    iItemIndex: INT32,
    sGridNo: INT16,
    bZLevel: INT8,
  ...
  export function SoldierGetItemFromWorld(
    pSoldier: SOLDIERTYPE,
    iItemIndex: INT32,
    sGridNo: INT16,
    bZLevel: INT8,
    pfSelectionList: boolean[] | null,
  ): void {
    ...
    let fSaidBoobyTrapQuote__Pointer = createPointer(
      () => fSaidBoobyTrapQuote,
      (v) => (fSaidBoobyTrapQuote = v),
    );

    // OK. CHECK IF WE ARE DOING ALL IN THIS POOL....
    if (
      iItemIndex == ITEM_PICKUP_ACTION_ALL ||
      iItemIndex == ITEM_PICKUP_SELECTION
    ) {
      // DO all pickup!
      ...
      while (pItemPool) {
        if (ItemPoolOKForPickup(pSoldier, pItemPool, bZLevel)) {
          ...
          if (fPickup) {
            if (
              ContinuePastBoobyTrap(
                pSoldier,
                sGridNo,
                bZLevel,
                pItemPool.iItemIndex,
                false,
                fSaidBoobyTrapQuote__Pointer,
              )
            ) {
              // Make copy of item
              ...
              if (Object.usItem == Enum225.SWITCH) {
                ...
                if (!AutoPlaceObject(pSoldier, Object, true)) {
                  ...
                  if (
                    Object.usItem !=
                      gWorldItems[pItemPool.iItemIndex].o.usItem ||
    ...
    if (fShouldSayCoolQuote && pSoldier.bTeam == gbPlayerNum) {
      ...
      if (
        ...
        Enum202.QUOTE_FOUND_SOMETHING_SPECIAL
      ) {
    ...
    if (
      pSoldier.bTeam == gbPlayerNum &&
      LookForHiddenItems(sGridNo, pSoldier.bLevel, true, 0)
    ) {
  ...
  export function HandleSoldierPickupItem(
    pSoldier: SOLDIERTYPE,
    iItemIndex: INT32,
    sGridNo: INT16,
    bZLevel: INT8,
    ...
    if ((pItemPool = GetItemPool(sGridNo, pSoldier.bLevel))) {
      ...
      if (pSoldier.bTeam != gbPlayerNum) {
        ...
        if (
          ...
          gpBoobyTrapItemPool = <ITEM_POOL>(
            GetItemPoolForIndex(sGridNo, iItemIndex, pSoldier.bLevel)
          );
          ...
          if (
            pSoldier.bTeam == gbPlayerNum &&
            DoesItemPoolContainAllHiddenItems(pItemPool)
          ) {
            // He's touched them....
            ...
            if (
              (usNum = GetNumOkForDisplayItemsInPool(pItemPool, bZLevel)) == 1
            ) {
              // Find first OK item....
              ...
              SoldierGetItemFromWorld(
                pSoldier,
                (<ITEM_POOL>pItemPool).iItemIndex,
                sGridNo,
                bZLevel,
                null,
              );
  ...
  function AddItemGraphicToWorld(
    pItem: INVTYPE,
    sGridNo: INT16,
    ubLevel: UINT8,
  ): LEVELNODE {
  ...
  function RemoveItemGraphicFromWorld(
    pItem: INVTYPE,
    sGridNo: INT16,
    ubLevel: UINT8,
  ...
  export function AddItemToPoolAndGetIndex(
    sGridNo: INT16,
    pObject: OBJECTTYPE,
    bVisible: INT8,
    ubLevel: UINT8,
  ...
  export function InternalAddItemToPool(
    ...
    usFlags: UINT16,
    bRenderZHeightAboveLevel: INT8,
    piItemIndex: Pointer<INT32> | null,
  ): OBJECTTYPE | null {
    let pItemPool: ITEM_POOL | null;
    ...
    if (psGridNo.value == NOWHERE) {
      ...
      ScreenMsg(
        FONT_MCOLOR_LTYELLOW,
        MSG_BETAVERSION,
        "Error: Item %d was given invalid grid location %d for item pool. Please Report.",
        pObject.usItem,
        psGridNo.value,
      );
    ...
    if (
      bTerrainID == Enum315.DEEP_WATER ||
      bTerrainID == Enum315.LOW_WATER ||
      bTerrainID == Enum315.MED_WATER
    ) {
    ...
    if (
      gpWorldLevelData[psGridNo.value].pStructureHead &&
      pObject.usItem != Enum225.OWNERSHIP &&
      ...
      while (pStructure) {
        if (
          ...
          if (pStructure.fFlags & STRUCTURE_OPENABLE) {
            ...
            bRenderZHeightAboveLevel = CONVERT_INDEX_TO_PIXELS(
              StructureHeight(pStructure),
            );
            break;
          }
          // Else can we place an item on top?
            ...
            if (
              StructureDensity(
                pStructure,
                createElementPointer(ubLevel, 0),
                createElementPointer(ubLevel, 1),
                createElementPointer(ubLevel, 2),
                createElementPointer(ubLevel, 3),
              )
            ) {
              if (
                ubLevel[3] == 0 &&
  ...
  export function ItemTypeExistsAtLocation(
    sGridNo: INT16,
    usItem: UINT16,
    ubLevel: UINT8,
    piItemIndex: Pointer<INT32> | null,
  ...
  function GetItemOfClassTypeInPool(
    sGridNo: INT16,
    uiItemClass: UINT32,
    ubLevel: UINT8,
  ): INT32 {
    ...
    if ((pItemPool = GetItemPool(sGridNo, ubLevel))) {
      ...
      while (pItemPoolTemp != null) {
        if (
          Item[gWorldItems[pItemPoolTemp.iItemIndex].o.usItem].usItemClass &
          uiItemClass
        ) {
  ...
  export function DoesItemPoolContainAnyHiddenItems(
  ...
  function DoesItemPoolContainAllHiddenItems(
    pItemPool: ITEM_POOL | null,
  ): boolean {
    // LOOP THROUGH LIST TO FIND NODE WE WANT
    while (pItemPool != null) {
  ...
  function LookForHiddenItems(
    ...
    if ((pItemPool = GetItemPool(sGridNo, ubLevel))) {
      ...
      while (pItemPool != null) {
        if (
          gWorldItems[pItemPool.iItemIndex].bVisible == HIDDEN_ITEM &&
          gWorldItems[pItemPool.iItemIndex].o.usItem != Enum225.OWNERSHIP
        ) {
          fFound = true;
  ...
  export function GetZLevelOfItemPoolGivenStructure(
    sGridNo: INT16,
    ubLevel: UINT8,
  ...
  export function GetLargestZLevelOfItemPool(
    pItemPool: ITEM_POOL | null,
  ): INT8 {
  ...
  function DoesItemPoolContainAllItemsOfHigherZLevel(
    pItemPool: ITEM_POOL | null,
  ): boolean {
    // LOOP THROUGH LIST TO FIND NODE WE WANT
    while (pItemPool != null) {
  ...
  function DoesItemPoolContainAllItemsOfZeroZLevel(
    pItemPool: ITEM_POOL | null,
  ): boolean {
    // LOOP THROUGH LIST TO FIND NODE WE WANT
    while (pItemPool != null) {
  ...
  function LoopLevelNodeForShowThroughFlag(
  ...
  export function SetItemPoolVisibilityOn(
    pItemPool: ITEM_POOL,
    bAllGreaterThan: INT8,
    fSetLocator: boolean,
  ): boolean {
    let pItemPoolTemp: ITEM_POOL | null;
    ...
    while (pItemPoolTemp != null) {
      ...
      if (bVisibleValue != VISIBLE) {
        if (
          gWorldItems[pItemPoolTemp.iItemIndex].o.usItem == Enum225.ACTION_ITEM
        ) {
          // NEVER MAKE VISIBLE!
          pItemPoolTemp = pItemPoolTemp.pNext;
          continue;
        ...
        if (
          bVisibleValue >= bAllGreaterThan &&
          gWorldItems[pItemPoolTemp.iItemIndex].o.usItem != Enum225.OWNERSHIP
        ) {
  ...
  export function RemoveItemFromPool(
    sGridNo: INT16,
    iItemIndex: INT32,
    ubLevel: UINT8,
  ): boolean {
    ...
    if ((pItemPool = GetItemPool(sGridNo, ubLevel))) {
      ...
      // REMOVE GRAPHIC
      RemoveItemGraphicFromWorld(
        Item[gWorldItems[iItemIndex].o.usItem],
        sGridNo,
        ubLevel,
      ...
      if (pItemPoolTemp.bRenderZHeightAboveLevel > 0) {
        ...
        if (
          (pTempPool = GetItemPool(
            pItemPoolTemp.sGridNo,
            pItemPoolTemp.ubLevel,
          )) === null
        ) {
          pStructure = FindStructure(
            pItemPoolTemp.sGridNo,
            STRUCTURE_HASITEMONTOP,
          );
  ...
  export function MoveItemPools(sStartPos: INT16, sEndPos: INT16): boolean {
    ...
    while ((pItemPool = GetItemPool(sStartPos, 0))) {
      ...
      AddItemToPool(
        ...
        TempWorldItem.o,
        -1,
        TempWorldItem.ubLevel,
        TempWorldItem.usFlags,
  ...
  function GetNumOkForDisplayItemsInPool(
    pItemPool: ITEM_POOL | null,
    bZLevel: INT8,
  ): INT16 {
    let cnt: INT32;
  ...
  export function AnyItemsVisibleOnLevel(
    pItemPool: ITEM_POOL | null,
    bZLevel: INT8,
  ): boolean {
  ...
  export function ItemPoolOKForDisplay(
    pItemPool: ITEM_POOL,
    bZLevel: INT8,
  ): boolean {
    if (gTacticalStatus.uiFlags & SHOW_ALL_ITEMS) {
  ...
  export function DrawItemPoolList(
    pItemPool: ITEM_POOL | null,
    sGridNo: INT16,
    bCommand: UINT8,
    bZLevel: INT8,
  ...
  function GetListMouseHotSpot(
    sLargestLineWidth: INT16,
    bNumItemsListed: INT8,
    sFontX: INT16,
    sFontY: INT16,
    bCurStart: INT8,
  ): INT8 {
  ...
  function SetItemPoolLocatorWithCallback(
    pItemPool: ITEM_POOL,
  ...
  function GetFreeFlashItemSlot(): INT32 {
    ...
    for (uiCount = 0; uiCount < guiNumFlashItemSlots; uiCount++) {
      if (FlashItemSlots[uiCount].fAllocated == false) return uiCount;
    }
  ...
  export function VerifyGiveItem(
    pSoldier: SOLDIERTYPE,
    ppTargetSoldier: Pointer<SOLDIERTYPE>,
  ): boolean {
    ...
    if (usSoldierIndex != NOBODY) {
      ...
      if (pSoldier.pTempObject != null) {
        ...
        ScreenMsg(
          FONT_MCOLOR_LTYELLOW,
          MSG_INTERFACE,
          TacticalStr[Enum335.ITEM_HAS_BEEN_PLACED_ON_GROUND_STR],
      ...
      if (fToTargetPlayer) {
        ...
        if (!AutoPlaceObject(pTSoldier, TempObject, true)) {
          ...
          ScreenMsg(
            FONT_MCOLOR_LTYELLOW,
            MSG_INTERFACE,
            TacticalStr[Enum335.ITEM_HAS_BEEN_GIVEN_TO_STR],
            ShortItemNames[usItemNum],
            pTSoldier.name,
          );
            ...
            if (
              ...
              if (gMercProfiles[pTSoldier.ubProfile].iBalance >= 0) {
                ...
                ScreenMsg(
                  FONT_MCOLOR_LTYELLOW,
                  MSG_INTERFACE,
                  TacticalStr[Enum335.GUY_HAS_BEEN_PAID_IN_FULL_STR],
                  pTSoldier.name,
                );
              } else {
                // report the payment
                ScreenMsg(
                  FONT_MCOLOR_LTYELLOW,
                  MSG_INTERFACE,
                  TacticalStr[Enum335.GUY_STILL_OWED_STR],
                  pTSoldier.name,
                  -gMercProfiles[pTSoldier.ubProfile].iBalance,
                );
              }
  ...
  export function AdjustGridNoForItemPlacement(
    pSoldier: SOLDIERTYPE,
    sGridNo: INT16,
  ): INT16 {
    let pStructure: STRUCTURE | null;
  ...
  function BombMessageBoxCallBack(ubExitValue: UINT8): void {
    if (gpTempSoldier) {
      if (
        gpTempSoldier.inv[Enum261.HANDPOS].usItem == Enum225.REMOTEBOMBTRIGGER
      ) {
        ...
        if (
          FindAttachment(
            gpTempSoldier.inv[Enum261.HANDPOS],
            Enum225.REMDETONATOR,
          ) != ITEM_NOT_FOUND
        ) {
          iResult = SkillCheck(
            gpTempSoldier,
            Enum255.PLANTING_REMOTE_BOMB_CHECK,
            0,
          );
        } else {
        ...
        if (iResult >= 0) {
          ...
          if (iResult >= -20) {
            ...
            IgniteExplosion(
              ...
              gpTempSoldier.sGridNo,
              gpTempSoldier.inv[Enum261.HANDPOS].usItem,
        ...
        if (ArmBomb(gpTempSoldier.inv[Enum261.HANDPOS], ubExitValue)) {
          gpTempSoldier.inv[Enum261.HANDPOS].bTrap = Math.min(
            10,
            Math.trunc(EffectiveExplosive(gpTempSoldier) / 20) +
              Math.trunc(EffectiveExpLevel(gpTempSoldier) / 3),
          );
          // HACK IMMINENT!
          ...
          gpTempSoldier.inv[Enum261.HANDPOS].ubBombOwner =
            gpTempSoldier.ubID + 2;
          AddItemToPool(
            gsTempGridno,
            gpTempSoldier.inv[Enum261.HANDPOS],
            1,
  ...
  function HandItemWorks(pSoldier: SOLDIERTYPE, bSlot: INT8): boolean {
    ...
    if (
      Item[pObj.usItem].fFlags & ITEM_DAMAGEABLE &&
      pObj.usItem != Enum225.MINE &&
      Item[pObj.usItem].usItemClass != IC_MEDKIT &&
      pObj.usItem != Enum225.GAS_CAN
    ) {
  ...
  function SetOffBoobyTrapInMapScreen(
    pSoldier: SOLDIERTYPE,
    pObject: OBJECTTYPE,
  ): void {
  ...
  function ContinuePastBoobyTrap(
    pSoldier: SOLDIERTYPE,
    sGridNo: INT16,
    bLevel: INT8,
    iItemIndex: INT32,
    fInStrategic: boolean,
    pfSaidQuote: Pointer<boolean>,
  ): boolean {
  ...
  function BoobyTrapMessageBoxCallBack(ubExitValue: UINT8): void {
    ...
    if (ubExitValue == MSG_BOX_RETURN_YES) {
      ...
      if (iCheckResult >= 0) {
        ...
        if (
          Object.ubBombOwner > 1 &&
          Object.ubBombOwner - 2 >= gTacticalStatus.Team[OUR_TEAM].bFirstID &&
        ...
        if (gfDisarmingBuriedBomb) {
          if (Object.usItem == Enum225.SWITCH) {
            ...
            Object.usItem == Enum225.ACTION_ITEM &&
            Object.bActionValue != Enum191.ACTION_ITEM_BLOW_UP
          ) {
        ...
        if (AutoPlaceObject(gpBoobyTrapSoldier, Object, true)) {
          ...
          RemoveItemFromPool(
            gsBoobyTrapGridNo,
            gpBoobyTrapItemPool.iItemIndex,
            gbBoobyTrapLevel,
          );
        } else {
          // make sure the item in the world is untrapped
          gWorldItems[gpBoobyTrapItemPool.iItemIndex].o.bTrap = 0;
          gWorldItems[gpBoobyTrapItemPool.iItemIndex].o.fFlags &=
            ~OBJECT_KNOWN_TO_BE_TRAPPED;
  ...
  export function NearbyGroundSeemsWrong(
    pSoldier: SOLDIERTYPE,
    sGridNo: INT16,
    fCheckAroundGridno: boolean,
    psProblemGridNo: Pointer<INT16>,
  ): boolean {
    ...
    */
    ...
    for (ubDirection = 0; ubDirection < 8; ubDirection++) {
      if (fCheckAroundGridno) {
        ...
        ubMovementCost =
          gubWorldMovementCosts[sNextGridNo][ubDirection][pSoldier.bLevel];
  ...
  export function MakeNPCGrumpyForMinorOffense(
    pSoldier: SOLDIERTYPE,
    pOffendingSoldier: SOLDIERTYPE | null,
  ): void {
    ...
    if (pOffendingSoldier) {
      ...
      pSoldier.usNextActionData = atan8(
        pSoldier.sX,
        pSoldier.sY,
        pOffendingSoldier.sX,
  ...
  function CheckForPickedOwnership(): void {
    ...
    while (pItemPool) {
      if (gWorldItems[pItemPool.iItemIndex].o.usItem == Enum225.OWNERSHIP) {
        ...
        if (
          gWorldItems[pItemPool.iItemIndex].o.ubOwnerCivGroup !=
          Enum246.NON_CIV_GROUP
        ) {
          ubCivGroup = gWorldItems[pItemPool.iItemIndex].o.ubOwnerCivGroup;
          if (
            ubCivGroup == Enum246.HICKS_CIV_GROUP &&
            CheckFact(Enum170.FACT_HICKS_MARRIED_PLAYER_MERC, 0)
          ) {
  ...
  function LoopLevelNodeForItemGlowFlag(
    pNode: LEVELNODE | null,
    sGridNo: INT16,
    ubLevel: UINT8,
    fOn: boolean,
  ...
  export function ContinuePastBoobyTrapInMapScreen(
    pObject: OBJECTTYPE,
    pSoldier: SOLDIERTYPE,
  ): boolean {
    let fBoobyTrapKnowledge: boolean;
  ...
  // Refresh item pools
  export function RefreshItemPools(
    pItemList: WORLDITEM[],
    iNumberOfItems: INT32,
  ): void {
    ClearAllItemPools();
  ...
  export function FindNearestAvailableGridNoForItem(
    sSweetGridNo: INT16,
    ubRadius: INT8,
  ): INT16 {
    let sTop: INT16;
  ...
  export function CanPlayerUseRocketRifle(
    pSoldier: SOLDIERTYPE,
    fDisplay: boolean,
  ): boolean {
    if (
      pSoldier.inv[pSoldier.ubAttackingHand].usItem == Enum225.ROCKET_RIFLE ||
      pSoldier.inv[pSoldier.ubAttackingHand].usItem == Enum225.AUTO_ROCKET_RIFLE
    ) {
      // check imprint ID
      // NB not-imprinted value is NO_PROFILE
      // imprinted value is profile for mercs & NPCs and NO_PROFILE + 1 for generic dudes
            ...
            if (pSoldier.bTeam == gbPlayerNum) {
              ...
              if (fDisplay) {
                ScreenMsg(
                  ...
                  MSG_UI_FEEDBACK,
                  '"%s"',
                  TacticalStr[Enum335.GUN_NOGOOD_FINGERPRINT],
                );
              }
            }
            return false;
