namespace ja2 {
  ...
  function readAirRaidSaveStruct(
    o: AIR_RAID_SAVE_STRUCT,
    buffer: Buffer,
    offset: number = 0,
  ): number {
    ...
    offset++; // padding
    o.uiSoundSample = buffer.readUInt32LE(offset);
    offset += 4;
    o.uiRaidLastUpdate = buffer.readUInt32LE(offset);
    offset += 4;
    ...
    offset++; // padding
    o.sDiveX = buffer.readInt16LE(offset);
    offset += 2;
    o.sDiveY = buffer.readInt16LE(offset);
    offset += 2;
    o.sDiveTargetLocation = buffer.readInt16LE(offset);
    offset += 2;
    o.ubDiveDirection = buffer.readUInt8(offset++);
    offset++; // padding
    o.sNumGridNosMoved = buffer.readInt16LE(offset);
    offset += 2;
    o.iNumTurnsSinceLastDive = buffer.readInt32LE(offset);
    offset += 4;
    o.iNumTurnsSinceDiveStarted = buffer.readInt32LE(offset);
    offset += 4;
    o.iNumGridNosMovedThisTurn = buffer.readInt32LE(offset);
    offset += 4;
    ...
    o.sRaidSoldierID = buffer.readInt16LE(offset);
    offset += 2;
    o.sNotLocatedYet = Boolean(buffer.readUInt16LE(offset));
    offset += 2;
    o.iNumFrames = buffer.readInt32LE(offset);
    offset += 4;
  ...
  function writeAirRaidSaveStruct(
    o: AIR_RAID_SAVE_STRUCT,
    buffer: Buffer,
    offset: number = 0,
  ): number {
  ...
  function ScheduleAirRaid(pAirRaidDef: AIR_RAID_DEFINITION): void {
    ...
    AddSameDayStrategicEvent(
      Enum132.EVENT_BEGIN_AIR_RAID,
      GetWorldMinutesInDay() + pAirRaidDef.ubNumMinsFromCurrentTime,
      0,
    );
  ...
  */
    ...
    for (
      ...
      if (pSoldier.bActive) {
        if (
          pSoldier.sSectorX == gAirRaidDef.sSectorX &&
          pSoldier.sSectorY == gAirRaidDef.sSectorY &&
          pSoldier.bSectorZ == gAirRaidDef.sSectorZ &&
    ...
    ChangeSelectedMapSector(
      gAirRaidDef.sSectorX,
      gAirRaidDef.sSectorY,
      gAirRaidDef.sSectorZ,
    );
    ...
    if (
      ...
      gfQuoteSaid = true;
      SayQuoteFromAnyBodyInThisSector(
        gAirRaidDef.sSectorX,
        gAirRaidDef.sSectorY,
        gAirRaidDef.sSectorZ,
        Enum202.QUOTE_AIR_RAID,
      );
      SpecialCharacterDialogueEvent(
        DIALOGUE_SPECIAL_EVENT_EXIT_MAP_SCREEN,
        gAirRaidDef.sSectorX,
        gAirRaidDef.sSectorY,
        gAirRaidDef.sSectorZ,
        0,
  ...
  function PickRandomLocationAtMinSpacesAway(
    sGridNo: INT16,
    sMinValue: INT16,
    sRandomVar: INT16,
  ): INT16 {
  ...
  function AirRaidStart(): void {
    // Begin ambient sound....
    guiSoundSample = PlayJA2Sample(
      Enum330.S_RAID_AMBIENT,
      RATE_11025,
      0,
      10000,
      MIDDLEPAN,
  ...
  function AirRaidLookForDive(): void {
    ...
    if (!(gTacticalStatus.uiFlags & INCOMBAT)) {
      ...
      if (gfQuoteSaid) {
        if (
          TIMECOUNTERDONE(giTimerAirRaidDiveStarted, AIR_RAID_DIVE_INTERVAL)
        ) {
  ...
  function BeginBombing(): void {
    ...
    // Get direction....
    gubDiveDirection = GetDirectionToGridNoFromGridNo(
      sGridNo,
      gsDiveTargetLocation,
    );
        ...
        if (sOldGridNo != sGridNo) {
          ...
          if (
            GridNoOnVisibleWorldTile(
              GETWORLDINDEXFROMWORLDCOORDS(sStrafeY, sStrafeX),
            )
          ) {
            // if ( gsNotLocatedYet && !( gTacticalStatus.uiFlags & INCOMBAT ) )
            //	{
            ...
            FireBulletGivenTarget(
              ...
              sStrafeY,
              0,
              gpRaidSoldier.usAttackingWeapon,
              10,
              false,
              false,
            );
        ...
        if (giNumGridNosMovedThisTurn >= 6) {
          if (gTacticalStatus.uiFlags & INCOMBAT) {
            ...
            DebugMsg(
              ...
              DBG_LEVEL_3,
              FormatString(
                "!!!!!!! Tried to free up attacker AIR RAID DIVE DONE FOR THIS TURN, attack count now %d",
                gTacticalStatus.ubAttackBusyCount,
              ),
            );
          }
          ...
          if (gsNumGridNosMoved % 4 == 0) {
            ...
            if (
              ...
              InternalIgniteExplosion(
                ...
                usItem,
                fLocate,
                Number(IsRoofPresentAtGridno(sBombGridNo)),
              );
            }
          }
      ...
      if (uiClock - guiRaidLastUpdate > SCRIPT_DELAY) {
        ...
        switch (gubAirRaidMode) {
          ...
          case Enum192.AIR_RAID_END_DIVE:
            giNumTurnsSinceLastDive = 0;
            giTimerAirRaidDiveStarted = RESETTIMECOUNTER(
              AIR_RAID_DIVE_INTERVAL,
            );
  ...
  export function HandleAirRaidEndTurn(ubTeam: UINT8): boolean {
    ...
    DebugMsg(
      ...
      DBG_LEVEL_3,
      FormatString(
        "!!!!!!! Starting attack AIR RAID, attack count now %d",
        gTacticalStatus.ubAttackBusyCount,
      ),
    );

    AddTopMessage(
      Enum216.AIR_RAID_TURN_MESSAGE,
      TacticalStr[Enum335.AIR_RAID_TURN_STR],
    );
  ...
  export function SaveAirRaidInfoToSaveGameFile(hFile: HWFILE): boolean {
    ...
    } else sAirRaidSaveStruct.sRaidSoldierID = -1;
  ...
  export function LoadAirRaidInfoFromSaveGameFile(hFile: HWFILE): boolean {
    ...
    } else gpRaidSoldier = <SOLDIERTYPE>(<unknown>null);
