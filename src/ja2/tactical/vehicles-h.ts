namespace ja2 {
  ...
  export function createVehicleType(): VEHICLETYPE {
    return {
      ...
      ubDriver: 0,
      sInternalHitLocations: createArray(
        Enum280.NUMBER_OF_EXTERNAL_HIT_LOCATIONS_ON_VEHICLE,
        0,
      ),
      sArmourType: 0,
      sExternalArmorLocationsStatus: createArray(
        Enum280.NUMBER_OF_EXTERNAL_HIT_LOCATIONS_ON_VEHICLE,
        0,
      ),
      sCriticalHits: createArray(
        Enum281.NUMBER_OF_INTERNAL_HIT_LOCATIONS_IN_VEHICLE,
        0,
      ),
      iOnSound: 0,
      iOffSound: 0,
  ...
  export function copyVehicleType(
    ...
    copyArray(
      destination.sExternalArmorLocationsStatus,
      source.sExternalArmorLocationsStatus,
    );
  ...
  export function readVehicleType(
    o: VEHICLETYPE,
    buffer: Buffer,
    offset: number = 0,
  ): number {
    ...
    offset += 2;
    o.sSectorZ = buffer.readInt16LE(offset);
    offset += 2;
    o.fBetweenSectors = Boolean(buffer.readUInt8(offset++));
    offset++; // padding
    o.sGridNo = buffer.readInt16LE(offset);
    offset += 2;
    offset = readUIntArray(
      <number[]>(<unknown>o.pPassengers),
      buffer,
      offset,
      4,
    );
    ...
    o.sArmourType = buffer.readInt16LE(offset);
    offset += 2;
    offset = readIntArray(o.sExternalArmorLocationsStatus, buffer, offset, 2);
    offset = readIntArray(o.sCriticalHits, buffer, offset, 2);
    offset += 2; // padding
    o.iOnSound = buffer.readInt32LE(offset);
    offset += 4;
    o.iOffSound = buffer.readInt32LE(offset);
    offset += 4;
    o.iMoveSound = buffer.readInt32LE(offset);
    offset += 4;
    o.iOutOfSound = buffer.readInt32LE(offset);
    offset += 4;
    o.fFunctional = Boolean(buffer.readUInt8(offset++));
    o.fDestroyed = Boolean(buffer.readUInt8(offset++));
    offset += 2; // padding
    o.iMovementSoundID = buffer.readInt32LE(offset);
    offset += 4;
  ...
  export function writeVehicleType(
    o: VEHICLETYPE,
    buffer: Buffer,
    offset: number = 0,
  ): number {
