...
*/
  ...
  function resetPath(o: path_t) {
    o.iLocation = 0;
    o.pNext.fill(<path_t>(<unknown>null)), (o.sPathNdx = 0);
    o.usCostSoFar = 0;
    o.usCostToGo = 0;
    o.usTotalCost = 0;
    o.bLevel = 0;
  ...
  const ISWATER = (t: number) =>
    t == TRAVELCOST_KNEEDEEP || t == TRAVELCOST_DEEPWATER;
  ...
  const ESTIMATEC = (dx: number, dy: number) =>
    dx < dy
      ? Math.trunc((TRAVELCOST_BUMPY * (dx * 14 + dy * 10)) / 10)
      : Math.trunc((TRAVELCOST_BUMPY * (dy * 14 + dx * 10)) / 10);
  ...
  const LEGDISTANCE = (x1: number, y1: number, x2: number, y2: number) =>
    Math.abs(x2 - x1) + Math.abs(y2 - y1);
  //#define FARTHER(ndx,NDX) ( LEGDISTANCE( ndx->sLocation,sDestination) > LEGDISTANCE(NDX->sLocation,sDestination) )
  const FARTHER = (ndx: path_t, NDX: path_t) =>
    ndx.ubLegDistance > NDX.ubLegDistance;
  ...
  const pathFound = (iDestination: INT32) =>
    pQueueHead.pNext[0].iLocation == iDestination;
...
*/
  ...
  function ReconfigurePathAI(
    iNewMaxSkipListLevel: INT32,
    iNewMaxTrailTree: INT32,
    iNewMaxPathQ: INT32,
  ): void {
    // make sure the specified parameters are reasonable
    iNewMaxSkipListLevel = Math.max(
      iNewMaxSkipListLevel,
  ...
  export function FindBestPath(
    ...
    let pUpdate: path_t[] /* Pointer<path_t>[ABSMAX_SKIPLIST_LEVEL] */ =
      createArray(ABSMAX_SKIPLIST_LEVEL, <path_t>(<unknown>null));
    ...
    fTurnBased =
      Boolean(gTacticalStatus.uiFlags & TURNBASED) &&
      Boolean(gTacticalStatus.uiFlags & INCOMBAT);
    ...
    if (fCloseGoodEnough) {
      sClosePathLimit = Math.min(
        PythSpacesAway(s.sGridNo, sDestination) - 1,
        PATH_CLOSE_RADIUS,
      );
    ...
    */
    ...
    if (fMultiTile) {
      ...
      */
        if (gfEstimatePath) {
          ...
          s.pLevelNode != null &&
          s.pLevelNode.pStructureData != null
        ) {
    ...
    if (iDestination == NOWHERE) {
      ...
      if (
        ISWATER(gubWorldMovementCosts[iOrigination][0][ubLevel]) &&
        ISWATER(gubWorldMovementCosts[iDestination][0][ubLevel])
      )
        iWaterToWater = 1;
      else iWaterToWater = 0;
    }
    ...
    if (fCopyReachable) {
      ...
      pathQ[1].usCostToGo = REMAININGCOST(
        iDestX,
        iDestY,
        iLocX,
        iLocY,
    ...
    do {
      ...
      prevCost =
        gubWorldMovementCosts[trailTree[sCurPathNdx].sGridNo][
          trailTree[sCurPathNdx].stepDir
        ][ubLevel];
      ...
      if (
        trailCostUsed[curLoc] == gubGlobalPathCount &&
        trailCost[curLoc] < curCost
      ) {
        firstRun = false;
      }
      ...
      if (firstRun && fContinuousTurnNeeded) {
        if (trailTreeNdx < 2) {
          ...
          trailTree[pCurrPtr.sPathNdx].fFlags & Enum248.STEP_BACKWARDS
        ) {
      ...
      NEXTDIR: while (true) {
        if (firstRun) {
          ...
          if (iCnt == iLoopEnd) {
            ...
            fContinuousTurnNeeded &&
            iCnt == gOppositeDirection[iLoopStart]
          ) {
            fCheckedBehind = true;
          }
        }
      ...
      */
            ...
            if (iCnt != iLastDir) {
              if (
                !OkayToAddStructureToWorld(
                  curLoc,
                  ubLevel,
                  (<STRUCTURE_FILE_REF>pStructureFileRef).pDBStructureRef[
                    iStructIndex
                  ],
                  usOKToAddStructID,
                )
                ...
                if (bLoopState == LOOPING_CLOCKWISE) {
                  ...
                  bLoopState == LOOPING_COUNTERCLOCKWISE &&
                  !fCheckedBehind
                ) {
                  // check rear dir
        ...
        if (
          gpWorldLevelData[newLoc].sHeight != gpWorldLevelData[curLoc].sHeight
        ) {
          continue NEXTDIR;
        }
        ...
        // AI check for mines
        if (
          gpWorldLevelData[newLoc].uiFlags & MAPELEMENT_ENEMY_MINE_PRESENT &&
          s.bSide != 0
        ) {
          continue NEXTDIR;
        }
          ...
          if (nextCost == TRAVELCOST_HIDDENOBSTACLE) {
            if (fPathingForPlayer) {
              ...
              if (
                ...
                if (!fHiddenStructVisible) {
                  // Set cost of terrain!
                  nextCost =
                    gTileTypeMovementCost[gpWorldLevelData[newLoc].ubTerrainID];
                }
              }
            }
            ...
            switch (nextCost) {
              ...
              case TRAVELCOST_DOOR_OPEN_N_N:
                fDoorIsObstacleIfClosed = false;
                iDoorGridNo =
                  newLoc + dirDelta[Enum245.NORTH] + dirDelta[Enum245.NORTH];
                break;
              case TRAVELCOST_DOOR_OPEN_NW_N:
                fDoorIsObstacleIfClosed = false;
                iDoorGridNo =
                  newLoc +
                  dirDelta[Enum245.NORTHWEST] +
                  dirDelta[Enum245.NORTH];
                break;
              case TRAVELCOST_DOOR_OPEN_NE_N:
                fDoorIsObstacleIfClosed = false;
                iDoorGridNo =
                  newLoc +
                  dirDelta[Enum245.NORTHEAST] +
                  dirDelta[Enum245.NORTH];
                break;
              case TRAVELCOST_DOOR_OPEN_W_W:
                fDoorIsObstacleIfClosed = false;
                iDoorGridNo =
                  newLoc + dirDelta[Enum245.WEST] + dirDelta[Enum245.WEST];
                break;
              case TRAVELCOST_DOOR_OPEN_SW_W:
                fDoorIsObstacleIfClosed = false;
                iDoorGridNo =
                  newLoc + dirDelta[Enum245.SOUTHWEST] + dirDelta[Enum245.WEST];
                break;
              case TRAVELCOST_DOOR_OPEN_NW_W:
                fDoorIsObstacleIfClosed = false;
                iDoorGridNo =
                  newLoc + dirDelta[Enum245.NORTHWEST] + dirDelta[Enum245.WEST];
                break;
              default:
                break;
            }
            ...
            if (
              fPathingForPlayer &&
              gpWorldLevelData[iDoorGridNo].ubExtFlags[0] &
                  ...
                  if (
                    ...
                    if (pDoor) {
                      if (!pDoor.fLocked || s.bHasKeys) {
                        ...
                        nextCost =
                          gTileTypeMovementCost[
                            gpWorldLevelData[newLoc].ubTerrainID
                          ];
                      } else {
                        nextCost = TRAVELCOST_OBSTACLE;
                      }
            ...
            (nextCost == TRAVELCOST_SHORE ||
              nextCost == TRAVELCOST_KNEEDEEP ||
...
//						case TRAVELCOST_VEINMID	:
        ...
        */
        ...
        if (gubNPCAPBudget) {
          ...
          if (ubNewAPCost > gubNPCAPBudget) continue NEXTDIR;
        }
        ...
        if (newLoc == iDestination) nextCost = 0;
      ...
      */
        ...
        if (
          ...
          pNewPtr.usCostToGo = REMAININGCOST(
            iDestX,
            iDestY,
            iLocX,
            iLocY,
  ...
  export function GlobalItemsReachableTest(
    sStartGridNo1: INT16,
    sStartGridNo2: INT16,
  ): void {
    let s: SOLDIERTYPE = createSoldierType();
  ...
  export function RoofReachableTest(
    sStartGridNo: INT16,
    ubBuildingID: UINT8,
  ): void {
    let s: SOLDIERTYPE = createSoldierType();
  ...
  export function PlotPath(
    ...
    if (
      gfRecalculatingExistingPathCost ||
      FindBestPath(
        pSold,
        sDestGridno,
        pSold.bLevel,
      ...
      if (
        gubWorldMovementCosts[sTestGridno][guiPathingData[0]][pSold.bLevel] ==
        TRAVELCOST_FENCE
      ) {
        if (
          usMovementMode == Enum193.RUNNING &&
          pSold.usAnimState != Enum193.RUNNING
        ) {
      ...
      for (iCnt = 0; iCnt < iLastGrid; iCnt++) {
        ...
        // Get switch value...
        sSwitchValue =
          gubWorldMovementCosts[sTempGrid][guiPathingData[iCnt]][pSold.bLevel];

        // get the tile cost for that tile based on WALKING
        sTileCost = TerrainActionPoints(
          pSold,
          sTempGrid,
          guiPathingData[iCnt],
          pSold.bLevel,
        ...
        if (
          gpWorldLevelData[sTempGrid].ubTerrainID == Enum315.DEEP_WATER ||
          gpWorldLevelData[sTempGrid].ubTerrainID == Enum315.MED_WATER ||
          gpWorldLevelData[sTempGrid].ubTerrainID == Enum315.LOW_WATER
        ) {
          usMovementModeToUseForAPs = Enum193.WALKING;
        }
        ...
        // if ( gTacticalStatus.uiFlags & TURNBASED && (gTacticalStatus.uiFlags & INCOMBAT) ) // OR USER OPTION "show paths" ON... ***
        {
          if (
            bPlot &&
            (iCnt < iLastGrid - 1 || (iCnt < iLastGrid && bStayOn))
          ) {
  ...
  export function RecalculatePathCost(
  ...
  export function EstimatePlotPath(
    pSold: SOLDIERTYPE,
    sDestGridno: INT16,
    bCopyRoute: INT8,
    bPlot: INT8,
  ...
  export function InternalDoorTravelCost(
    pSoldier: SOLDIERTYPE | null,
    iGridNo: INT32,
    ubMovementCost: UINT8,
    fReturnPerceivedValue: boolean,
    piDoorGridNo: Pointer<INT32> | null,
    fReturnDoorCost: boolean,
  ): UINT8 {
    ...
    if (IS_TRAVELCOST_DOOR(ubMovementCost)) {
      ...
      switch (ubMovementCost) {
        ...
        case TRAVELCOST_DOOR_OPEN_N_N:
          fDoorIsObstacleIfClosed = false;
          iDoorGridNo =
            iGridNo + dirDelta[Enum245.NORTH] + dirDelta[Enum245.NORTH];
          break;
        case TRAVELCOST_DOOR_OPEN_NW_N:
          fDoorIsObstacleIfClosed = false;
          iDoorGridNo =
            iGridNo + dirDelta[Enum245.NORTHWEST] + dirDelta[Enum245.NORTH];
          break;
        case TRAVELCOST_DOOR_OPEN_NE_N:
          fDoorIsObstacleIfClosed = false;
          iDoorGridNo =
            iGridNo + dirDelta[Enum245.NORTHEAST] + dirDelta[Enum245.NORTH];
          break;
        case TRAVELCOST_DOOR_OPEN_W_W:
          fDoorIsObstacleIfClosed = false;
          iDoorGridNo =
            iGridNo + dirDelta[Enum245.WEST] + dirDelta[Enum245.WEST];
          break;
        case TRAVELCOST_DOOR_OPEN_SW_W:
          fDoorIsObstacleIfClosed = false;
          iDoorGridNo =
            iGridNo + dirDelta[Enum245.SOUTHWEST] + dirDelta[Enum245.WEST];
          break;
        case TRAVELCOST_DOOR_OPEN_NW_W:
          fDoorIsObstacleIfClosed = false;
          iDoorGridNo =
            iGridNo + dirDelta[Enum245.NORTHWEST] + dirDelta[Enum245.WEST];
          break;
        default:
          break;
      }
      ...
      if (fDoorIsOpen) {
        if (fDoorIsObstacleIfClosed) {
          ubMovementCost =
            gTileTypeMovementCost[gpWorldLevelData[iGridNo].ubTerrainID];
        } else {
        ...
        if (fDoorIsObstacleIfClosed) {
          ...
          if (
            ...
            if (pDoor) {
              if (
                (!pDoor.fLocked || (pSoldier && pSoldier.bHasKeys)) &&
                !fReturnDoorCost
              ) {
                ubMovementCost =
                  gTileTypeMovementCost[gpWorldLevelData[iGridNo].ubTerrainID];
  ...
  export function DoorTravelCost(
    pSoldier: SOLDIERTYPE | null,
    iGridNo: INT32,
    ubMovementCost: UINT8,
    fReturnPerceivedValue: boolean,
