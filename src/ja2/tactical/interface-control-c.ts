namespace ja2 {
  ...
  let gsUICurIntTileEffectGridNo__Pointer = createPointer(
    () => gsUICurIntTileEffectGridNo,
    (v) => (gsUICurIntTileEffectGridNo = v),
  );
  ...
  export function SetUpInterface(): void {
    ...
    if (gfUIHandleSelection == NONSELECTED_GUY_SELECTION) {
      if (gsSelectedLevel > 0) {
        ...
        (<LEVELNODE>gpWorldLevelData[gsSelectedGridNo].pRoofHead).ubShadeLevel =
          DEFAULT_SHADE_LEVEL;
        (<LEVELNODE>(
          gpWorldLevelData[gsSelectedGridNo].pRoofHead
        )).ubNaturalShadeLevel = DEFAULT_SHADE_LEVEL;
      } else {
        AddObjectToHead(gsSelectedGridNo, Enum312.GOODRING1);
        (<LEVELNODE>(
          gpWorldLevelData[gsSelectedGridNo].pObjectHead
    ...
    if (gfUIHandleShowMoveGrid) {
      if (gusSelectedSoldier != NOBODY) {
        if (
          MercPtrs[gusSelectedSoldier].sGridNo != gsUIHandleShowMoveGridLocation
        ) {
          if (gfUIHandleShowMoveGrid == 2) {
            AddTopmostToHead(
              gsUIHandleShowMoveGridLocation,
              GetSnapCursorIndex(Enum312.FIRSTPOINTERS4),
            );
            (<LEVELNODE>(
              gpWorldLevelData[gsUIHandleShowMoveGridLocation].pTopmostHead
            )).ubShadeLevel = DEFAULT_SHADE_LEVEL;
            (<LEVELNODE>(
              gpWorldLevelData[gsUIHandleShowMoveGridLocation].pTopmostHead
            )).ubNaturalShadeLevel = DEFAULT_SHADE_LEVEL;
            ...
            if (MercPtrs[gusSelectedSoldier].bStealthMode) {
              AddTopmostToHead(
                gsUIHandleShowMoveGridLocation,
                GetSnapCursorIndex(Enum312.FIRSTPOINTERS9),
              );
              (<LEVELNODE>(
                gpWorldLevelData[gsUIHandleShowMoveGridLocation].pTopmostHead
              )).ubShadeLevel = DEFAULT_SHADE_LEVEL;
              ...
              AddTopmostToHead(
                gsUIHandleShowMoveGridLocation,
                GetSnapCursorIndex(Enum312.FIRSTPOINTERS2),
              );
              (<LEVELNODE>(
                gpWorldLevelData[gsUIHandleShowMoveGridLocation].pTopmostHead
              )).ubShadeLevel = DEFAULT_SHADE_LEVEL;
    ...
    if (gfUIShowCurIntTile) {
      pIntTile = GetCurInteractiveTileGridNo(
        gsUICurIntTileEffectGridNo__Pointer,
      );
  ...
  export function RenderTopmostTacticalInterface(): void {
        ...
        if (MercPtrs[cnt].bActive) {
          if (MercPtrs[cnt].sPlannedTargetX != -1) {
            ...
            if (RenderTopmostTacticalInterface__uiBogTarget == 0) {
              ...
              RenderTopmostTacticalInterface__uiBogTarget =
            ...
            if (
              GridNoOnScreen(
                MAPROWCOLTOPOS(
                  Math.trunc(MercPtrs[cnt].sPlannedTargetY / CELL_Y_SIZE),
                  Math.trunc(MercPtrs[cnt].sPlannedTargetX / CELL_X_SIZE),
                ),
              )
            ) {
              ...
              ({ sScreenX: sTempX_S, sScreenY: sTempY_S } =
                FromCellToScreenCoordinates(sOffsetX, sOffsetY));
    ...
    if (gfUIInDeadlock) {
      ...
      gprintfdirty(
        0,
        300,
        "OPPONENT %d DEADLOCKED - 'Q' TO DEBUG, <ALT><ENTER> END OPP TURN",
        ...
        if (pSoldier.fDisplayDamage) {
          ...
          if (pSoldier.sGridNo != NOWHERE && pSoldier.bVisible != -1) {
            ...
            if (pSoldier.ubBodyType == Enum194.QUEENMONSTER) {
              sDamageX =
                sMercScreenX + pSoldier.sDamageX - pSoldier.sBoundingBoxOffsetX;
              sDamageY =
                sMercScreenY + pSoldier.sDamageY - pSoldier.sBoundingBoxOffsetY;

              sDamageX += 25;
              sDamageY += 10;
            } else {
              sDamageX =
                pSoldier.sDamageX + (sMercScreenX + Math.trunc((2 * 30) / 3));
    ...
    if (
      ...
      if (gfUIOverItemPool) {
        if ((pSoldier = GetSoldier(gusSelectedSoldier)) !== null) {
          ...
          if (
            (pItemPool = GetItemPool(gfUIOverItemPoolGridNo, pSoldier.bLevel))
          ) {
            ...
            // Check if we are over an item pool
            if (
              (pItemPool = GetItemPool(gfUIOverItemPoolGridNo, bCheckLevel))
            ) {
              let pStructure: STRUCTURE | null = null;
              let sIntTileGridNo: INT16 = 0;
              let bZLevel: INT8 = 0;
