namespace ja2 {
  ...
  export let uiExternalStaticNPCFaces: UINT32[] /* [NUMBER_OF_EXTERNAL_NPC_FACES] */ =
    createArray(Enum203.NUMBER_OF_EXTERNAL_NPC_FACES, 0);
  ...
  export let gpCurrentTalkingFace: FACETYPE | null /* Pointer<FACETYPE> */ =
    null;
  ...
  let gusSubtitleBoxWidth__Pointer = createPointer(
    () => gusSubtitleBoxWidth,
    (v) => (gusSubtitleBoxWidth = v),
  );
  let gusSubtitleBoxHeight: UINT16;
  let gusSubtitleBoxHeight__Pointer = createPointer(
    () => gusSubtitleBoxHeight,
    (v) => (gusSubtitleBoxHeight = v),
  );
  ...
  let gzQuoteStr__Pointer = createPointer(
    () => gzQuoteStr,
    (v) => (gzQuoteStr = v),
  );
  ...
  export function InitalizeStaticExternalNPCFaces(): void {
    ...
    for (
      iCounter = 0;
      iCounter < Enum203.NUMBER_OF_EXTERNAL_NPC_FACES;
      iCounter++
    ) {
      uiExternalStaticNPCFaces[iCounter] = InitFace(
        uiExternalFaceProfileIds[iCounter],
        NOBODY,
        FACE_FORCE_SMALL,
      );
    }
  ...
  export function HandleDialogue(): void {
      ...
      if (gpCurrentTalkingFace.fTalking) {
        ...
        if (
          ...
          if (gfFacePanelActive) {
            ...
            gpCurrentTalkingFace.fCanHandleInactiveNow = true;
            SetAutoFaceInActive(gpCurrentTalkingFace.iID);
            HandleTacticalSpeechUI(
              gubCurrentTalkingID,
              gpCurrentTalkingFace.iID,
            );
        ...
        if (pMike) {
          ...
          if (sPlayerGridNo != NOWHERE) {
            ...
            if (ubPlayerID != NOBODY) {
              ...
              gMercProfiles[pMike.ubProfile].ubMiscFlags2 |=
    ...
    if (QItem.uiSpecialEventFlag == 0) {
      ...
      QItem.uiSpecialEventFlag & DIALOGUE_SPECIAL_EVENT_LOCK_INTERFACE
    ) {
      ...
      gMercProfiles[QItem.uiSpecialEventData].ubMiscFlags &=
        ~PROFILE_MISC_FLAG_FORCENPCQUOTE;
      UnRecruitEPC(QItem.uiSpecialEventData);
      ReBuildCharactersList();
    } else if (
      QItem.uiSpecialEventFlag & DIALOGUE_SPECIAL_EVENT_CONTRACT_WANTS_TO_RENEW
    ) {
      HandleMercIsWillingToRenew(QItem.uiSpecialEventData);
    } else if (
      QItem.uiSpecialEventFlag & DIALOGUE_SPECIAL_EVENT_CONTRACT_NOGO_TO_RENEW
    ) {
      HandleMercIsNotWillingToRenew(QItem.uiSpecialEventData);
    } else {
      if (
        QItem.uiSpecialEventFlag & DIALOGUE_SPECIAL_EVENT_USE_ALTERNATE_FILES
      ) {
      ...
      // We could have a special flag, but dialogue as well
        ...
        QItem.uiSpecialEventFlag & DIALOGUE_SPECIAL_EVENT_SHOW_CONTRACT_MENU
      ) {
        // Setup face pointer
        ...
        QItem.uiSpecialEventFlag & DIALOGUE_SPECIAL_EVENT_DO_BATTLE_SND
      ) {
      ...
      if (
        QItem.uiSpecialEventFlag & DIALOGUE_ADD_EVENT_FOR_SOLDIER_UPDATE_BOX
      ) {
        let iReason: INT32 = 0;
      ...
      if (QItem.uiSpecialEventFlag & DIALOGUE_SPECIAL_EVENT_SHOPKEEPER) {
        ...
        switch (QItem.uiSpecialEventData) {
          case 0:
            zText = swprintf(
              SkiMessageBoxText[Enum370.SKI_SHORT_FUNDS_TEXT],
              zMoney,
            );
      ...
      if (QItem.uiSpecialEventFlag & DIALOGUE_SPECIAL_EVENT_EXIT_MAP_SCREEN) {
        ...
        if (pSoldier) {
          ...
          wTempString = BuildStatChangeString(
            pSoldier.name,
            QItem.uiSpecialEventData,
            QItem.uiSpecialEventData2,
            QItem.uiSpecialEventData3,
        ...
        QItem.uiSpecialEventFlag & DIALOGUE_SPECIAL_EVENT_UNSET_ARRIVES_FLAG
      ) {
    ...
    */
        QItem.uiSpecialEventFlag & DIALOGUE_SPECIAL_EVENT_SKYRIDERMAPSCREENEVENT
      ) {
        // Setup face pointer
        gpCurrentTalkingFace = gFacesData[QItem.iFaceIndex];
        gubCurrentTalkingID = QItem.ubCharacterNum;
      ...
      if (QItem.uiSpecialEventFlag & DIALOGUE_SPECIAL_EVENT_GIVE_ITEM) {
        ...
        QItem.uiSpecialEventFlag & DIALOGUE_SPECIAL_EVENT_TRIGGER_NPC
      ) {
        if (QItem.bUIHandlerID == DIALOGUE_NPC_UI) {
          HandleNPCTriggerNPC(
            QItem.uiSpecialEventData,
            QItem.uiSpecialEventData2,
            QItem.uiSpecialEventData3,
            QItem.uiSpecialEventData4,
          );
        }
      } else if (
        QItem.uiSpecialEventFlag & DIALOGUE_SPECIAL_EVENT_GOTO_GRIDNO
      ) {
        if (QItem.bUIHandlerID == DIALOGUE_NPC_UI) {
          HandleNPCGotoGridNo(
            QItem.uiSpecialEventData,
        ...
        QItem.uiSpecialEventFlag & DIALOGUE_SPECIAL_EVENT_CLOSE_PANEL
      ) {
        if (QItem.bUIHandlerID == DIALOGUE_NPC_UI) {
          HandleNPCClosePanel();
        }
      } else if (
        QItem.uiSpecialEventFlag & DIALOGUE_SPECIAL_EVENT_SHOW_UPDATE_MENU
      ) {
        ...
        QItem.uiSpecialEventFlag & DIALOGUE_SPECIAL_EVENT_ENTER_MAPSCREEN
      ) {
        ...
        QItem.uiSpecialEventFlag & DIALOGUE_SPECIAL_EVENT_CONTRACT_ENDING
      ) {
        // grab soldier ptr from profile ID
        pSoldier = FindSoldierByProfileID(QItem.ubCharacterNum, false);
        ...
        QItem.uiSpecialEventFlag & DIALOGUE_SPECIAL_EVENT_MULTIPURPOSE
      ) {
        if (
          QItem.uiSpecialEventData &
  ...
  export function DelayedTacticalCharacterDialogue(
    ...
    if (
      ...
      usQuoteNum != Enum202.QUOTE_SERIOUSLY_WOUNDED
    )
      return false;
    ...
    return CharacterDialogue(
      ...
      usQuoteNum,
      pSoldier.iFaceIndex,
      DIALOGUE_TACTICAL_UI,
      true,
      true,
    );
  }

  export function TacticalCharacterDialogueWithSpecialEvent(
    pSoldier: SOLDIERTYPE,
    usQuoteNum: UINT16,
    uiFlag: UINT32,
    uiData1: any,
    ...
    if (
      uiFlag != DIALOGUE_SPECIAL_EVENT_DO_BATTLE_SND &&
      uiData1 != Enum259.BATTLE_SOUND_DIE1
    ) {
      if (pSoldier.bLife < CONSCIOUSNESS) return false;
    ...
    return CharacterDialogueWithSpecialEvent(
      pSoldier.ubProfile,
      usQuoteNum,
      pSoldier.iFaceIndex,
      DIALOGUE_TACTICAL_UI,
  ...
  export function TacticalCharacterDialogueWithSpecialEventEx(
    pSoldier: SOLDIERTYPE,
    usQuoteNum: UINT16,
    uiFlag: UINT32,
    uiData1: UINT32,
    ...
    return CharacterDialogueWithSpecialEventEx(
      pSoldier.ubProfile,
      usQuoteNum,
      pSoldier.iFaceIndex,
      DIALOGUE_TACTICAL_UI,
  ...
  export function TacticalCharacterDialogue(
    pSoldier: SOLDIERTYPE,
    usQuoteNum: UINT16,
  ): boolean {
    if (pSoldier.ubProfile == NO_PROFILE) {
    ...
    if (
      pSoldier.ubProfile == gTacticalStatus.ubLastQuoteProfileNUm &&
    ...
    if (pSoldier.uiStatusFlags & SOLDIER_ROBOT) {
      if (CanRobotBeControlled(pSoldier)) {
        return TacticalCharacterDialogue(
          MercPtrs[pSoldier.ubRobotRemoteHolderID],
          usQuoteNum,
        );
      } else {
        return false;
  ...
  export function CharacterDialogueWithSpecialEvent(
    ubCharacterNum: UINT8,
    usQuoteNum: UINT16,
    iFaceIndex: INT32,
    bUIHandlerID: UINT8,
  ...
  function CharacterDialogueWithSpecialEventEx(
    ubCharacterNum: UINT8,
    usQuoteNum: UINT16,
    iFaceIndex: INT32,
    bUIHandlerID: UINT8,
  ...
  export function SpecialCharacterDialogueEvent(
    uiSpecialEventFlag: UINT32,
    uiSpecialEventData1: any,
    uiSpecialEventData2: any,
    uiSpecialEventData3: any,
    iFaceIndex: INT32,
    bUIHandlerID: UINT8,
  ): boolean {
    let QItem: DIALOGUE_Q_STRUCT;
  ...
  export function SpecialCharacterDialogueEventWithExtraParam(
    uiSpecialEventFlag: UINT32,
    uiSpecialEventData1: any,
    uiSpecialEventData2: any,
    uiSpecialEventData3: any,
    uiSpecialEventData4: any,
    iFaceIndex: INT32,
    bUIHandlerID: UINT8,
  ): boolean {
    let QItem: DIALOGUE_Q_STRUCT;
  ...
  function ExecuteCharacterDialogue(
    ubCharacterNum: UINT8,
    usQuoteNum: UINT16,
    iFaceIndex: INT32,
    bUIHandlerID: UINT8,
    ...
    if (pSoldier != null) {
      ...
      if (pSoldier.fMercAsleep == true) {
        ...
        if (
          usQuoteNum == Enum202.QUOTE_NEED_SLEEP ||
          usQuoteNum == Enum202.QUOTE_OUT_OF_BREATH
        ) {
          // leave them alone
          return true;
        }
  ...
  function CreateTalkingUI(
    bUIHandlerID: INT8,
    iFaceIndex: INT32,
    ubCharacterNum: UINT8,
    pSoldier: SOLDIERTYPE,
    zQuoteStr: string /* Pointer<INT16> */,
  ): void {
    // Show text, if on
    if (
      gGameSettings.fOptions[Enum8.TOPTION_SUBTITLES] ||
      !gFacesData[iFaceIndex].fValidSpeech
    ) {
      switch (bUIHandlerID) {
  ...
  function GetDialogueDataFilename(
    ubCharacterNum: UINT8,
    usQuoteNum: UINT16,
    fWavFile: boolean,
  ): string /* Pointer<INT8> */ {
    ...
    if (gfUseAlternateDialogueFile) {
      if (fWavFile) {
        ...
        // #else
        GetDialogueDataFilename__zFileName = sprintf(
          "NPC_SPEECH\\d_%s_%s.wav",
          ubCharacterNum.toString().padStart(3, "0"),
          usQuoteNum.toString().padStart(3, "0"),
        );
        // #endif
      } else {
      ...
      (!(
        ...
        gMercProfiles[ubCharacterNum].ubMiscFlags &
          PROFILE_MISC_FLAG_FORCENPCQUOTE)
    ) {
      ...
      if (fWavFile) {
        GetDialogueDataFilename__zFileName = sprintf(
          "NPC_SPEECH\\%s_%s.wav",
          ubFileNumID.toString().padStart(3, "0"),
          usQuoteNum.toString().padStart(3, "0"),
        );
      } else {
        // assume EDT files are in EDT directory on HARD DRIVE
  ...
  export function DialogueDataFileExistsForProfile(
    ubCharacterNum: UINT8,
    usQuoteNum: UINT16,
    fWavFile: boolean,
    ppStr: Pointer<string> | null,
  ): boolean {
  ...
  function GetDialogue(
    ...
    usQuoteNum: UINT16,
    iDataSize: UINT32,
    zDialogueText: Pointer<string> /* Pointer<UINT16> */,
    puiSoundID: Pointer<UINT32>,
    zSoundString: Pointer<string> /* Pointer<CHAR8> */,
  ): boolean {
    let pFilename: string /* Pointer<UINT8> */ = "";
    ...
    // if ( gGameSettings.fOptions[ TOPTION_SUBTITLES ] )
    {
      if (
        ...
        zDialogueText.value = LoadEncryptedDataFromFile(
          pFilename,
          usQuoteNum * iDataSize,
          iDataSize,
        );
        if (zDialogueText.value == "") {
          zDialogueText.value = swprintf(
            "I have no text in the EDT file ( %d ) %s",
            usQuoteNum,
            pFilename,
          );
        ...
        zDialogueText.value = swprintf(
          "I have no text in the file ( %d ) %s",
          usQuoteNum,
          pFilename,
        );
  ...
  */
  ...
  function HandleTacticalNPCTextUI(
    ubCharacterNum: UINT8,
    zQuoteStr: string /* Pointer<INT16> */,
  ): void {
    ...
    zText = swprintf(
      '%s: "%s"',
      gMercProfiles[ubCharacterNum].zNickname,
      zQuoteStr,
    );
  ...
  // Handlers for tactical UI stuff
  function DisplayTextForExternalNPC(
    ubCharacterNum: UINT8,
    zQuoteStr: string /* STR16 */,
  ): void {
    let zText: string /* INT16[QUOTE_MESSAGE_SIZE] */;
    let sLeft: INT16;
  ...
  function ExecuteTacticalTextBoxForLastQuote(
    sLeftPosition: INT16,
  ...
  function ExecuteTacticalTextBox(
    sLeftPosition: INT16,
    pString: string /* STR16 */,
  ): void {
    ...
    iDialogueBox = PrepareMercPopupBox(
      ...
      pString,
      DIALOGUE_DEFAULT_SUBTITLE_WIDTH,
      0,
      0,
      0,
      gusSubtitleBoxWidth__Pointer,
      gusSubtitleBoxHeight__Pointer,
    );
  ...
  function HandleTacticalSpeechUI(
    ubCharacterNum: UINT8,
    iFaceIndex: INT32,
  ): void {
    let VideoOverlayDesc: VIDEO_OVERLAY_DESC = createVideoOverlayDesc();
    ...
    if (fDoExternPanel) {
      ...
      // Set flag to say WE control when to set inactive!
      gFacesData[iFaceIndex].uiFlags |=
        FACE_INACTIVE_HANDLED_ELSEWHERE | FACE_MAKEACTIVE_ONCE_DONE;
    ...
    if (gGameSettings.fOptions[Enum8.TOPTION_SPEECH]) {
      ...
      if (pFace.fTalking) {
        ScreenMsg(
          FONT_MCOLOR_LTYELLOW,
          MSG_TESTVERSION,
          "HandleDialogueEnd() face still talking.",
        );
        return;
      }
  ...
  function RenderFaceOverlay(pBlitter: VIDEO_OVERLAY): void {
    ...
    if (gfFacePanelActive) {
      pSoldier = FindSoldierByProfileID(
        gpCurrentTalkingFace.ubCharacterNum,
        false,
      );
      ...
      pSrcBuf = LockVideoSurface(
        gpCurrentTalkingFace.uiAutoDisplayBuffer,
        createPointer(
          () => uiSrcPitchBYTES,
          (v) => (uiSrcPitchBYTES = v),
        ),
      ...
      Blt16BPPTo16BPP(
        ...
        0,
        0,
        gpCurrentTalkingFace.usFaceWidth,
        gpCurrentTalkingFace.usFaceHeight,
      );
  ...
  function RenderSubtitleBoxOverlay(pBlitter: VIDEO_OVERLAY): void {
    if (giTextBoxOverlay != -1) {
      ...
      InvalidateRegion(
        pBlitter.sX,
        pBlitter.sY,
        pBlitter.sX + gusSubtitleBoxWidth,
    ...
    for (
      ...
      if (
        ...
        if (gTacticalStatus.bNumFoughtInBattle[ENEMY_TEAM] == 0) {
          ...
          if (
            usQuoteNum == Enum202.QUOTE_SECTOR_SAFE &&
            (pTeamSoldier.ubProfile == Enum268.IRA ||
              pTeamSoldier.ubProfile == Enum268.MIGUEL ||
              pTeamSoldier.ubProfile == Enum268.SHANK)
          ) {
            continue;
          }
          if (
            usQuoteNum == Enum202.QUOTE_ENEMY_PRESENCE &&
            (pTeamSoldier.ubProfile == Enum268.IRA ||
              pTeamSoldier.ubProfile == Enum268.DIMITRI ||
              pTeamSoldier.ubProfile == Enum268.DYNAMO ||
              pTeamSoldier.ubProfile == Enum268.SHANK)
          ) {
            continue;
          }
  ...
  export function SayQuoteFromAnyBodyInThisSector(
    sSectorX: INT16,
    sSectorY: INT16,
    bSectorZ: INT8,
    usQuoteNum: UINT16,
    ...
    for (
      ...
      if (pTeamSoldier.bActive) {
        // Add guy if he's a candidate...
        if (
          pTeamSoldier.sSectorX == sSectorX &&
          pTeamSoldier.sSectorY == sSectorY &&
          pTeamSoldier.bSectorZ == bSectorZ &&
          !AM_AN_EPC(pTeamSoldier) &&
          !(pTeamSoldier.uiStatusFlags & SOLDIER_GASSED) &&
          !AM_A_ROBOT(pTeamSoldier) &&
          !pTeamSoldier.fMercAsleep
  ...
  export function SayQuoteFromNearbyMercInSector(
    sGridNo: INT16,
    bDistance: INT8,
    usQuoteNum: UINT16,
  ): void {
  ...
  export function SayQuote58FromNearbyMercInSector(
    sGridNo: INT16,
    bDistance: INT8,
    usQuoteNum: UINT16,
    bSex: INT8,
    ...
    for (
      ...
      if (
        ...
        if (
          QuoteExp_GenderCode[pTeamSoldier.ubProfile] == 0 &&
          bSex == Enum272.FEMALE
        ) {
          continue;
        }
        ...
        if (
          QuoteExp_GenderCode[pTeamSoldier.ubProfile] == 1 &&
          bSex == Enum272.MALE
        ) {
          continue;
        }
  ...
  function TextOverlayClickCallback(
  ...
  function FaceOverlayClickCallback(
    pRegion: MOUSE_REGION,
    iReason: INT32,
  ): void {
    /* static */ let fLButtonDown: boolean = false;
  ...
  function ShouldMercSayPrecedentToRepeatOneSelf(
    ubMercID: UINT8,
    uiQuoteID: UINT32,
  ): boolean {
  ...
  export function GetMercPrecedentQuoteBitStatus(
    ubMercID: UINT8,
    ubQuoteBit: UINT8,
  ): boolean {
  ...
  export function SetMercPrecedentQuoteBitStatus(
    ubMercID: UINT8,
    ubBitToSet: UINT8,
  ): boolean {
    // Set the bit
  ...
  function IsQuoteInPrecedentArray(uiQuoteID: UINT32): boolean {
    ...
    if (
      uiQuoteID < gubMercValidPrecedentQuoteID[0] ||
      uiQuoteID >
        gubMercValidPrecedentQuoteID[NUMBER_VALID_MERC_PRECEDENT_QUOTES - 1]
    ) {
      return false;
    }
  ...
  export function HandleImportantMercQuote(
    pSoldier: SOLDIERTYPE,
    usQuoteNumber: UINT16,
  ): void {
  ...
  export function SetExternMapscreenSpeechPanelXY(
    sXPos: INT16,
    sYPos: INT16,
  ): void {
