namespace ja2 {
  ...
  export let gRottingCorpse: ROTTING_CORPSE[] /* [MAX_ROTTING_CORPSES] */ =
    createArrayFrom(MAX_ROTTING_CORPSES, createRottingCorpse);
  ...
  function GetFreeRottingCorpse(): INT32 {
    ...
    for (iCount = 0; iCount < giNumRottingCorpse; iCount++) {
      if (gRottingCorpse[iCount].fActivated == false) return iCount;
    }

    if (giNumRottingCorpse < MAX_ROTTING_CORPSES) return giNumRottingCorpse++;
  ...
  export function GetCorpseStructIndex(
    pCorpseDef: ROTTING_CORPSE_DEFINITION,
    fForImage: boolean,
  ): UINT16 {
    let bDirection: INT8;
  ...
  export function AddRottingCorpse(
    pCorpseDef: ROTTING_CORPSE_DEFINITION,
  ): INT32 {
    let iIndex: INT32;
    ...
    if (!(gTacticalStatus.uiFlags & LOADING_SAVED_GAME)) {
      ...
      if (
        GetWorldTotalMin() - pCorpse.def.uiTimeOfDeath > DELAY_UNTIL_ROTTING &&
      ...
      if (
        GetWorldTotalMin() - pCorpse.def.uiTimeOfDeath >
        DELAY_UNTIL_DONE_ROTTING
      ) {
        return -1;
      }
    }
    ...
    AniParams.uiFlags =
      ANITILE_CACHEDTILE |
      ANITILE_PAUSED |
      ANITILE_OPTIMIZEFORSLOWMOVING |
      ANITILE_ANIMATE_Z |
      ANITILE_ERASEITEMFROMSAVEBUFFFER |
      uiDirectionUseFlag;
    ...
    pCorpse.pAniTile.pLevelNode.ubShadeLevel = (<LEVELNODE>(
      gpWorldLevelData[pCorpse.def.sGridNo].pLandHead
    )).ubShadeLevel;
    pCorpse.pAniTile.pLevelNode.ubSumLights = (<LEVELNODE>(
      gpWorldLevelData[pCorpse.def.sGridNo].pLandHead
    )).ubSumLights;
    pCorpse.pAniTile.pLevelNode.ubMaxLights = (<LEVELNODE>(
      gpWorldLevelData[pCorpse.def.sGridNo].pLandHead
    )).ubMaxLights;
    pCorpse.pAniTile.pLevelNode.ubNaturalShadeLevel = (<LEVELNODE>(
      gpWorldLevelData[pCorpse.def.sGridNo].pLandHead
    )).ubNaturalShadeLevel;
    ...
    // Add structure data.....
    CheckForAndAddTileCacheStructInfo(
      pCorpse.pAniTile.pLevelNode,
      pCorpse.def.sGridNo,
      pCorpse.iCachedTileID,
  ...
  function CreateCorpsePalette(pCorpse: ROTTING_CORPSE): boolean {
    ...
    if (bBodyTypePalette == -1) {
      ...
      copyObjectArray(
        pCorpse.p8BPPPalette,
        (<TILE_IMAGERY>gpTileCache[pCorpse.iCachedTileID].pImagery).vo
          .pPaletteEntry,
        copySGPPaletteEntry,
      );
  ...
  export function TurnSoldierIntoCorpse(
    pSoldier: SOLDIERTYPE,
    fRemoveMerc: boolean,
    fCheckForLOS: boolean,
  ): boolean {
    ...
    if (pSoldier.uiStatusFlags & SOLDIER_VEHICLE) {
      ...
      if (
        pSoldier.ubBodyType != Enum194.ICECREAMTRUCK &&
        pSoldier.ubBodyType != Enum194.HUMVEE
      ) {
        Corpse.bDirection = 7;
      } else {
    ...
    if (
      ubType == Enum249.QUEEN_MONSTER_DEAD ||
      ubType == Enum249.BURNT_DEAD ||
      ubType == Enum249.EXPLODE_DEAD
    ) {
      Corpse.bDirection = 7;
    }
  ...
  function AddCrowToCorpse(pCorpse: ROTTING_CORPSE): void {
    ...
    if (
      ...
      sGridNo = FindRandomGridNoFromSweetSpot(
        pSoldier,
        pCorpse.def.sGridNo,
        2,
        createPointer(
  ...
  export function HandleRottingCorpses(): void {
    ...
    // Couint how many we have now...
    {
      ...
      for (
        bLoop = gTacticalStatus.Team[CIV_TEAM].bFirstID,
          pSoldier = MercPtrs[bLoop];
        bLoop <= gTacticalStatus.Team[CIV_TEAM].bLastID;
  ...
  function MakeCorpseVisible(
    pSoldier: SOLDIERTYPE,
    pCorpse: ROTTING_CORPSE,
  ): void {
    pCorpse.def.bVisible = 1;
    SetRenderFlags(RENDER_FLAG_FULL);
  }

  function AllMercsOnTeamLookForCorpse(
    pCorpse: ROTTING_CORPSE,
    bTeam: INT8,
  ): void {
    let cnt: INT32;
  ...
  function CreateCorpsePaletteTables(pCorpse: ROTTING_CORPSE): boolean {
    ...
    for (uiCount = 0; uiCount < 256; uiCount++) {
      ...
      LightPal[uiCount].peRed = Math.min(
        pCorpse.p8BPPPalette[uiCount].peRed + gpLightColors[0].peRed,
        255,
      );
      LightPal[uiCount].peGreen = Math.min(
        pCorpse.p8BPPPalette[uiCount].peGreen + gpLightColors[0].peGreen,
        255,
      );
      LightPal[uiCount].peBlue = Math.min(
        pCorpse.p8BPPPalette[uiCount].peBlue + gpLightColors[0].peBlue,
        255,
      );
    }
  ...
  function CreateCorpseShadedPalette(
    pCorpse: ROTTING_CORPSE,
    uiBase: UINT32,
    ...
    for (uiCount = 1; uiCount < 16; uiCount++) {
      pCorpse.pShades[uiBase + uiCount] = Create16BPPPaletteShaded(
        pShadePal,
  ...
  function FindCorpseBasedOnStructure(
    sGridNo: INT16,
  ...
  export function FindNearestAvailableGridNoForCorpse(
    pDef: ROTTING_CORPSE_DEFINITION,
    ubRadius: INT8,
  ): INT16 {
  ...
  export function GetCorpseAtGridNo(
    sGridNo: INT16,
    bLevel: INT8,
  ): ROTTING_CORPSE | null {
  ...
  export function DecapitateCorpse(
    pSoldier: SOLDIERTYPE,
    sGridNo: INT16,
    bLevel: INT8,
  ): void {
  ...
  export function ReduceAmmoDroppedByNonPlayerSoldiers(
    pSoldier: SOLDIERTYPE,
    iInvSlot: INT32,
  ): void {
    let pObj: OBJECTTYPE;
  ...
  export function LookForAndMayCommentOnSeeingCorpse(
    pSoldier: SOLDIERTYPE,
    sGridNo: INT16,
    ubLevel: UINT8,
  ): void {
