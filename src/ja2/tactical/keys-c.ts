namespace ja2 {
  let gpDoorStatus: DOOR_STATUS[] /* Pointer<DOOR_STATUS> */ = <DOOR_STATUS[]>(
    (<unknown>null)
  );
  ...
  KeyTable = KeyTable.concat(
    createArrayFrom(NUM_KEYS - KeyTable.length, createKey),
  );
  ...
  export let LockTable: LOCK[] /* [NUM_LOCKS] */ = createArrayFrom(
    NUM_LOCKS,
    createLock,
  );
...
*/
  ...
  export function LoadLockTable(): boolean {
    ...
    if (!hFile) {
      DebugMsg(
        TOPIC_JA2,
        DBG_LEVEL_3,
        FormatString("FAILED to LoadLockTable from file %s", pFileName),
      );
      return false;
    }
  ...
  export function KeyExistsInKeyRing(
    pSoldier: SOLDIERTYPE,
    ubKeyID: UINT8,
    pubPos: Pointer<UINT8> | null,
  ): boolean {
  ...
  function DoUnlockDoor(pDoor: DOOR, ubKeyID: UINT8): boolean {
    ...
    if (pDoor.fLocked && ValidKey(pDoor, ubKeyID)) {
      // Play lockpicking
      PlayJA2Sample(
        Enum330.UNLOCK_DOOR_1,
        RATE_11025,
        SoundVolume(MIDVOLUME, pDoor.sGridNo),
        1,
  ...
  export function AttemptToUnlockDoor(
    pSoldier: SOLDIERTYPE,
    pDoor: DOOR,
  ): boolean {
    let ubLoop: UINT8;
  ...
  export function AttemptToLockDoor(
    pSoldier: SOLDIERTYPE,
    pDoor: DOOR,
  ): boolean {
    let ubLoop: UINT8;
  ...
  export function AttemptToCrowbarLock(
    pSoldier: SOLDIERTYPE,
    pDoor: DOOR,
  ): boolean {
    let iResult: INT32;
    ...
    bStress = Math.min(
      EffectiveStrength(pSoldier),
      LockTable[pDoor.ubLockID].ubSmashDifficulty + 30,
    );
    ...
    if (LockTable[pDoor.ubLockID].ubSmashDifficulty == OPENING_NOT_POSSIBLE) {
      ...
      iResult = SkillCheck(
        pSoldier,
        Enum255.OPEN_WITH_CROWBAR,
        -(LockTable[pDoor.ubLockID].ubSmashDifficulty - pDoor.bLockDamage),
      );
    }
  ...
  export function AttemptToSmashDoor(
    pSoldier: SOLDIERTYPE,
    pDoor: DOOR,
  ): boolean {
    let iResult: INT32;
  ...
  export function AttemptToPickLock(
    pSoldier: SOLDIERTYPE,
    pDoor: DOOR,
  ): boolean {
    let iResult: INT32;
    ...
    if (iResult > 0) {
      ...
      StatChange(
        pSoldier,
        MECHANAMT,
        Math.trunc(pLock.ubPickDifficulty / 5),
        0,
      );
  ...
  export function AttemptToUntrapDoor(
    pSoldier: SOLDIERTYPE,
    pDoor: DOOR,
  ): boolean {
    let iResult: INT32;
    ...
    if (pDoor.ubTrapID == Enum227.EXPLOSION) {
      ...
      iResult = SkillCheck(
        pSoldier,
        Enum255.DISARM_ELECTRONIC_TRAP_CHECK,
        pDoor.ubTrapLevel * 7,
      );
    }
  ...
  export function ExamineDoorForTraps(
    pSoldier: SOLDIERTYPE,
    pDoor: DOOR,
  ): boolean {
  ...
  export function HasDoorTrapGoneOff(
    pSoldier: SOLDIERTYPE,
    pDoor: DOOR,
  ): boolean {
      ...
      switch (pDoor.ubTrapID) {
        case Enum227.BROTHEL_SIREN:
          ScreenMsg(
            MSG_FONT_YELLOW,
            MSG_INTERFACE,
            TacticalStr[Enum335.LOCK_TRAP_HAS_GONE_OFF_STR],
            pDoorTrapStrings[Enum227.SIREN],
          );
          break;
        case Enum227.SUPER_ELECTRIC:
          ScreenMsg(
            ...
            MSG_INTERFACE,
            TacticalStr[Enum335.LOCK_TRAP_HAS_GONE_OFF_STR],
            pDoorTrapStrings[Enum227.ELECTRIC],
          );
          break;
        default:
          ScreenMsg(
  ...
  export function AttemptToBlowUpLock(
    pSoldier: SOLDIERTYPE,
    pDoor: DOOR,
  ): boolean {
    let iResult: INT32;
    ...
    if (iResult >= -20) {
      ...
      pDoor.bLockDamage +=
        Explosive[Item[Enum225.SHAPED_CHARGE].ubClassIndex].ubDamage * 2;
  ...
  export function SaveDoorTableToDoorTableTempFile(
    sSectorX: INT16,
    sSectorY: INT16,
    bSectorZ: INT8,
  ): boolean {
  ...
  export function ModifyDoorStatus(
    sGridNo: INT16,
    fOpen: boolean | undefined,
    fPerceivedOpen: boolean | undefined,
  ): boolean {
    let ubCnt: UINT8;
      ...
      for (ubCnt = 0; ubCnt < gubNumDoorStatus; ubCnt++) {
        ...
        if (gpDoorStatus[ubCnt].sGridNo == pBaseStructure.sGridNo) {
          ...
          if (fPerceivedOpen != DONTSETDOORSTATUS) {
            ...
            else gpDoorStatus[ubCnt].ubFlags &= ~DOOR_PERCEIVED_OPEN;
          ...
          if (fOpen != DONTSETDOORSTATUS) {
            if (fOpen) gpDoorStatus[ubCnt].ubFlags |= DOOR_OPEN;
            else gpDoorStatus[ubCnt].ubFlags &= ~DOOR_OPEN;
          }
    ...
    gpWorldLevelData[pBaseStructure.sGridNo].ubExtFlags[0] |=
  ...
  function IsDoorOpen(sGridNo: INT16): boolean {
    ...
    if (gpDoorStatus) {
      ...
      for (ubCnt = 0; ubCnt < gubNumDoorStatus; ubCnt++) {
        // if this is the door
        if (gpDoorStatus[ubCnt].sGridNo == pBaseStructure.sGridNo) {
          if (gpDoorStatus[ubCnt].ubFlags & DOOR_OPEN) return true;
          else return false;
        }
      }
    }
  ...
  export function AllMercsLookForDoor(
    sGridNo: INT16,
    fUpdateValue: boolean,
  ): boolean {
    let cnt: INT32;
    let cnt2: INT32;
    let bDirs: INT8[] /* [8] */ = [
  ...
  export function MercLooksForDoors(
    pSoldier: SOLDIERTYPE,
    fUpdateValue: boolean,
  ): boolean {
    let cnt: INT32;
  ...
  export function UpdateDoorGraphicsFromStatus(
    fUsePerceivedStatus: boolean,
    fDirty: boolean,
  ): void {
    let cnt: INT32;
    let pDoorStatus: DOOR_STATUS;
    ...
    for (cnt = 0; cnt < gubNumDoorStatus; cnt++) {
      ...
      InternalUpdateDoorGraphicFromStatus(
        pDoorStatus,
        fUsePerceivedStatus,
        fDirty,
      );
  ...
  function InternalUpdateDoorGraphicFromStatus(
    pDoorStatus: DOOR_STATUS,
    fUsePerceivedStatus: boolean,
    fDirty: boolean,
  ): void {
    let pStructure: STRUCTURE | null;
  ...
  function InternalIsPerceivedDifferentThanReality(
    pDoorStatus: DOOR_STATUS,
  ): boolean {
  ...
  function InternalSetDoorPerceivedOpenStatus(
    pDoorStatus: DOOR_STATUS,
    fPerceivedOpen: boolean,
  ): boolean {
    if (fPerceivedOpen) pDoorStatus.ubFlags |= DOOR_PERCEIVED_OPEN;
    else pDoorStatus.ubFlags &= ~DOOR_PERCEIVED_OPEN;
  ...
  function SetDoorPerceivedOpenStatus(
    sGridNo: INT16,
    fPerceivedOpen: boolean,
  ): boolean {
    let pDoorStatus: DOOR_STATUS | null = null;
  ...
  export function SaveDoorStatusArrayToDoorStatusTempFile(
    sSectorX: INT16,
    sSectorY: INT16,
    bSectorZ: INT8,
  ): boolean {
    ...
    if (gubNumDoorStatus != 0) {
      ...
      uiNumBytesWritten = FileWrite(
        hFile,
        buffer,
        DOOR_STATUS_SIZE * gubNumDoorStatus,
      );
  ...
  export function DropKeysInKeyRing(
    pSoldier: SOLDIERTYPE,
    sGridNo: INT16,
    bLevel: INT8,
    bVisible: INT8,
    fAddToDropList: boolean,
    iDropListSlot: INT32,
    fUseUnLoaded: boolean,
  ): void {
    let ubLoop: UINT8;
    let ubItem: UINT8;
