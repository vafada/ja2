namespace ja2 {
  ...
  let gpUIPlannedSoldier: SOLDIERTYPE /* Pointer<SOLDIERTYPE> */ = <
    SOLDIERTYPE
  >(<unknown>null);
  let gpUIStartPlannedSoldier: SOLDIERTYPE /* Pointer<SOLDIERTYPE> */ = <
    SOLDIERTYPE
  >(<unknown>null);
  ...
  export function AddUIPlan(sGridNo: UINT16, ubPlanID: UINT8): boolean {
    ...
    if (
      !NewOKDestination(gpUIPlannedSoldier, sGridNo, false, gsInterfaceLevel)
    ) {
      return false;
    }
    ...
    if (ubPlanID == UIPLAN_ACTION_MOVETO) {
      ...
      sAPCost = PlotPath(
        ...
        NO_PLOT,
        TEMPORARY,
        gpUIPlannedSoldier.usUIMovementMode,
        NOT_STEALTH,
        FORWARD,
      ...
      if (EnoughPoints(gpUIPlannedSoldier, sAPCost, 0, false)) {
        ...
        if (
          ...
          pPlanSoldier.bActionPoints =
            gpUIPlannedSoldier.bActionPoints - sAPCost;

          pPlanSoldier.ubPlannedUIAPCost = pPlanSoldier.bActionPoints;

          // Get direction
          bDirection =
            gpUIPlannedSoldier.usPathingData[
              gpUIPlannedSoldier.usPathDataSize - 1
            ];
        ...
        ScreenMsg(
          FONT_MCOLOR_LTYELLOW,
          MSG_INTERFACE,
          "Merc will not have enough action points",
        );
      }
      ...
      sAPCost = CalcTotalAPsToAttack(
        ...
        sGridNo,
        1,
        Math.trunc(gpUIPlannedSoldier.bShownAimTime / 2),
      );
      ...
      if (EnoughPoints(gpUIPlannedSoldier, sAPCost, 0, false)) {
        ...
        if (gpUIPlannedSoldier.ubID < MAX_NUM_SOLDIERS) {
          ...
          if (
            ...
            for (iLoop = 0; iLoop < MAX_NUM_SOLDIERS; iLoop++) {
              if (gpUIPlannedSoldier.bOppList[iLoop] < 0) {
                pPlanSoldier.bOppList[iLoop] =
                  gpUIPlannedSoldier.bOppList[iLoop];
              }
            }

            EVENT_SetSoldierPosition(
              pPlanSoldier,
              gpUIPlannedSoldier.dXPos,
              gpUIPlannedSoldier.dYPos,
            );
            EVENT_SetSoldierDestination(
              pPlanSoldier,
              gpUIPlannedSoldier.sGridNo,
            );
            pPlanSoldier.bVisible = 1;
            pPlanSoldier.usUIMovementMode = gpUIPlannedSoldier.usUIMovementMode;
