namespace ja2 {
  let ubDrugTravelRate: UINT8[] /* [] */ = [4, 2];
  let ubDrugWearoffRate: UINT8[] /* [] */ = [2, 2];
  let ubDrugEffect: UINT8[] /* [] */ = [15, 8];
  let ubDrugSideEffect: UINT8[] /* [] */ = [20, 10];
  let ubDrugSideEffectRate: UINT8[] /* [] */ = [2, 1];
  ...
  export function ApplyDrugs(
    ...
    if (ubDrugType < NUM_COMPLEX_DRUGS) {
      ...
      if (ubDrugType == DRUG_TYPE_ADRENALINE) {
        if (
          gMercProfiles[pSoldier.ubProfile].ubNumTimesDrugUseInLifetime != 255
        ) {
      ...
      if (ubDrugType == DRUG_TYPE_ALCOHOL) {
        ...
        if (
          pSoldier.bDrugSideEffectRate[ubDrugType] >
          ubDrugSideEffect[ubDrugType] * 3
        ) {
          // Keel over...
      ...
      if (ubDrugType == DRUG_TYPE_REGENERATION) {
        ...
        bRegenPointsGained = Math.trunc(
          (REGEN_POINTS_PER_BOOSTER * pObject.bStatus[0]) / 100,
        );
        // are there fractional %s left over?
        if (
          pObject.bStatus[0] % Math.trunc(100 / REGEN_POINTS_PER_BOOSTER) !=
          0
        ) {
          // chance of an extra point
          if (
            PreRandom(Math.trunc(100 / REGEN_POINTS_PER_BOOSTER)) <
            pObject.bStatus[0] % Math.trunc(100 / REGEN_POINTS_PER_BOOSTER)
          ) {
            bRegenPointsGained++;
          }
        ...
        if (bRegenPointsGained > 0) {
          ...
          pSoldier.bRegenerationCounter = Math.min(
            pSoldier.bRegenerationCounter + bRegenPointsGained,
            REGEN_POINTS_PER_BOOSTER,
          );
        }
    ...
    if (ubDrugType == DRUG_TYPE_ALCOHOL) {
      ScreenMsg(
        ...
        MSG_INTERFACE,
        pMessageStrings[Enum333.MSG_DRANK_SOME],
        pSoldier.name,
      ...
      ScreenMsg(
        FONT_MCOLOR_LTYELLOW,
        MSG_INTERFACE,
        pMessageStrings[Enum333.MSG_MERC_TOOK_DRUG],
        pSoldier.name,
      );
    }
  ...
  export function HandleEndTurnDrugAdjustments(pSoldier: SOLDIERTYPE): void {
    ...
    for (cnt = 0; cnt < NUM_COMPLEX_DRUGS; cnt++) {
      ...
      if (pSoldier.bDrugEffectRate[cnt] < 0) {
        ...
        if (pSoldier.bDrugEffect[cnt] <= 0) {
          ...
          iNumLoops =
            Math.trunc(pSoldier.bDrugSideEffect[cnt] / ubDrugSideEffect[cnt]) +
            1;
    ...
    if (pSoldier.bRegenerationCounter > 0) {
      ...
      // increase life
      pSoldier.bLife = Math.min(
        pSoldier.bLife + LIFE_GAIN_PER_REGEN_POINT,
        pSoldier.bLifeMax,
      );
  ...
  export function HandleAPEffectDueToDrugs(
    pSoldier: SOLDIERTYPE,
    ubPoints: UINT8,
  ): UINT8 {
  ...
  export function HandleBPEffectDueToDrugs(
    pSoldier: SOLDIERTYPE,
    sPointReduction: INT16,
  ): INT16 {
    let bDrunkLevel: INT8;
  ...
  export function GetDrunkLevel(pSoldier: SOLDIERTYPE): INT8 {
    ...
    if (
      pSoldier.bDrugEffect[DRUG_TYPE_ALCOHOL] == 0 &&
    ...
    bNumDrinks = Math.trunc(
      pSoldier.bDrugEffect[DRUG_TYPE_ALCOHOL] / ubDrugEffect[DRUG_TYPE_ALCOHOL],
    );
  ...
  export function EffectStatForBeingDrunk(
    pSoldier: SOLDIERTYPE,
    iStat: INT32,
  ): INT32 {
