namespace ja2 {
  ...
  function createBattleSoundStructFrom(
    zName: string,
    ubRandomVal: UINT8,
    fPreload: boolean,
    fBadGuy: boolean,
  ...
  export let bHealthStrRanges: UINT8[] /* [] */ = [15, 30, 45, 60, 75, 90, 101];
  ...
  function HandleVehicleMovementSound(
    ...
    if (fOn) {
      if (pVehicle.iMovementSoundID == NO_SAMPLE) {
        pVehicle.iMovementSoundID = PlayJA2Sample(
          pVehicle.iMoveSound,
          RATE_11025,
  ...
  export function AdjustNoAPToFinishMove(
    pSoldier: SOLDIERTYPE,
    fSet: boolean,
  ): void {
  ...
  export function CalcActionPoints(pSold: SOLDIERTYPE): INT8 {
    ...
    ubPoints =
      5 +
      Math.trunc(
        (10 * EffectiveExpLevel(pSold) +
          3 * EffectiveAgility(pSold) +
          2 * pSold.bLifeMax +
          2 * EffectiveDexterity(pSold) +
          20) /
          40,
      );
    ...
    if (pSold.bLife < pSold.bLifeMax) {
      ubPoints -= Math.trunc(
        (2 *
          ubPoints *
          (pSold.bLifeMax - pSold.bLife + Math.trunc(bBandage / 2))) /
          (3 * pSold.bLifeMax),
      );
    }
    ...
    if (pSold.ubBodyType == Enum194.BLOODCAT) {
      ...
      if (
        pSold.ubBodyType == Enum194.YAF_MONSTER ||
        pSold.ubBodyType == Enum194.YAM_MONSTER ||
        pSold.ubBodyType == Enum194.INFANT_MONSTER
      ) {
        ubPoints = Math.trunc((ubPoints * AP_YOUNG_MONST_FACTOR) / 10);
      }
    ...
    if (pSold.ubProfile != NO_PROFILE) {
      if (
        gMercProfiles[pSold.ubProfile].bPersonalityTrait ==
  ...
  export function CalcNewActionPoints(pSoldier: SOLDIERTYPE): void {
    ...
    if (!GetDrugEffect(pSoldier, DRUG_TYPE_ADRENALINE)) {
      pSoldier.bActionPoints = Math.min(
        pSoldier.bActionPoints,
        gubMaxActionPoints[pSoldier.ubBodyType],
      );
    }
  ...
  export function DoNinjaAttack(pSoldier: SOLDIERTYPE): void {
    ...
    if (usSoldierIndex != NOBODY) {
      ...
      if (
        (pTSoldier.bLife <= 30 || pTSoldier.bBreath <= 30) &&
        ubTargetStance != ANIM_PRONE
      ) {
        // Do finish!
        ...
        if (ubTargetStance != ANIM_PRONE) {
          ...
          if (
            ...
            if (
              !(
                pTSoldier.uiStatusFlags &
                (SOLDIER_MONSTER | SOLDIER_ANIMAL | SOLDIER_VEHICLE)
              )
            ) {
              ubTDirection = GetDirectionFromGridNo(
                pSoldier.sGridNo,
                pTSoldier,
              );
  ...
  export function CreateSoldierCommon(
    ubBodyType: UINT8,
    pSoldier: SOLDIERTYPE,
    usSoldierID: UINT16,
    usState: UINT16,
    ...
    if (pSoldier.ubBodyType == Enum194.QUEENMONSTER) {
      pSoldier.iPositionSndID = NewPositionSnd(
        NOWHERE,
        POSITION_SOUND_FROM_SOLDIER,
        pSoldier,
        Enum330.QUEEN_AMBIENT_NOISE,
      );
    }
    ...
    do {
      ...
      if (InitAnimationCache(usSoldierID, pSoldier.AnimCache) == false) {
        DebugMsg(
          TOPIC_JA2,
          DBG_LEVEL_0,
          FormatString("Soldier: Failed animation cache creation"),
        );
        break;
      }
      ...
      if (!(gTacticalStatus.uiFlags & LOADING_SAVED_GAME)) {
        ...
        if (
          !gfWorldLoaded &&
          (usState == Enum193.HOPFENCE ||
            usState == Enum193.CLIMBDOWNROOF ||
            usState == Enum193.FALLFORWARD_ROOF ||
            usState == Enum193.FALLOFF ||
            usState == Enum193.CLIMBUPROOF)
        ) {
      ...
      if (CreateSoldierPalettes(pSoldier) == false) {
        DebugMsg(
          TOPIC_JA2,
          DBG_LEVEL_0,
          FormatString("Soldier: Failed in creating soldier palettes"),
        );
        break;
      }
      ...
      if (
        ...
        if ((pSoldier.iLight = LightSpriteCreate("Light3", 0)) == -1) {
          DebugMsg(
            TOPIC_JA2,
            DBG_LEVEL_0,
            FormatString("Soldier: Failed loading light"),
          );
          return false;
        } else {
  ...
  export function ChangeSoldierState(
    pSoldier: SOLDIERTYPE,
    usNewState: UINT16,
    ...
    EVENT_InitNewSoldierAnim(
      pSoldier,
      SChangeState.usNewState,
      SChangeState.usStartingAniCode,
      SChangeState.fForce,
    );
  ...
  export function ReevaluateEnemyStance(
    pSoldier: SOLDIERTYPE,
    usAnimState: UINT16,
  ): boolean {
    let cnt: INT32;
        ...
        if (gAnimControl[usAnimState].uiFlags & (ANIM_MERCIDLE | ANIM_BREATH)) {
          if (pSoldier.bOppCnt > 0) {
            ...
            for (
              ...
              cnt <= gTacticalStatus.Team[OUR_TEAM].bLastID;
              cnt++
            ) {
            ...
            if (iClosestEnemy != NOBODY) {
              ...
              ({ sX: sTargetXPos, sY: sTargetYPos } = ConvertGridNoToXY(
                MercPtrs[iClosestEnemy].sGridNo,
              ));

              pSoldier.fDontChargeReadyAPs = true;

              // Ready weapon
              fReturnVal = SoldierReadyWeapon(
                pSoldier,
                sTargetXPos,
                sTargetYPos,
                false,
  ...
  function CheckForFreeupFromHit(
    pSoldier: SOLDIERTYPE,
    uiOldAnimFlags: UINT32,
    uiNewAnimFlags: UINT32,
    ...
    if (
      usOldAniState != usNewState &&
      uiOldAnimFlags & ANIM_HITSTART &&
      !(uiNewAnimFlags & ANIM_HITFINISH) &&
      !(uiNewAnimFlags & ANIM_IGNOREHITFINISH) &&
      ...
      DebugMsg(
        ...
        DBG_LEVEL_3,
        FormatString(
          "@@@@@@@ Releasesoldierattacker, normal hit animation ended NEW: %s ( %d ) OLD: %s ( %d )",
          gAnimControl[usNewState].zAnimStr,
          usNewState,
          gAnimControl[usOldAniState].zAnimStr,
    ...
    if (
      uiOldAnimFlags & ANIM_HITWHENDOWN &&
  ...
  export function EVENT_InitNewSoldierAnim(
    pSoldier: SOLDIERTYPE,
    usNewState: UINT16,
    usStartingAniCode: UINT16,
    fForce: boolean,
    ...
    if (!(gTacticalStatus.uiFlags & LOADING_SAVED_GAME)) {
      ...
      if (usNewState == pSoldier.usAnimState) {
        if (
          gAnimControl[pSoldier.usAnimState].uiFlags & ANIM_NORESTART &&
          !fForce
        ) {
      ...
      if (usStartingAniCode == 0) {
        if (
          gAnimControl[usNewState].ubHeight !=
            gAnimControl[pSoldier.usAnimState].ubEndHeight &&
          !(
            gAnimControl[usNewState].uiFlags &
            (ANIM_STANCECHANGEANIM | ANIM_IGNORE_AUTOSTANCE)
          )
        ) {
          ...
          // Check if we are in realtime and we are going from crouch to stand
            ...
            (gTacticalStatus.uiFlags & REALTIME ||
              !(gTacticalStatus.uiFlags & INCOMBAT)) &&
            pSoldier.usAnimState != Enum193.HELIDROP
          ) {
            pSoldier.ubDesiredHeight = gAnimControl[usNewState].ubEndHeight;
            // Continue with this course of action IE: Do animation and skip from stand to crouch
          } else {
      ...
      // if ( pSoldier->bLife > INJURED_CHANGE_THREASHOLD )
      {
        ...
        if (
          ...
          if (
            gAnimControl[usNewState].uiFlags & ANIM_RAISE_WEAPON &&
            !(
          ...
          if (
            gAnimControl[usNewState].uiFlags & ANIM_LOWER_WEAPON &&
            !(
              gAnimControl[pSoldier.usAnimState].uiFlags &
      ...
      if (
        gAnimControl[usNewState].uiFlags & ANIM_MOVING &&
        pSoldier.usAnimState == Enum193.COWERING &&
        gAnimControl[usNewState].ubEndHeight == ANIM_STAND
      ) {
        pSoldier.usPendingAnimation = usNewState;
        // Set new state to be animation to move to new stance
      ...
      if (
        pSoldier.usAnimState != Enum193.START_SWAT &&
        usNewState == Enum193.SWATTING
      ) {
        // Set new state to be animation to move to new stance
      ...
      if (
        pSoldier.usAnimState == Enum193.WALKING &&
        usNewState == Enum193.STANDING &&
      ...
      if (pSoldier.uiAnimSubFlags & SUB_ANIM_BIGGUYTHREATENSTANCE) {
        if (
          ...
          pSoldier.usAnimState != Enum193.BIGMERC_CROUCH_TRANS_INTO
        ) {
          let usItem: UINT16;
        ...
        if (
          ...
          pSoldier.usAnimState != Enum193.BIGMERC_CROUCH_TRANS_OUTOF
        ) {
          let usItem: UINT16;
      ...
      if (
        usNewState == Enum193.CLIMBUPROOF ||
        usNewState == Enum193.CLIMBDOWNROOF ||
        usNewState == Enum193.HOPFENCE
      ) {
        ...
        if (!pSoldier.usDontUpdateNewGridNoOnMoveAnimChange) {
          if (usNewState != Enum193.SWATTING) {
            DebugMsg(
              ...
              DBG_LEVEL_3,
              FormatString(
                "Handling New gridNo for %d: Old %s, New %s",
                pSoldier.ubID,
    ...
    if (
      !(gAnimControl[pSoldier.usAnimState].uiFlags & ANIM_FIREREADY) &&
      ...
      if (
        usNewState != Enum193.READY_RIFLE_STAND &&
        usNewState != Enum193.READY_RIFLE_PRONE &&
        usNewState != Enum193.READY_RIFLE_CROUCH &&
        usNewState != Enum193.ROBOT_SHOOT
      ) {
    ...
    if (pSoldier.ubID < MAX_NUM_SOLDIERS) {
      ...
      switch (usNewState) {
        ...
        case Enum193.FALLFORWARD_FROMHIT_CROUCH:
          DeductPoints(
            pSoldier,
            Math.trunc(AP_FALL_DOWN / 2),
            Math.trunc(BP_FALL_DOWN / 2),
          );
          break;
        ...
        case Enum193.JUMP_OVER_BLOCKING_PERSON:
          // Set path....
          {
            ...
            pSoldier.usPathIndex = 0;
            pSoldier.usPathingData[pSoldier.usPathDataSize] =
              pSoldier.bDirection;
            ...
            EVENT_InternalSetSoldierDestination(
              ...
              pSoldier.usPathingData[pSoldier.usPathIndex],
              false,
              Enum193.JUMP_OVER_BLOCKING_PERSON,
            );
          }
          break;
      ...
      if (
        ...
        switch (pSoldier.ubPendingAction) {
          case Enum257.MERC_GIVEITEM:
            ...
            MercPtrs[pSoldier.uiPendingActionData4].uiStatusFlags &=
    ...
    if (
      usNewState == Enum193.CHARIOTS_OF_FIRE ||
      usNewState == Enum193.BODYEXPLODING
    ) {
    ...
    if (
      usNewState == Enum193.VEHICLE_DIE ||
      usNewState == Enum193.CHARIOTS_OF_FIRE ||
      usNewState == Enum193.BODYEXPLODING
    ) {
  ...
  function InternalRemoveSoldierFromGridNo(
    pSoldier: SOLDIERTYPE,
    fForce: boolean,
  ): void {
    let bDir: INT8;
  ...
  export function EVENT_InternalSetSoldierPosition(
    pSoldier: SOLDIERTYPE,
    dNewXPos: FLOAT,
    dNewYPos: FLOAT,
    fUpdateDest: boolean,
    fUpdateFinalDest: boolean,
    fForceRemove: boolean,
  ): void {
  ...
  export function EVENT_SetSoldierPosition(
    pSoldier: SOLDIERTYPE,
    dNewXPos: FLOAT,
    dNewYPos: FLOAT,
  ): void {
  ...
  export function EVENT_SetSoldierPositionForceDelete(
    pSoldier: SOLDIERTYPE,
    dNewXPos: FLOAT,
    dNewYPos: FLOAT,
  ): void {
  ...
  function EVENT_SetSoldierPositionAndMaybeFinalDest(
    pSoldier: SOLDIERTYPE,
    dNewXPos: FLOAT,
    dNewYPos: FLOAT,
    fUpdateFinalDest: boolean,
  ...
  export function EVENT_SetSoldierPositionAndMaybeFinalDestAndMaybeNotDestination(
    pSoldier: SOLDIERTYPE,
    dNewXPos: FLOAT,
    dNewYPos: FLOAT,
    fUpdateDest: boolean,
  ...
  function InternalSetSoldierHeight(
    pSoldier: SOLDIERTYPE,
    dNewHeight: FLOAT,
    fUpdateLevel: boolean,
  ): void {
    ...
    if (pSoldier.sHeightAdjustment > 0) {
      ...
      ApplyTranslucencyToWalls(
        Math.trunc(pSoldier.dXPos / CELL_X_SIZE),
        Math.trunc(pSoldier.dYPos / CELL_Y_SIZE),
      );
  ...
  export function SetSoldierHeight(
    pSoldier: SOLDIERTYPE,
    dNewHeight: FLOAT,
  ): void {
  ...
  function SetSoldierGridNo(
    ...
    if (sNewGridNo != pSoldier.sGridNo || pSoldier.pLevelNode == null) {
      ...
      for (bDir = 0; bDir < Enum245.NUM_WORLD_DIRECTIONS; bDir++) {
        gpWorldLevelData[pSoldier.sGridNo + DirIncrementer[bDir]]
      ...
      if (!(pSoldier.uiStatusFlags & (SOLDIER_DRIVER | SOLDIER_PASSENGER))) {
        ...
        if (
          ...
          (gGameSettings.fOptions[Enum8.TOPTION_MERC_CASTS_LIGHT] &&
            NightTime())
        ) {
          if (
            pSoldier.bLevel > 0 &&
            gpWorldLevelData[pSoldier.sGridNo].pRoofHead != null
          ) {
            (<LEVELNODE>(
              gpWorldLevelData[pSoldier.sGridNo].pMercHead
            )).ubShadeLevel = (<LEVELNODE>(
              gpWorldLevelData[pSoldier.sGridNo].pRoofHead
            )).ubShadeLevel;
            (<LEVELNODE>(
              gpWorldLevelData[pSoldier.sGridNo].pMercHead
            )).ubSumLights = (<LEVELNODE>(
              gpWorldLevelData[pSoldier.sGridNo].pRoofHead
            )).ubSumLights;
            (<LEVELNODE>(
              gpWorldLevelData[pSoldier.sGridNo].pMercHead
            )).ubMaxLights = (<LEVELNODE>(
              gpWorldLevelData[pSoldier.sGridNo].pRoofHead
            )).ubMaxLights;
            (<LEVELNODE>(
              gpWorldLevelData[pSoldier.sGridNo].pMercHead
            )).ubNaturalShadeLevel = (<LEVELNODE>(
              gpWorldLevelData[pSoldier.sGridNo].pRoofHead
            )).ubNaturalShadeLevel;
          } else {
            (<LEVELNODE>(
              ...
              gpWorldLevelData[pSoldier.sGridNo].pLandHead
            )).ubShadeLevel;
            (<LEVELNODE>(
              gpWorldLevelData[pSoldier.sGridNo].pMercHead
            )).ubSumLights = (<LEVELNODE>(
      ...
      if (!(pSoldier.uiStatusFlags & (SOLDIER_DRIVER | SOLDIER_PASSENGER))) {
        ...
        if (
          pSoldier.bOverTerrainType == Enum315.DEEP_WATER &&
          pSoldier.bOldOverTerrainType != Enum315.DEEP_WATER
        ) {
      ...
      if (GetSmokeEffectOnTile(sNewGridNo, pSoldier.bLevel)) {
        ...
        if (
          ...
          pSoldier.inv[Enum261.HEAD1POS].bStatus[0] >= GASMASK_MIN_STATUS
        ) {
          fSetGassed = false;
        }
        if (
          pSoldier.inv[Enum261.HEAD2POS].usItem == Enum225.GASMASK &&
          pSoldier.inv[Enum261.HEAD2POS].bStatus[0] >= GASMASK_MIN_STATUS
        ) {
          fSetGassed = false;
        }
        ...
        if (pSoldier.bOppCnt > 0) {
          ...
          for (cnt = 0; cnt < MAX_NUM_SOLDIERS; cnt++) {
            ...
            if (pEnemy.bActive && pEnemy.bInSector && pEnemy.bLife >= OKLIFE) {
              ...
              if (
                !pEnemy.bNeutral &&
                pSoldier.bSide != pEnemy.bSide &&
                pEnemy.ubBodyType != Enum194.COW &&
                pEnemy.ubBodyType != Enum194.CROW
              ) {
                // if we SEE this particular oppponent, and he DOESN'T see us... and he COULD see us...
                if (
                  pSoldier.bOppList[cnt] == SEEN_CURRENTLY &&
  ...
  export function EVENT_FireSoldierWeapon(
    pSoldier: SOLDIERTYPE,
    sTargetGridNo: INT16,
  ): void {
    let sTargetXPos: INT16;
    ...
    if (Item[pSoldier.inv[Enum261.HANDPOS].usItem].usItemClass & IC_GUN) {
      if (pSoldier.bDoBurst) {
        ...
        pSoldier.bBulletsLeft = Math.min(
          Weapon[pSoldier.inv[pSoldier.ubAttackingHand].usItem].ubShotsPerBurst,
      ...
      if (
        pSoldier.inv[pSoldier.ubAttackingHand].ubGunAmmoType ==
        Enum286.AMMO_BUCKSHOT
      ) {
    ...
    DebugMsg(
      ...
      DBG_LEVEL_3,
      FormatString(
        "!!!!!!! Starting attack, bullets left %d",
  ...
  function SelectFireAnimation(pSoldier: SOLDIERTYPE, ubHeight: UINT8): UINT16 {
    ...
    if (
      pSoldier.ubBodyType == Enum194.TANK_NW ||
      pSoldier.ubBodyType == Enum194.TANK_NE
    ) {
    ...
    switch (ubHeight) {
      ...
      default:
        AssertMsg(
          false,
          FormatString(
            "SelectFireAnimation: ERROR - Invalid height %d",
            ubHeight,
          ),
        );
        break;
  ...
  export function GetMoveStateBasedOnStance(
    pSoldier: SOLDIERTYPE,
    ubStanceHeight: UINT8,
  ): UINT16 {
    // Determine which animation to do...depending on stance and gun in hand...
    switch (ubStanceHeight) {
      case ANIM_STAND:
        if (
          pSoldier.fUIMovementFast &&
      ...
      default:
        AssertMsg(
          false,
          FormatString(
            "GetMoveStateBasedOnStance: ERROR - Invalid height %d",
            ubStanceHeight,
          ),
        );
        break;
  ...
  export function SoldierReadyWeapon(
    pSoldier: SOLDIERTYPE,
    sTargetXPos: INT16,
    sTargetYPos: INT16,
    fEndReady: boolean,
  ...
  export function InternalSoldierReadyWeapon(
    pSoldier: SOLDIERTYPE,
    sFacingDir: UINT8,
    fEndReady: boolean,
  ): boolean {
  ...
  export function PickSoldierReadyAnimation(
    pSoldier: SOLDIERTYPE,
    fEndReady: boolean,
  ): UINT16 {
    ...
    if (
      ...
      pSoldier.inv[Enum261.HANDPOS].usItem != Enum225.GLAUNCHER
    ) {
      return INVALID_ANIMATION;
    }
  ...
  export function EVENT_SoldierGotHit(
    pSoldier: SOLDIERTYPE,
    ...
    if (!(pSoldier.uiStatusFlags & SOLDIER_VEHICLE)) {
      ...
      DebugMsg(
        ...
        DBG_LEVEL_3,
        FormatString(
          "!!!!!!! Person got hit, attack count now %d",
          gTacticalStatus.ubAttackBusyCount,
        ),
      );
    }
    ...
    if (sDamage > 25) {
      if (
        ...
        HandleMoraleEvent(
          ...
          Enum234.MORALE_DID_LOTS_OF_DAMAGE,
      ...
      if (pSoldier.bTeam == gbPlayerNum) {
        HandleMoraleEvent(
          pSoldier,
          Enum234.MORALE_TOOK_LOTS_OF_DAMAGE,
          pSoldier.sSectorX,
          pSoldier.sSectorY,
          pSoldier.bSectorZ,
        );
    ...
    if (ubSpecial == FIRE_WEAPON_TOSSED_OBJECT_SPECIAL) {
      ...
      DebugMsg(
        ...
        DBG_LEVEL_3,
        FormatString(
          "Soldier Control: Weapon class not handled in SoldierGotHit( ) %d",
          usWeaponIndex,
        ),
      );
    }
    ...
    if (ubReason != TAKE_DAMAGE_TENTACLES && ubReason != TAKE_DAMAGE_OBJECT) {
      ...
      if (Item[usWeaponIndex].usItemClass & IC_GUN) {
        pNewSoldier = FreeUpAttackerGivenTarget(
          pSoldier.ubAttackerID,
          pSoldier.ubID,
        );
      } else {
        pNewSoldier = ReduceAttackBusyGivenTarget(
          pSoldier.ubAttackerID,
          pSoldier.ubID,
        );
      }
      ...
      DebugMsg(
        ...
        DBG_LEVEL_3,
        FormatString(
          "!!!!!!! Tried to free up attacker, attack count now %d",
          gTacticalStatus.ubAttackBusyCount,
        ),
      );
    }
    ...
    // DEDUCT LIFE
    ubCombinedLoss = SoldierTakeDamage(
      pSoldier,
      ANIM_CROUCH,
      sDamage,
      sBreathLoss,
    ...
    if (
      !(gAnimControl[pSoldier.usAnimState].uiFlags & ANIM_HITSTOP) ||
      pSoldier.usAnimState != Enum193.JFK_HITDEATH_STOP
    ) {
      MakeNoise(
        pSoldier.ubID,
        pSoldier.sGridNo,
    ...
    if (Item[usWeaponIndex].usItemClass & IC_BLADE) {
      ...
      PlayJA2Sample(
        Enum330.BULLET_IMPACT_1 + Random(3),
        RATE_11025,
        SoundVolume(MIDVOLUME, pSoldier.sGridNo),
        1,
        SoundDir(pSoldier.sGridNo),
    ...
    if (pSoldier.bLife >= CONSCIOUSNESS) {
      if (pSoldier.ubBodyType == Enum194.CROW) {
        ...
        if (
          ...
          pSoldier.usAniCode != Enum193.STANDING_BURST_HIT
        ) {
        } else {
    ...
    if (gAnimControl[pSoldier.usAnimState].uiFlags & ANIM_HITSTOP) {
      switch (pSoldier.usAnimState) {
        ...
        case Enum193.PRONE_HITDEATH_STOP:
          ChangeSoldierState(
            pSoldier,
            Enum193.PRONE_HIT_DEATHTWITCHB,
            0,
            false,
          );
          break;
        ...
        case Enum193.FALLFORWARD_HITDEATH_STOP:
          ChangeSoldierState(
            pSoldier,
            Enum193.GENERIC_HIT_DEATHTWITCHB,
            0,
            false,
          );
          break;
        ...
        case Enum193.FALLBACK_HITDEATH_STOP:
          ChangeSoldierState(
            pSoldier,
            Enum193.FALLBACK_HIT_DEATHTWITCHB,
            0,
            false,
          );
          break;
        ...
        case Enum193.FALLOFF_FORWARD_DEATH_STOP:
          ChangeSoldierState(
            pSoldier,
            Enum193.FALLOFF_FORWARD_TWITCHB,
            0,
            false,
          );
          break;
        ...
        case Enum193.FALLOFF_FORWARD_STOP:
          ChangeSoldierState(
            pSoldier,
            Enum193.FALLOFF_FORWARD_TWITCHNB,
            0,
            false,
          );
          break;
    ...
    if (
      Item[usWeaponIndex].usItemClass & IC_EXPLOSV ||
  ...
  function CalcScreamVolume(
    pSoldier: SOLDIERTYPE,
    ubCombinedLoss: UINT8,
  ): UINT8 {
  ...
  */
  ...
  function SoldierGotHitGunFire(
      ...
      if (gAnimControl[pSoldier.usAnimState].ubEndHeight == ANIM_STAND) {
        if (ubSpecial == FIRE_WEAPON_HEAD_EXPLODE_SPECIAL) {
          if (gGameSettings.fOptions[Enum8.TOPTION_BLOOD_N_GORE]) {
            if (
              SpacesAway(pSoldier.sGridNo, Menptr[ubAttackerID].sGridNo) <=
            ...
            if (
              ...
              if (pSoldier.bDirection == bDirection) {
                ...
                if (
                  ...
                  usNewGridNo = NewGridNo(
                    usNewGridNo,
                    DirectionInc(gOppositeDirection[bDirection]),
                  );
    ...
    if (pSoldier.uiStatusFlags & SOLDIER_DEAD) {
      DebugMsg(
        TOPIC_JA2,
        DBG_LEVEL_3,
        FormatString("@@@@@@@ Releasesoldierattacker,Dead soldier hit"),
      );
      ReleaseSoldiersAttacker(pSoldier);
      return;
    }
  ...
  function SoldierGotHitExplosion(
    pSoldier: SOLDIERTYPE,
    usWeaponIndex: UINT16,
    sDamage: INT16,
    bDirection: UINT16,
    ...
    if (gGameSettings.fOptions[Enum8.TOPTION_BLOOD_N_GORE]) {
      if (
        Explosive[Item[usWeaponIndex].ubClassIndex].ubRadius >= 3 &&
        pSoldier.bLife == 0 &&
  ...
  export function EVENT_InternalGetNewSoldierPath(
    pSoldier: SOLDIERTYPE,
    sDestGridNo: UINT16,
    usMovementAnim: UINT16,
    fFromUI: UINT8 /* boolean */,
    fForceRestartAnim: boolean,
  ): boolean {
    let iDest: INT32;
    let sNewGridNo: INT16;
    ...
    let usPathingData: UINT16[] /* [MAX_PATH_LIST_SIZE] */ = createArray(
      MAX_PATH_LIST_SIZE,
      0,
    );
    ...
    if (fContinue) {
      ...
      DebugMsg(
        TOPIC_JA2,
        DBG_LEVEL_0,
        FormatString("Soldier %d: Get new path", pSoldier.ubID),
      );
  ...
  export function EVENT_GetNewSoldierPath(
    pSoldier: SOLDIERTYPE,
    sDestGridNo: UINT16,
    usMovementAnim: UINT16,
  ): void {
  ...
  export function ChangeSoldierStance(
    pSoldier: SOLDIERTYPE,
    ubDesiredStance: UINT8,
  ): void {
  ...
  export function EVENT_InternalSetSoldierDestination(
    pSoldier: SOLDIERTYPE,
    ...
    if (pSoldier.bReverse && usAnimState == Enum193.SIDE_STEP) {
      ...
      ubPerpDirection =
        gPurpendicularDirection[pSoldier.bDirection][usNewDirection];
  ...
  export function EVENT_SetSoldierDestination(
    pSoldier: SOLDIERTYPE,
    usNewDirection: UINT16,
  ): void {
    EVENT_InternalSetSoldierDestination(
  ...
  function MultiTiledTurnDirection(
    pSoldier: SOLDIERTYPE,
    bStartDirection: INT8,
    ...
    while (bLoop < 2) {
      while (bCurrentDirection != bDesiredDirection) {
        ...
        fOk = OkayToAddStructureToWorld(
          pSoldier.sGridNo,
          pSoldier.bLevel,
          pStructureFileRef.pDBStructureRef[gOneCDirection[bCurrentDirection]],
          usStructureID,
        );
        if (!fOk) {
          break;
  ...
  export function EVENT_InternalSetSoldierDesiredDirection(
    pSoldier: SOLDIERTYPE,
    usNewDirection: UINT16,
    fInitalMove: boolean,
    usAnimState: UINT16,
    ...
    if (
      ...
      gAnimControl[usAnimState].uiFlags & ANIM_MOVING
    ) {
    ...
    if (pSoldier.bDesiredDirection != pSoldier.bDirection) {
      if (
        gAnimControl[usAnimState].uiFlags &
          (ANIM_BREATH | ANIM_OK_CHARGE_AP_FOR_TURN | ANIM_FIREREADY) &&
        !fInitalMove &&
        !pSoldier.fDontChargeTurningAPs
      ) {
      ...
      if (
        gAnimControl[usAnimState].uiFlags & ANIM_STATIONARY ||
        pSoldier.fNoAPToFinishMove ||
        fInitalMove
      ) {
        if (gAnimControl[usAnimState].ubHeight == ANIM_PRONE) {
    ...
    pSoldier.ubHiResDesiredDirection =
      ubExtDirection[pSoldier.bDesiredDirection];
    ...
    if (pSoldier.uiStatusFlags & SOLDIER_VEHICLE) {
      pSoldier.bTurningIncrement = ExtQuickestDirection(
        pSoldier.ubHiResDirection,
        pSoldier.ubHiResDesiredDirection,
      );
    } else {
      if (pSoldier.uiStatusFlags & SOLDIER_MULTITILE) {
        pSoldier.bTurningIncrement = MultiTiledTurnDirection(
          pSoldier,
          pSoldier.bDirection,
          pSoldier.bDesiredDirection,
        );
      } else {
        pSoldier.bTurningIncrement = QuickestDirection(
          pSoldier.bDirection,
          pSoldier.bDesiredDirection,
        );
      }
  ...
  export function EVENT_SetSoldierDesiredDirection(
    pSoldier: SOLDIERTYPE,
    usNewDirection: UINT16,
  ): void {
    EVENT_InternalSetSoldierDesiredDirection(
  ...
  export function EVENT_SetSoldierDirection(
    pSoldier: SOLDIERTYPE,
    usNewDirection: UINT16,
  ): void {
  ...
  export function EVENT_BeginMercTurn(
    pSoldier: SOLDIERTYPE,
    fFromRealTime: boolean,
    iRealTimeCounter: INT32,
  ): void {
    ...
    if (pSoldier.bLife) {
      ...
      if (pSoldier.sNoiseGridno != NOWHERE) {
        if (pSoldier.ubNoiseVolume) {
          // and the noise volume is still positive
          pSoldier.ubNoiseVolume--; // the volume of the noise "decays" by 1 point

          if (!pSoldier.ubNoiseVolume) {
            // if the volume has reached zero
...
savedPts = MAX_AP_CARRIED;
          ...
          switch (gMercProfiles[pSoldier.ubProfile].bPersonalityTrait) {
            case Enum270.FEAR_OF_INSECTS:
              if (MercSeesCreature(pSoldier)) {
                HandleMoraleEvent(
                  pSoldier,
                  Enum234.MORALE_INSECT_PHOBIC_SEES_CREATURE,
                  pSoldier.sSectorX,
                  pSoldier.sSectorY,
                  pSoldier.bSectorZ,
                );
  ...
  let gDirectionFrom8to2: UINT8[] /* [] */ = [0, 0, 1, 1, 0, 1, 1, 0];

  export function ConvertAniCodeToAniFrame(
    pSoldier: SOLDIERTYPE,
    usAniFrame: UINT16,
  ): boolean {
    let usAnimSurface: UINT16;
    ...
    pSoldier.usAniFrame =
      usAniFrame +
      gAnimSurfaceDatabase[usAnimSurface].uiNumFramesPerDir * ubTempDir;
  ...
  export function TurnSoldier(pSoldier: SOLDIERTYPE): void {
    ...
    if (pSoldier.uiStatusFlags & SOLDIER_LOOK_NEXT_TURNSOLDIER) {
      if (
        gAnimControl[pSoldier.usAnimState].uiFlags & ANIM_STATIONARY &&
        pSoldier.usAnimState != Enum193.CLIMBUPROOF &&
        pSoldier.usAnimState != Enum193.CLIMBDOWNROOF
      ) {
        // HANDLE SIGHT!
      ...
      if (pSoldier.bDirection == pSoldier.bDesiredDirection) {
        ...
        // Else check if we are trying to shoot and once was prone, but am now crouched because we needed to turn...
          ...
          if (IsValidStance(pSoldier, ANIM_PRONE)) {
            ...
            pSoldier.usPendingAnimation = SelectFireAnimation(
              pSoldier,
              ANIM_PRONE,
            );
          } else {
            EVENT_InitNewSoldierAnim(
              pSoldier,
              SelectFireAnimation(pSoldier, ANIM_CROUCH),
              0,
              false,
            );
          }
    ...
    if (pSoldier.bDirection == pSoldier.bDesiredDirection) {
      ...
      if (
        pSoldier.fTurningFromPronePosition ==
          TURNING_FROM_PRONE_ENDING_UP_FROM_MOVE &&
        pSoldier.usAnimState != Enum193.PRONE_UP &&
        pSoldier.usAnimState != Enum193.PRONE_DOWN
      ) {
        if (IsValidStance(pSoldier, ANIM_PRONE)) {
    ...
    */
      ...
      if (
        ...
        if (pSoldier.iTuringSoundID == NO_SAMPLE) {
          pSoldier.iTuringSoundID = PlaySoldierJA2Sample(
            pSoldier.ubID,
            Enum330.TURRET_MOVE,
            RATE_11025,
            SoundVolume(HIGHVOLUME, pSoldier.sGridNo),
      ...
      if (OKToAddMercToWorld(pSoldier, sDirection)) {
        ...
        if (
          gubWaitingForAllMercsToExitCode ==
            Enum238.WAIT_FOR_MERCS_TO_WALKOFF_SCREEN ||
          gubWaitingForAllMercsToExitCode ==
        ...
        if (
          ...
          !MercInWater(pSoldier) &&
          pSoldier.bOverTerrainType != Enum315.DIRT_ROAD &&
          pSoldier.bOverTerrainType != Enum315.PAVED_ROAD
        ) {
          PlaySoldierFootstepSound(pSoldier);
        }
      } else {
  ...
  export function CreateSoldierPalettes(pSoldier: SOLDIERTYPE): boolean {
    ...
    let Temp8BPPPalette: SGPPaletteEntry[] /* [256] */ = createArrayFrom(
      256,
    ...
    if (
      (bBodyTypePalette = GetBodyTypePaletteSubstitutionCode(
        pSoldier,
        pSoldier.ubBodyType,
        createPointer(
          () => zColFilename,
      ...
      usPaletteAnimSurface = LoadSoldierAnimationSurface(
        pSoldier,
        Enum193.STANDING,
      );
    ...
    //}
    pSoldier.pEffectShades[0] = Create16BPPPaletteShaded(
      pSoldier.p8BPPPalette,
      100,
      100,
      100,
    ...
    pSoldier.pEffectShades[1] = Create16BPPPaletteShaded(
      pSoldier.p8BPPPalette,
      100,
      150,
      100,
    ...
    pSoldier.pGlowShades[0] = Create16BPPPaletteShaded(
      pSoldier.p8BPPPalette,
      255,
      255,
      255,
    ...
    for (cnt = 1; cnt < 10; cnt++) {
      pSoldier.pGlowShades[cnt] = CreateEnemyGlow16BPPPalette(
        pSoldier.p8BPPPalette,
        gRedGlowR[cnt],
        255,
        false,
    ...
    // Now for gray guy...
    pSoldier.pGlowShades[10] = Create16BPPPaletteShaded(
      pSoldier.p8BPPPalette,
      100,
      100,
      100,
    ...
    for (cnt = 11; cnt < 19; cnt++) {
      pSoldier.pGlowShades[cnt] = CreateEnemyGreyGlow16BPPPalette(
        pSoldier.p8BPPPalette,
        gRedGlowR[cnt],
        0,
        false,
    ...
    pSoldier.pGlowShades[19] = CreateEnemyGreyGlow16BPPPalette(
      pSoldier.p8BPPPalette,
      gRedGlowR[18],
      0,
      false,
    ...
    // ORANGE, VISIBLE GUY
    pSoldier.pShades[20] = Create16BPPPaletteShaded(
      pSoldier.p8BPPPalette,
      255,
      255,
      255,
    ...
    // ORANGE, GREY GUY
    pSoldier.pShades[30] = Create16BPPPaletteShaded(
      pSoldier.p8BPPPalette,
      100,
      100,
      100,
    ...
    pSoldier.pShades[39] = CreateEnemyGreyGlow16BPPPalette(
      pSoldier.p8BPPPalette,
      gOrangeGlowR[18],
      gOrangeGlowG[18],
      true,
  ...
  function AdjustAniSpeed(pSoldier: SOLDIERTYPE): void {
    if (gTacticalStatus.uiFlags & SLOW_ANIMATION) {
      if (gTacticalStatus.bRealtimeSpeed == -1) {
        ...
        pSoldier.sAniDelay =
          pSoldier.sAniDelay *
          Math.trunc((1 * gTacticalStatus.bRealtimeSpeed) / 2);
      }
    }
  ...
  function CalculateSoldierAniSpeed(
    pSoldier: SOLDIERTYPE,
    pStatsSoldier: SOLDIERTYPE,
  ): void {
    ...
    switch (pSoldier.usAnimState) {
      ...
      case Enum193.STANDING:
        pSoldier.sAniDelay =
          pStatsSoldier.bBreath * 2 + (100 - pStatsSoldier.bLife);
  ...
  export function SetSoldierAniSpeed(pSoldier: SOLDIERTYPE): void {
    ...
    if (
      ...
      if (
        ((pSoldier.bVisible == -1 &&
          pSoldier.bVisible == pSoldier.bLastRenderVisibleValue) ||
          gTacticalStatus.fAutoBandageMode) &&
  ...
  export function LoadPaletteData(): boolean {
    ...
    // Malloc!
    gpPaletteSubRanges = createArrayFrom(
      guiNumPaletteSubRanges,
      createPaletteSubRangeType,
    );
    ...
    // Malloc!
    gpPalRep = createArrayFrom(
      guiNumReplacements,
      createPaletteReplacementType,
    );
  ...
  export function SetPaletteReplacement(
    p8BPPPalette: SGPPaletteEntry[],
    ...
    for (
      cnt2 = gpPaletteSubRanges[ubType].ubStart;
      cnt2 <= gpPaletteSubRanges[ubType].ubEnd;
      cnt2++
    ) {
      p8BPPPalette[cnt2].peRed =
        gpPalRep[ubPalIndex].r[cnt2 - gpPaletteSubRanges[ubType].ubStart];
      p8BPPPalette[cnt2].peGreen =
        gpPalRep[ubPalIndex].g[cnt2 - gpPaletteSubRanges[ubType].ubStart];
      p8BPPPalette[cnt2].peBlue =
        gpPalRep[ubPalIndex].b[cnt2 - gpPaletteSubRanges[ubType].ubStart];
    }
  ...
  function GetNewSoldierStateFromNewStance(
    pSoldier: SOLDIERTYPE,
    ubDesiredStance: UINT8,
  ): UINT16 {
    let usNewState: UINT16;
    ...
    bCurrentHeight =
      ubDesiredStance - gAnimControl[pSoldier.usAnimState].ubEndHeight;
  ...
  export function MoveMercFacingDirection(
    pSoldier: SOLDIERTYPE,
    fReverse: boolean,
    dMovementDist: FLOAT,
  ): void {
  ...
  export function BeginSoldierClimbUpRoof(pSoldier: SOLDIERTYPE): void {
    ...
    if (
      ...
      if (EnoughPoints(pSoldier, GetAPsToClimbRoof(pSoldier, false), 0, true)) {
        ...
        pSoldier.sTempNewGridNo = NewGridNo(
          pSoldier.sGridNo,
          DirectionInc(bNewDirection),
        );
  ...
  function HandleTakeDamageDeath(
    pSoldier: SOLDIERTYPE,
    bOldLife: UINT8,
    ubReason: UINT8,
  ): void {
    ...
    if (ubReason == TAKE_DAMAGE_ELECTRICITY) {
      if (pSoldier.bLife >= OKLIFE) {
        DebugMsg(
          TOPIC_JA2,
          DBG_LEVEL_3,
          FormatString("Freeing up attacker from electricity damage"),
        );
        ReleaseSoldiersAttacker(pSoldier);
      }
    }
  ...
  export function SoldierTakeDamage(
    pSoldier: SOLDIERTYPE,
    bHeight: INT8,
    sLifeDeduct: INT16,
    sBreathLoss: INT16,
    ...
    switch (pSoldier.bTeam) {
      case ENEMY_TEAM:
        ...
        if (
          StrategicMap[CALCULATE_STRATEGIC_INDEX(gWorldSectorX, gWorldSectorY)]
            .fEnemyControlled ||
          SectorInfo[SECTOR(gWorldSectorX, gWorldSectorY)].ubTraversability[
            Enum186.THROUGH_STRATEGIC_MOVE
          ] != Enum127.TOWN
        ) {
      ...
      case CIV_TEAM:
        if (
          ...
          let pMaria: SOLDIERTYPE | null = FindSoldierByProfileID(
            Enum268.MARIA,
            false,
          );
    ...
    if (CREATURE_OR_BLOODCAT(pSoldier)) {
      ...
      if (sReductionFactor > 1) {
        sLifeDeduct = Math.trunc(
          (sLifeDeduct + Math.trunc(sReductionFactor / 2)) / sReductionFactor,
        );
      ...
      if (sReductionFactor > 1) {
        sBreathLoss = Math.trunc(
          (sBreathLoss + Math.trunc(sReductionFactor / 2)) / sReductionFactor,
        );
      }
    }
    ...
    ubCombinedLoss =
      Math.trunc(sLifeDeduct / 10) + Math.trunc(sBreathLoss / 2000);
    ...
    if (ubReason != TAKE_DAMAGE_BLOODLOSS && !AM_A_ROBOT(pSoldier)) {
      ...
      if (
        pSoldier.ubAttackerID != NOBODY &&
        MercPtrs[pSoldier.ubAttackerID].ubBodyType == Enum194.BLOODCAT
      ) {
    ...
    if (ubAttacker != NOBODY) {
      ...
      if (
        MercPtrs[ubAttacker].bTeam == gbPlayerNum &&
        pSoldier.bTeam != gbPlayerNum
      ) {
        if (ubReason == TAKE_DAMAGE_EXPLOSION) {
    ...
    if (PTR_OURTEAM(pSoldier)) {
      ...
      if (!(pSoldier.usQuoteSaidFlags & SOLDIER_QUOTE_SAID_BEING_PUMMELED)) {
        ...
        if (ubAttacker != NOBODY && ubAttacker != pSoldier.ubID) {
          ...
          if (
            pSoldier.bNumHitsThisTurn >= 3 &&
            pSoldier.bLife - pSoldier.bOldLife > 20
          ) {
            if (Random(100) < 40 * (pSoldier.bNumHitsThisTurn - 2)) {
              DelayedTacticalCharacterDialogue(
                pSoldier,
                Enum202.QUOTE_TAKEN_A_BREATING,
              );
              pSoldier.usQuoteSaidFlags |= SOLDIER_QUOTE_SAID_BEING_PUMMELED;
              pSoldier.bNumHitsThisTurn = 0;
            }
    ...
    if (
      ubAttacker != NOBODY &&
      Menptr[ubAttacker].bTeam == OUR_TEAM &&
      pSoldier.ubProfile != NO_PROFILE &&
  ...
  export function InternalDoMercBattleSound(
    pSoldier: SOLDIERTYPE,
    ...
    if (
      ubBattleSoundID == Enum259.BATTLE_SOUND_HIT1 ||
      ubBattleSoundID == Enum259.BATTLE_SOUND_HIT2
    ) {
      switch (pSoldier.ubBodyType) {
        case Enum194.COW:
          fDoSub = true;
    ...
    if (fDoSub) {
      if (guiCurrentScreen != Enum26.GAME_SCREEN) {
        ...
        PlayJA2Sample(
          uiSubSoundID,
          RATE_11025,
          SoundVolume(CalculateSpeechVolume(HIGHVOLUME), pSoldier.sGridNo),
          1,
          SoundDir(pSoldier.sGridNo),
        );
      }
    ...
    if (
      ...
      if (
        GetJA2Clock() - pSoldier.uiTimeSameBattleSndDone <
        MIN_SUBSEQUENT_SNDS_DELAY
      ) {
        return true;
      }
    }
    ...
    if (ubSoundID == Enum259.BATTLE_SOUND_OK1) {
      if (gGameSettings.fOptions[Enum8.TOPTION_MUTE_CONFIRMATIONS]) return true;
      // else a speech sound is to be played
      else fSpeechSound = true;
    }
    ...
    if (pSoldier.ubProfile != NO_PROFILE) {
      zFilename = sprintf(
        "BATTLESNDS\\%s_%s.wav",
        pSoldier.ubProfile.toString().padStart(3, "0"),
      ...
      if (
        ...
        if (ubSoundID == Enum259.BATTLE_SOUND_DIE1) {
          zFilename = sprintf(
            "BATTLESNDS\\kid%d_dying.wav",
            pSoldier.ubBattleSoundID,
          );
        } else {
          zFilename = sprintf(
            "BATTLESNDS\\kid%d_%s.wav",
            pSoldier.ubBattleSoundID,
            gBattleSndsData[ubSoundID].zName,
          );
        }
        ...
        if (ubSoundID == Enum259.BATTLE_SOUND_DIE1) {
          zFilename = sprintf(
            "BATTLESNDS\\bad%d_die.wav",
            pSoldier.ubBattleSoundID,
          );
        } else {
          zFilename = sprintf(
            "BATTLESNDS\\bad%d_%s.wav",
            pSoldier.ubBattleSoundID,
            gBattleSndsData[ubSoundID].zName,
          );
        }
  ...
  export function DoMercBattleSound(
    pSoldier: SOLDIERTYPE,
    ubBattleSoundID: UINT8,
  ): boolean {
    // We WANT to play some RIGHT AWAY.....
    if (
      gBattleSndsData[ubBattleSoundID].fStopDialogue == 1 ||
      pSoldier.ubProfile == NO_PROFILE ||
      InOverheadMap()
    ) {
  ...
  function PreloadSoldierBattleSounds(
    pSoldier: SOLDIERTYPE,
    fRemove: boolean,
  ): boolean {
    let cnt: UINT32;
  ...
  export function CheckSoldierHitRoof(pSoldier: SOLDIERTYPE): boolean {
    ...
    if (
      ...
      if (gAnimControl[pSoldier.usAnimState].ubHeight == ANIM_STAND) {
        ...
        sNewGridNo = NewGridNo(
          pSoldier.sGridNo,
          DirectionInc(gOppositeDirection[bNewDirection]),
        );
        ...
        if (
          pSoldier.bDirection == gOneCDirection[bNewDirection] ||
        ...
        if (fDoForwards) {
          ...
          EVENT_SetSoldierDesiredDirection(
            pSoldier,
            gOppositeDirection[bNewDirection],
          );
          pSoldier.fTurningUntilDone = true;
          pSoldier.usPendingAnimation = Enum193.FALLFORWARD_ROOF;
  ...
  export function GetDirectionFromGridNo(
    sGridNo: INT16,
    pSoldier: SOLDIERTYPE,
  ): INT16 {
    let sXPos: INT16;
  ...
  export function GetDirectionToGridNoFromGridNo(
    sGridNoDest: INT16,
    sGridNoSrc: INT16,
  ): INT16 {
    let sXPos2: INT16;
  ...
  export function GetDirectionFromXY(
    sXPos: INT16,
    sYPos: INT16,
    pSoldier: SOLDIERTYPE,
  ): INT16 {
  ...
  export function atan8(
    ...
    do {
      if (angle >= -Math.PI * 0.375 && angle <= -Math.PI * 0.125) {
      ...
      if (angle >= Math.PI * 0.623 && angle <= Math.PI * 0.875) {
      ...
      if (
        (angle > Math.PI * 0.875 && angle <= Math.PI) ||
        (angle > -Math.PI && angle < -Math.PI * 0.875)
      ) {
    ...
    for (cnt = 0; cnt < MAX_FULLTILE_DIRECTIONS; cnt++) {
      ...
      if (
        ...
        if (pSoldier.usFrontArcFullTileList[cnt] != 0) {
          RemoveTopmost(
            pSoldier.usFrontArcFullTileGridNos[cnt],
  ...
  function CheckForFullStruct(
    ...
    while (pStruct != null) {
      if (
        pStruct.usIndex != NO_TILE &&
        pStruct.usIndex < Enum312.NUMBEROFTILES
      ) {
  ...
  function AdjustForFastTurnAnimation(pSoldier: SOLDIERTYPE): void {
    ...
    if (
      gAnimControl[pSoldier.usAnimState].uiFlags & ANIM_FASTTURN &&
      pSoldier.bTeam == gbPlayerNum &&
      !(pSoldier.uiStatusFlags & SOLDIER_TURNINGFROMHIT)
    ) {
      if (pSoldier.bDirection != pSoldier.bDesiredDirection) {
  ...
  function SendSoldierPositionEvent(
    pSoldier: SOLDIERTYPE,
    dNewXPos: FLOAT,
    dNewYPos: FLOAT,
  ): void {
  ...
  function SendSoldierDestinationEvent(
    pSoldier: SOLDIERTYPE,
  ...
  function SendSoldierSetDirectionEvent(
    pSoldier: SOLDIERTYPE,
    usNewDirection: UINT16,
  ): void {
    // Sent event for position update
  ...
  export function SendSoldierSetDesiredDirectionEvent(
    pSoldier: SOLDIERTYPE,
    usDesiredDirection: UINT16,
  ): void {
    // Sent event for position update
    let SSetDesiredDirection: EV_S_SETDESIREDDIRECTION =
      createEvSSetDesiredDirection();
  ...
  export function SendGetNewSoldierPathEvent(
    pSoldier: SOLDIERTYPE,
    sDestGridNo: UINT16,
    usMovementAnim: UINT16,
  ): void {
  ...
  export function SendChangeSoldierStanceEvent(
    pSoldier: SOLDIERTYPE,
    ubNewStance: UINT8,
  ): void {
  ...
  export function SendBeginFireWeaponEvent(
    pSoldier: SOLDIERTYPE,
    sTargetGridNo: INT16,
  ): void {
      ...
      // if ( pSoldier->ubAttackerID != NOBODY )
      {
        ...
        for (cnt = 0; cnt < ubNumToFree; cnt++) {
          DebugMsg(
            ...
            DBG_LEVEL_3,
            FormatString(
              "@@@@@@@ Freeing up attacker of %d (attacker is %d) - releasesoldierattacker num to free is %d",
              pSoldier.ubID,
              pSoldier.ubAttackerID,
              ubNumToFree,
            ),
  ...
  function HandleAnimationProfile(
    pSoldier: SOLDIERTYPE,
    usAnimState: UINT16,
    fRemove: boolean,
  ): void {
    ...
    if (bProfileID != -1) {
      ...
      for (iTileCount = 0; iTileCount < pProfileDir.ubNumTiles; iTileCount++) {
        ...
        sGridNo =
          pSoldier.sGridNo +
          (WORLD_COLS * pProfileTile.bTileY + pProfileTile.bTileX);
        ...
        if (!OutOfBounds(pSoldier.sGridNo, sGridNo)) {
          if (fRemove) {
            ...
            {
              (<LEVELNODE>gpWorldLevelData[sGridNo].pMercHead).uiFlags |=
                LEVELNODE_MERCPLACEHOLDER;
              (<LEVELNODE>(
                gpWorldLevelData[sGridNo].pMercHead
              )).uiAnimHitLocationFlags = pProfileTile.usTileFlags;
            }
          }
        }
      }
  ...
  export function GetAnimProfileFlags(
    sGridNo: UINT16,
    usFlags: Pointer<UINT16>,
    ppTargSoldier: Pointer<SOLDIERTYPE | null>,
    pGivenNode: LEVELNODE | null,
  ): LEVELNODE | null {
  ...
  function GetProfileFlagsFromGridno(
    pSoldier: SOLDIERTYPE,
    usAnimState: UINT16,
    sTestGridNo: UINT16,
    usFlags: Pointer<UINT16>,
  ...
  export function EVENT_SoldierBeginBladeAttack(
    pSoldier: SOLDIERTYPE,
    sGridNo: INT16,
    ubDirection: UINT8,
  ): void {
    ...
    if (pSoldier.uiStatusFlags & SOLDIER_MONSTER) {
      ...
      if (
        ubTargetID != NOBODY &&
        ((MercPtrs[ubTargetID].bLife < OKLIFE &&
          MercPtrs[ubTargetID].bLife > 0) ||
          (MercPtrs[ubTargetID].bBreath < OKBREATH &&
            MercPtrs[ubTargetID].bCollapsed))
      ) {
      ...
      if (
        pSoldier.inv[Enum261.HANDPOS].usItem == Enum225.BLOODCAT_CLAW_ATTACK
      ) {
        EVENT_InitNewSoldierAnim(pSoldier, Enum193.BLOODCAT_SWIPE, 0, false);
      } else {
        EVENT_InitNewSoldierAnim(
          pSoldier,
          Enum193.BLOODCAT_BITE_ANIM,
          0,
          false,
        );
      }
  ...
  export function EVENT_SoldierBeginPunchAttack(
    pSoldier: SOLDIERTYPE,
    sGridNo: INT16,
    ubDirection: UINT8,
  ): void {
    ...
    if (fMartialArtist && !AreInMeanwhile() && usItem != Enum225.CROWBAR) {
      ...
      if (
        pSoldier.usAnimState != Enum193.NINJA_BREATH &&
  ...
  export function EVENT_SoldierBeginKnifeThrowAttack(
    pSoldier: SOLDIERTYPE,
    sGridNo: INT16,
    ubDirection: UINT8,
  ): void {
    ...
    DebugMsg(
      ...
      DBG_LEVEL_3,
      FormatString(
        "!!!!!!! Starting knifethrow attack, bullets left %d",
        pSoldier.bBulletsLeft,
      ),
    );
  ...
  export function EVENT_SoldierBeginFirstAid(
    pSoldier: SOLDIERTYPE,
    sGridNo: INT16,
    ubDirection: UINT8,
  ): void {
    ...
    if (usSoldierIndex != NOBODY) {
      ...
      if (pTSoldier.bTeam != gbPlayerNum) {
        if (
          pTSoldier.ubProfile != NO_PROFILE &&
          pTSoldier.ubProfile >= FIRST_RPC &&
        ...
        if (!fRefused) {
          if (CREATURE_OR_BLOODCAT(pTSoldier)) {
            ...
            ScreenMsg(
              FONT_MCOLOR_LTYELLOW,
              MSG_UI_FEEDBACK,
              Message[Enum334.STR_REFUSE_FIRSTAID_FOR_CREATURE],
            );
          } else if (
            !pTSoldier.bNeutral &&
            pTSoldier.bLife >= OKLIFE &&
            ...
            ScreenMsg(
              FONT_MCOLOR_LTYELLOW,
              MSG_UI_FEEDBACK,
              Message[Enum334.STR_REFUSE_FIRSTAID],
            );
          }
        }
      }
  ...
  export function EVENT_SoldierEnterVehicle(
    pSoldier: SOLDIERTYPE,
    sGridNo: INT16,
    ubDirection: UINT8,
  ): void {
  ...
  export function SoldierDressWound(
    pSoldier: SOLDIERTYPE,
    pVictim: SOLDIERTYPE,
    sKitPts: INT16,
    sStatus: INT16,
    ...
    uiDressSkill =
      (3 * EffectiveMedical(pSoldier) + // medical knowledge
    ...
    if (uiActual > uiDeficiency)
      // if we were about to apply too much
  ...
  function InternalReceivingSoldierCancelServices(
    pSoldier: SOLDIERTYPE,
    fPlayEndAnim: boolean,
  ): void {
    let pTSoldier: SOLDIERTYPE;
    let cnt: INT32;
    ...
    if (pSoldier.ubServiceCount > 0) {
      ...
      for (
        pTSoldier = Menptr[0], cnt = 0;
        cnt < MAX_NUM_SOLDIERS;
        cnt++, pTSoldier = Menptr[cnt]
      ) {
  ...
  export function InternalGivingSoldierCancelServices(
    pSoldier: SOLDIERTYPE,
    fPlayEndAnim: boolean,
  ): void {
    let pTSoldier: SOLDIERTYPE;
  ...
  export function HaultSoldierFromSighting(
    pSoldier: SOLDIERTYPE,
    fFromSightingEnemy: boolean,
  ): void {
    // SEND HUALT EVENT!
    ...
    if (
      fFromSightingEnemy ||
      (pSoldier.pTempObject == null && !pSoldier.fTurningToShoot)
    ) {
      UnSetUIBusy(pSoldier.ubID);
    }
  ...
  export function EVENT_StopMerc(
    pSoldier: SOLDIERTYPE,
    sGridNo: INT16,
    bDirection: INT8,
  ): void {
  ...
  export function ReLoadSoldierAnimationDueToHandItemChange(
    pSoldier: SOLDIERTYPE,
    usOldItem: UINT16,
    usNewItem: UINT16,
  ): void {
    ...
    if (usOldItem != NOTHING) {
      if (Item[usOldItem].usItemClass == IC_GUN) {
        if (
          Item[usOldItem].fFlags & ITEM_TWO_HANDED &&
          usOldItem != Enum225.ROCKET_LAUNCHER
        ) {
          fOldRifle = true;
        }
      }
    ...
    if (usNewItem != NOTHING) {
      if (Item[usNewItem].usItemClass == IC_GUN) {
        if (
          Item[usNewItem].fFlags & ITEM_TWO_HANDED &&
  ...
  function CreateEnemyGlow16BPPPalette(
    pPalette: SGPPaletteEntry[],
    rscale: UINT32,
    gscale: UINT32,
    fAdjustGreen: boolean,
  ): Uint16Array {
    ...
    for (cnt = 0; cnt < 256; cnt++) {
      ...
      if (gusRedShift < 0) r16 = r >> -gusRedShift;
      else r16 = r << gusRedShift;

      if (gusGreenShift < 0) g16 = g >> -gusGreenShift;
      else g16 = g << gusGreenShift;

      if (gusBlueShift < 0) b16 = b >> -gusBlueShift;
      else b16 = b << gusBlueShift;
  ...
  function CreateEnemyGreyGlow16BPPPalette(
    pPalette: SGPPaletteEntry[],
    rscale: UINT32,
    gscale: UINT32,
    fAdjustGreen: boolean,
    ...
    for (cnt = 0; cnt < 256; cnt++) {
      lumin =
        Math.trunc((pPalette[cnt].peRed * 299) / 1000) +
        Math.trunc((pPalette[cnt].peGreen * 587) / 1000) +
        Math.trunc((pPalette[cnt].peBlue * 114) / 1000);
      ...
      if (EnoughPoints(pSoldier, sAPCost, 0, pSoldier.bTeam == gbPlayerNum)) {
        ...
        if (pSoldier.bTeam == gbPlayerNum) {
          ...
          if (pSoldier.iFaceIndex != -1) {
            gFacesData[pSoldier.iFaceIndex].fDisplayTextOver =
              FACE_ERASE_TEXT_OVER;
          }
        }
  ...
  export function CheckForBreathCollapse(pSoldier: SOLDIERTYPE): boolean {
    ...
    if (
      ...
      !(
        ...
        (SOLDIER_VEHICLE | SOLDIER_ANIMAL | SOLDIER_MONSTER)
      )
    ) {
      // Collapse!
  ...
  export function InternalIsValidStance(
    pSoldier: SOLDIERTYPE,
    bDirection: INT8,
    bNewStance: INT8,
  ): boolean {
    ...
    switch (bNewStance) {
      ...
      default:
        ...
        ScreenMsg(
          FONT_MCOLOR_LTYELLOW,
          MSG_BETAVERSION,
          "Wrong desired stance given: %d, %d.",
          bNewStance,
          pSoldier.usAnimState,
        );
    }
    ...
    if (pStructureFileRef != null) {
      ...
      if (
        !OkayToAddStructureToWorld(
          pSoldier.sGridNo,
          pSoldier.bLevel,
          pStructureFileRef.pDBStructureRef[gOneCDirection[bDirection]],
          usOKToAddStructID,
        )
      ) {
        return false;
  ...
  export function IsValidMovementMode(
    pSoldier: SOLDIERTYPE,
    usMovementMode: INT16,
  ): boolean {
    // Check, if dest is prone, we can actually do this!
    ...
    if (MercInWater(pSoldier)) {
      if (
        ...
        usMovementMode == Enum193.SWATTING ||
        usMovementMode == Enum193.CRAWLING
      ) {
        return false;
      }
    }
  ...
  function GetActualSoldierAnimDims(pSoldier: SOLDIERTYPE): {
    sHeight: INT16;
    sWidth: INT16;
  } {
    let sHeight: INT16;
    ...
    pTrav =
      gAnimSurfaceDatabase[usAnimSurface].hVideoObject.pETRLEObject[
        pSoldier.usAniFrame
      ];
  ...
  function GetActualSoldierAnimOffsets(pSoldier: SOLDIERTYPE): {
    sOffsetX: INT16;
    sOffsetY: INT16;
  } {
    let sOffsetX: INT16;
  ...
  function CheckBleeding(pSoldier: SOLDIERTYPE): INT32 {
    ...
    if (pSoldier.bLife != 0) {
      ...
      if (
        pSoldier.bBleeding > MIN_BLEEDING_THRESHOLD ||
        pSoldier.bLife < OKLIFE
      ) {
        ...
        if (
          ...
          if (pSoldier.bTilesMoved < 1) {
            iBlood = Math.trunc(
              (pSoldier.bBleeding - MIN_BLEEDING_THRESHOLD) / BLOODDIVISOR,
            ); // + pSoldier->dying;
          ...
          if (pSoldier.dNextBleed <= 0) {
            ...
            } // soldier is either not bandaged at all or is dying
            else {
              if (pSoldier.bLife < OKLIFE) {
                // if he's dying
                ...
                if (
                  ...
                  !pSoldier.fDyingComment
                ) {
                  TacticalCharacterDialogue(
                    pSoldier,
                    Enum202.QUOTE_SERIOUSLY_WOUNDED,
                  );
  ...
  export function SoldierCollapse(pSoldier: SOLDIERTYPE): void {
    ...
    if (pSoldier.uiStatusFlags & SOLDIER_ENEMY) {
      if (
        !gTacticalStatus.bPanicTriggerIsAlarm &&
  ...
  function CalcSoldierNextBleed(pSoldier: SOLDIERTYPE): FLOAT {
    ...
    return (
      1 +
      Math.trunc(
        (pSoldier.bLife + Math.trunc(bBandaged / 2)) /
          (10 + pSoldier.bTilesMoved),
      )
    ); // min = 1
  }
  ...
  export function HandlePlacingRoofMarker(
    pSoldier: SOLDIERTYPE,
    sGridNo: INT16,
    fSet: boolean,
    fForce: boolean,
  ...
  export function PositionSoldierLight(pSoldier: SOLDIERTYPE): void {
    ...
    // if ( pSoldier->ubID == gusSelectedSoldier )
    {
      ...
      LightSpritePosition(
        pSoldier.iLight,
        Math.trunc(pSoldier.sX / CELL_X_SIZE),
        Math.trunc(pSoldier.sY / CELL_Y_SIZE),
      );
    }
  }
  ...
  export function PickPickupAnimation(
    pSoldier: SOLDIERTYPE,
    iItemIndex: INT32,
    sGridNo: INT16,
    bZLevel: INT8,
  ...
  export function EVENT_SoldierBeginCutFence(
    pSoldier: SOLDIERTYPE,
    sGridNo: INT16,
    ubDirection: UINT8,
  ): void {
  ...
  export function EVENT_SoldierBeginRepair(
    pSoldier: SOLDIERTYPE,
    sGridNo: INT16,
    ubDirection: UINT8,
  ): void {
    ...
    // Make sure we have a structure here....
    bRepairItem = IsRepairableStructAtGridNo(
      sGridNo,
      createPointer(
        () => ubID,
        (v) => (ubID = v),
  ...
  export function EVENT_SoldierBeginRefuel(
    pSoldier: SOLDIERTYPE,
    sGridNo: INT16,
    ubDirection: UINT8,
  ): void {
    ...
    // Make sure we have a structure here....
    bRefuelItem = IsRefuelableStructAtGridNo(
      sGridNo,
      createPointer(
        () => ubID,
        (v) => (ubID = v),
  ...
  export function EVENT_SoldierBeginTakeBlood(
    pSoldier: SOLDIERTYPE,
    sGridNo: INT16,
    ubDirection: UINT8,
  ): void {
    ...
    if (pCorpse != null) {
      ...
      EVENT_InitNewSoldierAnim(
        pSoldier,
        Enum193.TAKE_BLOOD_FROM_CORPSE,
        0,
        false,
      );
    } else {
  ...
  export function EVENT_SoldierBeginAttachCan(
    pSoldier: SOLDIERTYPE,
    sGridNo: INT16,
    ubDirection: UINT8,
  ): void {
    ...
    if (
      pSoldier.inv[Enum261.HANDPOS].usItem != Enum225.STRING_TIED_TO_TIN_CAN
    ) {
      return;
    }
  ...
  export function EVENT_SoldierBeginReloadRobot(
    pSoldier: SOLDIERTYPE,
    sGridNo: INT16,
    ubDirection: UINT8,
    ubMercSlot: UINT8,
  ...
  function ChangeToFlybackAnimation(
    pSoldier: SOLDIERTYPE,
    bDirection: INT8,
  ): void {
    let usNewGridNo: UINT16;
  ...
  export function ChangeToFallbackAnimation(
    pSoldier: SOLDIERTYPE,
    bDirection: INT8,
  ): void {
    let usNewGridNo: UINT16;
  ...
  export function SetSoldierCowerState(
    pSoldier: SOLDIERTYPE,
    fOn: boolean,
  ): void {
    // Robot's don't cower!
    if (pSoldier.ubBodyType == Enum194.ROBOTNOWEAPON) {
      DebugMsg(
        TOPIC_JA2,
        DBG_LEVEL_3,
        FormatString("ERROR: Robot was told to cower!"),
      );
      return;
    }
  ...
  export function MercStealFromMerc(
    pSoldier: SOLDIERTYPE,
    pTarget: SOLDIERTYPE,
  ): void {
  ...
  export function PlayerSoldierStartTalking(
    pSoldier: SOLDIERTYPE,
    ubTargetID: UINT8,
    fValidate: boolean,
  ): boolean {
    ...
    if (
      pTSoldier.ubProfile != NO_PROFILE &&
      pTSoldier.ubWhatKindOfMercAmI == Enum260.MERC_TYPE__EPC
    ) {
      return InitiateConversation(
        pTSoldier,
        pSoldier,
        Enum296.APPROACH_EPC_WHO_IS_RECRUITED,
        0,
      );
      ...
      return InitiateConversation(
        pTSoldier,
        pSoldier,
        Enum296.APPROACH_ENEMY_NPC_QUOTE,
        0,
      );
    }
  ...
  export function IsValidSecondHandShot(pSoldier: SOLDIERTYPE): boolean {
    if (
      ...
      !(
        Item[pSoldier.inv[Enum261.SECONDHANDPOS].usItem].fFlags &
        ITEM_TWO_HANDED
      ) &&
      !pSoldier.bDoBurst &&
      pSoldier.inv[Enum261.HANDPOS].usItem != Enum225.GLAUNCHER &&
      Item[pSoldier.inv[Enum261.HANDPOS].usItem].usItemClass == IC_GUN &&
      pSoldier.inv[Enum261.SECONDHANDPOS].bGunStatus >= USABLE &&
      pSoldier.inv[Enum261.SECONDHANDPOS].ubGunShotsLeft > 0
    ) {
      return true;
    }
  ...
  export function IsValidSecondHandShotForReloadingPurposes(
    pSoldier: SOLDIERTYPE,
  ): boolean {
  ...
  export function ControllingRobot(pSoldier: SOLDIERTYPE): boolean {
    ...
    if (pRobot.bActive) {
      ...
      if (
        pRobot.sSectorX == pSoldier.sSectorX &&
        ...
        if (pRobot.fBetweenSectors == pSoldier.fBetweenSectors) {
          ...
          if (pRobot.fBetweenSectors) {
            ...
            if (
              pRobot.bAssignment == Enum117.VEHICLE &&
              pRobot.iVehicleId != pSoldier.iVehicleId
            ) {
              return false;
            }
          }
  ...
  export function GetRobotController(
    pSoldier: SOLDIERTYPE,
  ): SOLDIERTYPE | null {
    if (pSoldier.ubRobotRemoteHolderID == NOBODY) {
      return null;
  ...
  export function UpdateRobotControllerGivenController(
    pSoldier: SOLDIERTYPE,
  ): void {
    let pTeamSoldier: SOLDIERTYPE;
    let cnt: INT32 = 0;
  ...
  export function HandleSystemNewAISituation(
    pSoldier: SOLDIERTYPE,
    fResetABC: boolean,
  ): void {
    // Are we an AI guy?
    ...
    if (!(pSoldier.uiStatusFlags & SOLDIER_VEHICLE)) {
      ...
      if (pSoldier.uiStatusFlags & SOLDIER_ROBOT) {
        PlaySoldierJA2Sample(
          ...
          Enum330.ROBOT_BEEP,
          RATE_11025,
          SoundVolume(bVolume, pSoldier.sGridNo),
          1,
          SoundDir(pSoldier.sGridNo),
          true,
        );
      ...
      // if ( SoldierOnScreen( pSoldier->ubID ) )
      {
        ...
        PlaySoldierJA2Sample(
          pSoldier.ubID,
          ubSoundBase + pSoldier.ubLastFootPrintSound,
          RATE_11025,
          SoundVolume(bVolume, pSoldier.sGridNo),
          1,
          SoundDir(pSoldier.sGridNo),
  ...
  export function PlayStealthySoldierFootstepSound(
    pSoldier: SOLDIERTYPE,
  ): void {
  ...
  export function CrowsFlyAway(ubTeam: UINT8): void {
    ...
    for (
      cnt = gTacticalStatus.Team[ubTeam].bFirstID, pTeamSoldier = MercPtrs[cnt];
      cnt <= gTacticalStatus.Team[ubTeam].bLastID;
      cnt++, pTeamSoldier = MercPtrs[cnt]
    ) {
      if (pTeamSoldier.bActive && pTeamSoldier.bInSector) {
        if (
          pTeamSoldier.ubBodyType == Enum194.CROW &&
          pTeamSoldier.usAnimState != Enum193.CROW_FLY
        ) {
  ...
  export function SetSoldierAsUnderAiControl(pSoldierToSet: SOLDIERTYPE): void {
    ...
    for (
      pSoldier = MercPtrs[cnt];
      cnt <= gTacticalStatus.Team[LAST_TEAM].bLastID;
      cnt++, pSoldier = MercPtrs[cnt]
    ) {
      if (pSoldier.bActive) {
        pSoldier.uiStatusFlags &= ~SOLDIER_UNDERAICONTROL;
  ...
  export function HandlePlayerTogglingLightEffects(
    fToggleValue: boolean,
  ): void {
    if (fToggleValue) {
      // Toggle light status
      gGameSettings.fOptions[Enum8.TOPTION_MERC_CASTS_LIGHT] =
