namespace ja2 {
  ...
  function GetFreeWorldBombIndex(): INT32 {
    ...
    for (uiCount = 0; uiCount < guiNumWorldBombs; uiCount++) {
      if (gWorldBombs[uiCount].fExists == false) return uiCount;
    }
  ...
  function RemoveBombFromWorldByItemIndex(iItemIndex: INT32): void {
    ...
    for (uiBombIndex = 0; uiBombIndex < guiNumWorldBombs; uiBombIndex++) {
      if (
        gWorldBombs[uiBombIndex].fExists &&
        gWorldBombs[uiBombIndex].iItemIndex == iItemIndex
      ) {
  ...
  export function FindWorldItemForBombInGridNo(
    sGridNo: INT16,
    bLevel: INT8,
  ): INT32 {
    let uiBombIndex: UINT32;
  ...
  export function FindPanicBombsAndTriggers(): void {
    ...
    for (uiBombIndex = 0; uiBombIndex < guiNumWorldBombs; uiBombIndex++) {
      if (gWorldBombs[uiBombIndex].fExists) {
        ...
        if (
          pObj.bFrequency == PANIC_FREQUENCY ||
          pObj.bFrequency == PANIC_FREQUENCY_2 ||
  ...
  function GetFreeWorldItemIndex(): INT32 {
    ...
    for (uiCount = 0; uiCount < guiNumWorldItems; uiCount++) {
      if (gWorldItems[uiCount].fExists == false) return uiCount;
    }
  ...
  export function AddItemToWorld(
    sGridNo: INT16,
    pObject: OBJECTTYPE,
    ubLevel: UINT8,
    usFlags: UINT16,
  ...
  export function LoadWorldItemsFromMap(
    buffer: Buffer,
    offset: number,
  ): number {
    // Start loading itmes...
    ...
    uiNumWorldItems = buffer.readUInt32LE(offset);
    offset += 4;
      ...
      for (i = 0; i < uiNumWorldItems; i++) {
        ...
        if (gfEditMode || dummyItem.ubNonExistChance <= PreRandom(100)) {
          if (!gfEditMode) {
            ...
            if (
              (dummyItem.usFlags & WORLD_ITEM_SCIFI_ONLY &&
                !gGameOptions.fSciFi) ||
            ...
            if (!gGameOptions.fGunNut) {
              ...
              if (Item[dummyItem.o.usItem].usItemClass == IC_GUN) {
                ...
                if (usReplacement) {
                  ...
                  bNewAmmo = Math.trunc(
                    (Weapon[usReplacement].ubMagSize * bAmmo) /
              ...
              if (Item[dummyItem.o.usItem].usItemClass == IC_AMMO) {
                usReplacement = StandardGunListAmmoReplacement(
                  dummyItem.o.usItem,
                );
                if (usReplacement) {
                  let ubLoop: UINT8;
                  ...
                  for (
                    ubLoop = 0;
                    ubLoop < dummyItem.o.ubNumberOfObjects;
                    ubLoop++
                  ) {
                    dummyItem.o.bStatus[ubLoop] = Math.trunc(
                      (dummyItem.o.bStatus[ubLoop] *
                        Magazine[Item[usReplacement].ubClassIndex].ubMagSize) /
                        Magazine[Item[dummyItem.o.usItem].ubClassIndex]
                          .ubMagSize,
                    );
                  }
          ...
          if (
            dummyItem.o.usItem == Enum225.ACTION_ITEM &&
            gfLoadPitsWithoutArming
          ) {
            // if we are loading a pit, they are typically loaded without being armed.
            if (
              dummyItem.o.bActionValue == Enum191.ACTION_ITEM_SMALL_PIT ||
              dummyItem.o.bActionValue == Enum191.ACTION_ITEM_LARGE_PIT
            ) {
            ...
            dummyItem.bVisible == HIDDEN_ITEM &&
            dummyItem.o.bTrap > 0 &&
            (dummyItem.o.usItem == Enum225.MINE ||
              dummyItem.o.usItem == Enum225.TRIP_FLARE ||
              dummyItem.o.usItem == Enum225.TRIP_KLAXON)
          ) {
            ArmBomb(dummyItem.o, Enum224.BOMB_PRESSURE);
            dummyItem.usFlags |= WORLD_ITEM_ARMED_BOMB;
            // this is coming from the map so the enemy must know about it.
            gpWorldLevelData[dummyItem.sGridNo].uiFlags |=
          ...
          AddItemToPoolAndGetIndex(
            ...
            dummyItem.ubLevel,
            dummyItem.usFlags,
            dummyItem.bRenderZHeightAboveLevel,
            createPointer(
              () => iItemIndex,
              (v) => (iItemIndex = v),
            ),
      ...
      for (uiLoop = 0; uiLoop < guiNumWorldItems; uiLoop++) {
        ...
        if (
          ...
          if (IsProfileATerrorist(gWorldItems[uiLoop].o.ubOwnerProfile)) {
            ...
            if (
              gMercProfiles[gWorldItems[uiLoop].o.ubOwnerProfile].sSectorX !=
                gWorldSectorX ||
              gMercProfiles[gWorldItems[uiLoop].o.ubOwnerProfile].sSectorY !=
                gWorldSectorY
            ) {
              ...
              for (uiLoop2 = 0; uiLoop2 < guiNumWorldItems; uiLoop2++) {
                // loop through all items, look for ownership
                if (
                  gWorldItems[uiLoop2].fExists &&
                  gWorldItems[uiLoop2].sGridNo == sGridNo &&
  ...
  function DeleteWorldItemsBelongingToQueenIfThere(): void {
    ...
    if (
      gMercProfiles[Enum268.QUEEN].sSectorX == gWorldSectorX &&
      gMercProfiles[Enum268.QUEEN].sSectorY == gWorldSectorY &&
      gMercProfiles[Enum268.QUEEN].bSectorZ == gbWorldSectorZ
    ) {
      for (uiLoop = 0; uiLoop < guiNumWorldItems; uiLoop++) {
        // loop through all items, look for ownership
        if (
              ...
              if (
                ...
                switch (gWorldItems[uiLoop2].o.usItem) {
                  case Enum225.AUTO_ROCKET_RIFLE:
                    bSlot = FindObjectInSoldierProfile(
                      Enum268.QUEEN,
                      Enum225.ROCKET_RIFLE,
                    );
                    if (bSlot != NO_SLOT) {
                      // give her auto rifle
                      gMercProfiles[Enum268.QUEEN].inv[bSlot] =
                        Enum225.AUTO_ROCKET_RIFLE;
                    }
                    break;
                  case Enum225.SPECTRA_HELMET_18:
                    gMercProfiles[Enum268.QUEEN].inv[Enum261.HELMETPOS] =
                      Enum225.SPECTRA_HELMET_18;
                    break;
                  case Enum225.SPECTRA_VEST_18:
                    gMercProfiles[Enum268.QUEEN].inv[Enum261.VESTPOS] =
                      Enum225.SPECTRA_VEST_18;
                    break;
                  case Enum225.SPECTRA_LEGGINGS_18:
                    gMercProfiles[Enum268.QUEEN].inv[Enum261.LEGPOS] =
  ...
  // Refresh item pools
  export function RefreshWorldItemsIntoItemPools(
    pItemList: WORLDITEM[],
