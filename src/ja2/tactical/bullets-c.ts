namespace ja2 {
  ...
  let gBullets: BULLET[] /* [NUM_BULLET_SLOTS] */ = createArrayFrom(
    NUM_BULLET_SLOTS,
    createBullet,
  );
  ...
  function GetFreeBullet(): INT32 {
    ...
    for (uiCount = 0; uiCount < guiNumBullets; uiCount++) {
      if (gBullets[uiCount].fAllocated == false) return uiCount;
    }

    if (guiNumBullets < NUM_BULLET_SLOTS) return guiNumBullets++;
  ...
  export function HandleBulletSpecialFlags(iBulletIndex: INT32): void {
    ...
    if (pBullet.fReal) {
      ...
      if (pBullet.usFlags & BULLET_FLAG_KNIFE) {
        ...
        AniParams.sZ = CONVERT_HEIGHTUNITS_TO_PIXELS(
  ...
  export function UpdateBullets(): void {
    ...
    for (uiCount = 0; uiCount < guiNumBullets; uiCount++) {
      if (gBullets[uiCount].fAllocated) {
        if (
          gBullets[uiCount].fReal &&
          !(gBullets[uiCount].usFlags & BULLET_STOPPED)
        ) {
          ...
          // if ( !( gGameSettings.fOptions[ TOPTION_HIDE_BULLETS ] ) )
          {
            if (gBullets[uiCount].usFlags & BULLET_FLAG_KNIFE) {
              if (gBullets[uiCount].pAniTile != null) {
                (<ANITILE>gBullets[uiCount].pAniTile).sRelativeX =
                  FIXEDPT_TO_INT32(gBullets[uiCount].qCurrX);
                (<ANITILE>gBullets[uiCount].pAniTile).sRelativeY =
                  FIXEDPT_TO_INT32(gBullets[uiCount].qCurrY);
                (<ANITILE>gBullets[uiCount].pAniTile).pLevelNode.sRelativeZ =
                  CONVERT_HEIGHTUNITS_TO_PIXELS(
                    FIXEDPT_TO_INT32(gBullets[uiCount].qCurrZ),
                  );

                if (gBullets[uiCount].usFlags & BULLET_FLAG_KNIFE) {
                  (<ANITILE>gBullets[uiCount].pShadowAniTile).sRelativeX =
                    FIXEDPT_TO_INT32(gBullets[uiCount].qCurrX);
                  (<ANITILE>gBullets[uiCount].pShadowAniTile).sRelativeY =
                    FIXEDPT_TO_INT32(gBullets[uiCount].qCurrY);
                }
              }
            }
            // Are we a missle?
              ...
              pNode = <LEVELNODE>(
                AddStructToTail(gBullets[uiCount].sGridNo, Enum312.BULLETTILE1)
              );
              pNode.ubShadeLevel = DEFAULT_SHADE_LEVEL;
              pNode.ubNaturalShadeLevel = DEFAULT_SHADE_LEVEL;
              pNode.uiFlags |=
                LEVELNODE_USEABSOLUTEPOS | LEVELNODE_IGNOREHEIGHT;
              pNode.sRelativeX = FIXEDPT_TO_INT32(gBullets[uiCount].qCurrX);
              pNode.sRelativeY = FIXEDPT_TO_INT32(gBullets[uiCount].qCurrY);
              pNode.sRelativeZ = CONVERT_HEIGHTUNITS_TO_PIXELS(
                FIXEDPT_TO_INT32(gBullets[uiCount].qCurrZ),
              );

              // Display shadow
              pNode = <LEVELNODE>(
                AddStructToTail(gBullets[uiCount].sGridNo, Enum312.BULLETTILE2)
              );
              pNode.ubShadeLevel = DEFAULT_SHADE_LEVEL;
              pNode.ubNaturalShadeLevel = DEFAULT_SHADE_LEVEL;
              pNode.uiFlags |=
              ...
              pNode.sRelativeY = FIXEDPT_TO_INT32(gBullets[uiCount].qCurrY);
              pNode.sRelativeZ =
                gpWorldLevelData[gBullets[uiCount].sGridNo].sHeight;
            }
          }
        } else {
          if (gBullets[uiCount].fToDelete) {
  ...
  export function AddMissileTrail(
    pBullet: BULLET,
    qCurrX: FIXEDPT,
    qCurrY: FIXEDPT,
    qCurrZ: FIXEDPT,
  ...
  export function LoadBulletStructureFromSavedGameFile(hFile: HWFILE): boolean {
    ...
    for (usCnt = 0; usCnt < guiNumBullets; usCnt++) {
      ...
      else gBullets[usCnt].pFirer = <SOLDIERTYPE>(<unknown>null);
