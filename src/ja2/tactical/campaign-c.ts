namespace ja2 {
  ...
  type StatTimerKey =
    | "uiChangeHealthTime"
    | "uiChangeAgilityTime"
    | "uiChangeDexterityTime"
    | "uiChangeWisdomTime"
    | "uiChangeMedicalTime"
    | "uiChangeExplosivesTime"
    | "uiChangeMechanicalTime"
    | "uiChangeMarksmanshipTime"
    | "uiChangeLevelTime"
    | "uiChangeStrengthTime"
    | "uiChangeLeadershipTime";
  ...
  export function StatChange(
    ...
    if (!PTR_OURTEAM(pSoldier)) return;
    ...
    if (pSoldier.bAssignment == Enum117.ASSIGNMENT_POW) {
      ScreenMsg(
        FONT_ORANGE,
        MSG_BETAVERSION,
        "ERROR: StatChange: %s improving stats while POW! ubStat %d",
        pSoldier.name,
        ubStat,
      );
      return;
    ...
    ProcessStatChange(
      gMercProfiles[pSoldier.ubProfile],
      ubStat,
      usNumChances,
      ubReason,
    );
  ...
  function ProfileStatChange(
    pProfile: MERCPROFILESTRUCT,
    ubStat: UINT8,
    usNumChances: UINT16,
    ubReason: UINT8,
  ): void {
  ...
  function ProcessStatChange(
    ...
    if (pProfile.bEvolution == Enum274.NO_EVOLUTION) return; // No change possible, quit right away
    ...
    switch (ubStat) {
      ...
      default:
        ...
        ScreenMsg(
          FONT_ORANGE,
          MSG_BETAVERSION,
          "ERROR: ProcessStatChange: Rcvd unknown ubStat %d",
          ubStat,
        );
        return;
    }
    ...
    for (uiCnt = 0; uiCnt < usNumChances; uiCnt++) {
      if (pProfile.bEvolution == Enum274.NORMAL_EVOLUTION) {
        // Evolves!
        // if this is improving from a failure, and a successful roll would give us enough to go up a point
        if (
          ubReason == FROM_FAILURE &&
          pProfile[psStatGainPtr] + 1 >= usSubpointsPerPoint
        ) {
        ...
        if (ubStat != EXPERAMT) {
          ...
          usChance =
            100 -
            (bCurrentRating +
              Math.trunc(pProfile[psStatGainPtr] / usSubpointsPerPoint));

          // prevent training beyond the training cap
          if (
            ubReason == FROM_TRAINING &&
            bCurrentRating +
              Math.trunc(pProfile[psStatGainPtr] / usSubpointsPerPoint) >=
              TRAINING_RATING_CAP
          ) {
        ...
        if (usChance > 0 && fAffectedByWisdom) {
          usChance += Math.trunc(
            (usChance *
              (pProfile.bWisdom +
                Math.trunc(
                  pProfile.sWisdomGain /
                    SubpointsPerPoint(WISDOMAMT, pProfile.bExpLevel),
                ) -
                50)) /
              100,
          );
      ...
      */
        ...
        if (PreRandom(100) < usChance) {
          ...
          if (ubStat != EXPERAMT && ubReason != FROM_TRAINING) {
            uiEffLevel =
              pProfile.bExpLevel +
              Math.trunc(pProfile.sExpLevelGain / usSubpointsPerLevel);
      ...
      if (ubStat == EXPERAMT && fChangeTypeIncrease) {
        ...
        if (pSoldier != null) {
          switch (pSoldier.ubWhatKindOfMercAmI) {
            ...
            case Enum260.MERC_TYPE__MERC:
              ...
              AddStrategicEvent(
                Enum132.EVENT_MERC_MERC_WENT_UP_LEVEL_EMAIL_DELAY,
                GetWorldTotalMin() + 60 + Random(60),
                ubMercMercIdValue,
              );
        ...
        if (fChangeSalary) {
          ...
          for (uiLevelCnt = 0; uiLevelCnt < sPtsChanged; uiLevelCnt++) {
            pProfile.sSalary = CalcNewSalary(
              pProfile.sSalary,
              fChangeTypeIncrease,
              MAX_DAILY_SALARY,
            );
            pProfile.uiWeeklySalary = CalcNewSalary(
              pProfile.uiWeeklySalary,
              fChangeTypeIncrease,
              MAX_LARGE_SALARY,
            );
            pProfile.uiBiWeeklySalary = CalcNewSalary(
              pProfile.uiBiWeeklySalary,
              fChangeTypeIncrease,
              MAX_LARGE_SALARY,
            );
            pProfile.sTrueSalary = CalcNewSalary(
              pProfile.sTrueSalary,
              fChangeTypeIncrease,
              MAX_DAILY_SALARY,
            );
            pProfile.sMedicalDepositAmount = CalcNewSalary(
              pProfile.sMedicalDepositAmount,
              fChangeTypeIncrease,
              MAX_DAILY_SALARY,
            );
  ...
  function ProcessUpdateStats(
    ...
    for (
      ubStat = FIRST_CHANGEABLE_STAT;
      ubStat <= LAST_CHANGEABLE_STAT;
      ubStat++
    ) {
  ...
  export function HandleAnyStatChangesAfterAttack(): void {
    ...
    for (
      cnt = 0, pSoldier = MercPtrs[0];
      cnt <= gTacticalStatus.Team[MercPtrs[0].bTeam].bLastID;
  ...
  function RoundOffSalary(uiSalary: UINT32): UINT32 {
    ...
    if (uiSalary % uiMultiple) {
      ...
      uiSalary =
        Math.trunc((uiSalary + Math.trunc(uiMultiple / 2)) / uiMultiple) *
        uiMultiple;
    }
    ...
    switch (ubStat) {
      ...
      default:
        ...
        ScreenMsg(
          FONT_ORANGE,
          MSG_BETAVERSION,
          "SubpointsPerPoint: ERROR - Unknown ubStat %d",
          ubStat,
        );
        return 100;
    }
  ...
  export function HandleUnhiredMercImprovement(
    pProfile: MERCPROFILESTRUCT,
  ): void {
  ...
  export function CurrentPlayerProgressPercentage(): UINT8 {
    ...
    if (uiPossibleIncome > 0) {
      ubCurrentProgress = Math.trunc(
        (uiCurrentIncome * PROGRESS_PORTION_INCOME) / uiPossibleIncome,
      );
    } else {
      ubCurrentProgress = 0;
    }
    ...
    usKillsProgress = Math.trunc(
      gStrategicStatus.usPlayerKills / ubKillsPerPoint,
    );
  ...
  export function HourlyProgressUpdate(): void {
    ...
    if (ubCurrentProgress > gStrategicStatus.ubHighestProgress) {
      ...
      ScreenMsg(
        MSG_FONT_RED,
        MSG_DEBUG,
        "New player progress record: %d%%",
        gStrategicStatus.ubHighestProgress,
      );
    }
  }

  export function AwardExperienceBonusToActiveSquad(
    ubExpBonusType: UINT8,
  ): void {
    let usXPs: UINT16 = 0;
    let ubGuynum: UINT8;
    let pSoldier: SOLDIERTYPE;
    ...
    for (
      ubGuynum = gTacticalStatus.Team[gbPlayerNum].bFirstID,
        pSoldier = MercPtrs[ubGuynum];
      ubGuynum <= gTacticalStatus.Team[gbPlayerNum].bLastID;
      ubGuynum++, pSoldier = MercPtrs[ubGuynum]
    ) {
      if (
        pSoldier.bActive &&
        pSoldier.bInSector &&
  ...
  export function BuildStatChangeString(
    wName: string /* STR16 */,
    fIncrease: boolean,
    sPtsChanged: INT16,
    ubStat: UINT8,
  ...
  function CalcImportantSectorControl(): UINT8 {
    ...
    for (ubMapX = 1; ubMapX < MAP_WORLD_X - 1; ubMapX++) {
      for (ubMapY = 1; ubMapY < MAP_WORLD_Y - 1; ubMapY++) {
        ...
        if (
          StrategicMap[CALCULATE_STRATEGIC_INDEX(ubMapX, ubMapY)]
  ...
  export function MERCMercWentUpALevelSendEmail(
    ubMercMercIdValue: UINT8,
  ): void {
    let ubEmailOffset: UINT8 = 0;

    ubEmailOffset =
      MERC_UP_LEVEL_BIFF + MERC_UP_LEVEL_LENGTH_BIFF * ubMercMercIdValue;
    AddEmail(
      ubEmailOffset,
