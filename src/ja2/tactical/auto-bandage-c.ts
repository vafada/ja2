namespace ja2 {
  ...
  let iDoctorList: INT32[] /* [MAX_CHARACTER_COUNT] */ = createArray(
    MAX_CHARACTER_COUNT,
    0,
  );
  let iPatientList: INT32[] /* [MAX_CHARACTER_COUNT] */ = createArray(
    MAX_CHARACTER_COUNT,
    0,
  );
  ...
  let giAutoBandagesSoldierFaces: INT32[] /* [2 * MAX_CHARACTER_COUNT] */ =
    createArray(2 * MAX_CHARACTER_COUNT, 0);
    ...
    if (gTacticalStatus.fAutoBandageMode) {
      ...
      while (DequeueEvent(InputEvent) == true) {
        if (InputEvent.usEvent == KEY_UP) {
          if (
            (InputEvent.usParam == ESC && fAutoBandageComplete == false) ||
  ...
  function CreateAutoBandageString(): boolean {
    ...
    if (ubDoctors == 1) {
      uiDoctorNameStringLength +=
        Message[Enum334.STR_IS_APPLYING_FIRST_AID].length;
    } else {
      uiDoctorNameStringLength +=
        Message[Enum334.STR_ARE_APPLYING_FIRST_AID].length;
    }

    if (ubDoctors == 1) {
      sAutoBandageString = swprintf(
        Message[Enum334.STR_IS_APPLYING_FIRST_AID],
      ...
      sAutoBandageString = swprintf(
        Message[Enum334.STR_ARE_APPLYING_FIRST_AID],
        sTemp,
  ...
  export function AutoBandage(fStart: boolean): void {
    ...
    if (fStart) {
      ...
      gsX =
        Math.trunc((aRect.iRight - aRect.iLeft - gusTextBoxWidth) / 2) +
        aRect.iLeft;
      gsY =
        Math.trunc((aRect.iBottom - aRect.iTop - gusTextBoxHeight) / 2) +
        aRect.iTop;
        ...
        if (pSoldier.bActive) {
          ...
          if (pSoldier.bSlotItemTakenFrom != NO_SLOT) {
            ...
            SwapObjs(
              ...
              pSoldier.inv[pSoldier.bSlotItemTakenFrom],
            );
          }
  ...
  function DisplayAutoBandageUpdatePanel(): void {
    ...
    // the doctors
    iNumberDoctorsHigh =
      Math.trunc(iNumberDoctors / NUMBER_MERC_FACES_AUTOBANDAGE_BOX) + 1;
    ...
    if (iNumberPatients % NUMBER_MERC_FACES_AUTOBANDAGE_BOX) {
      // now the patients
      iNumberPatientsHigh =
        Math.trunc(iNumberPatients / NUMBER_MERC_FACES_AUTOBANDAGE_BOX) + 1;
    } else {
      // now the patients
      iNumberPatientsHigh = Math.trunc(
        iNumberPatients / NUMBER_MERC_FACES_AUTOBANDAGE_BOX,
      );
    }
    ...
    iTotalPixelsHigh =
      (iNumberPatientsHigh + iNumberDoctorsHigh) *
      TACT_UPDATE_MERC_FACE_X_HEIGHT;
    ...
    for (iCounterA = 0; iCounterA < iNumberDoctorsHigh; iCounterA++) {
      for (iCounterB = 0; iCounterB < iNumberDoctorsWide; iCounterB++) {
        ...
        if (iDoctorList[iIndex] != -1) {
          ...
          // there is a face
          RenderSoldierSmallFaceForAutoBandagePanel(
            iIndex,
            sCurrentXPosition,
            sCurrentYPosition,
          );
          ...
          sString = swprintf(
            "%s",
            Menptr[iDoctorList[iCounterA * iNumberDoctorsWide + iCounterB]]
              .name,
          );
          ({ sX, sY } = FindFontCenterCoordinates(
            sCurrentXPosition,
            sCurrentYPosition,
            TACT_UPDATE_MERC_FACE_X_WIDTH - 25,
            0,
            sString,
            TINYFONT1(),
          ));
    ...
    for (iCounterA = 0; iCounterA < iNumberPatientsHigh; iCounterA++) {
      for (iCounterB = 0; iCounterB < iNumberPatientsWide; iCounterB++) {
        ...
        sCurrentYPosition =
          sYPosition +
          (iCounterA + iNumberDoctorsHigh) * TACT_UPDATE_MERC_FACE_X_HEIGHT;
    ...
    for (iCounterA = 0; iCounterA < iNumberPatientsWide; iCounterA++) {
      ...
      BltVideoObject(
        ...
        hBackGroundHandle,
        1,
        sXPosition + TACT_UPDATE_MERC_FACE_X_WIDTH * iCounterA,
        sYPosition - 13,
        VO_BLT_SRCTRANSPARENCY,
        null,
      );
    ...
    if (iNumberPatientsWide == 2) {
      ...
      CreateTerminateAutoBandageButton(
        sXPosition,
        sYPosition + iTotalPixelsHigh + 3,
      );
    } else {
      BltVideoObject(
        FRAME_BUFFER,
      ...
      CreateTerminateAutoBandageButton(
        sXPosition + TACT_UPDATE_MERC_FACE_X_WIDTH,
        sYPosition + iTotalPixelsHigh + 3,
      );
    }
  ...
  function CreateTerminateAutoBandageButton(sX: INT16, sY: INT16): void {
    ...
    // grab the image
    iEndAutoBandageButtonImage[0] = LoadButtonImage(
    ...
    iEndAutoBandageButton[0] = QuickCreateButton(
      ...
      StopAutoBandageButtonCallback,
    );

    // the cancel button
    // grab the image
    iEndAutoBandageButtonImage[1] = LoadButtonImage(
      "INTERFACE\\group_confirm_tactical.sti",
      -1,
      7,
      -1,
  ...
  function RenderSoldierSmallFaceForAutoBandagePanel(
    iIndex: INT32,
    sCurrentXPosition: INT16,
    sCurrentYPosition: INT16,
  ): boolean {
    let iStartY: INT32 = 0;
    let pSoldier: SOLDIERTYPE;
    let iCounter: INT32 = 0;
    ...
    ColorFillVideoSurfaceArea(
      FRAME_BUFFER,
      sCurrentXPosition + 36,
      sCurrentYPosition + 2,
      sCurrentXPosition + 44,
      sCurrentYPosition + 30,
      0,
    );
    ...
    iStartY =
      sCurrentYPosition + 29 - Math.trunc((27 * pSoldier.bLifeMax) / 100);
    ColorFillVideoSurfaceArea(
      FRAME_BUFFER,
      sCurrentXPosition + 36,
      iStartY,
      sCurrentXPosition + 37,
      sCurrentYPosition + 29,
    ...
    // BREATH BAR
    iStartY =
      sCurrentYPosition + 29 - Math.trunc((27 * pSoldier.bBreathMax) / 100);
    ColorFillVideoSurfaceArea(
      FRAME_BUFFER,
      sCurrentXPosition + 39,
      iStartY,
      sCurrentXPosition + 40,
      sCurrentYPosition + 29,
    ...
    // MORALE BAR
    iStartY =
      sCurrentYPosition + 29 - Math.trunc((27 * pSoldier.bMorale) / 100);
    ColorFillVideoSurfaceArea(
      FRAME_BUFFER,
      sCurrentXPosition + 42,
      iStartY,
      sCurrentXPosition + 43,
      sCurrentYPosition + 29,
