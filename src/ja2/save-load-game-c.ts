namespace ja2 {
  ...
  export function createGeneralSaveInfo(): GENERAL_SAVE_INFO {
    return {
      ...
      fRefuelingSiteAvailable: createArray(
        Enum137.NUMBER_OF_REFUEL_SITES,
        false,
      ),
      ...
      gfInMeanwhile: false,
      sDeadMercs: createArrayFrom(Enum275.NUMBER_OF_SQUADS, () =>
  ...
  export function readGeneralSaveInfo(
    o: GENERAL_SAVE_INFO,
    buffer: Buffer,
    offset: number = 0,
  ): number {
    o.uiCurrentScreen = buffer.readUInt32LE(offset);
    offset += 4;
    o.uiCurrentUniqueSoldierId = buffer.readUInt32LE(offset);
    offset += 4;
    o.ubMusicMode = buffer.readUInt8(offset++);
    o.fHavePurchasedItemsFromTony = Boolean(buffer.readUInt8(offset++));
    o.usSelectedSoldier = buffer.readUInt16LE(offset);
    offset += 2;
    o.sRenderCenterX = buffer.readInt16LE(offset);
    offset += 2;
    o.sRenderCenterY = buffer.readInt16LE(offset);
    offset += 2;
    ...
    offset++; // padding
    o.iHelicopterVehicleId = buffer.readInt32LE(offset);
    offset += 4;
    o.UNUSEDiTotalHeliDistanceSinceRefuel = buffer.readInt32LE(offset);
    offset += 4;
    o.iTotalAccumulatedCostByPlayer = buffer.readInt32LE(offset);
    offset += 4;
    ...
    offset += 3; // padding
    o.uiStartHoverTime = buffer.readUInt32LE(offset);
    offset += 4;
    o.uiHelicopterSkyriderTalkState = buffer.readUInt32LE(offset);
    offset += 4;
    ...
    offset++; // padding
    o.uiEnvWeather = buffer.readUInt32LE(offset);
    offset += 4;
    ...
    o.sContractRehireSoldierID = buffer.readInt16LE(offset);
    offset += 2;
    offset = readGameOptions(o.GameOptions, buffer, offset);
    o.uiSeedNumber = buffer.readUInt32LE(offset);
    offset += 4;
    o.uiBaseJA2Clock = buffer.readUInt32LE(offset);
    offset += 4;
    o.sCurInterfacePanel = buffer.readInt16LE(offset);
    offset += 2;
    ...
    o.iSortStateForMapScreenList = buffer.readInt32LE(offset);
    offset += 4;
    o.fFoundTixa = Boolean(buffer.readUInt8(offset++));
    offset += 3; // padding
    o.uiTimeOfLastSkyriderMonologue = buffer.readUInt32LE(offset);
    offset += 4;
    ...
    offset++; // padding
    o.usOldMeanWhileFlags = buffer.readUInt16LE(offset);
    offset += 2;
    offset += 2; // padding
    o.iPortraitNumber = buffer.readInt32LE(offset);
    offset += 4;
    o.sWorldSectorLocationOfFirstBattle = buffer.readInt16LE(offset);
    offset += 2;
    ...
    offset += 3; // padding
    o.uiUNUSED = buffer.readUInt32LE(offset);
    offset += 4;
    ...
    offset++; // padding
    o.sSelMapX = buffer.readInt16LE(offset);
    offset += 2;
    o.sSelMapY = buffer.readInt16LE(offset);
    offset += 2;
    o.iCurrentMapSectorZ = buffer.readInt32LE(offset);
    offset += 4;
    o.usHasPlayerSeenHelpScreenInCurrentScreen = buffer.readUInt16LE(offset);
    offset += 2;
    ...
    o.sMercArriveSectorX = buffer.readInt16LE(offset);
    offset += 2;
    o.sMercArriveSectorY = buffer.readInt16LE(offset);
    offset += 2;
    ...
    o.uiMeanWhileFlags = buffer.readUInt32LE(offset);
    offset += 4;
    ...
    o.iHospitalTempBalance = buffer.readInt32LE(offset);
    offset += 4;
    o.iHospitalRefund = buffer.readInt32LE(offset);
    offset += 4;
  ...
  export function writeGeneralSaveInfo(
    o: GENERAL_SAVE_INFO,
    buffer: Buffer,
    offset: number = 0,
  ): number {
    ...
    offset = buffer.writeUInt8(
      Number(o.fSkyriderSaidCongratsOnTakingSAM),
      offset,
    );
    ...
    offset = buffer.writeUInt8(
      Number(o.fDisableMapInterfaceDueToBattle),
      offset,
    );
    ...
    offset = buffer.writeUInt8(
      o.ubPlayerProgressSkyriderLastCommentedOn,
      offset,
    );
    ...
    offset = buffer.writeUInt16LE(
      o.usHasPlayerSeenHelpScreenInCurrentScreen,
      offset,
    );
  ...
  export function SaveGame(
    ubSaveGameID: UINT8,
    pGameDesc: Pointer<string> /* STR16 */,
  ): boolean {
    ...
    FAILED_TO_SAVE: do {
      ...
      iSaveLoadGameMessageBoxID = PrepareMercPopupBox(
        iSaveLoadGameMessageBoxID,
        Enum324.BASIC_MERC_POPUP_BACKGROUND,
        Enum325.BASIC_MERC_POPUP_BORDER,
        zSaveLoadText[Enum371.SLG_SAVING_GAME_MESSAGE],
        300,
        0,
        0,
        0,
    ...
    */
      ...
      if (gfWorldLoaded) {
        ...
        SaveGameHeader.ubLoadScreenID = GetLoadScreenID(
          gWorldSectorX,
          gWorldSectorY,
          gbWorldSectorZ,
        );
      ...
      if (
        !SaveGameClock(
          hFile,
          fPausedStateBeforeSaving,
          fLockPauseStateBeforeSaving,
        )
      ) {
        break FAILED_TO_SAVE;
      }
      ...
      if (ubSaveGameID == 0) {
        ScreenMsg(
          FONT_MCOLOR_WHITE,
          MSG_INTERFACE,
          pMessageStrings[Enum333.MSG_SAVESUCCESS],
        );
      }
      ...
      //#endif
        ...
        ScreenMsg(
          FONT_MCOLOR_WHITE,
          MSG_INTERFACE,
          pMessageStrings[Enum333.MSG_SAVESLOTSUCCESS],
        );
      }
  ...
  export function LoadSavedGame(ubSavedGameID: UINT8): boolean {
    ...
    } else if (!gbSaveGameArray[ubSavedGameID]) return false;
  ...
  */
    ...
    if (SaveGameHeader.fWorldLoaded || SaveGameHeader.uiSavedGameVersion < 50) {
      ...
      // Since there is no
      if (
        SaveGameHeader.sSectorX == -1 ||
        SaveGameHeader.sSectorY == -1 ||
    ...
    if (
      !LoadNPCInfoFromSavedGameFile(hFile, SaveGameHeader.uiSavedGameVersion)
    ) {
      FileClose(hFile);
      guiSaveGameVersion = 0;
      return false;
    ...
    if (
      !LoadArmsDealerInventoryFromSavedGameFile(
        hFile,
        SaveGameHeader.uiSavedGameVersion >= 54,
        SaveGameHeader.uiSavedGameVersion >= 55,
      )
    ) {
      FileClose(hFile);
      guiSaveGameVersion = 0;
    ...
    if (SaveGameHeader.uiSavedGameVersion >= 53) {
      if (
        !LoadBackupNPCInfoFromSavedGameFile(
          hFile,
          SaveGameHeader.uiSavedGameVersion,
        )
      ) {
    ...
    if (SaveGameHeader.uiSavedGameVersion >= 58) {
      ...
      copyMeanwhileDefinition(
        gMeanwhileDef[gCurrentMeanwhileDef.ubMeanwhileID],
        gCurrentMeanwhileDef,
      );
    }
    ...
    SetRelativeStartAndEndPercentage(
      ...
      uiRelStartPerc,
      uiRelEndPerc,
      "Contract renweal sequence stuff...",
    );
    RenderProgressBar(0, 100);
    uiRelStartPerc = uiRelEndPerc;
    ...
    if (gTacticalStatus.uiFlags & INCOMBAT) {
      DebugMsg(
        TOPIC_JA2,
        DBG_LEVEL_3,
        FormatString("Setting attack busy count to 0 from load"),
      );
    ...
    if (
      SaveGameHeader.uiSavedGameVersion >= 61 &&
      SaveGameHeader.uiSavedGameVersion <= 65
    ) {
  ...
  function SaveMercProfiles(hFile: HWFILE): boolean {
    ...
    for (cnt = 0; cnt < NUM_PROFILES; cnt++) {
      gMercProfiles[cnt].uiProfileChecksum = ProfileChecksum(
        gMercProfiles[cnt],
      );
  ...
  function LoadSavedMercProfiles(hFile: HWFILE): boolean {
    ...
    for (cnt = 0; cnt < NUM_PROFILES; cnt++) {
      if (guiSaveGameVersion < 87) {
        ...
        uiNumBytesRead = NewJA2EncryptedFileRead(
          hFile,
          buffer,
          MERC_PROFILE_STRUCT_SIZE,
        );
      ...
      if (
        gMercProfiles[cnt].uiProfileChecksum !=
        ProfileChecksum(gMercProfiles[cnt])
      ) {
        return false;
      }
    }
    ...
    for (cnt = 0; cnt < TOTAL_SOLDIERS; cnt++) {
      ...
      if (!Menptr[cnt].bActive) {
        ...
        if (guiSavedGameVersion < 87) {
          ...
          uiNumBytesWritten = NewJA2EncryptedFileWrite(
            hFile,
            buffer,
            uiSaveSize,
          );
        ...
        // Save the pMercPath
        if (!SaveMercPathFromSoldierStruct(hFile, cnt)) return false;
        ...
        if (Menptr[cnt].pKeyRing != null) {
          ...
          writeObjectArray(
            <KEY_ON_RING[]>Menptr[cnt].pKeyRing,
            buffer,
            0,
            writeKeyOnRing,
          );
  ...
  function LoadSoldierStructure(hFile: HWFILE): boolean {
    ...
    let uiSaveSize: UINT32 = SOLDIER_TYPE_SIZE;
    let ubId: UINT8 = 0;
    let ubId__Pointer = createPointer(
      () => ubId,
      (v) => (ubId = v),
    );
    let ubOne: UINT8 = 1;
    ...
    for (cnt = 0; cnt < TOTAL_SOLDIERS; cnt++) {
      ...
      // else if there is a soldier
        ...
        if (!TacticalCreateSoldier(CreateStruct, ubId__Pointer)) return false;
        ...
        if (
          Menptr[cnt].ubWhatKindOfMercAmI ==
            Enum260.MERC_TYPE__PLAYER_CHARACTER &&
        ...
        if (guiSaveGameVersion < 83) {
          ...
          if (Menptr[cnt].ubProfile != NO_PROFILE) {
            if (Menptr[cnt].ubWhatKindOfMercAmI == Enum260.MERC_TYPE__MERC) {
              ...
              gMercProfiles[Menptr[cnt].ubProfile].uiTotalCostToDate =
                gMercProfiles[Menptr[cnt].ubProfile].sSalary *
                Menptr[cnt].iTotalContractLength;
            }
          }
        }
    ...
    if (guiSaveGameVersion <= 87) {
      ...
      if (
        gMercProfiles[Enum268.ROBOT].inv[Enum261.VESTPOS] ==
        Enum225.SPECTRA_VEST
      ) {
        // update this
        gMercProfiles[Enum268.ROBOT].inv[Enum261.VESTPOS] =
          Enum225.SPECTRA_VEST_18;
        gMercProfiles[Enum268.ROBOT].inv[Enum261.HELMETPOS] =
          Enum225.SPECTRA_HELMET_18;
        gMercProfiles[Enum268.ROBOT].inv[Enum261.LEGPOS] =
...
*/
  ...
  export function SaveFilesToSavedGame(
    pSrcFileName: string /* STR */,
    hFile: HWFILE,
  ): boolean {
  ...
  export function LoadFilesFromSavedGame(
    pSrcFileName: string /* STR */,
    hFile: HWFILE,
  ): boolean {
    let uiFileSize: UINT32;
    ...
    if (uiNumBytesWritten != uiFileSize) {
      ...
      DebugMsg(
        TOPIC_JA2,
        DBG_LEVEL_3,
        FormatString("FAILED to Write to the %s File", pSrcFileName),
      );
  ...
  export function CopySavedSoldierInfoToNewSoldier(
    pDestSourceInfo: SOLDIERTYPE,
    pSourceInfo: SOLDIERTYPE,
  ): boolean {
  ...
  export function CreateSavedGameFileNameFromNumber(
    ubSaveGameID: UINT8,
  ): string {
    ...
    if (ubSaveGameID == 0) {
      zNewFileName = path.join(
        pMessageStrings[Enum333.MSG_SAVEDIRECTORY],
        sprintf(
          "%s.%s",
          pMessageStrings[Enum333.MSG_QUICKSAVE_NAME],
          pMessageStrings[Enum333.MSG_SAVEEXTENSION],
        ),
      );
    }
    //#ifdef JA2BETAVERSION
      ...
      zNewFileName = path.join(
        ...
        sprintf(
          "Auto%s.%s",
          guiLastSaveGameNum.toString().padStart(2, "0"),
          pMessageStrings[Enum333.MSG_SAVEEXTENSION],
        ),
      );
    ...
    //#endif
      ...
      zNewFileName = path.join(
        ...
        sprintf(
          "%s%s.%s",
          pMessageStrings[Enum333.MSG_SAVE_NAME],
          ubSaveGameID.toString().padStart(2, "0"),
          pMessageStrings[Enum333.MSG_SAVEEXTENSION],
        ),
      );
  ...
  */
  ...
  function SaveGeneralInfo(hFile: HWFILE): boolean {
    ...
    sGeneralInfo.fSkyriderSaidCongratsOnTakingSAM =
      gfSkyriderSaidCongratsOnTakingSAM;
    sGeneralInfo.ubPlayerProgressSkyriderLastCommentedOn =
      gubPlayerProgressSkyriderLastCommentedOn;
    ...
    else sGeneralInfo.sContractRehireSoldierID = -1;
    ...
    if (gpSMCurrentMerc) sGeneralInfo.ubSMCurrentMercID = gpSMCurrentMerc.ubID;
    else sGeneralInfo.ubSMCurrentMercID = 255;
    ...
    sGeneralInfo.fDisableMapInterfaceDueToBattle =
    ...
    // Meanwhile stuff
    copyMeanwhileDefinition(
      sGeneralInfo.gCurrentMeanwhileDef,
      gCurrentMeanwhileDef,
    );
  ...
  function LoadGeneralInfo(hFile: HWFILE): boolean {
    ...
    else pContractReHireSoldier = Menptr[sGeneralInfo.sContractRehireSoldierID];
    ...
    else gpSMCurrentMerc = Menptr[sGeneralInfo.ubSMCurrentMercID];
  ...
  */
  ...
  function LoadMeanwhileDefsFromSaveGameFile(hFile: HWFILE): boolean {
    ...
    if (guiSaveGameVersion < 72) {
      ...
      buffer = Buffer.allocUnsafe(
        MEANWHILE_DEFINITION_SIZE * (Enum160.NUM_MEANWHILES - 1),
      );
      ...
      // Load the array of meanwhile defs
      buffer = Buffer.allocUnsafe(
        MEANWHILE_DEFINITION_SIZE * Enum160.NUM_MEANWHILES,
      );
      uiNumBytesRead = FileRead(
        hFile,
        buffer,
  ...
  export function GetBestPossibleSectorXYZValues(
    psSectorX: Pointer<INT16>,
    psSectorY: Pointer<INT16>,
    pbSectorZ: Pointer<INT8>,
  ): void {
    // if the current sector is valid
    if (gfWorldLoaded) {
      ...
      iCurrentTacticalSquad != NO_CURRENT_SQUAD &&
      ...
      for (
        pSoldier = MercPtrs[sSoldierCnt];
        sSoldierCnt <= bLastTeamID;
        sSoldierCnt++, pSoldier = MercPtrs[sSoldierCnt]
      ) {
        if (pSoldier.bActive) {
          if (
  ...
  function TruncateStrategicGroupSizes(): void {
    ...
    for (i = Enum123.SEC_A1; i < Enum123.SEC_P16; i++) {
      ...
      // militia
      if (
        pSector.ubNumberOfCivsAtLevel[0] +
          pSector.ubNumberOfCivsAtLevel[1] +
        ...
        if (
          ...
          if (
            pSector.ubNumberOfCivsAtLevel[0] > pSector.ubNumberOfCivsAtLevel[2]
          ) {
          ...
          pSector.ubNumberOfCivsAtLevel[1] > pSector.ubNumberOfCivsAtLevel[2]
        ) {
          if (
            pSector.ubNumberOfCivsAtLevel[1] > pSector.ubNumberOfCivsAtLevel[0]
          ) {
            pSector.ubNumberOfCivsAtLevel[0] = 0;
          ...
          if (
            pSector.ubNumberOfCivsAtLevel[2] > pSector.ubNumberOfCivsAtLevel[1]
          ) {
            pSector.ubNumberOfCivsAtLevel[0] = 0;
            pSector.ubNumberOfCivsAtLevel[1] = 0;
            pSector.ubNumberOfCivsAtLevel[2] = 20;
      ...
      if (!pGroup.fPlayer) {
        ...
        if (
          ...
          if (pGroup.pEnemyGroup.ubNumAdmins > pGroup.pEnemyGroup.ubNumTroops) {
            if (
              pGroup.pEnemyGroup.ubNumAdmins > pGroup.pEnemyGroup.ubNumElites
            ) {
            ...
            if (
              pGroup.pEnemyGroup.ubNumTroops > pGroup.pEnemyGroup.ubNumAdmins
            ) {
              pGroup.pEnemyGroup.ubNumAdmins = 0;
  ...
  function UpdateMercMercContractInfo(): void {
    ...
    for (ubCnt = Enum268.BIFF; ubCnt <= Enum268.BUBBA; ubCnt++) {
      ...
      gMercProfiles[ubCnt].iMercMercContractLength =
  ...
  export function GetNumberForAutoSave(fLatestAutoSave: boolean): INT8 {
    ...
    if (!fFile1Exist && !fFile2Exist) return -1;
    else if (fFile1Exist && !fFile2Exist) {
      if (fLatestAutoSave) return 0;
      else return -1;
    } else if (!fFile1Exist && fFile2Exist) {
      if (fLatestAutoSave) return 1;
      else return -1;
    } else {
      if (CompareSGPFileTimes(LastWriteTime1, LastWriteTime2) > 0) return 0;
      else return 1;
    }
  }
  ...
  function HandleOldBobbyRMailOrders(): void {
    ...
    if (LaptopSaveInfo.usNumberOfBobbyRayOrderUsed != 0) {
      ...
      for (
        iCnt = 0;
        iCnt < LaptopSaveInfo.usNumberOfBobbyRayOrderItems;
        iCnt++
      ) {
