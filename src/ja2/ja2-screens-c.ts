namespace ja2 {
  ...
  export function DisplayFrameRate(): void {
    ...
    if (gbFPSDisplay == SHOW_FULL_FPS) {
      // FRAME RATE
      VideoOverlayDesc.pzText = swprintf(
        "%ld",
        Math.min(DisplayFrameRate__uiFPS, 1000),
      );
  ...
  export function ErrorScreenHandle(): UINT32 {
    ...
    if (!fFirstTime) {
      DebugMsg(
        TOPIC_JA2,
        DBG_LEVEL_0,
        FormatString("Runtime Error: %s ", gubErrorText),
      );
      fFirstTime = true;
    }
  ...
  export function InitScreenHandle(): UINT32 {
    ...
    if (InitScreenHandle__ubCurrentScreen == 0) {
      ...
      mprintf(
        ...
        430,
        "%s: %s (%s)",
        pMessageStrings[Enum333.MSG_VERSION],
        zVersionLabel,
        czVersionNumber,
      );
  ...
  export function PalEditScreenHandle(): UINT32 {
    ...
    if (PalEditScreenHandle__FirstTime) {
      ...
      guiBackgroundRect = RegisterBackgroundRect(
        BGND_FLAG_PERMANENT,
        null,
        50,
        10,
  ...
  function PalEditKeyboardHook(pInputEvent: InputAtom): boolean {
    ...
    if (
      pInputEvent.usEvent == KEY_DOWN &&
      pInputEvent.usParam == "h".charCodeAt(0)
    ) {
      // Get Soldier
      pSoldier = <SOLDIERTYPE>GetSoldier(gusSelectedSoldier);
    ...
    if (
      pInputEvent.usEvent == KEY_DOWN &&
      pInputEvent.usParam == "v".charCodeAt(0)
    ) {
      // Get Soldier
      pSoldier = <SOLDIERTYPE>GetSoldier(gusSelectedSoldier);
    ...
    if (
      pInputEvent.usEvent == KEY_DOWN &&
      pInputEvent.usParam == "p".charCodeAt(0)
    ) {
      // Get Soldier
      pSoldier = <SOLDIERTYPE>GetSoldier(gusSelectedSoldier);
    ...
    if (
      pInputEvent.usEvent == KEY_DOWN &&
      pInputEvent.usParam == "s".charCodeAt(0)
    ) {
      // Get Soldier
      pSoldier = <SOLDIERTYPE>GetSoldier(gusSelectedSoldier);
  ...
  function DebugKeyboardHook(pInputEvent: InputAtom): boolean {
    if (
      pInputEvent.usEvent == KEY_UP &&
      pInputEvent.usParam == "q".charCodeAt(0)
    ) {
      gfExitDebugScreen = true;
      return true;
    }
  ...
  export function SetDebugRenderHook(
    pDebugRenderOverride: RENDER_HOOK,
    ubPage: INT8,
  ): void {
  ...
  export function SexScreenHandle(): UINT32 {
    ...
    if (SexScreenHandle__ubCurrentScreen == 1) {
      ...
      if (
        SexScreenHandle__uiTime - SexScreenHandle__uiTimeOfLastUpdate >
        SMILY_DELAY
      ) {
