namespace ja2 {
  ...
  const OPT_TOGGLE_BOX_SECOND_COLUMN_START_Y =
    OPT_TOGGLE_BOX_FIRST_COLUMN_START_Y;

  const OPT_TOGGLE_BOX_TEXT_WIDTH =
    OPT_TOGGLE_BOX_SECOND_COLUMN_X - OPT_TOGGLE_BOX_FIRST_COLUMN_X - 20;

  // Text
  const OPT_TOGGLE_BOX_FIRST_COL_TEXT_X =
  ...
  const OPT_TOGGLE_BOX_SECOND_TEXT_X =
  ...
  let guiOptionsToggles: UINT32[] /* [NUM_GAME_OPTIONS] */ = createArray(
    Enum8.NUM_GAME_OPTIONS,
    0,
  );

  // Mouse regions for the name of the option
  let gSelectedOptionTextRegion: MOUSE_REGION[] /* [NUM_GAME_OPTIONS] */ =
    createArrayFrom(Enum8.NUM_GAME_OPTIONS, createMouseRegion);
  ...
  function EnterOptionsScreen(): boolean {
    ...
    // Save game button
    giOptionsButtonImages = LoadButtonImage(
      "INTERFACE\\OptionScreenAddons.sti",
      -1,
      2,
      -1,
      3,
    ...
    guiOptGotoSaveGameBtn = CreateIconAndTextButton(
      giOptionsButtonImages,
      zOptionsText[Enum372.OPT_SAVE_GAME],
      OPT_BUTTON_FONT(),
      OPT_BUTTON_ON_COLOR,
      DEFAULT_SHADOW,
    ...
    // Load game button
    giGotoLoadBtnImage = UseLoadedButtonImage(
      giOptionsButtonImages,
      -1,
      2,
      -1,
    ...
    guiOptGotoLoadGameBtn = CreateIconAndTextButton(
      giGotoLoadBtnImage,
      zOptionsText[Enum372.OPT_LOAD_GAME],
      OPT_BUTTON_FONT(),
      OPT_BUTTON_ON_COLOR,
      DEFAULT_SHADOW,
      OPT_BUTTON_OFF_COLOR,
    ...
    giQuitBtnImage = UseLoadedButtonImage(
      giOptionsButtonImages,
      -1,
      2,
      -1,
    ...
    guiQuitButton = CreateIconAndTextButton(
      giQuitBtnImage,
      zOptionsText[Enum372.OPT_MAIN_MENU],
      OPT_BUTTON_FONT(),
      OPT_BUTTON_ON_COLOR,
      DEFAULT_SHADOW,
      OPT_BUTTON_OFF_COLOR,
    ...
    // Done button
    giDoneBtnImage = UseLoadedButtonImage(
      giOptionsButtonImages,
      -1,
      2,
      -1,
    ...
    for (cnt = 0; cnt < gubFirstColOfOptions; cnt++) {
      ...
      if (usTextWidth > OPT_TOGGLE_BOX_TEXT_WIDTH) {
        ...
        let ubNumLines: UINT8 = Math.trunc(
        ...
        MSYS_DefineRegion(
          gSelectedOptionTextRegion[cnt],
          OPT_TOGGLE_BOX_FIRST_COLUMN_X + 13,
          usPosY,
          OPT_TOGGLE_BOX_FIRST_COL_TEXT_X + usTextWidth,
          usPosY + usTextHeight * ubNumLines,
          MSYS_PRIORITY_HIGH,
      ...
      SetRegionFastHelpText(
        gSelectedOptionTextRegion[cnt],
        zOptionsScreenHelpText[cnt],
      );
      SetButtonFastHelpText(
        guiOptionsToggles[cnt],
        zOptionsScreenHelpText[cnt],
      );
    ...
    for (cnt = gubFirstColOfOptions; cnt < Enum8.NUM_GAME_OPTIONS; cnt++) {
      ...
      if (usTextWidth > OPT_TOGGLE_BOX_TEXT_WIDTH) {
        ...
        MSYS_DefineRegion(
          ...
          usPosY,
          OPT_TOGGLE_BOX_SECOND_TEXT_X + usTextWidth,
          usPosY + usTextHeight * ubNumLines,
          MSYS_PRIORITY_HIGH,
          Enum317.CURSOR_NORMAL,
          SelectedOptionTextRegionMovementCallBack,
    ...
    gfSettingOfTreeTopStatusOnEnterOfOptionScreen =
      gGameSettings.fOptions[Enum8.TOPTION_TOGGLE_TREE_TOPS];

    // Get the status of the item glow option
    gfSettingOfItemGlowStatusOnEnterOfOptionScreen =
      gGameSettings.fOptions[Enum8.TOPTION_GLOW_ITEMS];

    gfSettingOfDontAnimateSmoke =
      gGameSettings.fOptions[Enum8.TOPTION_ANIMATE_SMOKE];
    return true;
  }
  ...
  function RenderOptionsScreen(): void {
    ...
    DisplayWrappedString(
      OPT_SOUND_FX_TEXT_X,
      OPT_SOUND_FX_TEXT_Y,
      OPT_SLIDER_TEXT_WIDTH,
      2,
      OPT_MAIN_FONT(),
      OPT_MAIN_COLOR,
      zOptionsText[Enum372.OPT_SOUND_FX],
      FONT_MCOLOR_BLACK,
      false,
      CENTER_JUSTIFIED,
    );
    ...
    DisplayWrappedString(
      ...
      OPT_MAIN_FONT(),
      OPT_MAIN_COLOR,
      zOptionsText[Enum372.OPT_SPEECH],
      FONT_MCOLOR_BLACK,
      false,
      CENTER_JUSTIFIED,
    );
    ...
    DisplayWrappedString(
      ...
      OPT_MAIN_FONT(),
      OPT_MAIN_COLOR,
      zOptionsText[Enum372.OPT_MUSIC],
      FONT_MCOLOR_BLACK,
      false,
      CENTER_JUSTIFIED,
    );
  ...
  function HandleOptionToggle(
    ...
    if (fState) {
      ...
      if (
        ubButton == Enum8.TOPTION_SPEECH ||
        ubButton == Enum8.TOPTION_SUBTITLES
      ) {
        // make sure that at least of of the toggles is still enabled
        if (
          !(
            ButtonList[guiOptionsToggles[Enum8.TOPTION_SPEECH]].uiFlags &
            BUTTON_CLICKED_ON
          )
        ) {
          if (
            !(
              ButtonList[guiOptionsToggles[Enum8.TOPTION_SUBTITLES]].uiFlags &
              BUTTON_CLICKED_ON
            )
          ) {
            gGameSettings.fOptions[ubButton] = true;
            ButtonList[guiOptionsToggles[ubButton]].uiFlags |=
              BUTTON_CLICKED_ON;
  ...
  export function DoOptionsMessageBoxWithRect(
    ubStyle: UINT8,
    ...
    giOptionsMessageBox = DoMessageBox(
      ubStyle,
      zString,
      uiExitScreen,
  ...
  function SetOptionsScreenToggleBoxes(): void {
    ...
    for (cnt = 0; cnt < Enum8.NUM_GAME_OPTIONS; cnt++) {
      ...
      else ButtonList[guiOptionsToggles[cnt]].uiFlags &= ~BUTTON_CLICKED_ON;
    }
  }
  ...
  function GetOptionsScreenToggleBoxes(): void {
    ...
    for (cnt = 0; cnt < Enum8.NUM_GAME_OPTIONS; cnt++) {
      ...
      else gGameSettings.fOptions[cnt] = false;
    }
  }
  ...
  function HandleSliderBarMovementSounds(): void {
    ...
    if (
      uiLastSoundFxTime - OPT_MUSIC_SLIDER_PLAY_SOUND_DELAY >
      guiSoundFxSliderMoving
    ) {
      ...
      if (!SoundIsPlaying(uiLastPlayingSoundID))
        uiLastPlayingSoundID = PlayJA2SampleFromFile(
          "Sounds\\Weapons\\LMG Reload.wav",
          RATE_11025,
          HIGHVOLUME,
          1,
          MIDDLEPAN,
        );
    } else uiLastSoundFxTime = GetJA2Clock();

    if (
      uiLastSpeechTime - OPT_MUSIC_SLIDER_PLAY_SOUND_DELAY >
      guiSpeechSliderMoving
    ) {
      guiSpeechSliderMoving = 0xffffffff;

      if (!SoundIsPlaying(uiLastPlayingSpeechID))
        uiLastPlayingSpeechID = PlayJA2GapSample(
          "BattleSnds\\m_cool.wav",
          RATE_11025,
          HIGHVOLUME,
          1,
          MIDDLEPAN,
    ...
    } else uiLastSpeechTime = GetJA2Clock();
  }

  function SelectedOptionTextRegionCallBack(
    ...
    if (iReason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
      HandleOptionToggle(
        ubButton,
        !gGameSettings.fOptions[ubButton],
        false,
        true,
      );
  ...
  function SelectedOptionTextRegionMovementCallBack(
    pRegion: MOUSE_REGION,
    reason: INT32,
  ): void {
  ...
  function HandleHighLightedText(fHighLight: boolean): void {
    ...
    if (bHighLight != -1) {
      if (bHighLight < OPT_FIRST_COLUMN_TOGGLE_CUT_OFF) {
        ...
        usPosY =
          OPT_TOGGLE_BOX_FIRST_COLUMN_START_Y +
          OPT_TOGGLE_TEXT_OFFSET_Y +
          bHighLight * OPT_GAP_BETWEEN_TOGGLE_BOXES;
      } else {
        usPosX = OPT_TOGGLE_BOX_SECOND_TEXT_X;
        usPosY =
          OPT_TOGGLE_BOX_SECOND_COLUMN_START_Y +
          OPT_TOGGLE_TEXT_OFFSET_Y +
          (bHighLight - OPT_FIRST_COLUMN_TOGGLE_CUT_OFF) *
            OPT_GAP_BETWEEN_TOGGLE_BOXES;
      }

      // If we are to hide the blood and gore option, and we are to highlight an option past the blood and gore option
      // reduce the highlight number by 1
      if (
        bHighLight >= Enum8.TOPTION_BLOOD_N_GORE &&
        gfHideBloodAndGoreOption
      ) {
        bHighLight++;
      }

      usWidth = StringPixLength(
        zOptionsToggleText[bHighLight],
        OPT_MAIN_FONT(),
      );
  ...
  function SelectedToggleBoxAreaRegionMovementCallBack(
    pRegion: MOUSE_REGION,
    reason: INT32,
  ): void {
    if (reason & MSYS_CALLBACK_REASON_LOST_MOUSE) {
      ...
      for (ubCnt = 0; ubCnt < Enum8.NUM_GAME_OPTIONS; ubCnt++) {
        ButtonList[guiOptionsToggles[ubCnt]].Area.uiFlags &=
          ~MSYS_MOUSE_IN_AREA;
      }
