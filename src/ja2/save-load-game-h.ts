namespace ja2 {
  ...
  export function readSavedGameHeader(
    o: SAVED_GAME_HEADER,
    buffer: Buffer,
    offset: number = 0,
  ): number {
    o.uiSavedGameVersion = buffer.readUInt32LE(offset);
    offset += 4;
    o.zGameVersionNumber = readStringNL(
      buffer,
      "ascii",
      offset,
      offset + GAME_VERSION_LENGTH,
    );
    offset += GAME_VERSION_LENGTH;
    o.sSavedGameDesc = readStringNL(
      buffer,
      "utf16le",
      offset,
      offset + SIZE_OF_SAVE_GAME_DESC * 2,
    );
    offset += SIZE_OF_SAVE_GAME_DESC * 2;
    o.uiFlags = buffer.readUInt32LE(offset);
    offset += 4;
    o.uiDay = buffer.readUInt32LE(offset);
    offset += 4;
    ...
    offset++; // padding
    o.uiRandom = buffer.readUInt32LE(offset);
    offset += 4;
  ...
  export function writeSavedGameHeader(
    o: SAVED_GAME_HEADER,
    buffer: Buffer,
    offset: number = 0,
  ): number {
    offset = buffer.writeUInt32LE(o.uiSavedGameVersion, offset);
    offset = writeStringNL(
      o.zGameVersionNumber,
      buffer,
      offset,
      GAME_VERSION_LENGTH,
