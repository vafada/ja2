namespace ja2 {
  ...
  export function readGameSettings(
    ...
    o.zVersionNumber = readStringNL(buffer, "ascii", offset, offset + 14);
    offset += 14;
    offset += 3; // padding
    o.uiSettingsVersionNumber = buffer.readUInt32LE(offset);
    offset += 4;
    o.uiMeanwhileScenesSeenFlags = buffer.readUInt32LE(offset);
    offset += 4;
  ...
  export function copyGameOptions(
    destination: GAME_OPTIONS,
    source: GAME_OPTIONS,
  ) {
  ...
  export function writeGameOptions(
    o: GAME_OPTIONS,
    buffer: Buffer,
    offset: number = 0,
  ): number {
