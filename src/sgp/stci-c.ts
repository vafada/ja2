namespace ja2 {
  export function LoadSTCIFileToImage(
    hImage: ImageType,
    fContents: UINT16,
  ): boolean {
    let hFile: HWFILE;
    ...
    if (
      (uiBytesRead = FileRead(hFile, buffer, STCI_HEADER_SIZE)) === -1 ||
    ...
    if (Header.fFlags & STCI_RGB) {
      ...
      if (!STCILoadIndexed(TempImage, fContents, hFile, Header)) {
        DbgMessage(
          TOPIC_HIMAGE,
          DBG_LEVEL_3,
          "Problem loading palettized image.",
        );
        FileClose(hFile);
        return false;
      }
      ...
      DbgMessage(
        TOPIC_HIMAGE,
        DBG_LEVEL_3,
        "Unknown data organization in STCI file.",
      );
      FileClose(hFile);
      return false;
    }
  ...
  function STCILoadRGB(
    ...
    if (fContents & IMAGE_BITMAPDATA) {
      ...
      if (
        (uiBytesRead = FileRead(hFile, buffer, pHeader.uiStoredSize)) === -1 ||
      ...
      if (pHeader.ubDepth == 16) {
        ...
        if (
          gusRedMask != pHeader.RGB.uiRedMask ||
          gusGreenMask != pHeader.RGB.uiGreenMask ||
          ...
          DbgMessage(
            TOPIC_HIMAGE,
            DBG_LEVEL_3,
            "Converting to current RGB distribution!",
          );
          ...
          if (gusRedMask > gusGreenMask && gusGreenMask > gusBlueMask) {
            ...
            if (
              ...
              ConvertRGBDistribution565To655(
                hImage.p16BPPData,
                pHeader.usWidth * pHeader.usHeight,
              );
              return true;
              ...
              ConvertRGBDistribution565To556(
                hImage.p16BPPData,
                pHeader.usWidth * pHeader.usHeight,
              );
              return true;
              ...
              // take the long route
              ConvertRGBDistribution565ToAny(
                hImage.p16BPPData,
                pHeader.usWidth * pHeader.usHeight,
              );
              return true;
  ...
  function STCILoadIndexed(
    ...
    if (fContents & IMAGE_PALETTE) {
      ...
      if (pHeader.Indexed.uiNumberOfColours != 256) {
        DbgMessage(
          TOPIC_HIMAGE,
          DBG_LEVEL_3,
          "Palettized image has bad palette size.",
        );
        return false;
      }
      uiFileSectionSize =
        pHeader.Indexed.uiNumberOfColours * STCI_PALETTE_ELEMENT_SIZE;

      // Read in the palette
      buffer = Buffer.allocUnsafe(uiFileSectionSize);
      if (
        (uiBytesRead = FileRead(hFile, buffer, uiFileSectionSize)) === -1 ||
      ...
      if (!STCISetPalette(buffer, hImage)) {
        DbgMessage(
          TOPIC_HIMAGE,
          DBG_LEVEL_3,
          "Problem setting hImage-format palette!",
        );
        FileClose(hFile);
        return false;
      }
    ...
    if (fContents & IMAGE_BITMAPDATA) {
      if (pHeader.fFlags & STCI_ETRLE_COMPRESSED) {
        ...
        hImage.pETRLEObject = createArrayFrom(
        ...
        if (
          ...
          DbgMessage(
            TOPIC_HIMAGE,
            DBG_LEVEL_3,
            "Error loading subimage structures!",
          );
          FileClose(hFile);
          if (fContents & IMAGE_PALETTE) {
      ...
      // then there's a point in seeking ahead
      if (
        FileSeek(hFile, pHeader.uiStoredSize, FILE_SEEK_FROM_CURRENT) == false
      ) {
        DbgMessage(
          TOPIC_HIMAGE,
          DBG_LEVEL_3,
          "Problem seeking past image data!",
        );
        FileClose(hFile);
        return false;
      }
    ...
    if (fContents & IMAGE_APPDATA && pHeader.uiAppDataSize > 0) {
      ...
      if (
        (uiBytesRead = FileRead(
          hFile,
          hImage.pAppData,
          pHeader.uiAppDataSize,
        )) === -1 ||
        ...
        DbgMessage(
          TOPIC_HIMAGE,
          DBG_LEVEL_3,
          "Error loading application-specific data!",
        );
        FileClose(hFile);
