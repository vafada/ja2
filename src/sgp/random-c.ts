namespace ja2 {
  ...
  export let guiPreRandomNums: UINT32[] /* [MAX_PREGENERATED_NUMS] */ =
    createArray(MAX_PREGENERATED_NUMS, 0);
  ...
  export function InitializeRandom(): void {
    ...
    for (
      guiPreRandomIndex = 0;
      guiPreRandomIndex < MAX_PREGENERATED_NUMS;
      guiPreRandomIndex++
    ) {
  ...
  export function PreRandom(uiRange: UINT32): UINT32 {
    ...
    uiNum =
      Math.trunc((guiPreRandomNums[guiPreRandomIndex] * uiRange) / RAND_MAX) %
      uiRange;
    ...
    guiPreRandomIndex++;
    if (guiPreRandomIndex >= MAX_PREGENERATED_NUMS) guiPreRandomIndex = 0;
    return uiNum;
  }
