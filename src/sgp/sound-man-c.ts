namespace ja2 {
  ...
  let cWAVChunks: string[] /* Pointer<CHAR8>[3] */ = ["RIFF", "FMT ", "DATA"];
...
*/
  ...
  let pSampleList: SAMPLETAG[] /* [SOUND_MAX_CACHED] */ = createArrayFrom(
    SOUND_MAX_CACHED,
    createSampleTag,
  );
  // Sound channel list for output channels
  let pSoundList: SOUNDTAG[] /* [SOUND_MAX_CHANNELS] */ = createArrayFrom(
    SOUND_MAX_CHANNELS,
    createSoundTag,
  );
  ...
  export function SoundPlay(
    ...
    if (fSoundSystemInit) {
      if (!SoundPlayStreamed(pFilename)) {
        ...
        FastDebugMsg(
          FormatString(
            "SoundPlay: ERROR: Trying to play %s sound is too lardge to load into cache, use SoundPlayStreamedFile() instead\n",
            pFilename,
          ),
        );
      }
  ...
  //
  //*******************************************************************************
  export function SoundPlayStreamedFile(
    pFilename: string /* STR */,
    pParms: SOUNDPARMS,
  ): UINT32 {
    let uiChannel: UINT32;
    ...
    if (fSoundSystemInit) {
      if ((uiChannel = SoundGetFreeChannel()) != SOUND_ERROR) {
        ...
        if (uiRetVal != SOUND_ERROR) pSoundList[uiChannel].hFile = hFile;
  ...
  //
  //*******************************************************************************
  export function SoundPlayRandom(
    pFilename: string /* STR */,
    pParms: RANDOMPARMS,
  ): UINT32 {
    let uiSample: UINT32;
    ...
    if (fSoundSystemInit) {
      if ((uiSample = SoundLoadSample(pFilename)) != NO_SAMPLE) {
        ...
        if (pParms.uiTimeMin == SOUND_PARMS_DEFAULT) return SOUND_ERROR;
        else pSampleList[uiSample].uiTimeMin = pParms.uiTimeMin;

        if (pParms.uiTimeMax == SOUND_PARMS_DEFAULT)
          pSampleList[uiSample].uiTimeMax = pParms.uiTimeMin;
        else pSampleList[uiSample].uiTimeMax = pParms.uiTimeMax;
        ...
        pSampleList[uiSample].uiTimeNext =
          ...
          Random(
            pSampleList[uiSample].uiTimeMax - pSampleList[uiSample].uiTimeMin,
          );
        return uiSample;
      }
    }
  ...
  export function SoundServiceRandom(): boolean {
    ...
    for (uiCount = 0; uiCount < SOUND_MAX_CACHED; uiCount++) {
      if (
        !(pSampleList[uiCount].uiFlags & SAMPLE_RANDOM_MANUAL) &&
    ...
    if (pSampleList[uiSample].uiFlags & SAMPLE_RANDOM)
      if (pSampleList[uiSample].uiTimeNext <= GetTickCount())
        if (
          ...
          pSampleList[uiSample].uiMaxInstances
        )
          return true;
  ...
  export function SoundServiceStreams(): boolean {
    ...
    if (fSoundSystemInit) {
      for (uiCount = 0; uiCount < SOUND_MAX_CHANNELS; uiCount++) {
        if (
          pSoundList[uiCount].hMSS ||
          pSoundList[uiCount].hMSSStream ||
          ...
          if (!SoundIsPlaying(pSoundList[uiCount].uiSoundID))
            ...
            if (
              uiVolume != pSoundList[uiCount].uiFadeVolume &&
              uiTime >=
              ...
              if (uiVolume < pSoundList[uiCount].uiFadeVolume)
                ...
                if (!uiVolume && pSoundList[uiCount].fStopAtZero)
                  SoundStopIndex(uiCount);
                else SoundSetVolumeIndex(uiCount, uiVolume);
              }
  ...
  export function SoundGetPosition(uiSoundID: UINT32): UINT32 {
    ...
    if (fSoundSystemInit) {
      ...
      */
        ...
        else uiPosition = uiTime - pSoundList[uiSound].uiTimeStamp;
  ...
  function SoundLoadDisk(pFilename: string /* STR */): UINT32 {
    ...
    if ((hFile = FileOpen(pFilename, FILE_ACCESS_READ, false)) != 0) {
      ...
      if (uiSize + guiSoundMemoryUsed > guiSoundMemoryLimit) {
        FastDebugMsg(
          FormatString(
            "SoundLoadDisk:  ERROR:  trying to play %s, not enough memory\n",
            pFilename,
          ),
        );
        FileClose(hFile);
      ...
      if ((pSampleList[uiSample].pData = Buffer.allocUnsafe(uiSize)) == null) {
        FastDebugMsg(
          FormatString(
            "SoundLoadDisk:  ERROR: Trying to play %s, AIL channels are full\n",
            pFilename,
          ),
        );
        FileClose(hFile);
  ...
  function SoundCleanCache(): boolean {
    ...
    for (uiCount = 0; uiCount < SOUND_MAX_CACHED; uiCount++) {
      if (
        pSampleList[uiCount].uiFlags & SAMPLE_ALLOCATED &&
        !(pSampleList[uiCount].uiFlags & SAMPLE_LOCKED)
      ) {
        if (
          uiLowestHits == NO_SAMPLE ||
  ...
  */
  ...
  function SoundShutdownHardware(): boolean {
    if (fSoundSystemInit) hSoundDriver.close();
  ...
  function SoundGetFreeChannel(): UINT32 {
    ...
    for (uiCount = 0; uiCount < SOUND_MAX_CHANNELS; uiCount++) {
      ...
      if (
        pSoundList[uiCount].hMSS == null &&
        pSoundList[uiCount].hMSSStream == null &&
  ...
  //
  //*******************************************************************************
  function SoundStartSample(
    uiSample: UINT32,
    uiChannel: UINT32,
    pParms: SOUNDPARMS | null,
  ): UINT32 {
    let uiSoundID: UINT32;
    ...
    if (
      !AIL_set_named_sample_file(
        pSoundList[uiChannel].hMSS,
        pSampleList[uiSample].pName,
        pSampleList[uiSample].pData,
    ...
    pSampleList[uiSample].uiSpeed = AIL_sample_playback_rate(
      pSoundList[uiChannel].hMSS,
    );
    ...
    if (pSampleList[uiSample].uiFlags & SAMPLE_RANDOM) {
      if (
        ...
        let uiSpeed: UINT32 =
          ...
          Random(
            pSampleList[uiSample].uiSpeedMax - pSampleList[uiSample].uiSpeedMin,
          );
    ...
    else pSoundList[uiChannel].uiPriority = PRIORITY_MAX;
  ...
  function SoundStartStream(
    ...
    if (
      (pSoundList[uiChannel].hMSSStream = AIL_open_stream(
        hSoundDriver,
        pFilename,
        SOUND_DEFAULT_STREAM,
      )) == null
    ) {
      SoundCleanCache();
      pSoundList[uiChannel].hMSSStream = AIL_open_stream(
    ...
    else uiSpeed = AIL_stream_playback_rate(pSoundList[uiChannel].hMSSStream);
    ...
    uiSoundID = SoundGetUniqueID();
    pSoundList[uiChannel].uiSoundID = uiSoundID;
    if (pParms) pSoundList[uiChannel].uiPriority = pParms.uiPriority;
    else pSoundList[uiChannel].uiPriority = SOUND_PARMS_DEFAULT;
  ...
  function SoundGetUniqueID(): UINT32 {
    if (SoundGetUniqueID__uiNextID == SOUND_ERROR) SoundGetUniqueID__uiNextID++;
      ...
      if (uiChannel != NO_SAMPLE) {
        if (pSoundList[uiChannel].hMSS != null) {
          ...
          if (pSoundList[uiChannel].EOSCallback != null)
            pSoundList[uiChannel].EOSCallback!(
              pSoundList[uiChannel].pCallbackData,
            );
  ...
  export function SoundRemoveSampleFlags(
    uiSample: UINT32,
    uiFlags: UINT32,
  ): void {
  ...
  function AIL_set_named_sample_file(
    sample: HSAMPLE,
    fileName: string,
    data: Buffer | null,
    size: number,
  ...
  function AIL_start_sample(sample: HSAMPLE) {
    ...
    if (audioBuffer) {
      ...
      sample.audioBufferSourceNode.addEventListener(
        "ended",
        sampleEnded.bind(sample),
      );
  ...
  function AIL_sample_playback_rate(sample: HSAMPLE) {
    return Math.trunc(
      sample.audioBufferSourceNode.playbackRate.value *
        sample.context.sampleRate,
    );
  }

  function AIL_set_sample_playback_rate(sample: HSAMPLE, playbackRate: number) {
    sample.audioBufferSourceNode.playbackRate.value =
      playbackRate !== 0 ? sample.context.sampleRate / playbackRate : 0;
  }
  ...
  function AIL_start_stream(stream: HSTREAM) {
    stream.audioBufferSourceNode.addEventListener(
      "ended",
      streamEnded.bind(stream),
    );
  ...
  function AIL_stream_playback_rate(stream: HSTREAM) {
    return Math.trunc(
      stream.audioBufferSourceNode.playbackRate.value *
  ...
  function AIL_set_stream_playback_rate(stream: HSTREAM, playbackRate: number) {
    stream.audioBufferSourceNode.playbackRate.value =
      playbackRate !== 0 ? stream.context.sampleRate / playbackRate : 0;
  }
  ...
  function decodeWav(buffer: Buffer): AudioBuffer | null {
    ...
    while (offset < buffer.length) {
      chunkTag = buffer.readUInt32LE(offset);
      offset += 4;
      chunkSize = buffer.readUInt32LE(offset);
      offset += 4;
    ...
    if (audioFormat === 0x0001) {
      ...
      while (numberOfBlocks--) {
        decodingContext.predictor = buffer.readInt16LE(offset);
        offset += 2;
        decodingContext.stepIndex = buffer.readUInt8(offset);
        offset += 2;

        for (let i = 4; i < blockAlign; i++) {
          const nibble = buffer.readUInt8(offset++);
          audioData[0][frame++] =
            adpcmDecode(decodingContext, nibble & 0x0f, 3) / 32768;
          audioData[0][frame++] =
            adpcmDecode(decodingContext, nibble >> 4, 3) / 32768;
        }
      }
  ...
  function adpcmDecode(
    context: WavDecodingContext,
    nibble: number,
    shift: number,
  ) {
