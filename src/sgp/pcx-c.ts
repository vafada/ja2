namespace ja2 {
  ...
  export function LoadPCXFileToImage(
    hImage: ImageType,
    fContents: UINT16,
  ): boolean {
  ...
  function BlitPcxToBuffer(
    pCurrentPcxObject: PcxObject,
    pBuffer: Uint8Array,
    usBufferWidth: UINT16,
    usBufferHeight: UINT16,
    usX: UINT16,
    usY: UINT16,
    fTransp: boolean,
  ): boolean {
    ...
    if (
      pCurrentPcxObject.usWidth + usX == usBufferWidth &&
      pCurrentPcxObject.usHeight + usY == usBufferHeight
    ) {
