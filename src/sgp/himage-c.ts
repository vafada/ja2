namespace ja2 {
  ...
  export function CreateImage(
    ...
    if (StrPtr == -1) {
      ...
      DbgMessage(
        TOPIC_HIMAGE,
        DBG_LEVEL_2,
        "No extension given, using default",
      );
      ImageFile += ".PCX";
      Extension = ".PCX";
    } else {
    ...
    if (!FileExists(ImageFile)) {
      DbgMessage(
        TOPIC_HIMAGE,
        DBG_LEVEL_2,
        FormatString("Resource file %s does not exist.", ImageFile),
      );
  ...
  function LoadImageData(hImage: ImageType, fContents: UINT16): boolean {
    ...
    switch (hImage.iFileLoader) {
      ...
      default:
        DbgMessage(
          TOPIC_HIMAGE,
          DBG_LEVEL_2,
          "Unknown image loader was specified.",
        );
    }
    ...
    if (!fReturnVal) {
      DbgMessage(
        TOPIC_HIMAGE,
        DBG_LEVEL_2,
        "Error occured while reading image data.",
      );
    }
  ...
  export function CopyImageToBuffer(
    hImage: ImageType,
    fBufferType: UINT32,
    pDestBuf: Uint8Array | Uint8ClampedArray,
    usDestWidth: UINT16,
    usDestHeight: UINT16,
    usX: UINT16,
    usY: UINT16,
    ...
    if (hImage.ubBitDepth == 8 && fBufferType == BUFFER_8BPP) {
      ...
      return Copy8BPPImageTo8BPPBuffer(
        hImage,
        pDestBuf,
        usDestWidth,
        usDestHeight,
    ...
    if (hImage.ubBitDepth == 8 && fBufferType == BUFFER_16BPP) {
      ...
      DbgMessage(
        TOPIC_HIMAGE,
        DBG_LEVEL_3,
        "Copying 8 BPP Imagery to 16BPP Buffer.",
      );
      return Copy8BPPImageTo16BPPBuffer(
        hImage,
        pDestBuf,
        usDestWidth,
        usDestHeight,
    ...
    if (hImage.ubBitDepth == 16 && fBufferType == BUFFER_16BPP) {
      DbgMessage(
        TOPIC_HIMAGE,
        DBG_LEVEL_3,
        "Automatically Copying 16 BPP Imagery.",
      );
      return Copy16BPPImageTo16BPPBuffer(
        hImage,
        pDestBuf,
        usDestWidth,
        usDestHeight,
  ...
  function Copy8BPPImageTo8BPPBuffer(
    hImage: ImageType,
    pDestBuf: Uint8Array | Uint8ClampedArray,
    usDestWidth: UINT16,
    usDestHeight: UINT16,
  ...
  function Copy16BPPImageTo16BPPBuffer(
    hImage: ImageType,
    pDestBuf: Uint8Array | Uint8ClampedArray,
    usDestWidth: UINT16,
    usDestHeight: UINT16,
  ...
  function Extract8BPPCompressedImageToBuffer(
    hImage: ImageType,
    pDestBuf: Uint8Array,
  ): boolean {
    return false;
  ...
  function Extract16BPPCompressedImageToBuffer(
    hImage: ImageType,
    pDestBuf: Uint8Array,
  ): boolean {
    return false;
  ...
  function Copy8BPPImageTo16BPPBuffer(
    hImage: ImageType,
    pDestBuf: Uint8Array | Uint8ClampedArray,
    usDestWidth: UINT16,
    usDestHeight: UINT16,
    ...
    DbgMessage(
      TOPIC_HIMAGE,
      DBG_LEVEL_3,
      FormatString("Start Copying at %p", pDest),
    );
    ...
    for (rows = 0; rows < uiNumLines - 1; rows++) {
      ...
      for (cols = 0; cols < uiLineSize; cols++) {
        ...
        pDestBuf[pDestTemp] =
          p16BPPPalette[hImage.p8BPPData[pSrcTemp]] & 0xffff;
        pDestTemp += 2;
        pSrcTemp++;
      }
    ...
    DbgMessage(
      TOPIC_HIMAGE,
      DBG_LEVEL_3,
      FormatString("End Copying at %p", pDest),
    );
  ...
  export function Create16BPPPalette(pPalette: SGPPaletteEntry[]): Uint16Array {
    ...
    for (cnt = 0; cnt < 256; cnt++) {
      ...
      if (gusRedShift < 0) r16 = r >> Math.abs(gusRedShift);
      else r16 = r << gusRedShift;

      if (gusGreenShift < 0) g16 = g >> Math.abs(gusGreenShift);
      else g16 = g << gusGreenShift;

      if (gusBlueShift < 0) b16 = b >> Math.abs(gusBlueShift);
...
**********************************************************************************************/
  export function Create16BPPPaletteShaded(
  ...
  export function GetRGBColor(Value16BPP: UINT16): UINT32 {
    ...
    if (gusRedShift < 0) r = r16 << Math.abs(gusRedShift);
    else r = r16 >> gusRedShift;

    if (gusGreenShift < 0) g = g16 << Math.abs(gusGreenShift);
    else g = g16 >> gusGreenShift;

    if (gusBlueShift < 0) b = b16 << Math.abs(gusBlueShift);
    else b = b16 >> gusBlueShift;
  ...
  function ConvertRGBToPaletteEntry(
    sbStart: UINT8,
    sbEnd: UINT8,
    pOldPalette: Uint8Array,
  ): SGPPaletteEntry[] {
    let Index: UINT16;
  ...
  export function GetETRLEImageData(
    hImage: ImageType,
    pBuffer: ETRLEData,
  ): boolean {
    // Assertions
    ...
    pBuffer.pETRLEObject = createArrayFrom(
      pBuffer.usNumberOfObjects,
      createETRLEObject,
    );
  ...
  export function ConvertRGBDistribution565To555(
    p16BPPData: Uint16Array,
    uiNumberOfPixels: UINT32,
  ): void {
    let pPixel: UINT16;
    let uiLoop: UINT32;
  ...
  export function ConvertRGBDistribution565To655(
    p16BPPData: Uint16Array,
    uiNumberOfPixels: UINT32,
  ): void {
    let uiLoop: UINT32;
  ...
  export function ConvertRGBDistribution565To556(
    p16BPPData: Uint16Array,
    uiNumberOfPixels: UINT32,
  ): void {
    let uiLoop: UINT32;
  ...
  export function ConvertRGBDistribution565ToAny(
    p16BPPData: Uint16Array,
    uiNumberOfPixels: UINT32,
  ): void {
    let pPixel: UINT16;
