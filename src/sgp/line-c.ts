namespace ja2 {
  ...
  export function SetClippingRegionAndImageWidth(
    iImageWidth: number,
    iClipStartX: number,
    iClipStartY: number,
    iClipWidth: number,
  ...
  function Clipt(
    ...
    if (denom > 0.0) {
      t = num / denom;
      if (t > tL.value) accept = false;
      else if (t > tE.value) tE.value = t;
    } else if (denom < 0.0) {
      t = num / denom;
      if (t < tE.value) accept = false;
      else if (t < tL.value) tL.value = t;
  ...
  function ClipPoint(x: number, y: number): boolean {
    return (
      x <= giClipXMax && x >= giClipXMin && y <= giClipYMax && y >= giClipYMin
    );
  }
  ...
  function Clip2D(
    ...
    let visible: boolean;
    let te: FLOAT = 0;
    let te__Pointer = createPointer(
      () => te,
      (v) => (te = v),
    );
    let tl: FLOAT = 0;
    let tl__Pointer = createPointer(
      () => tl,
      (v) => (tl = v),
    );
    let dx: FLOAT;
  ...
  /* Draws a rectangle between the specified endpoints in color Color. */
  export function RectangleDraw8(
    fClip: boolean,
    XStart: number,
    YStart: number,
    XEnd: number,
